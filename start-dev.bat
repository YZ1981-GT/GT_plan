...
if exist "%ROOT_DIR%\.venv\Scripts\python.exe" (
  ...
  where py >nul 2>nul
  if not errorlevel 1 (
    set "BACKEND_START_CMD=py -3 -m uvicorn app.main:app --host 0.0.0.0 --port %BACKEND_PORT% --reload"
  ) else (
    ...
    set "BACKEND_START_CMD=python -m uvicorn app.main:app --host 0.0.0.0 --port %BACKEND_PORT% --reload"
  )
)
