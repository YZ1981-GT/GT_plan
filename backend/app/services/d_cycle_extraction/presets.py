    ...
    "D6": [
        {
            ...
            "description": (
                "审定表 TB↔审定核对行 ← Tier A 可编辑公式 TB('1141','期末余额')（1141 合同"
                "资产总额）。🔴 2026-08-01 科目码纠正：原登记 TB('1402','期末余额') —— 1402 是"
                "**在途物资**（存货类，属 F2 循环），活体实证该码有 12 行/136,268.13 数据"
                "→ 错误预设会把存货的钱当合同资产带进审定表（不是恒空，是显示错数）。"
                "合同资产科目为 1141（标准科目表实证 direction=debit），report_config 报表行"
                "BS-011 四准则一律 TB('1141','期末余额')。"
            ),
        },
        {
            "sheet_name": "D6-1",
            ...
            "description": (
                "审定表 一、原值(block1) 期初/期末未审 ← tb_balance 1141 叶子子科目"
                "（期初/期末余额，get_active_filter 数据集版本，只取叶子防双算，手工优先）"
            ),
        },
        {
            "sheet_name": "D6-1",
            "anchor": "d6_impairment_tb resolver",
            "description": (
                "合同资产减值准备期初/期末未审 ← trial_balance **1142「合同资产减值准备」**"
                "或 **1231-05「坏账准备-合同资产」**（标准科目表实证，客户二选一取并集）。"
                "🔴 2026-08-01 科目码纠正：原两版 `_IMPAIRMENT_ACCOUNT_CODES` 都错——先是"
                "`['140201','1402.01']`（1402=在途物资，连科目族都错），纠正 1402 后机械平移"
                "成 `['114101','1141.01']`（把借方资产科目 1141 的子科目当备抵，仍取不到"
                "减值数据）。"
            ),
        },
        {
            ...
            "anchor": "D6-2-rows",
            "description": (
                "明细表 期初未审/借贷方发生额/期后结算 ← tb_aux_balance 1141 按客户/合同"
                "维度归集 + 次年序时账 1141 贷方（一键取数，Tier B 复杂归集，非单条公式）"
            ),
        },
    ],
    ...
    "D2": [
        ...
        },
        {
            ...
            "description": (
                ...
                "D2-adj-tb-amount，已作 Tier A 可编辑公式**净额口径**"
                "TB('1122','期末余额') - TB('1231-02','期末余额')（原值 1122 − 坏账准备"
                "1231-02；🔴 2026-08-01 由原值口径修正：源模板 审定表D2-1 被比较项是"
                "「三、应收账款净值」合计 A27，report_config BS-006 soe_standalone 同口径，"
                "取原值会产生恰好等于坏账准备的假差异，与 D1 同款缺陷同期修正）。"
            ),
        },
    ],
    ...
    "D5": [
        ...
        },
        {
            ...
            "description": (
                ...
                "OCI 公允价值变动减项来自 D5-4 公允价值测算，非四表库。"
            ),
        },
        {
            "sheet_name": "D5-1",
            "anchor": "D5-1-tb-amount（🔴 已移除，不再有 Tier A 预设）",
            "description": (
                "🔴 2026-08-01 诚实化（Requirement 4）：原预设 TB('1124','期末余额') 已移除。"
                "「应收款项融资」是报表项目（以公允价值计量且其变动计入其他综合收益的应收票据/"
                "应收账款），准则无对应一级科目；标准科目表实证 1124 不存在；活体 trial_balance/"
                "tb_balance 的 1124% 均 0 行 → 该预设求值恒为 0，保留比没有更糟（审计师以为"
                "取过数）。D5-1「试算平衡表数」核对行改手工录入 + D5-2 明细聚合核对。"
                "注：report_config 报表行 BS-007 应收款项融资四准则也都写 TB('1124')，"
                "同为平台级报表配置缺陷（该报表行同样恒为 0），只报告不改。"
            ),
        },
    ],
