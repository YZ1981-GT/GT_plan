...
from app.services.workpaper_sync.phase5_d4_return_sheet import (  # noqa: E402
    sheet_payload_d420,
    instrumentation_specs_d420,
    build_store_projection_d420,
...
from app.services.workpaper_sync.phase5_d4_other_cutoff_sheet import (  # noqa: E402
    STORE_ITEM_ID_D436,
    sheet_payload_d436,
    instrumentation_specs_d436,
    build_store_projection_d436,
...
#: D4-20 销售退货检查表接入（批次B 第七张，2026-09-20）。单 sheet **4 区**：summary(static固定2行)+
...
#: D4-36 其他业务收入截止性测试接入（批次B 第十张，同 sheet 双动态区 · dict store，2026-09-20）。
#: 2 dynamic 区（账到单据 forward R16-23 / 单据到账 backward R36-43），各自 UUID 列 L/M、footer marker
#: =下方 section 标题；11 字段（voucher* A-E / doc* F-J / isCrossing K），K 跨期为手工标记非公式（无
...
def build_combined_store_projection(
    ...
    *,
    ...
    d411_projs = (
        [build_store_projection_d411(payloads.get(STORE_ITEM_ID_D411, []), contract=contract, limits=limits)]
        if _INCLUDE_D411_PRICE_SHEET
        else []
    )
    d410_projs = (
    ...
    d420_projs = (
        [build_store_projection_d420(
            {sid: payloads.get(sid, []) for sid in store_item_ids_d420()}, contract=contract, limits=limits
        )]
        if _INCLUDE_D420_RETURN_SHEET
        else []
    )
    ...
    d436_projs = (
        [build_store_projection_d436(payloads.get(STORE_ITEM_ID_D436, {}), contract=contract, limits=limits)]
        if _INCLUDE_D436_CUTOFF_SHEET
        else []
    )
    d47_projs = (
...
def merge_projection_into_all_d4_stores(
    *,
    ...
    d411_results = (
        {
            STORE_ITEM_ID_D411: merge_projection_into_d411_rows(
                projection=projection,
                base_payload=base_by_item.get(STORE_ITEM_ID_D411, []),
            )
        }
        if _INCLUDE_D411_PRICE_SHEET
        else {}
    ...
    d420_results = (
        merge_projection_into_d420_stores(
            projection=projection,
            base_by_item={sid: base_by_item.get(sid, []) for sid in store_item_ids_d420()},
        )
        if _INCLUDE_D420_RETURN_SHEET
        else {}
    )
...
#: D4-36 store item（dict 形态 {forward,backward,...params}，双区非行数组）。同 D4-9/D4-34：**在**
#: STORE_ITEM_IDS 里，但 oo_to_html 镜像走专用 dict 块（3-tuple 门面 merge_d436_from_projection），
