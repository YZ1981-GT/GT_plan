...
见 design.md §5 / Sprint 1 Task 16。

职责：

- ``id = "kingdee"`` / ``priority = 80``。
- 只定义 **关键列 + 次关键列** 的中文别名（``balance`` / ``ledger`` 两张表）；
  ``aux_balance`` / ``aux_ledger`` / ``account_chart`` 返回空 dict，走 ``GenericAdapter``
  的通用模糊匹配兜底。
- 识别特征（``match``）：
    - 文件名命中 ``金蝶|kingdee|K3|EAS|Cloud\\s*account`` → ``+0.5``
    - 某个 sheet 的 header 与金蝶关键信号集 ``{"凭证摘要","制单","客户","期初余额",
      "期末余额"}`` 交集 ≥ 2 → ``+0.3``
    - 总分 ``min(score, 1.0)``
...
_FILENAME_RE = re.compile(r"(?i)(金蝶|kingdee|K3|EAS|Cloud\s*account)")
...
class KingdeeAdapter(BaseAdapter):
    """金蝶 K3/EAS/Cloud 适配器。"""

    id = "kingdee"
    display_name = "金蝶 K3/EAS/Cloud"
    priority = 80
    ...
    def get_column_aliases(self, table_type: "TableType") -> dict[str, list[str]]:
        if table_type == "balance":
            return {
                "account_code": ["科目代码", "科目编号"],
                "account_name": ["科目名称", "账户名称"],
                "opening_balance": ["期初余额"],
                "debit_amount": ["借方发生额", "本期借方"],
                "credit_amount": ["贷方发生额", "本期贷方"],
                "closing_balance": ["期末余额"],
            }
        if table_type == "ledger":
            return {
                "voucher_date": ["凭证日期", "业务日期"],
                "voucher_no": ["凭证编号"],
                "summary": ["凭证摘要", "摘要"],
                "account_code": ["科目代码"],
                "debit_amount": ["借方金额"],
