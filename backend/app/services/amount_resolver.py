"""金额解析器抽象 (AmountResolver) — A1/A2 公式引擎统一

把"金额从哪取"抽象为可注入接口，让 report_engine 的一套公式求值逻辑
同时服务单体报表（trial_balance）与合并报表（consol_trial）两个数据源。

设计原则（关联 ADR-CONSOL-101 / 设计 §三 组件1）：
- Resolver 只负责"取数"，不含公式解析/求值（那是 report_engine 的职责）。
