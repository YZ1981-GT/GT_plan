...
2个resolver:
- d6_tb_unadjusted: trial_balance 科目1402 期初/期末未审数
- d6_impairment_tb: trial_balance 坏账准备相关科目（1402对应减值）期初/期末未审数
...
# ═══════════════════════════════════════════════════════════════════════════════
# Resolver: d6_tb_unadjusted — 科目1402期初/期末未审数
# ═══════════════════════════════════════════════════════════════════════════════
...
async def _resolve_d6_tb_unadjusted(
    ...
    **kwargs: Any,
    ...
    """从 trial_balance 获取科目1402的本期/上期未审数。
    ...
    current = await db.execute(
        sa.text("""
            ...
            WHERE project_id = :pid AND year = :year AND is_deleted = false
              AND standard_account_code = '1402'
        """),
    ...
    summary = f"合同资产1402: 期末未审={cur:,.0f}，期初未审={pri:,.0f}"
...
# 合同资产减值准备对应科目：1402对应的备抵科目
# 通常在 trial_balance 中以 standard_account_code LIKE '1402%' 且为贷方性质
# 或者直接查明细科目如 140201（合同资产减值准备）
_IMPAIRMENT_ACCOUNT_CODES = ["140201", "1402.01"]
...
async def _resolve_d6_impairment_tb(
    ...
    **kwargs: Any,
    ...
    查询逻辑：
    1. 优先精确匹配 140201/1402.01
    2. 若无结果则模糊匹配 LIKE '1402%' 且 direction='贷' 的明细科目
