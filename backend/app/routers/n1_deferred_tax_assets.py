...
_SUPPORTED_SHEETS: set[str] = {"N1-2"}

# N1-2 明细表列头（11列，对应可抵扣暂时性差异明细）
_SHEET_HEADERS: dict[str, list[str]] = {
    "N1-2": [
        "项目名称", "分类", "账面价值", "计税基础",
        "可抵扣暂时性差异", "适用税率", "期初递延税资产",
        "本期确认", "本期转回", "期末递延税资产", "备注",
    ],
}
...
_NUMERIC_FIELDS: dict[str, set[str]] = {
    "N1-2": {
        ...
        "openingDta", "currentRecognized", "currentReversed", "closingDta",
    },
}
...
async def n1_export_template(
    wp_id: str,
    sheet: str = Query("N1-2", description="Sheet编码: N1-2"),
