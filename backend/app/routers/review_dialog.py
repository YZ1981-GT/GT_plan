...
# I 循环六组披露表（I1 无形资产 / I2 开发支出 / I3 商誉 / I4 长期待摊费用 /
# I5 其他非流动资产 / I6 研发费用）。前端 section_id = `{wpCode}-disc-{variant}-{key}`。
_I_CYCLE_DISCLOSURE_PROMPTS: dict[str, dict[str, dict[str, str]]] = {
    "I1": {
        "listed": {
            "note": (
                "撰写无形资产附注披露说明（上市版）。依据源模板四层列转置结构（账面原值 / 累计摊销 / "
                "减值准备 / 账面价值），按分类（土地使用权 / 专利权 / 非专利技术 / 商标权 / 软件 / "
                "特许经营权 / 其他）说明期末与上年年末的构成、变动原因，以及摊销方法、"
                "预计使用寿命与残值的会计估计。"
            ),
            "impairment": (
                "撰写无形资产减值测试的附注披露说明。依据 15 号文：长期资产本期进行减值测试的，"
                "应披露可收回金额的具体确定方法；按公允价值减处置费用后的净额确定的，说明公允价值与"
                "处置费用的确定方式、关键参数及依据；按预计未来现金流量现值确定的，说明预测期、"
                "折现率、增长率等关键假设。即使本期未计提减值，也需披露测试方法与参数。"
            ),
        ...
        "soe": {
            "note": (
                "撰写无形资产附注披露说明（国企版）。依据源模板四层结构说明各分类的期末余额与期初余额"
                "构成（土地使用权 / 专利权 / 非专利技术 / 软件 / 其他），以及本期增减变动的主要原因"
                "（购入 / 内部研发转入 / 处置 / 摊销），并说明摊销政策与会计估计是否与上期一致。"
            ),
            "impairment": (
                "撰写无形资产减值测试的附注披露说明（国企版）。说明本期计提无形资产减值准备的"
                "项目、金额与计提原因，以及可收回金额的确定方法与关键假设。"
                "无减值事项时说明已执行的减值评估过程。"
            ),
        },
    },
    "I2": {
        "listed": {
            "note": (
                "撰写开发支出附注披露说明（上市版）。依据源模板变动表（期初 / 本期研发支出增加 / "
                "其他增加 / 确认为无形资产 / 其他减少 / 期末）说明各研发项目的构成与增减原因，"
                "并说明资本化的判断标准（技术可行性 / 完成意图 / 使用或出售能力 / 经济利益流入 / "
                "归属支出可靠计量）。"
            ),
        },
        "soe": {
            "note": (
                "撰写开发支出附注披露说明（国企版）。说明各研发项目的期初余额与期末余额构成，"
                "本期资本化的研发支出金额及其占研发投入的比重，确认为无形资产的时点判断，"
                "以及资本化条件满足的依据。"
            ),
        },
    },
    "I3": {
        "listed": {
            "note": (
                "撰写商誉附注披露说明（上市版）。依据源模板变动表（期初 / 企业合并形成 / 其他增加 / "
                "减值准备转回 / 处置或其他减少 / 本期计提减值 / 期末）说明各被收购方形成的商誉"
                "账面原值与累计减值，以及商誉所在资产组或资产组组合的确定方法、"
                "减值测试的关键假设与参数。"
            ),
            "impairment": (
                "撰写商誉减值测试的附注披露说明。依据 CAS8 与 15 号文：说明商誉所属资产组或"
                "资产组组合的确定方法、可收回金额的确定方法（公允价值减处置费用的净额或预计未来"
                "现金流量现值）、预测期收入增长率、折现率、永续增长率等关键假设，"
                "以及关键假设变化对减值测试结果的敏感性。"
            ),
        },
        "soe": {
            "note": (
                "撰写商誉附注披露说明（国企版）。说明各项商誉形成的业务背景、"
                "被收购方名称与收购时点、商誉账面原值与累计减值准备，"
                "以及本期增减变动的原因。"
            ),
            "impairment": (
                "撰写商誉减值测试的附注披露说明（国企版）。说明减值测试所依据的资产组或"
                "资产组组合、可收回金额的确定方法与关键假设，以及本期计提或转回减值准备的金额与原因。"
                "即使未计提减值也须披露测试方法。"
            ),
        },
    },
    "I4": {
        "listed": {
            "note": (
                "撰写长期待摊费用附注披露说明（上市版）。依据源模板变动表（期初 / 本期增加 / "
                "本期摊销 / 其他减少 / 期末）说明各项目的性质（装修费 / 开办费 / 租赁改良 / "
                "固定资产大修理 / 其他）、摊销方法与摊销期限，以及本期重大增减变动的原因。"
            ),
        },
        "soe": {
            "note": (
                "撰写长期待摊费用附注披露说明（国企版）。说明各项目的期初余额与期末余额构成、"
                "本期增加与摊销金额、摊销方法与剩余摊销期限，以及重大变动原因。"
            ),
        },
    },
    "I5": {
        "listed": {
            "note": (
                "撰写其他非流动资产附注披露说明（上市版）。依据源模板（账面余额 / 减值准备 / "
                "账面价值）说明各项目（预付设备款 / 预付工程款 / 待抵扣进项税 / 合同资产 / "
                "其他）的构成与期末期初变动原因，以及减值准备的计提依据。"
            ),
        },
        "soe": {
            "note": (
                "撰写其他非流动资产附注披露说明（国企版）。说明各项目的期末余额与期初余额、"
                "减值准备金额与账面价值，以及与流动资产的划分口径与重分类情况。"
            ),
        },
    },
    "I6": {
        "listed": {
            "note": (
                "撰写研发费用附注披露说明（上市版）。依据源模板平表结构按项目说明本期研发费用的构成"
                "（直接材料 / 直接人工 / 折旧摊销 / 设计费 / 试验检测费 / 其他），"
                "并说明研发费用与开发支出的划分口径、费用化与资本化的判断标准，"
                "以及与利润表「研发费用」行的核对情况。"
            ),
        },
        "soe": {
            "note": (
                "撰写研发费用附注披露说明（国企版）。按项目说明本期研发费用的构成与金额，"
                "并说明费用化研发支出与资本化研发支出的划分口径，以及科技创新专项要求的分类信息。"
            ),
        },
    },
}
...
_SECTION_PROMPTS: dict[str, str] = {
    ...
    **{
        ...
        for wp_code, variants in _I_CYCLE_DISCLOSURE_PROMPTS.items()
    ...
    **{
    ...
    # ─── E0 发函清单（4 表 × 说明/结论 = 8 条） ─────────────────────────────────
    "e0-3-send-list-audit-note": (
        "撰写 E0-3 货币资金发函记录表的审计说明。依据函证范围（含零余额账户和本期内注销的"
        "账户，见 E0A 程序 1）、金额口径（账面余额/对账单余额）、覆盖比例，说明已编制的"
        "发函清单的完整性与正确性；如有未函证账户须记录理由（E0-1 O29 原文要求）。\n"
        + _NO_FABRICATION
    ),
    "e0-3-send-list-audit-conclusion": (
        "撰写 E0-3 货币资金发函记录表的审计结论。依据函证范围覆盖率与受限标记情况，就"
        "发函清单是否完整覆盖所有应函证银行账户给出结论；存在未函证账户时须说明是否满足"
        "审计准则规定的免函条件（CAS 1312.A19-A21）。\n"
        + _NO_FABRICATION
    ),
    "e0-4-send-list-audit-note": (
        "撰写 E0-4 借款发函记录表的审计说明。依据发函清单所列借款（短期/长期），说明已执行"
        "的函证程序安排、覆盖金额及比率、借款品种分布。\n"
        + _NO_FABRICATION
    ),
    "e0-4-send-list-audit-conclusion": (
        "撰写 E0-4 借款发函记录表的审计结论。就借款函证清单的完整性给出结论。\n"
        + _NO_FABRICATION
    ),
    "e0-5-send-list-audit-note": (
        "撰写 E0-5 应付银行承兑汇票发函记录表的审计说明。依据票面金额合计与 F3-2 应付银行"
        "承兑汇票明细的勾稽情况，说明发函范围覆盖与一函多票的分组安排。\n"
        + _NO_FABRICATION
    ),
    "e0-5-send-list-audit-conclusion": (
        "撰写 E0-5 应付银行承兑汇票发函记录表的审计结论。就银承函证清单是否完整覆盖所有"
        "待函证银行承兑汇票给出结论；与 F3-2 或 trial_balance 2201 有差异时须说明原因。\n"
        + _NO_FABRICATION
    ),
    "e0-6-send-list-audit-note": (
        "撰写 E0-6 理财产品发函记录表的审计说明。依据发函清单所列理财产品，说明产品类型"
        "分布（封闭式/开放式）、期末净值合计、受限情况。\n"
        + _NO_FABRICATION
    ),
    "e0-6-send-list-audit-conclusion": (
        "撰写 E0-6 理财产品发函记录表的审计结论。就理财产品函证清单的完整性给出结论。\n"
        + _NO_FABRICATION
    ),
    # ─── G0 投资循环函证 G0-1 下区（审计说明 5 段 + 审计结论） ────────────────
    #
    # spec: g0-confirmation-source-alignment Task 19（R3.7 / R3.9）
    #
    # 🔴 与 F0 的区别（F0 那 6 条已撤回，见下方）：**G0 这 6 条是可达的**。
    #    可达链路（逐环实证 2026-08-04）：
    #      GtWpRenderer → useWorkpaperScaffold → useWorkpaperReviewProvide
    #        → provide('openReviewDialog')            ← 没有它 GtReviewTrigger 的
    #                                                    `v-if="openReviewDialog"` 不渲染
    ...
    #      section-id 真源 = `g0SummaryLowerZone.G0_AUDIT_NOTE_DEFS[].reviewSectionId`
    #        + `G0_CONCLUSION_REVIEW_SECTION_ID`
    #
    # 🔴 `resolve_review_ai_prompt` 是 `.get()` 回退语义（**无门**）→ 缺登记不报错、
    #    只是退化成笼统的通用 prompt。故本组登记的收益是 prompt 质量，不是可达性。
    #
    # 源模板锚点（`backend/wp_templates/G/G0 投资循环函证.xlsx` 的
    # `函证结果汇总表G0-1`，openpyxl `data_only=False` 直读逐格核对）：
    #   S19 三、审计说明 / S20 第1项 / X20 第2项（提示语 X21+X22）
    #   S24 第3项 / S25 第4项（提示语 S26）/ S28 第5项 / C30 四、审计结论
    #   参考结论 A/B/C 内容在 B55/B56/B57（A55~A57 只是 `A、`/`B、`/`C、` 标签）
    #
    # 🔴 源模板笔误原样保留：S24 原文写「（G0-6）」，而回函可靠性验证表实为
    #    **G0-7**（`邮件传真回函可靠性验证G0-7`），G0-6 是替代程序检查表。
    #    prompt 里按**意图**指向 G0-7，但不得去改源模板文字断言（Task 20 源缺陷登记）。
    "G0-1-audit-note-1": (
        "撰写 G0-1 投资循环函证结果汇总表「1、对询证函保持的控制的说明」（源模板 S20）。"
        "依据 CAS 1312 第十条，说明注册会计师对询证函全过程保持控制的具体措施："
        "被函证方名称与地址的独立核实（G0-2）、询证函的编制与寄发由项目组亲自实施、"
        "回函直接寄至会计师事务所而非被审计单位、跟函过程留痕（G0-3）。"
        "\n" + _NO_FABRICATION
    ),
    "G0-1-audit-note-2": (
        "撰写 G0-1「2、对误差的分析」（源模板 X20）。按源模板界定误差构成条件"
        "（源 X21+X22：不符事项金额高于或低于账户余额人民币（）万元，"
        "并且被审计单位不能合理解释其差异并提供相应依据），逐项分析回函不符事项："
        "区分估值时点差异、交易日与结算日差异、计量方法差异，并判断是否构成误差、"
        "是否需推 A13 错报。"
        "\n" + _NO_FABRICATION
    ),
    "G0-1-audit-note-3": (
        "撰写 G0-1「3、对以传真或电子邮件形式收到的回函的可靠性的考虑」（源模板 S24）。"
        "依据回函可靠性验证底稿 **G0-7**（源模板该处交叉引用写「（G0-6）」属笔误，"
        "G0-6 是替代程序检查表）说明已执行的可靠性程序：回函方身份确认、"
        "邮箱域名与被函证方公示信息的一致性核对、致电原始联系人复核、"
        "必要时索取纸质原件，并对电子回函作为审计证据的可靠性作出评价。"
        "\n" + _NO_FABRICATION
    ),
    "G0-1-audit-note-4": (
        "撰写 G0-1「4、针对不符事项的程序」（源模板 S25）。说明对回函不符项执行的"
        "追加程序（调取原始交易凭证、对账单、持仓证明，重新核对差异构成，"
        "与被审计单位沟通并取得书面解释）。按源模板提示语（S26）："
        "如果回函中存在未函证的其他信息，应考虑未函证信息的影响，"
        "并考虑实施进一步审计程序。"
        "\n" + _NO_FABRICATION
    ),
    "G0-1-audit-note-5": (
        "撰写 G0-1「5、针对未回函的替代程序」（源模板 S28）。依据替代程序检查表 G0-6，"
        "说明对未回函投资项目执行的替代程序及其结果："
        "期末持仓证明或托管对账单检查、股利/利息收入原始凭证检查、"
        "投资处置收益的银行流水与结算单核对、公允价值 Level1-3 佐证检查，"
        "并评价所获证据对存在性与计价认定的充分适当性。"
        "\n" + _NO_FABRICATION
    ),
    "G0-1-conclusion": (
        "撰写 G0-1「四、审计结论」（源模板 C30）。依据函证覆盖率、回函确认金额、"
        "替代程序确认金额与账面金额的勾稽结果给出结论，并从源模板参考结论三选一为基准"
        "（源 B55 未见异常 / B56 除以下重大不符事项应当作为调整事项予以调整外，其余未见异常 / "
        "B57 由于存在以下重大未调整事项（或审计范围受到限制无法获取充分、适当证据），不可确认）。"
        "\n" + _NO_FABRICATION
    ),
    # ─── F0 存货循环函证 ──────────────────────────────────────────────────────
    # 🔴 2026-08-03 已撤回原 6 条 F0 prompt（f0-confirmation-linkage Task 29）。
    #    撤回依据（openpyxl 逐格实证 + 全仓 grep，三条都成立）：
    #    ① **不可达** —— `review-dialog/ai-generate` 的 section_id 来自
    #       `useReviewDialog(props.sectionId)`，而 confirmation 目录下没有任何组件
    #       使用 `GtReviewTrigger` / 传 sectionId（F0 的审计说明走的是
    #       `ai-generate-notes` 端点 + 平台通用 5 段 `ConfirmationNotes`）。
    #    ② **F0-1 段划分错** —— 源模板「三、审计说明」实为 5 段
    #       （S29 对询证函保持的控制的说明 / W29 对误差的分析 / S33 对以传真或电子邮件
    #        形式收到的回函的可靠性的考虑 / S34 针对不符事项的程序 / S37 针对未回函的
    #        替代程序），原 4 条既自造了不存在的「第三方平台评估」段，又漏了两段。
    #    ③ **F0-4 压根没有审计说明区** —— 源模板 F0-4 只有 9 列明细 + R21 合计行。
    #    若将来要给 F0 做源模板忠实的分段审计说明（替代平台通用 5 段），
    #    需先裁决「F0 是否脱离共享 `ConfirmationNotes`」——属共享组件级变更，
    #    见 spec Notes「F0-1 下区审计说明分段」。不得只补 prompt 不接 UI。
}
