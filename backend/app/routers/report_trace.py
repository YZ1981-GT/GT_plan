...
async def lock_project(project_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def create_temp_project(req: TempProjectRequest, db: AsyncSession = Depends(get_db)):
