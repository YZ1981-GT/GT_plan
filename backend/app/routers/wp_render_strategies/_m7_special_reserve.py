...
component_type = "m7-special-reserve"

M7 前端组件 GtM7SpecialReserve 为自加载组件（子组件各自拉取 checklist-responses），
按 sheetName 分发到各子组件（审定表/明细/计提测试/支出检查/调整/附注等）。
...
M7_SHEETS = [
    {"sheet_name": "底稿目录", "component_type": "m7-special-reserve"},
    {"sheet_name": "专项储备实质性程序表M7A", "component_type": "m7-special-reserve"},
    {"sheet_name": "审定表M7-1", "component_type": "m7-special-reserve"},
    {"sheet_name": "附注披露信息（上市公司）", "component_type": "m7-special-reserve"},
    {"sheet_name": "附注披露信息（国有企业）", "component_type": "m7-special-reserve"},
    {"sheet_name": "明细表M7-2", "component_type": "m7-special-reserve"},
    {"sheet_name": "调整分录汇总M7-3", "component_type": "m7-special-reserve"},
    {"sheet_name": "专项储备计提测试表M7-4", "component_type": "m7-special-reserve"},
    {"sheet_name": "专项储备支出检查表M7-5", "component_type": "m7-special-reserve"},
]
    ...
    try:
        ...
        stmt = (
            ...
            .where(
                ...
                TbBalance.standard_account_code == _M7_ACCOUNT_CODE,
                active_filter,
            )
            .limit(1)
        )
...
async def render(ctx: RenderContext) -> dict[str, Any]:
    ...
    前端 GtM7SpecialReserve 为自加载组件（各子组件独立拉取 checklist_responses）。
    """
    ...
    try:
        ...
        logger.warning("M7 render: checklist_responses 查询失败: %s", e)
    ...
    try:
        ...
        logger.warning("M7 render: project context 查询失败: %s", e)
    ...
    for item_id, data in responses_snapshot.items():
        if not item_id.startswith("M7-1-"):
            continue
    ...
    return {
        ...
        "formula_direction": {
            ...
            "note": "权益类贷方科目：安全生产费计提在贷方增加，使用（费用化/资本化）在借方减少",
        },
        # 公式校验错误（前端可展示红色提示）
