"""H9 租赁负债 — 导入导出3端点（明细表/调整分录）.

POST /api/workpapers/{wp_id}/h9/export-template
POST /api/workpapers/{wp_id}/h9/export-data
POST /api/workpapers/{wp_id}/h9/import-data
"""
...
_SPECS: dict[str, dict[str, Any]] = {
    "H9-2": {
        ...
        "guidance": ["H9-2 编制说明", "", "请按项目逐行填写明细数据。"],
    },
    "H9-3": {
        "item_id": "H9-3-adj-entries",
        "storage_field": "remark",
        "title": "调整分录H9-3",
        "headers": _ADJ_HEADERS,
        "field_keys": _ADJ_KEYS,
        "guidance": ["H9-3 编制说明", "", "记录审计调整分录（AJE）和重分类分录（RJE）。"],
    },
}

router = create_cycle_import_export_router(
    tag="h9-import-export",
    api_prefix="h9",
    specs=_SPECS,
    storage_field="remark",
)
