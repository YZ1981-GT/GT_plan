...
from app.services.four_table.g_cycle_specs import G11_SPEC
...
#: G11 科目定位改走**语义驱动**（单一真源 `four_table/g_cycle_specs.G11_SPEC`）。
G11_ACCOUNT_SPEC = G11_SPEC
...
async def _fetch_tb_data(ctx: RenderContext) -> dict:
    ...
    try:
        accounts = await resolve_semantic_accounts(ctx, G11_ACCOUNT_SPEC)
