...
L1 前端组件 GtL1ShortTermLoans 为自加载组件（子组件各自拉取 checklist-responses），
...
循环把 L1 各 sheet 误判为非白名单而重写成 onlyoffice-sheet。

科目2001短期借款（贷方/负债类！）：期末=期初+贷方-借方
数据持久化在 checklist_responses 表，item_id 前缀为 "L1-*"。
...
async def render(ctx: RenderContext) -> dict | None:
    """L1 短期借款渲染策略 — 返回轻量 html_data。

    返回 dict（非 grid cells），确保前端 GtWpRenderer 走 rendererEntry 分发到
    GtL1ShortTermLoans，而非 grid 兜底或 OnlyOffice。
    ...
    # ─── 从 checklist_responses 加载 L1-* 数据快照 ───────────────────────
    ...
    try:
        ...
        logger.warning("L1 render: checklist_responses 查询失败 wp_id=%s: %s", wp_id, e)
    ...
    try:
        ...
        logger.warning("L1 render: project context 查询失败: %s", e)

    # ─── TB 取数（L 循环四表取数 spec） ─────────────────────────────────────
    tb_payload = await build_l_tb_payload(ctx, "L1")
