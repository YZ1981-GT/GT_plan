...
sections:
  ...
  - adjustment-conclusion    (G7-3 调整分录审计结论)
  - adjustment-note          (G7-3 调整分录审计说明)
"""
...
_SECTION_PROMPTS: dict[str, str] = {
    ...
    "adjustment-conclusion": (
        "请基于G7-3调整分录汇总数据，生成审计结论。需包含：\n"
        "1. 分录完整性：账项调整(AJE)与报表调整(RJE)分类是否恰当\n"
        "2. 借贷是否平衡，重大调整是否均有依据与索引\n"
        "3. 对长期股权投资(1511)及减值准备(1512)的净影响是否合理\n"
        "4. 来自G7-14/G7-13的建议草稿是否已复核采纳或剔除\n"
        "5. 总体结论：调整分录是否可据以回写G7-1审定表"
    ),
    "adjustment-note": (
        "请基于G7-3调整分录汇总数据，生成审计说明（程序与证据，非最终结论）。需包含：\n"
        "1. 已执行的分录复核程序（核对上游测算、凭证、重要性）\n"
        "2. 借贷不平衡或未解释差异的处理情况（如有）\n"
        "3. 建议草稿与手工分录的区分及采纳依据\n"
        "4. 拟调整/未调整事项对报表的影响说明"
    ),
}
