...
from app.services.four_table.i_cycle_specs import I4_SPEC
...
async def _fetch_tb_data(ctx: RenderContext):
    ...
    # 科目定位（语义驱动）
    try:
        _sem_accounts = await resolve_semantic_accounts(ctx, I4_SPEC)
    except Exception:  # noqa: BLE001
        _sem_accounts = None

    try:
        extraction = await load_i_cycle_extraction(ctx, "I4")
    except Exception as e:  # noqa: BLE001 — 取数失败不阻断 render
        logger.warning("I4 四表取数失败: %s", e)
        return {}, None
...
async def render(ctx: RenderContext) -> dict | None:
    ...
    if settings.HI_CYCLE_FOUR_TABLE_EXTRACTION_ENABLED:
        try:
            ...
            _logging.getLogger(__name__).warning("HI extraction prefill failed (%s): %s", "I4", e)
