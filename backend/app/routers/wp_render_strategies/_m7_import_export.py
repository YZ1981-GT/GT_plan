"""M7 专项储备 — 导入导出3端点（明细表/调整分录）.

POST /api/workpapers/{wp_id}/m7/export-template
POST /api/workpapers/{wp_id}/m7/export-data
POST /api/workpapers/{wp_id}/m7/import-data
"""
...
_SPECS: dict[str, dict[str, Any]] = {
    "M7-2": {
        "item_id": "M7-2-full-data",
        "storage_field": "remark",
        "title": "明细表M7-2",
        "headers": _DETAIL_HEADERS,
        "field_keys": _DETAIL_KEYS,
        "guidance": ["M7-2 编制说明", "", "请按项目逐行填写明细数据。"],
    },
    "M7-3": {
        "item_id": "M7-3-adj-entries",
        "storage_field": "remark",
        "title": "调整分录M7-3",
        "headers": _ADJ_HEADERS,
        "field_keys": _ADJ_KEYS,
        "guidance": ["M7-3 编制说明", "", "记录审计调整分录（AJE）和重分类分录（RJE）。"],
    },
}

router = create_cycle_import_export_router(
    tag="m7-import-export",
    api_prefix="m7",
    specs=_SPECS,
    storage_field="remark",
)
