...
_I2_CUTOFF_GUIDANCE_FORWARD = [
    ...
    "",
    "一、本表目的",
    "从期末前后开发支出(1717)记账凭证出发，追查支出凭单真实性与期间归属（关注多记/虚假入账）。",
    "",
    "二、填写要求",
    "1. 样本范围：资产负债表日前后N天、金额大于阈值的1717记账凭证。",
    "2. 凭单日期须查原件后填写；自动取数导入时该列可留空。",
    "3. 跨期判定：记账日期与凭单日期分处截止日两侧即为跨期（系统导入后自动重算）。",
    "4. 结论示例：正常 / 跨期多记 / 跨期漏记。",
    "5. 请与 I2-14（单据→账）交叉评价；摘要含6602/费用化时联动 I6-5/I6-6。",
]
...
_I2_CUTOFF_GUIDANCE_BACKWARD = [
    ...
    "",
    "一、本表目的",
    "从期末前后支出凭单出发，追查是否已记入开发支出(1717)（关注漏记/跨期）。",
    "",
    "二、填写要求",
