"""N4 税金及附加 — 导入导出3端点（明细表/调整分录）.

POST /api/workpapers/{wp_id}/n4/export-template
POST /api/workpapers/{wp_id}/n4/export-data
POST /api/workpapers/{wp_id}/n4/import-data
"""
...
_SPECS: dict[str, dict[str, Any]] = {
    "N4-2": {
        "item_id": "N4-2-detail-rows",
        "storage_field": "remark",
        "title": "明细表N4-2",
        "headers": _DETAIL_HEADERS,
        "field_keys": _DETAIL_KEYS,
        "guidance": ["N4-2 编制说明", "", "请按项目逐行填写明细数据。"],
    },
    "N4-3": {
        "item_id": "N4-3-adj-entries",
        "storage_field": "remark",
        "title": "调整分录N4-3",
        "headers": _ADJ_HEADERS,
        "field_keys": _ADJ_KEYS,
        "guidance": ["N4-3 编制说明", "", "记录审计调整分录（AJE）和重分类分录（RJE）。"],
    },
}

router = create_cycle_import_export_router(
    tag="n4-import-export",
    api_prefix="n4",
    specs=_SPECS,
    storage_field="remark",
)
