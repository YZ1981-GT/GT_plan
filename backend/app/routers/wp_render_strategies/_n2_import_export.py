"""N2 应交税费 — 导入导出3端点（明细表/调整分录）.

POST /api/workpapers/{wp_id}/n2/export-template
POST /api/workpapers/{wp_id}/n2/export-data
POST /api/workpapers/{wp_id}/n2/import-data
"""
...
_SPECS: dict[str, dict[str, Any]] = {
    "N2-2": {
        ...
        "guidance": ["N2-2 编制说明", "", "请按项目逐行填写明细数据。"],
    },
    "N2-3": {
        "item_id": "N2-3-adj-entries",
        "storage_field": "remark",
        "title": "调整分录N2-3",
        "headers": _ADJ_HEADERS,
        "field_keys": _ADJ_KEYS,
        "guidance": ["N2-3 编制说明", "", "记录审计调整分录（AJE）和重分类分录（RJE）。"],
    },
}

router = create_cycle_import_export_router(
    tag="n2-import-export",
    api_prefix="n2",
    specs=_SPECS,
    storage_field="remark",
)
