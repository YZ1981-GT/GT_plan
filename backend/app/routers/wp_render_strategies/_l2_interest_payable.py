...
L2 前端组件 GtL2InterestPayable 为自加载组件（子组件各自拉取 checklist-responses），
按 sheetName 分发到各子组件（审定表/明细/调整分录/检查表/附注等）。
...
循环把 L2 各 sheet 误判为非白名单而重写成 onlyoffice-sheet。

科目2231应付利息（贷方/负债类！）：期末=期初+贷方-借方
数据持久化在 checklist_responses 表，item_id 前缀为 "L2-*"。
...
async def render(ctx: RenderContext) -> dict | None:
    """L2 应付利息渲染策略 — 返回轻量 html_data。

    返回 dict（非 grid cells），确保前端 GtWpRenderer 走 rendererEntry 分发到
    GtL2InterestPayable，而非 grid 兜底或 OnlyOffice。
    ...
    # ─── 从 checklist_responses 加载 L2-* 数据快照 ───────────────────────
    ...
    try:
        ...
        logger.warning("L2 render: checklist_responses 查询失败 wp_id=%s: %s", wp_id, e)
    ...
    try:
        ...
        logger.warning("L2 render: project context 查询失败: %s", e)

    # ─── TB 取数（L 循环四表取数 spec） ─────────────────────────────────────
    tb_payload = await build_l_tb_payload(ctx, "L2")
