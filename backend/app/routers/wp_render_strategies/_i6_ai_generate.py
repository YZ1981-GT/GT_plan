...
_SECTION_PROMPTS: dict[str, str] = {
    "adjudication": (
        "请生成I6研发费用审定表的审计说明和结论，分析科目6604的变动情况。"
        "需涵盖：\n"
    ...
    "targeted": (
        "请生成I6-4研发费用针对性检查表审计说明/结论草稿，基于抽凭核对结果：\n"
        "1) 测试原因与样本选取（总体/特定样本/抽样方法）及检查覆盖率\n"
        "2) 逐笔核对内容1~5的执行情况（原始凭证、账证相符、账务处理、期间、I6-2/I2项目归集）\n"
        "3) 异常样本及核对「×」的处理（是否需调整分录或扩大测试）\n"
        "4) 与I6-2本期审定发生额、I2资本化划分的交叉印证\n"
        "5) 是否达成测试目标（发生/分类/准确性/列报）\n"
        "注：加计扣除合规性见N5-6-1，不在本表主程序范围"
    ),
    "i6-disclosure-listed": (
