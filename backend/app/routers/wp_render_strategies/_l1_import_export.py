"""L1 短期借款 — 导入导出3端点（明细表/调整分录）.

POST /api/workpapers/{wp_id}/l1/export-template
POST /api/workpapers/{wp_id}/l1/export-data
POST /api/workpapers/{wp_id}/l1/import-data
"""
...
_SPECS: dict[str, dict[str, Any]] = {
    "L1-2": {
        "item_id": "L1-L1-2-rows",
        "storage_field": "remark",
        "title": "明细表L1-2",
        "headers": _DETAIL_HEADERS,
        "field_keys": _DETAIL_KEYS,
        "guidance": ["L1-2 编制说明", "", "请按项目逐行填写明细数据。"],
    },
    "L1-3": {
        "item_id": "L1-3-adj-entries",
        "storage_field": "remark",
        "title": "调整分录L1-3",
        "headers": _ADJ_HEADERS,
        "field_keys": _ADJ_KEYS,
        "guidance": ["L1-3 编制说明", "", "记录审计调整分录（AJE）和重分类分录（RJE）。"],
    },
}

router = create_cycle_import_export_router(
    tag="l1-import-export",
    api_prefix="l1",
    specs=_SPECS,
    storage_field="remark",
)
