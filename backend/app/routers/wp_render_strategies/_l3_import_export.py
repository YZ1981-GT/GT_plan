"""L3 长期借款 — 导入导出3端点（明细表/调整分录）.

POST /api/workpapers/{wp_id}/l3/export-template
POST /api/workpapers/{wp_id}/l3/export-data
POST /api/workpapers/{wp_id}/l3/import-data
"""
...
_SPECS: dict[str, dict[str, Any]] = {
    "L3-2": {
        "item_id": "L3-L3-2-rows",
        "storage_field": "remark",
        "title": "明细表L3-2",
        "headers": _DETAIL_HEADERS,
        "field_keys": _DETAIL_KEYS,
        "guidance": ["L3-2 编制说明", "", "请按项目逐行填写明细数据。"],
    },
    "L3-3": {
        "item_id": "L3-3-adj-entries",
        "storage_field": "remark",
        "title": "调整分录L3-3",
        "headers": _ADJ_HEADERS,
        "field_keys": _ADJ_KEYS,
        "guidance": ["L3-3 编制说明", "", "记录审计调整分录（AJE）和重分类分录（RJE）。"],
    },
}

router = create_cycle_import_export_router(
    tag="l3-import-export",
    api_prefix="l3",
    specs=_SPECS,
    storage_field="remark",
)
