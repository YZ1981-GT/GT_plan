...
def _parse_d4_1_row(row: tuple, actual_headers: list[str], expected_headers: list[str]) -> dict:
    ...
    section_label = _safe_str(_col_val("区块"))  # 兼容旧模板（含区块列）
    row_key = _safe_str(_col_val("行键"))        # 兼容旧模板（含行键列）
...
_SHEET_GUIDANCE: dict[str, list[str]] = {
    ...
    # ─── 以下 26 条由批量补充（2026-09-23） ───────────────────────────────
    "D4-1": [
        "D4-1 营业收入审定表 编制说明",
        "",
        "一、本表目的",
        "汇总各科目（6001+6051）的未审数、调整和审定数，完成营业收入审定。",
        "",
        "二、表格列说明",
        "项目/本期未审/本期AJE/本期RJE/上期未审/上期AJE/上期RJE",
        "",
        "三、填写要求",
        "1. 「项目」列填写产品/服务名称，与D4-2/D4-3明细表对应。",
        "2. 「本期未审」取自试算平衡表，「AJE/RJE」取自D4-4调整分录。",
        "3. 审定数 = 未审数 + AJE + RJE（自动计算，导入时忽略）。",
        "4. 编制完成后请点击「确认审定」回写试算表。",
        "",
        "四、导入说明",
        "1. 请勿修改表头（第1行列名），否则导入时会校验失败。",
        "2. 空行将被自动跳过。最多支持500行数据。",
        "3. 金额列填写数值（正数），日期格式YYYY-MM-DD。",
    ],
    "D4-8": [
        "D4-8 重要产品毛利分析 编制说明",
        "",
        "一、本表目的",
        "按产品分析主营业务收入的月度毛利率变动，识别异常产品或月份。",
        "",
        "二、填写要求",
        "1. 每行一个产品+月份组合。",
        "2. 销量和金额可从ERP导出或D4-2数据派生。",
        "3. 毛利率异常波动（与上期差异>10%）需解释原因。",
    ],
    "D4-9": [
        "D4-9 重要客户结构分析 编制说明",
        "",
        "一、本表目的",
        "列示本期/上期 Top 10 客户的销售金额及占比，分析客户集中度变化。",
        "",
        "二、填写要求",
        "1. 「期间」列填'本期'或'上期'，导入时按此列区分两期数据。",
        "2. 「销售金额占比」为自动计算列，导入时忽略。",
        "3. 前10客户合计占比>50%时需关注客户集中度风险。",
    ],
    "D4-10": [
        "D4-10 重要客户销售价格分析 编制说明",
        "",
        "一、本表目的",
        "对重要客户的销售价格与年度均价、市场价格进行对比，发现定价异常。",
        "",
        "二、填写要求",
        "1. 数据可从D4-9客户结构自动导入，再补充价格维度。",
        "2. 与均价/市价差异>20%标记为价格异常。",
        "3. 差异原因需逐客户解释（量大折扣、区域差异等）。",
    ],
    "D4-11": [
        "D4-11 产品销售价格分析 编制说明",
        "",
        "一、本表目的",
        "按品种规格分析销售单价与定价表、市场价的偏离度，验证收入计价合理性。",
        "",
        "二、填写要求",
        "1. 定价表单价取自企业内部定价文件。",
        "2. 同期市场价格标注来源（行业报告/公开报价等）。",
        "3. 关注是否存在低于成本价销售的情况。",
    ],
    "D4-13": [
        "D4-13 营业收入账面金额与ERP系统核对记录 编制说明",
        "",
        "一、本表目的",
        "将账面营业收入与ERP系统数据核对，验证完整性和准确性。",
        "",
        "二、填写要求",
        "1. 分两个区块：核对过程和核对结论。",
        "2. 核对过程：描述取数路径、核对口径和计算过程。",
        "3. 核对结论：核对是否一致，差异原因。",
    ],
    "D4-16": [
        "D4-16 出口收入电子口岸系统核对 编制说明",
        "",
        "一、本表目的",
        "将出口收入与电子口岸结关数据、纳税申报外营收入交叉核对。",
        "",
        "二、填写要求",
        "1. 口岸数据从电子口岸系统导出。",
        "2. 申报数据从增值税纳税申报表取得。",
        "3. 差异需逐项查明原因（汇率/时间性差异等）。",
    ],
    "D4-19": [
        "D4-19 销售折扣与折让检查 编制说明",
        "",
        "一、本表目的",
        "检查销售折扣/折让/退回的真实性和合理性，验证是否按CAS14正确处理。",
        "",
        "二、填写要求",
        "1. 折扣类型：商业折扣/现金折扣/销售折让/销售退回。",
        "2. 审批流程完整性（审批人/日期不得空）。",
        "3. 折扣比例异常偏高（>10%）需重点关注。",
    ],
    "D4-20-provision": [
        "D4-20 销售退货准备金重新测算表 编制说明",
        "",
        "一、本表目的",
        "重新测算销售退货准备金的合理性，对比账面已计提金额。",
        "",
        "二、填写要求",
        "1. 计提基数为该产品本期销售收入。",
        "2. 计提比例参照历史退货率或行业经验值。",
    ...
    "D4-20-current": [
        ...
        "",
        "一、本表目的",
        "列示本期发生的销售退货交易，检查退货真实性和会计处理。",
        "",
        "二、填写要求",
        "1. 标记「是否异常=是」的退货将推送至A13未更正错报汇总。",
        "2. 退货原因需具体到产品质量/客户取消/物流损坏等类别。",
    ],
    "D4-20-post": [
        "D4-20 期后销售退货明细 编制说明",
        "",
        "一、本表目的",
        "列示资产负债表日后至审计报告日期间的销售退货，评估期后调整事项。",
        "",
        "二、填写要求",
        "1. 期后退货可能需要追溯调整报告期收入。",
        "2. 关注大额退货是否属于资产负债表日后调整事项。",
    ],
    "D4-21": [
        "D4-21 关联方销售情况及价格分析 编制说明",
        "",
        "一、本表目的",
        "列示与关联方的销售交易，分析关联交易定价公允性。",
        "",
        "二、填写要求",
        "1. 关联关系按CAS36填写（母子/同一控制/关键管理人员等）。",
        "2. 与非关联方价格差异>10%需详细说明原因。",
        "3. 关联交易占比显著上升需评估商业合理性。",
    ],
    "D4-24": [
        "D4-24 第三方回款检查 编制说明",
        "",
        "一、本表目的",
        "检查客户通过第三方回款的情况，评估收入真实性风险。",
        "",
        "二、填写要求",
        "1. 所有第三方回款均需取得代付协议或合理解释。",
        "2. 第三方与被审计单位存在关联关系时需高度关注。",
        "3. 大额第三方回款建议函证确认。",
    ],
    "D4-25": [
        "D4-25 经销商检查 编制说明",
    ...
    "D4-26": [
        ...
        "",
        "一、本表目的",
        "对境外销售收入的真实性和完整性进行专项核查。",
        "",
        "二、填写要求",
        "1. 贸易模式（自营/转口/委托代理等）影响收入确认时点。",
        "2. 核查程序确认金额与账面差异需逐项解释。",
        "3. 至少执行一项外部核查程序（函证/海关/口岸）。",
    ],
    "D4-27": [
        "D4-27 识别未披露的关联方 编制说明",
        "",
        "一、本表目的",
        "将客户信息与公司股东/高管/员工信息交叉比对，识别未披露的关联方。",
        "",
        "二、填写要求",
        "1. 重名客户需进一步查验是否为同一人。",
        "2. 标记为关联方的客户需补充至关联方清单和附注披露。",
    ],
    "D4-28": [
        "D4-28 客户信息核查清单 编制说明",
        "",
        "一、本表目的",
        "对选取的重要客户进行多维度背景核查，评估客户真实性。",
        "",
        "二、填写要求",
        "1. 选取原因标注（金额前N/新增客户/异常波动等）。",
        "2. 各核查手段标注已执行/未执行/不适用。",
        "3. 至少对前5大客户执行工商资料查询。",
    ],
    "D4-29": [
        "D4-29 客户信息检查表 编制说明",
        "",
        "一、本表目的",
        "详细记录每个核查客户的工商、股权、经营信息，评估交易对手方资质。",
        "",
        "二、填写要求",
        "1. 信息来源：国家企业信用信息公示系统/天眼查/企查查/信用中国等。",
        "2. 关注：成立时间近（<2年）+注册资本小+人员少→壳公司风险。",
        "3. 经营状态异常（吊销/注销）的客户交易需重点核查。",
    ],
    "D4-30": [
        "D4-30 客户访谈记录汇总表 编制说明",
        "",
        "一、本表目的",
        "汇总所有客户访谈记录的关键信息，便于审阅和交叉比对。",
        "",
        "二、填写要求",
        "1. 一客户一行，与D4-31访谈记录逐一对应。",
        "2. 访谈方式：实地走访/视频访谈/电话访谈。",
        "3. 访谈结论需明确是否发现异常。",
    ],
    "D4-31": [
        "D4-31 客户访谈记录 编制说明",
        "",
        "一、本表目的",
        "记录单份客户访谈的详细问答内容（键值对格式）。",
        "",
        "二、填写要求",
        "1. 每行一个问答字段（如：被访谈人姓名 = 张三）。",
        "2. 与D4-30汇总表通过「访谈表索引」关联。",
        "3. 导入时按字段名匹配，不按行号。",
    ],
    "D4-32": [
        "D4-32 客户/供应商资金流水检查 编制说明",
        "",
        "一、本表目的",
        "检查客户与供应商之间的资金流水，识别异常资金往来。",
        "",
        "二、填写要求",
        "1. 组别分6组（客户银行收/非银收/供应商银行付/非银付/关联方收/关联方付）。",
        "2. 导入时按「组别」列恢复分组，不按行号。",
        "3. 异常交易包括：大额现金/频繁小额/个人账户等。",
    ],
    "D4-33": [
        "D4-33 其他业务毛利率分析表 编制说明",
        "",
        "一、本表目的",
        "按月度分析其他业务收入各子类别的毛利率变动趋势。",
        "",
        "二、填写要求",
        "1. 预填16行（1~12月+合计+上年数+变动额+变动比例）。",
        "2. 毛利率=（收入-成本）/收入×100%，自动计算。",
        "3. 各月毛利率波动异常需解释原因。",
    ],
    "D4-34-rental": [
        "D4-34 其他业务收入合同测算表（房屋租赁）编制说明",
        "",
        "一、本表目的",
        "根据租赁合同条款测算应计收入，与账面实计收入比较。",
        "",
        "二、填写要求",
        "1. 应计收入 = 合同单价 × 租赁面积 × 实际租赁月数。",
        "2. 差异 = 应计收入 - 实计收入，差异>5%需查明原因。",
    ],
    "D4-34-consult": [
        "D4-34 其他业务收入合同测算表（咨询业务）编制说明",
        "",
        "一、本表目的",
        "根据咨询合同条款测算应计收入，与账面实计收入比较。",
        "",
        "二、填写要求",
        "1. 应计收入按履约进度或完工百分比法确认。",
        "2. 差异 = 应计收入 - 实计收入。",
    ],
    "D4-36-forward": [
        "D4-36 其他业务收入截止性测试（账到单据）编制说明",
        "",
        "一、本表目的",
        "检查期末前已入账的其他业务收入是否有支持性单据，验证截止性。",
        "",
        "二、填写要求",
        "1. 从记账凭证追查到发货单/验收单/合同等。",
        "2. 「是否跨期」：单据日期在下一年度的标记为跨期。",
    ],
    "D4-36-backward": [
        "D4-36 其他业务收入截止性测试（单据到账）编制说明",
        "",
        "一、本表目的",
        "检查期后已取得单据但未入账的其他业务收入，验证完整性。",
        "",
        "二、填写要求",
        "1. 从发货单/验收单追查到记账凭证。",
        "2. 「是否跨期」：凭证日期在下一年度但单据在报告期内的标记为跨期。",
    ],
}
