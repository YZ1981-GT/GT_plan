"""L5 长期应付款 — 导入导出3端点（明细表/调整分录）.

POST /api/workpapers/{wp_id}/l5/export-template
POST /api/workpapers/{wp_id}/l5/export-data
POST /api/workpapers/{wp_id}/l5/import-data
"""
...
_SPECS: dict[str, dict[str, Any]] = {
    "L5-2": {
        ...
        "guidance": ["L5-2 编制说明", "", "请按项目逐行填写明细数据。"],
    },
    "L5-3": {
        "item_id": "L5-3-adj-entries",
        "storage_field": "remark",
        "title": "调整分录L5-3",
        "headers": _ADJ_HEADERS,
        "field_keys": _ADJ_KEYS,
        "guidance": ["L5-3 编制说明", "", "记录审计调整分录（AJE）和重分类分录（RJE）。"],
    },
}

router = create_cycle_import_export_router(
    tag="l5-import-export",
    api_prefix="l5",
    specs=_SPECS,
    storage_field="remark",
)
