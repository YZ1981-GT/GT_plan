...
from app.services.four_table.m_cycle_specs import M5_SPEC

logger = logging.getLogger(__name__)

_M5_ACCOUNT_CODE = "4101"
_M5_ROW_CODE = "BS-083"  # 盈余公积报表行（listed_standalone；soe 为 BS-118）
...
async def _fetch_tb_data(ctx: RenderContext) -> dict[str, Any]:
    ...
    # 科目定位（语义驱动）
    try:
        _sem_accounts = await resolve_semantic_accounts(ctx, M5_SPEC)
...
async def render(ctx: RenderContext) -> dict[str, Any]:
    ...
    try:
        resolved_codes = await resolve_report_line_account_codes(
            ctx.db, ctx.project_id, _M5_ROW_CODE,
            fallback=[_M5_ACCOUNT_CODE],
        )
    except Exception:  # noqa: BLE001
        resolved_codes = [_M5_ACCOUNT_CODE]
    tb_source_codes = {
        "row_code": _M5_ROW_CODE,
        "codes": resolved_codes,
        "resolved_from": "report_config" if resolved_codes != [_M5_ACCOUNT_CODE] else "fallback",
        "basis": "balance",
    }
