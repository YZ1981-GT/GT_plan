...
_SECTION_PROMPTS: dict[str, str] = {
    ...
    "inventory-conclusion": (
        ...
        "1. 是否针对资产负债表中记录的债权投资验证存在性，并确认已记入适当账户\n"
        "2. 盘点程序充分性：盘点日期/地点、监盘人与被审计单位盘点人、是否与现金同时盘点、多地点是否同时盘点\n"
        "3. 现场盘点明细（名称/面值/数量/利率/到期日）是否完整清晰，权属或质押限制是否已关注\n"
        "4. 若盘点日不是资产负债表日，是否已说明衔接 G4-8 倒轧调节\n"
        "5. 异常情况及处理；最终就存在性认定给出明确结论"
    ),
    "reconciliation-conclusion": (
        "请生成G4-8盘点倒轧表的审计结论，评价以下方面：\n"
        "1. 资产负债表日至盘点日增减变动是否已取得交割单/对账单等证据并核对\n"
        "2. 倒轧公式（报表日=盘点日−增加+减少）推算结果与账面结存（数量/面值）的勾稽\n"
        "3. 差异原因是否合理，是否需要调整；账面摊余成本与债权投资账面价值勾对情况\n"
        "4. 通过倒轧程序确认的有价证券存在性与完整性结论（可参考A/B/C结论表述）"
    ),
}
