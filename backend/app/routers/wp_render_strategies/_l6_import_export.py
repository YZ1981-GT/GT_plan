"""L6 专项应付款 — 导入导出3端点（明细表/调整分录）.

POST /api/workpapers/{wp_id}/l6/export-template
POST /api/workpapers/{wp_id}/l6/export-data
POST /api/workpapers/{wp_id}/l6/import-data
"""
...
_SPECS: dict[str, dict[str, Any]] = {
    "L6-2": {
        ...
        "guidance": ["L6-2 编制说明", "", "请按项目逐行填写明细数据。"],
    },
    "L6-3": {
        "item_id": "L6-3-adj-entries",
        "storage_field": "remark",
        "title": "调整分录L6-3",
        "headers": _ADJ_HEADERS,
        "field_keys": _ADJ_KEYS,
        "guidance": ["L6-3 编制说明", "", "记录审计调整分录（AJE）和重分类分录（RJE）。"],
    },
}

router = create_cycle_import_export_router(
    tag="l6-import-export",
    api_prefix="l6",
    specs=_SPECS,
    storage_field="remark",
)
