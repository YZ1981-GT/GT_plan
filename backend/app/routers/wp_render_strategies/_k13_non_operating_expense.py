...
async def _build_adjudication_prefill(ctx: RenderContext) -> list[dict]:
    """从 tb_balance 6711 明细子科目预填 K13-1 审定表行（损益借方取发生额）.

    6711 营业外支出为借方科目：净发生额 = 借方 - 贷方。
    ...
    try:
        ...
        result = await ctx.db.execute(
            ...
            .where(
                active_filter,
                TbBalance.account_code.startswith(_K13_ACCOUNT_PREFIX),
                TbBalance.account_code != _K13_ACCOUNT_PREFIX,
            )
        ...
        logger.warning("K13 adjudication prefill build failed: %s", e)
    return rows
