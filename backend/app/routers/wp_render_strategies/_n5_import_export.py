"""N5 所得税费用 — 导入导出3端点（明细表/调整分录）.

POST /api/workpapers/{wp_id}/n5/export-template
POST /api/workpapers/{wp_id}/n5/export-data
POST /api/workpapers/{wp_id}/n5/import-data
"""
...
_SPECS: dict[str, dict[str, Any]] = {
    "N5-2": {
        ...
        "guidance": ["N5-2 编制说明", "", "请按项目逐行填写明细数据。"],
    },
    "N5-3": {
        "item_id": "N5-3-adj-entries",
        "storage_field": "remark",
        "title": "调整分录N5-3",
        "headers": _ADJ_HEADERS,
        "field_keys": _ADJ_KEYS,
        "guidance": ["N5-3 编制说明", "", "记录审计调整分录（AJE）和重分类分录（RJE）。"],
    },
}

router = create_cycle_import_export_router(
    tag="n5-import-export",
    api_prefix="n5",
    specs=_SPECS,
    storage_field="remark",
)
