...
支持sheets: D1-2, D1-3, D1-4, D1-6, D1-7, D1-8, D1-8T, D1-9, D1-15
...
_SUPPORTED_SHEETS: set[str] = {
    "D1-2", "D1-3", "D1-4", "D1-6", "D1-7", "D1-8", "D1-8T", "D1-9", "D1-15",
}
...
async def d1_import_data(
    ...
    missing_cols = _validate_columns(ws, sheet)
    if missing_cols:
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        if row_count > _ROW_LIMIT:
            truncated = True
            break
        rows_data.append(parse_fn(row, actual_headers))
    ...
    item_ids = _SHEET_ITEM_ID[sheet]
    if isinstance(item_ids, list):
        # D1-4: 全部写入 individual-rows（导入不区分 individual/portfolio）
        item_id = item_ids[0]
    else:
        item_id = item_ids
