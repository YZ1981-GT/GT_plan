"""L7 其他非流动负债 — 导入导出3端点（明细表/调整分录）.

POST /api/workpapers/{wp_id}/l7/export-template
POST /api/workpapers/{wp_id}/l7/export-data
POST /api/workpapers/{wp_id}/l7/import-data
"""
...
_SPECS: dict[str, dict[str, Any]] = {
    "L7-2": {
        ...
        "guidance": ["L7-2 编制说明", "", "请按项目逐行填写明细数据。"],
    },
    "L7-3": {
        "item_id": "L7-3-adj-entries",
        "storage_field": "remark",
        "title": "调整分录L7-3",
        "headers": _ADJ_HEADERS,
        "field_keys": _ADJ_KEYS,
        "guidance": ["L7-3 编制说明", "", "记录审计调整分录（AJE）和重分类分录（RJE）。"],
    },
}

router = create_cycle_import_export_router(
    tag="l7-import-export",
    api_prefix="l7",
    specs=_SPECS,
    storage_field="remark",
)
