...
返回 allResponses + projectContext + TB数据(2701)
供前端 K5-1 审定表试算表列（只读）seed。

**负债口径**：期末 = 期初 + 计提(增加) - 转销/冲回(减少)。
...
# 科目前缀：2701预计负债(贷方/负债类)
_K5_ACCOUNT_PREFIXES = {
    "2701": ("provisions_2701_unadjusted", "provisions_2701_audited"),
}

K5_SHEETS = [
    {"sheet_name": "底稿目录", "component_type": "k5-provisions"},
    {"sheet_name": "预计负债实质性程序表K5A", "component_type": "k5-provisions"},
    {"sheet_name": "审定表K5-1", "component_type": "k5-provisions"},
    {"sheet_name": "附注披露信息（上市公司）", "component_type": "k5-provisions"},
    {"sheet_name": "附注披露信息（国有企业）", "component_type": "k5-provisions"},
    {"sheet_name": "明细表K5-2", "component_type": "k5-provisions"},
    {"sheet_name": "调整分录汇总K5-3", "component_type": "k5-provisions"},
    {"sheet_name": "产品质量保修检查表K5-4", "component_type": "k5-provisions"},
    {"sheet_name": "弃置费用检查表K5-5", "component_type": "k5-provisions"},
    {"sheet_name": "未决诉讼检查表K5-6", "component_type": "k5-provisions"},
    {"sheet_name": "预计负债检查表K5-7", "component_type": "k5-provisions"},
]


_K5_ACCOUNT_PREFIX = "2701"


async def _build_adjudication_prefill(ctx: RenderContext) -> list[dict]:
    """从 tb_balance 2701 明细子科目预填 K5-1 审定表行（负债类取余额）.
    ...
    try:
        ...
        result = await ctx.db.execute(
            ...
            .where(
                active_filter,
                TbBalance.account_code.startswith(_K5_ACCOUNT_PREFIX),
                TbBalance.account_code != _K5_ACCOUNT_PREFIX,
            )
        ...
        logger.warning("K5 adjudication prefill build failed: %s", e)
    return rows


async def _fetch_tb_data(ctx: RenderContext) -> dict:
    """取科目2701的期初/期末余额及借贷发生额（负债口径：正数=贷方余额）."""
    tb: dict[str, float] = {}
    try:
        active_filter = await get_active_filter(
            ctx.db, TbBalance.__table__, ctx.project_id, ctx.year
        ...
        for row in result.fetchall():
            ...
            for prefix, (unadj_key, audited_key) in _K5_ACCOUNT_PREFIXES.items():
        ...
        logger.warning("K5 TB balance fetch failed: %s", e)
    ...
    try:
        result = await ctx.db.execute(
            sa.text("""
                ...
                WHERE project_id = :pid AND year = :year AND is_deleted = false
                  AND standard_account_code LIKE '2701%'
            """),
        ...
        logger.warning("K5 trial_balance fetch failed: %s", e)
...
async def _load_project_context(ctx: RenderContext) -> dict:
    ...
    try:
        ...
        logger.warning("K5 project context load failed: %s", e)
    return project_ctx


async def render(ctx: RenderContext) -> dict | None:
    """K5预计负债渲染策略：allResponses + projectContext + TB数据（负债口径）."""
    ...
    try:
        ...
        logger.warning("K5 render responses load failed: %s", e)
