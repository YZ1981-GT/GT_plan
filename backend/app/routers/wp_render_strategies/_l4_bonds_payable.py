...
component_type = "l4-bonds-payable"

L4 前端组件 GtL4BondsPayable 为自加载组件（子组件各自拉取 checklist-responses），
按 sheetName 分发到各子组件（审定表/明细89列/初始计量/后续计量2分支/账面核对2分支/
权益负债划分/检查表/附注等）。因此本渲染策略只需返回轻量 html_data（project_context
+ responses_snapshot），关键作用是：让 component_type 在 RENDERER_DISPATCH 中命中，
避免多 sheet dispatch 循环把 L4 各 sheet 误判为非白名单而重写成 onlyoffice-sheet。

科目2502应付债券（贷方/负债类！）：期末=期初+贷方-借方
核心特殊：实际利率法EIR后续计量2分支（到期一次还本付息 / 分期付息到期一次还本）
数据持久化在 checklist_responses 表，item_id 前缀为 "L4-*"。
...
async def render(ctx: RenderContext) -> dict | None:
    """L4 应付债券渲染策略 — 返回轻量 html_data。

    返回 dict（非 grid cells），确保前端 GtWpRenderer 走 rendererEntry 分发到
    GtL4BondsPayable，而非 grid 兜底或 OnlyOffice。
    ...
    # ─── 从 checklist_responses 加载 L4-* 数据快照 ───────────────────────
    ...
    try:
        ...
        logger.warning("L4 render: checklist_responses 查询失败 wp_id=%s: %s", wp_id, e)
    ...
    # ─── TB 取数（L 循环四表取数 spec） ─────────────────────────────────────
    tb_payload = await build_l_tb_payload(ctx, "L4")
