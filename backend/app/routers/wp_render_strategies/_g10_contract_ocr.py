"""G10 交易性金融负债 — 凭证附件 OCR.

POST /api/workpapers/{wp_id}/g10/contract-ocr
...
class G10VoucherOcrResponse(BaseModel):
    attachment_id: str
    ocr_text: str
...
def _resp(
    ...
    *,
    reused: bool = False,
    written_back: bool = False,
) -> G10VoucherOcrResponse:
    return G10VoucherOcrResponse(
...
@router.post("/api/workpapers/{wp_id}/g10/contract-ocr", response_model=G10VoucherOcrResponse)
async def g10_contract_ocr(
    wp_id: str,
    ...
    try:
        ...
        logger.warning("G10 voucher OCR failed: %s", e)
    ...
    try:
        messages = [
            {
                "role": "system",
                "content": (
                    "你是审计凭证识别专家。从OCR文本提取交易性金融负债相关凭证字段，"
        ...
        logger.warning("G10 voucher LLM extract failed: %s", e)
