"""M2 实收资本 — 导入导出3端点（明细表/调整分录）.

POST /api/workpapers/{wp_id}/m2/export-template
POST /api/workpapers/{wp_id}/m2/export-data
POST /api/workpapers/{wp_id}/m2/import-data
"""
...
_SPECS: dict[str, dict[str, Any]] = {
    "M2-2": {
        "item_id": "M2-M2-2-listed-full-data",
        "storage_field": "remark",
        "title": "明细表M2-2",
        "headers": _DETAIL_HEADERS,
        "field_keys": _DETAIL_KEYS,
        "guidance": ["M2-2 编制说明", "", "请按项目逐行填写明细数据。"],
    },
    "M2-3": {
        "item_id": "M2-3-adj-entries",
        "storage_field": "remark",
        "title": "调整分录M2-3",
        "headers": _ADJ_HEADERS,
        "field_keys": _ADJ_KEYS,
        "guidance": ["M2-3 编制说明", "", "记录审计调整分录（AJE）和重分类分录（RJE）。"],
    },
}

router = create_cycle_import_export_router(
    tag="m2-import-export",
    api_prefix="m2",
    specs=_SPECS,
    storage_field="remark",
)
