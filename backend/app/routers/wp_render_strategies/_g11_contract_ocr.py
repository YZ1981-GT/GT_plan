...
POST /api/workpapers/{wp_id}/g11/contract-ocr
...
_EMPTY = {k: (0 if k == "creditAmount" else "") for k in _VOUCHER_FIELDS}


class G11VoucherOcrResponse(BaseModel):
    attachment_id: str
    ocr_text: str
...
def _resp(
    ...
    *,
    reused: bool = False,
    written_back: bool = False,
) -> G11VoucherOcrResponse:
    return G11VoucherOcrResponse(
...
@router.post("/api/workpapers/{wp_id}/g11/contract-ocr", response_model=G11VoucherOcrResponse)
async def g11_contract_ocr(
    wp_id: str,
    ...
    try:
        ...
        logger.warning("G11 voucher OCR failed: %s", e)
    ...
    try:
        ...
        logger.warning("G11 voucher LLM extract failed: %s", e)
