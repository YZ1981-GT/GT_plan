...
- G6-13 预期信用损失计量测试：方法评价、组合划分及PD/LGD或损失率测算
- G6-15 凭证检查表：样本计划、本期发生额、期后处置新增及六项逐笔核对

其他债权投资（FVOCI-Debt）ECL减值的特殊性：
1. 减值准备按摊余成本口径计算（非公允价值口径），与AC类(G4)方法一致
2. 减值损失在利润表确认，但不减少账面金额（通过OCI调整）
3. 三阶段判定优先级：已发生信用减值(Stage3) > 信用风险显著增加且未适用较低信用风险豁免(Stage2) > 未显著增加或适用低风险豁免(Stage1)
4. 较低信用风险豁免须三项同时满足；即使存在SICR信号，适用豁免时仍可归入Stage1
5. ECL公式链：③=①×② / ⑥=⑤×②A+①×(②A-②) / ⑦=①+⑤ / ⑧=③+⑥ / ⑨=⑦-⑧
...
_SECTION_PROMPTS: dict[str, str] = {
    "stage-conclusion": (
        ...
        "3. 较低信用风险豁免适用是否充分（三项同时满足）\n"
        "4. 对已发生信用减值事件(Stage3)的识别是否充分\n"
        "5. 企业划分阶段与审计判断阶段的一致性分析\n"
        "6. 对不一致项目的差异原因说明是否合理\n"
        "7. 各阶段分布(S1/S2/S3)数量及金额的合理性\n"
        "8. 综合评价三阶段划分对ECL计提基数的影响"
    ),
    ...
    "voucher-conclusion": (
        ...
        "1. 测试总体、特定项目、抽样方法和样本量是否合理\n"
        "2. 本期发生额及期后处置、新增样本的借贷方向与截止是否正确\n"
        "3. 原始凭证完整性、授权批准和账务处理是否适当\n"
        "4. 初始成本、利息计算及公允价值计量是否符合准则要求\n"
        "5. 减值金额与G6-12、转回核销事项与G6-14是否一致，OCI处理是否正确\n"
        "6. 借方及贷方检查比例是否充分（不要求抽样集合借贷合计相等）\n"
        "7. 异常样本的性质、金额、处理结果及对总体结论的影响"
    ),
}
