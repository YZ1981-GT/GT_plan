...
科目映射链路（report_config DB 实证，四准则一致）：
  IS-015 公允价值变动收益 = TB('6101','本期发生额')
  标准科目 6101 公允价值变动收益（借贷双方/损益类）
"""
...
from app.services.four_table.g_cycle_specs import G13_SPEC
...
#: G13 科目定位改走**语义驱动**（单一真源 `four_table/g_cycle_specs.G13_SPEC`）。
G13_ACCOUNT_SPEC = G13_SPEC

_ADJUDICATED_ITEM_ID = "G13-1-adjudicated-amount"

G13_SHEETS = [
    {"sheet_name": "底稿目录", "component_type": "g13-fair-value-changes"},
    {"sheet_name": "公允价值变动收益实质性程序表G13A", "component_type": "g13-fair-value-changes"},
    {"sheet_name": "审定表G13-1", "component_type": "g13-fair-value-changes"},
    {"sheet_name": "附注披露信息（上市公司）", "component_type": "g13-fair-value-changes"},
    {"sheet_name": "附注披露信息（国企）", "component_type": "g13-fair-value-changes"},
    {"sheet_name": "明细表G13-2", "component_type": "g13-fair-value-changes"},
    {"sheet_name": "调整分录汇总G13-3", "component_type": "g13-fair-value-changes"},
]
...
async def _fetch_tb_data(ctx: RenderContext) -> dict:
    """取 6101 公允价值变动收益的四表数据。"""
    result: dict = {"tb_values": {}, "tb_source_codes": {}, "adjudication_prefill": {}}

    try:
        accounts = await resolve_semantic_accounts(ctx, G13_ACCOUNT_SPEC)
        ...
        logger.warning("G13 TB fetch failed: %s", e)
...
async def render(ctx: RenderContext) -> dict | None:
    ...
    try:
        ...
        logger.warning("G13 render failed: %s", e)
    ...
    try:
        ...
        logger.warning("G13 render: project context failed: %s", e)
    ...
    return {
        "component_type": "g13-fair-value-changes",
        "account_code": "6101",
        "prefix": "G13",
