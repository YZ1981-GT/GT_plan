# -*- coding: utf-8 -*-
"""生成 / 校验 Task 43 G7 两级动态表 pilot 的 per-entry contract 磁盘文件。

spec: workpaper-html-onlyoffice-bidirectional-writeback-closure · Wave 4 Task 43

payload 的**唯一真源**是
:func:`app.services.workpaper_sync.pilot_g7_two_level_dynamic.build_contract_payload`。
本脚本只负责序列化落盘与 `--check`，不在这里第二次拼字段 —— 复制一份的后果是任一侧被短路
都不改变行为（变异检验判 GREEN）。

用法（仓库根）::

    py -3 backend/scripts/gen/generate_pilot_g7_two_level_dynamic_contract.py --check
    py -3 backend/scripts/gen/generate_pilot_g7_two_level_dynamic_contract.py --apply
"""
...
def observe_template_resolution() -> Any:
    ...
    判定逻辑在 `pilot_g7_two_level_dynamic` 的 assert_* 函数，这里只取事实 ——
    一份观测器同时喂给本脚本与 `test_task43_g7_two_level_dynamic_pilot.py`，不抄第二份。
...
def _render(payload: dict) -> bytes:
    ...
    而基于 `read_text()` 的字节断言因 universal newlines 两头都转、查不出来。
    """
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Task 43 G7 pilot 契约生成/校验")
    ...
    #    entry 一漂移（宿主消失 / 变成 parent duplicate / wp_code 变了 / matcher 域被别的
    #    entry 抢占 / 配置指到别的工作簿 / finder 开始回退 / `G7L` 变成真码）就意味着这些
    #    单元格属于另一份底稿。
    entry = P.assert_pilot_entry_selectable(resolution=observe_template_resolution())
    assert entry["entry_id"] == P.PILOT_ENTRY_ID
    # 四边真源第 2/4 边也必须仍然成立（seed 描述同一份工作簿、渲染层真的两级渲染）。
    P.assert_managed_tables_are_not_exempted()
    P.assert_render_layer_renders_two_level()
    ...
    matrix = next(t for t in contract.sheets[0].tables if t.table_key == P.MATRIX_TABLE_KEY)
    record = next(t for t in contract.sheets[0].tables if t.table_key == P.RECORD_TABLE_KEY)
    summary = (
        f"bytes={len(blob)} fields={len(contract.all_fields())} "
        f"protected={len(contract.protected_field_keys())} "
        f"matrix_header_rows={matrix.header_rows} "
        f"dynamic_identity={matrix.dynamic_columns.identity if matrix.dynamic_columns else None} "
        f"record_formula_mask={list(record.formula_mask)} "
