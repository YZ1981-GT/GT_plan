# -*- coding: utf-8 -*-
"""生成 Task 61 probe 注册表数据文件（`backend/data/workpaper_task61_word_pilot_gate_probes.json`）。

spec: workpaper-html-onlyoffice-bidirectional-writeback-closure · Wave 5 Task 61
Requirements: 12.3, 12.10, 14.7, 14.9
...
🔴 投影里**不含**任何 per-run 结果：`probe_registry_payload()` 只读 tasks.md / design.md /
生产 oracle 表 / Task 44 的门，不碰真实库与环境 ⇒ 本文件的字节不随"今天有没有跑 OO"变化。
这是它能当 stale 判据的前提（AC 14.16 / Property 71）。

用法（仓库根，Windows PowerShell）::

    .\\.venv\\Scripts\\python.exe backend/scripts/gen/generate_workpaper_task61_word_pilot_probe_registry.py --apply
    .\\.venv\\Scripts\\python.exe backend/scripts/gen/generate_workpaper_task61_word_pilot_probe_registry.py --check
"""
...
def load_gate() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("_task61_gate", _GATE_PY)
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = (
        f"probes={len(payload['probes'])} per_entry={payload['per_entry_probe_count']} "
        f"scenario={payload['scenario_probe_count']} gate={payload['gate_probe_count']} "
        f"rows={payload['total_probe_rows']} "
