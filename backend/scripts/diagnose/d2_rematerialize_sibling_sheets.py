# -*- coding: utf-8 -*-
"""D2 multi-sheet rematerialize：把已发布 representation 升级到 d22+d23+d21 契约 bundle。
...
用法::
    ...
    .\\.venv\\Scripts\\python.exe backend/scripts/diagnose/d2_rematerialize_sibling_sheets.py --check
    .\\.venv\\Scripts\\python.exe backend/scripts/diagnose/d2_rematerialize_sibling_sheets.py --apply
"""
...
async def _read_store_map(
    ...
    """读 D2 的全部 store item（D2-2 rows + D2-3 三键 + D2-1 per-cell）。"""
    out: dict[str, str] = {}
...
async def run(*, apply: bool, force: bool = False) -> dict[str, Any]:
    ...
    try:
        async with Session() as session:
            ...
            desired = await _desired_bundle(session, contract_id=D2.PILOT_ADAPTER_ID)
            report["desired_bundle"] = desired
            if current_sha == desired["sha256"] and not force:
                report["status"] = "already_on_desired_bundle"
                return report

            store_item_ids = tuple(D2.all_store_item_ids())
            ...
            with tempfile.TemporaryDirectory(prefix="d2-remat-") as tmp:
