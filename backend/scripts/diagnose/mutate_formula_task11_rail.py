"""Mutations for formula Task 11 / G-RAIL consumption."""
...
CASES = [
    Case(
        id="T11-M1-invisible-occupies-slot",
        ...
        wants=("visible=false / denied / epoch-stale never occupy trigger slots",),
    ),
]
...
def run_vitest() -> tuple[int, str]:
    proc = subprocess.run(
        "npx vitest run src/shell/formula/__tests__/rightRailArbiter.spec.ts",
        cwd=str(FE),
        capture_output=True,
        env=ENV,
        shell=True,
