# -*- coding: utf-8 -*-
"""净化 D5 应收款项融资权威模板的孤儿外链（过 OOXML external_relationships 门）。

G5-1 Phase 5 · 对齐 DEC-11（B60/D7/D3/D6 净化范式）。**4 个 externalLink，0 个 [n] 公式格**
（同 D3/D6，无需中和公式）。受管 sheet = `应收款项融资明细表D5-2`（17 列 A-Q）。

用法（仓库根，PowerShell）：
  校验预演: & .venv/Scripts/python.exe backend/scripts/fix/sanitize_d5_template_external_links.py --check
  真净化:   & .venv/Scripts/python.exe backend/scripts/fix/sanitize_d5_template_external_links.py --apply

🔴 判成败一律用 openpyxl 打开 + D5-2 逐格 diff + 门判据，不看退出码。
"""
...
TEMPLATE = _REPO / "backend" / "wp_templates" / "D" / "D5 应收款项融资.xlsx"
...
_MANAGED_SHEET = "应收款项融资明细表D5-2"


def _strip_external_from_content_types(xml: str) -> str:
    return re.sub(
        r'<Override PartName="/xl/externalLinks/externalLink\d+\.xml"[^>]*/>', "", xml
    )


def _strip_external_from_workbook_rels(xml: str) -> str:
    return re.sub(
        r'<Relationship [^>]*Target="externalLinks/externalLink\d+\.xml"[^>]*/>', "", xml
    )
...
def main() -> int:
    ...
    print(f"D5-2 managed cells before={len(before_snap)} after={len(after_snap)} diffs={len(diffs)}")
    for k, b, a in diffs[:30]:
        print(f"  {k}: {b!r} -> {a!r}")
    print(f"D5-2 merged same={before_merged == after_merged}")
