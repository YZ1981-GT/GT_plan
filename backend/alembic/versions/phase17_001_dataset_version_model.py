...
_DATASET_STATUS = PgEnum("staged", "active", "superseded", "failed", "rolled_back", name="dataset_status", create_type=False)
_JOB_STATUS = PgEnum("pending", "queued", "running", "validating", "writing", "activating", "completed", "failed", "canceled", "timed_out", name="job_status", create_type=False)
_ARTIFACT_STATUS = PgEnum("active", "expired", "consumed", name="artifact_status", create_type=False)
_ACTIVATION_TYPE = PgEnum("activate", "rollback", name="activation_type", create_type=False)


def upgrade() -> None:
    # 枚举类型（PG 不支持 CREATE TYPE IF NOT EXISTS，用 DO 块 + 异常吞掉）
    op.execute(
        """
        DO $$ BEGIN
            CREATE TYPE dataset_status AS ENUM ('staged','active','superseded','failed','rolled_back');
    ...
    op.execute(
        """
        DO $$ BEGIN
            CREATE TYPE job_status AS ENUM ('pending','queued','running','validating','writing','activating','completed','failed','canceled','timed_out');
        EXCEPTION WHEN duplicate_object THEN null; END $$;
        """
    )
    op.execute(
        """
        DO $$ BEGIN
            CREATE TYPE artifact_status AS ENUM ('active','expired','consumed');
        EXCEPTION WHEN duplicate_object THEN null; END $$;
        """
    )
    op.execute(
        """
        DO $$ BEGIN
            CREATE TYPE activation_type AS ENUM ('activate','rollback');
        EXCEPTION WHEN duplicate_object THEN null; END $$;
        """
    )
    ...
    op.create_table(
        ...
        sa.Column("status", _ARTIFACT_STATUS, server_default="active", nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("project_id", UUID(as_uuid=True), sa.ForeignKey("projects.id"), nullable=False),
        sa.Column("year", sa.Integer, nullable=False),
        sa.Column("status", _DATASET_STATUS, server_default="staged", nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("action", _ACTIVATION_TYPE, nullable=False),
        sa.Column("previous_dataset_id", UUID(as_uuid=True), sa.ForeignKey("ledger_datasets.id"), nullable=True),
