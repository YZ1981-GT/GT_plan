...
class TestNLCommandService:
    ...
    @pytest.mark.asyncio
    async def test_parse_data_command(self):
        """测试解析数据命令"""
        from app.services.nl_command_service import NLCommandService
        ...
        result = await service.parse_command(
            project_id=uuid4(),
            command_text="显示本月收入数据",
        )
        assert result["type"] in ["data_query", "unknown"]

    @pytest.mark.asyncio
    async def test_parse_workpaper_command(self):
        """测试解析底稿命令"""
        from app.services.nl_command_service import NLCommandService
        ...
        result = await service.parse_command(
            project_id=uuid4(),
            command_text="打开货币资金底稿",
        )
        assert result["type"] == "workpaper"

    @pytest.mark.asyncio
    async def test_parse_ai_command(self):
        """测试解析 AI 命令"""
        from app.services.nl_command_service import NLCommandService
        ...
        result = await service.parse_command(
            project_id=uuid4(),
            command_text="用 AI 分析这份合同",
        )
        assert result["type"] == "ai_action"

        result = await service.parse_command(
            project_id=uuid4(),
            command_text="OCR 识别这张发票",
        )
        assert result["type"] == "ai_action"
...
class TestConfirmationAIService:
    ...
    @pytest.mark.asyncio
    async def test_audit_confirmation_structure(self):
        """测试函证审核结构"""
        ...
        assert hasattr(service, 'audit_confirmation')

    @pytest.mark.asyncio
    async def test_audit_discrepancies(self):
        """测试审核差异"""
        from app.services.confirmation_ai_service import ConfirmationAIService
        ...
        # 无差异
        result = await service.audit_discrepancies(
            book_value=1000000,
            confirmation_value=1000000,
            tolerance=1000,
        )
...
class TestAIIntegration:
    ...
    @pytest.mark.asyncio
    async def test_evidence_chain_lifecycle(self):
        """测试证据链完整生命周期"""
        from app.services.evidence_chain_service import EvidenceChainService
        ...
        assert chain_data["chain_name"] == "测试证据链"

        # 添加证据项
        items = [
            {"evidence_name": "采购合同", "evidence_type": "documentary"},
            {"evidence_name": "发票", "evidence_type": "external"},
        ]
