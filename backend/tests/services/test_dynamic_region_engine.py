"""Sprint A.2 Batch 1 — dynamic_region_engine unit tests.

A.2.1 row expansion / A.2.2 column expansion + multi-header /
A.2.7 label auto-fill + purity/idempotence.
...
def _col(cid, label="", header_path=None, col_type="fixed", **extra):
    out = {
        "id": cid,
        "label": label,
        "header_path": header_path or [label or cid],
        "col_type": col_type,
    }
    out.update(extra)
    return out
...
def _rr(name, start, end, source="manual"):
    return {"name": name, "axis": "row", "start_idx": start,
            "end_idx": end, "dynamic_source": source}


def _cr(name, start, end, source="manual"):
    return {"name": name, "axis": "column", "start_idx": start,
            "end_idx": end, "dynamic_source": source}
...
class TestRowExpansion:
    ...
    def test_empty_region_no_change(self):
        ...
        result = expand_dynamic_rows(td, ctx={})
        assert result["rows"][1]["row_type"] == "dynamic_anchor"
    ...
    def test_single_anchor_explode_3_items(self):
        ...
        ctx = {"manual": {"客户": [
            ...
            {"label": "B", "values": [300, 400]},
            {"label": "C", "values": [500, 600]},
        ]}}
        result = expand_dynamic_rows(td, ctx=ctx)
        rows = result["rows"]
        assert len(rows) == 6
        assert rows[1]["label"] == "A" and rows[1]["values"] == [100, 200]
        assert rows[1]["row_type"] == "dynamic_data"
        assert rows[3]["label"] == "C"
    ...
    def test_dynamic_source_manual_top_level_items(self):
        ...
        ctx = {"items": [{"label": "a", "values": [1]},
                         {"label": "b", "values": [2]}]}
        result = expand_dynamic_rows(td, ctx=ctx)
        assert [r["label"] for r in result["rows"][:2]] == ["a", "b"]

    def test_region_with_subtotal_preserved(self):
        td = {"rows": [
            _row("c", row_type="header_label"),
            _row("", values=[None, None], row_type="dynamic_anchor"),
            _row("sub", values=[0, 0], row_type="subtotal", is_total=True),
            _row("tot", values=[0, 0], row_type="total", is_total=True),
        ], "_dynamic_regions": [_rr("c", 1, 1)]}
        ctx = {"manual": {"c": [{"label": "A", "values": [1, 2]}]}}
        result = expand_dynamic_rows(td, ctx=ctx)
        assert [r["row_type"] for r in result["rows"]] == [
            "header_label", "dynamic_data", "dynamic_marker_end",
            "subtotal", "total",
        ]
        assert result["rows"][3]["label"] == "sub"
        assert result["rows"][4]["label"] == "tot"


# A.2.2 column expansion --------------------------------------------------


class TestColumnExpansion:

    def test_empty_column_region_no_change(self):
        td = {"_columns_meta": [_col("c1", "p"), _col("c2", "a")],
    ...
    def test_add_2_columns_extends_meta_and_values(self):
        td = {"_columns_meta": [_col("c_label", "p"), _col("c_amount", "a")],
              "rows": [_row("X", values=["X", 10]),
                       _row("Y", values=["Y", 20])],
              "_dynamic_regions": [_cr("ext", 2, 2)]}
        ctx = {"columns": {"ext": [
            _col("c_new1", "n1", col_type="dynamic"),
            _col("c_new2", "n2", col_type="dynamic"),
        ]}}
    ...
    def test_multi_level_header_preserved(self):
        td = {"_columns_meta": [_col("c_label", "p")],
              "rows": [_row("A", values=["A"])],
              "_dynamic_regions": [_cr("y", 1, 1)]}
        ctx = {"columns": {"y": [
            {"id": "c_2024_end", "label": "end",
             "header_path": ["yr", "end"], "col_type": "dynamic"},
            {"id": "c_2024_start", "label": "start",
             "header_path": ["yr", "start"], "col_type": "dynamic"},
        ]}}
        result = expand_dynamic_columns(td, ctx=ctx)
        assert result["_columns_meta"][1]["header_path"] == ["yr", "end"]
        assert result["_columns_meta"][2]["header_path"] == ["yr", "start"]

    def test_column_id_uniqueness_ci3(self):
        td = {"_columns_meta": [_col("c_label", "p"), _col("c_dup", "d")],
              "rows": [_row("A", values=["A", 1])],
              "_dynamic_regions": [_cr("z", 2, 2)]}
        ctx = {"columns": {"z": [
            _col("c_dup", "d2", col_type="dynamic"),
            _col("c_unique", "u", col_type="dynamic"),
        ]}}
        result = expand_dynamic_columns(td, ctx=ctx)
...
# A.2.7 label auto-fill ---------------------------------------------------


class TestAutoPopulateLabels:

    def test_empty_label_filled_from_ctx(self):
        td = {"rows": [_row("", values=[v], row_type="dynamic_data")
                       for v in (1, 2, 3)],
              "_dynamic_regions": [_rr("c", 0, 2, source="aux_balance")]}
        ctx = {"labels": {"c": ["A", "B", "C"]}}
        result = auto_populate_row_labels(td, ctx=ctx)
        assert [r["label"] for r in result["rows"]] == ["A", "B", "C"]

    def test_existing_label_preserved(self):
        td = {"rows": [
            _row("kept", values=[1], row_type="dynamic_data"),
            _row("", values=[2], row_type="dynamic_data"),
        ], "_dynamic_regions": [_rr("x", 0, 1)]}
        ctx = {"labels": {"x": ["NA", "NB"]}}
        result = auto_populate_row_labels(td, ctx=ctx)
        assert result["rows"][0]["label"] == "kept"
        assert result["rows"][1]["label"] == "NA"
    ...
    def test_mismatched_count_partial_fill(self, caplog):
        ...
        with caplog.at_level(
            logging.WARNING,
            logger="app.services.dynamic_region_engine",
        ):
            result = auto_populate_row_labels(td, ctx=ctx)
...
class TestPurityAndIdempotent:
    ...
    def test_input_unchanged_after_call_pure(self):
        td = {"rows": [
            _row("", values=[None], row_type="dynamic_anchor"),
            _row("tot", values=[0], row_type="total", is_total=True),
        ], "_dynamic_regions": [_rr("x", 0, 0)]}
    ...
    def test_round_trip_expand_reload_expand(self):
        td = {"_columns_meta": [_col("c_label", "p"), _col("c_amt", "a")],
              "rows": [
                  _row("", values=[None, None], row_type="dynamic_anchor"),
                  _row("tot", values=[0, 0], row_type="total",
                       is_total=True),
              ], "_dynamic_regions": [_rr("x", 0, 0)]}
        ctx = {"manual": {"x": [
            {"label": "A", "values": [1, 2]},
