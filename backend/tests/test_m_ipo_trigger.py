"""M-F8 IPO 专项触发器（占位实现）— 单元测试

测试范围:
- _IPO_CONFIG 注册表含 'M2' 入口（codes=[]）
- _ensure_ipo_loaded(prefix='M2') 不抛异常，返回 empty result
- D/F/H/J/K/L 既有 IPO 触发器回归

对应 spec: workpaper-m-equity-cycle M-F8 / Sprint 3 Task 3.1
"""
...
def test_m2_ipo_config_registered():
    """_IPO_CONFIG 必须含 M2 入口"""
    from app.services.wp_template_init_service import _IPO_CONFIG

    assert "M2" in _IPO_CONFIG
    assert _IPO_CONFIG["M2"]["audit_cycle"] == "M"
    assert "codes" in _IPO_CONFIG["M2"]
    assert _IPO_CONFIG["M2"]["codes"] == []


def test_m2_ipo_config_has_name_prefix():
    """M2 入口必须有 name_prefix 字段"""
    from app.services.wp_template_init_service import _IPO_CONFIG

    assert "name_prefix" in _IPO_CONFIG["M2"]
    assert isinstance(_IPO_CONFIG["M2"]["name_prefix"], str)
    assert len(_IPO_CONFIG["M2"]["name_prefix"]) > 0


# ---- _ensure_ipo_loaded(prefix='M2') 行为测试 -------------------------------


@pytest.mark.asyncio
async def test_ensure_ipo_loaded_m2_empty_result():
    """M2 codes=[]，_ensure_ipo_loaded 返回 empty result（不抛异常）"""
...
@pytest.mark.asyncio
async def test_ensure_ipo_loaded_m2_lowercase_prefix():
    """大小写不敏感：'m2' 也能命中 M2 配置"""
    from app.services.wp_template_init_service import _ensure_ipo_loaded
...
def test_d4_ipo_config_unchanged():
    """D spec 回归：_IPO_CONFIG['D4'] 不受 M2 添加影响"""
...
def test_f2_ipo_config_unchanged():
    """F spec 回归：_IPO_CONFIG['F2'] 不受 M2 添加影响"""
...
def test_h1_ipo_config_unchanged():
    """H spec 回归：_IPO_CONFIG['H1'] 不受 M2 添加影响"""
    from app.services.wp_template_init_service import _IPO_CONFIG
...
def test_j1_ipo_config_unchanged():
    """J spec 回归：_IPO_CONFIG['J1'] 不受 M2 添加影响"""
    from app.services.wp_template_init_service import _IPO_CONFIG
...
def test_k8_ipo_config_unchanged():
    """K spec 回归：_IPO_CONFIG['K8'] 不受 M2 添加影响"""
    from app.services.wp_template_init_service import _IPO_CONFIG
...
def test_l1_ipo_config_unchanged():
    """L spec 回归：_IPO_CONFIG['L1'] 不受 M2 添加影响"""
    from app.services.wp_template_init_service import _IPO_CONFIG
...
def test_ipo_config_total_prefixes():
    """_IPO_CONFIG 应含 D4 + F2 + H1 + J1 + K8 + L1 + M2 = 7 个 prefix"""
    from app.services.wp_template_init_service import _IPO_CONFIG

    assert len(_IPO_CONFIG) >= 7
    assert {"D4", "F2", "H1", "J1", "K8", "L1", "M2"}.issubset(set(_IPO_CONFIG.keys()))
