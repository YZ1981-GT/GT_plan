...
# 合法 formula_type 枚举
#
# 🔴 `PLACEHOLDER` 是平台既有的一等取值（`prefill_formula_mapping.json` 内 10 处：
#    N1「税会差异汇总」/ E1 数字货币两条 / N3 递延负债本期变动 / H0-1 两条），
#    语义 = 「该单元格的值无法用单一科目码的公式表达，取数真源在别处」。
#    本白名单原来漏了它 —— 2026-08-04 H0 公式预设按九品种口径改成 PLACEHOLDER 后
#    本条打红，属白名单不完整（不是数据错），已补齐。
VALID_FORMULA_TYPES = {
    "TB", "TB_SUM", "ADJ", "PREV", "WP", "AUX", "LEDGER", "LEDGER_DETAIL",
    "COUNT_LEDGER", "NOTE", "TB_AUX", "SUM_TB", "PLACEHOLDER",
}
...
# TB 公式合法科目编码（H 循环涉及的科目）
#
# 🔴 2026-08-04 按 `account_chart` 实测校正 —— 原清单的注释与真实科目码大面积不符，
#    导致 H3-2/H8-2 里**正确**的科目码被判「非 H 循环科目」（本条长期为红）：
#      · 投资性房地产累计折旧/累计摊销/减值准备 真实码是 1525/1526/1527，
#        原清单写的 1522/1523 在全库两张科目表里都不存在；
#      · 1621/1622 是**生产性生物资产**及其累计折旧（H7），不是使用权资产；
#        使用权资产族真实码 1641/1642/1643（部分客户用 1651/1652）；
#      · 租赁负债族真实码 2601/2602（部分客户用 2651），原清单写的 2802/2803 不存在；
#      · 1811 是**递延所得税资产**（N1），与「长期待摊费用」无关，仅 1801 是。
#    保留原有条目以免打断既有通过用例，只补齐真实码。
VALID_H_ACCOUNT_CODES = {
    "1601", "1602", "1603", "1606",   # 固定资产/累计折旧/减值准备/固定资产清理
    "1604", "1605",                   # 在建工程/工程物资
    "1521", "1525", "1526", "1527",   # 投资性房地产/累计折旧/累计摊销/减值准备
    "1522", "1523",                   # （历史遗留声明，全库无此码；保留不删以免打断既有用例）
    "1631", "1632", "1633",           # 油气资产/累计折耗/减值准备
    "1621", "1622", "1623",           # 生产性生物资产/累计折旧/减值准备
    "1641", "1642", "1643",           # 使用权资产/累计折旧/减值准备（客户变体 1651/1652）
    "1651", "1652",
    "2601", "2602",                   # 租赁负债/未确认融资费用（客户变体 2651）
    "2651",
    "2802", "2803",                   # （历史遗留声明，全库无此码；保留不删）
    "2202",                           # 应付账款（部分场景）
    "1801", "1811",                   # 长期待摊费用 / 递延所得税资产（跨循环引用）
    "6115",                           # 资产处置损益
}
