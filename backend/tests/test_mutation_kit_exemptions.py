...
def test_active_spec_scan_mechanism_is_healthy() -> None:
    """扫描机制必须有效 —— 失效时会把**所有**豁免误判为失效（假红）。

    🔴 **判据既不用「数量 >= N」也不用「结果非空」**：两者都是「把当时的环境值当基线」，
    只是阈值不同。演进史即证据 ——
    第一版 `len(active) >= 3` 于 2026-08-16 被打红（当时 active 只剩 2 个）；
    降为 `assert active` 后**同日再次被打红**：全部 spec 归档完毕，active 真的是 0。
    「全部归档」是合法终态，不是扫描失效，故任何基于结果规模的断言都必然在此刻假红。

    改用**与规模无关的结构判据**，三条各有独立区分能力：
    1. `SPECS_DIR` 可枚举 —— 常量路径写错（如 `.kiro/spec`）即红
    2. `_archive` 实际存在 —— 证明确有内容可扫，排除「目录空壳」
    3. `_archive` 不出现在结果里 —— 下划线前缀排除逻辑生效，坏掉即红

    active 为 0 时本测试仍绿；而上述任一环节坏掉时仍红。
    """
    assert SPECS_DIR.is_dir(), f"spec 根目录不可枚举 ⇒ 常量路径写错：{SPECS_DIR}"
    active = active_spec_dirs()
    archive = SPECS_DIR / "_archive"
    assert archive.is_dir(), (
        f"归档区 {archive} 不存在 ⇒ 无从判断扫描是否真的读到了内容"
    )
    assert "_archive" not in active, (
        f"归档区被当成 active spec ⇒ 下划线前缀排除逻辑失效。实扫：{sorted(active)}"
    )
...
def test_reverse_selfcheck_stale_detection_passes_active_spec() -> None:
    ...
    🔴 `find_stale` 是纯函数，自检**不得依赖真实环境里恰好存在 active spec** ——
    原实现 `next(iter(sorted(active)))` 在全部 spec 归档后抛 StopIteration（假红）。
    真实 active 集合有内容时优先用它（更贴近实际），为空则构造，两种路径下判据同样成立。
    """
    active = active_spec_dirs()
    picked = next(iter(sorted(active)), None)
    if picked is None:
        # active 区可以合法为空（全部归档）；纯函数自检与环境无关，构造一个即可
        picked = "constructed-active-spec-for-selfcheck"
        active = {picked}
    entries = [{
        "script": "backend/scripts/diagnose/mutate_e_cycle_guards.py",
        "spec": picked,
