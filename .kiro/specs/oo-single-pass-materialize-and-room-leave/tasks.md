...
- [x] 2. binding 顺序依赖调查（**结论进 design.md 附录，不写代码**）
...
- [x] 3. 实现单趟写入路径（一次 load → 全部 binding → 一次 save）
...
- [x] 8. materialize / extract / verify 共用一个 `workbook_read_scope()`（P4）
...
- [x] 10. 复用命中/未命中进 metrics + 「连续两次必命中」守卫（P6）
