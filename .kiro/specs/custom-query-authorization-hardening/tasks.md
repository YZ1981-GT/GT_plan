...
Bugfix for custom-query authorization hardening (§5.12 + §5.13). Closes IDOR read on execute/batch-execute and cross-project write on cell-writeback by adding project-level visibility/edit permission checks using existing auth infrastructure (`get_visible_project_ids` + `require_project_access`).

## Tasks

- [ ] 1. Write bug condition exploration test (IDOR read + cross-project write)
  - **Property 1: Bug Condition** — Custom Query Authorization IDOR & Cross-Project Write
  ...
  - **Scoped PBT Approach**: Scope the property to concrete failing cases:
    - (a) User with visible_projects={P1} calls execute with project_id=P2 → currently returns 200+data (should 403)
    - (b) User with visible_projects={P1} calls batch-execute with project_id=P2 → currently returns 200 (should 403)
    - (c) User without edit permission on project_id calls cell-writeback (module=workpaper) → currently no auth check (should 403)
    - (d) Projects P1,P2 both have wp_code="D2-1", user in P1 submits writeback without project_id filter → LIMIT 1 may hit P2's workpaper (should only hit P1 or 404)
  ...
  - **EXPECTED OUTCOME**: Test FAILS (this is correct — it proves the bug exists: endpoints return 200 instead of 403, writes succeed cross-project)
...
- [ ] 2. Write preservation property tests (BEFORE implementing fix)
  - **Property 2: Preservation** — Legitimate Same-Project Access Unchanged
  - **IMPORTANT**: Follow observation-first methodology
  - **Observe on UNFIXED code** (cases where isBugCondition returns false):
    - (a) Admin user calls execute with any project_id → returns 200 + query results (admin sees all projects via get_visible_project_ids)
    - (b) User with project membership calls execute on their visible project → returns normal query results
    - (c) Same user, same visible project, repeated query → returns X-Cache: HIT header (Redis short TTL cache behavior)
    - (d) User with edit permission calls cell-writeback on workpaper with stale X-File-Opened-At → returns 409 {conflict: true} (optimistic lock)
    - (e) User with edit permission calls cell-writeback successfully → returns {success: true, updated_at: ...}, writes parsed_data JSONB, marks prefill_stale, syncs xlsx cache
    - (f) Partner role accesses any project → visible all (get_visible_project_ids returns all for partner)
  - Write property-based tests (Hypothesis) capturing observed behavior:
    - PBT: for all (user, project) where project ∈ get_visible_project_ids(user), execute returns same result structure as before fix
    ...
    - PBT: admin/partner for random project_id → always passes visibility check
  - Verify tests PASS on UNFIXED code (confirms baseline behavior to preserve)
  - **EXPECTED OUTCOME**: Tests PASS (this confirms baseline behavior to preserve)
  - Mark task complete when tests are written, run, and passing on unfixed code
  - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7_

- [ ] 3. Fix for custom-query authorization hardening (§5.12 + §5.13)

  - [ ] 3.1 Add project visibility check to execute_query (BEFORE cache read)
  ...
  - [ ] 3.2 Add project visibility check to batch_execute
  ...
  - [ ] 3.3 Add project_id field to CellWritebackRequest schema
    - In `backend/app/routers/custom_query.py`, locate `CellWritebackRequest` Pydantic model
    - Add `project_id: str` as required field (breaking change — frontend must send it)
    - Missing project_id → automatic 422 validation error from Pydantic
    - _Bug_Condition: isBugCondition_5_13 — workpaper lookup previously ignored project_id_
    - _Expected_Behavior: Schema enforces project_id presence; without it, 422_
    - _Requirements: 2.5_

  - [ ] 3.4 Replace role whitelist with unified project edit permission check in cell_writeback
  ...
  - [ ] 3.5 Add project_id filter to workpaper lookup query in cell_writeback
    ...
    - If no row returned → raise HTTPException(404, detail="Working paper not found in this project")
    - This eliminates cross-project wp_code collision (multiple projects with same D2-1)
    - _Bug_Condition: isBugCondition_5_13 — lookup_ignores_project_id previously true_
  ...
  - [ ] 3.6 Add belt+suspenders project_id validation in snapshot_writer._write_workpaper_cell
    ...
    - This is defense-in-depth — even if router check is bypassed, writer rejects cross-project writes
  ...
  - [ ] 3.7 Update frontend to pass project_id in cell-writeback requests
    - Locate the frontend API call for cell-writeback (likely in `frontend/src/` — search for `cell-writeback` or `cellWriteback`)
  ...
  - [ ] 3.8 Verify bug condition exploration test now passes
    - **Property 1: Expected Behavior** — IDOR & Cross-Project Write Blocked
...
- Implementation tasks 3.1–3.7 are ordered by dependency: schema change (3.3) before router changes (3.4, 3.5) that use `body.project_id`; frontend update (3.7) after schema change.
- Task 3.6 (snapshot_writer belt+suspenders) is defense-in-depth and can be done in parallel with 3.4/3.5.
- All auth checks reuse existing `deps.py` infrastructure — no new custom auth logic introduced.
