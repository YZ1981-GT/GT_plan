...
**统计**：Active **17**（2026-09-22 逐目录实扫 `.kiro/specs/*/` 且**带 tasks.md** 口径；目录数 18，其中 2 个为空壳：`procedure-delegation-visibility-isolation` / `visibility-isolation-go-live-hardening`） / Archived **580**（`_archive/*/*` 实扫）
...
| spec | 进度 | 一句话 |
|------|------|--------|
| `d2-sync-coverage-via-row-table-engine` | **0/18** | **D2 受管覆盖扩容（1→4 张）+ 两处死代码清理**（2026-09-25 新建，三件套齐 0 diagnostics，Design-First 未实施）。是 `workpaper-sync-row-table-engine-and-d1-coverage` 的**消费方**（引擎/注册表/`AdjudicationSheetSpec`/四态状态机由它交付），Task 0 前置门 = 「D1 spec 框架层已入 HEAD」。<br>🔴 **最关键结构事实：D2 是三册模板，不是一册**（与 D1 单册 21 sheet / D4 单册 46 sheet 根本不同）。实测 `backend/wp_templates/D/` 有 `D2-1至D2-4  应收账款- 审定表明细表（Leap-常规程序）.xlsx`(11 sheets) / `D2-5  …分析程序.xlsx`(4) / `D2-6至D2-13  …检查.xlsx`(12)；而 **entry ↔ template blob 是 1:1**（`TEMPLATE_RELATIVE_PATH` 单路径）且 `generate_workpaper_sync_manifest._entry_id(document_type, source_file)` **从宿主文件路径派生** + `seen_entry_ids` 碰撞检查 ⇒ **一宿主恰一 entry**，而 D2 的 16 张 sheet 全在同一宿主 `GtD2AccountsReceivable.vue` 按 tab 分流 ⇒ **D2 只能覆盖第一册**。三个替代方案均否掉：改 `_entry_id` 派生规则（它是持久化键，`working_paper_sync_entry_state`/`workpaperSyncModeKey`/room key 全用它，改则既有状态全失配）/ 合册（权威模板是审计方法论产物且 `template_sha256=31e7992b…` 已冻结在契约）/ 现在建新宿主（跨前端路由+`htmlRendererRegistry`+manifest 生成+两条完整发布链，应单独立项）。<br>**第一册几何实测（openpyxl 直读）**：`审定表D2-1` 62行×13列 **311 公式**（密度 **38%**，主列 E43 I40 J39 K39）⇒ **逐格 mask**，走 `AdjudicationSheetSpec` · `明细表D2-2` 35行×39列 107 公式（已接，`d22-managed`）· `坏账准备明细表D2-3` 27行×14列 69 公式（主列 **E13 K11 N11**，计数≈数据行数 ⇒ **列向**）⇒ 行表引擎 · `调整分录汇总表D2-4` 25行×10列 **仅 6 公式** ⇒ 最简形态基线 · 另有 底稿目录 / D2A 程序表 / **附注披露×4** / GT_Custom **不接**。模板原始态**无任何 Excel Table**（Table 由 instrumentation 注入，正常）。<br>**5 条裁决**：E1 只扩第一册（后两册另立 `d2-analysis-and-inspection-sync-hosts`）· 🔴 **E2 已于 2026-09-25 复盘反转：D2-3 是三受管区**（实测 `useD2BadDebt.ts:64-68` 的 `CATEGORY_KEYS` dict = `D2-bd-individual-rows`/`D2-bd-aging-rows`/`D2-bd-customer-rows` **三个独立 store 键**，与 D1-4 同型；**首版写成「单受管区」并立了反向判据**，根因是首轮 grep 用 `^const \w+_KEY\s*=\s*'` 匹配不到 dict 字面量里的键值 ⇒ 据不完整事实下了相反裁决。连带三个后果首版全漏：①D2-3 成为**同 sheet 多受管区**须走兄弟 Table ref 位移路径、依赖 D1 spec 任务 24 先落 ②binding 序列 `1→2→3→4` 改为 **`1→4→5→6`**（受管 sheet 4 张 / 受管区 6 个）③三键有 **5 处下游消费方**（`useD2Adjudication.eclCrossValidation`/`useD2Ecl.d3BadDebtTotal`/`useD2WriteoffCheck.reversalConsistencyWarning`/`useD2DisclosureNote.badDebtSummary`+`importIndividualFromBadDebt`/`useD2CrossSheet`）零回归须覆盖，只验自身读回等值会放过「下游看不到回写」。**教训固化：store 键 grep 必须同时覆盖 `= '常量'` 与 `= { ...: '值' }` 两形态，宁可用按值匹配 `'D2-[a-z0-9-]+-rows'` 兜**）· E3 D2-1 以 `sections`(1 vs 3) + `row_mode`(`fixed_rows` vs `dynamic_identity`) 参数表达与 D1-1 的差异，**不得**在引擎加 `if is_d1`/`if is_d2`（会让 D1 spec 的框架层 AST 卡点打红）· **E4 删代码不删数据** · **E5 非受管 sheet 保持禁用不引入 legacy 假双向**（D1 落 `GtOnlyOfficeSheet` 那条 `migration_state=legacy_fake_bidirectional`、OO 改动不合并回 store 切回即丢；D2 现状直接禁用+中文原因**更正确**，不能为「对齐 D1」而退化）。<br>✅ **O5 已查明（不再待业务确认）**：`useD2VoucherCheck`（旧套）**全仓零消费方** = 死模块 310 行，前端实际挂载 `Enhanced`；真库 `D2-voucher-params`(1行87B `{"method":"随机","populationSize":0,…}`) / `D2-voucher-samples`(1行332B 全空骨架行，同 wp `e2c95d10`) 为**空骨架/默认值**，新套 `D2-vc-current-rows`/`-post-rows` **全库 0 行** ⇒ 删代码 + 保留读兼容。另删 `useD2Adjudication:182-188` 死降级路径（逐行读 `D2-detail-{i}-{field}`，`useD2Detail` 只写 `D2-detail-rows` ⇒ 零写入方恒返 0）。<br>🔴 **性能是一等约束不是附带项**：`D2-detail-rows` 真库 **4 行 / 3,061,466 字节**（平均 765KB/行）；D2-2 模板 **35 行** vs 真库 **1260 行** ⇒ 单张 materialize 要插 **1200+ 行**；离线剖析 729 行 **59.3s**（超线性），走统一 materialize 曾 HTTP **>300s 超时**。⇒ 每接一张实测整册 materialize 耗时，**超软上限即停并转性能 spec，不带退化铺量**。判据纪律：**不得**用 `store_field_count` 与 `field_count` 作差推断数据丢失（D4 spec 曾误判 1648 vs 992）。<br>**前端接线**：`entryId` 已是 `Ref`（桥内运行时取值），但 `capability: capabilityForEntry(字面量)` 与 `flushHtml` 闭包内 `entryId`/`sheetKey` 是**构造时字面量** ⇒ 改读 ref；`isD2DetailSheet`(`currentSheet==='D2-2'`) → 按**从 provider 受管清单派生**的集合判定，不前端硬编码。Q15 必测「连续切两张受管 sheet 都能进 OO」（同 entryId 不同子 sheet 的 `store-projection`/`materialize` 是同一 URL、http.ts 去重层会 abort 在飞请求，该坑已在桥内处理但须真栈复验）。<br>**8 Req / 17 Property（含复盘新增 Q4b 下游联动）/ 18 Task / 6 阶段 / 12 wave**。真栈三陷阱沿用 D4 结论（不能用 `page.on('response')` 判 callback；不能用 `asc_*` API 写格；模式切换条选择器须实测）。<br>📌 **顺带发现（登记不处理）**：① 四张附注披露 sheet **跨册重复**（第一册 4 张、第二三册各有前 2 张同名副本）⇒ 同一披露表多个物理载体，将来接须先裁权威 ② 披露表命名陷阱：源 tab 名 `附注披露信息（国企）D2-1`，先跑 `/D2(?:-\d+)?[A-Z]?$/` 会判成 `D2-1` 审定表 ⇒ 披露组件永远挂不上（`d2Constants.ts` 已有注释 + 2026-07-30 Playwright 实测过）③ 每册都有 `GT_Custom`(8行×2列/0公式) 须确认 instrumentation 不误算进受管清单 ④ `lock_room_oo_apply` 是 per-room 会话级锁横跨整个 CPU 段 ⇒ D2 多人同编同底稿串行排队（既有事实，归性能 spec ROI-6）。 |
| `workpaper-sync-row-table-engine-and-d1-coverage` | **0/35** | **双向回写行表引擎泛化 + D1 全覆盖**（2026-09-25 新建，三件套齐 0 diagnostics，Design-First 未实施）。用户裁决：**D1/D2/D4 各自独立成套（像 D4 现在这样）**，顺序**先 D1 再 D2**。<br>**红基线（全部实测）**：7 家 provider **33 个函数名逐字相同**（约 5600 行复制 6 份：D1 902 / D3 847 / D5 781 / D6 849 / D7 844 / D2 1361）；几何常量 **6 个 100% 同构**（`TEMPLATE_ID`/`FIRST_DATA_ROW`/`LAST_DATA_ROW`/`FOOTER_ROW`/`UUID_COL`/`TABLE_NAME`，D1/D4 多一个可选 `HEADER_ROW`）；`FORMULA_MASK` **7 家形态一致**（全列向区间 `{COL}{FIRST}:{COL}{LAST}`，无一逐格）；D2/D3/D6/D7 的 `tuple(sorted(SCALAR_FIELD_SPECS + _aging_field_specs(), key=_col_index))` **逐字相同**、`_snake`/`_col_index` **各四份复制**；`MANAGED_FIELD_SPECS` **D5 是 7 元组**（内联 `group_header_cell`）其余六家 6 元组 + 独立 `GROUP_HEADER_CELLS` mapping ⇒ 同语义两表达；`oo_to_html.py` **89 处 D4 提及 + 9 分支 `elif adapter_id ==` 链 + 9 处 `hasattr`**，`excel_extract` 8 处 `contract_id` 分支 / `merge` 5 / `adapters/excel` 2 / `excel_materialize` 1；**D1 受管覆盖 1/21**（模板 openpyxl 直读 21 张 sheet，仅 D1-3 受管），D4 是 **30 受管 sheet / 36 binding / 18 个 `_INCLUDE_*` 开关全 True**。<br>**三层架构**：框架层（全平台唯一实现，**零 wp_code 分支**，AST 卡点守）/ 循环层（每循环一 entry，≤150 行声明；D2 放宽 ≤300）/ sheet 声明层（每张一薄模块 + 灰度开关）。**不合并 entry** —— materialize 是整册的（逐 binding 跑 + verify 逐 binding 全跑，任一失败整册 500）、`lock_room_oo_apply` 是 per-room 锁横跨 23–99s CPU 段。<br>**成功先例照抄**：`phase5_transposed_sheet.py`(500) + `transposed_registry.py`(32) 已把 D4-29 从单例泛化成「引擎 + 两份 spec」且逐字节零回归。**已有框架件复用不重造**：`_align_specs_to_sibling_tables`(:1139) / `_sibling_identity_bindings`(:1256) / `_static_region_bindings`(:1293) **三者已接受 `provider=` 参数**；`_attach_sibling_bindings` 的 D4 特化**只有一行 import** ⇒ 泛化=提成参数、函数体不动。<br>**6 条裁决**：D1 AST 卡点守框架层纯净 · D2 sibling binding 泛化不重写对齐规则 · **D3 审定表不进行表引擎**（实测三循环审定表形态互不相同：D1-1 三区×动态票据种类 per-cell 锚点 / D2-1 **写死 4 行**×SUMIF per-cell 锚点 / D4-1 两区×动态行+四态状态机 ⇒ 差异大于共性，另立 `AdjudicationSheetSpec`，共享的是前端四态状态机而非后端投影引擎）· D4 `rowType`+`source` **两维并存**（正交：`summary` 在 source 无对应、`tb` 在 rowType 无对应；冗余 `isFixed` 收敛掉）· D5 灰度开关逐张接入 · D6 零回归门用 **canonical JSON sha256**（8 contract × 3 = 24 digest）但 **materialize 段必须真跑穿过 `verify_unmanaged_regions`**（D4 spec 教训：判据只覆盖 adapter 两方法 ⇒「判据绿而生产 500」）。<br>**D1 六批次**：①D1-2 + D1-4（**三区**）②D1-8/D1-16（各双区）+ D1-5 ③D1-9/10/11/12/15 ④D1-7（嵌套 dict）+ D1-14/D1-13（标量）⑤D1-6（真二维矩阵，评估 `TransposedSheetSpec` 能否表达）⑥D1-1 审定表（per-cell→行数组双读单写 + 四态状态机修「cross-sheet 无条件盖掉手工值」静默丢数据）。**范围外**：D1A 程序表 / 附注披露两张 / 底稿目录 / 提示页。<br>**白拿能力**：D1-4/D1-8/D1-16 同 sheet 多区直接复用已修的三层（兄弟 Table ref 位移 + `_GT_SYNC` footer 重冻结 + `CompositeRowShift` 累积归一化），且 `test_sibling_table_ref_row_shift.py` 判据 6 清单从 `instrumentation_specs()` **动态算**（实测 30 受管 sheet 中 5 张多区）⇒ 需扩成 provider 参数化即自动覆盖。<br>**性能只承诺不劣化**（≤基线 110%，脚本现测）；三端点实测 store-projection 37–46s / pending-mutations 24–33s / materialize 23–99s 的根因归两个性能 spec。**每批次接完必跑整册 materialize 实测耗时，超软上限就停下转性能 spec**，不带退化铺量。<br>**10 Req / 18 Property / 35 Task / 10 阶段 / 19 wave**（Task 0 = 前置依赖入库核查）。<br>🔴 **2026-09-25 复盘按值 grep 重测 D1 全部 store 键，补正两处受管区计数（与 D2-3 同源错误）**：**D1-15 是双区**（`D1-ecl-individual-rows`/`D1-ecl-portfolio-rows`，实测 `useD1EclCalc.ts:208-209` 的 dict 形式，首版写成占位「ECL rows」按单区处理 ⇒ 另一键数据在 OO 里会不可见、回写丢失）、**D1-13 是双区**（`D1-sampling-vouching-rows`/`D1-sampling-specific-samples`，首版只写「2 行表」未列键名）。⇒ **受管区总数首版从未算过**，tasks 里的 binding 增长数按「一 sheet 一区」估的全部偏低。重算路线：`1 → 2(D1-2) → 5(D1-4 三区) → 10(批次2) → 16(批次3 含 D1-15 双区) → 19(批次4 含 D1-13 双区) → 20~21(批次5) → 23~24(批次6 D1-1 三区)`。**教训固化：store 键 grep 必须按值匹配 `'D1-[A-Za-z0-9_-]+'`，`^const \w+_KEY\s*=\s*'` 这类按声明匹配会漏掉 dict 字面量里的键**（本轮这一条模式共抓出 3 处遗漏：D2-3 三键 / D1-15 双键 / D1-13 双键）。<br>🔴 **2026-09-25 复盘补两条需求 + 两条 Property（D1-4 接入的隐藏冲突面，首版全漏）**：**需求 5.7 / P17 第三写入方定序** —— `useD1WriteoffCheck.syncReversalToD14` 会**回写** `D1-bd-portfolio-rows` 的「按组合计提」父行（其注释写明「D1-4 的期末未审随之重算，并沿 D1-4 → D1-1 → 披露 → 附注 逐级联动」）⇒ D1-4 接 sync 后该键有**三个写入方**（HTML 保存 / OO 回写 / 跨 sheet 回写），若不定序会绕过 sync 的 CAS 直写、与 materialize 产物分叉（下次 extract 反读到非预期值 ⇒ roundtrip 门红或静默覆盖 OO 改动）。裁决方向：OO 模式期间禁用该入口给中文原因，或改走 pending-mutations 通道，**不得**两条路同时直写。**触类旁通已 grep：D1-4 是全仓唯一此情形** —— D2 侧 `useD2WriteoffCheck.reversalConsistencyWarning` 只读不写（仅告警）。**需求 5.8 / P18 下游 computed 零回归** —— D1-4 三键下游有 `useD1EclCalc.d1_4DataAvailable`(:475)+`parseD1_4Rows`(:497/:499) / `useD1Adjudication` 坏账区 / `D1TabIndex.vue:49` `progressKeys`，只验自身读回等值会放过「下游看不到回写」。两者并入任务 26 作两个必须同波交付的子目标（不单列以免全量重编号）。<br>🔴 **Requirement 10：前置依赖必须先入库**。调研中一度把 `merge._protection` 登记成「已修复」，复核 `git show HEAD:…/merge.py` 后发现**修复只在工作树、HEAD 仍是只比列旧实现**，判据 `test_masked_cell_protection_is_cell_level.py` 仍 `??` 未跟踪 ⇒ 立 Task 0 作开工第一件事，判定**必须用 `git show HEAD:`不读工作树**。未入库则批次 6（D1-1 逐格 mask）阻塞，批次 1~5 不受影响（明细类 mask 全列向区间、行范围恰等数据区 ⇒ 只比列与格级判定等价）。<br>📌 **顺带发现两处文档过时（不改原文只登记指针）**：① `d4-html-to-oo-store-contract-alignment/tasks.md` 任务 18 的「`merge._protection` 修法待拍板、本轮未改」**部分过时** —— 工作树已改格级判定（`cell_in_ranges` + `_mask_spans_data_column`，`merge.py:663/761`）且 33 判据实跑全绿、`column_in_ranges` 只剩 `contracts.py:1150` 一处反向校验（语义正确），但**尚未入库**（见上 Req 10）② `instrumentation_specs()` 中 D4-1 那条「默认不接 …… 2 spec 会打挂整个 entry」**已过时** —— `_INCLUDE_D41_ADJUDICATION_INSTRUMENTATION` 实测 `True`。<br>📌 **待 D2 spec 确认（已登记防遗忘）**：D2-7 **两套并存**（`useD2VoucherCheck` 旧套 `D2-voucher-params`/`-samples` vs `Enhanced` 新套 `D2-vc-current-rows`/`-post-rows`），删旧套前须查真库行数 + 业务确认；`useD2Adjudication:182-188` **死降级路径**（逐行读 `D2-detail-{i}-{field}`，全仓零写入方 ⇒ 恒返 0，主路径 JSON 正常故非活跃 bug）。 |
| `d4-html-to-oo-store-contract-alignment` | **17/18** | **D4 HTML→OO store 契约对齐（D4-1 审定表 / D4-35 / D4-13）**（2026-09-23 新建 + 同日实施，三件套齐，0 diagnostics）。起因：用户实测「表格视图有数据、切在线编辑后 OO 里是空的」，真栈复现于 D4-1（HTML 主营段 7 行有金额，OO R8:R11 全空、小计 0）。<br>**范围裁定不是「D4 逐一修 34 张」** —— 逐表实测 43 张 row table 后，20 张「有行零值」里 19 张对应 store item 在库里本就不存在/空数组（HTML 侧同样空，非缺陷），真正「HTML 有数据而 projection 没有」只有审定表两区。<br>**三处缺陷、两个类**：① **A（D4-1 独有，出/回双向皆死）** 后端 provider 双向都按「`D4-1-rows` 行对象携带 label+6 金额」写（`build_store_projection_d41` 读 `row.get(store_key)` / `merge_projection_into_d41_rows` 写 `target[store_key]`），而前端共享件 `serializeRows()` 只落 `{rowId,label,source,accountCode}`、金额落独立 item `D4-1-{rowId}-{field}` ⇒ 出方向 6 金额恒 None、回方向 OO 改动前端从不读。库里唯一一条真 `D4-1-rows`（E2E 种子 wp `d4e2e000…d403`）实证无任何金额键。② **B（D4-1 独有）** HTML 显示的行主体是派生行（`crossSheetMainRows` 读 `D4-2-rows` 现算，`rowKey=xsheet-*`）**从不落库** ⇒ 两区贡献 0 行、overlay 退回模板占位空行 `GTROW-D41MAIN-0008…0011`。③ **C（D4-35/D4-13，只错在出方向）** `store_projection_response.py` 只喂 `STORE_ITEM_IDS` + `STORE_ITEM_IDS_D45_FIXED`，而 `D4-35-data`（不在任何清单）与 `STORE_ITEM_IDS_D413_FIXED` 被漏掉；探针实证 D4-35 `0 vs 32 字段`、D4-13 两键 `'' vs 探针正文`（**固定键恒存在故必须比值不能比键数**）；反方向 `oo_to_html.py` 三批清单都消费了 ⇒ 两侧真源不一致。<br>**爆炸半径**：`*TabAdjudication.vue` 全仓只有 `D4TabAdjudication.vue` 接了同步桥，K2/D2/D6/D7/F3/E1/F5/G8/G9/G12/K4/L8/M5 + `shared/CycleTabAdjudication.vue` 均未接桥 ⇒ A/B 在它们身上**潜伏**；`useK2Adjudication` 与 D4-1 共用同一 per-field 模型 ⇒ 修复落**共享层** `shared/dynamicAdjudicationRows`，那 13 张将来接桥自动继承。<br>**关键裁决**：金额权威载体迁到行清单行对象（**后端零改动**，已归档 12/12 判据继续有效）；迁移走**双写 + 读侧行对象优先、缺则回落 per-field**（旧项目零迁移脚本不归零，且保证 OO 回写不被旧值盖住）；派生行落库为 `source='tb'`（**原「显示口径一字不改」已因选项 b 作废**，见下方 O1 段：`sections` 改逐格合并）；store item 清单收敛成 provider 单一 `all_store_item_ids()` + CI 卡点 `check_store_item_ids_fully_wired.py`（不吃 `tests/workpaper_sync/` 既存失败分母）。<br>**7 Req / 17 Property / 18 Task / 5 阶段 / 13 wave**。<br>✅ **O1 已由用户拍板：选项 b**（OO 改了就算，store 为准，HTML 标「已人工覆盖」）。为此新增**逐格四态状态机**：行对象加 HTML-only 键 `derivedSnapshot`，按 `(stored vs snap, snap vs derived)` 穷举 S1 纯派生 / S2 人工覆盖 / S3 上游变了自动跟随 / S4 覆盖且上游也变（**双值同时可见、不自动二选一** + 「恢复取数」）。🔴 覆盖判定**禁止**用「store ≠ 当前派生值」—— 上游一变全部纯派生格都会被误判成覆盖，Property 12 是专门钉这个错法的反证式判据。`derivedSnapshot` 能穿过回方向有结构依据：`merge_projection_into_d41_rows` 整行 `dict(row)` 拷贝后只覆写 7 个受管 store_key。代价已登记：`sections` 从「整行跳过」改为「逐格合并」，原「显示口径一字不改」不再成立（Property 7 改条件式）。<br>🔴 **连带挖出一处既有缺陷（模板实测）**：`C12 = =SUM(C8:C11)`（OO 本期账项调整小计**逐行汇总**）vs HTML `buildSubtotalRow(..., adjTotals.mainAje)`（取 **D4-4 汇总额**、不逐行汇总，而上期 AJE/RJE 小计反而是逐行汇总）⇒ **两侧本期 AJE/RJE 小计今天就可能不等且无告警**。裁决 D6：HTML 改逐行汇总 + 「逐行和 ≠ D4-4」显式告警；Property 16 必须以**模板公式作第三边**（两侧互比会同错自洽，I 循环踩过同型）。同处另清两条死代码：`buildCrossSheetRow` 把派生行 AJE/RJE 硬编码 `0` 而上方注释声称「仍从 per-field 键读、审计师填的调整不丢」（注释与实现不符）；`isEditable: r.source !== 'tb' || true` 的 `|| true` 使前半段成为死表达式。✅ **原 O4 已澄清收口（不再阻塞）**：小计口径与「恒等于 D4-4」分属两层——算法恒按模板逐行汇总，「小计 vs D4-4」作为**第三条同构跨底稿校验告警**（套既有 `mainCrossValidation`/`otherCrossValidation` 范式，超容差亮提示、不改任一侧、不阻塞）；审计要求恒等时由审计师依告警调平，非系统盖掉某侧。之前写成「改口径 or 阻塞」的二选一是表述错误。<br>✅ **实施完成 17/18（2026-09-23）**。**交付**：后端 `all_store_item_ids()` 单一口径（46 item）+ 出方向 `store_projection_response` / 回方向 `oo_to_html` 双向改走它（老 provider 回退旧并集，行为不变）+ CI 卡点 `check_store_item_ids_fully_wired.py`（AST 静态声明 + 运行时 `dir(P)` 兜住 import 来的 tuple，已接 `governance-checks.yml` 两 step）；前端共享层 `serializeRows(rows,{readField,readDerivedSnapshot})` 金额随行落 `number`（非字符串）+ `readRowFieldWithFallback` 行对象优先回落 per-field（K2 同时改走，**旧项目零迁移脚本不归零**）+ `syncDerivedRowsIntoStore()` watch 派生源幂等落库 + `resolveCellState()` 四态 + `restoreDerivedValue(rowId,field)` 逐格恢复 + `D4TabAdjudication.vue` 覆盖 tag / S4 三值 tooltip / 「恢复取数」+ `adjustmentTotalsValidation` 同构告警 + 小计本期 AJE/RJE 改逐行汇总 + 两处死代码已清（`\|\| true` / 派生行 AJE/RJE 硬编码 0）。**后端 D4-1 provider 零改动**（归档 12/12 判据继续有效），这是「改前端」路线的收益。<br>**验证（实跑数字）**：见该 spec `tasks.md` §实施进度实证与 `docs/operations/evidence/d4-store-contract-alignment/`；七项变异检验（P1/P4/P8/P8-前端/P9/P12/P16）**全 KILLED**；端点层真栈已过（D4-1 十四格逐值、D4-13 两段正文等值、D4-35 装配 32 格 vs 修复前 0 格）。<br>✅ **真栈端点层已在全栈在跑状态下复核通过（证据文档 §六）**：同一报障底稿的 `store-projection` 在 **overlay 之后**的最终投影里，`adjudication_main_rows` **21 格（7 个派生行 ×3）**、`adjudication_other_rows` **16 格**、D4-35 **32 格**（修复前 0）、D4-13 两键值 = store 正文（修复前恒 `''`）。主营 7 行金额（153431246.06 / 1528820416.32 / 89847600.46 / …）**正是用户报障时 HTML 有、OO 里空的那 7 行** ⇒ 缺陷 A1/B/C 真栈通达。顺带实证 Task 12 孤儿清理（tb 派生孤儿自动清、manual 行保留）。<br>🔴 **推翻一条错登记**：此前写「D4-35 端点最终 0 格，属既有 overlay 语义」是**探针读错响应层级**（投影体在 `data.projection`，不在 `data` 顶层）的误判；`overlay_store_on_baseline_projection` 实为**并集 + store 优先**，只丢「基线无该键且 store 值为 None」的占位，不滤 store 非空值。教训：`store_field_count`(1648) 与 `field_count`(992) 作差≠数据被吞。<br>✅ **Task 18 浏览器层真栈已闭环（2026-09-24）**：`e2e/d4-1-adjudication-oo-visibility.spec.ts --workers=1` ⇒ **1 passed**。OO canvas 实读 `B12..B18` = 7 个主营派生行金额（153431246.06 / 1528820416.32 / 89847600.46 / 16389521.26 / 175221.24 / 1100917.44 / 1844830.88）与 projection 逐值相等，`B19 = =SUM(B8:B18)` 合计公式正确扩张，`console_errors=[]`。**对照报障「切在线编辑后 OO 里是空的」（真栈形态 R8:R11 全空、小计 0）⇒ 报障闭环。**<br>🔴 **过程中逐层剥出并修掉三层缺陷（都在 materialize 行位移层，由本 spec 的派生行落库触发）**：①`_grow_managed_table_ref` 只更新本 binding 的 Table part ⇒ 同 sheet 兄弟区 ref 不位移（实测 `A14:X17→A14:X17`，应 `A21:X24`）；②`_refresh_gt_sync_runtime_binding` 的 footer 放行集只含本 tid，把「同 sheet 兄弟」与「不同 sheet」混为一类 ⇒ `FooterAnchorDriftError`；③`verify_unmanaged_regions` 按 `region.table_key` 取**单趟** shift，表达不了「同一 sheet 被多趟各插一次行」（D4-1 实测 main +7 / other +6）⇒ 新增 `CompositeRowShift`（链式 unshift）+ `normalise_propagated_part` 链尾优先逆替换（声明成链 `$A$18→$A$25→$A$31`，顺序反了会停在中间态）+ `_sheet_cumulative_shift` 把每趟 `total_formula_rows` 的中间口径映射回最初 before（其他区合计行模板 18、那趟声明 25）。错误码演进：`identity_carrier_missing`→`footer_anchor_drift`→`workbook_and_styles`→`managed_sheet_unmanaged_cells 280→340`→`292/292 内容不等`→全绿。<br>✅ **影响面覆盖 D4 全部 5 张同 sheet 多区底稿**（D4-20 **三区** / D4-34 / D4-36 / D4-1 / D4-9；清单从 `instrumentation_specs()` 动态算，新增受管区不会漏）。判据 `test_sibling_table_ref_row_shift.py` **14 passed**（含 5 张参数化 + 3 条单区纯增量纪律），变异 **3/3 KILLED**，零回归 `-k "d4 or D4 or materialize or shift or verify or propagat or unmanaged"` ⇒ **1032 passed / 0 failed**。**覆盖面论证**：materialize 是整册的（36 binding 逐趟 + verify 逐 binding 各自 assert_equivalent），拿到 materialize 200 即意味着 36 个 binding 全通。<br>✅ **D4-35 / D4-13 浏览器层已补齐（2026-09-24）**：`e2e/d4-35-d4-13-oo-visibility.spec.ts` **1 passed**（两次干净 run）。先澄清结构事实——D4-35/D4-13 与 D4-1 **同属一个 entry** 的同一份 store-projection（一次响应 27 个 table_key），画布层就是同一次切 OO 后多切两个 sheet。实测 D4-35 两个 store 行的三列**分别整齐落第 26/27 行**（`missing=[]`）、D4-13 两段正文**精确命中 A6/A16**（`stray_hits=[]`）。判据除逐值可见外另加两条几何断言（同一 store 行各列须落同一 Excel 行 / 不同 store 行须占不同 Excel 行）。顺带钉住 materialize 几何语义：**保留模板占位行、真实派生行追加在占位区之后**（D4-35 占位 15..25 ⇒ 落 26/27；D4-1 首行 8+4 占位 ⇒ `insert_at=12`）。<br>🔴 **真栈②（需求 5.3）：链路打通，但撞上更深缺陷 —— 上面「拿不到 callback」的归因已被推翻**（证据文档 §十三/§十四）。新增判据 `e2e/d4-1-override-roundtrip.spec.ts` 目前**正确地红**。① callback **一直是到的**：原判据用 `page.on('response')` 判它，而 OO 容器**直接** POST 给后端不经浏览器 ⇒ 那个 `callback_arrived=false` 是判据自己的错（后端 `application_bound_at` 有值即为证）② `host_dirty_before_forcesave=False` 是**无关变量**：保存按钮可点、forcesave 照样 202 且 `cs_error=0 accepted callback_expected=true`，OO 也真执行了保存（`asc_isDocumentModified()` true→false）③ 往返**走通**：`created → application_bound → extracting → merging → …`，`working_paper_sync_conflict` 精确认出 OO 改动（`incoming_value=153443591.73` 恰为「原值+12345.67」）④ ✅ **根因已修（2026-09-25 用户拍板实施；改动在工作树，HEAD 未入库）= `conflict_kind='protected'` / `read_only_masked_cell`**：`merge.py::_protection` 原用 `column_in_ranges(spec.cell.column, formula_mask)` **只比列不比行**，D4-1 逐格 48 格 mask 把小计/合计/差异行的 B–I 八列整列带进列跨度 ⇒ 全仓盘点实证 **311 个 `editable` 字段被误判只读**（静态格 286「其实不在 mask 里」+ 动态行 25「同列 mask 在数据区外」），**9 个非 D4 entry 误伤=0**（mask 恰覆盖数据区，列判定=格判定）。修法：新增 `contracts.cell_in_ranges(col,row,ranges)` 格级判定，`_protection` 按 `row_from` 分流——静态格走精确格级、动态行仅当 `_mask_spans_data_column`（同列 mask **多行区间且覆盖首数据行**=anchor+header_rows）为真才保护，精确区分「整列数据区 mask」（G7 `K8:K200` 该保护）与「模板固定行 mask」（D4-1 `B12` 插行后失效不该保护）；`column_in_ranges` 保留给 CS-13 声明校验。判据 `test_masked_cell_protection_is_cell_level.py` **33 passed**、变异 **3/3 KILLED**、`test_task14 masked_note`（G7 整列 mask 动态行）仍 protected 未回归。真栈 `d4-1-override-roundtrip.spec.ts` **1 passed**：operation `created→application_bound→rematerializing→applied`、store 落新值 153492974.41、HTML 显「已人工覆盖」⇒ **需求 1.5 后端可达、S2 态真栈出现、Task 15 四态 UI 不再是死代码**。零回归 1938 passed（唯一 failed=task42 H1 既存，git stash 基线对照）。证据文档 §十五。<br>📌 另两个独立问题待立项：① 同一 representation generation 内**重复 materialize** 会让 OO 按 doc_key 缓存的文档与磁盘 staged xlsx 分叉（OO 日志 `UpdateVersion expired`，`docker restart audit-onlyoffice` 可临时解），真实用户「切表格视图→再切在线编辑」即可触发 ② 多个历史 generation 的 room 仍 `state='active'`（g96/g97/g99/g100/g101 并存），AC 2.8 显式 supersede 未全程生效。<br>🧪 **真栈②判据自身修正的两个错法**（写进注释防复发）：不能用浏览器网络判 callback；不能用内部 `asc_*` API 写格（只改浏览器端、未经协同通道同步 ⇒ forcesave 回 `cs_error=4` no_changes、operation 直接 `rejected`）—— **只有真实键盘输入**（名称框 `#ce-cell-name` → `keyboard.type` → Enter）才产生 changes 并同步到服务端。<br>🔴 **顺带发现并修掉一个平台级缺陷（2026-09-24，原「domain error 以 500 返回该翻 4xx」归因已推翻，见证据文档 §十一）**：真因不是后端分类，而是**前端读不到 `error_code`**。真栈实测后端**两种错误形状并存** —— 业务 `HTTPException` 走平台全局 handler 输出 `{"code":404,"message":"项目不存在"}`（`exc.detail` 塞进 `message`，dict 原样保持 dict，**无 `detail` 键**），而 starlette 原生 404 / `RequestValidationError` 输出的才是 `detail`。前端下游普遍只读 `detail`：`utils/errorHandler.ts::handleApiError:43`（平台统一错误处理器，全仓大量视图在用）⇒ 400/409/503 的后端中文根因全退化成兜底文案、422 的 `AI_CONTENT_NOT_CONFIRMED`/`CROSS_MODULE_CONFLICT_UNRESOLVED` 特化分派**从未生效**；sync 桥 `readWireError` ⇒ `classifySyncFailure` 分派从未生效、domain error 被判可重试而重试 3 次。反差佐证：`http.ts::extractErrorDetail:187` 早就写对了 `detail ?? message`，所以拦截器自己的 toast 一直好的 —— 同一知识全仓重复实现 N 次只有 1 处正确。**修法收敛到单一真源**：`utils/http.ts` 是唯一业务 axios 实例（`services/*` 含 apiProxy、`workpaperSyncApi` 全基于它），故在**错误拦截器一处**新增 `normaliseErrorEnvelope()`（仅 `detail===undefined && message!==undefined` 时回填，跳过 Array/Blob/非对象 ⇒ starlette 404 与 422 字段级**数组**不被覆盖），与成功路径「统一解包 ApiResponse」对称；**不改后端**（`{code,message}` 是平台信封有意设计，仓里 `test_signed_report_rollback_protection.py:284` / `test_audit_log_enhanced.py:583` 两处注释为证）。判据 `errorEnvelopeNormalisation.spec.ts` **12 passed**（纯函数 6 形状 + 拦截器接线 1 + `handleApiError` 端到端 5），变异 **3/3 KILLED**；`bridgeFailureRealEnvelope.spec.ts` **9 passed**（修前 5 failed）；后端跨层契约锚点 `test_error_handler.py` 加 2 条（dict detail 不被字符串化 / 固化「业务 HTTPException 无 detail 键」= 前端适配存在的理由锚点）⇒ 9 passed。真栈回证：e2e 捕到真实 409 体 `{"code":409,"message":{"error_code":"participant_leave_refused_in_flight","message":"该 participant 还有 4 个未终结的写请求…"}}`。⚠️ **假绿第二源（未修，已登记）**：后端**几十个**测试文件断言 `resp.json()["detail"]["error_code"]` 且能过，因为用的是裸 `FastAPI()` fixture **没注册**全局 handler ⇒ 测到的形状生产不存在；建议统一 fixture 或加 CI 卡点。另：错误文案报 `dynamic_tables[0]`（恒为 D4-2）而非真正失败的 binding，现场极易误判。踩坑登记：D4-1 模式切换条是 `.sync-mode-bar`，非 D4-2 的 `.d4-mode-toolbar`。<br>⚠️ **一处实现偏差已显式登记**：Task 14 的四态状态机只作用于 `currentUnadjusted`/`priorUnadjusted` 两个**有派生值**的字段，AJE/RJE 改走单源读而非状态机（派生聚合不产生调整额 ⇒ `derived` 恒 0 ⇒ 状态机对它与单源读同义，多接一层是死代码）。<br>**不在范围**：同一轮会话报障的「切回表格视图后无法再切在线编辑」根因在 `utils/http.ts` 请求去重键插入/删除不对称（POST 键带 body 指纹入、不带指纹删 ⇒ 键泄漏 5 分钟，同 body POST 发出前即被 abort），**已当场根治 + 真栈 6/6 验收**，判据 `src/utils/__tests__/httpDedupe.postKeySymmetry.spec.ts`（变异 2 条打红）。 |
