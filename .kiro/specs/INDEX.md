...
**最后更新**：2026-09-08
**当前分支**：`work/2026-09-02-workpaper-sync-closure`
**统计**：Active **10**（2026-09-08 逐目录实扫 `.kiro/specs/*/`；`advanced-query-hardening-wiring-closure` 与 `dsh-agent-panel-integration` 均于 2026-09-08 归档，Active 由 12 → 10） / Archived **564**（2026-09-08 `_archive/*/*` 实扫；其中 6 个历史/替代目录无 `tasks.md`）

> **2026-09-08 附件预览格式扩展 spec 复盘**：`audit-evidence-attachment-preview-format-expansion` 已由空壳补齐三件套，当前 **0/20**（未实施生产代码）。机器校验：**9 Req / 69 AC / 32 Property / 20 Task / 13 Wave**，AC→Property **69/69**、AC→Task **69/69**、Property→Task **32/32**，DAG 无环且 wave/critical path 均为真实依赖。设计关键门：Type_Hint 非 MIME、路线资格先于体积、三族可终止 Worker、邮件 sanitizer+sandbox/CSP、宿主专属 legacy 矩阵、strict mutation target snapshot。`design.md` / `tasks.md` 当前仍为 git `??`，未提交前不得把 spec 产物记为已入库。
...
| spec | 进度 | 一句话 |
|------|------|--------|
| `audit-evidence-attachment-preview-format-expansion` | **0/20** | **压缩包 / 邮件 / DXF 浏览器内预览扩展**（2026-09-08 完成三件套复盘，尚未实施）。真实地基：`file_type` 是 Type_Hint 而非 MIME；两宿主 legacy 能力矩阵分别冻结；新增字节只走 download；共享 HTTP 需 `_dedupe:false` 才能保留取消信号；archive/email/dxf 解析进入可终止 Worker；邮件以私有 DOMPurify + sandbox/CSP 双层阻断外发；路线 A 必须 core+vue+真实插件挂载并先过资格门，GPL peer 禁入产品。结构 **9 Req / 69 AC / 32 Property / 20 Task / 13 Wave**，三向覆盖 **69/69、69/69、32/32**，DAG 无环。`design.md` / `tasks.md` 当前 `??` 未跟踪，未入库前不得视为交付。 |
...
| `workpaper-guidance-content-closure` | **1/23** | **逐 sheet 编制说明内容闭环 + 三 spec 共享 wire contract owner**（2026-09-07 语义复审收敛）。C0 单源定义 location/section/SourceRef/authority/candidate-finalized handoff/ConsumerAck/rail/evidence；catalog 与 runtime inventory 分离，`gross_required/effective_required` 可核算；标准 exact 只认 active publication，custom exact 只认 durable ACK；C2 改为固定 cutoff 的不可变 milestone，后续由 `platform_guidance_health` 持续重算。三件套 **15 Req / 89 AC 零悬挂且 Property+Task 双层覆盖 / 27 Property / 23 Task / 15 Wave**；当前仅 Task 1 调查标绿，未改生产代码、未假绿。 |
| `workpaper-page-formula-toolbar-closure` | **0/15** | **workpaper route 公式入口与公共能力壳层闭环**（2026-09-07 三件套完成语义复审，尚未实施）。范围不误删 TB/report/note domain dialog；采用唯一 `CanonicalLocationState` runtime owner、capability 前置、真实 named toolbar outlets + registration lease、dirty draft pin、四维 provider descriptor、user formula API v2/durable audit、AI review/assist 分流和 review/guidance/AI 单一 shell；发布 `F-SHELL` 供 custom 消费。三件套 **13 Req / 74 AC 零悬挂且 Property+Task 双层覆盖 / 22 Property / 15 Task / 11 Wave**，进度真实为 0/15。 |
| `custom-workpaper-template-ingestion-and-sync-closure` | **0/19** | **自定义 Excel 模板摄取与整册同步闭环**（2026-09-07 三件套完成语义复审，尚未实施）。一份多 sheet 文件只建一份 `ProjectWorkbookInstance` + child entries，共享 current/generation/OO room；分离四生命周期，前置 tenant/capability/不可变审批；补 ZIP/XML/资源预算与 scanner digest，`.xlsm`/外链 v1 阻断；candidate 仅静态预览；editable 强制 manifest+adapter；HTML 采用 immutable staging + client-base CAS；finalize 采用 pending visibility + durable ACK saga，并显式依赖 room/resolver/durable/namespace gates。三件套 **17 Req / 117 AC 零悬挂且 Property+Task 双层覆盖 / 25 Property / 19 Task / 16 Wave**，Task 19 只发布 X evidence，不等待 guidance C2，进度 0/19。 |
