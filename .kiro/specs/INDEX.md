...
**统计**：Active **15**（2026-09-25 22:5x 逐目录实扫 `.kiro/specs/*/` 且**带 tasks.md** 口径；目录数 17，另 2 个只剩 `evidence/`：`procedure-delegation-visibility-isolation` / `visibility-isolation-go-live-hardening` —— 三件套早已归档，**但这两个 evidence 目录被活测试 `test_task5_capacity_frozen_profile.py` 硬读，不得移动**，见下方 2026-09-25 节） / Archived **587**（`_archive/*/*` 实扫）

> ⏱ **本数字半天内变过三次**：归档 7 个后是 13，之后并发会话把 `workpaper-sync-row-table-engine-and-d1-coverage` 改名为 `d1-sync-row-table-engine-and-d1-coverage` 并新增 `d3-sync-coverage-via-row-table-engine` / `d567-sync-coverage-via-row-table-engine`（两者三件套均已齐），于是变 15。**再次印证本文件 §四：报数一律现扫目录，不读这一行。**

> ## 2026-09-25 全量 spec 审计｜归档 7 个 → Active 20 降到 13
>
> 逐目录实扫 20 个带 tasks.md 的 spec（复选框四态 `[x]/[ ]/[~]/[-]` 全覆盖 + 未编号复选框单独抽查），
> **10 个编号任务全绿**。其中 **7 个已归档**，**3 个全绿但被「生产代码/CI 硬读其 spec 路径」挡住不能移动**。
>
> ### 已归档 7 个
>
> **新建分组 `_archive/15-workpaper-sync-engine-hardening/`**（有分组 README，含一览 / 归档口径 /
> 残留移交表 / 6 条方法论教训，读那份即可）：
> `d4-html-to-oo-store-contract-alignment`(18/18) · `workpaper-sync-static-cell-sheet-writeback`(12/12) ·
> `oo-single-pass-materialize-and-room-leave`(11/11) · `workpaper-sync-materialize-large-table-performance`(12/12) ·
> `multi-sheet-materialize-defined-name-shift-normalization`(10/10)。
>
> 另两个按主题归位：`tb-writeback-explicit-publish-gate`(21/21) → `_archive/06-engineering-governance/` ·
> `formula-row-name-alignment-confirmation`(15/15，evidence T01–T16 齐，含交付后复盘修 5 项缺陷)
> → `_archive/05-business-features/`。
>
> 归档后回归验证：`test_mutation_kit_exemptions` + tb publish gate ×2 + row_name_alignment ×2 = **87 passed**
> （`mutation_kit_exemptions.json` 现为 0 条豁免，无 stale 风险）。原位置残留 0，未跟踪 `.log` 随 `git mv` 一并迁走。
>
> ### 🔴 3 个全绿但**归档被硬阻塞**（本轮最重要的发现，不是「忘了归」）
>
> | spec | 进度 | 阻塞点（实证） |
> |---|---|---|
> | `d-cycle-sheet-bidirectional-expansion` | 9/9 | **生产代码硬读它的 evidence**：`phase5_d4_policy_check_sheet.py:112` 与 `phase5_d4_erp_check_sheet.py:76` 用 `Path(...)/".kiro/specs/d-cycle-sheet-bidirectional-expansion/evidence"/"T09|T10-*.json"` 算 `mapping_digest`，而 `phase5_d4_revenue_detail.py:1088-1090` 在**契约构建路径**上调 `assert_mapping_digest_d45()` / `_d413()` ⇒ 移目录即 D4 契约构建 `FileNotFoundError`。另 `e2e/g5-1-d43-unified-path.spec.ts` / `g5-1-d45-policy-path.spec.ts` 往其 evidence **写**文件 |
> | `excel-workbook-wide-row-change-propagation` | 30/30 | 它在 `backend/data/workpaper_sync_program_milestone_definitions.json` 的 `specs[]` **8 个名单**内；`generate_workpaper_sync_program_milestones.py` 用 `_SPECS_ROOT/<name>/tasks.md` 组路径且 `_record_input` **无 `_archive` 回退**（抛 `ProgramMilestoneError`）⇒ CI job「Recompute and byte-check program milestone registry」硬红。另 `test_workbook_row_change_reachability.py` 读其 `design.md` |
> | `published-representation-production-path-and-lane-adjudication` | 28/28 | 同上（同一份 8 spec 名单） |
>
> **⇒ 结论：这是一条通用债 —— 生产代码 / CI 门禁不该依赖 `.kiro/specs/` 路径。** 解阻有两条路，
> 都超出「归档」动作本身、需单独裁决：① 把 `T09/T10` 两份 mapping digest evidence 移进
> `backend/data/`（真源该在那儿）再改 2 处 provider + 2 个 e2e 路径；② 给 milestone 生成器加
> `_archive/*/<name>/` 回退（会改动一个「以字节稳定为目的」的治理产物 + 需重生成 registry）。
> **在这两件事做完前，这 3 个目录留在 active 区是正确的，不是漏归。**
>
> ### 另两个「空壳」目录同理不得移动
>
> `procedure-delegation-visibility-isolation` / `visibility-isolation-go-live-hardening` 的三件套
> 早已在 `_archive/06-engineering-governance/` + `_archive/99-superseded/`，active 区只剩
> `evidence/artifacts/`；但 `backend/tests/visibility_go_live_hardening/test_task5_capacity_frozen_profile.py`
> 硬读 `.kiro/specs/visibility-isolation-go-live-hardening/evidence/artifacts/task5` ⇒ **必须留在原位**。
> （此前索引反复把它们记成「空壳可清」，实际是被活测试消费的证据库。）
>
> ### 🔴 本轮实测打翻的旧读数（报数请以此为准）
>
> | 项 | 索引旧记 | 2026-09-25 实测 |
> |---|---|---|
> | manifest entries | 186 | **155**（`legacy_fake_bidirectional` **137** / `parent_duplicate` 12 / `adapter_registered` 4 / `adapter_candidate` 1 / `unreachable_pending_delete` 1） |
> | capability 分布 | — | `single_onlyoffice` **145** / `single_html` 5 / `bidirectional` **4** / `unreachable` 1 |
> | `working_paper_sync_entry_state` / `content_version` / `content_representation` | 1 / 1 / 1 | **12 / 259 / 265** |
> | `working_paper_artifact` | 19→20 | **1078** |
> | definition bundle / artifact | 3→5 / 6→14 | **59 / 126** |
> | `working_paper_oo_room` | — | **99**（`active` 69 / `opening` 22 / `superseded` 8 —— 「多 generation room 仍 active」这条残留属实） |
> | guidance 文件分母 | 373 | **402**（`migrate_guidance_canonical_schema.py --check`：files=402 unmapped=**159** changed=0 errors=0）⇒ T7 工作卡与 C1 catalog 的 373 分母**全部 stale** |
>
> 📌 `entry_state` 12 行 = **11 个 entry**：`xlsx/gt-d4-operating-revenue` **gen=163**（重度使用）·
> D1/D2/D3/D5/D6/D7/B60 各 **gen=1**（发过首版就没再动）· G7 gen=6 · H1 gen=2+1 · 1 个 `opaque-…`。
> **⇒ 155 个 manifest entry 里只有约 11 个有 published representation** —— 这是总纲 spec 的真实覆盖率。
>
> 📌 `generate_workpaper_sync_program_milestones.py --check` 当前 **已是 pre-existing STALE**
> （63 个 input 里 4 个 DRIFT：`workpaper-html-onlyoffice-…/tasks.md`、`GtD2AccountsReceivable.vue`、
> `excel_row_shift.py`、`excel_workbook_row_change.py`；**0 个 MISSING** ⇒ 与本次归档无关，是上游改了代码没重生成）。
>
> ### 🔴 统计方法再次踩到的坑
>
> 上一轮记「只统计 `[x]/[ ]/[~]` 会漏 `[-]`」；本轮又发现第二种漏法：**`- [x] * 18. …` 这种
> 「星号在编号前」的写法会被「行首 → 复选框 → 数字」的正则漏掉**（`d4-html-to-oo-store-contract-alignment`
> 因此被数成 17 而非 18，`tb-writeback` 数成 19 而非 21）。⇒ 实扫必须**另跑一遍未编号复选框**并逐条看原文，
> 确认它们是真任务还是说明性清单项。
>
> 另：本表此前 **5 个真实在办 spec 从未收录**（`excel-structural-row-insertion-…` /
> `excel-template-override-layer-…` / `excel-workbook-wide-row-change-propagation` /
> `oo-html-writeback-performance` / `d-cycle-sheet-bidirectional-expansion`），
> 且留着 2 行早已不存在的（`browserless-headless-integration` 蒸发 / `platform-architecture-convergence` 已归档）。
> 本轮已全部补齐并划掉。**再次印证：报 Active 数必须现扫目录，不能读本表行数。**
>
> ### 在办 10 个 spec 的剩余量分档（按「能不能现在动手」排）
>
> | 档 | spec | 剩余 | 性质 |
> |---|---|---|---|
> | **A 可立即动手** | `four-table-extraction-entry-completion` | 7 条 | 纯功能实现，**零外部依赖**；逐符号实证真未做 |
> | **A 可立即动手** | `oo-html-writeback-performance` | 4 条 | 其中 **2 条只需复核回标**（已被后继 spec 实质交付），真活只剩 ROI-6 锁粒度 + 回归压测 |
> | **C 待泳道归属** | `excel-structural-row-insertion-…` · `excel-template-override-layer-…` | 各 1~2 条 | 变异脚本按分工书归 E 泳道，清单/预期态/交接件已交；**不是技术阻塞** |
> | **D 待上游 gate** | `custom-workpaper-template-ingestion-and-sync-closure` | 4 条 | 四 sync gate 现算全 BLOCKED（`multi_resolver_count=4` 等），自有域模型已交付 |
> | **D 待上游 gate** | `workpaper-guidance-content-closure` | 4 条 | T22/T23 等 custom spec 的 X-HANDOFF / X-RUNTIME 里程碑 ⇒ **两 spec 串死** |
> | **E 大体量人工** | `workpaper-guidance-content-closure` T7 | 402 份 | **不可脚本化**：materials/data_sources/formulas/completion/judgments 五段在任何来源里都不存在，只能人工按 sheet 撰写 + 挂 registry ref + 独立 publication；另 1158 个 canonical section `source_refs=[]` |
> | **F 平台总纲** | `workpaper-html-onlyoffice-bidirectional-writeback-closure` | 11 条 | Word lane（T60/61/62/64）+ 容量/故障恢复/删 legacy（T71/72）+ writer-version 五条准则（T74）+ 公共观测器（T75） |
> | **G Design-First 待实施** | `d1-sync-row-table-engine-and-d1-coverage` · `d2-sync-coverage-via-row-table-engine` | 35 + 18 | **真 0%**：`row_table_engine.py` / `adjudication_sheet_spec.py` / `row_table_registry.py` 全部 MISSING，待删死代码 `useD2VoucherCheck.ts` 仍在；声称照抄的先例 `phase5_transposed_sheet.py` + `transposed_registry.py` 确实存在。依赖链 D1 → D2 |
>
> 📌 **一条跨 spec 债（本轮新发现，建议单独立项）**：生产代码与 CI 门禁**不应依赖 `.kiro/specs/` 路径**。
> 现存 3 处：①两个 D4 provider 从 spec evidence 读 `mapping_digest` ②milestone 生成器按 spec 名组
> active 区路径 ③`test_task5_capacity_frozen_profile.py` 读已归档 spec 的 evidence 目录。
> 它直接导致 **3 个 100% 完成的 spec 无法归档**，并让「归档」这个纯文档动作变成有生产风险的操作。
>
> ---
>
> **2026-09-22 归档｜D4 营业收入底稿双向回写 14 个 spec → `_archive/14-d4-bidirectional-writeback/`**
> （分组 README 有完整一览、验收分层与残留移交表，读那份即可，不必逐个翻）：
> `d4-1-adjudication-bidirectional-writeback-and-formula-io`(12/12) · `d4-revenue-matrix-bidirectional`(13/13) ·
> `d4-9-customer-structure-bidirectional-writeback`(12/12) · `d4-price-analysis-writeback-linkage`(9/9) ·
> `d4-12-transposed-writeback`(15/15) · `d4-14-walkthrough-writeback`(12/12) ·
> `d4-inspection-writeback-formula-io`(T1-T7+B1-B3=10/10) · `d4-cutoff-return-writeback-formula-io`(13/13) ·
> `d4-21-24-oo-bidirectional-and-cross-sheet-formula`(10/10) · `d4-ipo-checklist-dual-mode-writeback-and-formula`(28/28) ·
> `d4-ipo-fraud-writeback-formula-io`(11/11) · `d4-33-36-writeback-formula-and-io-closure`(35/35) ·
> `d4-adjustment-and-analysis-gap-closure`(1/1 + 4 条显式 N/A 作废) · `d4-dual-mode-formula-governance`(16/16，总纲契约 C0~C4)。
>
> **归档口径 = 自有产物全部就位 + 真栈证据齐 + 残留有明确接收方**，不是「全部验完」。两级分层如实登记：
> **L1 逐张 30/30 全绿**（进在线编辑走统一 `/sync/**` → store-projection 200 → materialize 200 → callbackUrl
> 四项齐 → `wp-sync-host` + OO iframe → 无 `/d2-sync/*` 旁路），证据
> `docs/operations/evidence/d4-bidirectional-acceptance/D4-{1..36}.json`（36 份 `console_errors`/`http_errors` 全 0；
> 缺 D4-4 = owner 去重后不独立成表）。**L2 真 OO canvas 完整往返只做了 1 张（D4-2）** —— `forcesave cs_error=0`
> + `operation/application.state=applied` + `content_version.source=onlyoffice` + `store_mirror.marker_in_store=true`
> （证据 `_archive/14-d4-bidirectional-writeback/d4-revenue-matrix-bidirectional/evidence/g5-1-d4-unified-path/`）。
> 其余 29 张 per-sheet L2 **未逐张跑**，按总纲 Req 14.1「不得用一条 applied operation 覆盖全部场景」，D4-2 这条
> 不能替它们签收 ⇒ 按 entry 粒度（D4 全 30 张同属唯一父 entry `xlsx/gt-d4-operating-revenue`）**移交
> `workpaper-html-onlyoffice-bidirectional-writeback-closure` Task 70 的「全 entry required scenario」口径**。
> 另两处残留：A13 失败重试队列/持久 outbox（durable 幂等已做实 V164，重试队列属平台级异步设施宜单立 spec）·
> D4-33~36 的导出导入/A13/跨项目隔离浏览器实测（代码层守卫已覆盖同一行为）。
> ⚠️ **逐张 L1 必须 `--workers=1` 串行**：多 worker 会因 OnlyOffice 8080 单实例并发 contention 假失败（已实证）。
>
> **未随本批归档（仍 active）**：① `workpaper-html-onlyoffice-bidirectional-writeback-closure` 平台总纲
> 仍有 **5 个 `[-]`**（Tasks 61 / 71 / 72 / 74 / 75），`legacy_fake_bidirectional` 现 **137**，不具备归档条件；
> 其 `requirements.md` / `tasks.md` 末尾已追加 2026-09-22 现状刷新节（§背景 旧快照 185/276/186/43/136 中 5 个数字
> 已过期，现读数 175/265/176/33/132；并登记 D4 lane 与「四类 Excel pilot」不是同一组 —— Task 40 的 checklist entry
> `xlsx/b60/gt-b60-bundle` 现仍 `single_onlyoffice`+`legacy_fake_bidirectional`，**D4 事实上替代它成为第 4 个真双向
> entry**）。② `d-cycle-sheet-bidirectional-expansion` 9/9 但**无 requirements/design**（只有 tasks.md + README），
> 范围是 D1–D7 全循环而非纯 D4，留待单独裁决。
>
> 🔴 **统计方法教训（本轮实际踩到）**：只统计 `[x]/[ ]/[~]` 会漏 **`[-]`**，据此会把总纲误判成「72/72 可归档」。
> 实扫复选框必须覆盖 `[x]` `[ ]` `[~]` `[-]` 四态；另 `d4-inspection-writeback-formula-io` 存在**两代任务编号**
> （草案 1-7 `[ ]` 已被 T1-T7/B1-B3 承接做实），机械统计会得出与实际相反的结论 —— 以其 tasks.md 内的
> 「任务状态归并表」为唯一权威对照。
...
>
> **未入库（`git ls-files` 为空）的 spec 目录**（2026-09-12 实扫，蒸发风险同 browserless）：`custom-workpaper-template-ingestion-and-sync-closure`（15/19）· ~~`workpaper-page-formula-toolbar-closure`~~（**2026-09-25 已 17/17 全绿并 `git mv` 归档至 `_archive/05-business-features/`，35 tracked 文件随迁移入库**）· `workpaper-sync-materialize-large-table-performance` · `audit-evidence-…` 空壳。本日新建的 3 个 spec 已随本次改动一并入库。
...
| spec | 进度 | 一句话 |
|------|------|--------|
| `e1-sync-coverage-and-first-canary` | **0/23** | **E1 货币资金：从 legacy 假双向接成真双向（首张 canary）+ 第一册覆盖（受管区 1→9）**（2026-09-25 新建，三件套齐 0 diagnostics，Design-First 未实施）。🔴 **与四个 D 类 spec 的根本差异：E1 是从零接入**。D 类每家都已有 1 张明细 `bidirectional_verified`（D1-3/D2-2/D3-2/D4-2/D5-2/D6-2/D7-2），而 E1 实测**一张都没接** —— `GtE1MonetaryFund.vue` 只有 legacy `GtOnlyOfficeSheet`，无 `useWorkpaperSyncBridge`/`WorkpaperSyncEditorHost` ⇒ 本 spec **前半段是完整 canary 链路**（provider 从零建 + 契约发布五环 + adapter 注册 + 宿主接桥），相当于补做 `d-cycle-sheet-bidirectional-expansion` 的 Wave 0。<br>**manifest 实测**：entry `xlsx/gt-e1-monetary-fund` 已存在、`independentEntry=true`、`scenarioProfileId=xlsx.editable.shared.single.room_service_wired.v1`（**与 D1/D2/D3 canary 同型** ⇒ 选型门可过）、`roomServiceState=room_service_wired`；但 `migrationState=legacy_fake_bidirectional` + 三条 `reasonCodes`（`template_only_open`/`no_durable_forcesave_ack`/**`missing_adapter`**）⇒ 🔴 **当前所有 E1 sheet 切「在线编辑」都是假双向**（OO 改动不合并回 `checklist_responses`、切回即丢），这是比 D 类更差的起点，也是本 spec 的直接业务价值。<br>🔴 **E 循环 5 册 / 56 sheets**（远超 D 类任何循环；D2 三册是次高）：①`E1-1至E1-11 …常规程序`(16 sheets) ← **本 spec 范围** ②`E1-14至E1-15 分析`(4) ③`E1-18至E1-23 检查`(7) ④`E1-26至E1-32 IPO舞弊`(9) ⑤`E0 函证`(**20 sheets / 987KB**)。`entry↔template blob` 1:1 + 一宿主一 entry ⇒ **只能覆盖第一册**（同 D2 裁决 E1，三替代方案同样否掉）。<br>**第一册几何**：`货币资金审定表E1-1` 47r×10c/**193f**（密度 **41%**，**全平台审定表最高**）· `现金明细表E1-2` 34r×22c/38f（**首张 canary**，结构最简失败面最小）· 🔴 **`银行存款及其他货币资金明细表E1-3` 同编号两张**（`(仅人民币)` 92r×28c/185f 与 `(人民币及外币)` 89r×**41c**/**567f** —— 全平台单 sheet 公式最多，而前端只有一个 `E1-bank-detail-rows`；**D 类从未出现此形态**，裁决 H4 要求先核「引擎能否一 store item 投两 sheet」）· `数字货币明细表E1-4` 22r×17c/52f · `调整分录汇总E1-5` 26r×10c/7f（🔍 可行性核）· `银行存款余额调节表E1-6` 56r×17c/14f · `库存现金盘点E1-7/E1-8` 67r/78r · `E1-9`/`E1-10`/`E1-11` 各仅 7 公式。<br>🔴 **2026-09-25 第三轮复盘：本 spec 首版有三处错误 + 四处缺口，已全部修入三件套。**<br>**错误①（最承重）裁决 H3 + Property 3 整段错** —— 首版按「各只 7 公式」把 `E1-9`/`E1-10`/`E1-11` 三张全判成 `static_region` 强命中并排在 canary 之后第二位。前端实证只有 **E1-11** 成立：`E1-9` 走 `useE1CashCount(variant='cert')`（键 `E1-cash-count-cert-rows`，`addRow`/删行 ×4）、`E1-10` 走 `useE1AccountList`(340)（键 `E1-account-list-rows` + 跨 sheet 读 `E1-bank-detail-rows` + `E1-account-commit-snapshot` 与 E1-11 联动 + **OCR ×43**），两张都是**动态行表**；`E1-11` 零 `-rows` 键（仅 `E1-account-commit` + `-check-summary`）才是唯一 `static_region`。⇒ **模板公式数是 xlsx 侧几何量，与前端是否有动态行无因果关系**。**新增裁决 H8：「禁推演」铁律适用面从键名扩展到形态** —— `store_item_id` 按值 grep / `BindingKind` 按前端三元组`(store 键, addRow 信号, composable 归属)` / `row_identity_key` 按真跑一次，三项都不得几何推演。变异清单第 3 条是**自省变异**（把判据改回公式数阈值），判据必须能打红自己的历史错误。<br>**错误②** 首版记「`useE1CashCount` 实测无 `-rows` 键、形态待核」有误 —— 它的键是**模板化**的 `E1-cash-count-${variant}-rows`，按字面量 grep `-rows` 查不到、按前缀才查得到。**错误③** 首版把 `useE1Adjudication`(725) 标「属第 2/4 册范围外」，它其实是第一册 E1-1 审定表、**在范围内**。<br>🔴 **store 键实测是四种风格不是三种**：①语义 ②**编号嵌套**（`E1-ipo-E1-26-rows`）③**模板化**（`E1-cash-count-${variant}-rows`，variant ∈ `rmb|fx|cert`；副键 `${key}-summary`；legacy 兜底 `E1-cashcount-fx-summary-fx` 双 `-fx` 后缀仍在读）④**无连字符**（`E1-cashcount-audit-note-${variant}`）。最危险是③④同张并存：**E1-8 同时有 `E1-cash-count-fx-rows` 与 `E1-cashcount-audit-note-fx`，`cash-count` 与 `cashcount` 只差一个连字符**。沿用 D3 裁决 F2，该裁决现有**六次事故背书**（D2-3 三键/D1-15 双键/D1-13 双键/D3 语义缩写/D567 三处必错/**E1 四风格 + 一字之差**）。<br>**修正后接入顺序**：`1(canary E1-2) → 2(E1-4 最小行表) → 3(E1-11 唯一 static_region) → 4(E1-6) → 7(E1-7/8/9 共享 variant) → 8(E1-10 首遇 OCR) → 9(E1-3 共享键) → 10(E1-1 + TB 发布门)`。**E1-7/8/9 三张合一个声明文件**（共享 `useE1CashCount` 只差 `variant`，同 D5/D6/D7 只差 `aging_layout` 一参的范式），并产出 `HTML_ONLY_ITEM_IDS_E1`（三张各有 `elements`/`audit-note`/`audit-conclusion` 三个 per-variant 文本键，与 `HTML_ONLY_ITEM_IDS_D45` 形态逐一对应）。<br>✅ **裁决 H5：公式管理不改 —— E1 是平台范式源头**。实证 `GtE1MonetaryFund.vue:256` 的 `openFormulaManager()` 注释 +`D4TabOtherMargin.vue:32` 明写「**同 E1 范式**…平台唯一一套公式：`wp_formula` 表权威存储，后端权威执行 + CAS + 审计」⇒ **D4 是照 E1 做的**，E1 侧已完整。本 spec 只加两条判据：①接 sync 后公式管理入口在**两种渲染模式下行为一致**（Property 8，D 类 spec 都没这条 —— 它们宿主没公式管理入口）②不新建第二个按钮 owner（`workpaper-page-formula-toolbar-closure` 需求 1.4/2.4 红线）。🔴 **两个「公式」概念不在一层**：公式管理（`wp_formula` 表 / `workpaper` scope / 全局弹窗，审计师维护的跨底稿取数公式）vs `formula_mask`（契约声明的模板 Excel 内部公式，materialize 不覆盖）—— 本 spec 只碰后者。<br>🔴 **缺口①（系统性，五个 spec 共有）缺上游锚定** —— 本 spec 实为 umbrella `workpaper-html-onlyoffice-bidirectional-writeback-closure` 的 **Task 47「逐一迁移 E 循环 Excel 独立 entry」**（复选框 `[x]`，正文写「处理账户动态行、币种变体和稳定账号/UUID；防 variant 切换抹零；验证 Property 23/69/70」）的下游 lane spec，与 D4 lane 同型。首版零引用 ⇒ 重复推导已冻结事实、并推出错的形态判定。已存在产物必须复用：**E 循环 slice** `backend/data/workpaper_sync_e_cycle_manifest_slice.json`(322 行，已冻结 `entry_id`/五个 null 供给位/`scenario_profile_id`/三条 `manifest_legacy_reasons`/五条 `unverifiable_reasons`/`not_single_html_because`/`not_bidirectional_because`) · 行身份守卫 `e1SyncEntryRowIdentity.spec.ts`（umbrella **Property 23** 的单点守卫，真跑 `useE1BankDetail`）· variant 金额守卫 `e1BankVariantIntegrity.spec.ts`。⇒ 新增 Task 1 做 **slice 核对**（slice 可能已过期），`assert_entry_selectable` 四条事实不再重新调研。**Property 编号改为 spec-scoped `E1-P{N}`** —— umbrella 有 Property 1–71，其 `Property 3`/`Property 23` 与本 spec 同号不同义；该纪律有上游事故背书（umbrella Task 61 附注记录「全局 `BP-16`~`BP-22` 已被 Tasks 60/63/64 重复占用、同号不同义」，修法是 task-scoped 前缀 `BP-61-x`）。<br>🔴 **缺口②结构性前置：E1 的 `parent_duplicate_count = 0`，D4 是 31** —— slice 原文：E1 的 30+ 子 sheet 由同一宿主按 `currentSheet` 分发，子组件里没有第二个 `GtOnlyOfficeSheet` 挂载点 ⇒ manifest 不产生 `parent_duplicate` 条目。这是**接入机制差异**不是统计差异：「声明层 +1 per sheet」在 E1 上**不足以**让 manifest 扫出受管 sheet。裁决取**路线 α**（仿 D4，子组件各挂挂载点，改动面大但全在前端声明层、工具链全复用），否决**路线 β**（改 `_entry_id` 派生规则 —— 持久化键，裁决 H2 已否决同类改动，且会让 E1 成为全平台唯一例外）。<br>🔴 **缺口③ OCR 是 E1 独有的第二写入方（裁决 H10 / E1-P16）** —— 实测 e1 目录 **7 个 OCR 确认弹窗 / 提及 454 次**，而 **d4 是 0 个 / 0 次**（d5/d6/d7 全 0，d1/d2/d3 仅 66/46/34 次零散提及无确认流）。⇒ 受管 sheet 的 `-rows` 键有**两个批量写入方**（OCR 整表替换 + OO forcesave），D4 的冲突模型只有「HTML 单格 vs OO 单格」**不含批量覆盖**，且 OCR 正落在要接的 E1-10(×43)/E1-11(×43) 上。裁决：**受管 sheet 处于 OO 编辑态时 OCR 确认入口禁用**+中文原因，不做字段级三方合并（`E1*OcrConfirmDialog` 只有整表确认粒度、无 per-field 溯源）。否决「OCR 后自动 forcesave」（OO 未持编辑态时写脏）与「OCR 为权威覆盖 OO」（未保存编辑静默丢失）。<br>🔴 **缺口④ TB 显式发布门（裁决 H9 / E1-P17）** —— `E1TabAdjudication.vue:336` 有 `data-testid="e1-publish-tb"`、`:338` 绑 `publishToTb`，E1-1 是平台 `tb-writeback-explicit-publish-gate` 的接入方（同型遍布 F1~F5/G1~G14/H1~H10/D2）。三条红线：①sync 回写路径对 `trial_balance` 写次数为 **0** ②未点发布前 `trial_balance` 不变 ③两道 CI 守卫 `check_tb_writeback_no_direct_call`/`check_tb_publish_confirm_gate` 保持绿。**D 类四个 spec 均无此条** —— E1-1 是第一次「受管 sheet 与 TB 发布门落在同一张底稿」。<br>🔴 **E1-3 风险从「待裁决」升级为数据损坏级（裁决 H4 强化 / E1-P18）** —— `useE1BankDetail.ts:9` `BankDetailVariant='rmb'|'multi'`、`E1TabBankDetail.vue:35` variant **由 sheet 名推导**、`:61` `VARIANT_KEY='E1-bank-variant'`（variant 选择本身持久化）、`:110` 注释明写「两 variant 字段集不同是 AC 1.9 的意图」—— `rmb` 版**不下发**原币列。两 variant 共用同一个 `E1-bank-detail-rows` 但**列集不同** ⇒ 两张同时受管时 OO 在 `rmb` sheet forcesave 会把 `multi` 侧原币列抹零，**正是既有守卫 `e1BankVariantIntegrity.spec.ts` 在守的缺陷形态**（只是从 OO 回写这第二个方向引入）。裁决：**默认只接 `(人民币及外币)`**（`multi` 是列集超集），两张都接的硬前置是**先拆键**而非让引擎支持「一 store item 投两张列集不同的 sheet」。<br>✅ **复盘查明的正面事实**：`E1TabDirectory.vue` 19 编号 × note/conclusion = **38 键，逐键反查写入方零缺失** —— 与 D6/D7 四键零写入（完成度恒「未填」）正好相反 ⇒ **不需要** D567 spec Task 1 那类修复。⚠️ 反向教训：其中 10 个是模板化键，**查「零写入键」必须同时匹配模板化拼接**，否则会造出不存在的 bug。<br>🔴 **吸收 D4 批量 e2e 范式（新增 Task 12）** —— 照搬 `d4-l2-oo-to-html-all.spec.ts`(541)/`d4-l2-cases.json`(389)/`seed_d4_publish_e2e.py`(395) 产出 `e1-l2-cases.json`+`e1-l2-oo-to-html-all.spec.ts`+`seed_e1_publish_e2e.py`。四条纪律：**逐张而非批量**（UI forcesave 卸编辑态；批量多格曾触发 `footer_anchor_drift`/conflict）·**只改安全目标**（`|amount|>1` 且 key 不在 `SKIP_KEYS`=`seq/index/row_no/no/order/month`；文本/空记 `no_safe_target`）·**API 直打后端 9980** 绕 vite 代理（避免 `ECONNREFUSED`）·**七态枚举** `applied_store_ok/applied_store_miss/no_safe_target/type_fail/op_error/op_timeout/enter_fail`（区分「引擎错」与「无可安全编辑的格」）。E1 需加两个 D4 没有的 fixture 字段：`variant` 与 `ocr_dialog`；**E1 的 seed 须额外解除 `missing_adapter`**。<br>**9 Req / 18 Property / 23 Task / 8 阶段 / 17 wave**（复盘后，原 15 Property / 18 Task / 7 阶段 / 15 wave）。<br>📌 **触类旁通：调整分录汇总表从七张变八张** —— `E1TabAdjustment.vue` 接 `useAdjustmentCentralSync` ⇒ `E1-adjustment-rows` 与 D1-5/D2-4/D3-3/D4-4(已判 `single_html`)/D5-3/D6-4/D7-3 同型。E1-5 的加入证明**它不只是 D 类问题** ⇒ 统一裁决 spec 建议从 `d-cycle-adjustment-sheets-single-html-adjudication` **改名为 `cycle-adjustment-sheets-single-html-adjudication`**（去掉 `d-` 前缀）。<br>📌 **E 循环模板治理债（登记不处理，比 D6/D7 各 1 张残留严重得多）**：E0 函证册 **9 张**残留/参考 sheet（`函证程序表-原版本备份`/`参考用-往来函证程序`/`函证结果汇总表E0-1（原）`/`（备份）`/`回函情况汇编`/`…-旧版`×2/**`核实被函证单位信息F1-10-原`**/**`邮件传真回函核对记录F1-12`** —— 后两张 **F1 编号出现在 E0 册**）· **`E0-5` 编号重复两张**（`应付银行承兑汇票发函记录表E0-5` / `银行函证其他信息核对表E0-5`，接第 5 册前必须先裁决）· **`货币资金分析表F1-6 (修订前)`** 在 E1-14 册里（**F1 编号出现在 E1 册**）· 范围外还有全平台公式最多的 `存款规模与利息收入匹配性分析E1-30`（389r×22c / **1187 公式**）。这些残留会让 sheet 分派正则、generated 映射、将来受管清单都面临「同名/异循环编号」歧义，建议模板治理单独立项。 |
| `d567-sync-coverage-via-row-table-engine` | **0/23** | **D5/D6/D7 三循环受管覆盖扩容（各 1 → 合计 20 受管区）+ 修跨循环底稿目录聚合键缺陷**（2026-09-25 新建，三件套齐 0 diagnostics，Design-First 未实施）。**三家合一 spec 是用户 B 方案裁决**，依据实测同型度最高：同一套 33 函数 provider（D5 781 / D6 849 / D7 844 行，函数名逐字相同）+ `FORMULA_MASK` 均列向 + 差异只在 `aging_layout` 一参（**nested**=D7 / **flat**=D6 / **无**=D5）+ 各自只接 1 张明细。分三份写会八成样板重复，且三家恰构成引擎三条账龄路径的完整样本集。消费上游 `d1-sync-row-table-engine-and-d1-coverage` + `d-cycle-sheet-bidirectional-expansion`（其 **Wave 5 = D5-3/D6-3/D7-3** 正是本 spec 作业面）。<br>**模板实测：三家全单册**（D5 8 sheets / D6 15 / D7 13）⇒ 三 entry 并行、**爆炸半径彼此隔离**，一家挂了不影响另两家（裁决 G1：受管区计数/materialize 实测/灰度开关按循环独立）。<br>🔴 **裁决 G3：三处「按模式推演必错」的实测值** —— ①`审定表D5` **无 `-1` 后缀**（推演 `审定表D5-1` 会 sheet 找不到、attach fail-closed）②D6-8 的键是 **`D6-8-single-rows`** 不是 `D6-8-rows` ③D7-4/D7-7 是双键不是聚合键。四个聚合键（`D6-6-rows`/`D6-8-rows`/`D7-4-rows`/`D7-7-rows`）**全仓零写入点**，误用会让整个受管区投影恒空。该裁决有**四次事故背书**（D2-3 三键 / D1-15 双键 / D1-13 双键 / D3 语义缩写键名），Property 2 专项钉住。<br>🔴 **需求 5 / Task 1：修跨循环底稿目录聚合键缺陷（独立交付、不依赖任何前置）** —— `D6TabIndex.vue:54/:56` 与 `D7TabIndex.vue:50/:56` 的 `hasJsonRows()` 读那四个零写入点聚合键 ⇒ 这四张底稿完成度**恒显示「未填」**。与 D1 那个「四处拼锚点三处拼错、测试镜像同款错误恒绿而生产恒死」同型 ⇒ **判据必须以真实写入方的键播种、不得镜像错误键名**（裁决 G4）。<br>**裁决 G2：D6(flat) 与 D7(nested) 同批推进**作引擎两条账龄路径对照 —— 上游把四家逐字相同的 `sorted(SCALAR + _aging(), key=_col_index)` 收进 `managed_field_specs()` 并按 `aging_layout` 分派，D6 是 flat **唯一**样本；同批使差异一旦出现能立即归因到该参数而非别处。<br>**裁决 G5：三张审定表 `sections`/`row_mode` 全部待实测** —— 已证四循环审定表形态互不相同（D1-1 三区×动态票据种类 / D2-1 一区×写死4行+SUMIF / D4-1 两区×动态行+四态 / D3-1 待实测），本 spec 三张公式数 52 / **177**（D6-1 密度 **32%**，四循环最高）/ 84，形态大概率也不同；**不得**在引擎加 `if is_d5/d6/d7` 分支。<br>🔴 **裁决 G6：三家 `adapter_registered` 全 False**（真库 `register_from_manifest()` 只注册 `{d2,d4,g7,h1}`，D5/D6 store 全库 0 行、无 current published representation）⇒ 真栈整册 materialize 跑不起来；处置同 D3：代码与合成判据照常推进，真栈判据如实标 `[ ]*` 写明卡点，**不得**以合成测试冒充真栈。<br>**另两处模板事实**：D6 册里有 `合同资产实质性程序表 D7A（原）`(104r)、D7 册里有 `合同负债实质性程序表 D8A（原）`(66r) —— 历史残留 sheet 命名与所属循环不符，须显式排除并确认分派正则不误判（Property 12）。<br>**受管区路线（三家独立）**：D5 `1→2(D5-4)→3(审定表D5)` · D6 `1→2(D6-3)→3(D6-5)→5(D6-6双区)→6(D6-8)→8(D6-9双区)→9(D6-1)` · D7 `1→3(D7-4双区)→4(D7-5)→5(D7-6)→7(D7-7双区)→8(D7-1)`。每家先单区后双区最后审定表。**8 Req / 15 Property / 23 Task / 7 阶段 / 12 wave**。<br>📌 **触类旁通闭环：七张调整分录汇总表全部同型**（D1-5 `D1-entry-rows` / D2-4 `D2-entry-rows` / D3-3 `D3-aje-rows` / D4-4 已判 `single_html` / D5-3 / D6-4 / D7-3，各自宿主均接 `useAdjustmentCentralSync` 3 处 ⇒ 经后端 `AdjustmentSyncService` 中央登记、均为 hub store）⇒ **建议统一裁决另立 `d-cycle-adjustment-sheets-single-html-adjudication`**，避免在五个 spec 里重复五遍同一判据。另 D6-7 减值准备会计政策检查（58r×18c/仅 7f）做形态核：候选 `paragraph_block_bidirectional`（D4-5 范式）或 `single_html`。<br>**D 类全景（本 spec 完成后 D1~D7 各有三件套）**：D1 `d1-sync-row-table-engine-and-d1-coverage`(含框架层, 1→22~23) · D2 `d2-sync-coverage`(1→5) · D3 `d3-sync-coverage`(1→8) · D4 `d4-html-to-oo-store-contract-alignment`(bugfix, 已有 30 受管 sheet) · D5/D6/D7 本 spec(1→3 / 1→9 / 1→8)。 |
| `d3-sync-coverage-via-row-table-engine` | **0/17** | **D3 预收账款受管覆盖扩容（受管区 1→8）**（2026-09-25 新建，三件套齐 0 diagnostics，Design-First 未实施）。消费两个上游：`d1-sync-row-table-engine-and-d1-coverage`（引擎/注册表/`AdjudicationSheetSpec`/四态状态机）+ **`d-cycle-sheet-bidirectional-expansion`（9/9 全绿，D 循环扩容的裁决与分波真源）**，继承其四条纪律（一 entry 一 adapter · 诚实边界 · 可行性核硬门「有行身份列+无专用同步链冲突」· D4-4 已判 `single_html` 的判据）。<br>**模板实测：单册 12 sheets**（与 D1 同，不同于 D2 的三册）。接入 6 张：`审定表D3-1`(30r×12c/**88f** 密度 24% ⇒ 逐格 `AdjudicationSheetSpec`)·`预收账款分析表D3-4`(38r×9c/22f **双区**)·`账龄1年以上的预收账款检查表D3-5`(20r×8c/9f)·`关联关系及交易检查表D3-6`(34r×12c/16f 首张最简)·`预收账款检查表D3-7`(48r×18c/19f **双区**)+ 已接 `预收账款明细表D3-2`(d32-managed)。不接：底稿目录/D3A 程序表/附注披露×2/GT_Custom。<br>🔴 **裁决 F2：`store_item_id` 必须逐个按值 grep 实测，禁止按编号推演** —— D3 的键用**语义缩写**与 sheet 编号无对应关系：`D3-det-rows`(D3-2) / `D3-aje-rows`(D3-3) / `D3-ana-credit-rows`+`D3-ana-debit-rows`(D3-4) / `D3-lt-rows`(D3-5) / `D3-rp-rows`(D3-6) / `D3-vc-current-rows`+`D3-vc-post-rows`(D3-7)，与 D5/D6/D7 大多用编号的惯例不同。该裁决有三次事故背书（D2-3 三键/D1-15 双键/D1-13 双键的遗漏全源于按模式推演），Property 2 专项钉住。<br>**裁决 F3：D3-1 的 `sections`/`row_mode` 待实测，不照 D1-1（3 区×动态票据种类）或 D2-1（1 区×写死 4 行）推演** —— 三循环审定表形态实测互不相同，这正是 `AdjudicationSheetSpec` 要参数化的维度；**不得**在引擎加 `if is_d3` 分支（会让上游 AST 卡点打红）。<br>🔴 **裁决 F5：`adapter_registered=False` 是 D1/D2 没有的真实卡点** —— 真库 `register_from_manifest()` 当前只注册 `{d2,d4,g7,h1}`，D3 因 `D3-det-rows` 全库 0 行、无 current published representation 而**未注册成功** ⇒ 整册 materialize 真栈实测跑不起来。处置：代码与合成判据照常推进，真栈判据如实标 `[ ]*` 写明卡点，**不得**以合成测试冒充真栈。<br>**裁决 F4 + 触类旁通闭环**：D3-3 走可行性核（`D3TabAdjustment.vue` 接 `useAdjustmentCentralSync` **3 处** ⇒ hub store）；实证**六张调整分录汇总表全部同型**（D1-5 `D1-entry-rows` / D2-4 `D2-entry-rows` / D3-3 `D3-aje-rows` / D4-4 已判 / D5-3 / D6-4 / D7-3，宿主均接中央同步）⇒ 建议统一裁决另立 `d-cycle-adjustment-sheets-single-html-adjudication`，不逐循环重复核。<br>**受管区路线**：`1(D3-2) → 2(D3-6) → 4(D3-4 双区) → 5(D3-5) → 7(D3-7 双区) → 8(D3-1)`。**8 Req / 12 Property / 17 Task / 6 阶段 / 12 wave**。<br>📌 **顺带发现（登记不处理）：D6/D7 底稿目录完成度判定引用无写入方的聚合键** —— `D6TabIndex.vue:54` 读 `D6-6-rows`（真实写入方用 `D6-6-block1-rows`/`-block2-rows`）、`:56` 读 `D6-8-rows`（真实 `D6-8-single-rows`）、`D7TabIndex.vue:50` 读 `D7-4-rows`（真实 `D7-4-credit-rows`/`-debit-rows`）、`:56` 读 `D7-7-rows`（真实 `D7-7-period-rows`/`-post-rows`）；四键全仓**零写入点** ⇒ 这四张在目录里完成度恒显示「未填」。与 D1 那个「四处拼锚点三处拼错、测试镜像同款错误恒绿而生产恒死」同型。D3 侧未见同类，建议随 D5/D6/D7 spec 一并修。 |
| `d2-sync-coverage-via-row-table-engine` | **0/18** | **D2 受管覆盖扩容（1→4 张）+ 两处死代码清理**（2026-09-25 新建，三件套齐 0 diagnostics，Design-First 未实施）。是 `d1-sync-row-table-engine-and-d1-coverage` 的**消费方**（引擎/注册表/`AdjudicationSheetSpec`/四态状态机由它交付），Task 0 前置门 = 「D1 spec 框架层已入 HEAD」。<br>🔴 **最关键结构事实：D2 是三册模板，不是一册**（与 D1 单册 21 sheet / D4 单册 46 sheet 根本不同）。实测 `backend/wp_templates/D/` 有 `D2-1至D2-4  应收账款- 审定表明细表（Leap-常规程序）.xlsx`(11 sheets) / `D2-5  …分析程序.xlsx`(4) / `D2-6至D2-13  …检查.xlsx`(12)；而 **entry ↔ template blob 是 1:1**（`TEMPLATE_RELATIVE_PATH` 单路径）且 `generate_workpaper_sync_manifest._entry_id(document_type, source_file)` **从宿主文件路径派生** + `seen_entry_ids` 碰撞检查 ⇒ **一宿主恰一 entry**，而 D2 的 16 张 sheet 全在同一宿主 `GtD2AccountsReceivable.vue` 按 tab 分流 ⇒ **D2 只能覆盖第一册**。三个替代方案均否掉：改 `_entry_id` 派生规则（它是持久化键，`working_paper_sync_entry_state`/`workpaperSyncModeKey`/room key 全用它，改则既有状态全失配）/ 合册（权威模板是审计方法论产物且 `template_sha256=31e7992b…` 已冻结在契约）/ 现在建新宿主（跨前端路由+`htmlRendererRegistry`+manifest 生成+两条完整发布链，应单独立项）。<br>**第一册几何实测（openpyxl 直读）**：`审定表D2-1` 62行×13列 **311 公式**（密度 **38%**，主列 E43 I40 J39 K39）⇒ **逐格 mask**，走 `AdjudicationSheetSpec` · `明细表D2-2` 35行×39列 107 公式（已接，`d22-managed`）· `坏账准备明细表D2-3` 27行×14列 69 公式（主列 **E13 K11 N11**，计数≈数据行数 ⇒ **列向**）⇒ 行表引擎 · `调整分录汇总表D2-4` 25行×10列 **仅 6 公式** ⇒ 最简形态基线 · 另有 底稿目录 / D2A 程序表 / **附注披露×4** / GT_Custom **不接**。模板原始态**无任何 Excel Table**（Table 由 instrumentation 注入，正常）。<br>**5 条裁决**：E1 只扩第一册（后两册另立 `d2-analysis-and-inspection-sync-hosts`）· 🔴 **E2 已于 2026-09-25 复盘反转：D2-3 是三受管区**（实测 `useD2BadDebt.ts:64-68` 的 `CATEGORY_KEYS` dict = `D2-bd-individual-rows`/`D2-bd-aging-rows`/`D2-bd-customer-rows` **三个独立 store 键**，与 D1-4 同型；**首版写成「单受管区」并立了反向判据**，根因是首轮 grep 用 `^const \w+_KEY\s*=\s*'` 匹配不到 dict 字面量里的键值 ⇒ 据不完整事实下了相反裁决。连带三个后果首版全漏：①D2-3 成为**同 sheet 多受管区**须走兄弟 Table ref 位移路径、依赖 D1 spec 任务 24 先落 ②binding 序列 `1→2→3→4` 改为 **`1→4→5→6`**（受管 sheet 4 张 / 受管区 6 个）③三键有 **5 处下游消费方**（`useD2Adjudication.eclCrossValidation`/`useD2Ecl.d3BadDebtTotal`/`useD2WriteoffCheck.reversalConsistencyWarning`/`useD2DisclosureNote.badDebtSummary`+`importIndividualFromBadDebt`/`useD2CrossSheet`）零回归须覆盖，只验自身读回等值会放过「下游看不到回写」。**教训固化：store 键 grep 必须同时覆盖 `= '常量'` 与 `= { ...: '值' }` 两形态，宁可用按值匹配 `'D2-[a-z0-9-]+-rows'` 兜**）· E3 D2-1 以 `sections`(1 vs 3) + `row_mode`(`fixed_rows` vs `dynamic_identity`) 参数表达与 D1-1 的差异，**不得**在引擎加 `if is_d1`/`if is_d2`（会让 D1 spec 的框架层 AST 卡点打红）· **E4 删代码不删数据** · **E5 非受管 sheet 保持禁用不引入 legacy 假双向**（D1 落 `GtOnlyOfficeSheet` 那条 `migration_state=legacy_fake_bidirectional`、OO 改动不合并回 store 切回即丢；D2 现状直接禁用+中文原因**更正确**，不能为「对齐 D1」而退化）。<br>✅ **O5 已查明（不再待业务确认）**：`useD2VoucherCheck`（旧套）**全仓零消费方** = 死模块 310 行，前端实际挂载 `Enhanced`；真库 `D2-voucher-params`(1行87B `{"method":"随机","populationSize":0,…}`) / `D2-voucher-samples`(1行332B 全空骨架行，同 wp `e2c95d10`) 为**空骨架/默认值**，新套 `D2-vc-current-rows`/`-post-rows` **全库 0 行** ⇒ 删代码 + 保留读兼容。另删 `useD2Adjudication:182-188` 死降级路径（逐行读 `D2-detail-{i}-{field}`，`useD2Detail` 只写 `D2-detail-rows` ⇒ 零写入方恒返 0）。<br>🔴 **性能是一等约束不是附带项**：`D2-detail-rows` 真库 **4 行 / 3,061,466 字节**（平均 765KB/行）；D2-2 模板 **35 行** vs 真库 **1260 行** ⇒ 单张 materialize 要插 **1200+ 行**；离线剖析 729 行 **59.3s**（超线性），走统一 materialize 曾 HTTP **>300s 超时**。⇒ 每接一张实测整册 materialize 耗时，**超软上限即停并转性能 spec，不带退化铺量**。判据纪律：**不得**用 `store_field_count` 与 `field_count` 作差推断数据丢失（D4 spec 曾误判 1648 vs 992）。<br>**前端接线**：`entryId` 已是 `Ref`（桥内运行时取值），但 `capability: capabilityForEntry(字面量)` 与 `flushHtml` 闭包内 `entryId`/`sheetKey` 是**构造时字面量** ⇒ 改读 ref；`isD2DetailSheet`(`currentSheet==='D2-2'`) → 按**从 provider 受管清单派生**的集合判定，不前端硬编码。Q15 必测「连续切两张受管 sheet 都能进 OO」（同 entryId 不同子 sheet 的 `store-projection`/`materialize` 是同一 URL、http.ts 去重层会 abort 在飞请求，该坑已在桥内处理但须真栈复验）。<br>**8 Req / 17 Property（含复盘新增 Q4b 下游联动）/ 18 Task / 6 阶段 / 12 wave**。真栈三陷阱沿用 D4 结论（不能用 `page.on('response')` 判 callback；不能用 `asc_*` API 写格；模式切换条选择器须实测）。<br>📌 **顺带发现（登记不处理）**：① 四张附注披露 sheet **跨册重复**（第一册 4 张、第二三册各有前 2 张同名副本）⇒ 同一披露表多个物理载体，将来接须先裁权威 ② 披露表命名陷阱：源 tab 名 `附注披露信息（国企）D2-1`，先跑 `/D2(?:-\d+)?[A-Z]?$/` 会判成 `D2-1` 审定表 ⇒ 披露组件永远挂不上（`d2Constants.ts` 已有注释 + 2026-07-30 Playwright 实测过）③ 每册都有 `GT_Custom`(8行×2列/0公式) 须确认 instrumentation 不误算进受管清单 ④ `lock_room_oo_apply` 是 per-room 会话级锁横跨整个 CPU 段 ⇒ D2 多人同编同底稿串行排队（既有事实，归性能 spec ROI-6）。 |
| `d1-sync-row-table-engine-and-d1-coverage` | **0/35** | **双向回写行表引擎泛化 + D1 全覆盖**（2026-09-25 新建，三件套齐 0 diagnostics，Design-First 未实施）。用户裁决：**D1/D2/D4 各自独立成套（像 D4 现在这样）**，顺序**先 D1 再 D2**。<br>**红基线（全部实测）**：7 家 provider **33 个函数名逐字相同**（约 5600 行复制 6 份：D1 902 / D3 847 / D5 781 / D6 849 / D7 844 / D2 1361）；几何常量 **6 个 100% 同构**（`TEMPLATE_ID`/`FIRST_DATA_ROW`/`LAST_DATA_ROW`/`FOOTER_ROW`/`UUID_COL`/`TABLE_NAME`，D1/D4 多一个可选 `HEADER_ROW`）；`FORMULA_MASK` **7 家形态一致**（全列向区间 `{COL}{FIRST}:{COL}{LAST}`，无一逐格）；D2/D3/D6/D7 的 `tuple(sorted(SCALAR_FIELD_SPECS + _aging_field_specs(), key=_col_index))` **逐字相同**、`_snake`/`_col_index` **各四份复制**；`MANAGED_FIELD_SPECS` **D5 是 7 元组**（内联 `group_header_cell`）其余六家 6 元组 + 独立 `GROUP_HEADER_CELLS` mapping ⇒ 同语义两表达；`oo_to_html.py` **89 处 D4 提及 + 9 分支 `elif adapter_id ==` 链 + 9 处 `hasattr`**，`excel_extract` 8 处 `contract_id` 分支 / `merge` 5 / `adapters/excel` 2 / `excel_materialize` 1；**D1 受管覆盖 1/21**（模板 openpyxl 直读 21 张 sheet，仅 D1-3 受管），D4 是 **30 受管 sheet / 36 binding / 18 个 `_INCLUDE_*` 开关全 True**。<br>**三层架构**：框架层（全平台唯一实现，**零 wp_code 分支**，AST 卡点守）/ 循环层（每循环一 entry，≤150 行声明；D2 放宽 ≤300）/ sheet 声明层（每张一薄模块 + 灰度开关）。**不合并 entry** —— materialize 是整册的（逐 binding 跑 + verify 逐 binding 全跑，任一失败整册 500）、`lock_room_oo_apply` 是 per-room 锁横跨 23–99s CPU 段。<br>**成功先例照抄**：`phase5_transposed_sheet.py`(500) + `transposed_registry.py`(32) 已把 D4-29 从单例泛化成「引擎 + 两份 spec」且逐字节零回归。**已有框架件复用不重造**：`_align_specs_to_sibling_tables`(:1139) / `_sibling_identity_bindings`(:1256) / `_static_region_bindings`(:1293) **三者已接受 `provider=` 参数**；`_attach_sibling_bindings` 的 D4 特化**只有一行 import** ⇒ 泛化=提成参数、函数体不动。<br>**6 条裁决**：D1 AST 卡点守框架层纯净 · D2 sibling binding 泛化不重写对齐规则 · **D3 审定表不进行表引擎**（实测三循环审定表形态互不相同：D1-1 三区×动态票据种类 per-cell 锚点 / D2-1 **写死 4 行**×SUMIF per-cell 锚点 / D4-1 两区×动态行+四态状态机 ⇒ 差异大于共性，另立 `AdjudicationSheetSpec`，共享的是前端四态状态机而非后端投影引擎）· D4 `rowType`+`source` **两维并存**（正交：`summary` 在 source 无对应、`tb` 在 rowType 无对应；冗余 `isFixed` 收敛掉）· D5 灰度开关逐张接入 · D6 零回归门用 **canonical JSON sha256**（8 contract × 3 = 24 digest）但 **materialize 段必须真跑穿过 `verify_unmanaged_regions`**（D4 spec 教训：判据只覆盖 adapter 两方法 ⇒「判据绿而生产 500」）。<br>**D1 六批次**：①D1-2 + D1-4（**三区**）②D1-8/D1-16（各双区）+ D1-5 ③D1-9/10/11/12/15 ④D1-7（嵌套 dict）+ D1-14/D1-13（标量）⑤D1-6（真二维矩阵，评估 `TransposedSheetSpec` 能否表达）⑥D1-1 审定表（per-cell→行数组双读单写 + 四态状态机修「cross-sheet 无条件盖掉手工值」静默丢数据）。**范围外**：D1A 程序表 / 附注披露两张 / 底稿目录 / 提示页。<br>**白拿能力**：D1-4/D1-8/D1-16 同 sheet 多区直接复用已修的三层（兄弟 Table ref 位移 + `_GT_SYNC` footer 重冻结 + `CompositeRowShift` 累积归一化），且 `test_sibling_table_ref_row_shift.py` 判据 6 清单从 `instrumentation_specs()` **动态算**（实测 30 受管 sheet 中 5 张多区）⇒ 需扩成 provider 参数化即自动覆盖。<br>**性能只承诺不劣化**（≤基线 110%，脚本现测）；三端点实测 store-projection 37–46s / pending-mutations 24–33s / materialize 23–99s 的根因归两个性能 spec。**每批次接完必跑整册 materialize 实测耗时，超软上限就停下转性能 spec**，不带退化铺量。<br>**10 Req / 18 Property / 35 Task / 10 阶段 / 19 wave**（Task 0 = 前置依赖入库核查）。<br>🔴 **2026-09-25 复盘按值 grep 重测 D1 全部 store 键，补正两处受管区计数（与 D2-3 同源错误）**：**D1-15 是双区**（`D1-ecl-individual-rows`/`D1-ecl-portfolio-rows`，实测 `useD1EclCalc.ts:208-209` 的 dict 形式，首版写成占位「ECL rows」按单区处理 ⇒ 另一键数据在 OO 里会不可见、回写丢失）、**D1-13 是双区**（`D1-sampling-vouching-rows`/`D1-sampling-specific-samples`，首版只写「2 行表」未列键名）。⇒ **受管区总数首版从未算过**，tasks 里的 binding 增长数按「一 sheet 一区」估的全部偏低。重算路线：`1 → 2(D1-2) → 5(D1-4 三区) → 10(批次2) → 16(批次3 含 D1-15 双区) → 19(批次4 含 D1-13 双区) → 20~21(批次5) → 23~24(批次6 D1-1 三区)`。**教训固化：store 键 grep 必须按值匹配 `'D1-[A-Za-z0-9_-]+'`，`^const \w+_KEY\s*=\s*'` 这类按声明匹配会漏掉 dict 字面量里的键**（本轮这一条模式共抓出 3 处遗漏：D2-3 三键 / D1-15 双键 / D1-13 双键）。<br>🔴 **2026-09-25 复盘补两条需求 + 两条 Property（D1-4 接入的隐藏冲突面，首版全漏）**：**需求 5.7 / P17 第三写入方定序** —— `useD1WriteoffCheck.syncReversalToD14` 会**回写** `D1-bd-portfolio-rows` 的「按组合计提」父行（其注释写明「D1-4 的期末未审随之重算，并沿 D1-4 → D1-1 → 披露 → 附注 逐级联动」）⇒ D1-4 接 sync 后该键有**三个写入方**（HTML 保存 / OO 回写 / 跨 sheet 回写），若不定序会绕过 sync 的 CAS 直写、与 materialize 产物分叉（下次 extract 反读到非预期值 ⇒ roundtrip 门红或静默覆盖 OO 改动）。裁决方向：OO 模式期间禁用该入口给中文原因，或改走 pending-mutations 通道，**不得**两条路同时直写。**触类旁通已 grep：D1-4 是全仓唯一此情形** —— D2 侧 `useD2WriteoffCheck.reversalConsistencyWarning` 只读不写（仅告警）。**需求 5.8 / P18 下游 computed 零回归** —— D1-4 三键下游有 `useD1EclCalc.d1_4DataAvailable`(:475)+`parseD1_4Rows`(:497/:499) / `useD1Adjudication` 坏账区 / `D1TabIndex.vue:49` `progressKeys`，只验自身读回等值会放过「下游看不到回写」。两者并入任务 26 作两个必须同波交付的子目标（不单列以免全量重编号）。<br>🔴 **Requirement 10：前置依赖必须先入库**。调研中一度把 `merge._protection` 登记成「已修复」，复核 `git show HEAD:…/merge.py` 后发现**修复只在工作树、HEAD 仍是只比列旧实现**，判据 `test_masked_cell_protection_is_cell_level.py` 仍 `??` 未跟踪 ⇒ 立 Task 0 作开工第一件事，判定**必须用 `git show HEAD:`不读工作树**。未入库则批次 6（D1-1 逐格 mask）阻塞，批次 1~5 不受影响（明细类 mask 全列向区间、行范围恰等数据区 ⇒ 只比列与格级判定等价）。<br>📌 **顺带发现两处文档过时（不改原文只登记指针）**：① `d4-html-to-oo-store-contract-alignment/tasks.md` 任务 18 的「`merge._protection` 修法待拍板、本轮未改」**部分过时** —— 工作树已改格级判定（`cell_in_ranges` + `_mask_spans_data_column`，`merge.py:663/761`）且 33 判据实跑全绿、`column_in_ranges` 只剩 `contracts.py:1150` 一处反向校验（语义正确），但**尚未入库**（见上 Req 10）② `instrumentation_specs()` 中 D4-1 那条「默认不接 …… 2 spec 会打挂整个 entry」**已过时** —— `_INCLUDE_D41_ADJUDICATION_INSTRUMENTATION` 实测 `True`。<br>📌 **待 D2 spec 确认（已登记防遗忘）**：D2-7 **两套并存**（`useD2VoucherCheck` 旧套 `D2-voucher-params`/`-samples` vs `Enhanced` 新套 `D2-vc-current-rows`/`-post-rows`），删旧套前须查真库行数 + 业务确认；`useD2Adjudication:182-188` **死降级路径**（逐行读 `D2-detail-{i}-{field}`，全仓零写入方 ⇒ 恒返 0，主路径 JSON 正常故非活跃 bug）。 |
| `excel-structural-row-insertion-and-shift-aware-verification` | **21/22**（2026-09-25 首次登记；此前本表**从未收录**）。Excel 结构化插行 + 位移感知校验。**只剩 Task 20 变异脚本，且不是「没做」而是「归属冲突让路」** —— 5-lane 分工书 §2 把 `backend/scripts/diagnose/mutate_*.py` 判给 **E 泳道**，B 只交清单与预期态。清单已实测 **28 条全 RED**（实施中从 6 条长到 28），基线 `137 passed`，交接件 `docs/operations/excel-row-insertion-mutation-handoff.md`。<br>🔴 该变异检验**捞出过一个真缺陷（M28）**：首轮 GREEN 暴露「T4 只调 `apply_plan_zip_with_report`，从未走完整 `materialize_projection`」⇒ `assert_shifted_footer_gates` 调用点一次都没被执行、两个新参数在生产链路上是**零消费死参数**。补 `TestMaterializeProjectionRunsTheWholeChain` 后转 RED。<br>另两条四态判读教训：①`errors` 必须计入「红」（变异写出语法错时 pytest 报 `N error` 而非 `N failed`，只数 failed 会把「模块没跑起来」判成 GREEN）②mutation 本身必须语法合法，否则 RED 与 WRONG-TEST 分不开。遗留给 E：`--check-anchors` 独立子命令 + M15/M28 锚点含 `\n`（CRLF 下跨行锚点必 MISS）。 |
| `excel-template-override-layer-and-onlyoffice-template-editor` | **25/27**（2026-09-25 首次登记）。模板覆盖层 + OnlyOffice 模板编辑器。两条 `[-]` 都是**已取证后的诚实保留**，不是待办：<br>**T103（xlsm 放开评估）**：两条路径都已真跑取证 —— ConvertService 往返 + Playwright 真开 DocEditor 编辑保存，样本 `B30-2B …（2021年6月）.xlsm` 的 `vbaProject.bin` **52224 字节 / sha `abb2c4470223b7a1` 逐字节保留**。**但同时发现另一处真损失：DocEditor 保存路径把 `printerSettings*.bin` 全丢**（K11 xlsx 7 sheet 实测 zip 部件 37→27、`printerSettings` 7→**0**、`pageSetup` 的 `r:id` 7→0；而 `pageSetup` 元素本身 7→7 保留、跨 sheet 引用 291→291 无损；对照 ConvertService **不丢**）。**已裁决不还原** —— 丢的是 DEVMODE（模板作者当年那台打印机的驱动私有结构），业务语义（纸张/缩放/方向/页边距/页码/打印区域）全在 `<pageSetup>` 属性里被完整内联保留，实测**真差异 0 项**；搬回来要给 `pageSetup` 补 `r:id` + 重建 `worksheets/_rels` + 改 `[Content_Types]`，属**内容级改写**，风险远大于收益。替代方案**已实现**：`wp_template_override.diff_page_setup()`（11 项业务属性参与比对 / 4 项设备绑定属性刻意排除，两集合不相交由判据锁死 + 默认值归一），经 `SaveResultOut.page_setup_changes` 回前端 + callback 逐条 WARNING。⇒ xlsm 维持保守排除，标 `[-]` 而非 `[x]`。<br>**T23（变异脚本）**：同上让路 E 泳道。D 已用一次性 tmp 脚本真跑验过 **RED**（三信号全命中、还原后 md5 一致、还原后回绿）并按规则清理；3 条实测必守项已写进分工书 §12.7（①预期信号写 `error_code` 会误判 WRONG-TEST，traceback 打印的是异常类名 ②变异脚本必须 `read_bytes`/`write_bytes`，`core.autocrlf=true` 下 `write_text` 会把 LF 写成 CRLF、还原后 md5 必不一致 ⇒ 报**假** FATAL ③import 期断言的变异是**收集期 ERROR**，不算 WRONG-TEST）。 |
| `oo-html-writeback-performance` | **9/13**（2026-09-25 首次登记）。OO↔HTML 回写性能三条 ROI。🔴 **实证发现 T9/T10 已被后继 spec 实质交付，只是从未回标**：<br>**T9 ROI-2**（多受管 sheet 循环合并为单簿一次 load / 一次 save）→ 由 `oo-single-pass-materialize-and-room-leave` 的 `excel_materialize.materialize_projection_single_pass` 交付（substrate 解析 78 → **2**）。⚠️ **口径差要记清**：字面要求「`_read_cell_view` 的 `data_only` True+False **合并为单次 load**」**未做**，实际是 `workbook_read_scope()` 按 `(文件身份, data_only)` 各缓存一次（openpyxl 两视图本就不能合并）⇒ 效果达成、字面未达成。<br>**T10 ROI-3**（`resolve_managed_sheet` 单请求内缓存）→ 已交付：`excel_extract._PARSE_MEMO_TAG` 记忆化 + `identity_inventory` 接受已算 `fingerprint`/`sync_pairs`（后者由 `workpaper-sync-materialize-large-table-performance` 交付，真库一次 collect **34.2s → 1.7s**）。<br>**T11 ROI-6 真未做** —— `repository.lock_room_oo_apply` 仍是**会话级** advisory lock，`oo_to_html.py:2463` 注释明写它「跨越下方 commit + rematerialize CPU」⇒ 多人同编同底稿仍串行排队（`d2-sync-coverage` spec 也把这条登记为自己的既有风险并指回 ROI-6）。**T12** 批次 2 全链路回归 + 真实 PG 探针 + 多人并发压测未做。<br>⇒ **下一步不是重做 T9/T10，而是先按后继 spec 的实测数据复核并回标，再只做 T11/T12。** |
| `excel-workbook-wide-row-change-propagation` | **30/30 ✅ 但归档被阻塞**（2026-09-25 首次登记 + 实扫）。工作簿级行变更传播（插行/删行 → 跨 sheet 引用、命名区域、合并单元格的 A1 改写）。复用 `excel_row_shift._rewrite_formula_refs` 作**唯一** A1 改写入口（Property 27 用 AST 锁死不新造第二入口）。**不能移进 `_archive`**：同 `published-representation-…`，它在 milestone `specs[]` 8 名单内；另 `backend/tests/workpaper_sync/test_workbook_row_change_reachability.py` 直接读它的 `design.md`。spec 内自述的唯一 caveat：门后各 Wave（T2/T3/T4）的变异清单待 C 交付后另行落地，已声明**不阻塞** Task 26 的交付面。 |
| `d-cycle-sheet-bidirectional-expansion` | **9/9 ✅ 但归档被阻塞 + 缺三件套**（2026-09-25 实扫）。D 循环 sheet 级双向扩容的**滚动登记册**（只有 `tasks.md` + `README.md`，**无 requirements/design**）。已做实：D4-3 首 canary 全链 §9.6 绿 · D4-4 诚实改判 `single_html`（无 UUID 行身份列 + `AdjustmentSyncService`/A13 hub 冲突）· D4-5 改判 `paragraph_block_bidirectional` · D4-6「从零静态行 sheet」范式 · D4-13 撤销旧 N/A 改走 static_region（顺带修 9 个 provider 共用的 `build_store_projection` 枚举未转换 bug）。<br>🔴 **归档双重阻塞**：①**生产代码硬读它的 evidence**（`phase5_d4_policy_check_sheet.py:112` / `phase5_d4_erp_check_sheet.py:76` 读 `T09`/`T10-*.json` 算 `mapping_digest`，`phase5_d4_revenue_detail.py:1088-1090` 在契约构建路径上调 `assert_mapping_digest_d45/_d413`；另 2 个 e2e 往其 evidence **写**文件）②README 分波表 **Wave 5（D5-3 / D6-3 / D7-3）无接收方** —— Wave 2(D1-2) 已移交 `d1-sync-row-table-engine-and-d1-coverage`、Wave 4(D2-3) 已移交 `d2-sync-coverage-via-row-table-engine`，但 D3/D5/D6/D7 的 sheet 扩容按「各循环独立成套」裁决应另立 spec，**尚未立项**。 |
| ~~`d4-revenue-matrix-bidirectional`~~ | **已归档 13/13** | → `_archive/14-d4-bidirectional-writeback/`（2026-09-22，随 D4 双向回写 14 spec 整批归档）。**本行原记 0/12 已严重过期。** 该 spec 是 **D4 全组唯一拿到完整 L2 真 OO canvas → applied 证据**的一家：`forcesave cs_error=0` + `operation/application.state=applied` + `content_version.source=onlyoffice` + `store_mirror.marker_in_store=true`，且 `months` 仍是长度 12 的位置数组 ⇒ **DEC-D4-1「12 个月做 12 条独立 field」的位置数组往返等值在真栈成立**，`Task 2` 阻塞门已过。证据 `_archive/14-d4-bidirectional-writeback/d4-revenue-matrix-bidirectional/evidence/g5-1-d4-unified-path/`。以下为原始设计摘要（保留作史实）：**D4 收入矩阵型底稿双向回写**（2026-09-12 新建，三件套齐）。D 循环 5 个 canary（D1/D3/D5/D6/D7）全 `bidirectional_verified` 后，D4 是唯一未做实的 —— partial 评估结论「技术可行但不复用现成八步」，故单立。<br>**与已做实 5 家的三点结构性差异**：① store 行是 `months[12]` **位置数组**（product×12 月矩阵，B~M 列按数组下标映射），而 D3/D7 的 nested 账龄是 dict 键 ⇒ **数组下标 json_pointer 是未验证的新形态** ② 模板 **270 个公式格**，D4-2 的 N 列 `=SUM(B:M)` 是合法内部公式**不能删**，净化必须区分外链 vs 内部公式（D7 只有 6 处） ③ **46 sheet** 且 `D4-2-rows` **有真载荷**（2 行 1739B，落 wp `b3ab3c46`@project `0ec33ac9` + wp `21d8089b`@project `c8621493`）⇒ 非空首版，materialize 更复杂。工作量约单 canary 的 2–3 倍。<br>**DEC-D4-1**：12 个月做 **12 条独立 field**，不做 1 条 array field（后者退回「把整 JSON 当一字段比较」，违反 D2 的 AC 6.9/6.12）。**DEC-D4-2**：若共享 extract 侧不支持数组段，则升级为先改 `contracts.py` 的前置任务并通知其余 canary owner。<br>🔴 **Task 2（位置数组往返等值）是阻塞门**，不过则 Wave 2 起全停。做实后可辐射 I6-2 / J1 月度 / E1 现金月度等 `months[]` 型底稿。**5 Req / 24 AC 零悬挂 / 6 Property / 12 Task / 5 Wave**。 |
| `four-table-extraction-entry-completion` | **16/23**（🔴 2026-09-25 实扫更正：此前记 16/16 ✅ 已过期，Phase 3 的任务 16~23 是后加的，**7 条全 `[ ]`**。逐符号实证真未做：`build_d4_indicator_prefill`/`indicator_prefill` 全仓 **0 命中**（T16/17）· `D4TabMarginMonthly.loadMonthly()` 无 seed 逻辑直接归零（T18）· `_parse_d4_7_monthly_row` **0 命中**（T20）· `_d4_import_export.py:2580` 仍写着 `"priorQty": 0,  # 上期数量不在简化导入列中，默认0`（T21）。**本 spec 是当前唯一零外部依赖、可立即动手的**） | **四表库取数入口补齐 + 两套实现收敛 + 账龄空骨架/枚举联动（Phase 2）**（2026-09-12 新建；Tasks 1–10 Phase 1 全交付，Tasks 11–15 Phase 2 全交付，收口完成）。两条主线：横向补齐缺前端键的明细表、纵向把两套 aux 取数实现合成一套；Phase 2 修复 K1 账龄活体伪造并让账龄骨架配置驱动、随账龄枚举联动。<br>🔴 **Phase 2 交付摘要（Tasks 11–15）**：① **修 K1 账龄伪造**——`four_table/k1_aux_detail.py` 删 `_aging()` 整额落首段 + `["within1"]` 硬编码兜底，改 `_empty_aging(seg_keys)`（每段 0）；`_k1_import_export.py` 新增 `_resolve_k1_segments` → `get_effective_segments(project_id,"K1",db)`（读失败按 subject 默认 preset 兜底、不回退单段），message 改「账龄留空，请按实际账龄人工填列」② **D 循环账龄骨架对齐**——D3/D7/`d_aux_import.py` 端点账龄留空 + 段键由 `get_effective_segments` 生成（证据 `evidence/task12-d-cycle-aging-skeleton-audit.md`；D5 N/A、D6 deferred 平键）③ **前端骨架同键 + 枚举切换重映射**——`useK1Detail.ts` 用 `useAgingConfig(projectId,"K1")` 取 segments/bands、监听 `aging-config:changed` → `remapRowAgingData`；审定/披露与骨架同一 `effective_segments` 真源 ④ **守卫 + 变异**——后端 `tests/four_table/test_aging_skeleton_properties.py`（Property 5 `sum(agingAudited)==0` + Property 8 键集==`get_effective_segments`）+ `tests/d_cycle_extraction/test_d_cycle_aging_skeleton.py` + 变异 `scripts/diagnose/mutate_aging_skeleton_guards.py`（塞首段/硬编码段键必红）；前端 `composables/__tests__/agingSkeletonSync.guard.spec.ts` + `.../composables/__tests__/fourTableAgingSkeletonSameKey.smoke.spec.ts` ⑤ **真栈实测 Task 15**（证据 `evidence/task15-aging-skeleton-realstack.json`）：真栈 K1 wp `6e6348d0`（重庆和平药房_2024）新取 400 行**全段 aging 恒 0**（`max_abs_aging_value=0`、无段 == 余额，Property 5/8 实证 + UI 侧「账龄合计0 ≠ 期末」告警佐证）；手动录 `y2to3=4800` 重取不被覆盖/不重复（Requirement 3.4/7.4）；项目账龄枚举 FIVE_YEAR→THREE_YEAR（`PUT /api/projects/{id}/aging/config` 广播 `aging-config:changed`）后 `effective_segments` 6→4、K1-2「账龄区段」列同步渲染「1年以内/1-2年/2-3年/3年以上」、共享段 y2to3=4800 经 `useAgingMigration` 重映射保留（Requirement 7.2/7.4/7.5），测后已还原 FIVE_YEAR。**Phase 2 收口**：三件套 `get_diagnostics` 全 clean；Phase 2 产物（5 守卫/测试 + 改动端点/服务 + 2 证据 + 三件套）已 `git add` 暂存无 `??` 漏登记（未 commit）；未产生 `tmp_*`/`_wip_*`。**⚠️ 已知遗留（不在本 spec 修）**：THREE_YEAR 末段键 `over3` 与 FIVE_YEAR `over5` 不同 ⇒ 末段值在三↔五切换时会掉（`PRESET_SEGMENTS` 固有，非本 spec 缺陷；建议单立 spec 统一末段键或让 `useAgingMigration` 按位置映射末段）。<br>**Phase 1 交付摘要（收口 Task 10 登记）**：① 共享件 `four_table/aux_aggregation.py` 新增 `aggregate_aux_by_name_ex(...) -> AuxAggregationResult`（entries/aux_type/total_units/reason 六值 `ok\|no_prefixes\|no_aux_type\|no_rows\|error`，异常记 ERROR 治 fail-open 假绿），旧 `aggregate_aux_by_name` 改薄壳（4 消费者零改动，有守卫断言）② 新建共享 helper `d_cycle_extraction/d_aux_import.py`（`aggregate_d_cycle_aux` + `aux_reason_message`，科目前缀走报表映射 `BS-006/046/007/011/047` 解析、兜底常量注 `source_ref`，账龄留空不塞全额）③ **5 个 G-C 端点迁移到共享件**（`_d3/_d5/_d6/_d7_import_export.py` 删各自裸 SQL；`_d2_import_export.py` **新建** `POST /{wp_id}/d2/import-aux-balance` 替代前端第 5 套客户端 GROUP BY + 硬编码 1122）④ **K1 G-A 补手动入口**（`k1/core/K1TabDetail.vue`「从余额表导入」，与 AutoSeed 语义区分）⑤ 前端 reason→中文提示单一真源 `composables/fourTableAuxImportFeedback.ts`（各循环 composable 统一消费）⑥ 后端守卫 `tests/four_table/test_four_table_entry_guards.py` + 变异 `scripts/diagnose/mutate_four_table_entry_guards.py`（≥3 锚点四态判定）⑦ 前端守卫 `composables/__tests__/fourTableAuxImportEntry.guard.spec.ts` + `fourTableAuxImportFeedback.spec.ts`（端点 URL 字面量 + reason 映射全分支 + 改错循环前缀必打红）⑧ **真栈 Playwright 实测**证据 `evidence/task9-realstack-playwright.json`。<br>**长期资产**：缺口清册 `evidence/gap-inventory.{md,json}`（G-A={K1} / G-B={} / G-C={D2,D3,D5,D6,D7} / G-D={} / 合规基线={D1,F1,G7}，含 source_digest 冻结 `sha256:da5aeae6…`）落 spec `evidence/` 作唯一真源。**收口 Task 10**：三件套 get_diagnostics 无 error（tasks.md 仅 3 条 cosmetic「recommended section」warning，Property/Validates/waves JSON 全合规）；8 个 `??` 未跟踪产物已 `git add` 暂存（evidence×7 + tasks.meta.json + d_aux_import.py + 2 后端测试/变异 + 3 前端 composable/守卫 = 14 files），无 `??` 漏登记；本 spec 未产生 `tmp_*`/`_wip_*`（诊断产物直接落 evidence）。<br>**原始基线（实扫）**：已有 8 个后端端点 `POST /api/workpapers/{wp_id}/{d1\|d3\|d5\|d6\|d7\|f1\|g7\|k1}/import-aux-balance`；已有可见「从余额表导入」按钮的宿主 8 个（F1/D7/D6/D3/D5/D2/D1/G7 TabDetail）；**K1 只有 AutoSeed 无手动按钮**（空表自动 seed，用户动过数据后无法重取）＝既有缺口样本。<br>🔴 **纠正一条已过期的红基线**：G7 与 `four_table/aux_aggregation.py` 的注释都写「禁止照 D3/D5/D6/D7 历史版本（其 SQL 引用不存在的列 `period_type`/`balance`，运行必 500）」—— 实扫 `period_type` 在 `wp_render_strategies/**` 下**只出现在那句注释本身**，四家实际用的是 `aux_name`/`opening_balance`/`closing_balance`/`account_code`/`is_deleted`，**列都存在不会 500**。真实缺陷是：① 裸写 `is_deleted = false` **不走 `get_active_filter`** ⇒ 跨数据集版本双算（实测 2×）② 直接 `GROUP BY aux_name` **未先锁定单一 `aux_type`** ⇒ 多维度挂账双算 ③ 科目码**硬编码**（`'2203%'`/`'1124%'`/`'2205%'`/`'1141%'`）不从报表映射解析 ④ 把余额全额塞进账龄首段（`agingPrior.within1`）＝**伪造账龄分布**。四家 `get_active_filter`/`pick_aux_type`/`aux_aggregation` 全部零引用（grep 实证）⇒ 结论仍成立、理由必须改写。<br>**收敛目标**：全部走 `four_table.aux_aggregation.aggregate_aux_by_name`，禁止第 5 套归集 SQL。并治一处 **fail-open 假绿** —— 现版 `except Exception: return [], None, 0` 连 `rollback` 也吞，接线错误会伪装成「本项目无此数据」⇒ 新增 `aggregate_aux_by_name_ex` 带 `reason` 码（`ok`/`no_prefixes`/`no_aux_type`/`no_rows`/`error`）且异常记 ERROR，旧函数改薄壳保持既有 4 消费者零改动。<br>**缺口清单必须结构性推导**（registry / RENDERER_DISPATCH / render-config / `wp_code_overrides.json`），**禁 grep 按钮文案或写死页面数量**；gap 分 G-A（缺前端）/ G-B（缺端点）/ G-C（违铁律待迁移）/ G-D（不适合，登记理由不动代码）。**6 Req / 31 AC 零悬挂 / 7 Property / 11 Task / 5 Wave**。 |
...
| ~~`workpaper-page-formula-toolbar-closure`~~ | **已归档 17/17 ✅**（2026-09-25 Task 17 闭合并 `git mv` 归档 → `_archive/05-business-features/`。本轮环境登录态可用，突破历史「3030 重定向 `/login`」阻塞：定向行为测试 4 文件 29 例绿 + Playwright 真实 D2 底稿验收（唯一公式入口 formulaBtnCount=1、打开唯一 FormulaManagerDialog、真实全册 45 公式、来源位置 2025 年度、0 console error，截图 `basis/T17-e2e-formula-manager-open.png`）。**根因修复** render-config `UnboundLocalError` 500（`ovr` 空 sheet 集底稿未初始化）+ 防御测试 `TestOvrLoopInvariantRegression`（GREEN+手工 RED，41 passed）。**回归修复** `GtWpAiReviewToolbar` 缺 Task 9 `data-ai-action` 接线已补，16 文件 100 例绿。F-SHELL outlet 已交付并被 `formula-row-name-alignment-confirmation` 消费。证据 `basis/T17-e2e-closure.md`） | **workpaper route 公式入口与公共能力壳层闭环**（2026-09-09 Task 15 全量清册门 PASS）。`F-SHELL` 已发布且 `inventoryDigest` 非空；五宿主裁决 html/univer/onlyoffice/word reachable + grid formal exemption；forbidden capability duplicates=0；evidence `evidence/F-SHELL/INDEX.md` + `basis/T15-*`；变异 T15 3/3 RED；Playwright T14 4/4。三件套 **13 Req / 74 AC / 22 Property / 15 Task**。待物理归档至 `_archive/`（可独立于 custom 实例复现）。 |
