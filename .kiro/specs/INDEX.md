...
**最后更新**：2026-08-15
**当前分支**：`work/2026-08-15-report-line-account-resolution`
**统计**：Active **7**（2026-08-15 归档 `k-cycle-extraction-formula-and-disclosure-closure` 后**重新实扫** `.kiro/specs/*/` 共 9 个目录，其中**带 tasks.md 的 7 个**、**空壳 2 个**）/ Archived **555**（同轮实扫 `_archive/*/*` 一级子目录所得，含 6 个仅剩 evidence 无三件套的历史残留目录）

> **🔴 本轮（归档 k-cycle 后）逐目录实扫，带 tasks.md 的 7 个**：`amount-input-migration-and-column-typing` · `e1-variant-recalc-and-mutation-denominator-closure` · `guard-assertion-attribution-refactor` · `i-cycle-extraction-formula-and-disclosure-closure` · `l-cycle-extraction-formula-and-disclosure-completion` · `workpaper-import-export-lifecycle-closure` · `x3-adjustment-entry-import-export`；空壳 2 个 = `procedure-delegation-visibility-isolation` / `visibility-isolation-go-live-hardening`（又一次出现在 Active 区且 git 未跟踪）。
