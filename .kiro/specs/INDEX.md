...
**最后更新**：2026-09-12
**当前分支**：`work/2026-09-12-d-cycle-bidirectional-canaries`
**统计**：Active **12**（2026-09-12 逐目录实扫 `.kiro/specs/*/` 且**带 tasks.md** 口径；目录数 13，多出的 `audit-evidence-attachment-preview-format-expansion` 只剩空壳 `evidence/`，spec 文档已在 `_archive/05-business-features/`） / Archived **566**（`_archive/*/*` 实扫）
...
| `d4-9-customer-structure-bidirectional-writeback` | **10/15** | **D4-9 重要客户结构分析 HTML↔OnlyOffice 双向回写**（分支 `work/2026-05-30-wp-specs`）。核心难点：单 sheet 内两个动态行区（本期 R13-22 + 上期 R27-36）+ 4 表级标量（C24/E24/C38/E38）+ 占比/合计物理公式。<br>**已交付（Task 1-4/7/8/10/11/12 + Task9 核心）**：多区 instrumentation 注入器（`instrument_workbook_bytes_multi`，合并进单 `<tableParts count=2>`）· 前端 rowId 单一真源 `d4CustomerRowIdentity.ts` · per-entry contract `d4.customer_structure.json`（3 table + 生成器 + 磁盘锁死）· 三区投影/合并 · materialize/extract 多区扩展 + 真往返守卫 · 移除 `D4TabCustomerStructure.vue` legacy 双入口 · IO 修复（本期/上期分区 + 总额 + 专用 parser）· 用户公式保护集/血缘 · 变异 5 锚点 RED。合计守卫 **52 passed**（后端 46 + 前端 6），全行为级。<br>🔴 **Task 5/6 registry attach 尾部 + Task 13 真栈实测 UNVERIFIABLE（环境不可用，非实现失败，Req 8.6）**：Task 13 需真实 OnlyOffice runtime + DeepHorness SDK `D:\DeepHorness`（本机**不存在**）+ 前端 dev server（3030 **未运行**）驱动 HTML↔OO 端到端。2026-09-13 实测环境：后端 9980 UP（`import app.main` OK）· OnlyOffice 8080 docker healthy · **前端 3030 DOWN · `D:\DeepHorness` 缺失** ⇒ 双向 request 路径命中 USER_SYNC_PREFIX / OO 改行改总额往返 / 用户公式不被覆盖等行为级判据无法真实驱动，记 `UNVERIFIABLE`。curated entry `xlsx/gt-d4-customer-structure` 已在 `workpaper_sync_entry_overlay.json` 声明（capability=bidirectional），但 manifest `--apply` 重生成被既有 `approved_source_digest` 漂移堵死（并发会话在编辑 D4 宿主 .vue）。<br>**Task 13 可做的确定性收口已完成**：三件套 `get_diagnostics` **0 diagnostics**（修 requirements 标题格式 + design 标准 section + Property 标题）；`git status --porcelain -- <产物清单>` 核出 **11 个 `??`**（`phase5_d4_customer_structure.py` · `d4.customer_structure.json` · 生成器 · 6 测试 · 变异脚本 · `d4CustomerRowIdentity.{ts,spec.ts}`）+ 4 个 ` M`（`_d4_import_export.py` · `excel_instrumentation.py` · `workpaper_sync_entry_overlay.json` · `D4TabCustomerStructure.vue`），`workpaper_capability.py` 已被并发会话 commit（TRACKED，不再是 `??`）；本 spec 未产生 `tmp_*`/`_wip_*`。<br>**解阻需**：(a) 部署 DeepHorness SDK 到 `D:\DeepHorness` + 前端 dev server (b) mount-diff 复核并重批 `approved_source_digest` 让 manifest 干净重生成。**9 Req / AC 全覆盖 / 10 Property / 15 Task / 7 Wave**。 |
...
| `workpaper-sync-curated-entry-facility` | **6/6 ✅** | **底稿 sync manifest curated-entry 设施**（2026-09-13 全 6 Task 交付，未 commit，产物全 `??`/2 `M`）。给共享 discovery 生成器 `generate_workpaper_sync_manifest.py` 加**附加式、reviewed、双向锁死**的 curated-entry 段：让一个动态宿主（`GtD4OperatingRevenue.vue`，同挂 D4-1/2/3/9）能承载多个逻辑 sync entry。空 curated ⇒ 字节 identical（Property 1）；bidirectional 必带 adapter/html_store/contract-test/review_status（Property 2）；collision/host 门 fail-closed（Property 3）；`curated_source_digest` 进 `manifest_digest`（Property 4）；registry attach RG-4 保留（Property 5）；变异 ≥4 锚点全 RED（Property 6）。**动机消费者 = D4-9**：`xlsx/gt-d4-customer-structure` + adapter `d4.customer_structure`。<br>**Task 6 跨 spec 闭合**：D4-9 spec Task 6 的 `assert_manifest_capability_enabled`/`assert_entry_selectable`（sibling `phase5_d4_revenue_detail.py` 定义、带 `manifest` in-memory seam、`None` 时读磁盘 manifest；D4-9 本体 `phase5_d4_customer_structure.py` **尚未定义**这俩符号，正是被本 facility 落盘所阻塞）。闭合守卫 `test_curated_entry_cross_spec_closure.py` **8 passed + 1 xfailed**：Part A PROVEN（内存现建 manifest 驱动同一真实底层逻辑，D4-9 capability=bidirectional、可选中、registry 挂 adapter、RG-4 保留）· Part B BLOCKED（磁盘 manifest 186 entry 无 D4-9、宿主 entry 仍 single_onlyoffice —— 既有 `approved_source_digest` 漂移 `d9fddb64…` vs `a18a531d…` 所致、与 curated 正交 Req 3.4，**未强推 disk regen / 未 bump digest**）· Part C UNVERIFIABLE（无 OO runtime，浏览器双向证明记 UNVERIFIABLE，Req 5.3）。证据 `backend/tests/workpaper_sync/data/curated_entry_facility_evidence.md`。三件套 `get_diagnostics` 0 error。<br>🔴 **产物长期 `??` 未跟踪（CI 风险）**：spec 目录 + 6 守卫（baseline/builder/integration/facility/d4_9_wiring/cross_spec_closure）+ 4 fixtures（discovery/manifest/frontend.generated.ts/_capture 脚本）+ evidence.md + 变异脚本 `mutate_curated_entry_facility_guards.py` 全 `??`；`overlay.json`/`generate_workpaper_sync_manifest.py` 为 ` M`。**干净 checkout 下引用这些文件的 CI job 必挂**，待 owner `git add`。 |
