...
> | **B 待环境/登录态** | `workpaper-page-formula-toolbar-closure` | 1 条 | 只差一次有登录态的 Playwright |
...
| `workpaper-page-formula-toolbar-closure` | **16/17**（2026-09-25 实扫；此前记 15/15 已过期。**只剩 Task 17 真栈 Playwright** —— 3030 重定向 `/login` 无可用登录态。F-SHELL outlet **已交付**：`GtWpToolbar.vue` L25-31 命名 slot `page-capabilities-compatibility` + `data-toolbar-outlet` 属性，下游 `formula-row-name-alignment-confirmation` 已成功消费并随本轮归档） | **workpaper route 公式入口与公共能力壳层闭环**（2026-09-09 Task 15 全量清册门 PASS）。`F-SHELL` 已发布且 `inventoryDigest` 非空；五宿主裁决 html/univer/onlyoffice/word reachable + grid formal exemption；forbidden capability duplicates=0；evidence `evidence/F-SHELL/INDEX.md` + `basis/T15-*`；变异 T15 3/3 RED；Playwright T14 4/4。三件套 **13 Req / 74 AC / 22 Property / 15 Task**。待物理归档至 `_archive/`（可独立于 custom 实例复现）。 |
