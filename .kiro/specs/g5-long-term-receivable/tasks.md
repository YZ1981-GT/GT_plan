...
- [ ] 1. 注册四件套 + render策略
  - [ ] 1.1 后端注册：VALID_COMPONENT_TYPES添加'g5-long-term-receivable' + RENDERER_DISPATCH注册render_g5_long_term_receivable策略函数
  ...
  - [ ] 1.2 前端注册：htmlRendererRegistry添加映射 + 创建主入口骨架GtG5LongTermReceivable.vue
  ...
  - [ ] 1.3 wp_code_overrides.json添加16条映射
    - G5A/G5-1~G5-12/附注披露(上市)/附注披露(国企)/底稿目录 → 'g5-long-term-receivable'
    - _Requirements: 1.4_
...
- [ ] 2. 公式引擎composable + PBT测试
  - [ ] 2.1 实现useG5FormulaEngine.ts（22个纯函数 + parseNum）
...
- [ ] 3. 主入口sheetName分发 + selfLoad + 数据composable
  - [ ] 3.1 GtG5LongTermReceivable.vue 完整实现
  ...
  - [ ] 3.2 useG5FormData.ts composable
    - 数据加载/保存统一入口
...
- [ ] 4. G5-1 审定表（87行多层结构）
  ...
  - [ ] 4.2 useG5Adjudication.ts composable
    - 五层分组数据结构管理
...
- [ ] 5. G5-2 余额明细表（22列→2区段Tab）
  - [ ] 5.1 G5TabBalanceDetail.vue 组件
  ...
  - [ ] 5.2 useG5BalanceDetail.ts composable
    - 期末余额 = 合同总额 - 已收回金额
    - 净额 = 期末余额 - 未实现融资收益
    - 账龄合计 = 各账龄段之和
    - 账龄合计≠净额 → 红色高亮
...
- [ ] 6. G5-3 坏账准备明细表（20列→2区段Tab）
  - [ ] 6.1 G5TabBadDebtDetail.vue 组件
  ...
  - [ ] 6.2 useG5BadDebtDetail.ts composable
    - ECL公式链：③=①×② / ⑥=⑤×②A+①×(②A-②) / ⑦=①+⑤ / ⑧=③+⑥ / ⑨=⑦-⑧
    - 本年计提 = 审定坏账-上年+转回
    - _Requirements: 6.2~6.7_

- [ ] 7. G5-4 调整分录汇总
  - [ ] 7.1 G5TabAdjustment.vue + useG5Adjustment.ts
...
- [ ] 8. G5-5 融资租赁测算（内含利率法，18列→2区段Tab）
  - [ ] 8.1 G5TabLeaseAmortization.vue 组件
  ...
  - [ ] 8.2 useG5LeaseAmortization.ts composable
    - 净投资额 = 应收 - 未实现
    - 融资收益 = 净投资额 × 内含利率（核心）
    - 期末应收 = 期初 - 收款
    - 期末未实现 = 期初 - 收益
...
- [ ] 9. G5-6 分期销售测算（实际利率法）
  - [ ] 9.1 G5TabInstallmentSales.vue + useG5InstallmentSales.ts
...
- [ ] 10. G5-7 保理核查 + G5-8 会计政策检查
  - [ ] 10.1 G5TabFactoringCheck.vue + useG5FactoringCheck.ts
  ...
  - [ ] 10.2 G5TabEclPolicy.vue + useG5EclPolicy.ts
    - 46行×15列，四section问卷
...
- [ ] 11. G5-9 三阶段划分 + G5-10 坏账准备测算
  - [ ] 11.1 G5TabStageClassification.vue + useG5StageClassification.ts
    - 列式转置→行式交互视图（同G4-9方案）
  ...
  - [ ] 11.2 G5TabImpairmentCalc.vue + useG5ImpairmentCalc.ts
...
- [ ] 12. G5-11 转回核销 + G5-12 凭证检查
  - [ ] 12.1 G5TabReversalWriteoff.vue + useG5ReversalWriteoff.ts
  ...
  - [ ] 12.2 G5TabVoucherCheck.vue + useG5VoucherCheck.ts
...
- [ ] 13. 附注披露 + 底稿目录
  - [ ] 13.1 G5TabDisclosureListed.vue（上市公司附注）
    - 113行×11列结构化表格 + 虚拟滚动
  ...
  - [ ] 13.2 G5TabDisclosureSOE.vue（国企附注）
    - 109行×12列结构化表格 + 虚拟滚动
    - 同上市版本EventBus逻辑
    - _Requirements: 4.2, 4.3, 4.4, 4.5_

  - [ ] 13.3 G5TabDirectory.vue（底稿目录）
    - 27行×8列底稿目录
    - 索引号列GtIndexChip可点击跳转
    - _Requirements: 18.9_

- [ ] 14. 导入导出 + AI辅助 + 双模式
  - [ ] 14.1 useG5ImportExport.ts composable
    - 9张动态行表格导入导出：G5-2/G5-3/G5-4/G5-5/G5-6/G5-7/G5-10/G5-11/G5-12
    - 后端三端点：export-template / export-data / import-data
    - 宽表区段分sheet导出：G5-2(2)/G5-3(2)/G5-5(2)/G5-10(2)/G5-11(2)/G5-12(3)
    - 使用http(axios)不能用原生fetch
    - StreamingResponse中文文件名RFC5987编码
    - _Requirements: 17.1~17.3_

  - [ ] 14.2 后端 _g5_long_term_receivable_import_export.py
    - 27个端点（9表×3端点）
    - openpyxl解析验证 + 格式不匹配422
    - 宽表按区段分sheet
    - _Requirements: 17.1~17.3_

  - [ ] 14.3 后端 _g5_long_term_receivable_ai.py
    - 9个AI section端点
    - adjudication-analysis/lease-amortization-conclusion/sales-amortization-conclusion/factoring-conclusion/ecl-policy-conclusion/stage-conclusion/impairment-conclusion/reversal-writeoff-conclusion/voucher-conclusion
    - _Requirements: 17.4, 17.5_

  - [ ] 14.4 useG5DualMode.ts + HTML↔OnlyOffice切换
    - localStorage持久化用户偏好
    - 切换时数据不丢失
    - _Requirements: 17.6_
...
- [ ] 15. 六大集成联动
  ...
  - [ ] 15.2 抽凭引擎集成
    - G5-12凭证检查表集成GtVoucherSamplingEngine（dialog→样本填入）
    - G5A程序表集成抽凭（科目范围1531*）
    - _Requirements: 16.2_

  - [ ] 15.3 截止自动提取集成
    - G5A程序表集成useCutoffAutoSampling（accountCode='1531', days=5）
    - _Requirements: 16.3_

  - [ ] 15.4 附注EventBus集成
    - G5-1 → publish substantive:adjudicated(1531)
    - 附注(上市/国企) → subscribe + publish disclosure:note-text-updated
    - _Requirements: 16.4_

  - [ ] 15.5 行级OCR集成
    - G5-12 Tab1📎附件列：上传→POST /d4/contract-ocr→ElMessageBox确认→merge填入
    - _Requirements: 16.5_
...
- [ ] 17. 集成测试 + E2E
  - [ ] 17.1 后端集成测试
    - render策略返回16 sheets配置正确
    - 导入导出27端点基础契约测试
    - AI 9端点响应格式
    - trial_balance取数(1531)
    - _Requirements: 1.1~1.5_

  - [ ] 17.2 后端PBT（hypothesis, max_examples=5）
    - P1借方余额 / P3内含利率法 / P4实际利率法 / P7 ECL链 / P11借贷平衡 / P12转回有效性
    - _Requirements: 15.1~15.22_

  - [ ]* 17.3 Playwright E2E
    - 场景1：审定表填写→公式联动→EventBus→附注刷新
    - 场景2：融资租赁测算→内含利率法逐期→期间连续性
    - 场景3：保理核查→有追索+终止确认→橙色警告
    - 场景4：凭证检查→OCR→Tab2核对→Tab3异常
    - 场景5：导入导出往返（导出→修改→导入→数据一致）
    - _Requirements: 3.1~18.9_


## Notes

- G5特色区别于G4：内含利率法(G5-5)+实际利率法(G5-6)两种测算模式、保理终止确认核查(G5-7)、87行超大审定表五层结构
- G5-9三阶段划分同G4-9列式转置方案，可复用G4-9的转置渲染逻辑
- ECL公式链(G5-10)同G4-10，公式引擎可复用核心算法
