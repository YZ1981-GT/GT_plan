# Implementation Plan: K9 管理费用底稿专属HTML精美组件

## Overview

K9管理费用底稿专属组件`k9-admin-expenses`。K循环损益类底稿（12有效sheet/1 xlsx/~120+公式）。与K8销售费用同款损益类方法论。

主入口 GtK9AdminExpenses.vue（sheetName v-if分发，defineAsyncComponent lazy）+ 11个子组件 + composable分层（含实质性分析引擎+截止引擎）+ 后端3个py文件。

科目：6602管理费用（**损益类！取发生额**，从tb_ledger取数）
