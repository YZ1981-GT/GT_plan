...
- [x] 0.1 openpyxl脚本读取H3投资性房地产.xlsx全部22 sheet
...
- [x] 2.1 创建 `composables/useH3FormulaEngine.ts`，实现全部纯函数
...
- [x] 2.2 创建 `composables/useH3TransferEngine.ts`，实现互转纯函数
...
- [x]* 2.6 编写 Property P4 PBT：三角勾稽（成本模式）
...
- [x]* 2.8 编写 Property P6 PBT：互转公允值（自用→投资）
...
- [x]* 2.9 编写 Property P7 PBT：互转账面值（投资→自用）
...
- [x]* 2.10 编写 Property P8 PBT：折旧公式（成本模式直线法）
...
- [x]* 2.11 编写 Property P9 PBT：公允价值变动损益
...
- [x]* 2.12 编写 Property P10 PBT：租金收入测算
...
- [x]* 2.13 编写 Property P11 PBT：DCF现值（仅成本模式）
...
- [x]* 2.16 编写 Property P14 PBT：借贷平衡
...
- [x] 3.1 创建 `composables/useH3FormData.ts`
...
- [x] 3.2 创建 `composables/useH3MeasurementModel.ts`
...
- [x] 3.3 创建 `composables/useH3CrossSheet.ts`
...
- [x] 4.1 创建 `h3/core/H3TabIndex.vue`（~150行）
  - 底稿目录（22行进度条+完成状态+计量模式标识）
  - _Requirements: 1.2_

- [x] 4.2 创建 `h3/core/H3TabAdjudicationCost.vue`（~450行）
  - 双区块(原值+折旧) + 三角勾稽校验 + TB回写
  - 审计说明/结论 + AI + 💬复核 + GtIndexChip→H3-6
  - _Requirements: 2.1-2.9_

- [x] 4.3 创建 `h3/core/H3TabAdjudicationFair.vue`（~400行）
  - 单区块(公允价值) + 公允变动列 + TB回写
  - 审计说明/结论 + AI + 💬复核
  - _Requirements: 2.1-2.9_

- [x] 4.4 创建 `h3/core/H3TabDetailCost.vue`（~500行）
  - 3区段Tab切换(基本/折旧/增减) + 行同步 + 固定列
  - 合计行 + 交叉验证H3-1 + 导入导出
  - _Requirements: 3.1, 3.3-3.9_

- [x] 4.5 创建 `h3/core/H3TabDetailFair.vue`（~400行）
  - 2区段Tab切换(基本/公允变动) + 行同步 + 固定列
  - 合计行 + 交叉验证H3-1 + 导入导出
  - _Requirements: 3.2-3.9_

- [x] 4.6 创建 `h3/core/H3TabAdjustment.vue`（~250行）
  - el-table 10列 + 借贷平衡 + 推送A13
  - _Requirements: 4.1-4.5_

- [x] 4.7 创建 `h3/core/H3TabDisclosureListed.vue` + `H3TabDisclosureSoe.vue`（各~350行）
  - 多子节卡片 + 计量模式说明 + 动态行 + 合计
  - _Requirements: 15.1-15.5_

- [x] 4.8 创建 `h3/inspection/H3TabPolicyCheck.vue`（~350行）
  - CAS3五段落卡片 + 计量模式突出 + 进度条 + AI + 💬复核
  - _Requirements: 5.1-5.5_

- [x] 4.9 创建 `h3/inspection/H3TabAdditionCost.vue`（~350行）
  - el-table 21列（成本模式）+ 抽凭+OCR + 汇总
  - _Requirements: 6.1, 6.3-6.6_

- [x] 4.10 创建 `h3/inspection/H3TabAdditionFair.vue`（~300行）
  - el-table 20列（公允模式）+ 抽凭+OCR + 汇总
  - _Requirements: 6.2-6.6_

- [x] 4.11 创建 `h3/inspection/H3TabTransferReview.vue`（~500行）
  - 三方向分区(36列25公式) + 转出=转入验证 + 方法论上下文
  - GtIndexChip→H1/H2 + EventBus + AI + 💬复核
  - _Requirements: 7.1-7.10_

- [x] 4.12 创建 `h3/inspection/H3TabStocktakeCheck.vue`（~300行）
  - el-table 13列 + 空置黄色高亮 + 汇总 + 导入导出
  - _Requirements: 10.1-10.5_

- [x] 4.13 创建 `h3/inspection/H3TabTitleCheck.vue`（~300行）
  - el-table 16列 + 产权异常红色 + 面积差异黄色
  - _Requirements: 12.1-12.5_

- [x] 4.14 创建 `h3/inspection/H3TabRelatedParty.vue`（~250行）
  - el-table 11列 + 差异率>10%红色 + 合计行
  - _Requirements: 13.1-13.5_

- [x] 4.15 创建 `h3/depreciation/H3TabDepreciationNoImpair.vue`（~400行）
  - 28列(42公式) + 分支选择器 + 差异高亮（仅成本模式）
  - _Requirements: 8.1-8.8_

- [x] 4.16 创建 `h3/depreciation/H3TabDepreciationWithImpair.vue`（~400行）
  - 28列(62公式) + 减值影响 + 差异高亮（仅成本模式）
  - _Requirements: 8.1-8.8_

- [x] 4.17 创建 `h3/impairment/H3TabImpairment.vue`（~350行）
  - 减值迹象+测算表（仅成本模式）+ GtIndexChip→H3-11
  - _Requirements: 11.1-11.5_

- [x] 4.18 创建 `h3/impairment/H3TabRecoverable.vue`（~400行）
  - DCF模型+敏感性矩阵（仅成本模式）
  - _Requirements: 11.3-11.5_

- [x] 4.19 创建 `h3/fairvalue/H3TabFairValueReview.vue`（~450行）
  - 四区域(评估师/方法/复核计算15公式/假设挑战)
  - 独立测算 + 范围判断 + AI + 💬复核
  - _Requirements: 9.1-9.8_

- [x] 4.20 创建 `h3/rental/H3TabRentalIncome.vue`（~450行）
...
- [x] 5.1 创建 `_h3_investment_property.py` render策略
...
- [x] 5.2 创建 `_h3_import_export.py` 导入导出3端点
...
- [x] 5.3 创建 `_h3_ai_generate.py` AI生成8 section
...
- [x] 5.4 创建 `_h3_transfer_engine.py` 互转引擎端点
  - POST /h3/transfer/calculate → 三方向计算（自用→投资/投资→自用/在建→投资）
  - POST /h3/transfer/validate → 验证转出=转入一致性
  - 纯函数实现（与前端对等逻辑）
  - _Requirements: 7.2-7.5_

- [x] 5.5 创建 `_h3_investment_property.py` auto_data_resolver
...
- [x] 6.1 完成 GtH3InvestmentProperty.vue 主入口集成
...
- [x] 6.2 创建 `composables/useH3DualMode.ts`
  - HTML↔OnlyOffice el-segmented切换
  - OO健康检查 + 切换前autoSave
  - _Requirements: 16.1-16.2_

- [x] 6.3 创建 `composables/useH3ImportExport.ts`
  - el-dropdown三级UI + axios请求
  - 根据measurement_model导出对应版本
  - _Requirements: 16.3_

- [x] 6.4 H3-5 抽凭引擎+OCR集成
...
- [x] 7.3 前端PBT (fast-check) — P1~P14全部验证
