...
- [ ] 1. 数据模型：`qc_rule_definitions` + `qc_inspections` + `project_quality_ratings` 等表
...
- [ ] 13. 前端：QCDashboard 扩展
  - 项目列表新增"评级"列（A/B/C/D 颜色区分）
  - 新增"复核人画像"Tab（雷达图 + 明细表）
  - _需求_ 3, 6

- [ ] 14. 前端：IssueTicketList Q 整改单 UI
...
- [ ] 25. 前端：QcInspectionWorkbench 日志合规 Tab
  - 新增 Tab "日志合规抽查"
  - 命中条目列表 + 标记状态
