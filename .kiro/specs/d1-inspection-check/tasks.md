...
- [ ] 1. 纯函数与共享类型定义
  ...
  - [ ]* 1.2 编写 Property 1 PBT：D1-11 期末余额公式 F=C+D-E
  ...
  - [ ]* 1.5 编写 Property 4 PBT：D1-12 质押比例计算与零值守卫
  ...
  - [ ]* 1.6 编写 Property 5 PBT：D1-13 例外占比公式
  ...
  - [ ]* 1.7 编写 Property 7 PBT：负数金额括号格式
    - **Property 7: 负数金额括号格式**
...
- [ ] 3. 实现 useD1InventoryCount.ts（D1-10 监盘表composable ~300行）
  - [ ] 3.1 创建 `composables/useD1InventoryCount.ts` 核心逻辑
  ...
  - [ ]* 3.2 编写 Property 6 PBT：动态行增删计数不变量（D1-10）
...
- [ ] 4. 实现 useD1RelatedPartyCheck.ts（D1-11 关联方检查composable ~250行）
  - [ ] 4.1 创建 `composables/useD1RelatedPartyCheck.ts` 核心逻辑
  ...
  - [ ]* 4.2 编写 Property 6 PBT：动态行增删计数不变量（D1-11）
    - **Property 6: 动态行增删计数不变量**
    - 生成器：`fc.array(RelatedPartyRowArbitrary, {minLength:0, maxLength:20})`
    - 断言：addRow后长度=N+1；removeRow(validId)后长度=N-1
    - **Validates: Requirements 4.3**

- [ ] 5. 实现 useD1PledgeCheck.ts（D1-12 质押检查composable ~250行）
  - [ ] 5.1 创建 `composables/useD1PledgeCheck.ts` 核心逻辑
  ...
  - [ ]* 5.2 编写 Property 6 PBT：动态行增删计数不变量（D1-12）
    - **Property 6: 动态行增删计数不变量**
    - 生成器：`fc.array(PledgeRowArbitrary, {minLength:0, maxLength:20})`
    - 断言：addRow后长度=N+1；removeRow(validId)后长度=N-1
    - **Validates: Requirements 7.4**

- [ ] 6. Checkpoint - 三个composable验证
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. 实现 useD1SamplingVouching.ts（D1-13 抽样凭证核对composable ~350行）
  - [ ] 7.1 创建 `composables/useD1SamplingVouching.ts` 核心逻辑
  ...
  - [ ]* 7.2 编写 Property 6 PBT：动态行增删计数不变量（D1-13 凭证核对+特定样本）
    - **Property 6: 动态行增删计数不变量**
...
- [ ] 8. Checkpoint - 四个composable全部验证
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. 实现 D1TabInventoryCount.vue（D1-10 监盘表组件 ~400行）
  - [ ] 9.1 创建 `d1/D1TabInventoryCount.vue`
...
- [ ] 10. 实现 D1TabRelatedPartyCheck.vue（D1-11 关联方检查组件 ~350行）
  - [ ] 10.1 创建 `d1/D1TabRelatedPartyCheck.vue`
...
- [ ] 11. 实现 D1TabPledgeCheck.vue（D1-12 质押检查组件 ~350行）
  - [ ] 11.1 创建 `d1/D1TabPledgeCheck.vue`
...
- [ ] 12. 实现 D1TabSamplingVouching.vue（D1-13 抽样凭证核对组件 ~400行）
  - [ ] 12.1 创建 `d1/D1TabSamplingVouching.vue`
...
- [ ] 13. Checkpoint - 四个Vue组件验证
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 14. 主入口集成与代码拆分
  - [ ] 14.1 在 GtD1NotesReceivable.vue 中集成四个独立Tab组件
  ...
  - [ ] 14.2 从 useD1NotesReceivable.ts 中删除已拆出的监盘核查逻辑
...
- [ ] 15. 实现双模式切换（四表通用）
  - [ ] 15.1 在四个Vue组件中实现 HTML ↔ OnlyOffice 双模式
...
- [ ] 16. 实现导入导出三级（四表通用）
  ...
  - [ ]* 16.2 编写 Property 8 PBT：导入导出 Round-Trip
...
- [ ] 17. 复核对话集成与持久化验证
  - [ ] 17.1 在四个Vue组件中集成 GtReviewDialog
...
- [ ] 19. P0共享基础设施抽取
  - [ ] 19.1 创建 `composables/d1SharedFormulas.ts`（~80行）
  ...
  - [ ] 19.2 创建 `composables/useD1ImportExport.ts`（~150行）
  ...
  - [ ] 19.3 创建 `composables/useD1DualMode.ts`（~80行）
  ...
  - [ ] 19.4 创建 `d1/D1AuditNoteSection.vue`（~100行）
...
- [ ] 20. P1 D1-12从备查簿D1-7自动导入已质押票据
  - [ ] 20.1 在 useD1PledgeCheck.ts 中实现"从备查簿导入"逻辑
    - 从allResponses读取D1-memo-rows JSON → 筛选"是否质押=是"的行
    - 映射到PledgeRow的A-I列（9列票据基本信息），J-P留空
    - 返回映射结果供Vue组件调用
    - _Requirements: 19.1, 19.2, 19.3_

  - [ ] 20.2 在 D1TabPledgeCheck.vue 中添加"从备查簿导入"按钮和交互
