...
- [ ] 1. Phase0 双源核对：S35 结构落定
  - 运行 `analyze_s_category.py`，确认 S35-1~5 程序表 + 明细核查子表（S35-x-1）sheet 名、公式与核查判断列
  - 产出 5 核查底稿 Tab 标签 + 子表映射表
  - _Requirements: 3.1, 3.3, 4.1_

- [ ] 2. 后端注册与 skip 映射
  - [ ] 2.1 VALID_COMPONENT_TYPES 新增 `s35-refinance-bundle`
    - _Requirements: 1.4_
  - [ ] 2.2 wp_code_overrides：`S35→s35-refinance-bundle`，S35-1~5 及子表 → `skip`
    - _Requirements: 1.2, 2.1, 2.2_
  - [ ] 2.3 validate_overrides 校验通过（单测）
    - _Requirements: 1.4_

- [ ] 3. useS35BundleState composable
  - [ ] 3.1 wpIdMap（wp_index 过滤 S35-*，含子表）+ 可见 Tab 计算
    - _Requirements: 6.1, 6.2_
  - [ ] 3.2 completionMap + progressSummary + refreshCompletion
    - _Requirements: 8.1_

- [ ] 4. GtS35Bundle.vue 主入口 + 子表
  - [ ] 4.1 5 核查底稿 TabDef + el-tabs；子表子 sheet 切换（核查程序/明细核查表）
    - _Requirements: 3.1, 3.2, 3.3_
  - [ ] 4.2 明细子表公式实时重算（公式列只读）+ 保留核查判断列 + 动态行导入导出
    - _Requirements: 4.1, 4.2, 4.3_
  - [ ] 4.3 sheetName 路由 + readonly 透传
    - _Requirements: 5.1, 5.2, 5.3, 5.4, 8.4, 8.5_

- [ ] 5. 引用与前端注册
  - [ ] 5.1 索引列 GtIndexChip（prop `value`）内部/外部跳转分流 + 灰态兜底
    - _Requirements: 7.1, 7.2, 7.3, 7.4_
  - [ ] 5.2 htmlRendererRegistry 注册 `s35-refinance-bundle`（defineAsyncComponent, contextProps standard）
...
- [ ] 7. PBT 与集成测试
  - [ ] 7.1 PBT（fast-check ≥100 次）：P1 skip / P2 wp_id / P3 可见性 / P4 路由 / P5 子表公式 / P6 进度 / P7 readonly
    - _Requirements: 1.2, 2.1, 2.2, 6.1, 6.2, 6.3, 6.4, 5.1, 5.4, 4.2, 8.1, 8.4_
  - [ ] 7.2 集成 + Playwright：点击 S35→页签切换→子表填写重算→导入导出往返→只读模式
