...
Bugfix for QC Python Rule Load Hardening: `_load_class_from_dotted_path` 无前缀限制 + �?admin 可创�?python 类型规则 �?受限 RCE 面。修复策略：前缀白名�?+ admin-only 限制 + �?沙箱"误导注释�?
...
- [x] 3. Fix for QC Python Rule Load Hardening

  - [x] 3.1 Add `_ALLOWED_RULE_PREFIXES` constant
  ...
  - [x] 3.2 Add prefix whitelist check in `_load_class_from_dotted_path`
  ...
  - [x] 3.3 Add admin-only check in `create_rule` endpoint for python type
  ...
  - [x] 3.4 Add admin-only check in `update_rule` endpoint for python type
  ...
  - [x] 3.5 Replace "沙箱" misleading comments
