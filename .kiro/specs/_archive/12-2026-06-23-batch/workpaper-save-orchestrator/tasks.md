...
- [x] 1. Create `WorkpaperSaveOrchestrator` class
...
- [x] 2. Unit tests for `WorkpaperSaveOrchestrator`
...
- [x] 3. Migrate snapshot_writer to use orchestrator (Phase 1 �?消除孤立总线)
  - [x] 3.1 Import orchestrator in snapshot_writer.py
  - [x] 3.2 After _write_workpaper_cell succeeds, call `orchestrator.after_save(...)` instead of manual `prefill_stale` + local `event_bus.emit`
  - [x] 3.3 Delete `_EventBus` class and `event_bus` instance from `custom_query/metrics.py`
  - [x] 3.4 Verify cross_ref/stale/SSE triggered after writeback (integration test)
  - _Requirements: 1.3, 3.1, 3.2_

- [x] 4. Migrate wp_html_save to use orchestrator (Phase 2a)
...
- [x] 5. Migrate wp_editor_router.save_univer_data to use orchestrator (Phase 2b)
...
- [x] 6. Migrate onlyoffice_callback_service to use orchestrator (Phase 3)*
...
- [x] 7. Integration test: concurrent writes across different paths �?one 409
