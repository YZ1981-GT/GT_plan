...
- [x] 1. 数据库迁移：创建 dispatch_records 表
  - [x] 1.1 创建 V092__create_dispatch_records.sql 迁移文件
  ...
  - [x] 1.2 创建 R092__create_dispatch_records.sql 回滚文件
    - `DROP TABLE IF EXISTS dispatch_records;`
    - _Requirements: 1.1_

- [x] 2. 后端 ORM 模型
  - [x] 2.1 创建 dispatch_models.py
...
- [x] 5. 后端 REST Router
  ...
  - [x] 5.2 注册 router 到主 app
    - 在 `backend/app/main.py` 或 router_registry 中注册 dispatch_records router
    - _Requirements: 2.1_

- [x] 6. EventBus 事件类型扩展
  - [x] 6.1 新增 DISPATCH_CREATED / DISPATCH_REVOKED 事件类型
...
- [x] 9. 改造 useConfirmationDispatch.ts
  - [x] 9.1 接入后端 API 替代纯内存 dispatchedMap
