# Implementation Plan: S33 应对14号公告核查聚合组件

## Overview

对齐 `s34-ipo-bundle` / `a17-bundle` 聚合模式：1 个 bundle componentType + 9 核查底稿 skip + 可滚动页签 + GtAProgramConsole embedded + 隐藏程序表变体切换 + 提示折叠 + 完成仪表盘。PBT 用 fast-check。
...
- S33-4 含隐藏程序表变体（默认渲染可见版，提供切换完整版入口）。
