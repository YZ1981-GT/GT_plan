...
- [x] 1. 创建 C2~C15 Guidance JSON 文件（L1 Dialog 用）
  - [x] 1.1 创建 C2~C8 的 Guidance JSON 文件
  ...
  - [x] 1.2 创建 C9~C15 的 Guidance JSON 文件
    - 在 `backend/data/wp_guidance/` 目录下创建 `C9.json` ~ `C15.json`（7 个文件）
    - 格式与 C2~C8 一致：wp_code/title/sections/source
    - sections 内容从对应源模板提取
    - _Requirements: 1.1, 1.2, 1.3, 1.4_

- [x] 2. 创建 C2~C15 Guidance JSON 文件（Cx-2 偏差评价用）
  - [x] 2.1 创建 C2-2~C8-2 的 Guidance JSON 文件
  ...
  - [x] 2.2 创建 C9-2~C15-2 的 Guidance JSON 文件
    - 在 `backend/data/wp_guidance/` 目录下创建 `C9-2.json` ~ `C15-2.json`（7 个文件）
    - 格式与 C2-2~C8-2 一致
    - _Requirements: 1.1, 1.2, 1.3, 1.4, 3.1_

- [x] 3. 后端 render 策略增强 `_load_guidance`
  - [x] 3.1 在 `_c_control_test.py` 中实现 `_load_guidance` 函数
...
- [x] 4. Checkpoint - 后端 Guidance 加载验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. 前端 L1 Dialog 琥珀块渲染
  - [x] 5.1 在 GtCControlTest.vue 的 L1 Dialog 中添加琥珀块模板
...
- [x] 6. 前端 Cx-2 偏差评价弹窗琥珀块渲染
  - [x] 6.1 在偏差评价区域添加琥珀块模板
...
- [x] 7. 创建 useOcrAttachmentCache composable
  - [x] 7.1 实现 composable 核心逻辑
  ...
  - [x]* 7.2 编写 useOcrAttachmentCache 属性测试
    - **Property 3: OCR 可识别性分类**
    - **Property 4: OCR 缓存避免重复调用**
    - **Validates: Requirements 4.2, 4.3, 6.1, 6.2, 6.3**

- [x] 8. 创建 OcrAttachmentPicker.vue 组件
  - [x] 8.1 实现 OcrAttachmentPicker 组件
  ...
  - [x]* 8.2 编写 OCR 文本截断属性测试
    - **Property 5: OCR 文本拼接与截断**
    - **Property 6: OCR 部分失败容错**
    - **Validates: Requirements 4.4, 4.6, 5.3**

- [x] 9. Checkpoint - 前端组件核心验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. L1 Dialog AI 按钮集成 OCR 附件选择
  - [x] 10.1 改造 CControlTestSubPage.vue AI 生成流程
...
- [x] 11. Cx-2 偏差评价 AI 按钮集成 OCR 附件选择
  - [x] 11.1 改造偏差评价区域 AI 生成流程
...
- [x] 12. Dialog 生命周期缓存管理
  - [x] 12.1 集成缓存管理到 Dialog 开关逻辑
