...
- [x] 0.1 openpyxl脚本读取M7专项储备.xlsx全部9 sheet
...
- [x] 4.1 创建 M7TabIndex.vue 底稿目录
  - _Requirements: 1.2_

- [x] 4.2 创建 M7TabAdjudication.vue 审定表M7-1
  - 权益类单区块+计提/使用+TB回写
  - _Requirements: 2.1-2.7_

- [x] 4.3 创建 M7TabDetail.vue 明细表M7-2
  - 27列区段Tab(计提/费用化/资本化)+22公式+动态行+导入导出
  - _Requirements: 3.1-3.5_

- [x] 4.4 创建 M7TabAccrualTest.vue 计提测试M7-4
  - 安全生产费按产量/收入计提测试+9公式
  - _Requirements: 4.1-4.6_

- [x] 4.5 创建 M7TabExpenditureCheck.vue 支出检查M7-5
  - 资本化/费用化区分+结论区+AI辅助
  - _Requirements: 5.1-5.3_

- [x] 4.6 创建 M7TabAdjustment.vue + M7TabDisclosureListed/Soe.vue
  - 调整借贷平衡 + 附注上市/国企切换
  - _Requirements: 6.1-6.3_

### Phase 5: 后端

- [x] 5.1 创建 m7_special_reserve_renderer.py
  - RENDERER_DISPATCH注册 + 权益类公式验证
  - _Requirements: 1.6_

- [x] 5.2 创建 m7_special_reserve.py 路由
  - 导出模板/导出数据/导入数据 + 计提测试API
  - _Requirements: 7.6_

- [x] 5.3 创建 m7_special_reserve_service.py
  - 安全生产费计提测试+汇总
...
- [x] 7.1 单元测试：useM7FormulaEngine + useM7AccrualEngine
  - 权益类方向 + 按产量/收入计提 + 计提差异
  - _Requirements: P1-P6_

- [x] 7.2 集成测试：计提测试 + 资本化→H1
...
- M7专项储备是权益类贷方科目（期末=期初+贷方-借方）
- 安全生产费按产量/收入分档计提是M7核心特殊功能
- 资本性支出联动H1固定资产
...
```json
{
  "waves": [
    ...
    { "id": "wave-2", "tasks": ["2.1", "2.2", "2.3", "2.4", "2.5", "2.6", "2.7", "2.8"] },
    { "id": "wave-3", "tasks": ["3.1", "3.2", "3.3", "3.4"] },
    { "id": "wave-4", "tasks": ["4.1", "4.2", "4.3", "4.4", "4.5", "4.6"] },
    { "id": "wave-5", "tasks": ["5.1", "5.2", "5.3"] },
    { "id": "wave-6", "tasks": ["6.1", "6.2", "6.3"] },
    { "id": "wave-7", "tasks": ["7.1", "7.2", "7.3"] }
  ]
}
```
