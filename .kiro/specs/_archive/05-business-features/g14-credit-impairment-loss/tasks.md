...
- [x] 2. 实现公式引擎 useG14FormulaEngine.ts
  - [x] 2.1 创建 `composables/useG14FormulaEngine.ts`，实现7个纯函数+parseNum
  ...
  - [x]* 2.3 编写 Property 2 PBT：净信用减值损失公式
  ...
  - [x]* 2.4 编写 Property 3 PBT：坏账准备滚动恒等
  ...
  - [x]* 2.5 编写 Property 4 PBT：坏账准备滚动验证检测
  ...
  - [x]* 2.6 编写 Property 5 PBT：变动率方向性与除零保护
  ...
  - [x]* 2.8 编写 Property 7 PBT：parseNum健壮性
...
- [x] 4. 实现数据层composable
  ...
  - [x] 4.2 创建 `composables/useG14ImportExport.ts` + `composables/useG14DualMode.ts`
...
- [x] 5. 实现Vue子组件（6个sheet）
  - [x] 5.1 创建 `g14-credit-impairment-loss/G14TabProcedure.vue`（G14A程序表）
  ...
  - [x] 5.2 创建 `g14-credit-impairment-loss/G14TabAdjudication.vue`（G14-1审定表36行×11列）
    - 固定9类减值来源行+合计（与xlsx一致）
    - 列结构：项目|本期(未审|调整|审定)|上期(未审|调整|审定)|变动额|变动率|原因分析
    - 本期自G14-2同步；上期独立录入；|变动率|>30%橙色+原因必填
    - trial_balance(6702) + EventBus publish
    - _Requirements: 2.2, 2.3, 2.4, 2.5_

  - [x] 5.3 创建 `g14-credit-impairment-loss/G14TabDetail.vue`（G14-2明细表36行×13列）
    - 固定9行+合计；减值准备滚动+核对列
    - 期末滚动不平衡红色高亮
    - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5, 3.6_

  - [x] 5.4 创建 `g14-credit-impairment-loss/G14TabAdjustment.vue`（G14-3调整分录22行×10列）
    - 调整事项汇总表（D3调整分录模块同构）+借贷平衡+同步至G14-2
    - _Requirements: 4.1_

  - [x] 5.5 创建 `g14-credit-impairment-loss/G14TabDisclosureListed.vue` + `G14TabDisclosureSOE.vue`（附注披露）
...
## D4-level polish (Waves 5–9)

- [x] Wave 5–8：目录跳转、附注drawer、截止面板、G14-2 Tab拆分、ECL索引、交叉验证栏
- [x] Wave 9：截止样本回填 G14-3；G14-3 行级 GtReviewDot
- [x] Wave 10：GtWpRenderer b-index→G14 TabDirectory 委托（D4TabIndex 同级）
- [x] Wave 11：GCycleBIndexExtras — 底稿架构 + 本循环跨底稿目录

## D4-level polish (Wave 12)

- [x] Wave 12：去掉 legacy 复核按钮；useG14ExternalCross + G14-2 ECL 源科目勾稽栏；G14-1 pending 交叉栏
- [x] Wave 12：G14A 程序表 E2E；gCycleExternalCross 单测
