...
- [x] 0.1 openpyxl脚本读取K13营业外支出.xlsx全部9 sheet
  - 产出：k13_structure_summary.json（确认K13-1 69公式/K13-2 31公式）
  - _Requirements: 双源输入流程_

- [x] 0.2 K营业外支出循环底稿模板库md交叉验证
...
- [x] 2.1 创建 useK13FormulaEngine.ts（损益类！）
...
- [x]* 2.2 编写 Property CP-K13-01 PBT：审定数公式链
  - 断言：calcAuditedAmount(u, a, r) === u + a + r
  - **Feature: k13-non-operating-expense, Property CP-K13-01: 审定数公式链**
  - **Validates: Requirements 2.3**

- [x]* 2.3 编写 Property CP-K13-02 PBT：损益类发生额（借-贷）
...
- [x]* 2.4 编写 Property CP-K13-03 PBT：同比变动率
...
- [x]* 2.5 编写 Property CP-K13-04 PBT：占比公式
  - 生成器：item∈R, total>0
  - 断言：calcProportion(item, total) === item/total
  - **Feature: k13-non-operating-expense, Property CP-K13-04: 占比=单项/合计**

- [x]* 2.6 编写 Property CP-K13-05 PBT：合计行恒等
  - 断言：calcSubtotal(arr) === Σarr
  - **Feature: k13-non-operating-expense, Property CP-K13-05: 合计行恒等**

### Phase 3: Composable层

- [x] 3.1 创建 useK13FormData.ts
  - selfLoad + checklist_responses + writebackTB(**发生额**，科目6711，从tb_ledger取数)
  - _Requirements: 1.9, 1.10, 2.6, 5.1-5.2_

- [x] 3.2 创建 useK13CrossSheet.ts（adjudicationVsDetail）
  - _Requirements: 2.5, 3.2_

- [x] 3.3 创建 useK13DualMode.ts + useK13ImportExport.ts
  - _Requirements: 3.3, 6.2_

- [x] 3.4 创建 useK13Adjudication.ts + useK13Detail.ts + useK13Check.ts
  - _Requirements: 2~6 全部_

### Phase 4: Vue子组件

- [x] 4.1 创建 K13TabIndex.vue 底稿目录
  - _Requirements: 1.2_

- [x] 4.2 创建 K13TabAdjudication.vue 审定表K13-1（损益类69公式+发生额取数+按去向分行+TB回写）
  - _Requirements: 2.1-2.7_

- [x] 4.3 创建 K13TabDetail.vue 明细表K13-2（26列3区段+动态行+27行+导入导出）
  - _Requirements: 3.1-3.4_

- [x] 4.4 创建 K13TabNonOperatingCheck.vue K13-4（分类正确性+税前扣除性检查+抽凭+OCR+不合规摘要）
  - _Requirements: 4.1-4.3_

- [x] 4.5 创建 K13TabAdjustment.vue 调整分录（借贷平衡+EventBus）
  - _Requirements: 6.2_

- [x] 4.6 创建 K13TabDisclosureListed.vue + K13TabDisclosureSoe.vue 附注双版本
  - 按去向披露+自动取数
  - _Requirements: 6.1_

### Phase 5: 后端

- [x] 5.1 创建 _k13_non_operating_expense.py render策略 + RENDERER_DISPATCH注册（**损益类取数**，tb_ledger发生额）
  - _Requirements: 1.6, 5.1_

- [x] 5.2 创建 _k13_import_export.py 导入导出3端点
  - _Requirements: 3.3, 6.2_

- [x] 5.3 创建 _k13_ai_generate.py AI生成 + 更新 k13-non-operating-expense.yaml
  - section: non-operating-eval / overall-opinion
  - _Requirements: 6.1_

### Phase 6: 集成联动

- [x] 6.1 EventBus: TB回写(6711发生额) + substantive:adjudicated → 附注
...
- [x] 7.1 单元测试：useK13FormulaEngine（损益方向(借-贷) + 同比/占比 + 边界）
  - _Requirements: CP-K13-01~05_

- [x] 7.2 集成测试：损益取数(发生额) + 明细聚合 + 审定回写
  - _Requirements: 2.5, 5.1_

- [x] 7.3 Playwright E2E：打开K13→审定(验证发生额)→明细→检查→保存
  - _Requirements: 全部_
