...
- [x] UAT-1: 陕西华氏试算表有 ≥100 行数据
- [x] UAT-2: 点"数据质量检查"，能看到差异报告
- [x] UAT-3: 进入报表页面，BS 有非零数据行（货币资金/应收账款等）
- [x] UAT-4: 点击报表某行金额，穿透到科目明细
- [x] UAT-5: 进入底稿列表，有 ≥20 个底稿
- [x] UAT-6: 进入附注编辑器，目录树有章节
- [x] UAT-7: 流程进度条在每个页面正确显示
- [x] UAT-8: 宜宾大药房也能走通试算表→报表流程
