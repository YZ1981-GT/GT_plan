...
- [x] 0.1 openpyxl脚本读取M9其他综合收益.xlsx全部9 sheet
  - 提取结构 + 确认审定表M9-1（47×12, 41公式✅）+明细M9-2（46×30, 145公式/42模式）+核对表M9-4（42×9, 51公式/11模式）
  - 产出：m9_structure_summary.json (117KB)
  - 脚本：scripts/m9_structure_extract.py
  - _Requirements: 双源输入流程_

- [x] 0.2 M股东权益循环底稿模板库md交叉验证
  - 核对：权益类方向/OCI两大类/税后净额/G8-J2来源联动
  - 产出：m9_conflict_resolution.md
  - **结果**: md模板库不含M9(仅M0~M8)→xlsx为唯一权威源；4项验证全部✅无冲突
  - _Requirements: 双源输入流程_
...
- [x] 1.2 编写注册契约测试
  ...
  - **结果**: 13 passed + 1 todo(RENDERER_DISPATCH待Phase5)
  - _Requirements: 1.6, 1.7, 1.8_

### Phase 2: 公式引擎+OCI引擎+PBT

- [x] 2.1 创建 `useM9FormulaEngine.ts`（权益类！）
  - calcAuditedAmount / calcEquityEndBalance(b,cr,dr)=b+cr-dr / calcSubtotal
  - _Requirements: 2.3-2.4, 6.4_

- [x] 2.2 创建 `useM9OciEngine.ts`
...
- [x] 4.1 创建 M9TabIndex.vue 底稿目录
  - _Requirements: 1.2_

- [x] 4.2 创建 M9TabAdjudication.vue 审定表M9-1
  - 双大类(不可/可重分类)+TB回写
  - _Requirements: 2.1-2.7_

- [x] 4.3 创建 M9TabDetail.vue 明细表M9-2
  - 30列区段Tab(不可/可重分类/税额)+税后净额+34公式+动态行+导入导出
  - _Requirements: 3.1-3.7_

- [x] 4.4 创建 M9TabOciReconcile.vue 核对表M9-4（核心！）
  - 多来源核对(接收G8/J2/外币)+13公式+AI辅助
  - _Requirements: 4.1-4.8_

- [x] 4.5 创建 M9TabAdjustment.vue + M9TabDisclosureListed/Soe.vue
  - 调整借贷平衡 + 附注上市/国企(67×21,20公式)切换
  - _Requirements: 5.1-5.4_

### Phase 5: 后端

- [x] 5.1 创建 m9_other_comprehensive_income_renderer.py
  - RENDERER_DISPATCH注册 + 权益类公式验证
  - _Requirements: 1.6_

- [x] 5.2 创建 m9_other_comprehensive_income.py 路由
  - 导出模板/导出数据/导入数据 + OCI核对API
  - _Requirements: 6.6_

- [x] 5.3 创建 m9_other_comprehensive_income_service.py
  - OCI税后净额+多来源核对
...
- [x] 6.1 EventBus集成
  ...
  - **验证结果**: 全部EventBus连接已正确接线：
    - useM9FormData.writebackTB → emit 'substantive:adjudicated' {accountCode:'4103', auditedAmount, wpCode:'M9'} ✅
    - useM9Adjustment.saveAndPublish → emit 'adjustment:created' {wpCode:'M9', accountCode:'4103', ajeNet, rjeNet} ✅
    - useM9CrossSheet → on 'g8:fair-value-changed' / 'j2:remeasured' (持久化到checklist_responses) ✅
    - useM9OciReconcile → on 'g8:fair-value-changed' / 'j2:remeasured' (更新核对表来源值) ✅
    - M9TabAdjudication.vue → on 'adjustment:created' / subscribeAdjudicated / subscribeDisclosure ✅
    - M9TabDisclosureListed.vue → on 'substantive:adjudicated' (loadData刷新) ✅
    - M9TabDisclosureSoe.vue → on 'substantive:adjudicated' (loadData刷新) ✅
    - 修复: M9TabOciReconcile.vue console.log → no-console eslint error fixed
...
- [x] 7.1 单元测试：useM9FormulaEngine + useM9OciEngine
  - 权益类方向 + 税后净额 + 多来源核对 + 两大类汇总
  - _Requirements: P1-P6_

- [x] 7.2 集成测试：G8/J2→M9 OCI核对
  - _Requirements: 4.1-4.8_
