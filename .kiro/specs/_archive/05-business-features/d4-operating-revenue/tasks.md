...
D4营业收入底稿专属组件`d4-operating-revenue`。平台最大单科目底稿（42有效sheet/8 xlsx/~280+公式）。

主入口 GtD4OperatingRevenue.vue（sheetName v-if分发，defineAsyncComponent lazy）+ 38个子组件 + 18个composable + 后端4个py文件。

### 实际实现与原计划差异

1. **架构**：无内部el-tabs→sheetName prop v-if分发（外层GtWpRenderer已有目录行chips）
2. **useD4Inspection已删除**：拆为useD4ContractInspection + useD4WalkthroughTest + useD4CompletenessCheck三个独立composable
3. **附注**：useD4DisclosureListed/Soe合并为单个useD4Disclosure（variant参数区分）
4. **D4-12**：升级为三模式（卡片/矩阵/OO）+D4ContractCard+D4ContractMatrix子组件+OCR
5. **D4-14**：升级为穿行测试7维度+一致性引擎+D4WalkthroughCard+D4WalkthroughMatrix
6. **D4-5**：升级为真AI接入+GtIndexChip真跳转+引导式6步流程+GT紫配色
7. **D4-22A**：改为包装组件D4TabIpoProcedure.vue（selfLoad带sheet_name解决聚合包内数据错误）
8. **新增composable**：useD4KeyIndicator/useD4InvoiceCompare/useD4CustomerDetail（IPO组按sheet独立）
9. **后端新增**：_d4_contract_ocr.py（UnifiedOCR→vLLM结构化JSON提取20字段）
...
- [x] 1. 组件注册与基础配置
  - [x] 1.1 注册componentType和映射
    ...
    - 创建 `GtD4OperatingRevenue.vue` 主入口骨架（sheetName prop v-if分发 + defineAsyncComponent lazy + selfLoad）
    - _Requirements: 1.1, 1.6, 1.7, 1.8, 1.9_
    - **实际差异**：无内部el-tabs，外层GtWpRenderer提供sheet目录行chips导航
...
- [x] 11. 实现检查程序组 D4-12~20
  - [x] 11.1 创建检查程序组composable（实际拆为3个独立composable）
    - `useD4ContractInspection.ts` — D4-12合同检查（20+1字段×N份+覆盖率+OCR填充）
    - `useD4WalkthroughTest.ts` — D4-14穿行测试（7维度+一致性引擎+mapOcr/mapLedger/mapContract）
    - `useD4CompletenessCheck.ts` — D4-15完整性检查（3维度+自动√/×+reindex）
    - D4-13/D4-16~D4-20逻辑内联到各Vue子组件（体量不大无需独立composable）
    - _Requirements: 9.1-9.7, 10.1-10.8, 11.1-11.8, 12.1-12.8_
    - **原设计useD4Inspection.ts已删除(2026-07-02)**：无任何import引用

  - [x] 11.2 创建9+5个Vue子组件（d4/inspection/）
    - D4TabContract.vue — 三模式（卡片/矩阵/OO）+ el-segmented
    - D4ContractCard.vue — 5组20+1字段表单子组件
    - D4ContractMatrix.vue — 只读横向对比表（行=20字段,列=N份合同）
    - D4TabErpCheck.vue / D4TabOccurrence.vue / D4TabCompleteness.vue
    - D4WalkthroughCard.vue — 7维度el-collapse+维度级OCR+一致性校验卡片
    - D4WalkthroughMatrix.vue — 只读el-table行=事项列=7维度✓/×
    - D4TabExport.vue（三分组表格）/ D4TabCutoffForward.vue / D4TabCutoffBackward.vue
    - D4TabDiscount.vue / D4TabReturn.vue（6大结构区域711行）
...
- [x] 17. 实现后端导入导出端点
  ...
  - [x] 17.2 创建 `_d4_contract_ocr.py`（独立spec d4-12-contract-inspection追加）
    - POST /api/workpapers/{wp_id}/d4/contract-ocr
    - multipart/form-data文件上传 + UnifiedOCRService识别
    - vLLM结构化JSON提取20字段 + confidence分数
    - 返回 attachment_id + ocr_text + extracted_fields + confidence
    - 注册路由到router_registry "AI与辅助"组

  - [x]* 17.3 编写 Property 15 PBT（后端hypothesis）：Round-Trip
...
- [x] 20. 集成主入口 GtD4OperatingRevenue.vue
  - [x] 20.1 完善主入口组件
    - sheetName prop v-if分发（正则提取末尾编码如D4-1/D4-22A）
    - defineAsyncComponent lazy加载全部子组件
    - IPO/舞弊组v-if条件可见（ipoGroupVisible computed）
    ...
    - **实际差异**：无嵌套el-tabs，Tab顺序由account_package_registry sheets数组控制
...
- 由于36 sheet规模大，composable按功能域分组而非每sheet一个（useD4Analysis覆盖D4-6~11，useD4Ipo覆盖D4-22A~32）
- **useD4Inspection已删除(2026-07-02)**：原单个composable覆盖D4-12~20→拆为3个独立composable(useD4ContractInspection/useD4WalkthroughTest/useD4CompletenessCheck)+各sheet独立Vue子组件。旧composable无任何引用，已删除
- **附注合并为useD4Disclosure(variant)**：原计划分两个composable→实际用variant参数区分上市/国企，减少代码重复
- **D4-12三模式(2026-07-02追加)**：卡片视图(D4ContractCard×N份el-tabs card)/矩阵视图(D4ContractMatrix只读横向对比)/在线编辑(OO)，el-segmented三档切换
- **D4-14穿行测试(d4-14-walkthrough-test spec独立完成)**：7维度一致性引擎(compareAmounts/compareDates/compareProductNames/computeConsistency)+D4WalkthroughCard(7维度el-collapse+维度级OCR)+D4WalkthroughMatrix(只读汇总)，420行composable
- **D4-5政策检查(2026-07-01精修)**：从假AI(setTimeout)升级为真AI(/d4/ai-generate policy-evaluation/adj-conclusion)+假span chip换真GtIndexChip+引导式6步圆点流程+GT紫配色+el-segmented双模式
