...
- [x] 0.1 openpyxl脚本读取J3股份支付.xlsx全部6 sheet（含2个skip辅助说明）
...
- [x] 2.1 创建 `useJ3FormulaEngine.ts`（费用分摊）
...
- [x] 2.2 创建 `useJ3OptionPricingEngine.ts`（核心！Black-Scholes）
  - normalCDF / calcD1 / calcD2 / calcBlackScholes / validateBSParams
  - _Requirements: 3.1-3.6_

- [x]* 2.3 编写 Property CP-J3-01 PBT：Black-Scholes定价公式
...
- [x]* 2.6 编写 Property CP-J3-04 PBT：剩余费用=总FV-累计
...
- [x]* 2.7 编写 Property CP-J3-05 PBT：总公允=单位FV×数量
  - 生成器：unitFV>0, quantity>0 (整数)
  - 断言：calcTotalFairValue(u, q) === u × q
  - **Feature: j3-share-based-payment, Property CP-J3-05: 总公允价值公式**

- [x]* 2.8 编写 Property CP-J3-06 PBT：合计行恒等
  - 断言：calcSubtotal(arr) === Σarr
  - **Feature: j3-share-based-payment, Property CP-J3-06: 合计行恒等**

### Phase 3: Composable层

- [x] 3.1 创建 useJ3FormData.ts（selfLoad，注意：无单一TB回写！）
  - 跨多科目：不做writebackTB，改用EventBus通知
- [x] 3.2 创建 useJ3CrossSheet.ts（情况表vs检查表 + M4联动 + J1联动）
- [x] 3.3 创建 useJ3Detail.ts（18列情况表+费用分摊+BS参数）
- [x] 3.4 创建 useJ3Check.ts（19列检查表+段落型+CAS11）
- [x] 3.5 创建 useJ3Disclosure.ts + useJ3ImportExport.ts + useJ3DualMode.ts

### Phase 4: Vue子组件

- [x] 4.1 创建 J3TabIndex.vue 底稿目录
- [x] 4.2 创建 J3TabDetail.vue 情况表（18列+BS定价结果+费用分摊+权益/现金标注）
- [x] 4.3 创建 J3TabCheck.vue 检查表（19列段落型+CAS11+BS参数逐项验证+IPO面板）

### Phase 5: 后端

- [x] 5.1 创建 _j3_share_based_payment.py render策略 + RENDERER_DISPATCH
  - 跨科目逻辑 + BS参数格式
- [x] 5.2 创建 _j3_import_export.py 导入导出3端点
- [x] 5.3 创建 _j3_ai_generate.py AI生成（BS参数合理性分析）
- [x] 5.4 更新 wp_render_schema: j3-share-based-payment.yaml

### Phase 6: 集成联动

- [x] 6.1 EventBus: share-payment:expense-recognized → M4/J1联动
- [x] 6.2 cross_wp_references: J3→M4资本公积 / J3→J1应付职工薪酬
- [x] 6.3 GtIndexChip: J3→M4 / J3→J1跳转
- [x] 6.4 权益结算→M4 / 现金结算→J1 自动分流
- [x] 6.5 双模式OO + IPO适用性判断

### Phase 7: 测试验收

- [x] 7.1 Vitest: useJ3OptionPricingEngine（BS模型数学精度）+ useJ3FormulaEngine（费用分摊）
- [x] 7.2 Vitest组件: sheetName分发 + 权益/现金模式切换
- [x] 7.3 后端pytest: render+导入导出+BS参数验证
- [x] 7.4 Playwright E2E: J3费用确认→M4联动→J1联动全链路
