...
- [x] 0.1 openpyxl脚本读取H1固定资产.xlsx全部26 sheet
...
- [x] 2.1 创建 `composables/useH1FormulaEngine.ts`，实现全部纯函数
...
- [x] 2.2 创建 `composables/useH1DepreciationEngine.ts`，实现折旧纯函数
...
- [x]* 2.3 编写 Property P1 PBT：资产类审定数公式链
...
- [x]* 2.6 编写 Property P4 PBT：三角勾稽恒等式
...
- [x]* 2.8 编写 Property P6 PBT：直线法折旧正确性
...
- [x]* 2.9 编写 Property P7 PBT：双倍余额递减法（最后两年转直线）
...
- [x]* 2.10 编写 Property P8 PBT：年数总和法折旧逐年递减
...
- [x]* 2.11 编写 Property P9 PBT：折旧累计单调递增
...
- [x]* 2.12 编写 Property P10 PBT：DCF现值计算
...
- [x]* 2.13 编写 Property P11 PBT：可收回金额MAX选取
...
- [x]* 2.14 编写 Property P12 PBT：减值金额非负且≤账面
...
- [x]* 2.19 编写 Property P17 PBT：权属差异计算
...
- [x] 3.1 创建 `composables/useH1FormData.ts`
...
- [x] 3.2 创建 `composables/useH1CrossSheet.ts`
...
- [x] 3.3 创建 `composables/useH1Adjudication.ts`
...
- [x] 3.4 创建 `composables/useH1Detail.ts`
...
- [x] 4.1 创建 `h1/core/H1TabIndex.vue`（~150行）
  - 底稿目录（26行进度条+完成状态）
  - _Requirements: 1.2_

- [x] 4.2 创建 `h1/core/H1TabAdjudication.vue`（~450行）
  - 双区块el-table + 三角勾稽校验红色高亮
  - 审计说明/结论 + AI + 💬复核 + GtIndexChip→H1-6
  - _Requirements: 2.1-2.12, 19.5-19.6_

- [x] 4.3 创建 `h1/core/H1TabDetail.vue`（~500行）
  - 4区段Tab切换 + 行同步 + 固定列 + 虚拟滚动
  - 合计行 + 交叉验证H1-1 + 导入导出
  - _Requirements: 3.1-3.12_

- [x] 4.4 创建 `h1/core/H1TabAdjustment.vue`（~250行）
  - el-table 13列 + 借贷平衡 + 推送A13
  - _Requirements: 4.1-4.8_

- [x] 4.5 创建 `h1/core/H1TabAnalysis.vue`（~300行）
  - 双区域(结构+变动) + 阈值高亮 + GtIndexChip
  - _Requirements: 7.1-7.8_

- [x] 4.6 创建 `h1/core/H1TabDisclosureListed.vue` + `H1TabDisclosureSoe.vue`（各~350行）
  - 多子节卡片 + 跨sheet浅蓝色 + 动态行 + 合计
  - _Requirements: 16.1-16.10_

- [x] 4.7 创建 `h1/inspection/H1TabIdleCheck.vue`（~250行）
  - el-table 12列 + 减值迹象高亮 + GtIndexChip→H1-14
  - _Requirements: 5.1-5.8_

- [x] 4.8 创建 `h1/inspection/H1TabPolicyCheck.vue`（~400行）
  - CAS4六段落卡片 + 进度条 + 引导区 + AI + 💬复核
  - _Requirements: 6.1-6.7_

- [x] 4.9 创建 `h1/inspection/H1TabAdditionCheck.vue`（~400行）
  - 双区域(抽样参数+明细) + 固定列+滚动列 + OCR📎 + 抽凭
  - _Requirements: 8.1-8.10_

- [x] 4.10 创建 `h1/inspection/H1TabDisposalCheck.vue`（~400行）
  - 双区域 + 借方/贷方视觉分组 + 处置损益公式 + GtIndexChip→H10
  - _Requirements: 9.1-9.11_

- [x] 4.11 创建 `h1/inspection/H1TabTitleBuilding.vue`（~350行）
  - el-table 22列 + 权属异常红色 + 抵押黄色 + 汇总
  - _Requirements: 14.1-14.9_

- [x] 4.12 创建 `h1/inspection/H1TabTitleVehicle.vue`（~300行）
  - el-table 18列 + 所有人异常高亮 + 年检状态
  - _Requirements: 14.1-14.9_

- [x] 4.13 创建 `h1/inspection/H1TabRelatedParty.vue`（~300行）
  - el-table 15列 + 价格差异率>10%红色 + GtIndexChip
  - _Requirements: 15.1-15.3, 15.7-15.10_

- [x] 4.14 创建 `h1/inspection/H1TabOperatingLease.vue`（~350行）
  - el-table 25列(23公式) + 收益率 + 市场租金对比
  - _Requirements: 15.4-15.5, 15.7-15.9_

- [x] 4.15 创建 `h1/inspection/H1TabFinanceLease.vue`（~300行）
  - el-table 22列 + 5项分类判断 + 利息分摊
  - _Requirements: 15.6-15.8_

- [x] 4.16 创建 `h1/stocktake/H1TabStocktakePlan.vue`（~300行）
  - 3区域(基本信息+样本选取+时间安排) + 引导区
  - _Requirements: 10.1, 10.8_

- [x] 4.17 创建 `h1/stocktake/H1TabStocktakeCheck.vue`（~400行）
  - el-table 17列 + 盘亏红色 + 汇总 + 导入导出
  - _Requirements: 10.2, 10.4-10.5, 10.10_

- [x] 4.18 创建 `h1/stocktake/H1TabStocktakeSummary.vue`（~350行）
  - 段落型+表格(仪表板+盘盈表+盘亏表+结论)
  - _Requirements: 10.3, 10.7, 10.9_

- [x] 4.19 创建 `h1/depreciation/H1TabDepreciationStraight.vue`（~450行）
  - 28列(62公式) + 分支选择器 + 月度明细 + 差异高亮
  - _Requirements: 11.1-11.2, 11.6-11.13_

- [x] 4.20 创建 `h1/depreciation/H1TabDepreciationImpair.vue`（~400行）
  - 含减值(86公式) + 减值影响列 + 差异
  - _Requirements: 11.1, 11.3, 11.6-11.13_

- [x] 4.21 创建 `h1/depreciation/H1TabDepreciationMulti.vue`（~420行）
  - 多次减值(94公式) + 多时点 + 剩余年限
  - _Requirements: 11.1, 11.4, 11.6-11.13_

- [x] 4.22 创建 `h1/depreciation/H1TabDepreciationAlloc.vue`（~300行）
  - 11列 + 分配比例 + 核对行 + GtIndexChip→D5/K8/K9
  - _Requirements: 12.1-12.8_

- [x] 4.23 创建 `h1/impairment/H1TabImpairment.vue`（~400行）
  - 双区域(迹象判断+测算表) + 方法论上下文 + GtIndexChip→H1-15
  - _Requirements: 13.1-13.2, 13.6-13.10_

- [x] 4.24 创建 `h1/impairment/H1TabRecoverable.vue`（~400行）
  - DCF模型(假设+现金流预测+折现) + 敏感性矩阵
  - _Requirements: 13.3-13.5_

### Phase 5: 后端（render策略+import_export+ai_generate+折旧引擎endpoint）

- [x] 5.1 创建 `_h1_fixed_assets.py` render策略
...
- [x] 5.2 创建 `_h1_import_export.py` 导入导出3端点
...
- [x] 5.3 创建 `_h1_ai_generate.py` AI生成8 section
...
- [x] 5.4 创建 `_h1_depreciation_engine.py` 折旧引擎端点
  - POST /h1/depreciation/calculate → 批量计算（4方法+含减值+多次减值）
  - POST /h1/depreciation/validate → 验证账面vs测算差异
  - 纯函数实现（与前端对等逻辑）
  - _Requirements: 11.5_

- [x] 5.5 创建 `_h1_fixed_assets.py` auto_data_resolver
...
- [x] 6.1 完成 GtH1FixedAssets.vue 主入口集成
...
- [x] 6.2 创建 `composables/useH1DualMode.ts`
  - HTML↔OnlyOffice el-segmented切换
  - OO健康检查(`health.data?.data?.healthy`双层兼容)
  - 切换前autoSave
  - _Requirements: 18.1-18.2_

- [x] 6.3 创建 `composables/useH1ImportExport.ts`
  - el-dropdown三级UI + axios请求（非fetch）
  - H1-2宽表按4区段分sheet导出
  - 导入xlsx解析+验证+确认对话+写入
  - _Requirements: 18.3-18.4, 18.7-18.8_

- [x] 6.4 H1-7/H1-8 抽凭引擎集成
...
- [x] 7.3 前端PBT (fast-check) — P1~P17全部验证
