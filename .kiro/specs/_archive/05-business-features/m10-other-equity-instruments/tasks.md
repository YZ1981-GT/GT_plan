...
- [x] 0.1 openpyxl脚本读取M10其他权益工具.xlsx全部10 sheet
...
- [x] 2.1 创建 `useM10FormulaEngine.ts`（权益类！）
  - calcAuditedAmount / calcEquityEndBalance(b,cr,dr)=b+cr-dr / calcSubtotal
  - _Requirements: 2.3-2.4, 6.4_

- [x] 2.2 创建 `useM10ClassificationEngine.ts`（CAS37）
...
- [x] 4.1 创建 M10TabIndex.vue 底稿目录
  - _Requirements: 1.2_

- [x] 4.2 创建 M10TabAdjudication.vue 审定表M10-1
  - 权益类单区块+按工具类型(永续债/优先股)小计+TB回写
  - _Requirements: 2.1-2.7_

- [x] 4.3 创建 M10TabDetail.vue 明细表M10-2
  - 30列区段Tab+动态行(弹prompt工具名)+导入导出
  - _Requirements: 3.1-3.5_

- [x] 4.4 创建 M10TabClassificationCheck.vue 区分检查M10-4（核心！）
  - CAS37逐条判定(64×8)+权益/负债守恒+AI辅助
  - _Requirements: 4.1-4.5_

- [x] 4.5 创建 M10TabInstrumentCheck.vue 检查表M10-5
  - 核对清单+结论区+AI辅助
  - _Requirements: 5.1-5.2_

- [x] 4.6 创建 M10TabAdjustment.vue + M10TabDisclosureListed/Soe.vue
  - 调整借贷平衡 + 附注上市/国企切换
  - _Requirements: 5.3-5.5_

### Phase 5: 后端

- [x] 5.1 创建 m10_other_equity_instruments_renderer.py
  - RENDERER_DISPATCH注册 + 权益类公式验证
  - _Requirements: 1.6_

- [x] 5.2 创建 m10_other_equity_instruments.py 路由
  - 导出模板/导出数据/导入数据 + 负债权益区分API
  - _Requirements: 6.6_

- [x] 5.3 创建 m10_other_equity_instruments_service.py
  - CAS37分类判定+金额守恒校验
...
- [x] 7.1 单元测试：useM10FormulaEngine + useM10ClassificationEngine
  - 权益类方向 + CAS37分类判定 + 金额守恒
  - _Requirements: P1-P6_

- [x] 7.2 集成测试：CAS37区分 + 负债部分联动
  - _Requirements: 4.1-4.5_
