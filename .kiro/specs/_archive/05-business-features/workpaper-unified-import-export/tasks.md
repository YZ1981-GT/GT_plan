...
- [x] 1. 数据库迁移与 ORM 模型
  - [x] 1.1 创建 V071__wp_export_import_tables.sql
  ...
  - [x] 1.2 创建 R071__wp_export_import_tables.sql 回滚脚本
    - DROP TABLE IF EXISTS wp_version_archive; DROP TABLE IF EXISTS wp_export_snapshot;
    - _Requirements: 1.5, 6.1_
  - [x] 1.3 创建 ORM 模型 WpExportSnapshot + WpVersionArchive
...
- [x] 3. MetadataCodec 元数据编解码器
  ...
  - [x] 3.2 Property 2: Metadata Embed-Extract Round-Trip
...
- [x] 4. 确定性序列化与快照哈希
  - [x] 4.1 实现 serialize_cell_value + compute_snapshot_hash
  ...
  - [x] 4.2 Property 3: Snapshot Hash Determinism
    - **Property 3: 同内容多次哈希结果相同；不同内容哈希不同**
    - hypothesis max_examples=5
    - **Validates: Requirements 1.5, 4.1**
  - [x] 4.3 Property 5: Deterministic Serialization
    - **Property 5: 同内容两次独立序列化产生 byte-identical 单元格值**
    - hypothesis max_examples=5
    - **Validates: Requirements 10.2**

- [x] 5. WpExportEngine 核心导出 **【复用】wp_xlsx_export_service**
  - [x] 5.1 实现 WpExportEngine.export_single（薄封装层）
...
- [x] 8. FormatValidator 格式校验器
  - [x] 8.1 实现 FormatValidator 类
  ...
  - [x] 8.2 Property 14: MIME Type Validation
    - **Property 14: 扩展名与 MIME 不匹配时报 error 级**
    - hypothesis max_examples=5
    - **Validates: Requirements 5.1**
  - [x] 8.3 Property 15: Sheet Structure Validation
    - **Property 15: sheet 名与 render_schema 不匹配时报 error 级**
    - hypothesis max_examples=5
    - **Validates: Requirements 5.2**
  - [x] 8.4 Property 16: Required Cell Validation
    - **Property 16: required 字段为空时报 error 级**
    - hypothesis max_examples=5
    - **Validates: Requirements 5.3**
  - [x] 8.5 Property 17: Numeric Type Validation
    - **Property 17: 数值列非数值内容报 warning 级**
    - hypothesis max_examples=5
    - **Validates: Requirements 5.5**
  - [x] 8.6 Property 18: Validation Report Structure
    - **Property 18: overall=最差级别，三类计数=len(items)正确分区**
    - hypothesis max_examples=5
    - **Validates: Requirements 5.6**

- [x] 9. WpVersionManager 版本管理器 **【新建】归档能力（现有 upload 仅 +1 无归档）**
  - [x] 9.1 实现 WpVersionManager 类
  ...
  - [x] 9.2 Property 8: Version Increment Invariant
    - **Property 8: 导入成功后 file_version=prev+1，archive 记录 source=import**
    - hypothesis max_examples=5
    - **Validates: Requirements 6.1, 6.2**
  - [x] 9.3 Property 9: Version Archive Path Format
    - **Property 9: archive_path 格式 storage/projects/{pid}/archive/{wp_id}/v{n}/**
    - hypothesis max_examples=5
    - **Validates: Requirements 6.3**
  - [x] 9.4 Property 10: Maximum 10 Retained Version Files
    - **Property 10: N>10 时最多 10 条 file_retained=true，为最近 10 个版本**
    - hypothesis max_examples=5
    - **Validates: Requirements 6.4**

- [x] 10. WpImportEngine 核心导入 **【复用】WpUploadService.upload_file + 增强**
  - [x] 10.1 实现 WpImportEngine.import_file（编排层）
  ...
  - [x] 10.2 实现程序表导入 import_program_sheet
    - 按程序编号匹配行，仅更新可编辑列
    - 不可匹配的新增行报告给用户
    - _Requirements: 8.3, 8.4_
  - [x] 10.3 实现审定表导入 import_audit_sheet
    - 忽略审定数列（系统自动计算）
    - 仅接受备注和工作结论字段更新
    - _Requirements: 9.3_
  - [x] 10.4 Property 22: Program Sheet Editable Column Isolation
    - **Property 22: 程序表导入仅更新可编辑列，只读列不变**
    - hypothesis max_examples=5
    - **Validates: Requirements 8.2, 8.3**
  - [x] 10.5 Property 24: Audit Sheet Import Ignores Computed Columns
...
- [x] 11. ConflictDetector 冲突检测器 **【复用】check_version_conflict + 新增哈希层**
  - [x] 11.1 扩展冲突检测（非全新 class）
...
- [x] 15. BatchPackager 批量打包器 **【复用】WpDownloadService.download_pack + 增强**
  ...
  - [x] 15.2 Property 11: Batch Export Completeness
    - **Property 11: 打包包含指定循环+状态筛选的全部底稿，无遗漏无重复**
    - hypothesis max_examples=5
    - **Validates: Requirements 2.1, 2.6**
  - [x] 15.3 Property 12: ZIP Directory Structure
    - **Property 12: ZIP 内每文件路径匹配 {cycle}/{wp_code}_{wp_name}.{ext}**
    - hypothesis max_examples=5
    - **Validates: Requirements 2.2**
  - [x] 15.4 Property 13: Manifest Contains All Required Fields
...
- [x] 18. 导出路由 **【复用】现有 wp_download.py 已有 download-file/download-pack + 扩展**
  ...
  - [x] 18.2 在 router_registry 注册（若新文件）
...
- [x] 20. 模板复制路由
  ...
  - [x] 20.2 在 router_registry 注册模板复制路由
    - _Requirements: 7.1_

- [x] 21. Checkpoint - API 端点联通
...
- [x] 26. Round-Trip 集成测试
  - [x] 26.1 编写 E2E round-trip 测试
...
- 当前最高迁移 V069（report_line_mapping_sign 已占用），本 spec 使用 V071（V070 留给 bad-debt spec）
