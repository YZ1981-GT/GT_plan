...
- [x] 0.1 openpyxl脚本读取N2应交税费.xlsx全部18 sheet
...
- [x] 2.1 创建 `composables/useN2FormulaEngine.ts`（负债类！）
...
- [x] 2.2 创建 `composables/useN2VatEngine.ts`（增值税测算引擎，纯函数）
...
- [x] 2.3 创建 `composables/useN2MultiTaxEngine.ts`（多税种测算引擎，核心纯函数）
...
- [x]* 2.5 编写 Property P2 PBT：负债类期末余额（期初+贷-借）
...
- [x]* 2.12 编写 Property P9 PBT：土增税增值率
  - 生成器：app∈R, di≠0
...
- [x] 3.3 创建 useN2DualMode.ts + useN2ImportExport.ts
...
- [x] 7.1 单元测试：useN2FormulaEngine + useN2VatEngine + useN2MultiTaxEngine
  - 负债类方向(期初+贷-借) + 增值税销项进项 + 各税种计税依据×税率 + 土增税累进 + 边界(零值/大数/税率边界/留抵)
  - _Requirements: P1-P9_

- [x] 7.2 集成测试：负债类取数+多税种测算+跨底稿联动
