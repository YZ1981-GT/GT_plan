...
- [x] 0.1 openpyxl脚本读取L4应付债券.xlsx全部15 sheet
...
- [x] 2.1 创建 `useL4FormulaEngine.ts`（负债类！+初始计量）
...
- [x] 2.2 创建 `useL4EIREngine.ts`（核心！实际利率法）
...
- [x] 2.3 创建 `useL4EquityLiabEngine.ts`
...
- [x] 4.1 创建 L4TabIndex.vue 底稿目录
  - 15行+进度条
  - _Requirements: 1.2_

- [x] 4.2 创建 L4TabAdjudication.vue 审定表L4-1
  - 负债类单区块+摊余成本+品种小计+TB回写
  - _Requirements: 2.1-2.7_

- [x] 4.3 创建 L4TabDetail.vue 明细表L4-2（89列极宽表！）
  - 区段Tab拆分(基础/发行/计息付息/摊余成本/兑付)+行同步+动态行+多区段导出
  - _Requirements: 3.1-3.5_

- [x] 4.4 创建 L4TabInitialMeasure.vue 初始计量L4-6
  - 发行价-交易费用+溢折价+IRR求解
  - _Requirements: 6.1-6.5_

- [x] 4.5 创建 L4-7后续计量2分支组件（核心！）
  - L4TabSubsequentBullet.vue(到期一次还本付息) + L4TabSubsequentInstallment.vue(分期付息)
  - 分支选择器+完整摊销表+末期验证+L2/L8联动publish
  - _Requirements: 4.1-4.8_

- [x] 4.6 创建 L4-8账面核对2分支组件
  - L4TabBookReconBullet.vue + L4TabBookReconInstallment.vue（分支与L4-7同步）
  - 账面vs测算差异高亮
  - _Requirements: 5.1-5.6_

- [x] 4.7 创建 L4TabEquityLiabCheck.vue + L4TabFinLiabOther.vue
  - 权益负债划分(分拆+权益成分校验) + 其他金融工具明细
  - _Requirements: 7.1-7.5_

- [x] 4.8 创建 L4TabAdjustment.vue + L4TabBondCheck.vue + L4TabDisclosureListed/Soe.vue
  - 调整借贷平衡 + 检查表结论区 + 附注上市/国企切换
  - _Requirements: 8.1-8.4_

### Phase 5: 后端

- [x] 5.1 创建 l4_bonds_payable_renderer.py
  - RENDERER_DISPATCH注册 + 负债类公式验证
  - _Requirements: 1.6_

- [x] 5.2 创建 l4_bonds_payable.py 路由
  - 导出模板/导出数据/导入数据 + 后续计量生成+IRR求解API
  - _Requirements: 4.1, 6.4, 12.2_

- [x] 5.3 创建 l4_bonds_payable_service.py
...
- [x] 7.1 单元测试：useL4EIREngine + useL4FormulaEngine + useL4EquityLiabEngine
  - 实际利率法2分支 + 负债类方向 + 权益划分 + 边界(eir=0)
  - _Requirements: P1-P9_

- [x] 7.2 集成测试：后续计量2分支末期趋面值 + L4→L2/L8联动 + 账面核对
