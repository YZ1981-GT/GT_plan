...
- [x] 2. 公式引擎composable + PBT测试
  - [x] 2.1 实现useG8FormulaEngine.ts（7个纯函数 + parseNum）
  ...
  - [x] 2.10 单元测试：parseNum边界 + 除零保护 + 浮点精度
...
- [x] 3. 主入口sheetName分发 + selfLoad + 数据composable
  - [x] 3.1 GtG8OtherEquityInstruments.vue 完整实现
  ...
  - [x] 3.2 useG8FormData.ts composable
  ...
  - [x] 3.3 useG8DualMode.ts composable
    - HTML↔OnlyOffice切换
    - localStorage持久化用户偏好
    - 切换时数据不丢失
    - _Requirements: 12.6_

- [x] 4. G8-1 审定表（29行，借方科目公允价值计量）
  - [x] 4.1 G8TabAdjudication.vue 组件
...
- [x] 6. G8-2 明细表（24列→2区段Tab）+ G8-3 调整分录
  - [x] 6.1 G8TabDetail.vue 组件（2区段Tab）
...
- [x] 7. G8-4 公允价值测试表（19列→2区段Tab，Level3必填）+ G8-5 指定适当性检查
  - [x] 7.1 G8TabFairValueTest.vue 组件（2区段Tab + Level3必填校验）
  ...
  - [x] 7.2 G8TabDesignationCheck.vue 组件（问卷式CAS22合规）
  ...
  - [x]* 7.3 单元测试：Level3必填校验 + 公允价值差异
    - Level3时valuationTechnique/unobservableInputDesc为空→校验失败
    - Level1/Level2时Tab2非必填→校验通过
    - calcFairValueDiff正确计算差异
    - _Requirements: 7.3, 7.4_

- [x] 8. Checkpoint - 估值与适当性逻辑验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 9. G8-6 凭证检查表（18列→3区段Tab + 虚拟滚动 + OCR）
  - [x] 9.1 G8TabVoucherCheck.vue 组件（3区段Tab + 102行虚拟滚动）
...
- [x] 10. 附注披露 + 底稿目录
  - [x] 10.1 G8TabDisclosureListed.vue（上市公司附注，18行×7列）
    - 结构化表格
...
- [x] 11. 导入导出 + AI辅助
  ...
  - [x] 11.2 后端 _g8_other_equity_instruments_import_export.py
    - 12个端点（4表×3端点）
