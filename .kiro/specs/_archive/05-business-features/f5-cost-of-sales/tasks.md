...
- [x] 2. 实现公式引擎 useF5FormulaEngine.ts
  ...
  - [x]* 2.2 编写 Property 1 PBT：损益类审定公式
  ...
  - [x]* 2.3 编写 Property 2 PBT：成本倒轧恒等
  ...
  - [x]* 2.5 编写 Property 4 PBT：完工成本恒等
  ...
  - [x]* 2.6 编写 Property 5 PBT：营业成本倒轧全链非负
  ...
  - [x]* 2.7 编写 Property 6 PBT：数量差异公式
  ...
  - [x]* 2.9 编写 Property 8 PBT：波动系数非负
  ...
  - [x]* 2.10 编写 Property 9 PBT：月度合计恒等
  ...
  - [x]* 2.11 编写 Property 10 PBT：借贷平衡恒等
...
- [x] 3. 实现 useF5FormData.ts 基础数据加载/保存
  - [x] 3.1 创建 `composables/useF5FormData.ts`
...
- [x] 4. 实现审定表 composable
  - [x] 4.1 创建 `composables/useF5Adjudication.ts`
...
- [x] 5. 实现各sheet composable
  - [x] 5.1 创建 `composables/useF5MonthlyDetail.ts`（F5-2月度明细2区段逻辑）
  ...
  - [x] 5.2 创建 `composables/useF5OtherCost.ts`（F5-3其他业务成本逻辑）
    - 定义行类型（14列）
    - 实现公式链：变动额/率/占比/成本率
    - 实现变动>30%橙色标记 + 合计行 + 动态行增删
    - _Requirements: 5.1~5.7_

  - [x] 5.3 创建 `composables/useF5Comparison.ts`（F5-5比较分析逻辑）
  ...
  - [x] 5.4 创建 `composables/useF5QuantityRecon.ts`（F5-6数量核对逻辑）
  ...
  - [x] 5.5 创建 `composables/useF5CostRollforward.ts`（F5-7成本倒轧逻辑）
...
- [x] 6. 实现 Vue子组件
  - [x] 6.1 创建 `f5-cost-of-sales/F5TabAdjudication.vue`（F5-1审定表 损益类）
    - 调用useF5Adjudication
  ...
  - [x] 6.2 创建 `f5-cost-of-sales/F5TabMonthlyDetail.vue`（F5-2月度明细 24列→2区段Tab）
  ...
  - [x] 6.3 创建 `f5-cost-of-sales/F5TabQuantityRecon.vue`（F5-6数量核对 81行）
  ...
  - [x] 6.4 创建 `f5-cost-of-sales/F5TabCostRollforward.vue`（F5-7成本倒轧表 结构化）
...
- [x] 8. 实现后端 + 导入导出 + AI
  ...
  - [x] 8.2 创建 `composables/useF5ImportExport.ts` + `composables/useF5DualMode.ts`
