...
- [x] 2. 实现共享公式引擎 useF2SpecialFormulaEngine.ts
  - [x] 2.1 创建 `composables/useF2SpecialFormulaEngine.ts`，实现全部纯函数
  ...
  - [x]* 2.2 编写 Property 1 PBT：待发生成本公式
  ...
  - [x]* 2.3 编写 Property 2 PBT：减值公式非负性
  ...
  - [x]* 2.4 编写 Property 3 PBT：亏损判定一致性
  ...
  - [x]* 2.5 编写 Property 4 PBT：预计损失公式
  ...
  - [x]* 2.6 编写 Property 5 PBT：完工进度范围
  ...
  - [x]* 2.7 编写 Property 6 PBT：产能利用率公式
  ...
  - [x]* 2.10 编写 Property 9 PBT：期末余额公式
  ...
  - [x]* 2.11 编写 Property 10 PBT：分类小计公式
  ...
  - [x]* 2.12 编写 Property 11 PBT：投入产出比公式
  ...
  - [x]* 2.13 编写 Property 12 PBT：核查完成度公式
...
- [x] 3. 实现 useF2SpecialFormData.ts 基础数据加载/保存
  - [x] 3.1 创建 `composables/useF2SpecialFormData.ts`
...
- [x] 4. 实现合同履约成本组composable
  - [x] 4.1 创建 `composables/useF2ContractCost.ts`（F2-55/F2-56）
  ...
  - [x] 4.2 创建 `composables/useF2Impairment.ts`（F2-57）
  ...
  - [x] 4.3 创建 `composables/useF2LossContract.ts`（F2-58）
...
- [x] 5. 实现IPO组composable
  - [x] 5.1 创建 `composables/useF2IpoPurchaseAnalysis.ts`（F2-61/62/63/64）
  ...
  - [x] 5.2 创建 `composables/useF2SupplierAnalysis.ts`（F2-65~F2-72）
  ...
  - [x] 5.3 创建 `composables/useF2MasterDetail.ts`（F2-70/F2-72通用）
...
- [x] 6. Checkpoint - 公式引擎与composable验证
  - Ensure all PBT tests pass, ask the user if questions arise.

- [x] 7. 实现 Vue子组件 - contract/目录
  - [x] 7.1 创建 `f2-special/contract/F2TabContractProcedure.vue`（F2-55A程序表）
    - 复用 GtAProgramConsole 组件 + selfLoad逻辑
    - _Requirements: 1.1_

  - [x] 7.2 创建 `f2-special/contract/F2TabContractCostDetail.vue`（F2-55 合同履约成本明细 37列→6区段Tab）
  ...
  - [x] 7.3 创建 `f2-special/contract/F2TabContractCostCheck.vue`（F2-56 检查表 23列固定+滚动）
    - 调用 useF2ContractCost.ts
  ...
  - [x] 7.4 创建 `f2-special/contract/F2TabImpairment.vue` + `F2TabLossContract.vue`
...
- [x] 8. 实现 Vue子组件 - ipo/目录（采购+产能+单耗）
  - [x] 8.1 创建 `f2-special/ipo/F2TabIpoProcedure.vue`（F2-61A程序表）
    - 复用 GtAProgramConsole 组件 + selfLoad逻辑
    - _Requirements: 1.1_

  - [x] 8.2 创建 `f2-special/ipo/F2TabPurchasePrice.vue`（F2-61 采购价格 118行虚拟滚动）
  ...
  - [x] 8.3 创建 `f2-special/ipo/F2TabUnitPrice.vue`（F2-62 单价趋势）
    - 调用 useF2IpoPurchaseAnalysis.ts
    - 18列表格 + 偏离度>20%红色高亮
    - 动态行增删 + 导入导出
    - 底部分析结论textarea + AI
  ...
  - [x] 8.4 创建 `f2-special/ipo/F2TabCapacityEnergy.vue`（F2-63 产能能耗）
    - 调用 useF2IpoPurchaseAnalysis.ts
    - 16列表格
  ...
  - [x] 8.5 创建 `f2-special/ipo/F2TabUnitConsumption.vue`（F2-64 单耗分析 232行虚拟滚动+产品折叠）
    - 调用 useF2IpoPurchaseAnalysis.ts
...
- [x] 9. Checkpoint - 合同组+IPO采购分析验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. 实现 Vue子组件 - ipo/目录（关联方+供应商）
  - [x] 10.1 创建 `f2-special/ipo/F2TabRelatedPartyInquiry.vue`（F2-65 询价函）
  ...
  - [x] 10.2 创建 `f2-special/ipo/F2TabRelatedPartyMarket.vue`（F2-66 市场价）
    - 调用 useF2SupplierAnalysis.ts
    - 7列表格 + 价差率>10%红色高亮
    - 结论下拉 + 动态行增删 + 底部结论textarea + AI
    - _Requirements: 11.1~11.7_

  - [x] 10.3 创建 `f2-special/ipo/F2TabUndisclosedRelated.vue`（F2-67 未披露关联方 20列）
    - 调用 useF2SupplierAnalysis.ts
  ...
  - [x] 10.4 创建 `f2-special/ipo/F2TabSupplierStructure.vue`（F2-68 供应商结构 25列固定+滚动）
    - 调用 useF2SupplierAnalysis.ts
    - 固定列(5)+滚动列(20)模式
  ...
  - [x] 10.5 创建 `f2-special/ipo/F2TabSupplierChecklist.vue`（F2-69 核查清单 19列）
    - 调用 useF2SupplierAnalysis.ts
  ...
  - [x] 10.6 创建 `f2-special/ipo/F2TabSupplierInfoCheck.vue`（F2-70 供应商核查 Master-Detail）
  ...
  - [x] 10.7 创建 `f2-special/ipo/F2TabInterviewSummary.vue`（F2-71 访谈汇总）
    - 调用 useF2SupplierAnalysis.ts
    - 9列表格 + 访谈方式/结论下拉
    - 异常/存疑橙色高亮
  ...
  - [x] 10.8 创建 `f2-special/ipo/F2TabInterviewDetail.vue`（F2-72 访谈记录 Master-Detail）
    - 调用 useF2MasterDetail.ts
    - 左侧按供应商分组列表 + 搜索+折叠
...
- [x] 11. 实现导入导出 composable + 后端端点
  - [x] 11.1 创建 `composables/useF2SpecialImportExport.ts`
  ...
  - [x] 11.2 创建 `backend/app/routers/wp_render_strategies/_f2_special_import_export.py`
...
- [x] 12. 实现后端Render策略 + AI生成
  - [x] 12.1 创建 `backend/app/routers/wp_render_strategies/_f2_special.py`
  ...
  - [x] 12.2 创建 `backend/app/routers/wp_render_strategies/_f2_special_ai.py`
...
- [x] 13. 实现双模式切换 + 性能优化 + UI打磨
  - [x] 13.1 创建 `composables/useF2SpecialDualMode.ts`
...
- [x] 15. 抽凭引擎集成
  - [x] 15.1 F2-56合同履约成本检查表集成GtVoucherSamplingEngine
...
- [x] 16. 版本链集成
  - [x] 16.1 集成useVersionTrail到GtF2InventorySpecial主入口
...
- [x] 17. 附注模块联动
  - [x] 17.1 F2-55/F2-57 publish substantive:adjudicated事件
