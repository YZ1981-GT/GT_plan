...
- [x] 0.1 openpyxl脚本读取L8财务费用.xlsx全部10 sheet
...
- [x] 2.1 创建 `useL8FormulaEngine.ts`（损益类！取发生额）
...
- [x] 2.2 创建 `useL8InterestEngine.ts`
...
- [x] 4.1 创建 L8TabIndex.vue 底稿目录
  - 10行+进度条
  - _Requirements: 1.2_

- [x] 4.2 创建 L8TabAdjudication.vue 审定表L8-1
  - 损益类发生额（本期/上期/变动）+项目小计+TB回写(发生额口径)
  - _Requirements: 2.1-2.7_

- [x] 4.3 创建 L8TabDetail.vue 明细表L8-2
  - 费用项目分析(利息支出/收入/汇兑/手续费)+净财务费用+变动率+区段Tab
  - _Requirements: 3.1-3.6_

- [x] 4.4 创建 L8TabNonFinInterest.vue 非金融机构利息测算L8-4
  - 可扣除利息+超标利息(税务提示)
  - _Requirements: 5.1-5.4_

- [x] 4.5 创建 L8TabCutoffTest.vue 截止测试L8-5
  - 序时账±天数自动提取+跨期高亮+行级抽凭(GtVoucherSamplingEngine)
  - _Requirements: 6.1-6.5_

- [x] 4.6 创建 L8TabFinExpenseCheck.vue + L8TabAdjustment.vue + L8TabDisclosureListed/Soe.vue
  - 检查表结论区+AI辅助 + 调整借贷平衡 + 附注上市/国企切换
  - _Requirements: 7.1-7.5_

### Phase 5: 后端

- [x] 5.1 创建 l8_financial_expenses_renderer.py
  - RENDERER_DISPATCH注册 + 损益类发生额验证（从tb_ledger取数）
  - _Requirements: 1.6, 2.4_

- [x] 5.2 创建 l8_financial_expenses.py 路由
  - 导出模板/导出数据/导入数据 + 利息测算汇总+截止提取API
  - _Requirements: 4.5, 6.1, 8.8_

- [x] 5.3 创建 l8_financial_expenses_service.py
...
- [x] 7.1 单元测试：useL8FormulaEngine + useL8InterestEngine + useL8CutoffEngine
  - 损益类发生额 + 利息汇总 + 可扣除/超标利息 + 截止跨期
  - _Requirements: P1-P7_

- [x] 7.2 集成测试：L1/L3/L4/L5→L8利息汇聚 + 截止测试 + 发生额回写
