...
- [x] 1. 数据模型：新增 applicable_standard_v2
  - [x] 1.1 编写迁移 `V0XX__add_applicable_standard_v2.sql`
    - ALTER TABLE projects ADD COLUMN IF NOT EXISTS applicable_standard_v2 JSONB
    - _Requirements: 1.1_
  - [x] 1.2 更新 Project ORM 模型
    - 加 `applicable_standard_v2: Mapped[dict | None] = mapped_column(JSONB, nullable=True)`
    - _Requirements: 1.1_
  - [x] 1.3 新增 STANDARD_CHANGED 事件类型
    - EventType enum 加 `STANDARD_CHANGED = "standard_changed"`
    - _Requirements: 1.5_

- [x] 2. 统一源服务：standard_unification_service
  - [x] 2.1 新建 `backend/app/services/standard_unification_service.py`
...
- [x] 3. 底稿切换能力：wp_standard_conversion_service
  - [x] 3.1 新建 `backend/app/services/wp_standard_conversion_service.py`
...
- [x] 4. 切换 API + 预览 API
  - [x] 4.1 新建 `backend/app/routers/standard_conversion.py`
...
- [x] 5. 联动 handler 接入
  - [x] 5.1 附注层：STANDARD_CHANGED → note_conversion_service
    - 注册 handler，调已有 `execute_conversion`
    - 切换后更新 current_standard 与 v2 一致
    - _Requirements: 3.1, 3.2, 3.3_
  - [x] 5.2 报表层：STANDARD_CHANGED → 标记 stale + 更新 applicable_standard
    - _Requirements: 4.1, 4.2_

- [x] 6. 迁移脚本
  - [x] 6.1 新建 `backend/scripts/migrate/migrate_applicable_standard.py`
...
- [x] 7. 测试
  - [x]* 7.1 Property 1 属性测试：统一源一致性
    - 切换后 v2 与旧字段值一致
    - _Requirements: 1.3, 1.4_
  - [x]* 7.2 Property 2 属性测试：底稿数据不丢失
    - 共有底稿 parsed_data 切换前后不变
    - _Requirements: 2.2_
  - [x]* 7.3 Property 3 属性测试：roundtrip 不变量
