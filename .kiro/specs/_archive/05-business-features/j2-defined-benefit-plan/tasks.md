...
- [x] 0.1 openpyxl脚本读取J2长期应付职工薪酬.xlsx全部9 sheet（含1个skip L2A）
...
- [x]* 2.5 编写 Property CP-J2-03 PBT：利息成本=期初DBO×折现率
...
- [x]* 2.8 编写 Property CP-J2-06 PBT：合计行恒等
  - 断言：calcSubtotal(arr) === Σarr
  - **Feature: j2-defined-benefit-plan, Property CP-J2-06: 合计行恒等**

- [x]* 2.9 编写 Property CP-J2-07 PBT：精算假设范围校验
...
- [x] 3.1 创建 useJ2FormData.ts（selfLoad + writebackTB **期末余额** 2221）
  - 负债类：回写期末余额
- [x] 3.2 创建 useJ2CrossSheet.ts（明细vs审定 + B51联动 + 精算假设变动）
- [x] 3.3 创建 useJ2Adjudication.ts（三区块DBO/计划资产/净负债 + 67公式）
- [x] 3.4 创建 useJ2Detail.ts + useJ2AccrualCheck.ts（精算师工作利用+假设评价）
- [x] 3.5 创建 useJ2Disclosure.ts + useJ2ImportExport.ts + useJ2DualMode.ts

### Phase 4: Vue子组件

- [x] 4.1 创建 J2TabIndex.vue 底稿目录
- [x] 4.2 创建 J2TabAdjudication.vue 审定表（67公式！负债类+三区块+精算假设面板）
- [x] 4.3 创建 J2TabDetail.vue 明细表（DBO+计划资产+净负债）
- [x] 4.4 创建 J2TabAdjustment.vue 调整分录
- [x] 4.5 创建 J2TabAccrualCheck.vue 计提检查（精算师利用ISA620+假设逐项对比）
- [x] 4.6 创建 J2TabDisclosureListed.vue + J2TabDisclosureSoe.vue

### Phase 5: 后端

- [x] 5.1 创建 _j2_defined_benefit_plan.py render策略 + RENDERER_DISPATCH
  - 负债类公式验证 + 精算数据格式
- [x] 5.2 创建 _j2_import_export.py 导入导出3端点
- [x] 5.3 创建 _j2_ai_generate.py AI生成（精算假设合理性分析）
- [x] 5.4 更新 wp_render_schema: j2-defined-benefit-plan.yaml

### Phase 6: 集成联动

- [x] 6.1 EventBus: TB回写(2221期末余额) + substantive:adjudicated
- [x] 6.2 EventBus: actuarial:assumption-changed → B51联动
- [x] 6.3 cross_wp_references: J2→B51
- [x] 6.4 GtIndexChip: J2→B51舞弊三因素跳转
- [x] 6.5 附注EventBus + 双模式OO

### Phase 7: 测试验收

- [x] 7.1 Vitest: useJ2FormulaEngine（负债类公式）+ useJ2ActuarialEngine（精算公式）
- [x] 7.2 Vitest组件: sheetName分发 + 三区块审定表 + 精算假设面板
- [x] 7.3 后端pytest: render(负债类)+导入导出+精算假设验证
- [x] 7.4 Playwright E2E: J2精算假设变更→B51联动→审定回写全链路
