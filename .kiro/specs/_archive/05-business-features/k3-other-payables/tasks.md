...
- [x] 0.1 openpyxl脚本读取K3其他应付款.xlsx全部11 sheet
  - 产出：k3_structure_summary.json（确认K3-1 50公式/K3-2 19公式/K3-4 8公式）
  - _Requirements: 双源输入流程_

- [x] 0.2 K其他应付款循环底稿模板库md交叉验证
...
- [x] 2.1 创建 useK3FormulaEngine.ts
...
- [x] 4.1 创建 K3TabIndex.vue 底稿目录
  - _Requirements: 1.2_

- [x] 4.2 创建 K3TabAdjudication.vue 审定表K3-1（负债类50公式+三角勾稽+TB回写+完整性说明）
  - _Requirements: 2.1-2.7, 7.3_

- [x] 4.3 创建 K3TabDetail.vue 明细表K3-2（27列3区段+账龄+疑似未入账标记+动态行+3年以上高亮+导入导出）
  - _Requirements: 3.1-3.6, 7.4_

- [x] 4.4 创建 K3TabLargeAmount.vue K3-4 + K3TabLongOutstanding.vue K3-5
  - 大额分析+长期挂账检查+转营业外提示+抽凭
  - _Requirements: 4.1-4.4, 5.1-5.4_

- [x] 4.5 创建 K3TabRelatedParty.vue K3-6 + K3TabPayableCheck.vue K3-7（含反向截止）
  - 关联方+综合检查+反向截止测试+抽凭+OCR+不合规摘要
  - _Requirements: 6.1-6.5, 7.2_

- [x] 4.6 创建 K3TabAdjustment.vue + 附注（K3TabDisclosureListed/Soe.vue）
  - 调整分录借贷平衡+EventBus / 附注双版本+自动取数
  - _Requirements: 8.1-8.2_

### Phase 5: 后端

- [x] 5.1 创建 _k3_other_payables.py render策略 + RENDERER_DISPATCH注册（负债口径）
  - _Requirements: 1.6, 7.1_

- [x] 5.2 创建 _k3_import_export.py 导入导出3端点
  - _Requirements: 3.4, 8.2_

- [x] 5.3 创建 _k3_ai_generate.py AI生成 + 更新 k3-other-payables.yaml
  - section: large-amount-eval / long-outstanding-eval / related-party-eval / overall-opinion
  - _Requirements: 6.1, 8.1_

### Phase 6: 集成联动

- [x] 6.1 EventBus: TB回写(2241) + substantive:adjudicated → 附注
  - _Requirements: 2.6, 8.1_

- [x] 6.2 EventBus: adjustment:created → A13 + 附注subscribe刷新 + 长期挂账转销联动K12
...
- [x] 7.1 单元测试：useK3FormulaEngine（负债类方向 + 边界零值/大数）
  - _Requirements: CP-K3-01~06_

- [x] 7.2 集成测试：负债类回写 + 长期挂账联动 + 账龄勾稽
  - _Requirements: 2.5, 2.6, 5.4_

- [x] 7.3 Playwright E2E：打开K3→审定→明细账龄→大额→长期挂账→反向截止→保存
  - _Requirements: 全部_
