...
- [x] 1. V067 迁移 + ORM + 契约测试（deliverable_section_state 三层一致）
  ...
  - [x] 1.2 新增 ORM 模型 DeliverableSectionState（继承 TimestampMixin）
...
- [x] 2. LinkageFacadeService 扩展 deliverable source_type（只读穿透）
  ...
  - [x] 2.1 实现 _trace_from_deliverable 映射 + 上游链延续
  ...
  - [x]* 2.2 属性测试：deliverable→附注映射正确性
  ...
  - [x]* 2.6 单元测试：deliverable 分支存在 + 缺失 section_code
    - 1.1（deliverable 分支可达）、1.4（缺失 section_code 返回无匹配并记录）
    - _Requirements: 1.1, 1.4_

- [x] 3. ⚠️ 段落锚点写入（confirm 时写隐藏书签）【阻塞于 G1+G2】
  ...
  - [x] 3.1 实现 anchor_name / section_code_from_anchor 双向映射
  ...
  - [x] 3.2 实现 write_section_anchors 并接入 confirm 清理标记之前
...
- [x] 4. DeliverableSectionStateService（新建：快照计算 / stale 标记 / 查询）
  ...
  - [x] 4.1 实现 compute_source_snapshot_hash（确定性 sha256，与 doc 级 tb_hash 分层）
  ...
  - [x] 4.2 实现 snapshot_on_confirm / mark_section_stale / clear_section_stale / get_section_states / detect_upstream_drift
  ...
  - [x] 4.3 复用 DeliverableSnapshotService 实现分层 stale 检测（先 tb_hash 整份闸门，变了才逐章算）
  ...
  - [x]* 4.4 属性测试：confirm 计算并存储章节快照哈希
...
- [x] 5. 前端 Lineage_Panel + 溯源查询端点 + 跨层跳转（P0）
  ...
  - [x] 5.1 新增溯源查询端点 + 章节状态查询端点 + router_registry 注册
  ...
  - [x] 5.2 实现 Lineage_Panel.vue（参照 CellTraceDialog/ReportTracePanel/useLinkageTraceDrawer）
...
- [x] 7. StalePropagationEngine 扩展 DELIVERABLE: URI 前缀
  ...
  - [x] 7.1 实现 DELIVERABLE: 前缀分发 + SSE 推送
  ...
  - [x]* 7.2 属性测试：DELIVERABLE: URI 标记正确行
...
- [x] 8. event_handlers 新增 deliverable stale handler + 自触发防护
  ...
  - [x] 8.1 实现 on_upstream_changed_mark_deliverable_stale handler
...
- [x] 9. ⚠️ 单/批量章节增量刷新【阻塞于 G1+G2】
  ...
  - [x] 9.1 实现 refresh_section（单章节增量刷新）+ 端点
  ...
  - [x] 9.2 实现 refresh_all_stale_sections（批量刷新）+ 端点
  ...
  - [x]* 9.3 属性测试：增量刷新只影响目标 stale 章节集合
...
- [x] 10. 前端 stale 展示 + 刷新按钮（P1）
  - _Requirements: 4.5, 5.5, 11.4_ / _Design: 前端设计「OnlyOffice 集成点」_

  - [x] 10.1 Lineage_Panel 章节 stale 提示 + "刷新本章节"/"刷新所有过期"按钮
...
- [x] 12. 回填合规护栏分类算法（审计底线，强制 PBT）
  ...
  - [x]* 12.2 属性测试：回填合规护栏分类（审计底线，需求 6.7 强制覆盖）
...
- [x] 13. ⚠️ DeliverableWritebackService（下载→分块→diff→护栏→写回）【阻塞于 G1+G2】
  ...
  - [x] 13.1 实现章节级 diff（下载 docx → 按锚点分块 → 文本比对）
  ...
  - [x] 13.2 实现 writeback 主流程 + 终态检查 + 锚点丢失隔离 + 失败保留原值
  ...
  - [x]* 13.6 单元测试：自动保存不回填 + 携带来源标记 + 下载失败保留原值
...
- [x] 16. 前端回填按钮 + 冲突裁决弹窗（P2）
  - _Requirements: 7.1, 8.2, 8.3, 11.4_ / _Design: 前端设计「OnlyOffice 集成点」_

  - [x] 16.1 实现"回填到附注模块"按钮 + WritebackResult 分组展示
...
- [x] 20. 前端页面集成：LineagePanel 接入出品物编辑页
  - LineagePanel.vue / WritebackResultPanel.vue / WritebackConflictDialog.vue 当前是独立组件，未被任何页面 import
  - 需在出品物编辑页（OnlyOffice 容器页）中引入 LineagePanel 并接线 onBookmarkDetected
  - _Requirements: 3.1, 3.4_

  - [x] 20.1 在出品物编辑页引入 LineagePanel + WritebackResultPanel
    - 找到出品物 OnlyOffice 编辑页面（DeliverablePreview/DeliverableEditor 等），在侧栏/抽屉中引入 LineagePanel
    - 传入 projectId / wordExportTaskId / deliverableStatus / year props
    - 接入 OnlyOffice JS API 书签检测 → 调用 lineagePanelRef.onBookmarkDetected(anchorName)
    - 引入 WritebackResultPanel + WritebackConflictDialog 并接线 emit 事件
    - _Requirements: 3.1, 3.4, 7.1, 8.2_

- [x] 21. E2E 集成测试：完整链路验证（合成 docx，无需真实 OnlyOffice）
  - 用合成 docx + mock DB 跑通完整链路：confirm 写锚点 → 模拟上游变更 → stale 传播 → refresh_section → writeback
  - _Requirements: 全部_

  - [x] 21.1 编写后端集成测试 test_deliverable_lineage_e2e.py
    - 使用 python-docx 合成含 SECTION 块的 docx → write_section_anchors → 模拟 snapshot_on_confirm
    - 模拟上游 NOTE_SECTION_SAVED → 验证 stale 传播
    - 调用 refresh_section（注入 docx_bytes）→ 验证 hash 更新 + stale 清除
    - 调用 writeback（注入 docx_bytes）→ 验证 diff + 护栏 + 写回 + 留痕
    - 终态检查：signed 状态拒绝
    - _Requirements: 4.1, 4.3, 5.1, 7.3, 11.1_
