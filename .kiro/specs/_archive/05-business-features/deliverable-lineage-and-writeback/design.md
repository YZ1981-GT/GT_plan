...
### D1 段落锚点 POC 验证结论（2026-06-10 实证）

三项判据全部通过，采用隐藏书签方案：

| 判据 | 结果 | 验证方式 |
|------|------|----------|
| ① 不可见 | ✓ PASS | python-docx 写入后 visible text 不变 |
| ② 往返不丢 | ✓ PASS | OnlyOffice 9.4.0 Conversion API docx→docx 往返后 3 个书签完整保留 |
| ③ 可按 name 定位 | ✓ PASS | find_bookmarks() 按 w:id 配对 + extract_text_between_bookmarks() 切出正确区间文字 |

**最终决策**：采用 `w:bookmarkStart/End` 隐藏书签作为 Section_Anchor。无需评估 SDT 内容控件备选。

**POC 产物**（验证后归档/删除）：
- `backend/scripts/e2e/_poc_section_anchor.py`（步骤 1+3）
- `backend/scripts/e2e/_poc_section_anchor_oo_roundtrip.py`（步骤 2）
- `backend/scripts/e2e/_poc_output/`（合成 docx + 往返后 docx）
