...
- [x] 2.1 创建 `composables/useI1FormulaEngine.ts`，实现全部纯函数
...
- [x] 2.2 创建 `composables/useI1AmortizationEngine.ts`，实现摊销纯函数
...
- [x]* 2.8 编写 Property P6 PBT：直线法摊销
...
- [x]* 2.9 编写 Property P7 PBT：剩余年限法摊销（含减值）
...
- [x]* 2.10 编写 Property P8 PBT：DCF现值计算
...
- [x] 3.1 创建 useI1FormData.ts
  - allResponses Map加载 + selfLoad + writebackTB(1701+1702+1703)
  - _Requirements: 1.9, 2.10_

- [x] 3.2 创建 useI1CrossSheet.ts
  - detailTotals / adjudicationFromDetail / amortizationForAlloc / disclosureAutoFill
  - _Requirements: 2.4-2.9, 10.2, 11.7_

- [x] 3.3 创建 useI1Adjudication.ts
  - 三区块固定行 + 三角勾稽 + TB取数行
  - _Requirements: 2.1-2.11_

- [x] 3.4 创建 useI1Detail.ts
  - 4区段Tab + 行同步 + 合计行 + 交叉验证
  - _Requirements: 3.1-3.7_

- [x] 3.5 创建 useI1Amortization.ts
  - 分支选择器状态 + 摊销矩阵计算
  - _Requirements: 11.1-11.7_

- [x] 3.6 创建 useI1Impairment.ts
  - 减值测试+DCF+可收回金额联动
  - _Requirements: 12.1-12.4, 13.1-13.5_

- [x] 3.7 创建 useI1Disclosure.ts + useI1ImportExport.ts + useI1DualMode.ts
  - _Requirements: 14.1-14.4_

### Phase 4: Vue子组件

- [x] 4.1 创建 I1TabIndex.vue（底稿目录+进度）
- [x] 4.2 创建 I1TabAdjudication.vue（三区块审定表+51公式）
- [x] 4.3 创建 I1TabDetail.vue（56列4区段Tab）
- [x] 4.4 创建 I1TabAdjustment.vue（调整分录）
- [x] 4.5 创建 I1TabPolicyCheck.vue（摊销减值政策段落型）
- [x] 4.6 创建 I1TabAdditionCheck.vue（增加检查+OCR+抽凭）
- [x] 4.7 创建 I1TabDisposalCheck.vue（减少明细）
- [x] 4.8 创建 I1TabUsefulLifeCheck.vue（使用寿命检查）
- [x] 4.9 创建 I1TabTitleCheck.vue（权属检查94行+虚拟滚动）
- [x] 4.10 创建 I1TabAmortizationAlloc.vue（摊销分配7公式）
- [x] 4.11 创建 I1TabAmortizationNoImpair.vue（I1-10, 30公式）
- [x] 4.12 创建 I1TabAmortizationWithImpair.vue（I1-11, 63公式）
- [x] 4.13 创建 I1TabImpairmentTest.vue（减值测试14公式）
- [x] 4.14 创建 I1TabRecoverableTest.vue（DCF可收回金额10公式）
- [x] 4.15 创建 I1TabDisclosureListed.vue + I1TabDisclosureSoe.vue（附注双版本）

### Phase 5: 后端

- [x] 5.1 创建 _i1_intangible_assets.py render策略 + RENDERER_DISPATCH注册
- [x] 5.2 创建 _i1_import_export.py 导入导出3端点
- [x] 5.3 创建 _i1_ai_generate.py AI生成端点
- [x] 5.4 创建 _i1_amortization_engine.py 摊销引擎验证端点
- [x] 5.5 更新 wp_render_schema: i1-intangible-assets.yaml

### Phase 6: 集成联动

- [x] 6.1 EventBus集成：TB回写(1701+1702+1703) + substantive:adjudicated
- [x] 6.2 EventBus集成：adjustment:created → A13同步
- [x] 6.3 cross_wp_references: I2资本化转入→I1-5增加
- [x] 6.4 cross_wp_references: I1-9摊销分配→K8/K9/I6
- [x] 6.5 GtIndexChip跳转：I1→I2/K8/K9/I6/A13
- [x] 6.6 附注EventBus：disclosure:note-text-updated
- [x] 6.7 双模式OO集成 + 健康检查

### Phase 7: 测试验收

- [x] 7.1 Vitest单元测试：useI1FormulaEngine全函数覆盖
- [x] 7.2 Vitest单元测试：useI1AmortizationEngine全函数覆盖
- [x] 7.3 Vitest组件测试：GtI1IntangibleAssets sheetName分发
- [x] 7.4 后端pytest：render策略+导入导出+摊销验证
- [x] 7.5 Playwright E2E：审定表编辑→三角勾稽→TB回写全链路
- [x] 7.6 Playwright E2E：摊销分支切换→计算→分配联动
