...
- [x] 1. Bug 1 — httpx 系统代理绕过
  - [x] 1.1 修复 `llm_client.py` 中所有 httpx client 创建
    - 加 `mounts={}, trust_env=False`
    - _Bugfix: Bug 1_
  - [x] 1.2 修复 `ai_service.py` 中 3 处 client 创建
    - `_get_ollama_client` / `_get_llm_client` / `_get_chromadb_client`
    - _Bugfix: Bug 1_
  - [x] 1.3 修复 `availability_fallback_service.py` 中 `check_llm_available`
    - _Bugfix: Bug 1_
  - [x] 1.4 修复 `routers/system_settings.py` 中 `check_url`
    - _Bugfix: Bug 1_
  - [x] 1.5 触类旁通 grep 全仓确认无遗漏
...
- [x] 2. Bug 2 — chat_template_kwargs 提升到顶层
  - [x] 2.1 修改 `llm_client.py` payload 构建
...
- [x] 3. Bug 3 — finish_reason=length 处理
  - [x] 3.1 修改 `llm_client.py` 的 `_sync_completion` 返回值处理
...
- [x] 5. 集成验证（需 vLLM 服务可用）
  - [x] 5.1 启动 vLLM（端口 8100）
  - [x] 5.2 设置 `WP_AI_SERVICE_ENABLED=True`
  - [x] 5.3 调用 `/ai/suggest` 端点，断言返回非 stub 内容
  - [x] 5.4 调用 `/ai/analytical-review` 端点，断言返回非 stub 内容
  - [x] 5.5 验证系统代理开启时仍能正常调用（Bug 1 验证）
