...
- [x] 1. Sprint 1：JSON 数据文件创建与扩展
  - [x] 1.1 扩展 prefill_formula_mapping.json——Analysis_Sheet 公式（8 条）
  ...
  - [x] 1.2 扩展 prefill_formula_mapping.json——Detail_Sheet 公式（5 条）
  ...
  - [x] 1.3 扩展 prefill_formula_mapping.json——D5 子科目 + D0 函证覆盖率（2 条）
  ...
  - [x] 1.4 扩展 cross_wp_references.json——D 循环内部引用（4 条 CW-21~CW-24）
  ...
  - [x] 1.5 扩展 cross_wp_references.json——D 循环→附注引用（7 条 CW-25~CW-31）
  ...
  - [x] 1.6 扩展 cross_wp_references.json——D 循环→报表引用（7 条 CW-32~CW-38）
  ...
  - [x] 1.7 新建 d_cycle_validation_rules.json——balance_check 规则（8 条）
  ...
  - [x] 1.8 扩展 d_cycle_validation_rules.json——tb_consistency 规则（8 条）
  ...
  - [x] 1.9 扩展 d_cycle_validation_rules.json——note_consistency 规则（3 条）
  ...
  - [x] 1.10 扩展 d_cycle_validation_rules.json——detail_total_check 规则（2 条）
    - DV-020: D2 客户明细表合计 = 审定数（detail_sheet="客户明细表D2-6"）
    - DV-021: D2 账龄分析表合计 = 审定数（detail_sheet="账龄分析表D2-7"）
    - tolerance=0.01，severity=blocking
    - _Requirements: 9.1, 9.2, 9.3, 9.4_

  - [x] 1.11 新建 d_cycle_procedures.json——D2 应收账款程序（7 步）
  ...
  - [x] 1.12 扩展 d_cycle_procedures.json——D4 营业收入程序（6 步）
    - D4 步骤：获取收入明细→分析程序→截止测试→收入确认→关联方交易→结论
    - _Requirements: 10.2, 10.5, 10.6_

  - [x] 1.13 扩展 d_cycle_procedures.json——D0 函证程序（7 步）
    - D0 步骤：确定函证范围→编制函证→发函→回函统计→替代程序→差异调查→结论
    - _Requirements: 10.3, 10.5, 10.6_

  - [x] 1.14 扩展 d_cycle_procedures.json——D1/D3/D5/D6/D7 程序（各 5 步）
...
- [x] 2. Checkpoint - Sprint 1 数据完整性验证
...
- [x] 3. Sprint 2：属性测试 + 集成测试 + 验证
  ...
  - [x]* 3.2 Property 1: Analysis_Sheet formula coverage
  ...
  - [x]* 3.3 Property 2: Multi-account TB_SUM consistency
    - **Property 2: Multi-account TB_SUM consistency**
    - **Validates: Requirements 1.4**
    - 验证 account_codes 含多个编码的条目，Analysis_Sheet 使用 TB_SUM 而非单独 TB

  - [x]* 3.4 Property 3: D-cycle internal reference categorization
  ...
  - [x]* 3.5 Property 4: Cross-module reference structural completeness
  ...
  - [x]* 3.6 Property 5: Balance check validation correctness
  ...
  - [x]* 3.7 Property 6: TB consistency validation correctness
  ...
  - [x]* 3.8 Property 7: Cross-file account code consistency
  ...
  - [x]* 3.9 Property 8: Procedure step schema and completeness
  ...
  - [x]* 3.10 Property 9: Sub-account dynamic formula generation
    - **Property 9: Sub-account dynamic formula generation**
    - **Validates: Requirements 11.1, 11.2**
    - 验证 D5 条目含子科目级 TB 公式（112401/112402）或单一 TB('1124',...) 公式

  - [x]* 3.11 Property 10: Validation rule tolerance symmetry
