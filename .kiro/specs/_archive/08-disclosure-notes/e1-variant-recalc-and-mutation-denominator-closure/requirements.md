...
7.5 **7.1 清单之外的存量脚本一律不迁**（含立项时在办、其后归档的 `k_cycle` / `i_cycle` / `l_cycle` / `ie_lifecycle` / `report_line_resolution` / `guard_attribution` / 7 个 `mutate_task*` 等）。它们已入库且可运行，迁移需逐条验证判定等价（Property 26），成本不足以由本 spec 支付。采纳守卫对它们只输出 INFO，**不得因其所属 spec 归档而自动转为必迁项** —— 「归档即必迁」会让迁移工作量随归档数无限增长，是本 spec 不接受的形态（见 R8.1 的修订说明）
...
8.1 采纳守卫的作业面是**存量冻结名单**，不是全局阻断：维护一个「本 spec 之前已存在、不要求采纳共享件」的冻结集合，**名单外**的 `mutate*.py`（= 本 spec 之后新建的）未使用共享件即**失败**；名单内未采纳的只输出 INFO。该集合**只许缩小**（某存量脚本被迁移后从名单移除），不许新增条目 —— 新脚本自动落入「必须采纳」，存量不受打扰

> **🔴 8.1 的修订说明（2026-08-16，落地前重审）**：本条原文是「任一脚本若未使用共享件即失败」（全局阻断 + 豁免登记表）。该形态被判定为**过度强制**并在实现前撤回，理由是它没有自然终止条件：已归档 spec 的脚本无法合法豁免（R8.5 的失效检测会打红），于是**每归档一个 spec 就强制迁移它的变异脚本**（实测待迁面含 `mutate_k_cycle_guards.py` 1558 行 / `mutate_trim_decision_guards.py` 736 行，且各需逐条等价性验证），迁移工作量随归档数线性增长，而产出仅是「让 CI 变绿」。豁免登记表与失效检测本身也只是为管理这个强制而生的二级机构。**收敛的可行性依据**：本条与 Task 14 在重审时尚未实现、CI 亦零接线，故收敛是「不建造」而非「返工」。R8.1 要防的真实退化（下一个 spec 又抄一份不带分母的样板）由「冻结名单 + 名单外必须采纳」完整覆盖 —— 新脚本一律受约束，这正是 User Story 的原意

8.2 该守卫必须对「新增一个名单外、不使用共享件的变异脚本」打红；同时必须对「把已采纳的脚本从名单外挪进名单以逃避约束」这类退化可判别（名单是冻结集合，新增条目即违规）
8.3 共享件与守卫必须挂进 `governance-checks.yml`，且引用的每个文件在干净 checkout 下可解析（沿用本轮复盘用的「37/37 exists+tracked」判据形态）。**只挂三项**：A 组前端守卫（vitest）· 共享件能力守卫（pytest）· `--check-anchors` 静态自检。**不挂**全局采纳强制
8.4 挂 CI 时对 `governance-checks.yml` 的改动必须用**归因型**验收（变动落在本 spec 的字节区间内），不得用「其他 job 一个都没变」的全局等值型 —— 并发会话同时改该文件是常态
8.5 豁免登记表**收敛为仅服务入库守卫**（R5.6），不再承担「强制迁移共享件」的管理职能。其失效检测（登记的 spec 一旦归档即提示撤销）保留，但语义收窄为「该脚本归档后应由接手方**入库**」—— 这是一次性动作，不是永续迁移压力。已入库脚本的 tracked 豁免应当撤销
