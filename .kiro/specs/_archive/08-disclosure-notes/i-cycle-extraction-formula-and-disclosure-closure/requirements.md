...
| **行骨架** | 附注模板 `tables[].rows` 的 label 序列。它与「列结构」正交：列错了表头串位，行错了披露项张冠李戴或整项消失。2026-08-15 收口实测：I1 两版主表的行骨架都与源模板不符（见 R5.8）。 |
...
I 类报表行编码目前有两套并存声明，且被 6 个 render 消费的那一套 **12 个取值里 11 个指向错误报表行**
（🔴 立项初稿写「6/6 全部错」只抓到 soe 一半；`I_CYCLE_ROW_CODES` 是 `dict[wp][entity]` 双键结构，
listed 与 soe 各是一套**不同的**错值，仅 `I6.listed` 正确，逐条见 AC 1.1）。
...
8. 🔴 **行骨架也要三向**（2026-08-15 收口新增；旧三向只比列，行维度完全裸奔）：
   附注模板 `tables[].rows` 的 label 序列必须逐行等于源模板披露 sheet 首列。实测两处真错：
   - listed 「五、26 / 无形资产情况」第 33/34 行是 `（2）其他减少` + `……`，
     源模板 `A43:A44` 是 `（2）失效且终止确认的部分` + `（3）其他减少`
     ⇒ **减值准备减少段丢了一个真实披露项、并凭空多出第 4 个可扩位**
     （账面原值与累计摊销两层都有「失效且终止确认的部分」，只有减值准备层没有 = 复制漏改）
   - soe 「八、27 / 无形资产情况」是 **48 行 / 10 个类别**：缺「其他」、把源模板的「矿产权」
     拆成「采矿权」+「探矿权」、「特许经营权」写成「特许权」、首类别是「软件」而源模板是
     「土地使用权」⇒ 新建项目的附注开局就是错类别（运行态推送产出的 52 行是对的，
     错的只有 seed 路径）
   前端行模型（`I1_LISTED_MOVEMENT_ROWS`）与模板 rows 是同一处错（同批生成），
   故「模板 ↔ 载荷」自洽型判据必然放行 —— 判据必须拿源 xlsx 当第三边
9. 行骨架的类别行不得写死：soe 主表 52 行 = 4 层 ×（层标题 + 11 类别 + 1 可扩位），
   类别由 `i1_asset_categories` 派生（幂等脚本内零类别字面量）
...
3. I1 上市披露主表的类别列真源 = 源模板 `底稿目录!A9:A19`（🔴 **11 类**，不是初稿写的 12 类 ——
   `A9:A19` 只有 11 个单元格，`A20` 才是 `……` 可扩位；`I1_ASSET_CATEGORIES` 与
   `category_defs_payload()` 实测均为 11 条）+ `A20` 的 `……` 可扩位，不得写死类别清单。
   逐字为：土地使用权 / 住房使用权 / 专利权 / 非专利技术 / 商标权 / 著作权 / 特许经营权 /
   软件 / 矿产权 / 数据资源 / 其他（🔴 **不是**「房屋使用权 / 特许权 / 探矿权、采矿权」——
   2026-08-15 实测前端 `I1_LISTED_DEFAULT_CATEGORIES` 正是这三个错 label，见 AC 6.5）
4. 类别列 key 必须是稳定 key（`{slot}_{seq}`），**不得用中文 label 作 key**
5. 🔴 类别清单在前端只允许存在**一份**声明。改造前 `i1CategoryScope.I1_DEFAULT_CATEGORIES`（label 正确、
   带 `source_ref`、有守卫）与 `i1ListedDisclosureModel.I1_LISTED_DEFAULT_CATEGORIES`（label 3 处错、
   零守卫）两份并存，而**生产消费的是后者**（`useI1Disclosure` × 5 处 + `I1TabDisclosureListed` × 2 处），
   前者的 `resolveI1Categories()` 生产零消费方 = 死代码。后者的 label 直接进
   `buildI1ListedColumns()` 的列头 ⇒ 推给附注的列头 label 与 `note_template_listed`「五、26」不同构。
   收敛后 `I_LISTED_DEFAULT_CATEGORIES` 必须由前者派生（key 经 `I1_STANDARD_TO_LEGACY` 保留历史短 key，
   已持久化在 `checklist_responses` 不得改），且守卫断言该文件内无类别 label 字面量

### Requirement 7: 动态插行区识别与推送

**User Story:** 作为审计助理，我希望源模板留的可扩行位在底稿上真的能增行，且增的行能推到附注，这样客户实际科目类别多于模板预置时我不必手工改附注。

源模板动态标记实证（openpyxl 逐格冻结，真源 = `test_note_i_cycle_structure._SRC_DYNAMIC_MARK_COUNT`）：
I1 上市 **3**（A18/A29/A40）· I1 国企 **4**（A20/A33/A46/A59）· I2 上市 **2**（A15/A27）·
I2 国企 **1**（A13）· I3 **0/0** · I4 **0/0** · **I5 上市 1（A18）/ I5 国企 1（A17）** · I6 **0/0**。
🔴 初稿写「I2 上市 1 处 / I3~I6 各 0 处」漏了 I2 的第 2 处与 **I5 两版各 1 处**真实可扩位。
...
7. 🔴 **消费侧扩位数必须等于源侧标记数**（附注模板 `rows[row_type='expandable']` 与前端行模型
   `kind==='ellipsis'` 两处都算消费侧）。改造前 I1 上市源侧 3 处、模板与前端各 **4** 处 ——
   多出的第 4 处在「三、减值准备 / 3.本期减少金额」段末，源模板该段无标记。
   旧判据只锁源侧（`_SRC_DYNAMIC_MARK_COUNT`），结构上看不见消费侧多出来的扩位。
   本 AC 不对全部 12 个 (循环,变体) 生效 —— 「源侧标记 ↔ 模板 expandable」实测**不是** 1:1
   关系（I2/I5 的扩位落在动态行表上、I3/I6 的附注侧扩位源 xlsx 未标），
   免断言项必须逐条登记实证理由（禁空话理由，守卫有 ≥20 字闸）
...
2. I2 上市披露表 `A35:D39` 的 `=#REF!` 坏公式（🔴 **20 格** = A~D 列 × 35~39 行，
   初稿写「15 格」是错基线，守卫按 openpyxl 实测冻结 20）必须登记，
   按意图实现（资本化情况续表；上下文锚点 A33='续：' / A34='项目' / A40='合计'）
3. I1 上市披露表 `D20:L20` 的 `IF(...="购置",...)` 应为「处置」（复制粘贴笔误）必须登记。
   🔴 范围要精确到列：row20（A20='（1）处置'）里 **B20/C20 正确用「处置」、D20:L20 共 9 格误写「购置」**；
   而 row14（A14='（1）购置'）的 B14:L14 全用「购置」是**正确的**，守卫须钉死这 11 格不得被一起「修正」
