...
2. THE `users` table SHALL contain columns: `id` (UUID primary key), `username` (unique, not null), `email` (unique, not null), `hashed_password` (not null), `role` (enum: admin/partner/manager/auditor/qc_reviewer/readonly), `office_code` (varchar), `is_active` (boolean, default true), `is_deleted` (boolean, default false), `deleted_at` (timestamp, nullable), `created_at` (timestamp), `updated_at` (timestamp), `created_by` (UUID, nullable), `updated_by` (UUID, nullable)
