...
- [ ] 1. 注册四件套 + render schema/yaml
  - [ ] 1.1 后端注册：VALID_COMPONENT_TYPES添加'g4-bond-investment-main' + RENDERER_DISPATCH注册render_g4_bond_investment_main策略函数
  ...
  - [ ] 1.2 前端注册：htmlRendererRegistry添加'g4-bond-investment-main'映射 + 创建主入口骨架GtG4BondInvestmentMain.vue
  ...
  - [ ] 1.3 wp_code_overrides.json添加8条映射（G4A/G4-1/G4-2/G4-3/G4-4/附注披露(上市)/附注披露(国企)/底稿目录→g4-bond-investment-main）
    - 确保8个wp_code条目全部映射到 `g4-bond-investment-main`
    - _Requirements: 1.4_

  - [ ] 1.4 创建render schema YAML配置文件
    - 在 `backend/data/` 对应目录创建G4 main组的render schema yaml
    - 配置8个sheet的列定义、行定义、公式标记
    - _Requirements: 1.1, 1.4_

- [ ] 2. 公式引擎composable + PBT测试
  - [ ] 2.1 实现useG4MainFormulaEngine.ts（14个纯函数 + parseNum）
  ...
  - [ ]* 2.4 PBT: Property 3 — 余额小计公式（三要素加法）
...
- [ ] 3. 主入口 GtG4BondInvestmentMain.vue（sheetName v-if dispatch + selfLoad + defineAsyncComponent）
  - [ ] 3.1 实现sheetName正则提取 + v-if分发逻辑（8 sheets→对应子组件 + 未匹配→OnlyOffice fallback）
...
- [ ] 4. Checkpoint - 确认注册+公式+主入口基础联通
  - 确保所有测试通过，ask the user if questions arise.

- [ ] 5. 核心子组件 Wave1: G4-1审定表 + G4-3调整分录
  - [ ] 5.1 实现useG4MainAdjudication.ts composable（三层数据结构 + TB取数 + 公式计算）
  ...
  - [ ] 5.2 实现G4TabAdjudication.vue（三层结构表格 + 展开/折叠 + EventBus发布）
    - 46行×11列三层结构表格
  ...
  - [ ] 5.3 实现useG4MainAdjustment.ts composable（借贷校验 + AJE/RJE汇总回写G4-1）
    - 22行×10列数据管理
  ...
  - [ ] 5.4 实现G4TabAdjustment.vue（调整分录表格 + 借贷不平衡红色高亮 + 回写联动）
  ...
  - [ ]* 5.5 单元测试：G4-1审定表公式链 + G4-3借贷平衡校验
    - 测试三层结构小计汇总正确性
    - 测试借方余额公式链完整性
    - 测试G4-3借贷平衡→回写G4-1流程
    - _Requirements: 3.3~3.6, 6.2, 6.5_

- [ ] 6. 核心子组件 Wave2: G4-2明细表 + G4-4利息测算
  - [ ] 6.1 实现useG4MainDetail.ts composable（5区段列定义 + 行同步 + 公式链 + 分类逻辑）
  ...
  - [ ] 6.2 实现G4TabDetail.vue（5区段Tab切换 + el-table动态列 + 行选中同步 + 虚拟滚动）
  ...
  - [ ] 6.3 实现useG4MainInterestCalc.ts composable（分组管理 + 实际利率法公式 + Stage条件）
  ...
  - [ ] 6.4 实现G4TabInterestCalc.vue（双section分组表格 + 动态行 + 审计结论 + 编制提示）
    - 每个投资项目独立成组，组间分隔线+标题
  ...
  - [ ]* 6.5 单元测试：G4-2五区段Tab行同步 + G4-4 Stage条件计算
    - 测试Tab切换行索引保持
...
- [ ] 7. Checkpoint - 确认4个核心子组件功能完整
  - 确保所有测试通过，ask the user if questions arise.

- [ ] 8. 附注披露 + 底稿目录
  - [ ] 8.1 实现G4TabDisclosureListed.vue（上市公司附注130行 + 虚拟滚动 + EventBus订阅/发布）
    - 130行×11列结构化表格
  ...
  - [ ] 8.2 实现G4TabDisclosureSOE.vue（国企附注71行 + 虚拟滚动 + EventBus订阅/发布）
    - 71行×7列结构化表格
    - 启用虚拟滚动（>50行阈值）
    - 同样监听/发布EventBus事件
    - AI辅助按钮 + 复核按钮
  ...
  - [ ] 8.3 实现G4TabDirectory.vue（底稿目录页）
    - 27行×8列目录页渲染
    - 静态展示为主，支持索引跳转
    - _Requirements: 1.8, 1.9_

  - [ ] 8.4 实现G4TabProcedure.vue（G4A程序表：复用a-program-console + 抽凭引擎 + 截止自动提取）
...
- [ ] 9. 六大集成联动（版本链+抽凭+截止+附注EventBus+复核）
  - [ ] 9.1 完善EventBus联动：G4-1发布substantive:adjudicated → 附注(上市/国企)订阅刷新 → 发布disclosure:note-text-updated
    - 确保payload格式正确：`{accountCode:'1501', adjudicatedAmount}`
    - 附注订阅后正确刷新审定数据
    - 附注文本更新后发布 `disclosure:note-text-updated`
    - _Requirements: 3.9, 4.3, 4.4, 9.4_

  - [ ] 9.2 完善G4-3调整分录→G4-1审定表回写联动
    - 保存调整分录时汇总AJE/RJE金额
    - 自动更新G4-1对应行的AJE/RJE列
    - 触发审定数重算
    - _Requirements: 6.5_

  - [ ] 9.3 完善G4-4利息测算→G4-1比对联动
    - 利息测算完成后将合计与G4-1利息收入审定数比对
    - 差异>0.01元红色高亮
    - _Requirements: 7.11_

  - [ ]* 9.4 集成测试：EventBus跨组件传递验证
    - 测试G4-1审定数变更→附注自动刷新
    - 测试G4-3回写→G4-1重算
    - 测试G4-4比对差异高亮
...
- [ ] 10. 导入导出 + AI辅助
  - [ ] 10.1 实现useG4MainImportExport.ts composable（3张表×3端点=9端点调用）
    - 导出模板/导出数据/导入数据 三端点
  ...
  - [ ] 10.2 后端实现_g4_bond_investment_main_import_export.py（导出模板/导出数据/导入数据 3端点）
  ...
  - [ ] 10.3 后端实现_g4_bond_investment_main_ai.py（4 section AI生成端点）
  ...
  - [ ] 10.4 前端集成导入导出UI（el-dropdown"导入导出▾"）+ AI按钮联动
...
- [ ] 11. Checkpoint - 确认导入导出和AI功能完整
  - 确保所有测试通过，ask the user if questions arise.

- [ ] 12. UI精调 + 双模式OO切换
  - [ ] 12.1 公式列UI规范：虚线下划线 + cursor:help + tooltip显示公式来源
    - 所有公式列统一样式：border-bottom dashed + cursor:help
    - tooltip内容示例："摊余成本 = 小计 - 减值"、"期末未审 = 期初审定 + 借方 - 贷方"
    - _Requirements: 11.2_

  - [ ] 12.2 变动率高亮：|变动率|>20%橙色高亮 + 原因分析必填提示
    - G4-1变动率列条件格式化
    - 超阈值时原因分析列强制聚焦
    - _Requirements: 3.11, 11.2_

  - [ ] 12.3 编制提示details折叠 + 审计说明/结论el-card包裹 + 字体/对齐规范
...
- [ ] 13. 集成测试 + Playwright E2E + 文档收尾
  ...
  - [ ]* 13.3 后端集成测试：render策略 + 注册契约 + 导入导出round-trip
