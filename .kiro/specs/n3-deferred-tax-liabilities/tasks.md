...
- [x] 0.1 openpyxl脚本读取N3递延所得税负债.xlsx全部6 sheet
...
- [x] 2.1 创建 `composables/useN3FormulaEngine.ts`（负债类！）
...
- [x] 2.2 创建 `composables/useN3DeferredTaxEngine.ts`（核心税务引擎，与N1同源）
...
- [x]* 2.4 编写 Property P2 PBT：负债类期末余额（期初+贷-借）
...
- [x] 7.1 单元测试：useN3FormulaEngine + useN3DeferredTaxEngine
  - 负债类方向(期初+贷-借) + 应纳税差异×税率 + 边界(零值/大数/税率边界)
  - _Requirements: P1-P5_

- [x] 7.2 集成测试：负债类取数+跨底稿联动
