...
- [x] 15. PDF导出引擎
  ...
  - [x] 15.2 实现 `PDFExportEngine.render_document`：将单个文档数据渲染为 HTML → WeasyPrint 转 PDF，支持审计报告/报表/附注三种文档类型
    - _需求: 7.1_
  - [ ] 15.3 实现底稿 .xlsx → PDF 转换：openpyxl 读取 → 生成 HTML 表格（保留列宽/合并单元格/打印区域）→ WeasyPrint 渲染
    - _需求: 7.9_
    - _TODO: MVP 阶段跳过_
  - [ ] 15.4 实现 PDF 合并与后处理：PyPDF2 合并多个 PDF → 添加页眉（项目名/文档标题）→ 添加页脚（第X页 共Y页）→ 生成目录页
    - _需求: 7.5, 7.10_
    - _TODO: MVP 阶段跳过_
  - [ ] 15.5 实现 PDF 密码保护：PyPDF2 encrypt 设置打开密码
    - _需求: 7.6_
    - _TODO: MVP 阶段跳过_
