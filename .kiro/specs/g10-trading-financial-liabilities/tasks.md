...
- [ ] 1. 注册四件套 + 主入口sheetName分发
  - [ ] 1.1 创建主入口 GtG10TradingFinancialLiabilities.vue，实现sheetName prop接收、正则提取编码、v-if分发12子组件(defineAsyncComponent lazy)、selfLoad逻辑、OnlyOffice fallback、provide openReviewDialog、useVersionTrail autoSnapshot
  ...
  - [ ] 1.2 注册htmlRendererRegistry条目 + wp_code_overrides.json(12条override) + VALID_COMPONENT_TYPES + RENDERER_DISPATCH
...
- [ ] 2. 公式引擎 useG10FormulaEngine
  - [ ] 2.1 创建 useG10FormulaEngine.ts，实现6个纯函数 + parseNum
  ...
  - [ ]* 2.4 PBT: Property 3 — L3调节表恒等
    - **Property 3: calcL3Reconciliation 8参数恒等**
    - **Validates: Requirements 7.3, 10.1**

  - [ ]* 2.5 PBT: Property 4 — 变动率方向性与除零保护
    - **Property 4: calcChangeRate方向正确 + 除零返回null**
    - **Validates: Requirements 3.8, 10.1**

  - [ ]* 2.6 PBT: Property 5 — 借贷平衡恒等
    - **Property 5: isDebitCreditBalanced(debits, credits) ↔ |SUM差|<0.01**
    - **Validates: Requirements 6.1, 10.1**

  - [ ]* 2.7 PBT: Property 6 — parseNum健壮性
    - **Property 6: 非法输入→0, 有效数值→原值**
    - **Validates: Requirements 10.1**

- [ ] 3. G10-1 审定表（63行分组+虚拟滚动+贷方公式+TB取数）
  - [ ] 3.1 创建 G10TabAdjudication.vue，实现63行×12列审定表
  ...
  - [ ]* 3.2 单元测试：G10-1审定表贷方方向 + 分组折叠 + 变动率高亮
    - 验证calcCreditBalance方向与借方相反
    - 验证|变动率|>20%触发橙色高亮+必填
    - 验证分组折叠/展开状态
    - _Requirements: 3.3, 3.7, 3.8_

- [ ] 4. G10-2 明细表（24列→2区段Tab）
  - [ ] 4.1 创建 G10TabDetail.vue，实现39行×24列2区段Tab
  ...
  - [ ]* 4.2 单元测试：G10-2区段Tab行同步 + 动态行 + 合计
    - _Requirements: 5.1, 5.2_

- [ ] 5. Checkpoint — 核心表完成
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. G10-3 调整分录 + G10-4 分类检查
  - [ ] 6.1 创建 G10TabAdjustment.vue，实现23行×10列 AJE/RJE
    - 列：序号|类型(AJE/RJE)|日期|摘要|科目代码|科目名称|借方|贷方|编制人|备注
    - 借贷平衡校验(isDebitCreditBalanced) + 差额≠0红色
    - 动态行增删 + 回写审定表
    - _Requirements: 6.1, 6.4_

  - [ ] 6.2 创建 G10TabClassificationCheck.vue，实现28行×9列问卷式
  ...
  - [ ]* 6.3 单元测试：G10-3借贷平衡 + G10-4问卷下拉联动
    - _Requirements: 6.1, 6.2_

- [ ] 7. G10-5 公允价值测试 + G10-6 L3调节表
  - [ ] 7.1 创建 G10TabFairValueTest.vue，实现39行×18列2区段Tab
    - 方法论上下文(Level1/2/3定义)
  ...
  - [ ] 7.2 创建 G10TabL3Reconciliation.vue，实现23行×13列L3调节表
  ...
  - [ ]* 7.3 单元测试：Level3必填校验 + L3调节差异计算
    - _Requirements: 7.1, 7.3_

- [ ] 8. G10-7 凭证检查（99行3区段Tab+OCR+虚拟滚动）
  - [ ] 8.1 创建 G10TabVoucherCheck.vue，实现99行×17列3区段Tab
  ...
  - [ ]* 8.2 单元测试：异常自动检测 + Tab行同步 + 借贷平衡
    - 验证check1-4任一false→isAbnormal=true
    - _Requirements: 8.1, 8.2_

- [ ] 9. G10-8 衍生金融工具核查（78行5section问卷+虚拟滚动）
  - [ ] 9.1 创建 G10TabDerivativeCheck.vue，实现78行×10列5section问卷
  ...
  - [ ]* 9.2 单元测试：G10-8虚拟滚动 + section折叠 + 问卷下拉
    - _Requirements: 9.1, 9.3_

- [ ] 10. Checkpoint — 全部sheet组件完成
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 11. 附注披露 + G10A程序表 + 底稿目录 + EventBus联动
  - [ ] 11.1 创建 G10TabDisclosureListed.vue(上市42行) + G10TabDisclosureSOE.vue(国企79行虚拟滚动)
  ...
  - [ ] 11.2 创建 G10TabProcedure.vue(G10A程序表) + G10TabDirectory.vue(底稿目录)
...
- [ ] 12. 六大集成 + 导入导出5张表 + 双模式
  - [ ] 12.1 实现 useG10ImportExport.ts 导入导出composable(5张表)
  ...
  - [ ] 12.2 实现 useG10DualMode.ts 双模式切换 + useG10FormData.ts 数据加载/保存
    - 双模式(HTML ↔ OnlyOffice) + localStorage持久化
    - useG10FormData: selfLoad/save/writebackTB
    - _Requirements: 10.6, 1.4_

  - [ ] 12.3 集成验证：版本链+抽凭+截止+附注EventBus+OCR+复核 全链路连通
...
- [ ] 14. 后端4py文件
  - [ ] 14.1 创建 _g10_trading_financial_liabilities.py (render策略+RENDERER_DISPATCH)
    - render_g10_trading_financial_liabilities函数
    - 返回componentType='g10-trading-financial-liabilities' + sheets配置
    - _Requirements: 1.3_

  - [ ] 14.2 创建 _g10_trading_financial_liabilities_service.py (业务逻辑)
  ...
  - [ ] 14.3 创建 _g10_trading_financial_liabilities_import_export.py (导入导出15端点)
  ...
  - [ ] 14.4 创建 _g10_trading_financial_liabilities_ai.py (AI生成5section)
    - POST /api/workpapers/{wp_id}/g10/ai/{section}
