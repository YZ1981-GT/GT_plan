...
- [ ] 1. 注册四件套
  - [ ] 1.1 后端注册：VALID_COMPONENT_TYPES添加'g4-bond-investment-sppi' + RENDERER_DISPATCH注册render_g4_bond_investment_sppi策略函数
  ...
  - [ ] 1.2 前端注册：htmlRendererRegistry添加'g4-bond-investment-sppi'映射
    - 在 `htmlRendererRegistry` 中添加 `'g4-bond-investment-sppi': () => import('./workpaper/GtG4BondInvestmentSppi.vue')`
    - _Requirements: 1.1, 1.3_

  - [ ] 1.3 wp_code_overrides.json添加4条映射（G4-5/G4-6/G4-7/G4-8→g4-bond-investment-sppi）
  ...
  - [ ] 1.4 创建数据加载composable useG4SppiFormData.ts + 双模式composable useG4SppiDualMode.ts
...
- [ ] 2. 公式引擎 useG4SppiFormulaEngine.ts + 13个PBT
  - [ ] 2.1 实现useG4SppiFormulaEngine.ts（parseNum + 2个决策函数 + 9个纯函数 + 组合判定函数）
  ...
  - [ ]* 2.11 PBT: Property 10 — 倒轧平衡判定一致性
  ...
  - [ ]* 2.14 PBT: Property 13 — SPPI与业务模式组合分类
...
- [ ] 3. 主入口 GtG4BondInvestmentSppi.vue
  - [ ] 3.1 实现主入口GtG4BondInvestmentSppi.vue（sheetName v-if分发 + defineAsyncComponent懒加载 + 版本链 + 复核provide）
...
- [ ] 4. Checkpoint — 注册四件套+公式引擎+主入口骨架
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 5. G4-5 业务模式分析（问卷式+决策chip）
  - [ ] 5.1 实现useG4SppiBusinessModel.ts composable（问卷逻辑+答案→决策→chip映射）
  ...
  - [ ] 5.2 实现G4TabBusinessModel.vue完整UI（方法论上下文+问卷radio+说明textarea+结论chip+次级组合+审计结论）
...
- [ ] 6. G4-6 SPPI测试（两部分+方法论上下文+虚拟滚动）
  - [ ] 6.1 实现useG4SppiTest.ts composable（两部分逻辑+动态行+SPPI决策自动设结论）
  ...
  - [ ] 6.2 实现G4TabSppiTest.vue完整UI（部分一虚拟滚动表格+方法论上下文列+部分二三步表格）
...
- [ ] 7. G4-7 盘点表 + G4-8 倒轧表（3区段Tab）
  - [ ] 7.1 实现useG4SppiInventory.ts composable（盘点信息头+明细+公式+合计行）
  ...
  - [ ] 7.2 实现G4TabSecuritiesInventory.vue完整UI（信息头form+明细表格+合计行+审计结论）
  ...
  - [ ] 7.3 实现useG4SppiReconciliation.ts composable（3区段+行同步+差异高亮逻辑）
  ...
  - [ ] 7.4 实现G4TabInventoryReconciliation.vue完整UI（3区段Tab+el-table+行同步+差异红色高亮+合计行）
...
- [ ] 8. Checkpoint — G4-5~G4-8四个子组件完整实现
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. 导入导出 + AI（5 section）
  - [ ] 9.1 实现后端导入导出端点 _g4_bond_investment_sppi_import_export.py（3张表×3端点=9端点）
  ...
  - [ ] 9.2 实现前端useG4SppiImportExport.ts composable（el-dropdown导入导出▾ + 3张表）
  ...
  - [ ] 9.3 实现后端AI端点 _g4_bond_investment_sppi_ai.py（5 section）
  ...
  - [ ] 9.4 前端AI辅助按钮集成（5个section标题行右侧AI按钮）
...
- [ ] 10. UI精调 + 集成测试
  ...
  - [ ]* 10.2 集成测试（sheetName分发+公式链+UI交互）
