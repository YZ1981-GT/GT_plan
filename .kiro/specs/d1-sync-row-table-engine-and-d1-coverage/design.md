...
## 附：D4 已验证的形态谱系（2026-09-25 复盘补，本 spec 必须消费）

首版把「受管 sheet」等同于「行表」，漏掉了引擎**已有**并已被 D4 用于生产的另两条路径。以下三表
是从 D4 的 26 个 per-sheet 模块反推出的完整谱系，本 spec 与 D2/D3/D567 四个 spec 全部消费它。

### 表一：binding 二分（`excel_extract.BindingKind`，:606）

| | `excel_table`（动态） | `static_region`（静态） |
|---|---|---|
| 锚点 | Excel Table displayName | **workbook-scope definedName** |
| `table_name` / `uuid_column` | 必填 | **必空** |
| `defined_name` | 必空 | 必填 |
| 注入时 | 建 Excel Table + 注隐藏 UUID 列 | **只写 definedName**，不建 Table、不注 UUID 列 |
| materialize | `plan_managed_writes` 全门 | `_plan_static_writes` 按绝对坐标直写 |
| row_shift / footer 两门 / minted UUID / workbook 传播 | 全走 | **全不走** |
| binding 生成 | `_align_specs_to_sibling_tables` | `_static_region_bindings(provider=…)` **自动** |
| D4 用例 | D4-1/2/9/34/36… | **D4-33**（72 static cell）/ **D4-8** / **D4-13**（A6/A16 单 cell） |

⇒ 纯静态 sheet 走 `static_region` 能**绕开整条位移链**（无 row_shift、无 footer 重冻结、无兄弟
Table ref 维护）—— 这是本 spec 反复强调的最脆弱那段。能走静态就不该硬塞行表。

### 表二：行身份三形态

| 形态 | `row_identity_key` | 行可增删 | binding | UUID 列 | D4 用例 |
|---|---|---|---|---|---|
| UUID 动态行 | `rowId` | 是 | `excel_table` | 注入 | D4-2 / D1-3 / D2-2 |
| **稳定 key 固定行** | **`key`** | **否** | `excel_table` | **仍注入** | **D4-6**（12 固定指标，key 取前端 `DEFAULT_INDICATORS`；模板无空列 ⇒ 注入列 I） |
| 无行身份 | — | — | `static_region` | 无 | D4-13 / D4-33 |

⇒ 「行不增删」**不等于**「走静态区」。D4-6 是固定 12 行但仍是 `excel_table` binding + 注入 UUID 列
（`ROW_IDENTITY_KEY_D46 = "key"`）。判据是「有没有行维度」而不是「行会不会变」。

### 表三：`StoreKind` 与 `BindingKind` 正交（首版混为一谈）

```
StoreKind    rows / dict / fixed_text / dedicated     ← store 载荷形状
BindingKind  excel_table / static_region              ← Excel 侧几何
```

同一个 `fixed_text` store item 可能落在 `static_region`（D4-13 单 cell 锚点），也可能**寄生**在
姊妹动态表的 `excel_table` binding 上（D4-5 分组紧凑表）。⇒ 首版据 `StoreKind.fixed_text` 直接
判「0 受管区」是错的：**D1-14 的 10 个标量实测应各占一个 `static_region` 受管区**。

### 表四：HTML-only item 子集（受管 sheet ≠ 全部 item 受管）

`phase5_d4_policy_check_sheet.HTML_ONLY_ITEM_IDS_D45` 三项（`D4-5-credit-policy` /
`-audit-note` / `-audit-conclusion`）因「footer 下 `static_row` 与插行 fail-closed 冲突」保持
HTML-only。⇒ 本 spec 各 sheet 的 note / conclusion / procedures 类 item 若落在 footer 之下，
须逐项核是否同样命中该冲突，命中则登记进 HTML-only 子集而非强行受管。

### 表五：宿主两套 gating（漏一套会工具条叠加冲突）

D4 实测两套并存：`isD4DetailSheet`（主 detail 链）+ `isD4DedicatedSyncSheet`（专用同步 sheet 链，
如 D4-5 `D4TabPolicyCheck` / D4-13 `D4TabErpCheck` / D4-35，它们**不进** `isD4DetailSheet`）。
D4-35 与 D4-13 都曾因漏登记后者而工具条叠加冲突。⇒ 本 spec 若让审定表 D1-1 走独立宿主
（它与 detail 链的 sheetKey 不同），须同时登记两套。

### 对本 spec 批次表的影响（待 Task 1 逐项实测确认）

| sheet | 首版标记 | 复盘后候选形态 |
|---|---|---|
| D1-14 政策检查（10 标量） | `fixed_text` / **0 受管区** | **`static_region` ×N** 或 HTML-only |
| D1-13 的 15 标量 | `fixed_text` | `static_region` ×N 或 HTML-only（另两个行表不变） |
| D1-10 的 3 个 recon 标量 | 标量伴生 | `static_region` 或 HTML-only（看是否在 footer 下） |
| D1-2（2 固定行 + 动态行） | 动态行 | **混合身份**：固定行用稳定 key、动态行用 `rowId`，须确认引擎支持同表混合 |
| D1-6 `D1-bm-basis-rows`（3 固定行） | 行表 | **稳定 key 固定行**（D4-6 范式） |
| D1-6 `D1-bm-qa-matrix`（4×3 真二维） | 待评估转置表 | 另加候选：**`static_region`**（12 格绝对坐标，绕开位移链） |

🔴 **受管区总数因此会变**（首版 22~23 是按「标量 = 0 受管区」算的）。Task 1 实测后须重算路线。

## 上游锚定（2026-09-25 第三轮复盘补：首版缺这一节）

🔴 **本 spec 是 umbrella spec `workpaper-html-onlyoffice-bidirectional-writeback-closure` 的
Task 46「逐一迁移 D 循环 Excel 独立 entry」的下游 lane spec**，与 D4 lane 在该 umbrella 的
`2026-09-22 D4 lane 登记` 附注里的关系同型。首版零引用该 umbrella 与其已冻结的 slice，
属 spec 卫生缺陷（违反「spec 设计阶段必做现状 grep 确认：每项改进先确认依赖 spec 是否已有产物」）。

### 已存在的上游产物（复用，不重造）

| 产物 | 位置 | 已冻结内容 |
|---|---|---|
| D 循环 manifest slice | `backend/data/workpaper_sync_d_cycle_manifest_slice.json`（1137 行） | Task 46 冻结的 **7 个 D 循环独立 entry** 逐项裁决：`migration_state` / `adapter_id` / `published_representation` / `verification_state` + `unverifiable_reasons` |
| E 循环姊妹 slice | `backend/data/workpaper_sync_e_cycle_manifest_slice.json`（322 行） | slice 的 `sibling_slices` 互指 |
| umbrella Property 面 | umbrella design.md（1993 行） | Property 1–71。Task 46 正文声明验证 **Property 20 / 21 / 28 / 69 / 70** |

### 🔴 slice 实测推翻本 spec 的一条前提：D1 的 adapter 并未注册

首版（及 INDEX 登记）把 D1 描述为「已接 1 张（D1-3 受管）、扩到 N 张」的声明层工作。slice 实测：
...
D1 的 `verification_state=UNVERIFIABLE`，五条 `unverifiable_reasons` 含
**`no_registered_sync_adapter`** / `no_published_result_representation` /
`no_non_null_approved_definition_bundle` / `no_instrumentation_candidate_for_this_entry` /
`no_real_onlyoffice_94_probe_run_for_this_entry`。

⇒ **「D1-3 受管」指的是 provider 里已声明该 sheet，不等于 adapter 已注册、更不等于
`bidirectional_verified`。** 本 spec 的框架层与声明层工作照常推进（它们是 provider 侧的事），
但**真栈整册 materialize 与 §9.6 三谓词判据在 adapter 注册前跑不起来** ⇒ 须如实标 `[ ]*`
并写明卡点，处置同 D3 spec 裁决 F5 / D567 spec 裁决 G6，**不得以合成测试冒充真栈**。

### 该卡点是平台级的，不是 D1 独有

umbrella 的 **BP-61-1** 实测：`working_paper_sync_entry_state` /
`working_paper_content_version` / `working_paper_content_representation` 三表近空，
**186 个 planned entry 一个都注册不上**，连供给最完整的 Excel pilot 也一样；其生产者是
`ContentMutationService.commit(...)` 与 umbrella Tasks 36/77 的 finalize gate。

⇒ **D1/D3/D5/D6/D7 + E1 六个循环卡在同一个供给缺口**，不该在六个 spec 里各自把「发布链」
重做一遍。本 spec 只负责 provider 侧就绪 + 判据先行；发布链归 umbrella。

### Property 编号必须 spec-scoped

umbrella 有 Property 1–71 与 BP 编号，且**它自己踩过同号不同义的坑**：Task 61 附注记录
「全局 `BP-16`~`BP-22` 已被 Tasks 60/63/64 各自重复占用、同号不同义」，修法是改用 task-scoped
前缀 `BP-61-x` 并以 `re.fullmatch(r"BP-61-\d+")` 锁死。

⇒ 本 spec 的 `Property N` 一律读作 **`D1-P{N}`**，引用上游须写全 `umbrella Property N`。
已知冲突：本 spec Property 3 / 20 / 21 / 28 与 umbrella 同号不同义。
