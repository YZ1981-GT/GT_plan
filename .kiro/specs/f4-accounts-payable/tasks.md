...
- [ ] 4. 实现审定表 + 附注披露 composable
  - [ ] 4.1 创建 `composables/useF4Adjudication.ts`
...
- [ ] 5. 实现各sheet composable
  - [ ] 5.1 创建 `composables/useF4Detail.ts`（F4-2明细表3区段逻辑）
  ...
  - [ ] 5.2 创建 `composables/useF4SubstantiveAnalysis.ts`（F4-4实质性分析逻辑）
    - 定义行类型（8列）
    - 实现公式链：变动额=本期-上期 / 变动率 / 预期差异=本期-预期
    - 实现变动率>20%橙色标记
    - _Requirements: 7.1~7.6_

  - [ ] 5.3 创建 `composables/useF4LongOutstanding.ts`（F4-5长期挂账逻辑）
    - 定义 `LongOutstandingRow` 类型（11列）
    - 实现挂账天数公式 + 高亮标记（>2年橙色，>3年红色）
    - 实现合计行（总额/2年以上/3年以上/建议转收入）+ 动态行增删
    - _Requirements: 8.1~8.6_

  - [ ] 5.4 创建 `composables/useF4RelatedParty.ts`（F4-6关联方检查逻辑）
    - 定义 `RelatedPartyAPRow` 类型（15列）
    - 实现贷方余额公式 + 占比公式 + 集中度>30%橙色
    - 实现合计行 + 动态行增删
    - _Requirements: 9.1~9.6_

  - [ ] 5.5 创建 `composables/useF4UnrecordedCheck.ts`（F4-7未入账检查逻辑）
...
- [ ] 6. 实现 Vue子组件
  - [ ] 6.1 创建 `f4-accounts-payable/F4TabAdjudication.vue`（F4-1审定表两级结构）
    - 调用useF4Adjudication
  ...
  - [ ] 6.2 创建 `f4-accounts-payable/F4TabDetail.vue`（F4-2明细表 27列→3区段Tab）
    - 调用useF4Detail
  ...
  - [ ] 6.3 创建 `f4-accounts-payable/F4TabUnrecordedCheck.vue`（F4-7未入账检查 反向截止测试）
  ...
  - [ ] 6.4 创建 `f4-accounts-payable/F4TabVoucherCheck.vue`（F4-8检查表 借方/贷方区块）
  ...
  - [ ] 6.5 创建 `f4-accounts-payable/F4TabSupplierFinancing.vue`（F4-9供应商融资）
...
- [ ] 8. 实现后端 + 导入导出 + AI
  ...
  - [ ] 8.2 创建 `composables/useF4ImportExport.ts` + `composables/useF4DualMode.ts`
