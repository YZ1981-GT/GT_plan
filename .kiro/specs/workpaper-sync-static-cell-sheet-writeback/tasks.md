...
- [-] 11. * D4-8 census 裁决 + 接入：census 实测 D4-8 是「块计数动态（块内静态）」还是「真实可增删动态行」；静态则建 provider 走静态路径接入（发布链 + 前端接桥）；动态则记录裁决出本 spec 范围；census 前 blocked，不假设。
  ...
  - **仍 `[~]` 未完成项**：①rematerialize 发布 representation（需 live PG，D4 entry materialize 已 138s 逼近 120s 软上限）；②前端 `D4TabProductMargin.vue` 未接 `useWorkpaperSyncBridge`、宿主 `isD4DedicatedSyncSheet` 未含 `'D4-8'`。故 D4-8 当前为 🟡 半接入（后端契约已落、前端未接桥、representation 未发布），**UNVERIFIABLE 不假绿**。
