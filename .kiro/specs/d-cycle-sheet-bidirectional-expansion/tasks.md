...
- **D4-13 ERP 核对补裁决落地（2026-09-21，撤销旧 N/A 判定）**：`d4-inspection-writeback-formula-io/evidence/b1-provider-geometry.md` 原判「D4-13 纯文本，N/A」基于「行表 provider」范式，未评估静态字段直映射路径。全篇（A1:E19，openpyxl 实测）无插删动态行表、无公式，改用 static_region 静态受管区（同 D4-33/D4-8，非 D4-5 fixed 区寄生模式——D4-13 无姊妹动态表可寄生）：「核对过程」锚 A6、「核对结论」锚 A16，各占单 cell，无 Excel Table/无 UUID 列。新建 provider `phase5_d4_erp_check_sheet.py`；接入 `phase5_d4_revenue_detail`（`_INCLUDE_D413_ERP_CHECK_SHEET` 开关 + `static_sheets` 挂载于主 primary spec + sheets payload + sibling_stores 登记 + combined projection + 独立 merge 函数）+ `oo_to_html.py`（鸭子类型 mirror 块）。前端 `D4TabErpCheck.vue` 从手写双模式+legacy `GtOnlyOfficeSheet` 迁 `useD4SyncMode`。**顺带修复 7 处既有 bug**（D4-6/10/11/17/18/19/30/31/32 共用的 `build_store_projection` 构造 `FieldValue` 时 `value_type`/`mode` 传裸字符串未转枚举，`normalize_value` 的 `is` 恒等比较与 `.mode.value` 属性访问会炸；这是本次 rematerialize 首次让全 entry roundtrip 校验真正跑通时暴露，`==` 比较此前一直掩盖）+ 宿主 `isD4DedicatedSyncSheet` 漏登记 D4-13（会致工具条叠加冲突，同 D4-35 曾踩坑）。发布链跑全：contract --apply(digest 9fa97e7f)→provision(bundle+3)→rematerialize(gen 91→92,revision 115→116)。守卫：后端 `test_d4_13_erp_check_contract.py`(10 passed)+ 7 处修复 provider 既有测试(46 passed)+ D4 后端套件(362 passed)；前端 `d4InspectionSyncHostWiring.spec.ts` 扩展 D4-13(27 passed)。e2e L1 真实浏览器验证：D4-13 单张 + 13 张批次（含既有 12 张）全 passed，无回归。evidence 见 `d-cycle-sheet-bidirectional-expansion/evidence/T10-d413-erp-check-field-mapping.json`。
