...
- [ ] 1. Phase0 双源核对：S32 结构落定
  - 运行 `analyze_s_category.py`，确认 S32-1~13 的程序表 sheet 名、IC-0 导引表、IC-X 披露格式参考、提示 sheet 清单
  - 产出 13 舞弊情形 Tab 标签 + 子 sheet 映射表
  - _Requirements: 3.1, 3.3, 3.4_

- [ ] 2. 后端注册与 skip 映射
  - [ ] 2.1 VALID_COMPONENT_TYPES 新增 `s32-fraud-bundle`
    - _Requirements: 1.4_
  - [ ] 2.2 wp_code_overrides：`S32→s32-fraud-bundle`，S32-1~13 → `skip`
    - _Requirements: 1.2, 2.1, 2.2_
  - [ ] 2.3 validate_overrides 校验通过（单测）
    - _Requirements: 1.4_

- [ ] 3. useS32BundleState composable
  - [ ] 3.1 wpIdMap（wp_index 过滤 S32-*）+ 可见 Tab 计算
    - _Requirements: 5.1, 5.2_
  - [ ] 3.2 completionMap + progressSummary + refreshCompletion
    - _Requirements: 7.1_

- [ ] 4. GtS32Bundle.vue 主入口
  - [ ] 4.1 13 舞弊情形 TabDef + el-tabs 可滚动
    - _Requirements: 3.1, 3.2_
  - [ ] 4.2 含 IC-0/IC-X 底稿的子 sheet 切换（核查程序/导引/披露格式参考）
    - _Requirements: 3.3_
...
- [ ] 5. 引用与前端注册
  ...
  - [ ] 5.2 htmlRendererRegistry 注册 `s32-fraud-bundle`（defineAsyncComponent, contextProps standard）
...
- [ ] 7. PBT 与集成测试
  ...
  - [ ] 7.2 集成 + Playwright：点击 S32→情形页签切换→导引/披露子 sheet→只读模式
