...
- [ ] 2. 工程纪律 CI 守卫（报告模式）
  ...
  - [ ] 2.5 新增 `vite_transform_smoke.mjs`（Vite 全树冒烟）
  ...
  - [ ] 2.7 render 冒烟测试集骨架（Playwright，数据驱动）
  ...
  - [ ] 2.8 挂载全部新守卫到 `governance-checks.yml`（REPORT 模式）
...
- [ ] 4. Workpaper_Shell 骨架与 Coverage_Ledger
  - [ ] 3.1 实现 `useWorkpaperScaffold` composable
  ...
  - [ ] 3.2 实现 `GtWorkpaperShell.vue` 套壳组件
  ...
  - [ ] 3.5 实现 `check_coverage_ledger.py`（CI_Drift_Guard，fail-open）
  ...
  - [ ]* 3.6 编写属性测试 P3 · CI_Drift_Guard 仅正向检出时阻断
  ...
  - [ ] 3.7 实现 codemod `migrate_displayprefs_inject.py`
...
- [ ] 5. 分批迁移存量底稿 tab（每批 render 冒烟验证）
  - [ ] 4.1 迁移批次 D/E/F 循环 tab
    - 对 D/E/F 循环 tab 跑 codemod `--apply`，Playwright render 冒烟子集实测 0 console error 才合入
    - _Requirements: 1.3, 1.5_

  - [ ] 4.2 迁移批次 G/H/I 循环 tab
    - codemod `--apply` + 该批 render 冒烟实测
    - _Requirements: 1.3, 1.5_

  - [ ] 4.3 迁移批次 J/K/L 循环 tab
    - codemod `--apply` + 该批 render 冒烟实测
    - _Requirements: 1.3, 1.5_

  - [ ] 4.4 迁移批次 M/N/S 循环 tab
    - codemod `--apply` + 该批 render 冒烟实测
    - _Requirements: 1.3, 1.5_
...
- [ ] 9. Wp_Kit 组件库与 ESLint 规则
  - [ ] 6.1 实现 `<WpSection>` 与 `<WpFormulaCell>`
  ...
  - [ ] 6.6 新增 ESLint 规则 `no-adhoc-wp-structure`
...
- [ ] 11. 统一取数 SDK
  ...
  - [ ]* 7.6 编写属性测试 P6 · year 自动解析后请求永不缺失 year
...
- [ ] 12. Checkpoint · AuditData SDK
  - Ensure all tests pass, ask the user if questions arise.

### M3 · P2 降本（Wave 11–13，Req 6/7/8/9，全部 optional `*`，排期靠后）

- [ ] 13. 巨型文件拆分与 composable 工厂化（Req 6）
  - [ ]* 8.1 按域拆分前端巨型文件
...
- [ ] 14. 单一权限矩阵（Req 7）
  - [ ]* 9.1 实现 `Permission_Matrix.can(action, resource, context)`
    - 单一入口，前后端共享同一权限定义；支持 5 角色项目级权限区分
    - _Requirements: 7.1, 7.2, 7.3, 7.6_

  - [ ]* 9.2 前端权限判定走 Permission_Matrix
...
- [ ] 15. 统一溯源抽屉与刷新链回归（Req 8）
  - [ ]* 10.1 实现 `Trace_Drawer` 组件
    - 对任意金额展示「四表→报表→审定→底稿→调整→附注」完整链，每一跳可点击跳转到对应底稿/单元格/调整分录
    - _Requirements: 8.1, 8.2_

  - [ ]* 10.2 收敛 ReportTracePanel 与 GtIndexChip 为单一入口
    - 将两套溯源入口收敛为单一 Trace_Drawer 入口
    - _Requirements: 8.3_

  - [ ]* 10.3 draft-refresh 5 角色端到端回归（Playwright）
...
- [ ] 16. AI 上下文工厂（Req 9）
  ...
  - [ ]* 11.4 编写属性测试 P11 · AiContext 工厂产出非空且含约定键集
