...
- [x] 0.1 openpyxl脚本读取N4税金及附加.xlsx全部9 sheet
...
- [x] 2.1 创建 `composables/useN4FormulaEngine.ts`（损益类！）
...
- [x] 2.2 创建 `composables/useN4MultiTaxEngine.ts`（多税种测算引擎，与N2同源纯函数）
...
- [x]* 2.4 编写 Property P2 PBT：损益类本期发生额（借方发生-贷方发生）
...
- [x]* 2.5 编写 Property P3 PBT：合计行恒等
  - 断言：calcSubtotal(arr) === Σarr
  - **Feature: n4-taxes-and-surcharges, Property P3: 合计行恒等**

- [x]* 2.6 编写 Property P4 PBT：城建税及附加
...
- [x] 7.1 单元测试：useN4FormulaEngine + useN4MultiTaxEngine
  - 损益类方向(本期发生额) + 各税种计税依据×税率 + 同比变动 + 边界(零值/大数/上期为0除零)
  - _Requirements: P1-P6_

- [x] 7.2 集成测试：损益类取数+N2对应+跨底稿联动
