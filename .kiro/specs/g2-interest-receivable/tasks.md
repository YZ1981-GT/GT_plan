...
- [ ] 2. 实现公式引擎 useG2FormulaEngine.ts
  ...
  - [ ]* 2.2 编写 Property 1 PBT：利息测算公式(365天)
  ...
  - [ ]* 2.9 编写 Property 8 PBT：ECL与EAD正比
  ...
  - [ ]* 2.10 编写 Property 9 PBT：阶段判定确定性
  ...
  - [ ]* 2.11 编写 Property 10 PBT：净应收=应计-已收
...
- [ ] 3. 实现 useG2FormData.ts 基础数据加载/保存
  - [ ] 3.1 创建 `composables/useG2FormData.ts`
    - 实现 allResponses Map加载（GET /checklist-responses）
    - 实现 saveImmediate / debouncedSave / saveBatch
    - 实现 selfLoad逻辑
    - _Requirements: 1.6_

- [ ] 4. 实现审定表 + 附注披露 composable
  - [ ] 4.1 创建 `composables/useG2Adjudication.ts`
...
- [ ] 6. 实现 Vue子组件
  ...
  - [ ] 6.5 创建 `G2TabVoucherCheck.vue`（G2-8 凭证检查表 借方/贷方区块）
    - 调用useG2VoucherCheck
...
- [ ] 8. 实现后端 + 导入导出 + AI
  ...
  - [ ] 8.2 创建 `composables/useG2ImportExport.ts` + `composables/useG2DualMode.ts`
