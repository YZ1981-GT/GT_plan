...
- [ ] 14. 全量回归测试
  ...
  - [~] 14.3 Playwright E2E 核心链路（登录→项目→底稿→调整→报表→签字）（0.5 天）验证：全绿（**前后端未启动 ERR_CONNECTION_REFUSED，需 start-dev.bat 后执行**）
  - [~] 14.4 真实项目 UAT（YG2101 或类似，跑全流程）（1 天）验证：合伙人签字确认（**待真实环境**）
