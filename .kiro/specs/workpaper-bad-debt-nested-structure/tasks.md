...
- [x] 1. 基础设施：V070 迁移 + ORM 模型 + DTO（三层一致）
  - [x] 1.1 编写 V070 迁移 DDL 与 R070 回滚
  ...
  - [x] 1.2 实现 `BadDebtDetailRow` ORM 模型 + `ProvisionMethod` 枚举
...
- [x] 2. AutoSumEngine 纯计算模块
  - [x] 2.1 实现 `AutoSumEngine` 三级汇总 + 平衡校验
...
- [x] 3. NestedTableService CRUD + 层级管理
  ...
  - [x] 3.2 实现 `create_parent_row` / `create_child_row`
  ...
  - [x] 3.3 实现 `update_row`（乐观锁）/ `delete_row`（级联）
  ...
  - [x] 3.4 实现 `validate_integrity` 完整性校验
...
- [x] 4. Checkpoint - 基础数据层与汇总校验
  - Ensure all tests pass, ask the user if questions arise.

- [x] 5. 试算表辅助预填 PrefillService
  - [x] 5.1 实现 `BadDebtPrefillService.prefill_summary`
  ...
  - [x]* 5.3 PrefillService 单元测试
    - 1231 缺失（跳过）/ 已有值不覆盖 / 正常预填带来源标注
    - _Requirements: 4.4, 4.5_

- [x] 6. 调整分录联动 AjeGenerator
  - [x] 6.1 实现 `BadDebtAjeGenerator.generate_suggestion`
  ...
  - [x]* 6.3 AjeGenerator 单元测试
    - 零差额（不生成）/ 补提 / 冲回 / 用户修改后覆盖旧建议
    - _Requirements: 5.5, 5.6_

- [x] 7. 序列化 / 反序列化
  - [x] 7.1 实现 `NestedTableService.serialize` / `deserialize`
...
- [x] 9. Checkpoint - 服务层全链路（预填/AJE/序列化/公式）
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. API 路由 + router_registry 注册
  - [x] 10.1 实现 `bad_debt_rows` router 全部端点
  ...
  - [x] 10.2 在 router_registry/workpaper.py 数据组注册
  ...
  - [x]* 10.3 API 契约/集成测试（in-process ASGI httpx）
...
- [x] 12. 致同 D2-3 模板对齐与导出
  - [x] 12.1 实现 Export_Engine 14 列模板导出
...
- [x] 13. 数据校验完整性集成（Summary 与 TB 比对）
  - [x] 13.1 接入 Summary_Row 期末审定数与 TB 1231 audited_amount 比对
