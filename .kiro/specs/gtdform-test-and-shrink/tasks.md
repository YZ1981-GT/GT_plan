...
- [ ] 1. GtDFormReview 测试补齐（前置，必须先于拆分）
  - [ ] 1.1 创建 `GtDForm/__tests__/GtDFormReview.spec.ts`
  ...
  - [ ] 1.2 验证 vitest 该文件 0 fail
    - `npx vitest run audit-platform/frontend/src/components/workpaper/GtDForm/__tests__/GtDFormReview.spec.ts`
    - _Requirements: 1.5_

- [ ] 2. GtDFormConfirmation 测试补齐
  - [ ] 2.1 创建 `GtDForm/__tests__/GtDFormConfirmation.spec.ts`
    - 函证状态切换测试：sent→received→reconciled / exception 分支
    - 差异调节计算测试：金额差异 / 百分比差异
    - save payload 测试：状态切换后 payload 结构正确
    - _Requirements: 2.1, 2.2, 2.3, 2.4_
  - [ ] 2.2 验证 vitest 该文件 0 fail
    - _Requirements: 2.4_

- [ ] 3. GtDFormParagraph 测试补齐
  - [ ] 3.1 创建 `GtDForm/__tests__/GtDFormParagraph.spec.ts`
...
- [ ] 5. GtDFormReview 拆分
  - [ ] 5.1 抽取 `useReviewStateMachine.ts`
...
- [ ] 6. GtDFormConfirmation 拆分
  - [ ] 6.1 抽取 `useConfirmationState.ts`
    - 包含：状态切换 / 可用操作计算
    - _Requirements: 5.2_
  - [ ] 6.2 抽取 `useConfirmationFields.ts`
    - 包含：字段填充 / 差异调节 / 金额计算 / save payload
    - _Requirements: 5.2_
  - [ ] 6.3 重写 GtDFormConfirmation.vue 为 shell（≤400 行）
    - _Requirements: 5.1_
  - [ ] 6.4 验证 0 回归
    - vitest GtDFormConfirmation.spec.ts 0 fail + vue-tsc 0 errors
    - _Requirements: 5.3, 5.4_

- [ ] 7. GtDFormParagraph 拆分
  - [ ] 7.1 抽取 `useParagraphVariables.ts`
    - 包含：变量插值 / 上下文注入 / 模板解析
    - _Requirements: 6.2_
  - [ ] 7.2 重写 GtDFormParagraph.vue 为 shell（≤400 行）
    - _Requirements: 6.1_
  - [ ] 7.3 验证 0 回归
...
- [ ] 8. 白名单基线更新
  - [ ] 8.1 更新 `file_size_whitelist.txt`
    - GtDFormReview：1670 → 实测值+8%
    - GtDFormConfirmation：如已登记则收紧，未登记则新增
    - _Requirements: 7.1_
  - [ ] 8.2 验证 pre-commit check_file_size 全绿
    - _Requirements: 7.2_
