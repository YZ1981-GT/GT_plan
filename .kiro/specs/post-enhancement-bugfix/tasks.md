...
- [ ] 验证 Vite 构建通过，bundle 大小减小
- [ ] 验证页面图标正常显示（由 unplugin 自动处理）
...
- [ ] `addPending` 里对 POST/PUT/PATCH 请求设置 5 分钟自动清理定时器
- [ ] 或在 `removePending` 里确保超时/取消的请求也被清理
- [ ] 验证重复 POST 请求不会永久被拒

### Task 2.10 — EventBus 僵尸队列清理 [P1.6]
- [ ] `_notify_sse` 里 `QueueFull` 时调用 `self.remove_sse_queue(queue)`
- [ ] 验证 SSE 连接断开后队列被正确清理

### Task 2.11 — shortcuts.ts 输入框检查 [P1.7]
- [ ] `handleKeydown` 开头加 `isInputFocused` 检查
- [ ] 在 input/textarea/contenteditable 里忽略快捷键（Escape 除外）
- [ ] 验证在搜索框里按 Ctrl+S 不触发保存

### Task 2.12 — useProjectStore/dictStore 改用 apiProxy [P1.8/P1.9]
- [ ] `useProjectStore.loadProjectOptions` 改用 `api.get` 或 `listProjects`
- [ ] `dictStore.load` 改用 `api.get`，删除 `resp.data?.data ?? resp.data` 兼容代码
- [ ] 验证构建

### Task 2.13 — useAutoSave 改用 sessionStorage [P1.10]
- [ ] `useAutoSave.ts` 所有 `localStorage` 改为 `sessionStorage`
- [ ] 验证关闭标签页后草稿自动清除

### Task 2.14 — useEditMode guardRoute 选项 [P1.11]
- [ ] `useEditMode` 加 `guardRoute?: boolean` 选项（默认 true）
- [ ] `ConsolNoteTab` 等子组件调用时传 `{ guardRoute: false }`
- [ ] 验证控制台不再出现路由守卫警告

### Task 2.15 — displayPrefs.amountClass 修复 [P1.12]
- [ ] `prior === 0` 且本期不为 0 时直接高亮
- [ ] 验证新增科目（上期为0）的变动高亮正常
