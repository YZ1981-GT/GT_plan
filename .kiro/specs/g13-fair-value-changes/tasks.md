...
- [x] 2. 实现公式引擎 useG13FormulaEngine.ts
  - [x] 2.1 创建 `composables/useG13FormulaEngine.ts`，实现5个纯函数+parseNum
  ...
  - [x]* 2.5 编写 Property 4 PBT：变动率方向性与除零保护
  ...
  - [x]* 2.7 编写 Property 6 PBT：parseNum健壮性
...
- [x] 4. 实现数据层composable
  - [x] 4.1 创建 `composables/useG13FormData.ts`
  ...
  - [x] 4.2 创建 `composables/useG13ImportExport.ts` + `composables/useG13DualMode.ts`
...
- [x] 5. 实现Vue子组件（6个sheet）
  - [x] 5.1 创建 `g13-fair-value-changes/G13TabProcedure.vue`（G13A程序表）
  ...
  - [x] 5.2 创建 `g13-fair-value-changes/G13TabAdjudication.vue`（G13-1审定表35行×11列）
  ...
  - [x] 5.3 创建 `g13-fair-value-changes/G13TabDetail.vue`（G13-2明细表34行×12列）
  ...
  - [x] 5.4 创建 `g13-fair-value-changes/G13TabAdjustment.vue`（G13-3调整分录22行×10列）
  ...
  - [x] 5.5 创建 `g13-fair-value-changes/G13TabDisclosureListed.vue` + `G13TabDisclosureSOE.vue`（附注披露）
...
- [x] Wave 5–8：目录跳转、附注drawer、截止面板、交叉验证栏、guide strip（同 G12 模式）
- [x] Wave 9：G13-2合计行修复；G13 import E2E；截止样本回填 G13-3；G13-3 行级 GtReviewDot
- [x] Wave 10：GtWpRenderer b-index→G13 TabDirectory 委托（D4TabIndex 同级）
- [x] Wave 11：GCycleBIndexExtras — 底稿架构 + 本循环跨底稿目录

## D4-level polish (Wave 12)

- [x] Wave 12：去掉 legacy 复核按钮；useG13ExternalCross + G13-2 源科目勾稽栏；G13-1 pending 交叉栏
- [x] Wave 12：G1 发布 `g-cycle:source-fv`；G13A 程序表 E2E；gCycleExternalCross 单测
