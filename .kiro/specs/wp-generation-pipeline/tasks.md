...
- [ ] 2. 实现 parsed_data 填充服务（真断点 ⑥，最高优先）
  - [ ] 2.1 新增 `backend/app/services/wp_parsed_data_service.py`
  ...
  - [ ]* 2.3 编写 `_read_xlsx_structure` 单元测试
    - 对一个真实致同模板 xlsx 的解析（具体示例），断言 sheet 名 / cells / 表头单元格被正确读出
    - 覆盖边界：空 workbook、单 sheet、多 sheet
    - _Requirements: 2.2_

- [ ] 3. 改造 generate_from_codes：二段创建 + 失败隔离 + 结构化返回
  - [ ] 3.1 在单 wp_code 处理末尾接入 `populate_parsed_data`（在 `fill_workpaper_header` 之后调用，确保读到含表头的 xlsx）
    - 确保每个新建 `WpIndex` 都同步建出 `wp_index_id` 指向它、`file_path` 非空的 `WorkingPaper`
    - _Requirements: 1.1, 1.2, 2.1_

  - [ ] 3.2 用 savepoint 实现单条失败隔离
...
- [ ] 5. 前置门禁（语义纠正版）+ 端点接入
  - [ ] 5.1 在 `PrerequisiteChecker` 新增 `_check_generate_from_codes_prerequisites`
...
- [ ] 6. Checkpoint - 核心修复测试全绿
  - 运行 `python -m pytest backend/tests/... -v --tb=short`，确保 Task 2–5 的单元测试与属性测试全部通过，有疑问询问用户。

- [ ] 7. 标准科目取数路径澄清与验证（需求 4）
  - [ ] 7.1 在代码与文档中断言取数走 `trial_balance.standard_account_code`
...
- [ ] 9. 实跑验证脚本 `verify_wp_generation_pipeline.py`（需求 7，核心验收工具）
  - [ ] 9.1 实现脚本骨架（无 `_` 前缀，正式可重复工具）
...
- [ ] 10. 真实数据集成验收：对 df5b8403 实跑（终极验收，不可用单测绿替代）
  - [ ] 10.1 端到端实跑 `verify_wp_generation_pipeline.py` 并断言通过
