...
- [x] 0.1 openpyxl脚本读取H8使用权资产.xlsx全部20 sheet
...
- [x]* 2.6 编写 Property P4 PBT：CAS21初始计量
...
- [x]* 2.7 编写 Property P5 PBT：折旧期=min(租赁期,寿命)
...
- [x] 4.1 创建 H8TabIndex.vue 底稿目录
  - 20行+进度条+H9联动状态指示
  - _Requirements: 1.2_

- [x] 4.2 创建 H8TabAdjudication.vue 审定表H8-1
  - 双区块51公式+H9联动校验区域+TB回写
  - _Requirements: 2.1-2.8, 11.1-11.4_

- [x] 4.3 创建 H8TabDetail.vue + H8TabAdjustment.vue
  - 明细58列4区段(含初始计量公式) + 调整10列
  - _Requirements: 3.1-3.4_

- [x] 4.4 创建 H8TabLeaseIdentification/LeaseTerm/LeaseModification.vue
  - 租赁识别90行段落 + 租赁期52行段落 + 变更100行表格
  - _Requirements: 4.1-4.5_

- [x] 4.5 创建 H8TabMeasurementAnnual/Monthly.vue
  - H8-6分支选择器 + OO渲染 + CAS21公式说明
  - _Requirements: 5.1-5.6_

- [x] 4.6 创建 H8TabDepreciationNoImpair/WithImpair.vue + H8TabDepreciationAlloc.vue
  - H8-8分支选择器 + OO + 折旧分配
  - _Requirements: 6.1-6.6_

- [x] 4.7 创建 H8TabImpairment.vue + H8TabRecoverable.vue
  - 减值OO + DCF OO
  - _Requirements: 7.1-7.2_

- [x] 4.8 创建 H8TabDisposalCheck.vue
  - 28列租赁终止+终止损益计算+H9同步
  - _Requirements: 7.3-7.5_

- [x] 4.9 创建 H8TabSimplifiedCheck.vue
  - 18列+短期/低价值自动判断+统计摘要+不合规高亮
  - _Requirements: 8.1-8.4_

- [x] 4.10 创建 H8TabRelatedParty.vue + H8TabDisclosure(Listed/Soe).vue
  - 关联15列 + 附注
  - _Requirements: 9.1, 1.2_

### Phase 5: 后端

- [x] 5.1 创建 h8_right_of_use_assets_renderer.py
  - RENDERER_DISPATCH注册 + CAS21验证逻辑
  - _Requirements: 1.6_

- [x] 5.2 创建 h8_right_of_use_assets.py 路由（3端点）
  - 导出模板/导出数据/导入数据
  - _Requirements: 3.3_

- [x] 5.3 创建 h8_right_of_use_assets_service.py
...
- [x] 7.1 单元测试：useH8FormulaEngine + useH8CAS21Engine
  - CAS21核心公式 + 简化判断 + 终止损益
