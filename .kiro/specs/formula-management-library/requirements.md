...
- **一键刷新服务 (Draft_Refresh_Service)**：基于四表库未审数一键生成初稿（未审报表/底稿/附注等）的服务，当前入口在 `wp_render_config.py`。
- **初稿 (Draft)**：由一键刷新生成、尚未经人工复核的数据状态语义标记。
- **合伙人 (Partner_Role)**：角色 `partner` 或 `signing_partner`；一键刷新的唯一授权触发者。
...
- **预设公式库 (Preset_Formula_Library)**：平台内置的预设公式集合（`check_presets` / 预设库），供一键套用（preset 来源）。
...
4. WHEN Global_Refresh 入口收到合法合伙人调用, THE Draft_Refresh_Service SHALL 依据合伙人在全局刷新勾选弹窗（Requirement 19）选定的范围，执行从四表库未审数预填审定表/明细表/初稿的流程。
5. THE Role_Gate 的合伙人限制 SHALL 仅适用于 Global_Refresh 入口；模块/循环级局部刷新入口（Module_Refresh，如 `audit-sheet-refresh` 逐底稿刷新、报表模块内刷新、D 类底稿某循环的一键刷新）的授权由 Requirement 20 定义，不受合伙人限制，避免模块级刷新被一并锁死。
...
### Requirement 19: 合伙人全局刷新勾选弹窗

**User Story:** 作为业务合伙人，我想在点击"全局一键刷新"后弹出勾选弹窗自行勾选要刷新的内容，以便按需刷新报表/底稿/调整分录/附注等子集，而非每次都被迫全量刷新。

#### Acceptance Criteria

1. WHEN 合伙人在页面点击全局一键刷新, THE Refresh_Scope_Dialog SHALL 弹出并列出可刷新内容分类（Refresh_Scope_Item），至少包含报表、底稿（按循环）、调整分录、附注。
2. THE Refresh_Scope_Dialog 的 Refresh_Scope_Item 清单 SHALL 从平台既有模块/循环注册来源动态发现，而非在弹窗中硬编码固定列表，以避免遗漏后续新增模块。
3. WHEN 合伙人勾选一个 Refresh_Scope_Item 子集并确认, THE Draft_Refresh_Service SHALL 仅对被勾选范围执行全局刷数，未勾选范围保持不变。
...
6. WHEN 按勾选范围执行的全局刷新完成, THE Audit_Trail SHALL 记录本次刷新实际覆盖的 Refresh_Scope_Item 清单（依据 Requirement 4 的留痕契约）。

---

### Requirement 20: 模块/循环级局部刷新对其他角色开放
...
### Requirement 21: 公式作用域过滤与全局公式管理页
...
### Requirement 22: 公式三来源（预设 / 自定义 / 参照已有）
...
### Requirement 23: 公式三能力显式化（可视化 / 可编辑保存 / 可运算刷新）
...
| 需求 | 实证锚点 |
|------|----------|
| Req 1 | `wp_render_config.py:417-420` 一键刷新 `_user: User = Depends(get_current_user)` 无角色限制；`deps.py:125 require_role(allowed_roles)` |
| Req 2 | `report_trace.py` 子公司数据完整度前置校验可参考 |
| Req 3/4 | 一键刷新覆盖风险（memory：人人刷新覆盖彼此且刷完不复核） |
| Req 5 | auto_calc 已实现：WpFormula / ReportEngine / execute_note_formulas |
| Req 6 | logic_check 零散硬编码：`useReportCrossCheck` 7 条勾稽 / balance-check |
| Req 7 | reasonability 提示几乎缺失未落地 |
| Req 8 | 6 碎片化组件：FormulaRefPicker/FormulaEditDialog/FormulaManagerDialog/FormulaBar/NoteFormulaDialog/CellSelector |
| Req 9 | `wp_formula_service.py:44 save() -> tuple[WpFormula\|None, list[dict]]` |
| Req 10 | 公式列虚线下划线+tooltip 悬停当前仅底稿表格 |
| Req 11 | ACNR `.kiro/specs/acnr/` Req14；acnr-consumer-wiring Req9 校验路径 |
| Req 12 | 四表库叶子源；`get_active_filter` 统一入口；tb_balance 借正贷负；损益取发生额 |
| Req 13 | 审定表 `xxx-1` 回写 `trial_balance.audited_amount` |
| Req 14 | `WpFormulaService.save` full_resolve 校验；CrossSheetResolver 跨 sheet 追溯 |
| Req 15 | AJE/RJE → aje_adjustment → audited_amount → `ReportEngine.regenerate_affected` |
| Req 16 | `report_engine.py:1994 generate_unadjusted_report`；`generate_all_reports`；ReportFormulaParser |
| Req 17 | `note_formula_generator.py:314 execute_note_formulas`；acnr-consumer-wiring Req15 NoteFormulaDialog 修复 |
| Req 18 | word_export（审计报告/财报）+ report_excel_exporter（审定/未审导出） |
| Req 1（细化） | `wp_render_config.py:414 refresh_audit_sheet_from_ledger` 仅 `Depends(get_current_user)` 无角色限制=Module_Refresh 反例；全局 `/draft-refresh` 收敛为合伙人专属，模块级不锁死 |
| Req 19 | 全局刷新勾选弹窗；Refresh_Scope_Item 从模块/循环注册动态发现（避免硬编码遗漏）；留痕复用 Req 4 |
| Req 20 | `wp_render_config.py:414` audit-sheet-refresh 逐底稿刷新 + 报表模块内刷新 + D 类循环刷新；对有编辑权角色开放；Draft/last_computed 一致；留痕按模块粒度 |
| Req 21 | `FormulaManagerDialog.vue:453 FormulaManagerScope`（7 类）+ `scope` prop 默认 `report` + 树形导航 selectedNodeKey/selectedPath + `SCOPE_LABEL_MAP`；全局页跨 scope 浏览 |
| Req 22 | preset=`check_presets`/预设库套用；custom=`user_formulas` override（`UserFormulaItem` cell_key/formula/formula_type/is_preset_override/edited_at + `onRestorePresetFormula` 恢复预设 + `/api/workpapers/{wpId}/user-formulas/{cell_key}`）；reference=扩展 Req 9 复用 |
| Req 23 | 可视化=Req 10 tooltip+`FormulaStatusPanel` 列表；可编辑保存=Req 8/9；可运算刷新=Req 5/6/7 Formula_Engine 执行 |
| Req 24 | 前端 per-cycle 公式引擎 `useS34FormulaEngine`/`useD3FormulaEngine`/`useD3CrossSheet` 等 `useXFormulaEngine` composable 客户端硬编码计算未纳三类型；试点接入 useD3FormulaEngine + useS34FormulaEngine；跨表引用经 useAcnr 解析（协调 acnr-consumer-wiring Req 14） |
| Req 25 | `formula_parse_utils.evaluate_formula`/`FormulaEvaluator` 已标 `DeprecationWarning` 委托 `formula_engine.execute` L1 内核，consol_report_service 等 18 处调用待迁移；内联 `.formula-cell`+`title=`（如 `J2TabDetail.vue`）收敛到 `GtFormulaSourceTooltip`；`useStaleStatus.recalc`→`/trial-balance/recalc`+`prefill_stale` 衔接一键刷新 |

---

## Requirements（深化核验补充 — 前端公式引擎层 + 单一内核收口）

> 第二轮实证核验（codegraph）发现设计初稿之外仍有公式管理触点，补充如下，确保"无死角"：
> - **前端 per-cycle 公式引擎**：`useS34FormulaEngine`（FormulaCell/createSumFormula/createRatioFormula/createThresholdFormula/computeFormulas）、`useD3FormulaEngine`（aggregateByNature/aggregateByAging）、`useD3CrossSheet` 等一批 `useXFormulaEngine` composable，在**客户端硬编码**计算，未纳入三类型模型，也不经统一编辑/悬停。
> - **已废弃求值层**：`backend/app/services/formula_parse_utils.py` 的 `evaluate_formula` / `FormulaEvaluator` 已标 `DeprecationWarning`，委托 `formula_engine.execute`（L1 内核），但 consol_report_service 等仍有 18 处调用。
> - **散落内联 tooltip**：多处组件（如 `J2TabDetail.vue`）已用 `.formula-cell { border-bottom:1px dashed; cursor:help }` + `title="=..."` 内联展示公式，未收敛到统一 `GtFormulaSourceTooltip`。
> - **recalc / prefill_stale**：`useStaleStatus.recalc` 触发 `/trial-balance/recalc`，`prefill_stale` 标记过时，与一键刷新/公式重算相关联。

### Requirement 24: 前端 per-cycle 公式引擎纳入三类型治理（增量）
...
### Requirement 25: 求值内核收口 + 悬停 tooltip 收敛 + recalc 衔接

**User Story:** 作为公式管理库开发者，我想收口重复的求值实现、统一散落的公式悬停展示、并明确重算与一键刷新的衔接，以便公式计算有单一内核、展示一致、过时可控。

#### Acceptance Criteria

1. THE Formula_Engine SHALL 以 `formula_engine.execute`（L1 内核）作为后端求值的单一入口；`formula_parse_utils.evaluate_formula` / `FormulaEvaluator` SHALL 标注为废弃并逐步由调用方（如 consol_report_service 的 18 处调用）迁移到 L1 内核或 `report_engine.evaluate_formula`。
...
6. THE Formula_Management_Library SHALL 使一键刷新（合伙人专属，Req 1）与普通 `recalc`（团队成员可触发的试算表重算）语义清晰区分：recalc 仅重算既有公式不生成初稿、不打 Draft 标记；一键刷新生成初稿并受角色门禁约束。

---

## Requirements（补充 — 公式预设库 + 导入导出 + 模板库入口）

> 第三轮补充（用户）：公式预设是大活，需逐页分析实际结构预设公式作为公式库一部分；公式模块需导入导出（导出模板含**编报说明**）；模板库中的 ACNR 地址坐标名称库按近期改动更新；模板库中公式预设库需在合适位置可点击弹窗，弹窗显示公式管理说明文档。
> 实证（codegraph）：预设雏形已存在——`prefill_formula_mapping.json`（预填公式）、`check_presets`（附注公式预设）、`wide_table_presets`、`get_formula_coverage`/`_load_prefill_mappings`（覆盖度）、seed 模式（`seed_note_account_mappings.py` + `NoteTemplatesSeed`）；模板库入口 `TemplateLibraryButton`（挂 NoteTemplateTab/ReportConfigTab/WpTemplateDetail）。

### Requirement 22: 公式预设库（逐页分析 + 预设，作为公式库一部分）
...
3. THE Formula_Preset_Library SHALL 以 seed 数据形式持久化（遵循现有 seed 模式，如 `formula_presets_seed.json` + 幂等 seed 脚本），并收敛已有的 `prefill_formula_mapping` / `check_presets` / `wide_table_presets` 到统一预设库口径。
4. WHEN 合伙人一键刷新或底稿生成时, THE Formula_Management_Library SHALL 依据预设库为目标页面套用预设公式（作为初稿公式的来源之一）。
5. THE Formula_Preset_Library SHALL 提供预设覆盖度视图（复用/扩展 `get_formula_coverage`），标注每个页面"已预设/待预设"，使逐页预设工作可增量推进、进度可见。
6. THE 逐页预设工作 SHALL 采取增量交付（按循环/模块分批预设，如先 D/报表/附注核心页面），未预设页面保留现状不回归，并在清单中标注待预设。
7. THE Formula_Preset_Library 中预设公式的引用地址 SHALL 使用 ACNR addr_id / formula_ref，经 full_resolve 可解析，而非硬编码坐标字符串（协调 Req 11）。

### Requirement 23: 公式模块导入导出（导出模板含编报说明）
...
6. THE 编报说明 SHALL 与 Req 25 的公式管理说明文档口径一致（同一份说明的模板内嵌版本），避免说明两套漂移。

### Requirement 24: 模板库 ACNR 地址坐标名称库更新
...
### Requirement 25: 公式预设库入口 + 公式管理说明文档弹窗
...
3. THE 说明文档弹窗 SHALL 以单一文档源渲染，与 Req 23.6 的导出模板编报说明同源，避免多处说明漂移。
4. WHERE 用户在弹窗内, THE 弹窗 SHALL 可导航到公式预设库的浏览/编辑（复用 Req 8 的统一 Formula_Edit_Dialog 与 Req 22 的预设清单）。
