...
- [x] 1. 数据层与三层一致性（V100 迁移 + ORM 同步）
  ...
  - [x] 1.2 同步 ORM 模型（workpaper_models.py）
  ...
  - [x]* 1.3 编写 V100 迁移幂等测试
    - 断言 `wp_formula` 五个新列 + 三张新表建成；重复执行迁移不报错（IF NOT EXISTS + information_schema 守护）
    - _Requirements: 3.2, 4.1, 4.2_

- [x] 2. 公式引擎类型分派层（formula_management/engine.py）
  - [x] 2.1 实现 resolve_ref 引用解析封装
  ...
  - [x] 2.2 实现 execute_formula 三类型分派 + 结果结构
  ...
  - [x]* 2.4 编写属性测试 P8（引用一律经 ACNR full_resolve）
  ...
  - [x]* 2.8 编写属性测试 P9（auto_calc 成功执行记录最近计算时间）
...
- [x] 3. 底稿 WpFormula 编辑/计算/保存契约扩展
  - [x] 3.1 扩展 WpFormulaService（三类型 + full_resolve 校验 + 跨 sheet）
  ...
  - [x]* 3.3 编写属性测试 P7（悬空引用拒绝保存并返回问题清单）
...
- [x] 4. logic_check 收编 useReportCrossCheck 7 条勾稽
  - [x] 4.1 落库 7 条勾稽为 logic_check 公式 + 执行端点
  ...
  - [x]* 4.2 编写属性测试 P17（logic_check 收编等价于原硬编码勾稽）
...
- [x] 5. 合伙人一键刷新编排器（Draft Refresh Service）
  - [x] 5.1 实现 precheck 四表库完整度前置校验
...
- [x] 8. 报表引擎契约（增量重算 + 生成）
  ...
  - [x] 8.2 实现 regenerate_affected 调整分录增量重算
...
- [x] 12. 前端统一公式体验组件
  ...
  - [x] 12.2 实现 GtFormulaSourceTooltip 统一来源提示
  ...
  - [x] 12.3 实现初稿/已审定区分展示 + Issue/Hint 面板
  ...
  - [x]* 12.4 编写 vitest 示例测试（GtFormulaEditDialog）
  ...
  - [x]* 12.5 编写 vitest 示例测试（GtFormulaSourceTooltip，含 P14）
...
- [x] 13. 前端公式引擎治理 + 求值内核收口（Req 27-28）
  - [x] 13.1 建前端公式引擎清单 + 三类型映射元数据
  ...
  - [x] 13.2 试点接入 useD3FormulaEngine + useS34FormulaEngine（悬停 + ACNR 引用）
  ...
  - [x] 13.3 求值内核收口 + recalc/一键刷新语义区分
    ...
    - _Requirements: 28.1, 28.2, 28.4, 28.5, 28.6_

  - [x]* 13.4 编写属性测试 P19/P20/P21
    - **Property 19: 前端引擎三类型映射非破坏计算**（Validates: Requirements 27.2, 27.6）— vitest：接入前后引擎产出数值逐一相等
    - **Property 20: 求值内核单一性**（Validates: Requirements 28.1, 28.2）— Hypothesis：随机公式经收口路径都委托 L1 内核
    - **Property 21: recalc 与一键刷新语义区分**（Validates: Requirements 28.4, 28.6）— recalc 不打 Draft、一键刷新打 Draft，既有公式重算结果一致
  ...
  - [x] 13.5 tooltip 收敛（增量）
    - 将散落的 `.formula-cell`+`title="=..."` 内联实现（如 J2TabDetail）逐步替换为统一 `GtFormulaSourceTooltip`；未收敛前保留现状不回归
    - _Requirements: 28.3_

- [x] 14. 公式预设库 + 导入导出 + 模板库入口（Req 29-32）
  - [x] 14.1 建预设库结构 + Preset Inventory + 覆盖度
    ...
    - _Requirements: 29.1, 29.3, 29.5, 29.7_

  - [x] 14.2 预设套用到一键刷新/底稿生成
    - DraftRefreshService / 底稿生成按 page_key 查预设库套用为初稿公式；pending 页跳过保留现状（无回归）
    - 说明：全局刷新真正消费预设的完整编排链见 Task 16（Req 21.2），本任务提供 `build_preset_draft_units`/`refresh_with_presets` 的预设套用能力
    - _Requirements: 29.4, 29.6_

  - [x] 14.3 逐页预设（增量·按循环分批，试点核心页）
    - 按循环/模块逐页读取实际结构预设公式，产出 `formula_presets_seed.json` 分片；试点 D 循环 + 报表 + 附注核心页面；引用一律 ACNR addr_id
    - inventory 标注已预设/待预设，进度可见
    - _Requirements: 29.1, 29.2, 29.6, 29.7_

  - [x] 14.4 公式模块导入导出（导出模板含编报说明）
  ...
  - [x]* 14.7 编写预设库/导入导出/说明文档单元测试
    - 说明：Req 29-32（预设库/导入导出/模板库/入口）在 design 中无对应 Correctness Property（属 seed 数据/UI 导入导出/文档单一源，按 design PBT 适用性判定用示例/单元测试而非 PBT）；本子任务校验需求条款，不引用 design 属性号（design 现文 P22/P23/P24 已重编为"勾选范围/空勾选阻断/范围发现"，语义属 Req 19-20，见 Task 16-18）
    - 预设引用可解析：预设条目引用经 full_resolve 可解析或在 inventory 标 pending（Validates: Requirements 29.7）
    - 导入导出往返 + 编报说明存在：导出模板首区块含编报说明；导入→导出往返一致；悬空引用报告并跳过不入库（Validates: Requirements 30.2, 30.4）
    - 说明文档单一源：导出模板编报说明与预设库弹窗说明文档同源不分叉（Validates: Requirements 30.6, 32.3）
    - 文件：`backend/tests/formula_management/test_pbt_p22_preset_grammar.py` / `test_pbt_p23_import_export_roundtrip.py`（既有文件名沿用，内部断言按需求条款描述）；前端 `__tests__/GtFormulaPresetDialog.spec.ts`

- [x] 15. Checkpoint - 已完成范围全量测试（Req 1-18, 27-32）
  - 运行 `python -m pytest backend/tests/formula_management/ -v --tb=short` 与 `npx vitest --run src/components/formula/`，确保 P1–P21 及预设库/导入导出示例测试全绿；如有疑问询问用户。

---

## 待实现任务（P0/P1 缺口补齐 — Req 19-21, 23-25）

> 以下任务补齐 design §18-§20 新组件（DraftRefreshOrchestrator / RefreshScopeDiscovery / GtRefreshScopeDialog / logic_check 前端收编 / 作用域过滤 / reference 三来源）与 P22-P32 新属性。全部 `[ ]` 未开始；可选测试子任务 `[ ]*`（按约定仍会做完）。边界铁律沿用：service 只 flush、router commit；四表库叶子源只读；引用走 ACNR full_resolve；中文文件名 RFC5987。

- [x] 16. 全局刷新生成编排层（DraftRefreshOrchestrator，Req 21｜P0·核心）
  - [x] 16.1 实现 DraftRefreshOrchestrator 生成编排
    - 新建 `backend/app/services/formula_management/draft_refresh_orchestrator.py`：`generate(project_id, year, operator, scopes, confirm_overwrite)` 按勾选 scopes 分派各生成器产 `RefreshUnit`+`page_keys`，交 `DraftRefreshService.refresh_with_presets` 统一治理（幂等/Draft/覆盖/快照/审计）
    - `_dispatch(scope)` 映射（design §18 表）：`report`→`ReportEngine.generate_unadjusted_report`；`adjudication`/`workpaper:{cycle}`→`AdjudicationWritebackService.writeback_batch`+底稿生成；`note`→`execute_note_formulas`；未识别 scope 返回空
    - scopes 先经 `RefreshScopeDiscovery.discover` 校验合法性（未知键忽略并告警，Req 20 共用发现口径）；未勾选域完全不触碰（零 units/page_keys→零写入）；`affected_count=len(refreshed_units)`（复用治理层，不再恒为 0）
    - 按 page_key 套预设（复用 Task 14.2 的 `refresh_with_presets`/`build_preset_draft_units`）；生成单元复用治理层 Draft 标记 + last_computed_at（`trigger_recalc`）+ 覆盖前快照（Req 4）
    - service 只 flush，router commit
    - _Requirements: 21.1, 21.2, 21.3, 21.4, 21.5_

  - [x] 16.2 改造 `/draft-refresh` 由裸 refresh 改调 orchestrator
    - 改造 `backend/app/routers/draft_refresh.py`：`/draft-refresh` 由裸调 `service.refresh()`（不传 units→零初稿）改为调 `DraftRefreshOrchestrator.generate(...)`；保留合伙人门禁 `require_role(partner/signing_partner)`、precheck 阻断 422、`await db.commit()`（service flush、router commit）
    - `draft_refresh_audit.detail.scopes` 记录本次实际执行的勾选范围清单（Req 21.6，复用 Req 4 留痕契约）；响应返回 affected_count + preset_application + precheck_warnings
    - _Requirements: 21.4, 21.6_

  - [x]* 16.3 编写属性测试 P31（全局刷新按勾选范围生成初稿且范围隔离、affected_count 真实）
    - **Property 31: 全局刷新按勾选范围生成初稿且范围隔离、affected_count 真实**
    - **Validates: Requirements 21.1, 21.3, 21.4**
    - Hypothesis：随机非空勾选子集 → (a) Draft_Unit.unit_scope 均落被勾选域/循环内；(b) 未勾选域数据快照逐一不变；(c) affected_count==len(refreshed_units) 且勾选域有内容时 >0
    - 文件：`backend/tests/formula_management/test_pbt_p31_orchestrator_scope.py`

  - [x]* 16.4 编写属性测试 P22（勾选范围=执行范围=留痕范围一致）
    - **Property 22: 勾选范围=执行范围=留痕范围一致**
    - **Validates: Requirements 19.4, 21.3, 21.6**
    - Hypothesis：随机非空勾选子集（含"底稿:循环{X}"循环粒度项）→ 仅对被勾选范围执行生成、未勾选域快照不变、审计留痕 scope 集合恰等于勾选集合
    - 文件：`backend/tests/formula_management/test_pbt_p22_scope_consistency.py`

- [x] 17. 全局刷新范围动态发现（RefreshScopeDiscovery，Req 20｜P1）
  - [x] 17.1 实现 RefreshScopeDiscovery + 发现端点
    - 新建 `backend/app/services/formula_management/refresh_scope_discovery.py`：`discover(project_id, year)` 从三来源动态派生 `RefreshScopeItem`（key/label/group/cycle）并按 key 去重——① 模块注册固定顶层域（报表/调整分录/附注）② `cycleDialogRegistry` 循环集合 ③ `wp_index` 现存 wp_code 循环前缀（`re.match(r'([A-N])', wp_code)`）；禁硬编码固定清单
    - 新增 `GET /api/workpapers/refresh-scopes?project_id=&year=` 端点返回 `{items:[RefreshScopeItem]}`，合伙人门禁（与 `/draft-refresh` 同 `require_role`）；供弹窗（Req 19）与后端编排（Req 21）共用同一口径，避免前后端漂移
    - _Requirements: 20.1, 20.2, 20.3, 20.4, 20.5, 20.6_

  - [x]* 17.2 编写属性测试 P24（刷新范围项动态发现无遗漏）
    - **Property 24: 刷新范围项动态发现无遗漏**
    - **Validates: Requirements 20.1, 20.2, 20.3, 20.4**
    - Hypothesis：随机模块/循环注册来源（cycleDialogRegistry cycle 集合 ∪ wp_index 循环前缀 ∪ 固定顶层域）→ 发现集合覆盖全部注册派生项并按 key 去重（新增即现、无硬编码遗漏）
    - 文件：`backend/tests/formula_management/test_pbt_p24_scope_discovery.py`

- [x] 18. 合伙人全局刷新勾选弹窗（GtRefreshScopeDialog，Req 19｜P0）
  - [x] 18.1 实现 GtRefreshScopeDialog.vue 前端入口
    - 新建 `audit-platform/frontend/src/components/formula/GtRefreshScopeDialog.vue`：合伙人可见的"全局一键刷新"入口按钮（`usePermissionMatrix.currentRole ∈ {partner, signing_partner}` 才渲染，非合伙人不可见）→ 点击弹窗
    - 消费 `GET /api/workpapers/refresh-scopes` 发现端点渲染树形勾选（`el-tree show-checkbox`：顶层域整选/半选 + 底稿域展开为各循环子项，可按循环如 D 类勾选而非只能整选全部底稿）
    - 空勾选 → 禁用"确认刷新"并提示"请至少勾选一项刷新内容"，不发起任何请求；确认 → `POST /draft-refresh { project_id, year, scopes:[...], confirm_overwrite }`
    - _Requirements: 19.1, 19.2, 19.3, 19.4, 19.5, 19.6_

  - [x]* 18.2 编写 vitest 示例测试 + P23（GtRefreshScopeDialog）
    - 示例：弹窗含报表/底稿/调整分录/附注基础项（Req 19.3）、底稿按循环展开（Req 19.4）、合伙人可见/非合伙人不可见入口（Req 19.1/19.2）
    - **Property 23: 空勾选阻断刷新**
    - **Validates: Requirements 19.5**
    - 空勾选集合 → 禁止提交、不触发刷新、无请求/无留痕
    - 文件：`audit-platform/frontend/src/components/formula/__tests__/GtRefreshScopeDialog.spec.ts`

- [x] 19. 报表勾稽前端消费后端 logic_check（收编闭环，Req 23｜P1）
  - [x] 19.1 改造 useReportCrossCheck 消费后端 logic_check 端点
    - 改造 `audit-platform/frontend/src/.../useReportCrossCheck.ts`：主路径消费后端 logic_check 执行端点返回的 Issue_List 驱动前端勾稽展示；保留 `computeCrossCheckResults` 纯函数作降级（后端超时/5xx/网络错回退，不阻断报表页面渲染，try/catch fail-open）
    - 7 条勾稽可在 `GtFormulaEditDialog` 以 `formula_type='logic_check'` 查看/编辑，编辑经后端持久化并参与后续勾稽执行（复用统一弹窗 + logic_check 收编端点）
    - _Requirements: 23.1, 23.2, 23.3, 23.4, 23.5_

  - [x]* 19.2 编写属性测试 P32（前端 logic_check 收编与纯函数降级等价）
    - **Property 32: 前端 logic_check 收编与纯函数降级等价**
    - **Validates: Requirements 23.1, 23.3, 23.5**
    - model-based/fast-check：随机报表数据下，消费后端 Issue_List 的逐条勾稽判定 == 纯函数 `computeCrossCheckResults`；且后端不可用触发降级时降级路径判定 == 在线路径
    - 文件：前端 `audit-platform/frontend/src/.../__tests__/useReportCrossCheck.spec.ts`（fast-check）

- [x] 20. 公式作用域过滤 + 全局公式管理页（Req 24）
  - [x] 20.1 前端作用域过滤 + 全局公式页
    - 公式弹窗按传入 `Formula_Scope` 只加载本作用域公式（复用 `FormulaManagerDialog.vue` 既有 `scope` prop + 7 类 `FormulaManagerScope`(note/consol_note/consol_worksheet/consol_report/report/tb/workpaper) + `SCOPE_LABEL_MAP` 中文标签 + 树形导航 selectedNodeKey/selectedPath，不新造并行作用域机制）；不展示/编辑其他 scope 公式
    - 全局公式管理页跨全部 7 类 scope 总览；按 `(scope, addr_id)` 键隔离缓存 `allRowsMap` 使各作用域互不串扰；过滤后来源地址用 ACNR full_resolve 规范名（与 Req 10/P14 一致）
    - _Requirements: 24.1, 24.2, 24.3, 24.4, 24.6_

  - [x] 20.2 后端公式列表按 scope 过滤端点
    - 后端按 `Formula_Scope` 过滤公式列表（供弹窗按域加载、全局页取并集）；一个 scope 的编辑仅改该 scope 列表，其余 scope 列表逐一不变
    - _Requirements: 24.1, 24.5_

  - [x]* 20.3 编写属性测试 P27（公式作用域隔离不串扰）
    - **Property 27: 公式作用域隔离不串扰**
    - **Validates: Requirements 24.1, 24.2, 24.3, 24.5**
    - Hypothesis：随机跨多 scope 公式全集 + 目标 scope → 弹窗加载集合恰等于该 scope 子集（无泄漏）；全局页展示==各 scope 并集；任两不同 scope 其一编辑不改另一列表
    - 文件：`backend/tests/formula_management/test_pbt_p27_scope_isolation.py`

- [x] 21. 公式三来源 reference 链 + formula_source 落库（Req 25｜部分补全）
  - [x] 21.1 reference 来源解析 + 源变更失效链
    - 说明：`formula_source` / `reference_formula_id` 列已在既有 V100 落地（`wp_formula` 扩 7 列）并已在 `workpaper_models.py:WpFormula` ORM 同步 → 本任务**不重复建列**，仅补 reference 解析与失效传播
    - `reference` 来源解析取被参照源公式 `expression`（复用而非重录，扩展 Req 9 复用）；被参照源公式变更时经 **ACNR 失效链**（复用 Req 13/25 失效机制，不自建）使引用方标失效并可重算；reference_formula_id 悬空按 fail-open 记 Issue/告警不静默产错值
    - _Requirements: 25.5, 25.7_

  - [x] 21.2 三来源前端选择流 + formula_source 落库回显
    - 前端公式来源选择：`preset`（从 `check_presets`/预设库一键套用）/ `custom`（自定义覆盖，复用 `wp_user_formulas` `is_preset_override` + restore/delete 端点 `/api/workpapers/{wpId}/user-formulas/{cell_key}` 恢复预设，不重写该端点）/ `reference`（参照已保存公式）
    - 每条公式记录 `formula_source` 落库，前端据此区分展示预设/自定义/参照
    - _Requirements: 25.1, 25.2, 25.3, 25.4, 25.6_

  - [x]* 21.3 编写属性测试 P28（公式来源往返保真）
    - **Property 28: 公式来源往返保真**
    - **Validates: Requirements 25.1, 25.6**
    - 文件：`backend/tests/formula_management/test_pbt_p28_source_roundtrip.py`

  - [x]* 21.4 编写属性测试 P29（自定义覆盖—恢复预设往返）
    - **Property 29: 自定义覆盖—恢复预设往返**
    - **Validates: Requirements 25.3, 25.4**
    - round-trip：restore(override(preset)) == preset 且 User_Formula 覆盖被删除
    - 文件：`backend/tests/formula_management/test_pbt_p29_restore_preset.py`

  - [x]* 21.5 编写属性测试 P30（参照复用与源变更失效传播）
    - **Property 30: 参照复用与源变更失效传播**
    - **Validates: Requirements 25.5, 25.7**
    - reference 引用方解析表达式==源公式表达式；源变更经 ACNR 失效链使引用方失效可重算
    - 文件：`backend/tests/formula_management/test_pbt_p30_reference_invalidation.py`

- [x] 22. 最终 Checkpoint - P0/P1 缺口全量测试
  - 运行 `python -m pytest backend/tests/formula_management/ -v --tb=short` 与 `npx vitest --run src/components/formula/`，确保新增 P22/P23/P24/P27/P28/P29/P30/P31/P32 及既有 P1–P21 全绿；如有疑问询问用户。

## Notes

- 标 `*` 的子任务为测试类（属性测试/单元/vitest），按约定仍会实现，UI 中可视化区分。
- 属性映射：已完成 P1–P21 一一映射到独立测试子任务（Task 1-13）；新增 P22–P32 映射到 Task 16-21（Req 19-25 缺口）。每个属性用单一 property-based 测试实现，测试文件按属性拆分以支持并行执行。
- Req 26/27-28/29-32 的复用/无属性说明：Req 26（三能力显式化）复用 P5/P6/P9/P14 + 示例测试不新增属性；Req 27-28（前端引擎治理/内核收口）对应 P19/P20/P21（Task 13.4）；Req 29-32（预设库/导入导出/模板库/入口）在 design 无 Correctness Property，用示例/单元测试（Task 14.7），不引用 design 属性号。
- design 属性现编号（P1-P32）权威：P22=勾选范围一致 / P23=空勾选阻断 / P24=范围动态发现 / P25=模块编辑权门禁 / P26=模块初稿隔离 / P27=作用域隔离 / P28=来源往返 / P29=恢复预设往返 / P30=参照失效传播 / P31=全局刷新生成范围隔离 / P32=前端 logic_check 收编等价。Task 里所有 Property N 与 Validates 引用已与 design 现文对齐，无旧号残留。
- PBT 后端用 Hypothesis（每属性 ≥100 迭代，CI 快跑档可调 max_examples 但覆盖不减），前端用 vitest / fast-check；full_resolve 与 ACNR 失效链用桩注入（含基础设施异常分支验证 fail-open）。
- 三层一致性：V100 迁移（Task 1.1，含既有 formula_source/reference_formula_id 列）+ ORM（Task 1.2）+ service（各引擎/编排任务）必须同步落地；Task 21 不重复建列，仅补 reference 解析/失效链。
...
- 分层门禁：全局刷新（Global_Refresh，`/draft-refresh`、`/refresh-scopes`）走合伙人门禁 `require_role(partner/signing_partner)`；模块/循环级刷新（Module_Refresh，如 `audit-sheet-refresh`）走编辑权门禁 `require_wp_edit_permission`（Req 22，已实现）。
- Checkpoint（Task 6/11/15/22）不纳入依赖图，仅作阶段性验证节点。
...
- Req 22（模块刷新编辑权门禁）已实现；其 design 属性 P25/P26 未纳入本次新增测试子任务（Req 22 标 ✅）；如后续补测可挂 Task 5.5 门禁族或单列子任务。
...
```json
{
  "waves": [
    ...
    { "id": 12, "tasks": ["14.7"] },
    { "id": 13, "tasks": ["17.1", "19.1", "20.2", "21.1"] },
    { "id": 14, "tasks": ["16.1", "20.1", "21.2"] },
    { "id": 15, "tasks": ["16.2", "17.2", "19.2", "20.3", "21.3", "21.4", "21.5"] },
    { "id": 16, "tasks": ["16.3", "16.4", "18.1"] },
    { "id": 17, "tasks": ["18.2"] }
  ]
}
```

依赖说明：Req 20 范围发现（17.1）为编排前置；Req 21 编排（16.1）依赖 17.1；`/draft-refresh` 改造（16.2）依赖 16.1；Req 19 弹窗（18.1）依赖 17.1 的发现端点 + 16.2 的编排入口；Req 23/24/25（19.x/20.x/21.x）相对独立可并行。已完成任务的 wave 0-12 保留不变。
