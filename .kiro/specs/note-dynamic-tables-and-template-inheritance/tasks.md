...
> **重心调整**：用户原话「主要是修复单体附注模块，合并附注模块内容是连带完成」「支持国企版和上市版的丝滑切换」
> 130 验收 / **36 人天** + 外部 5 人天 / **3 Phase / 16 Sprint**
...
- [ ] **A.3.1** REGION + WP 公式函数完善
- [ ] **A.3.2** PRIOR 跨年动态匹配
- [ ] **A.3.3** auto_trim_v2 三级裁剪（章节+表格+段落，CI-8）
- [ ] **A.3.4** 验收
...
## Sprint A.5：D14 国企↔上市丝滑切换（v0.6 新增⭐⭐⭐，2 人天）
...
- [ ] **C.5.2** memory.md 沉淀（13 维度铁律 + Phase 1/2/3 完成度）
- [ ] **C.5.3** ADR-011~ADR-021 全部撰写（v0.6 新增 ADR-021 国企↔上市切换） + INDEX.md
...
┌────────────────── Phase 3：高级特性 + 收尾（9 人天）─────────────────┐
│                                                                      │
│  C.1 (AI) → C.2 (版本图) → C.3 (前端全维度) → C.4 (Word) → C.5 (收尾) │
│                                                            ↓         │
│                                                    C.6 (综合 UAT)    │
└──────────────────────────────────────────────────────────────────────┘
```
...
| 36 人天工作量 | 延期 | 16 Sprint 拆分 + Phase 化分批上线 |
