...
> 版本：v0.5（草稿，2026-05-28 五轮扩展）
> 编写：2026-05-28
> 范围：动态表格 + 模板继承 + **合并附注完整开发** + 文字段落动态 + 协作锁 + AI + 跨年版本 + 集团基线 + **合并↔单体映射** + **章节序号动态** **13 维全栈增强**
> 前置：disclosure-note-full-revamp（已完成 46/47）
> 原则：**能不写死就不写死**，全部支持动态调整
> **v0.5 关键变更**：D8（合并附注衔接）从「衔接补丁」升级为「合并附注模块完整开发」，因合并附注当前**未真正消费子公司单体附注数据**，仅生成 7 个合并专用章节
