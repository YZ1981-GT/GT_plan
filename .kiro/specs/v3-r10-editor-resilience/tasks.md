...
- [x] 1.1.1 新建 `backend/app/workers/worker_helpers.py`
  - `async def write_heartbeat(worker_name: str)` 函数
  - Redis 不可用时降级（仅日志）
  - TTL=60s，value 为 JSON `{last_heartbeat, pid, version, hostname}`

- [x] 1.1.2 sla_worker 接入心跳
  - 主循环每轮 sleep 前调 `await write_heartbeat("sla_worker")`

- [x] 1.1.3 import_recover_worker 接入心跳

- [x] 1.1.4 outbox_replay_worker 接入心跳

- [x] 1.1.5 import_worker 接入心跳

- [x] 1.1.6 单测 `test_worker_heartbeat.py`
  - 4 用例：写入成功 / Redis 不可用降级 / TTL 设置 / payload 格式

### 1.2 后端 event_cascade_health 端点（1.5 天）

- [x] 1.2.1 新建 `backend/app/services/event_cascade_health_service.py`
...
- [x] 1.2.2 新建 `backend/app/routers/event_cascade_health.py`
  - `GET /api/projects/{project_id}/event-cascade/health` 端点
  - 普通用户只看 status / lag_seconds（D3）
  - admin/partner 看完整响应

- [x] 1.2.3 router_registry.py 注册（建议 §55）

- [x] 1.2.4 集成测试 `test_event_cascade_health.py`
...
- [x] 1.4.4 单测 `degraded_banner.spec.ts`
  - 3 用例：三档切换 / 普通用户不展开详情 / 独立 axios 实例不递归触发

### 1.5 Sprint 1 验收（0.5 天）

- [x] 1.5.1 手动模拟测试：kill 后端 → 30s 内显示 🔴
- [x] 1.5.2 手动模拟测试：worker miss → 60s 内显示 🟡
- [x] 1.5.3 vue-tsc 0 错误 + getDiagnostics 0 错误
- [x] 1.5.4 UAT-1/2/3/4/5 通过
...
- [x] 2.1.1 `audit-platform/frontend/src/utils/confirm.ts` 新增 `confirmSign`
...
- [x] 2.2.1 `LedgerDataManager.vue` 删除按钮接入 `confirmDangerous`
...
- [x] 2.3.1 `EqcrMemoEditor.vue` "定稿" 按钮接入 `confirmDangerous`
...
- [x] 2.4.0 grep 现有 5 组件签字逻辑列差异表（前置）
...
- [x] 2.4.1 SignatureLevel1.vue 接入 confirmSign
  - 签字按钮调 `await confirmSign("一级复核签字", { userName, projectName })`

- [x] 2.4.2 SignatureLevel2.vue 接入 confirmSign

- [x] 2.4.3 PartnerSignDecision.vue 接入 confirmSign
  - 注意：Spec A 已加 5 卡片项目状态摘要区块，本 spec 仅加签字按钮的 confirmSign 不动其他 UI（与 2.4.0 差异表对齐）

- [x] 2.4.4 EqcrApproval.vue 接入 confirmSign

- [x] 2.4.5 ArchiveSignature.vue 接入 confirmSign

- [x] 2.4.6 单测验证 5 个组件签字流程
...
- [x] 2.5.1 新建 `docs/WORKPAPER_SIDE_PANEL_GUIDE.md`
  - 10 Tab 各一节：用途 / 数据来源 / 与编辑器交互 / 已知限制
  - 含 1 张数据流图（Tab → API → eventBus → 编辑器）

- [x] 2.5.2 新建 `docs/EVENT_CASCADE_HEALTH_GUIDE.md`
  - 4 worker 职责
  - outbox + DLQ 工作机制
  - lag/stuck/dlq 告警阈值
  - 故障排查 cookbook（7 种常见场景）

- [x] 2.5.3 UAT-10 通过（新加入开发者读文档可上手）

### 2.6 F8 EQCR 备忘录版本对比（可选，1.5 天）

**实施前置条件**：Sprint 2 前 5 天工时完成且有余力，否则降级到 Spec D。

- [x] 2.6.1*（可选）新建 `eqcr_memo_versions` 表 + Alembic 迁移
  - down_revision 链接最新（届时实测）

- [x] 2.6.2*（可选）`EqcrMemoEditor.vue` 保存时同步 INSERT 版本

- [x] 2.6.3*（可选）"📜 版本对比" 按钮 + diff 抽屉
...
- [x] 2.7.1 vue-tsc 0 错误
- [x] 2.7.2 全量集成测试通过
- [x] 2.7.3 视觉抽样验收（设计师抽 5 个签字组件 + DegradedBanner 三档）
