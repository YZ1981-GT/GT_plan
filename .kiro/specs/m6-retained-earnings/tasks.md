...
- [ ] 0.1 openpyxl脚本读取M6未分配利润.xlsx全部9 sheet
...
- [ ]* 2.5 编写 Property P3 PBT：利润分配结转核心公式链
...
- [ ] 4.1 创建 M6TabIndex.vue 底稿目录
  - _Requirements: 1.2_

- [ ] 4.2 创建 M6TabAdjudication.vue 审定表M6-1
  - 权益类单区块+净利润转入/分配+TB回写
  - _Requirements: 2.1-2.7_

- [ ] 4.3 创建 M6TabDetail.vue 明细表M6-2（核心！）
  - 利润分配结转公式链逐行勾稽+前期差错调整+13公式+动态行+导入导出
  - _Requirements: 3.1-3.6_

- [ ] 4.4 创建 M6TabRetainedCheck.vue 检查表M6-4
  - 结转核对清单+M5/M1联动核对+结论区+AI辅助
  - _Requirements: 4.1-4.7, 5.1-5.2_

- [ ] 4.5 创建 M6TabAdjustment.vue + M6TabDisclosureListed/Soe.vue
  - 调整借贷平衡 + 附注上市/国企切换
  - _Requirements: 5.3-5.5_

### Phase 5: 后端

- [ ] 5.1 创建 m6_retained_earnings_renderer.py
  - RENDERER_DISPATCH注册 + 权益类公式验证
  - _Requirements: 1.6_

- [ ] 5.2 创建 m6_retained_earnings.py 路由
  - 导出模板/导出数据/导入数据 + 利润分配结转API
  - _Requirements: 6.6_

- [ ] 5.3 创建 m6_retained_earnings_service.py
  - 分配结转+M5/M1联动核对
...
- [ ] 7.1 单元测试：useM6FormulaEngine + useM6DistributionEngine
  - 权益类方向 + 分配结转公式链 + 联动差异
  - _Requirements: P1-P7_

- [ ] 7.2 集成测试：M6→M5/M1驱动 + 反向核对闭环
  - _Requirements: 4.1-4.7_
