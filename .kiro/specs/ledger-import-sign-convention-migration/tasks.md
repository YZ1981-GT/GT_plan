...
- [x] 4. 方向推导规则
  - [x] 4.1 显式方向列：按借/贷/D/C/debit/credit 调符号
  - [x] 4.2 借贷分列：按借方列减贷方列计算净额并记录 `split_columns`
  - [x] 4.3 借贷两方同时非零：按净额判定方向并记录 warning
  - [x] 4.4 单一净额列：按 Account_Category / Contra_Asset 推断
  - [x] 4.5 metadata 缺失时标记 `unknown`，不强行猜测
  - [x] 4.6 测试：负债贷方余额存负、资产借方余额存正、资产备抵为贷方
  - [x] 4.7 源分列字段保持绝对值，仅净额字段带符号
  - [x] 4.8 科目编码前缀低置信推断只用于提示，不用于自动迁移改写
  - _Requirements: 1.1, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8_

- [x] 5. 符号异常与复核
  - [x] 5.1 识别与 Account_Category 正常方向冲突的余额
  - [x] 5.2 识别借贷并存、资产备抵反向、负债权益收入借方净额
  - [x] 5.3 将异常写入 `sign_anomaly_flags`
  - [x] 5.4 提供方向异常列表 API
  - [x] 5.5 提供用户确认/修正方向 API，记录原因和留痕
  - _Requirements: 4.1, 4.2, 4.3, 4.4, 4.5_

- [x] 6. 历史数据 dry-run 与迁移
  - [x] 6.1 编写 dry-run 脚本，输出建议改写和待复核清单
  - [x] 6.2 dry-run 报告包含项目、dataset、科目、原金额、建议金额、原因、风险等级
  - [x] 6.3 dry-run 输出 `safe_auto_fix`、`manual_review_required`、`no_change` 三类安全等级
  - [x] 6.4 重复执行不改变已符合约定的数据
  - [x] 6.5 测试：冲突项默认不改写
  - [x] 6.6 执行脚本仅处理人工确认 allowlist 或 `safe_auto_fix` allowlist
  - [x] 6.7 dry-run 报告输出 JSON/CSV，作为审计留痕附件保存
  - [x] 6.8 测试：2221 等可能真实反向余额不能仅凭科目类别自动改写
  - _Requirements: 5.1, 5.2, 5.4, 5.5, 5.6, 5.7, 5.8, 5.9_

- [x] 7. 试算表展示去补救化
  - [x] 7.1 后端试算表 API 返回方向、方向来源、复核状态和异常 flags
  - [x] 7.2 `TrialBalance.vue#getDirection()` 优先使用后端权威方向
  - [x] 7.3 移除普通科目按金额正负作为第一方向来源的逻辑
  - [x] 7.4 本地 `directionOverrides` 改为调用后端 overlay 持久化接口
  - [x] 7.5 历史 fallback 显示 `legacy_inferred` / "推断方向"
  - [x] 7.6 提供方向异常列表 API 与批量确认 API
  - [x] 7.7 Vitest：方向来源优先级和用户覆盖持久化
