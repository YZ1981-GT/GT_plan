...
- [ ] 1. 扩展共享公式引擎 useD1FormulaEngine.ts
  - [ ] 1.1 在 `composables/useD1FormulaEngine.ts` 中新增贴息+业务模式纯函数
  ...
  - [ ]* 1.2 编写 Property 1 PBT：QA矩阵业务模式判定正确性
  ...
  - [ ]* 1.5 编写 Property 4 PBT：应计贴现利息公式正确性
  ...
  - [ ]* 1.8 编写 Property 12 PBT：贴息差异高亮判定
...
- [ ] 3. 实现 useD1BusinessMode.ts composable
  - [ ] 3.1 创建 `composables/useD1BusinessMode.ts`，实现业务模式分析D1-6核心逻辑
...
- [ ] 4. 实现 useD1MemoReconciliation.ts composable
  - [ ] 4.1 创建 `composables/useD1MemoReconciliation.ts`，实现备查簿核对D1-7核心逻辑
  ...
  - [ ]* 4.2 编写 Property 6 PBT：备查簿核对区差异
    - **Property 6: 备查簿核对区差异等于备查簿减明细账**
    - 生成器：自定义备查簿SUM值 + D1-2值生成器
    - 断言：差异行每列 === 备查簿合计 - 明细账值
    - **Validates: Requirements 5.1, 5.2**

  - [ ]* 4.3 编写 Property 8 PBT：动态行添加保持结构不变量
  ...
  - [ ]* 4.4 编写 Property 9 PBT：动态行序列化Round-Trip
  ...
  - [ ]* 4.5 编写 Property 11 PBT：SUMIFS统计正确性
...
- [ ] 5. 实现 useD1EndorsementDetail.ts composable
  - [ ] 5.1 创建 `composables/useD1EndorsementDetail.ts`，实现贴现背书明细D1-8逻辑
  ...
  - [ ]* 5.2 编写 Property 10 PBT：从备查簿导入状态筛选正确性
    - **Property 10: 从备查簿导入状态筛选正确性**
    - 生成器：`fc.array(MemoRow简化)` + `fc.oneof('已贴现','已背书')`
    - 断言：导入后结果行仅包含对应状态；数量等于源中该状态行数
    - **Validates: Requirements 17.1, 17.2**

- [ ] 6. 实现 useD1InterestCheck.ts composable
  - [ ] 6.1 创建 `composables/useD1InterestCheck.ts`，实现贴息检查D1-9逻辑
...
- [ ] 8. 实现 D1TabBusinessMode.vue 组件
  - [ ] 8.1 创建 `d1/D1TabBusinessMode.vue`（~250行），业务模式分析D1-6 HTML渲染
...
- [ ] 9. 实现 D1TabMemoReconciliation.vue 组件
  - [ ] 9.1 创建 `d1/D1TabMemoReconciliation.vue`（~400行），备查簿核对D1-7 HTML渲染
...
- [ ] 10. 实现 D1TabEndorsementDetail.vue 组件
  - [ ] 10.1 创建 `d1/D1TabEndorsementDetail.vue`（~350行），贴现背书明细D1-8 HTML渲染
...
- [ ] 11. 实现 D1TabInterestCheck.vue 组件
  - [ ] 11.1 创建 `d1/D1TabInterestCheck.vue`（~300行），贴息检查D1-9 HTML渲染
...
- [ ] 13. 修改主入口组件集成
  - [ ] 13.1 在 `GtD1NotesReceivable.vue` 中集成四个新子组件
  ...
  - [ ] 13.2 从 `useD1NotesReceivable.ts` 中删除已拆出的背书贴现逻辑
...
- [ ] 14. 实现后端导入导出端点
  - [ ] 14.1 创建后端导入导出路由（扩展已有 `_d1_import_export.py`）
