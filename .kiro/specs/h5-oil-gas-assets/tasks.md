...
- [ ] 0.1 openpyxl脚本读取H5油气资产.xlsx全部24 sheet
...
- [ ] 1.3 创建 useH5IndustryGuard.ts 行业适用性守卫
...
- [ ] 2.1 创建 `composables/useH5FormulaEngine.ts`
...
- [ ] 2.2 创建 `composables/useH5DepletionEngine.ts`
...
- [ ]* 2.6 编写 Property P4 PBT：单位产量法折耗（正常情况）
...
- [ ]* 2.7 编写 Property P5 PBT：储量为0时折耗=0
...
- [ ]* 2.12 编写 Property P10 PBT：租赁收益率
...
- [ ] 4.1 创建 H5TabIndex.vue 底稿目录
  - 24行sheet列表+进度条+GtIndexChip
  - _Requirements: 1.2_

- [ ] 4.2 创建 H5TabAdjudication.vue 审定表H5-1
  - 双区块(原值+折耗)+51公式+TB回写+三角勾稽
  - _Requirements: 2.1-2.8_

- [ ] 4.3 创建 H5TabDetail.vue + H5TabAdjustment.vue
  - 明细表54列3区段 + 调整分录10列借贷平衡
  - _Requirements: 3.1-3.5_

- [ ] 4.4 创建 H5TabIdleCheck.vue + H5TabPolicyCheck.vue + H5TabAnalysis.vue
  - 闲置11列 + CAS27政策段落型 + 分析表OO/HTML
  - _Requirements: 4.1-4.4_

- [ ] 4.5 创建 H5TabAdditionCheck.vue + H5TabDisposalCheck.vue
  - 增加24列(勘探资本化) + 减少24列(联动H10)
  - _Requirements: 5.1-5.5_

- [ ] 4.6 创建 H5TabStocktakePlan/Check/Summary.vue 监盘三阶段
  - 计划15列 + 检查14列 + 小结叙述式 + 数据流联动
  - _Requirements: 6.1-6.4_

- [ ] 4.7 创建 H5TabDepletionNoImpair/WithImpair.vue + H5TabDepletionAlloc.vue
  - 折耗分支选择器 + OO渲染 + 折耗分配11公式
  - _Requirements: 7.1-7.6_

- [ ] 4.8 创建 H5TabImpairment.vue + H5TabRecoverable.vue
  - 减值OO + 可收回金额DCF OO
  - _Requirements: 8.1-8.2_

- [ ] 4.9 创建 H5TabTitleCheck.vue + H5TabRelatedParty.vue
  - 权属21列(采矿权到期预警) + 关联16列(价差率高亮)
  - _Requirements: 8.3-8.4, 8.7_

- [ ] 4.10 创建 H5TabOperatingLease.vue + H5TabFinanceLease.vue
  - 经营租出17列(收益率) + 融资租出20列(本金摊销)
  - _Requirements: 8.5-8.6, 11.1-11.3_

- [ ] 4.11 创建 H5TabDisclosureListed.vue + H5TabDisclosureSoe.vue
  - 附注嵌套表 + EventBus subscribe
  - _Requirements: 1.2_

### Phase 5: 后端

- [ ] 5.1 创建 h5_oil_gas_assets_renderer.py
  - RENDERER_DISPATCH注册 + 行业校验 + 渲染逻辑
  - _Requirements: 1.6, 12.3_

- [ ] 5.2 创建 h5_oil_gas_assets.py 路由（4端点）
  - POST /h5/export-template | /h5/export-data | /h5/import-data
  - GET /h5/industry-check（校验当前项目行业适用性）
  - _Requirements: 3.3, 12.3_

- [ ] 5.3 创建 h5_oil_gas_assets_service.py
...
- [ ] 7.1 单元测试：useH5FormulaEngine + useH5DepletionEngine全部纯函数
  - 边界：零值/储量0/产量>储量/大数/NaN防护
  - _Requirements: P1-P10_

- [ ] 7.2 集成测试：跨sheet数据流 + 行业守卫
