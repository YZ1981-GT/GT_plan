...
- [ ] 2. 实现公式引擎 useF3FormulaEngine.ts
  ...
  - [ ]* 2.2 编写 Property 1 PBT：带息票据利息公式
...
- [ ] 3. 实现 useF3FormData.ts 基础数据加载/保存
  - [ ] 3.1 创建 `composables/useF3FormData.ts`
...
- [ ] 4. 实现审定表 + 附注披露 composable
  - [ ] 4.1 创建 `composables/useF3Adjudication.ts`
...
- [ ] 5. 实现明细表/调整分录/利息测算/逾期检查 composable
  - [ ] 5.1 创建 `composables/useF3Detail.ts`（F3-2明细表3区段逻辑）
  ...
  - [ ] 5.2 创建 `composables/useF3InterestCalc.ts`（F3-4利息测算逻辑）
  ...
  - [ ] 5.3 创建 `composables/useF3OverdueCheck.ts`（F3-5逾期检查逻辑）
  ...
  - [ ] 5.4 创建 `composables/useF3RelatedParty.ts`（F3-6关联方检查逻辑）
...
- [ ] 6. 实现 Vue子组件
  - [ ] 6.1 创建 `f3-notes-payable/F3TabAdjudication.vue`（F3-1审定表）
    - 调用useF3Adjudication
  ...
  - [ ] 6.2 创建 `f3-notes-payable/F3TabDetail.vue`（F3-2明细表 25列→3区段Tab）
    - 调用useF3Detail
  ...
  - [ ] 6.3 创建 `f3-notes-payable/F3TabInterestCalc.vue`（F3-4利息测算）
    - 调用useF3InterestCalc
  ...
  - [ ] 6.4 创建 `f3-notes-payable/F3TabOverdueCheck.vue` + `F3TabRelatedParty.vue`
    - F3-5逾期检查：15列 + 风险高亮 + 底部汇总 + 动态行增删 + 导入导出 + AI
    - F3-6关联方检查：16列 + 集中度>30%橙色 + 底部汇总 + 动态行增删 + 导入导出 + AI
    - _Requirements: 8.1~8.7, 9.1~9.6_

  - [ ] 6.5 创建 `f3-notes-payable/F3TabVoucherCheck.vue`（F3-7检查表 借方/贷方区块）
...
- [ ] 8. 实现后端 + 导入导出 + AI
  ...
  - [ ] 8.2 创建 `composables/useF3ImportExport.ts` + `composables/useF3DualMode.ts`
