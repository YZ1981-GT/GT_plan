...
- [ ] 37. I4 长期待摊费用（GtI4LongTermPrepaid，i4/，10 sheet）
  - [ ] 37.1 打磨 i4/：Adjudication`01110`(补OA) PolicyCheck/TargetedCheck `01010` amortization/adjustment/detail/disclosures/index `01000`。补 OA/AN/AC/TT + 摊销列。
    - _Requirements: 1.1, 1.2, 2.1, 3.1, 4.1, 5.1_

- [ ] 38. I5 其他非流动资产（GtI5OtherNoncurrentAssets，i5/，7 sheet）
  - [ ] 38.1 打磨 i5/：Adjudication`01110`(补OA) TargetedCheck`01010` rest `01000`。补 OA/AN/AC/TT。
    - _Requirements: 1.1, 1.2, 2.1, 3.1, 4.1, 5.1_

- [ ] 39. I6 研发费用（GtI6ResearchDevelopmentExpense，i6/，9 sheet）
  - [ ] 39.1 打磨 i6/：Adjudication`01110`(补OA) TargetedCheck`01010` adjustment/detail/disclosures/index/cutoff `01000`。补 OA/AN/AC/TT + 研发费用归集/加计扣除列（结合研发特点）。
    - _Requirements: 1.1, 1.2, 2.1, 3.1, 3.3, 4.1, 5.1_

- [ ] 40. I 循环 Playwright 实测
  - [ ] 40.1 admin/admin123 → 导航 I 无形资产底稿 → 抽验 I1 明细/摊销 + I6 研发费用（或 I2 补全后）≥2 sheet：六项渲染 + console 0 error；`check_fghi_sheet_completion.py` I 循环段通过。
    - _Requirements: 8.1, 8.2, 8.3_

- [ ] 41. 全局最终验证
  - [ ] 41.1 `check_fghi_sheet_completion.py --strict` 全 F/G/H/I 通过（P1/P2/P6/P7）；`check_wp_ref_contract.py --strict` exit 0（P3）；`fix_wp_composables_import_depth.py --check` 通过（P4）；全改动 .vue 无 U+FFFD（P8）。
    - _Requirements: 6.2, 6.3, 6.5, 9.1_
  - [ ] 41.2 前端 vitest + fast-check 持久化往返幂等（P5，F/G/H/I 各 1 代表 sheet，numRuns=20）；全改动文件 get_diagnostics 零错误汇总。
