...
- [ ] 1.1 重写 `_k2_other_current_assets.py` 取数部分：声明 `K2_ACCOUNT_SPEC`
...
- [ ] 1.2 `_build_adjudication_prefill` 改纯函数（入参 leaves + accounts）：按原值侧
...
- [ ] 1.3 新建 `backend/tests/four_table/test_k2_account_scope.py`：
...
- [ ] 2.1 把 K1 的 `K1FourTableSourcePanel.vue` 提升为共用
  `workpaper/shared/WpFourTableSourcePanel.vue`（props: `sourceCodes` + `hints`），
  K1 侧改薄壳委托（零回归由 K1 的 111 例守卫证明），K2 侧复用。
  - Requirements: 1.7
  - Properties: 11

- [ ] 2.2 `GtK2OtherCurrentAssets.vue` 透传 `tbSourceCodes`；`K2TabAdjudication.vue`
  顶部挂溯源面板；`tbData` 口径注释更新（不再是 1231）。
  - Requirements: 1.7
  - Properties: 1

- [ ] 3.1 新建**平台级共享件** `composables/shared/dynamicAdjudicationRows.ts`
...
- [ ] 3.2 改造 `useK2Adjudication.ts`：行集由 `K2-1-rows` 动态清单驱动；
...
- [ ] 3.3 `K2TabAdjudication.vue`：行名可编辑（`el-input` `@input` 回写）、
...
- [ ] 4.3 端到端实测（chrome-devtools + postgres 只读，项目 `0ec33ac9` / wp `919e3387`、
...
- [ ] 4.4 收口：后端 four_table + K2 全量绿；前端 K2 相关全量绿；
