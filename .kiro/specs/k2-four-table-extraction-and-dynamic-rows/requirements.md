...
1. **与源模板不符**。源 xlsx `审定表K2-1` A7:A13 是
   `待摊费用 / 待抵扣进项税 / 房租物业费 / 预缴企业所得税 / 委托贷款 / 预缴其他税费 / 应收退货成本`。
2. **混入其它报表行已占用的项目** —— `预付款项`（`BS-008`，F1 循环）、`合同资产`（`BS-010` 附近，
   D6 循环）、`押金保证金`（K1 其他应收款的款项性质桶）。放在其他流动资产里会**重复计入资产**。
...
2.5 WHEN 迁移历史数据 THEN 属于其它报表行的三个旧行（`预付款项` / `合同资产` /
`押金保证金`）SHALL 保留其已录金额但在 UI 标注「疑似属于其它报表项目」警示，
由审计师决定删除 —— SHALL NOT 静默删除用户数据。

2.6 WHEN 从四表库 seed 行 THEN 系统 SHALL 按实际叶子科目名建行（宁缺勿造：
解析不出科目时不建行、不塞「其他」兜底行），且仅在无持久化行时生效。
