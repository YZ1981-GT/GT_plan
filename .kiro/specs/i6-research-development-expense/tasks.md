...
- [x] 0.1 openpyxl脚本读取I6研发费用.xlsx全部11 sheet
  - 确认损益类取数规则 + 月度12列结构
...
- [x] 2.1 创建 useI6FormulaEngine.ts（损益类！）
...
- [x]* 2.3 编写 Property P2 PBT：损益类净发生额（借-贷）
...
- [x]* 2.4 编写 Property P3 PBT：月度合计=SUM(12月)
...
- [x]* 2.5 编写 Property P4 PBT：VR-I6-01校验
...
- [x] 3.1 创建 useI6FormData.ts（selfLoad + writebackTB **发生额** 6602）
  - 注意：TB回写使用发生额而非期末余额！（与H10同款）
- [x] 3.2 创建 useI6CrossSheet.ts（I2双向联动 + VR-I6-01校验 + 月度聚合）
- [x] 3.3 创建 useI6Adjudication.ts（损益类73公式 + 联动面板）
- [x] 3.4 创建 useI6Detail.ts（月度12列横向矩阵 + 趋势图数据）
- [x] 3.5 创建 useI6Cutoff.ts（截止双向 + autoSampling）
- [x] 3.6 创建 useI6Disclosure.ts + useI6ImportExport.ts + useI6DualMode.ts

### Phase 4: Vue子组件

- [x] 4.1 创建 I6TabIndex.vue
- [x] 4.2 创建 I6TabAdjudication.vue（73公式！损益类+I2联动面板）
- [x] 4.3 创建 I6TabDetail.vue（月度12列横向65列+趋势图）
- [x] 4.4 创建 I6TabAdjustment.vue
- [x] 4.5 创建 I6TabTargetedCheck.vue
- [x] 4.6 创建 I6TabCutoffForward.vue + I6TabCutoffBackward.vue
- [x] 4.7 创建 I6TabDisclosureListed.vue + I6TabDisclosureSoe.vue

### Phase 5: 后端

- [x] 5.1 创建 _i6_research_development_expense.py render策略 + RENDERER_DISPATCH
  - 注意：损益类取数逻辑（tb_ledger发生额）
- [x] 5.2 创建 _i6_import_export.py 导入导出3端点
- [x] 5.3 创建 _i6_ai_generate.py AI生成
- [x] 5.4 更新 wp_render_schema: i6-research-development-expense.yaml

### Phase 6: 集成联动

- [x] 6.1 EventBus: TB回写(**发生额**6602) + substantive:adjudicated
- [x] 6.2 EventBus: I6↔I2双向联动
  - I6 publish 'research:expense-updated'
  - I6 subscribe 'development:capitalized-updated'
  - VR-I6-01实时校验
- [x] 6.3 cross_wp_references: I6↔I2双向引用
- [x] 6.4 GtIndexChip: I6→I2 / I2→I6双向跳转
- [x] 6.5 截止测试useCutoffAutoSampling集成
- [x] 6.6 附注EventBus + 双模式OO

### Phase 7: 测试验收

- [x] 7.1 Vitest: useI6FormulaEngine（损益类公式）
- [x] 7.2 Vitest组件: sheetName分发 + I2联动面板
- [x] 7.3 后端pytest: render(损益取数)+导入导出
- [x] 7.4 Playwright E2E: I6保存→I2联动→VR-I6-01校验全链路
- [x] 7.5 Playwright E2E: 月度明细横向滚动+趋势图
