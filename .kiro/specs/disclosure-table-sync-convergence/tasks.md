...
- [ ] 14. Playwright 端到端实测
  - [ ] 14.1 上市版披露表录入 → 推送 → 附注模块可见完整表格（中文列头 + 合计行）→ Word 导出含表；国企版同验（不同 note_section 各自正确）。
  - [ ] 14.2 二次空载荷同步 → 附注表格不丢失；`fetchDetailFresh` 后不命中旧缓存。
  - [ ] 14.3 未接入 `_columns` 的历史记录 → 降级呈现不 crash、不清空（Req7.2）。
...
- [x] 15. Coverage_Ledger 全绿 + 文档
  ...
  - [ ] 15.2 更新 `.kiro/specs/INDEX.md`；开发指南记录"披露表列头随 `_columns` 携带 + 后端单点投影"范式（可选收尾）。
