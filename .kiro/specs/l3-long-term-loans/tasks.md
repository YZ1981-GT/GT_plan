...
科目：2501长期借款（贷方/负债类）
公式特征：**负债类贷方**期末=期初+贷方-借方；利息=本金×利率×天数/360；一年内到期重分类；L3→L2/L8联动

## Tasks

### Phase 0: 双源输入验证

- [ ] 0.1 openpyxl脚本读取L3长期借款.xlsx全部14 sheet
...
- [ ] 2.1 创建 `useL3FormulaEngine.ts`（负债类！）
...
- [ ] 4.1 创建 L3TabIndex.vue 底稿目录
  - 14行+进度条
  - _Requirements: 1.2_

- [ ] 4.2 创建 L3TabAdjudication.vue 审定表L3-1
  - 负债类单区块+分类小计+一年内到期列+TB回写
  - _Requirements: 2.1-2.7_

- [ ] 4.3 创建 L3TabDetail.vue 明细表L3-2
  - 32列区段Tab切换+到期分类+动态行+导入导出
  - _Requirements: 3.1-3.6_

- [ ] 4.4 创建 L3TabInterestCalc.vue 利息测算表（核心！）
  - 利息测算+差异高亮+按合同筛选+L2/L8联动publish
  - _Requirements: 4.1-4.6_

- [ ] 4.5 创建 L3TabCreditCheck.vue + L3TabOverdueCheck.vue + L3TabPledgeCheck.vue
  - 征信核对+逾期检查(天数高亮)+抵质押(担保比例)
  - _Requirements: 6.1-6.5, 7.1-7.5_

- [ ] 4.6 创建 L3TabContractCheck.vue + L3TabLtLoanCheck.vue
  - 合同检查区段Tab+OCR + 检查表结论区
  - _Requirements: 8.1-8.4_

- [ ] 4.7 创建 L3TabAdjustment.vue + L3TabDisclosureListed/Soe.vue
  - 调整借贷平衡(含重分类RJE) + 附注上市/国企切换
  - _Requirements: 5.2, 9.1-9.3_

### Phase 5: 后端

- [ ] 5.1 创建 l3_long_term_loans_renderer.py
  - RENDERER_DISPATCH注册 + 负债类公式验证
  - _Requirements: 1.6_

- [ ] 5.2 创建 l3_long_term_loans.py 路由
  - 导出模板/导出数据/导入数据 + 利息测算+重分类API
  - _Requirements: 4.1, 5.2, 11.3_

- [ ] 5.3 创建 l3_long_term_loans_service.py
...
- [ ] 7.1 单元测试：useL3FormulaEngine + useL3InterestEngine + useL3ReclassEngine
  - 负债类方向 + 利息测算 + 重分类边界
  - _Requirements: P1-P8_

- [ ] 7.2 集成测试：L3→L2/L8联动 + 征信核对 + 逾期 + 重分类
