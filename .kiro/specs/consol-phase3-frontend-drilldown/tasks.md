...
- [ ] 0. 实证基线
  - [ ] 0.1 grep 确认 `ConsolBreakdownDialog.vue` 不存在 + `disclosure_notes` 无 source_project_id/consolidation_breakdown 字段
  - [ ] 0.2 readCode Phase 2 `report/consol-breakdown` 端点返回结构（穿透组件 report source 对接）
  - [ ] 0.3 readCode `generate_full_consol_notes` V2 汇总逻辑（确认在哪写 provenance）
  - [ ] 0.4 grep 合并报表/附注视图（ReportView/DisclosureEditor/ConsolNoteTab）右键菜单接入点
  - [ ] 0.5 readCode wizard report_scope 流程 + ConsolidationIndex 树渲染 + MiddleProjectList，作双向导航/自动建树接入点
  - [ ] 0.6 确认 DefaultLayout initGlobalBackspace 返回栈接入方式
...
- [ ] 1. provenance 字段 + V2 写入 + 端点
  - [ ] 1.1 迁移 V0XX：`disclosure_notes` 加 `source_project_id UUID` + `consolidation_breakdown JSONB` + GIN 索引 + 配套 R 回滚
  - [ ] 1.2 ORM `DisclosureNote` 同步加 `Mapped` 字段（三层一致，关联 T6）
  - [ ] 1.3 V2 `generate_full_consol_notes` 汇总每章节时写 `consolidation_breakdown` provenance（关联 T2）
  - [ ] 1.4 新建 `note_consol_drilldown_service` + `GET notes/{project_id}/{year}/{section_id}/consol-breakdown`（挂现有 consol_notes router 或登记）
  - [ ] 1.5 无 breakdown 返回空 + "请先用 V2 生成合并附注"（关联 EH1/EH3）
...
- [ ] 2. ConsolBreakdownDialog.vue
  - [ ] 2.1 新建组件（props.source=report|note，渲染子公司金额+占比+抵销+合并数，GtAmountCell）
  - [ ] 2.2 source=report 调 Phase 2 端点，source=note 调 Phase 3 端点（渲染契约统一，关联 T1）
  - [ ] 2.3 点子公司行 emit jump → 跳单体报表/附注 + 纳入 Backspace 返回栈（关联 T3）
  - [ ] 2.4 跳转前权限校验，无权 ElMessage 不跳（关联 EH2）
  - _需求：1.1~1.6_ _属性：T1/T3_ _铁律：UI 全中文化 / 穿透组件统一复用_

- [ ] 3. 右键菜单接入
  - [ ] 3.1 合并报表视图行右键加"查看合并明细"→ Dialog(source=report)
  - [ ] 3.2 DisclosureEditor/ConsolNoteTab 章节右键加"查看合并明细"→ Dialog(source=note)
  - [ ] 3.3 grep 全部相关视图逐一覆盖（关联 R5）
...
- [ ] 4. 单体↔合并跳转 + 锁定标签
  - [ ] 4.1 单体项目 header：parent_project_id 非空显示"所属集团：{名}"链接 → 跳合并项目
  - [ ] 4.2 ConsolidationIndex 树节点加"进入项目"按钮 → 路由单体项目
  - [ ] 4.3 合并项目列表锁定子公司显示"🔒 已锁定"标签（复用 Phase 1）
  - [ ] 4.4 所有跳转纳入 Backspace 返回栈（关联 T3）
...
- [ ] 5. wizard 配置合并范围 + scope 事件
  - [ ] 5.1 wizard report_scope=consolidated 完成弹"配置合并范围"步骤（选单体挂子公司）
  - [ ] 5.2 EventType 加 `CONSOL_SCOPE_CHANGED`；consol_scope 增删发事件 → 失效/重建树缓存
  - [ ] 5.3 前端 ConsolidationIndex 监听事件自动刷新树
  - [ ] 5.4 手动"刷新树"按钮兜底（事件丢失，关联 EH4）
  - [ ] 5.5 wizard 改动仅影响 consolidated 项目，回归测试非合并流程不变（关联 R3）
...
- [ ] 6. check_subsidiary_completeness
  ...
  - [ ] 6.2 不全 warning 不阻断刷新（关联 T5）
  - [ ] 6.3 子公司过多异步 + 超时降级（关联 EH5）
...
- [ ] 6A. stale SSE 推送 + 前端感知
  - [ ] 6A.1 `consol_note_stale_handler` 标记母项目 stale 时发 SSE 事件
  - [ ] 6A.2 合并页通过 SSE 感知 → 提示「子公司数据已更新，建议重新汇总」（warning 不阻断）
  - [ ] 6A.3 提示内含"立即重新汇总"快捷入口 → 跳 Phase 2 一键刷新
  - [ ] 6A.4 复用既有 SSE 基础设施（不新增轮询打爆 pool，关联 Phase 2 R5）
...
- [ ] 7. vitest + PBT + 集成
  - [ ] 7.1 T1 ConsolBreakdownDialog 对 report/note 渲染契约一致（vitest props 切换）
  - [ ] 7.2 T2 附注 provenance 自洽：Σ by_company amount == 章节汇总值（hypothesis）
  - [ ] 7.3 T4 scope 变更触发重建树（集成测试）
  - [ ] 7.4 T5 完整度校验不阻断（hypothesis：数据不全 warnings 非空但刷新执行）
  - [ ] 7.5 T6 disclosure_notes 新字段 drift 0（集成测试）
...
- [ ] 10. 文档与沉淀
  - [ ] 10.1 ADR-CONSOL-301/302/303/304 落地 `docs/adr/`（注册前查编号防冲突）
