...
- [x] 1. 扩展 useD1FormulaEngine.ts 公式函数
  - [x] 1.1 在 `useD1FormulaEngine.ts` 中新增 `safeDivide` 纯函数
  ...
  - [x]* 1.2 编写 Property 10 PBT：safeDivide 安全除法
...
- [x] 2. 实现 useD1Disclosure.ts composable 核心逻辑
  ...
  - [x] 2.8 实现国企专有票据分类子节逻辑
    - 实现 `categorySummaryRows` computed（从 crossSheetData 映射）
    - 实现 `categorySummaryTotal` computed（SUM）
    - 实现账面价值计算（余额 - 坏账准备）
    - _Requirements: 10.1, 10.2, 10.3, 10.4, 10.5, 10.6_

- [x] 3. Checkpoint - composable 核心逻辑验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. 编写 composable PBT 测试
  - [x]* 4.1 编写 Property 3 PBT：账面价值等于余额减坏账
  ...
  - [x]* 4.2 编写 Property 4 PBT：合计行恒等于明细行之和
  ...
  - [x]* 4.5 编写 Property 7 PBT：跨Sheet取数响应式一致性
...
- [x] 5. Checkpoint - PBT 验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. 实现 D1TabDisclosure.vue 组件
  - [x] 6.1 创建 `d1/D1TabDisclosure.vue`（~350行），附注披露 HTML 渲染
  ...
  - [x] 6.3 实现背书贴现子节 el-table 渲染
    - 列：票据种类 | 终止确认金额 | 未终止确认金额
    - 固定行 + 动态行 + 合计行
    - CAS23 提示文字（el-alert type="info"，表格下方）
    - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5, 3.6_

  - [x] 6.4 实现转应收账款子节 el-table 渲染
    - 列：票据种类 | 转应收账款金额
    - 固定行 + 合计行
    - _Requirements: 4.1, 4.2, 4.3, 4.4_

  - [x] 6.5 实现坏账分类子节渲染（最复杂子节）
  ...
  - [x] 6.8 实现国企票据分类子节渲染（v-if="variant==='soe'"）
    - 列：票据种类|期末余额|期末坏账|期末账面价值|期初余额|期初坏账|期初账面价值
    - 浅蓝色背景标记自动取数单元格 + tooltip
    - 合计行
    - _Requirements: 10.1, 10.2, 10.3, 10.4, 10.5, 10.6_

- [x] 7. Checkpoint - Vue 组件验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 8. 集成到主入口 GtD1NotesReceivable.vue
  - [x] 8.1 在 `GtD1NotesReceivable.vue` 中新增两个 tab-pane 引用 D1TabDisclosure
  ...
  - [x] 8.2 从 `useD1NotesReceivable.ts` 中删除附注披露相关旧代码
...
- [x] 9. 实现双模式切换
  - [x] 9.1 在 D1TabDisclosure.vue 中实现 HTML ↔ OnlyOffice 双模式
...
- [x] 9.5 实现提示文字展示
  - [x] 9.5.1 在 useD1Disclosure.ts 中定义 DISCLOSURE_GUIDANCE 常量
    - 顶部说明提示（云信/融信数字化债权）
  ...
  - [x] 9.5.2 在 D1TabDisclosure.vue 中实现提示折叠区域
    - 各子节对应位置插入 `<details>` 折叠区域（只读提示）
    - 蓝色左边线 + 浅蓝背景 + 默认收起
    - "可无限量添加行"位置显示浅灰色占位提示行
    - _Requirements: 17.4, 17.5_

- [x] 9.5.3 实现"说明"文本区域与附注模块双向回写
  - [x] 9.5.3.1 在各子节"说明："标签处渲染可编辑 textarea（区别于不可编辑的提示折叠区）
...
- [x] 9.6 实现联动跳转
  - [x] 9.6.1 在 D1TabDisclosure.vue 中添加 GtIndexChip 跳转
...
- [x] 9.7 实现行类型标识与导入规则
  - [x] 9.7.1 确保所有子节 row 数据包含 rowType 字段
    - fixed 行左侧显示锁定图标
    - dynamic 行左侧显示删除按钮
    - summary 行不可编辑（底部固定）
...
- [x] 10. 实现后端导入导出
  - [x] 10.1 创建 `backend/app/routers/wp_render_strategies/_d1_disclosure_export.py`（~150行）
  ...
  - [x] 10.2 在 D1TabDisclosure.vue 中实现导入导出 UI
...
- [x] 11. 实现持久化与自动保存
  - [x] 11.1 确保 useD1Disclosure 的持久化逻辑完整
...
- [x] 12. Checkpoint - 全功能集成验证
  - Ensure all tests pass, ask the user if questions arise.

- [x] 13. 回归测试
  - [x] 13.1 运行现有 D1 测试套件确保无回归
