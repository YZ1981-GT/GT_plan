...
| 场景 | 策略 |
|------|------|
| 穿透面板打开 | 实时查询，不缓存（金额可能刚变）|
| 影响范围查询 | 缓存 30s，stale 事件主动失效 |
| 签发一致性清单 | 每次打开签发页重新计算，不走缓存 |
| 前端 badge | 由 SSE 事件驱动更新，不轮询 |
| LinkageContract 列表 | 项目维度 LRU 缓存 60s，年度切换时全量失效 |

> 原则：金额相关查询宁可慢不可脏；状态 badge 走事件推送；签发清单必须实时。
