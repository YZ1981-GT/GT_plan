...
- [ ] 0.1 openpyxl脚本读取H4工程物资.xlsx全部13 sheet
...
- [ ] 2.1 创建 `composables/useH4FormulaEngine.ts`，实现全部纯函数
...
- [ ] 3.3 创建 useH4DualMode.ts + useH4ImportExport.ts
...
- [ ] 4.1 创建 H4TabIndex.vue 底稿目录
  - 13行sheet列表+进度条+GtIndexChip跳转
  - _Requirements: 1.2_

- [ ] 4.2 创建 H4TabAdjudication.vue 审定表H4-1
  - 51公式自动计算+TB取数+回写+三角勾稽+跨sheet校验
  - _Requirements: 2.1-2.10_

- [ ] 4.3 创建 H4TabDetail.vue 明细表H4-2
  - 67列→3区段Tab + 动态行 + 合计行 + 导入导出
  - _Requirements: 3.1-3.9_

- [ ] 4.4 创建 H4TabAdjustment.vue 调整分录H4-3
  - 10列 + 借贷平衡 + EventBus + 导入导出
  - _Requirements: 4.1-4.6_

- [ ] 4.5 创建 H4TabAdditionCheck.vue 增加检查H4-4
  - 19列 + 抽凭 + OCR + 差异计算 + 统计摘要
  - _Requirements: 5.1-5.7_

- [ ] 4.6 创建 H4TabDisposalCheck.vue 减少检查H4-5
  - 29列2区块 + 联动H2 + GtIndexChip + 抽凭
  - _Requirements: 6.1-6.6_

- [ ] 4.7 创建 H4TabStocktakeCheck.vue 盘点检查H4-6
  - 12列 + 差异自动计算 + 高亮
  - _Requirements: 7.1-7.3_

- [ ] 4.8 创建 H4TabImpairment.vue + H4TabRecoverable.vue
  - OnlyOffice为主渲染 + HTML摘要视图
  - _Requirements: 7.4-7.6_

- [ ] 4.9 创建 H4TabRelatedParty.vue 关联交易H4-9
  - 16列 + 价差率自动计算 + 异常高亮 + 统计摘要
  - _Requirements: 8.1-8.5_

- [ ] 4.10 创建 H4TabDisclosureListed.vue + H4TabDisclosureSoe.vue
  - 附注嵌套表 + EventBus subscribe
  - _Requirements: 1.2_

### Phase 5: 后端

- [ ] 5.1 创建 h4_engineering_materials_renderer.py
  - RENDERER_DISPATCH注册'h4-engineering-materials'
  - render逻辑：读取checklist_responses + TB数据 + 跨sheet合计
  - _Requirements: 1.6_

- [ ] 5.2 创建 h4_engineering_materials.py 路由（3端点）
...
- [ ] 7.1 单元测试：useH4FormulaEngine全部纯函数
