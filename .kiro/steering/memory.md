...
- **✅ bulk_copy_staged tenant_id 修复（2026-06-08）**：COPY 跳过 server_default 导致 `tenant_id` NULL→NOT NULL violation→fallback INSERT 95k 行超时心跳丢失；修复=NOT NULL 兜底逻辑加 `if idx_tenant and not row_list[idx_tenant]: row_list[idx_tenant] = "default"`
