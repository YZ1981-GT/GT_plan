...
- **✅ workpaper-account-multifile-aggregation 全部完成（5 Sprint/全任务/7验收门槛 V1-V7 全勾）**：科目底稿多文件聚合(D2=14sheet/D4=14sheet)+前端 HTML 格式选型+同名内容哈希合并+底稿间联动(审定→TB回写打通)+行业维度+自定义模板导出/导入(V090 表)。新增 4 service+2 端点+V090 迁移+5 测试文件(90 测试)，全程零回归。**待 commit**### ✅ 编制指导面板修复（2026-06-21，3 项，30 guidance 测试全绿）
