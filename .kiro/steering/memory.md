...
- **Active spec = 9**（并发会话 2026-07-30 新建 `d1-notes-receivable-disclosure-alignment` 26 任务 / `n1-deferred-tax-disclosure-template-alignment` 53 任务）。**🔴 「单 commit」按 spec 切分不成立 → 已定为按层分批**（2026-07-30 用户确认）：实测 411 改动项里 f2 专属仅 8 个 / columns-coverage 专属 9 个，但核心交付都落在 `note_template_{listed,soe}.json`·`disclosure_engine.py`·`note_sub_table_projector.py` 这些**被 8 个在飞 spec 同时改**的共享真源上；仓库历史本身也是按层分批。**已提交 8 个 commit 覆盖 352 文件**（未 push）：`21cdd06a` feat(backend)42 / `f5debfa4` data(note-templates)4 / `4678ec3b` test(backend)31 / `02f122b8` feat(frontend/workpaper)179 / `9a9cf12e` feat(frontend)16 / `a2a4feb2` test(frontend)63 / `1c3241af` ci1 / `030d2535` docs16。排除密钥类 0 项、产物/临时 59 项（并发会话的根目录 `tmp_*` 仍未跟踪）。**本机 pre-commit 钩子未安装**（`.git/hooks` 只有 `pre-push`；`file_size_whitelist.txt` 既未跟踪也不在盘上，故行数门禁本地必失败 137 项，只在 CI 生效）。以下 spec 状态：`d2-ar-disclosure-soe-alignment`（**实现 25/25 完成**：国企 八、5 底稿 6 处结构欠账 + 附注 11→13 表 + 附带修掉核销表假父表头、国企多推的「1年以内小计」行；**遗留 2 项待用户定口径**见下）/
  ...
  `f2-inventory-disclosure-template-alignment`（**✅ 全部完成并已提交**，含多区块导入导出；前端 95 + 后端 123 绿）/
