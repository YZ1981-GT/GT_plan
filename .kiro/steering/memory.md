...
- 全局重置跨组件通信（2026-04-25，commit b5bab49）：顶部栏重置成功后dispatch window CustomEvent 'gt-import-reset'，AccountImportStep和LedgerPenetration监听该事件立即清空所有导入状态（importing/previewing/phase/弹窗等），loading遮罩消失，界面恢复可操作
- 用户计划对每个细分程序逐一打磨升级
