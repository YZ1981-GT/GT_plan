...
- **全局联动架构改进方案已输出（2026-05-17，commit 34ab8e0）**：`docs/GLOBAL_LINKAGE_ARCHITECTURE_PROPOSAL.md`；核心方案 = Unified Linkage Bus（统一联动总线）+ 统一 URI 寻址 `{module}:{code}:{sheet_name}:{label}` + 5 数据源合并为统一依赖图（~43000 边）+ 7 变更触发点统一入口 + Stale Propagation Engine 替代两套并存机制 + 地址解析三层优先级（override→header_rules→启发式）+ 反向索引（FormulaReverseIndex）+ **公式管理全局穿透 UI 三向可达**（全局→具体/具体→公式/单元格→详情）；5 Sprint / 12 天；8 项量化验收标准；兼容策略 = 保留 event_handlers + 追加统一引擎调用
- **公式管理全局穿透 UI 设计（Sprint 5，3 天）**：3 个新 API（formula-usage/formulas-for/cell-detail）+ 3 个新组件（FormulaUsagePanel/DocumentFormulaList/CellFormulaDetail）+ 5 模块右键菜单统一接入（WorkpaperEditor/ReportView/DisclosureEditor/TrialBalance/Adjustments）+ 公式管理中心增强（按引用方分组/健康度卡片/搜索/批量操作）
