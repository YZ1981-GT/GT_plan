...
- 启动跑 `backend/migrations/V*.sql`；新加列写 `V0XX__*.sql`+`R0XX__*.sql` 配对，CREATE/ALTER 必 `IF NOT EXISTS`；按 version **数字**去重（撞号字母序靠后者静默丢失，scan_migrations 已加同号检测抛 RuntimeError）；**当前最高 V082**（V073=editing_lock迁移 / V074=disclosure_guidance / V075=drop_workpaper_editing_locks / V076=field_overrides / V077=business_category / V078=adjustments_passed_reason / V079=cf_verification / V080=adjustment_type_cf / V081=review_records_signing / V082=review_conversations_checklist_ref）
...
### git 状态（2026-06-16）
- 分支 `work/2026-05-30-wp-specs`，最高迁移 **V082**；HEAD `18b259d3`（group-tree-architecture Phase1+2 全量实现，2026-06-16 push）
