...
- **声明式映射规则引擎完成（2026-05-17，commit 806ef08）**：`sheet_mapping_rules.json` 4 级匹配（exact_rules→custom_rules→naming_patterns 45 条→category_defaults）+ `new_workpaper_defaults` 8 种底稿模板支持用户自定义新增；`step_sheet_mapping.json` V3 = 947/1040 步成功映射（91.1%）；`template_content_map.json` 366 文件 / 2737 sheets 完整结构数据；剩余 93 步（8.9%）为边界情况待用户前端手动指定后写入 custom_rules
