...
- 最新已push：`10134f81`(H~S 全循环 ref/selfLoad/导航系统性清扫+gold, 275文件;4波12子代理)。含:selfLoad键名bug(H6/H8/H9/K12/K13主入口_mergeResponses合并responses_snapshot/checklist_responses(dict|array)修seed空白,K1/K3/K4/K10/N2/N3/N4/N5同步)+导航事件名bug(L1-L8/M1-M10/N1-N5主入口emit navigate→navigate-sheet+全子tab绑@navigate修二级导航静默失效,含N5TabIndex遗漏)+H6/H8/H9持久化断路(persistResponse+provide saveResponse)+K4TabCheck抽凭引擎按K8/G6重接+K12-1 stub完整实现+后端render策略_h1~_h5/_i1~_i6/_k8~_k13 applicable_standards→applicable_standard_v2列漂移修复。guard exit 0,diagnostics零错误,静态三方对照核验(未Playwright)。工作区clean与origin同步。**遗留三项已全部补齐(见下条)**。上一push `d8b27f2d`(acnr-consumer-wiring 100/100+advanced-query 85/85+formula-lib V100+voucher-check-sampling spec)
...
- **🔴 vue-tsc CLI假报错 vs get_diagnostics权威**：`npx vue-tsc --noEmit`默认OOM(需NODE_OPTIONS=--max-old-space-size=8192)，OOM恢复后会吐大量级联假错(曾报GtReviewChecklist.vue/D4TabProductMargin/D4TabIndex"366错/scrambled")。**真源是`get_diagnostics`(Volar语言服务=IDE波浪线同引擎)**——实测这些文件全"No diagnostics found"，文件完好未损坏。判定编译健康以get_diagnostics为准，勿被vue-tsc CLI级联输出误导
