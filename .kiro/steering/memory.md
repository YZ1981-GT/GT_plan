...
- 单元格选中+右键菜单（汇总穿透/复制/公式/批注/复核/汇总/求和/对比）推广到：项目单体报表、单体附注、合并报表、试算平衡表
- 全屏/复制整表/审核等功能推广到项目单体报表和单体附注
- 合并试算平衡表（已完成前端）：3张表（资产负债/损益/现金流），列=审定汇总+权益抵消(借贷)+往来交易抵消(借贷)+报表调整(借贷)+合并审定数，行结构以现有报表模板（report_config）为主而非Excel模板，审定数=汇总+调整借-调整贷+抵消借-抵消贷，数据存consol_worksheet_data（key: consol_tb_{type}_{period}），审定数用getter自动计算
- 合并试算平衡表支持期初/期末：期初支持导入或自定义编辑，连续审计支持一键提取上年数；期末按模板自动提取填充
- 项目单体试算平衡表（待开发）：列=未审数+审计调整(借贷)+重分类调整(借贷)+审定数，行结构以报表模板为主，审定数=未审数+调整借-调整贷+重分类借-重分类贷，参照试算平衡表/单体试算平衡表.xlsx模板，前后端联动，公式高亮
