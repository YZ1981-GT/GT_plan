...
- **wp_templates 双引擎支持待办**（用户要求"装 Univer Doc preset 独立产品要都能解决"）：4 步骤 1-2 天工作量——(1) 已装包 ✅；(2) 后端 docx→Univer Doc JSON 转换器（python-docx 解析段落/标题/表格）；(3) 前端 WorkpaperEditor 按 wp_template_metadata.component_type 分发 sheets/docs 双引擎；(4) 实测 + mammoth+TipTap 兜底；待用户确认 3 个决策（A. 还原度 80-90% 可接受？B. 双引擎 800KB-1MB bundle 增量可接受？C. 转换失败兜底用 mammoth+TipTap 还是模板下载？）后实施
