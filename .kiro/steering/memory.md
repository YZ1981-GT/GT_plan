...
- **✅ L3长期借款**：全部完成(35/35任务,14sheet/11需求/8PBT/7波次全绿,主入口sheetName v-if分发+11子组件(core6+interest1+inspection5)+13composable+后端3py+useL3FormulaEngine负债类+useL3InterestEngine365天制+useL3ReclassEngine一年内到期+利息联动L2/L8+征信+逾期+抵质押+重分类RJE借2501贷2801+41单元+35集成+10 E2E)
- **✅ L4应付债券**：全部完成(37/37任务,15sheet/12需求/9PBT/7波次全绿,主入口sheetName v-if分发+14子组件(core6+measurement5+classification2+inspection1)+14composable(3纯函数引擎+4通用+7sheet-specific)+后端3py(renderer+router6端点+service)+useL4FormulaEngine负债类贷方+useL4EIREngine实际利率法2分支+useL4EquityLiabEngine权益负债划分+89列5区段Tab+solveEIR二分法+generateSchedule尾差调整+L2/L8联动EventBus+GtIndexChip+导入导出7sheet+85测试全绿)
- **✅ L5长期应付款**：全部完成(32/32任务,12sheet/8需求/7PBT/7波次全绿,主入口sheetName v-if分发+10子组件(core7+amortization1+inspection2)+12composable(2纯函数引擎+4通用+6sheet-specific)+后端3py(renderer+router5端点+service)+useL5FormulaEngine负债类贷方5函数+useL5AmortizationEngine实际利率法4函数+双区块审定(应付款+未确认融资费用)+净额=应付-未确认+L5-5摊销测算表EIR核心+L8财务费用联动EventBus+GtIndexChip+导入导出5sheet+42单元+15集成+10Phase6集成+E2E)
- **✅ L7其他非流动负债**：全部完成(28/28任务含5PBT,8sheet/6需求/5PBT/7波次全绿,主入口sheetName v-if分发+7子组件(core6+inspection1)+9composable(1纯函数引擎7函数+4通用+4sheet-specific)+后端3py(renderer+router3端点+service)+useL7FormulaEngine负债类贷方+27列2区段Tab+审定明细勾稽+导入导出3表+6大集成+67测试全绿)
- **✅ L8财务费用**：全部完成(33/33任务含7PBT,10sheet/8需求/7PBT/7波次全绿,主入口sheetName v-if分发+9子组件(core6+interest1+cutoff1+inspection1)+12composable(3纯函数引擎+4通用+5sheet-specific)+后端3py(renderer+router5端点+service)+useL8FormulaEngine损益类发生额+useL8InterestEngine利息汇聚L1/L3/L4/L5+useL8CutoffEngine截止±5天+非金融利息超标360制+23列3区段Tab+10项签名合计+TB回写6603发生额口径+GtIndexChip+导入导出3表+6大集成+133测试全绿)
- **✅ L6专项应付款**：全部完成(28/28任务含5PBT,9sheet/6需求/5PBT/7波次全绿,主入口sheetName v-if分发+7子组件(core6+inspection1)+9composable(1纯函数引擎8函数+4通用+4sheet-specific)+后端3py(renderer+router3端点+service)+useL6FormulaEngine负债类贷方+33列3区段Tab+专款专用核查L6-4+审定明细勾稽+导入导出L6-2+6大集成+TB writeback端点修复+~104测试全绿)
- **✅ M1应付股利**：全部完成(31/31任务含6PBT,10sheet/7需求/6PBT/7波次全绿,主入口sheetName v-if分发+9子组件(core6+calc2+inspection1)+11composable(3纯函数引擎+4通用+4sheet-specific)+后端3py(renderer+router6端点+service)+useM1FormulaEngine负债类贷方5函数+useM1FxEngine外币折算2函数+useM1DividendEngine股利测算2函数+外币汇率M1-4动态行+股利测算M1-5接收M6 EventBus+27列3区段Tab明细+借贷平衡调整+附注上市/国企自动切换+导入导出3表+6大集成+14契约+6PBT+43单元+22集成+9 E2E)
- **✅ M2实收资本（股本）**：全部完成(32/32任务含7PBT,11sheet/7需求/7PBT/7波次全绿,主入口sheetName v-if分发+10子组件(core6+calc1+inspection1+disclosure2)+11composable(3纯函数引擎+4通用+4sheet-specific)+后端3py(renderer+router7端点+service)+useM2FormulaEngine权益类贷方5函数+useM2FxEngine外币折算2函数+useM2VerifyEngine验资核对2函数+上市/非上市双版本明细el-segmented分支选择器+36列/24列3区段Tab+验资核对8项清单+外币折算差异→M4 EventBus+TB回写4001+导入导出3表+6大集成+54单元+27集成+9 E2E)
- **✅ M3库存股**：全部完成(31/31任务含6PBT,10sheet/7需求/6PBT/7波次全绿,主入口sheetName v-if分发+7子组件(core5+calc1+inspection1)+11composable(3纯函数引擎+4通用+4sheet-specific)+后端3py(renderer+router5端点+service)+useM3FormulaEngine权益备抵借方5函数+useM3FxEngine外币折算2函数+useM3TreasuryEngine回购注销2函数+按回购批次分类小计+19列3区段Tab明细+外币回购折算+回购注销核对(M2/M4冲减)+GtIndexChip跨底稿+TB回写4002+导入导出3表+6大集成+13契约+6PBT+48单元+20集成+9 E2E)
- **✅ M4资本公积**：全部完成(30/30任务含6PBT,9sheet/6需求/6PBT/7波次全绿,主入口sheetName v-if分发+7子组件(core6+inspection1)+10composable(2纯函数引擎+4通用+4sheet-specific)+后端3py(renderer+router6端点+service)+useM4FormulaEngine权益类贷方5函数+useM4ReserveEngine资本公积变动2函数+双区块审定(资本溢价+其他资本公积)+24列2区段Tab明细+J3股份支付权益结算EventBus联动+M2外币折算差异联动+TB回写4002+导入导出2表+6大集成+14契约+6PBT+16单元+20集成+9 E2E)
- **✅ M5盈余公积**：全部完成(31/31任务含6PBT,10sheet/6需求/6PBT/3波次并行全绿,主入口sheetName v-if分发+8子组件(core6+calc1+inspection1)+10composable(2纯函数引擎+4通用+4sheet-specific)+后端3py(renderer+router5端点+service)+useM5FormulaEngine权益类贷方3函数+useM5AccrualEngine法定10%计提3函数+双区块审定(法定+任意盈余公积)+M6净利润联动EventBus+50%注册资本上限判断+TB回写4101+导入导出M5-2+6大集成+6PBT+49单元+58集成+10 E2E)
- **✅ M7专项储备**：全部完成(31/31任务含6PBT,9sheet/7需求/6PBT/7波次全绿,主入口sheetName v-if分发+8子组件(core6+calc1+inspection1)+10composable(2纯函数引擎+4通用+4sheet-specific)+后端3py(renderer+router+service)+useM7FormulaEngine权益类贷方3函数+useM7AccrualEngine安全生产费2函数+按产量分档/按营收比例计提+资本化支出→H1联动EventBus+GtIndexChip+TB回写4201+导入导出+6大集成+107单元+32集成+8 E2E)
- **✅ M9其他综合收益**：全部完成(30/30任务含6PBT,9sheet/6需求/6PBT/7波次全绿,主入口sheetName v-if分发+7子组件(core5+inspection1)+10composable(2纯函数引擎+4通用+4sheet-specific)+后端3py(renderer+router5端点+service)+useM9FormulaEngine权益类贷方3函数+useM9OciEngine税后净额+多来源核对3函数+双大类(不可重分类G8/J2+可重分类债权/套期/外币)+OCI核对表M9-4核心(13公式+EventBus g8:fair-value-changed/j2:remeasured+GtIndexChip跳转+差异红色高亮)+30列3区段Tab明细+附注上市/国企自动切换+TB回写4103+导入导出M9-2+6大集成+cross_wp_references CW-403/CW-404+38单元+63集成(前端27+后端36)+8 E2E)
- **✅ M8一般风险准备**：全部完成(29/29任务含5PBT,9sheet/6需求/5PBT/7波次全绿,主入口sheetName v-if分发+7子组件(core6+calc1)+10composable(2纯函数引擎+4通用+4sheet-specific)+后端3py(renderer+router5端点+service)+useM8FormulaEngine权益类贷方3函数+useM8RiskEngine风险资产计提2函数+金融企业行业守卫isFinancialEntity+按风险资产1.5%计提测试M8-4+TB回写4104+导入导出M8-2+6大集成+50单元+24前端集成+21后端集成+9 E2E)
- **🔵 M循环M6三件套齐全待开发**：M6未分配利润(权益+利润分配结转枢纽)
- **✅ M10其他权益工具**：全部完成(31/31任务含6PBT,10sheet/6需求/6PBT/7波次全绿,主入口sheetName v-if分发+8子组件(core6+inspection2)+11composable(2纯函数引擎+4通用+5sheet-specific)+后端3py(renderer+router5端点+service)+useM10FormulaEngine权益类贷方7函数+useM10ClassificationEngine CAS37 3纯函数+7维度逐工具判定+负债权益区分(→L4/L7)+TB回写4003+30列3区段Tab+导入导出M10-2+6大集成+62单元+28集成+9 E2E)
- **✅ N2应交税费**：全部完成(42/42任务含9PBT,18sheet/13需求/9PBT/7波次全绿,主入口sheetName v-if分发+14子组件(core6+inspection3+calc5)+14composable(3纯函数引擎useN2FormulaEngine负债类+useN2VatEngine增值税+useN2MultiTaxEngine多税种+4通用+7sheet-specific)+后端3py(renderer+router6端点+service)+负债类期末=期初+贷-借+多税种测算(增值税销项-进项/城建税7%5%1%/教育附加3%2%/房产税从价从租/土增税四级累进/出口退税)+72单元+67集成+12 E2E+N4联动EventBus tax-accrual:updated+GtIndexChip+导入导出5sheet+6大集成)
- **✅ N3递延所得税负债**：全部完成(28/28任务含5PBT,6sheet/6需求/5PBT/7波次全绿,主入口sheetName v-if分发+5子组件(core5:Index/Adjudication/Detail/Adjustment/Disclosure)+8composable(2纯函数引擎useN3FormulaEngine负债类+useN3DeferredTaxEngine应纳税差异×税率+4通用FormData/CrossSheet/DualMode/ImportExport+2sheet-specific Adjudication/Detail)+后端3py(renderer+router3端点+service)+负债类期末=期初+贷-借+ROUND(差异×税率,2)+N1同源对应+N5递延税费用联动EventBus+GtIndexChip(N1-4/N5-8)+TB回写2901+导入导出N3-2+6大集成+73单元+28集成+5PBT+11契约+8 E2E)
- **✅ N5所得税费用**：全部完成(42/42任务含10PBT,15sheet/13需求/10PBT/7波次全绿,主入口sheetName v-if分发+13子组件(core6+calc3+benefit4)+11composable(3纯函数引擎useN5FormulaEngine损益类+useN5IncomeTaxEngine核心5函数+useN5TaxAdjustmentEngine纳税调整+4通用FormData/CrossSheet582行/DualMode/ImportExport分sheet+4sheet-specific CurrentTaxCalc/TaxAdjustment/RdSuperDeduction/DeferredReconcile)+后端3py(renderer损益类tb_ledger+router3端点纳税调整分sheet+service)+损益类取发生额6801+当期所得税计算链(会计利润±纳税调整×税率)+纳税调整107行虚拟滚动5分类+递延核对N1/N3 EventBus+研发加计I6/I2联动+高新认定18项红色警告+财产损失12公式+有效税率+TB回写+77单元+39集成+8 E2E+导入导出N5-2/N5-5分sheet+6大集成(抽凭/OCR不适用))
- **✅ N1递延所得税资产**：全部完成(34/34任务含7PBT,9sheet/8需求/7PBT/7波次全绿,主入口sheetName v-if分发+8子组件(core6:Index/Adjudication/Detail/Adjustment/DisclosureListed/DisclosureSoe+calc2:CalcTable/LossCheck)+11composable(3纯函数引擎useN1FormulaEngine资产类+useN1DeferredTaxEngine可抵扣差异×税率+useN1LossCompensationEngine谨慎性min限额+4通用FormData/CrossSheet/DualMode/ImportExport+4sheet-specific)+后端3py(renderer+router3端点+service资产类取数tb_balance+递延税测算+亏损确认+跨底稿合计)+资产类期末=期初+借-贷+N3同源对应(N1-4分列资产/负债)+N5递延税费用联动EventBus+GtIndexChip(N3-2/N5-8)+TB回写1811+导入导出N1-2+6大集成(②③⑤不适用)+86单元+73集成+10 E2E)
- **🔵 N循环N4三件套齐全待开发**：N4税金及附加(损益+多税种+N2计提对应)
...
- **🔵 S类特定项目程序6 spec三件套齐全待开发**(源模板87 xlsx，分析工具backend/scripts/analyze_s_category.py+dump_s34/s_special_content.py)：
  ...
  - ✅`s32-fraud-response-bundle`全部完成(23/23任务,13舞弊情形Tab+el-tabs可滚动+IC-0/IC-X子sheet切换+提示details折叠+useS32BundleState composable+完成仪表盘三色+空状态+sheetName路由+readonly透传+GtIndexChip灰态+htmlRendererRegistry注册+wp_code_overrides 14条+19PBT+10集成+6E2E)
  - ✅`s33-announcement14-bundle`全部完成(23/23任务,9核查底稿Tab+el-tabs可滚动+S33-4隐藏程序表变体精简/完整切换+提示details折叠5底稿+useS33BundleState composable(wpIdMap+completionMap+progressSummary)+完成仪表盘三色进度条+空状态+sheetName路由+readonly透传+GtIndexChip灰态+bundleSheetAliases 9条+htmlRendererRegistry注册+wp_code_overrides 10条+VALID_COMPONENT_TYPES+19PBT(6 Properties)+15集成+6E2E+4契约)
  - `s35-refinancing-bundle`(再融资5核查+明细子表,8需求/7波)
  - ✅`s-estimate-calculation-workpapers`全部完成(29/29任务,4componentType:s15-eps-roe/s21-data-asset/s20-revenue-deduction/s3-policy-change,4纯函数公式引擎+22Vue组件+TB回写3端点+导入导出3端点+EventBus联动+GtIndexChip+20PBT+35集成+6E2E=243测试,主入口sheetName v-if分发+defineAsyncComponent lazy+双模式+selfLoad+6大集成)
  - ✅`s-special-transaction-workpapers`全部完成(28/28任务,6专属componentType:s4-nonmonetary-exchange/s5-debt-restructuring/s6-fund-occupation/s12-cpa-expert/s13-mgmt-expert/s14-accounting-estimate+检查表型S1/S2/S8/S9/S10/S11/S16/S17走a-program-console,3纯函数引擎+~40Vue组件+TB回写4端点+6resolver+导入导出+17PBT前端+6PBT后端+48集成+8E2E,S4商业实质IF+S5不得提前确认+S12/S13专家3分支+S17旧xls转换+S15/S20交叉勾稽)
