...
- **🟢 spec 状态归零**：active=0 / archived=135（INDEX.md 已更新+V065 迁移标注）
...
- **🟢 去重功能审计加固完成（2026-06-08）**：按复盘修了 3 个审计合规底线——①**默认软删替代 hard delete**（`is_deleted=true + deleted_at=now()`，复用回收站可恢复；保留 `hard_delete=True` 选项）②**写 app_audit_log**（`audit_logger.log_action(action="ledger_dedup")` 记录 year/mode/各表删除数/删除 id 样例最多50/可恢复标志，失败不阻断）③**回归测试** `test_ledger_dedup.py`（3 passed：精确重复删留 distinct / superseded 不动 / 无重复返回 0，PG 真库验证软删 visible=3+soft_deleted=2）。端点传 user_id+ip_address；前端确认弹窗+成功提示改为"进回收站可恢复"。**软删模式 dedup CTE 加 `is_deleted=false` 过滤避免重复软删已删行**。剩余未做：TOCTOU(dry_run 与执行两请求间)用返回真实删除数兜底 / raw_extra::text 键序漏删(偏保守安全)：诊断确认真重复在序时账（active dataset 内 tb_ledger 1751 行 + tb_aux_ledger 71 行整行字节级完全相同，含 raw_extra 同、created_at 同微秒=同批 COPY 写入；余额表 812 行全 distinct 无重复——截图"重复"是清理前 superseded+active 双 dataset 都显示的显示层现象）。**后端**=`ledger_data_service.dedup_ledger_data(project_id, year, tables, dry_run)`：窗口函数 ROW_NUMBER() PARTITION BY 全部业务列（排除 id/created_at/updated_at/deleted_at/import_batch_id，raw_extra 用 ::text 比较）保留最小 id 删其余，只作用 active dataset，hard delete；安全=整行字节级完全相同才删，任一列有差异即保留。**端点**=`POST /api/projects/{pid}/ledger-data/dedup`（manager+ 权限，dry_run 预览）。**前端**=`LedgerPenetration.vue` 工具栏加"数据去重"按钮（红色 Delete 图标），先 dry_run 预览各表重复行数→ElMessageBox 确认→执行→刷新；apiPaths 加 `data.dedup`。dry_run 实测 total=1822。**根因仍是流式写入路径无去重（pipeline 未来应在写入前加整行去重防患），本功能是入库后补救**
