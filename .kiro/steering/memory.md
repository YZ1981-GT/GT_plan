...
- **角色体系治理（2026-05-20 复盘识别 5 项关键问题，需新建 spec）**：①后端 UserRole 枚举仅 6 种（admin/partner/manager/auditor/qc/readonly），但代码实际出现 11+ 种角色字符串（assistant/eqcr/quality_control/pm/qc_reviewer/archive_signer/project_manager），统一性几乎为零 ②assistant 角色后端不存在，前端处处判断 role==='assistant' 永远 false（实际写入 auditor），导致 assistant 视图永远不被默认选中 ③权限继承未明确：partner permissions 23 项 vs manager 21 项不是 superset（partner 看不到 manager 独有的 view_dashboard_manager/approve_workhours），违反审计实务 ④EQCR 是项目级角色（ProjectAssignment.role='eqcr'）但被混入系统级 ROLE_PERMISSIONS，权限判断错乱 ⑤角色显示名 i18n 缺失，无统一 roleDisplayName(role) 中文映射；建议新建 roles-governance spec（1 天起草+1.5 天实施）：后端枚举对齐+权限继承明确+EQCR 双层判断+角色矩阵文档化+i18n 字典
