...
- **✅ 列级契约测试落地（2026-06-01，新增 sqlglot 依赖）**：`test_raw_sql_column_contract.py`（标 `pg_only`，连真实 PG）= sqlglot 解析裸 SQL→别名映射→校验每个 `别名.列` 在真实表存在（只校验带别名限定列，保守零误报）；补全表级测试只覆盖一半的缺口（半数 bug 是查不存在列）；asyncpg 栈连库不引 psycopg2；sqlglot 已加 requirements.txt（test 段）
- **🔴 列级测试揪出 18 个 phantom 列引用债务（✅ 已全部清零，2026-06-01）**：data_validation_engine（financial_report.amount/line_code/line_name→current_period_amount/row_code/row_name + trial_balance.aje_amount/rje_amount→aje_adjustment/rje_adjustment + ae.account_code→standard_account_code/a.entry_type→adjustment_type + workpaper_tb 校验因 wp_account_mapping 表不存在改空占位同 layer5）/ linkage_service（a.summary→description AS summary、a.status→review_status AS status、status!='rejected'→review_status，保留 SQL 别名下游不变）/ wp_template_files（trial_balance 无 account_code/closing_balance/借贷发生额→standard_account_code/opening_balance/unadjusted_amount/audited_amount）/ report_trace+wp_evidence_index（working_paper 无 wp_code→JOIN wp_index 取 i.wp_code）；QC-19/20 程序裁剪治理(procedure_instances 无 trim_category/trim_status/trim_evidence_refs/working_paper_id/name)+QC-26(disclosure_notes 无 is_key_disclosure/source_cells/title)= schema 从未建+运行时已守卫→登记 `_COLUMN_ALLOWLIST` 作专项债务；**两契约测试 4 用例全绿**
- **契约测试自带 stale-debt 守护奏效**：移除 data_validation 的 wp_account_mapping 引用后，`test_phantom_debt_not_growing_and_still_real` 立即报"该表已不再被引用请从 _KNOWN_PHANTOM_DEBT 移除"——白名单不会变陈旧假绿，修一处自动催删一处
- **关键真实列速查（2026-06-01 实证）**：trial_balance 金额列=unadjusted_amount/aje_adjustment/rje_adjustment/audited_amount/opening_balance（无 closing_balance/account_code，科目列=standard_account_code）；financial_report=row_code/row_name/current_period_amount/source_accounts（无 amount/line_code）；adjustments=adjustment_no/adjustment_type/account_code/review_status（无 status/entry_type/summary）；adjustment_entries=standard_account_code（无 account_code）；procedure_instances=status/wp_id（无 trim_status/working_paper_id）；working_paper 无 wp_code（在 wp_index.wp_code，JOIN wp_index_id）
- **✅ 已提交并推送（2026-06-01，commit `f344f5a6`）**：40 文件 +658/-136（14 个 500 修复 + V045-V048 迁移配 R + test_raw_sql_schema_contract.py + 修 test_schema_drift_detector 假红），push 到 `work/2026-05-30-wp-specs`（未直推 main，pre-push 钩子过）；提交前还原了无害噪音 components.d.ts；**待走 PR 合 main**（gh CLI 未登录需网页发起）；`docs/proposals/platform-global-2026-06-01.md` 非本次产物留工作区未动
