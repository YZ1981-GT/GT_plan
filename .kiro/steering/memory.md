...
- **report_line_mapping_service 数据驱动 + 项目维度感知已落地**（2026-05-24，commit `da36de1`）：4 大根因问题全部修复 — ①row_code 格式 `BS001` → `BS-001`（与 report_config_seed 一致带连字符）②不区分国企/上市 → 4 套维度独立映射（133 个科目国企 vs 上市行号不同）③53/131 一级科目未覆盖 → 全 136 条覆盖 ④坏账分项 1231-01~05 接入；产出 = `backend/data/account_to_report_line_seed.json`（4 套 × 136 条）+ service 删除 3 套硬编码字典（_BALANCE_SHEET_LINES/_INCOME_STATEMENT_LINES/_NAME_TO_*）+ 新增 `_load_seed` LRU 缓存 + `_derive_applicable_standard` + `_lookup_report_line_from_seed`；27 tests passed；判定铁律 = 任何"科目→报表行"映射都必须项目维度感知（按 `(Project.template_type, Project.report_scope)` 派生 `applicable_standard` ∈ {soe_standalone, soe_consolidated, listed_standalone, listed_consolidated}），不能再有全局静态字典；4xxx 实际是成本类（生产成本/制造费用/工程施工/机械作业），权益类在 3xxx；坏账分项 1231-01~05 在 seed 直接精确查表（不走 4 位前缀） — 国企应收票据 BS-007 / 上市 BS-005
