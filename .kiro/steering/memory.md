...
- **🟡 ledger 三 spec 待执行（2026-06-07 分析完成）**：执行顺序=header-adapter→sign-convention→diagnostics（单向依赖链）；header-adapter P0 先落 Task 1-3+5（DTO+adapter+JSON 加载+submit gate）；sign-convention 分两批（P0=类型+迁移+converter，P1=推导规则+异常+dry-run+前端）；diagnostics 最后（等 sign_anomaly_flags 就位）；**执行前需 codegraph impact 实证**：`_detect_header_row`/`getDirection`/`ai_suggest_mappings`/`file_fingerprint` 是否已存在
