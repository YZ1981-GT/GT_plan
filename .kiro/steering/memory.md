...
- **附注 spec Sprint A.5 进行中**（2026-05-28，commits `b40fbae5`+`aff71ef9`，D14 国企↔上市丝滑切换 7/16 子任务已完成）：**已完成**：A.5.1 `convert_disclosure_notes_v2`（section_id 驱动互转：共有保留/SOE 独有归档到 template_lineage.archived_sections/Listed 独有创建空章节/format_diff 列重映射/更新 project.template_type）+ A.5.2 manual cells 保留（通过不动共有章节 table_data 实现，统计 _cell_modes=manual 数量返回 user_edits_preserved）+ A.5.3 `preview_conversion_v2`（返回 common/to_archive/to_create/format_diff/user_edits_preserved/warnings）+ A.5.4 `note_soe_listed_diff.json` mock（106 共有/60 SOE 独有/71 Listed 独有/33 格式差异）+ A.5.9 `note_template_diff.py`（load_diff_data/compute_diff_from_templates/compute_section_diff/adapt_table_data/classify_section_mapping）+ A.5.10 章节级映射 4 类 + A.5.11 adapt_table_data 列重映射；10 conversion_v2 测试全绿；**待完成**：A.5.5~A.5.8（template_type 一致性）+ A.5.12~A.5.14（前端 UI 留 C.3）+ A.5.15（PBT CI-20）+ A.5.16（验收）；**Phase 1 累计 43/65+ 子任务 / ~10.5 / 17 人天 / 348 测试 / 9 commits**
