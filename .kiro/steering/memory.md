...
- **`g7-column-alignment-and-extraction-closure` 已 24/24 全交付**（2026-08-12，待归档）：56 偏差点→0；补第三边（源xlsx↔seed↔运行时）+ **新增第四边「渲染层」**；2 个 CI job；变异 13 锚点全 RED。🔴 **10 个正式产物仍 `??` 未跟踪**（`g7_column_source_facts.json` + 3 诊断脚本 + 3 后端守卫 + `g7ColumnThreeWayAlignment.spec.ts` + `g7DisclosureHeaderBlocks.ts` / `G7DisclosureCell.vue`）⇒ **CI job 在干净 checkout 下必挂**，需先 `git add`。
- **Active spec ≈ 7~8 个**（🔴 数字一律以 `.kiro/specs/` 复选框实扫为准，行首锚定正则 `^\s*-\s\[([ x~-])\]`，别信 memory 旧数）：`k-cycle-…-closure`(进行中) · `custom-workpaper-dual-mode-…`(28/29) · `e-cycle-…` · `g7-column-alignment-and-extraction-closure` · `h-cycle-…`(17/18) · `note-template-columns-…`(21/23) · `procedure-trimming-and-delegation-intelligence` · `soe-listed-note-conversion-correctness`(17/19)。另有若干仅剩 evidence 的空壳目录（无 tasks.md）。
...
- **git/迁移**：分支 `work/2026-05-30-wp-specs`；最高迁移 **V144**（已应用真实库，`R1xx__` 是配对回滚脚本非同号冲突）；新迁移须重启后端才应用（MigrationRunner 只在启动时跑）；**push 前必先 fetch 看远端真实 base**（memory 里的旧 commit hash 会过期）；协作走 PR 不直推 main。
...
- **`.gitignore` 收的是 `_tmp_*`（带前导下划线），不收 `tmp_*` 也不收 `_wip_*`** ⇒ 这两类临时产物持续污染 `git status`（G7 收口前根目录有 **487** 个 `tmp_*`）。建议补两行；清理时按前缀 + mtime 判归属（今日仍有产出 = 并发会话在用，勿碰）。
