...
- **🔵 L循环L1~L8三件套齐全待开发(跳过L0走函证)**：L1短期借款+L3长期借款(负债+利息测算+征信+逾期+抵质押)/L2应付利息(负债)/L4应付债券(负债+实际利率法EIR2分支+权益负债划分+89列宽表)/L5长期应付款(负债+未确认融资费用摊销)/L6专项应付款+L7其他非流动负债(负债)/L8财务费用(损益+利息汇聚L1L3L4L5+截止)
