...
- **Spec 三件套一致性复盘 SOP**（2026-05-28，note-dynamic v0.6→v0.6.1）：每次大版本演进后必查 9 项 = ①问题陈述无残留旧表 ②维度按编号顺序 ③验收标题数字同步 ④章节编号无冲突（如 F 章节拆 F-Consol/F-Compat）⑤必做范围含全部新维度 ⑥依赖前置含全部新 P-N ⑦CI 卡点表数字同步 ⑧ADR 列表数字同步 ⑨Sprint 工作量与子任务数对齐（≥10 子任务=2.5 人天最低）
