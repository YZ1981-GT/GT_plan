...
- **高级查询左树按项目动态联动**（2026-05-23 沉淀）：`/api/custom-query/indicators` 接 `project_id` query — 读 `Project.template_type` (soe/listed) + `report_scope` (standalone/consolidated) 推 `applicable_standard` 三档（`soe_standalone` / `listed_standalone` / `listed_consolidated`，soe 暂无 consolidated）；附注大类→明细两层从 `backend/data/consol_note_sections_{soe,listed}.json` 按 `parent_section` 分组（91 大类，每个下含「五-1-1 货币资金」式明细），叶子 key 形如 `disclosure_note:五-1-1`，后端 execute 路由 split 冒号注入 filters.section_id；前端按 project_id 独立缓存 sessionStorage 键 `gt:custom-query:indicators-v2:{pid}`，切项目自动重拉；`allSources` computed 改递归 walk 支持 N 层嵌套（顶层/中间层/叶子三态），加 `findFirstLeaf` + `findTopCategoryOf` 工具让中间层点击也能反查顶层 category；判定 = 任何按项目模板差异化展示的树都走"项目维度查询参数 + 项目级缓存键 + 递归扁平化"三件套
