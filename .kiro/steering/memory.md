...
- **LibreOffice xlsx 公式重算策略**（2026-05-23 沉淀）：底稿模板从未在 Excel 打开过时 openpyxl `data_only=True` 读公式 cache 全 None；不引入 formulas/xlcalculator 纯 Python 引擎（中文场景大量自定义公式易爆错），用 `soffice --headless --calc --convert-to xlsx --outdir <tmp> <src>` 触发重算 + 自动回写 cache（首次 3-5s 后续 50ms），转换后 copy 覆盖原文件；按需触发（仅 has_unresolved_formula=True 时调）+ 回写 cache 一次永久有效避免每次查询重算；LibreOffice 不可用时 fallback = 返回 formula 字符串 + value="" 让前端展示「公式未求值」
