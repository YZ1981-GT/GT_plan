...
- **底稿查询零计算路径**（2026-05-23 沉淀，commit `ff7f20d`）：用户保存底稿时 Univer 已计算所有公式值，应直接从入库数据读取而非每次重算 — `univer-save` 端点保存时把 slim 版 `IWorkbookData`（仅留 v 和 f，剥离样式 s）落 `working_paper.parsed_data['univer_snapshot']` JSONB；`_query_workpaper_cell_range` + `wp_sheet_preview` 三级数据源优先级：① **`parsed_data['univer_snapshot']`**（首选，零计算，~50ms PG JSONB） ② storage xlsx cache（data_only=True 读，未保存模板时 None） ③ **LibreOffice 按需重算**（兜底，仅前两路径都无值时触发）；返回字段加 `source` 标识（`univer_snapshot` / `xlsx_cache` / `xlsx_recomputed`）便于前端区分数据新鲜度；snapshot 用 0-indexed key（与 Univer 一致），cell_range 1-indexed 时取值要 `cellData[str(r-1)][str(c-1)]`；判定 = 任何"模板/底稿+用户编辑"双源数据查询都遵循"用户编辑后入库 JSONB 优先 → 文件 cache 次之 → 计算引擎兜底"三级策略，避免每次查询触发昂贵计算
