...
- **fix: auto-match 客户科目为空时自动从 tb_balance 生成（40d461b）**：导入账套只写 tb_balance 不写 account_chart，导致 auto-match 返回 total_client=0；修复 = `_generate_client_accounts_from_balance` 从余额表唯一科目编码+名称生成 client 科目，按首位推断 category（1=资产/2=负债/3=权益/4,5=收入/6=费用）
