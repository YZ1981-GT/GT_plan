...
- **回收站级联删除最终方案（7778318）**：用 `SET session_replication_role = 'replica'` 临时禁用 FK 触发器，动态查 information_schema 获取所有 105 个引用 projects 的子表逐表删除，finally 恢复 `'origin'`；之前多轮 try/except 方案失败因为间接 FK（如 activation_records→ledger_datasets→projects）不在直接引用列表中
