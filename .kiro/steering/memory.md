...
- **已建 spec 待执行（6 平台级）**：`platform-maintenance-governance`(**MVP 已完成**，P0 待续) → `platform-context-permission-foundation` + `platform-ui-editing-consistency`(并行) → `platform-linkage-contract-stale` → `platform-evidence-knowledge-ai-governance` → `platform-role-workbench-quality-loop`；共享原子表+缓存策略+数据可用性审计+边界澄清+接口冻结检查机制已补
