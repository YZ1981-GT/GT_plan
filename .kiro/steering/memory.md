...
- **高级查询 P1-P2 增强 spec**（2026-05-23 启动 `.kiro/specs/advanced-query-enhancements-p1p2/`）：requirements.md 已完成 14 个 Requirement / 18 System Glossary / 7 维度 DoD / 17-23 天工时 / 4 轮迭代拆分（原 12 项扩到 14 项），按 Requirements-First 工作流推进，下一步进 design.md；范围扩展点 = ① 现有 12 项（多底稿批量 P1-5/双向编辑 P1-6/跨 sheet 追溯 P1-X/双源入 DB P2-7/GIN 索引 P2-9/structure.json 单源化 P2-10/审计节流 P2-11/LibreOffice 池化 P2-12/体验 4 项）+ ② **新增 Req 13 跨模块单元格级查询**（统一 source 命名空间 `report:` / `note:` / `adj:` / `tb:` 把底稿的 cell 选区+公式还原+写回扩展到报表/附注/调整分录/试算表 4 模块）+ ③ **新增 Req 14 模板页面双向联动**（WpTemplateDetail/报表模板/附注模板 3 页挂「📊 高级查询」按钮 emit `open-custom-query` 预填 source；查询结果右键「🔗 跳模板溯源」用 `address_registry.parse_uri` 反查模板；树视图 toggle 「按数据源分类」vs「按模板形态分组」）；依赖关系敏感：Req 4 wp_template_registry 入库必须先于 Req 13/14，Req 2 v1 底稿先 v2 再扩 4 模块；推荐落地顺序 = Req 4 → Req 5/6/7/8 架构 → Req 13 跨模块 → Req 14 联动 → Req 1/2/3 业务 → 体验细节
