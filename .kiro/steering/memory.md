...
- **附注 spec Sprint A.5 进行中**（2026-05-28，commit `b40fbae5`，D14 国企↔上市丝滑切换 4/16 子任务已完成）：**已完成**：A.5.4 `note_soe_listed_diff.json` mock 数据（106 共有 / 60 SOE 独有 / 71 Listed 独有 / 33 格式差异，由 `generate_note_soe_listed_diff.py` 按 section_title 完全匹配生成）+ A.5.9 `note_template_diff.py`（350 行纯函数：`load_diff_data` / `compute_diff_from_templates` / `compute_section_diff` / `adapt_table_data` / `classify_section_mapping`）+ A.5.10 章节级映射规则（4 类分类）+ A.5.11 差异表样自动适配（列重映射）；**待完成**：A.5.1~A.5.3（conversion_service v2 改造 + preview_v2 + manual 保留）+ A.5.5~A.5.8（template_type 一致性）+ A.5.12~A.5.14（前端 UI 留 C.3）+ A.5.15~A.5.16（PBT + 验收）；**子代理两次网络断连**（TLS socket disconnect），diff 服务已落盘但 conversion_v2 改造 + 测试文件未完成需下轮继续；**Phase 1 累计 40/65+ 子任务 / ~10 / 17 人天 / 338 测试 / 8 commits**
