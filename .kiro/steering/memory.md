...
- ~~合并工作底稿表样逐表完善~~ ✅ 14 张表功能完整性确认：全屏/头部/操作指引 14/14，导出模板+导入 11/14（3 张汇总表不需要），导出数据/公式/增删行/保存 14/14
...
- ~~内部往来抵消需支持：按科目细分+账龄分布+下级明细逐笔核对+坏账准备抵消还原~~ ✅ 已完成：逐笔核对（matched/diff 标记+核对率）+坏账本方对方分别冲回+差异提示行+按账龄段累加
...
- P0重构（已完成）：ConsolidationIndex.vue 试算平衡表已拆分为 ConsolTrialBalanceTab.vue 独立组件（1678→1350行，减少20%），通过 props/emit 通信+computed 兼容别名
