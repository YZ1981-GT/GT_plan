...
- **全局联动架构改进方案已输出（2026-05-17，commit 523b5df）**：`docs/GLOBAL_LINKAGE_ARCHITECTURE_PROPOSAL.md`；核心方案 = Unified Linkage Bus（统一联动总线）+ 统一 URI 寻址（WP:D2:sheet:cell / REPORT:BS-005 / NOTE:5.7 / ADJ:1122 / TB:1122 / FORMULA:*）+ 5 数据源合并为统一依赖图（~43000 边）+ 7 变更触发点统一入口 + Stale Propagation Engine 替代两套并存机制；4 Sprint / 8 天；8 项量化验收标准；兼容策略 = 保留 event_handlers + 追加统一引擎调用
