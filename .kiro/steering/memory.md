...
- **report_line_mapping_service 4 大根因问题待修**（2026-05-24 复盘待办）：①row_code 格式硬编码 `BS001` vs seed 真实 `BS-001` 命中率 0/63 ②不区分国企/上市（实际差异巨大：使用权资产 SOE=BS-044 / Listed=BS-032，存货 SOE=BS-018 / Listed=BS-012，国企 129 行 vs 上市 88 行）③53/131 一级科目未覆盖（含 1641-1643 使用权资产组、1602/1603 累计折旧+减值、1604-1606 工程物资、2601-2602 租赁负债、6402-6404 营业成本与税费、6701-6702 减值损失等）④坏账分项 1231-0x 也没接入 ai_suggest；改进方案 = 新建 `backend/data/account_to_report_line_seed.json`（4 套维度：soe_standalone/soe_consolidated/listed_standalone/listed_consolidated × 131 一级 + 5 个 1231-0x 分项，row_code 与 report_config_seed 严格一致格式 `BS-XXX`/`IS-XXX`）+ 重写 service 删硬编码字典 + 按 `(template_type, report_scope)` 派生 `applicable_standard` 加载对应 seed；判定 = 任何"科目→报表行"映射都必须项目维度感知（国企/上市 × 单体/合并），不能再有全局静态字典
