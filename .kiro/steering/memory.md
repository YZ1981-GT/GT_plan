...
- **2 个 spec 完成后复盘发现（2026-05-20，5 项 P0/P1 改进）**：①TrimmingSummaryPanel.vue 已创建（197 行）但无父组件 import，违反"形式合规但用户不可达"铁律 — 合伙人/质控看不到裁剪汇总（P0 半天修）②质控视图正则 `^(B15|A15|B50-4|[A-Z]\d+-1)$` 会误命中 B23-1 等控制底稿，应限定为 `^[D-N]\d+-1$`（P0 半天）③role-view-switching 3 PBT + procedure-trimming 12 PBT 全跳过未注明决策（违反"PBT 跳过决策显式列出"铁律，需写到 spec 末尾"已知缺口"段落，P1）④P5 sheet 灰显 iff 所有行 N/A 是核心 property 但完全未测（仅状态字段被测，缺真实灰显验证，P1）⑤无 Playwright E2E 真实用户流程验证（仅 vitest 单元测试不能证明可达性，P2）
