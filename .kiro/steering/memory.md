...
- **gt-plan MCP**：`tools/gt-plan-mcp/server.py`（FastMCP），依赖 `mcp` 包（venv 已装 1.28.1）；两处 mcp.json 的 command 必用 venv 绝对路径 `D:/GT_plan/.venv/Scripts/python.exe`（裸 `python` 会解析到没装 mcp 的系统 python → "Connection closed"）
- **git 代理**：GitHub 直连超时须配 `git config --global http(s).proxy http://127.0.0.1:7897`（本地 Clash）
...
| F | ✅ | F1/F2/F3/F4/F5/F0 全部完成✅；**F类~80tab精美化打磨✅**(F1 9+F2 core/analysis10/valuation15/stocktake+detail+inspection9/special contract+ipo18+F3 10+F4 11+F5 8;比照D4 gold:编制提示+审计目标+tab-toolbar(导入导出▾/F2SheetToolbar+GtIndexChip canonical+共N行)+opinion-card+auto-calc-col+formula虚线tooltip+13px;保留抽凭引擎/OCR/EventBus/CycleImportExportDropdown原样;不臆造AI/💬/导入导出;get_diagnostics全清) |
