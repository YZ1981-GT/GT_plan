/**
 * G4-1：G7 国企附注统一路径真实浏览器取证（复制 G4-0d / H1 §9.6 条件 3–7）。
 *
 * 真栈：frontend 3030 / backend 9980 / OnlyOffice 8080。
 * 目标底稿：canary 项目内活 G7 实例。
 *
 * 断言：
 * 1. 进入「附注披露信息（国企）」并点「在线编辑」后出现 USER_SYNC_PREFIX 请求；
 * 2. 该会话不出现 /d2-sync/*；
...
const WP_ID = '354e060f-15fa-41f0-8632-320082a54ed6'
const ENTRY = 'xlsx/gt-g7-long-term-equity-main'
const USER_SYNC_NEEDLE = `/api/projects/${PROJECT_ID}/workpapers/${WP_ID}/sync/entries/`
const EVIDENCE_DIR = resolve(
  dirname(fileURLToPath(import.meta.url)),
  '../../../.kiro/specs/workpaper-html-onlyoffice-bidirectional-writeback-closure/evidence/g4-1-g7-host-unified-path',
)
/** 受管金额格：流动资产 × 公司1 × 期末（C64） */
const OO_WRITE_CELL = 'C64'
const OO_SHEET_EXACT = '附注披露信息（国企）'
const OO_SHEET_RE = /附注披露信息（国企）|附注披露.*国企/
/** 金额格只能写数字；用 88 + 时间后缀作 marker */
const MARKER_PREFIX = '88'
const HOST_ROOT = '.g7-long-term-equity-main'
const MODE_TOOLBAR = '.g7-long-term-equity-main-toolbar'
const ARCH_CARD_RE = /附注披露信息（国企）|附注披露.*国企/
const HTML_ROOT_SEL = '.g7-soe, .g7-long-term-equity-main'
...
async function openSoeDisclosure(page: Page): Promise<void> {
  ...
  const card = page
    ...
    .first()
  await expect(card, '国企附注目录卡片应可见').toBeVisible({ timeout: 60_000 })
  await card.scrollIntoViewIfNeeded()
  await card.click()
  await expect(page.locator(`${HOST_ROOT}, .g7-soe`).first()).toBeVisible({
    timeout: 60_000,
  })
  await expect(page.locator(MODE_TOOLBAR)).toBeVisible({ timeout: 30_000 })
}

test.describe('G4-1 G7 SOE unified host path', () => {
  ...
  test('在线编辑走 USER_SYNC_PREFIX 且不打 /d2-sync/*', async ({ page }) => {
    ...
    try {
      ...
      // OO Community 无 createConnector：用 Asc.editor 写受管金额格 C64。
      ...
      if (writtenMarker && csOk) {
        ...
        try {
          ...
          // 展开全部 collapse，避免金额格被折叠藏起
          const collapseHeaders = page.locator('.g7-soe .el-collapse-item__header')
          const headerCount = await collapseHeaders.count()
          for (let i = 0; i < Math.min(headerCount, 12); i += 1) {
            const h = collapseHeaders.nth(i)
            ...
            .poll(
              async () => {
                return page.evaluate((m) => {
                  const root =
                    document.querySelector('.g7-soe') ||
                    document.querySelector('.g7-long-term-equity-main')
                  if (!root) return 0
    ...
    const tokenProbe = await page.evaluate(() => {
      ...
      const probe = (window as unknown as { __g4_1_g7_doc_editor_probe?: Record<string, unknown> })
        .__g4_1_g7_doc_editor_probe
      return {
    ...
    const evidence = {
      script: 'e2e/g4-1-g7-unified-path.spec.ts',
      ...
      notes: [
        ...
        'OO 写格：Asc.editor 写受管 C64(流动资产/公司1/期末)；权威判据是 forcesave 响应 cs_error（0=有变更，4=no_changes）',
