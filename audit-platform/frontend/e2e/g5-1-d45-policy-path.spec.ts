/**
 * G5-1：D4-5 独立宿主路径真实浏览器取证（非 unified-path）。
 *
 * D 循环 sheet 扩容：D4-5 会计政策检查（paragraph_block_bidirectional）。
 * 真栈：frontend 3030 / backend 9980 / OnlyOffice 8080。
 * 🔴 目标底稿：同 D4 entry wp b3ab3c46（project 0ec33ac9）。
 *    sheet_key=d45-managed；宿主在 D4TabPolicyCheck 内，不进 isD4DetailSheet。
 * 🔴 OO 写受管格 A18(biz_name)。根 .d4-policy-check；切回后扫分组输入。
 */
...
const OO_WRITE_TARGET = 'A18' // D4-5 紧凑分组表首数据行 biz_name
const MANAGED_SHEET_NAME = '营业收入会计政策检查D4-5'
const EVIDENCE_DIR = resolve(
  dirname(fileURLToPath(import.meta.url)),
  '../../../.kiro/specs/d-cycle-sheet-bidirectional-expansion/evidence/g5-1-d45-policy-path',
)
...
async function openD4Detail(page: Page): Promise<void> {
  ...
  const card = page
    ...
    .filter({ hasText: /D4-5(?!\d)/ })
    .first()
  await expect(card, 'D4-5 目录卡片应可见').toBeVisible({ timeout: 60_000 })
  await card.scrollIntoViewIfNeeded()
  await card.click()
  await expect(page.locator('.d4-policy-check').first()).toBeVisible({ timeout: 60_000 })
  await expect(page.locator('.d4-policy-check .mode-bar')).toBeVisible({ timeout: 30_000 })
}

test.describe('G5-1 D4-5 independent policy host path', () => {
  test.setTimeout(600_000)

  test('D4-5 独立宿主走 USER_SYNC_PREFIX 且不打 /d2-sync/*，OO 回写镜像回 HTML', async ({ page }) => {
    const hits: NetHit[] = []
    ...
    const ooItem = page
      .locator('.d4-policy-check .mode-bar .el-segmented__item')
      .filter({ hasText: '在线编辑' })
    ...
    try {
      ...
      throw new Error(
        `点「在线编辑」后 30s 内无 sync 请求；console=${JSON.stringify(consoleErrors.slice(0, 8))}；modeBar=${await page.locator('.d4-policy-check .mode-bar').innerText().catch(() => '<missing>')}`,
      )
    }
    ...
    try {
      ...
      try {
        const marker = `g5d45${Date.now().toString().slice(-6)}`
              ...
              ({ value, attemptNo, target, wantSheet }: { value: string; attemptNo: number; target: string; wantSheet: string }) => {
                ...
                try {
                  if (typeof api.asc_getWorksheetsCount === 'function') {
                    ...
                    if (want < 0) want = names.findIndex((nm) => nm.includes('D4-5'))
      ...
      if (writtenMarker && csOk) {
        ...
        try {
          ...
          await openD4Detail(page)
          await expect(page.locator('.d4-policy-check')).toBeVisible({ timeout: 120_000 })
          await expect(page.locator('[data-testid="wp-sync-host"]')).toHaveCount(0, { timeout: 60_000 })
          htmlDomRoundtrip.switched_to_html = true

          // D4TabPolicyCheck：切到会计政策章节后扫分组输入框
          await page.locator('.d4-policy-check .sec-nav-item').filter({ hasText: '会计政策' }).first().click().catch(() => {})
          await expect
            .poll(
              async () => {
                const byText = await page.locator('.d4-policy-check').getByText(writtenMarker, { exact: true }).count()
                const inputProp = await page.evaluate((m) => {
                  const root = document.querySelector('.d4-policy-check')
                  if (!root) return 0
                  let n = 0
                  for (const el of root.querySelectorAll('input, textarea')) {
                    const v = (el as HTMLInputElement).value || ''
                    if (v === m && !el.closest('.mode-bar')) n += 1
                  }
                  return n
                }, writtenMarker)
                const tableText = await page.evaluate((m) => {
                  const r = document.querySelector('.d4-policy-check')
          ...
          htmlDomRoundtrip.dom_probe = {
            by_exact_text: await page.locator('.d4-policy-check').getByText(writtenMarker, { exact: true }).count(),
    ...
    const evidence = {
      script: 'e2e/g5-1-d45-policy-path.spec.ts',
      ...
      notes: [
        ...
        `OO 写格：Asc.editor 写受管 ${OO_WRITE_TARGET}(biz_name)；权威判据是 forcesave cs_error（0=有变更，4=no_changes）`,
        'application applied / revision+1 由同目录 DB 核查脚本补齐（venv 查库）',
        '§9.6 条件 6：切回结构化视图后扫 .d4-policy-check 分组输入（排除 .mode-bar）',
        'D4-5 独立宿主：D4TabPolicyCheck 自管 bridge，sheet_key=d45-managed；不并入父级 isD4DetailSheet',
      ],
