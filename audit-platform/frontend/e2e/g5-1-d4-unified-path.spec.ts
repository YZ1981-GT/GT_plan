/**
 * G5-1：D4-2 统一路径真实浏览器取证（§9.6 条件 3–7 的可自动化部分）。
 *
 * Phase 5 第六个非 pilot canary（位置数组 months[12]）。
 * D4 收入底稿权威模板经净化过 OOXML 外链门（sanitize_d4_template_external_links.py + .preclean.bak）；N 列内部 SUM 保留。
 * 真栈：frontend 3030 / backend 9980 / OnlyOffice 8080。
 * 🔴 目标底稿：Task76/first_publication 选中的有真载荷 D4 = wp b3ab3c46（project 0ec33ac9，store 1739B）。
 *    D4-2-rows 有真载荷 2 行；OO 写 A12 product 文本列（非数值月列）。
 * 🔴 OO 写受管格 A12(product 文本列)。D4TabRevenueDetail 根 .d4-tab-revenue-detail；
 *    切回后用搜索框过滤 marker，再扫 product 输入框（排除 toolbar）。
 */
...
const OO_WRITE_TARGET = 'A12' // D4-2 受管区首数据行 product 文本列（B~M 为数值月列）
const MANAGED_SHEET_NAME = '主营业务收入明细表D4-2'
const EVIDENCE_DIR = resolve(
  dirname(fileURLToPath(import.meta.url)),
  '../../../.kiro/specs/d4-revenue-matrix-bidirectional/evidence/g5-1-d4-unified-path',
)
...
async function openD4Detail(page: Page): Promise<void> {
  ...
  const card = page
    ...
    .filter({ hasText: /D4-2(?!\d)/ })
    .first()
  await expect(card, 'D4-2 目录卡片应可见').toBeVisible({ timeout: 60_000 })
  await card.scrollIntoViewIfNeeded()
  await card.click()
  await expect(page.locator('.d4-tab-revenue-detail').first()).toBeVisible({ timeout: 60_000 })
  await expect(page.locator('.d4-mode-toolbar')).toBeVisible({ timeout: 30_000 })
}

test.describe('G5-1 D4-2 unified host path', () => {
  test.setTimeout(600_000)

  test('D4-2 在线编辑走 USER_SYNC_PREFIX 且不打 /d2-sync/*，OO 回写镜像回 HTML', async ({ page }) => {
    const hits: NetHit[] = []
    ...
    page.on('response', (res: Response) => {
      ...
      if (res.status() >= 400 && (url.includes('/sync/') || url.includes('/workpaper-sync/') || url.includes('/checklist'))) {
        void res
          .text()
          .then((text) => {
            httpErrors.push({
    ...
    await page.addInitScript(() => {
      const g = window as unknown as {
        __g5_d4_doc_editor_probe?: { called: boolean; has_root_token: boolean; document_mutable: boolean }
    ...
    const ooItem = page
      .locator('.d4-mode-toolbar .el-segmented__item')
      .filter({ hasText: '在线编辑' })
    ...
    try {
      ...
      throw new Error(
        `点「在线编辑」后 30s 内无 sync 请求；console=${JSON.stringify(consoleErrors.slice(0, 8))}；toolbar=${await page.locator('.d4-mode-toolbar').innerText().catch(() => '<missing>')}`,
      )
    }
    ...
    try {
      ...
      try {
        const marker = `g5d4${Date.now().toString().slice(-6)}`
              ...
              ({ value, attemptNo, target, wantSheet }: { value: string; attemptNo: number; target: string; wantSheet: string }) => {
                ...
                try {
                  if (typeof api.asc_getWorksheetsCount === 'function') {
                    ...
                    if (want < 0) want = names.findIndex((nm) => nm.includes('D4-2'))
        ...
        try {
          if (trackedOpId) {
            try {
              ...
              // apply 可能停在 application_bound 较久；权威门是 checklist 含 marker
            }
          }
          await expect
            .poll(
          ...
          await openD4Detail(page)
          await expect(page.locator('.d4-tab-revenue-detail')).toBeVisible({ timeout: 120_000 })
          await expect(page.locator('[data-testid="wp-sync-host"]')).toHaveCount(0, { timeout: 60_000 })
          htmlDomRoundtrip.switched_to_html = true

          // D4TabRevenueDetail 有搜索框：先按 marker 过滤，避免宽表/多行时漏扫
          const search = page.locator('.d4-tab-revenue-detail .tab-toolbar input').first()
          await expect(search).toBeVisible({ timeout: 15_000 })
          // 等 checklist 行进 DOM（芯片「共 N 行」且 >0）后再搜
          await expect
            .poll(async () => {
              const chip = await page.locator('.d4-tab-revenue-detail .tab-toolbar .el-tag').last().innerText().catch(() => '')
              const m = chip.match(/共\s*(\d+)\s*行/)
          ...
          await expect
            .poll(
              async () => {
                const byText = await page.locator('.d4-tab-revenue-detail').getByText(writtenMarker, { exact: true }).count()
                const inputProp = await page.evaluate((m) => {
                  const root = document.querySelector('.d4-tab-revenue-detail')
                  if (!root) return 0
                  let n = 0
                ...
                const virtualText = await page.evaluate((m) => {
                  const r = document.querySelector('.d4-tab-revenue-detail .virtual-table, .d4-tab-revenue-detail')
          ...
          htmlDomRoundtrip.dom_probe = {
            by_exact_text: await page.locator('.d4-tab-revenue-detail').getByText(writtenMarker, { exact: true }).count(),
            ...
            search_filled: true,
            row_chip: await page.locator('.d4-tab-revenue-detail .tab-toolbar .el-tag').last().innerText().catch(() => ''),
          }
        } catch (err) {
    ...
    const tokenProbe = await page.evaluate(() => {
      ...
      const probe = (window as unknown as { __g5_d4_doc_editor_probe?: Record<string, unknown> }).__g5_d4_doc_editor_probe
      return {
    ...
    const evidence = {
      script: 'e2e/g5-1-d4-unified-path.spec.ts',
      ...
      predicates: {
        ...
        http_errors: httpErrors.slice(0, 12),
      },
      notes: [
        'token/JWT/route_credential 值已脱敏',
        `OO 写格：Asc.editor 写受管 ${OO_WRITE_TARGET}(product 文本列，A 列是 category enum)；权威判据是 forcesave cs_error（0=有变更，4=no_changes）`,
        'application applied / revision+1 由同目录 DB 核查脚本补齐（venv 查库）',
        '§9.6 条件 6：切回结构化视图后用搜索框过滤 marker，再扫 product 输入框（排除 .tab-toolbar）',
        'Phase 5 第六个非 pilot canary（位置数组）：D4 权威模板经外链净化过 OOXML 门且保留 N=SUM(B:M)；目标 wp b3ab3c46(project 0ec33ac9) 有真载荷 1739B',
      ],
