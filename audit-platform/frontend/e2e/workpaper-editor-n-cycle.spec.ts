 ...
 * 4. useNCycleEditor composable 集成验证（API 层）
 *
...
/** 调 API 找出 N5 底稿 ID（fallback 任意 N 循环） */
async function findNCycleWpId(request: APIRequestContext, token: string): Promise<{ id: string; code: string } | null> {
  ...
  // 优先 N5（所得税），fallback 任意 N 循环
  const n5 = list.find((w: any) => (w.wp_code || '').toUpperCase() === 'N5')
  if (n5) return { id: n5.id, code: 'N5' }
  const nAny = list.find((w: any) => /^N\d/i.test(w.wp_code || ''))
  if (nAny) return { id: nAny.id, code: (nAny.wp_code || '').toUpperCase() }
  return null
}

test.describe('N 循环底稿编辑器实测（Phase 3 Task 3.9）', () => {
  test('3.9.1 — N5 所得税底稿加载无 ErrorBoundary + 无 console errors', async ({ page, request }) => {
    ...
    const wp = await findNCycleWpId(request, token)
    test.skip(!wp, '辽宁卫生项目无 N 循环底稿（需先跑 chain orchestrator）')
    ...
    if (await errorBoundary.count() > 0) {
      console.log('⚠️ Pre-existing ErrorBoundary detected — not caused by useNCycleEditor')
  ...
  test('3.9.2 — N 循环 sheet 导航正常渲染', async ({ page, request }) => {
    test.setTimeout(60_000)
    const token = await loginAs(page, 'admin', 'admin123')
    const wp = await findNCycleWpId(request, token)
    test.skip(!wp, 'N 循环底稿不存在')
    ...
    expect(groupCount, `N 循环应有 ≥1 个 sheet 分组，实际 ${groupCount}`).toBeGreaterThanOrEqual(1)
  })

  test('3.9.3 — N 循环 useNCycleEditor composable 集成验证（API 层）', async ({ request }) => {
    test.setTimeout(20_000)
    // 验证 N 循环底稿存在且可通过 API 访问
    ...
    // 验证 prerequisite-status 端点可达（N 循环前置状态）
