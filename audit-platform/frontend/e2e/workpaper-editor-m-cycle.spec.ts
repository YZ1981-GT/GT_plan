 ...
 * 4. useMCycleEditor composable 集成验证（API 层）
 *
...
/** 调 API 找出 M6 底稿 ID（fallback 任意 M 循环） */
async function findMCycleWpId(request: APIRequestContext, token: string): Promise<{ id: string; code: string } | null> {
  ...
  // 优先 M6（权益变动表），fallback 任意 M 循环
  const m6 = list.find((w: any) => (w.wp_code || '').toUpperCase() === 'M6')
  if (m6) return { id: m6.id, code: 'M6' }
  const mAny = list.find((w: any) => /^M\d/i.test(w.wp_code || ''))
  if (mAny) return { id: mAny.id, code: (mAny.wp_code || '').toUpperCase() }
  return null
}

test.describe('M 循环底稿编辑器实测（Phase 3 Task 3.8）', () => {
  test('3.8.1 — M6 权益变动表底稿加载无 ErrorBoundary + 无 console errors', async ({ page, request }) => {
    ...
    const wp = await findMCycleWpId(request, token)
    test.skip(!wp, '辽宁卫生项目无 M 循环底稿（需先跑 chain orchestrator）')
    ...
    if (await errorBoundary.count() > 0) {
      console.log('⚠️ Pre-existing ErrorBoundary detected — not caused by useMCycleEditor')
  ...
  test('3.8.2 — M 循环 sheet 导航正常渲染', async ({ page, request }) => {
    test.setTimeout(60_000)
    const token = await loginAs(page, 'admin', 'admin123')
    const wp = await findMCycleWpId(request, token)
    test.skip(!wp, 'M 循环底稿不存在')
    ...
    expect(groupCount, `M 循环应有 ≥1 个 sheet 分组，实际 ${groupCount}`).toBeGreaterThanOrEqual(1)
  })

  test('3.8.3 — M 循环 useMCycleEditor composable 集成验证（API 层）', async ({ request }) => {
    test.setTimeout(20_000)
    // 验证 M 循环底稿存在且可通过 API 访问
    ...
    // 验证 prerequisite-status 端点可达（M 循环前置状态）
