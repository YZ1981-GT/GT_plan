/**
 * G5-1：D7-2 统一路径真实浏览器取证（§9.6 条件 3–7 的可自动化部分）。
 *
 * Phase 5 第二个非 pilot canary（harness 无关的独立 entry 双向路径）。
 * D7 权威模板经净化过 OOXML 外链门（sanitize_d7_template_external_links.py + .preclean.bak）。
 * 真栈：frontend 3030 / backend 9980 / OnlyOffice 8080。
 * 目标底稿：已有 published representation 的 D7 实例（wp_id 6f23dcce，含真载荷 669B）。
 *
 * 断言（与 g5-1-d1 同构）：
 * 1. 进入 D7-2 并点「在线编辑」后出现 USER_SYNC_PREFIX 请求（store-projection / pending-mutations）；
 * 2. 该会话不出现 /d2-sync/*；
 * 3. materialize 响应的 callbackUrl query 含 room-bound 五项 key（值脱敏不落盘）；
 * 4. WorkpaperSyncEditorHost 挂载；OO 编辑受管格 A10(contract_name) → forcesave；
 * 5. §9.6 条件 6：CS0 后 checklist 镜像回 D7-2-rows 且切回结构化视图 DOM 可见 marker；
...
const WP_ID = '6f23dcce-6bb2-4456-9f18-b20b04043a55'
const ENTRY = 'xlsx/gt-d7-contract-liabilities'
const USER_SYNC_NEEDLE = `/api/projects/${PROJECT_ID}/workpapers/${WP_ID}/sync/entries/`
const OO_WRITE_TARGET = 'A10' // D7-2 受管区首数据行的 contract_name 列（A10..A22 数据区）
const MANAGED_SHEET_NAME = '明细表D7-2'
const EVIDENCE_DIR = resolve(
  dirname(fileURLToPath(import.meta.url)),
  '../../../.kiro/specs/workpaper-html-onlyoffice-bidirectional-writeback-closure/evidence/g5-1-d7-unified-path',
)
...
async function openD7Detail(page: Page): Promise<void> {
  await page.goto(`/projects/${PROJECT_ID}/workpapers/${WP_ID}/edit`, {
    waitUntil: 'domcontentloaded',
  })
  const card = page
    .locator('.gt-b-arch__card')
    .filter({ hasText: /D7-2(?!\d)/ })
    .first()
  await expect(card, 'D7-2 目录卡片应可见').toBeVisible({ timeout: 60_000 })
  await card.scrollIntoViewIfNeeded()
  await card.click()
  await expect(page.locator('.d7-detail').first()).toBeVisible({ timeout: 60_000 })
  await expect(page.locator('.d7-mode-toolbar')).toBeVisible({ timeout: 30_000 })
}

test.describe('G5-1 D7-2 unified host path', () => {
  test.setTimeout(600_000)

  test('D7-2 在线编辑走 USER_SYNC_PREFIX 且不打 /d2-sync/*，OO 回写镜像回 HTML', async ({ page }) => {
    const hits: NetHit[] = []
    ...
    await page.addInitScript(() => {
      const g = window as unknown as {
        __g5_d7_doc_editor_probe?: { called: boolean; has_root_token: boolean; document_mutable: boolean }
    ...
    await openD7Detail(page)

    const ooItem = page
      .locator('.d7-mode-toolbar .el-segmented__item')
      .filter({ hasText: '在线编辑' })
    ...
    try {
      ...
      throw new Error(
        `点「在线编辑」后 30s 内无 sync 请求；console=${JSON.stringify(consoleErrors.slice(0, 8))}；toolbar=${await page.locator('.d7-mode-toolbar').innerText().catch(() => '<missing>')}`,
      )
    }
    ...
    try {
      ...
      try {
        const marker = `g5d7${Date.now().toString().slice(-6)}`
              ...
              ({ value, attemptNo, target, wantSheet }: { value: string; attemptNo: number; target: string; wantSheet: string }) => {
                ...
                try {
                  if (typeof api.asc_getWorksheetsCount === 'function') {
                    ...
                    if (want < 0) want = names.findIndex((nm) => nm.includes('D7-2'))
      ...
      if (writtenMarker && csOk) {
        ...
        try {
          ...
          await openD7Detail(page)
          await expect(page.locator('.d7-detail')).toBeVisible({ timeout: 120_000 })
          await expect(page.locator('[data-testid="wp-sync-host"]')).toHaveCount(0, { timeout: 60_000 })
          htmlDomRoundtrip.switched_to_html = true

          const search = page.locator('.d7-detail .tab-toolbar .toolbar-left .el-input__inner').first()
          await expect(search, '单位/合同搜索框应可见').toBeVisible({ timeout: 30_000 })
          await search.click()
          await search.fill('')
          ...
          await expect
            .poll(
              async () => {
                const byText = await page.locator('.d7-detail').getByText(writtenMarker, { exact: true }).count()
                const inputProp = await page.evaluate((m) => {
                  const root = document.querySelector('.d7-detail')
                  if (!root) return 0
                  let n = 0
                ...
                const virtualText = await page.evaluate((m) => {
                  const r = document.querySelector('.d7-detail .virtual-table, .d7-detail')
          ...
          htmlDomRoundtrip.dom_probe = {
            by_exact_text: await page.locator('.d7-detail').getByText(writtenMarker, { exact: true }).count(),
    ...
    const tokenProbe = await page.evaluate(() => {
      ...
      const probe = (window as unknown as { __g5_d7_doc_editor_probe?: Record<string, unknown> }).__g5_d7_doc_editor_probe
      return {
    ...
    const evidence = {
      script: 'e2e/g5-1-d7-unified-path.spec.ts',
      ...
      notes: [
        ...
        `OO 写格：Asc.editor 写受管 ${OO_WRITE_TARGET}(contract_name)；权威判据是 forcesave cs_error（0=有变更，4=no_changes）`,
        'application applied / revision+1 由同目录 DB 核查脚本补齐（venv 查库）',
        '§9.6 条件 6：切回结构化视图后 D7TabDetail(.d7-detail) DOM 应出现 marker（getByText / input.value，排除搜索框）',
        'Phase 5 第二个非 pilot canary（两级表头+账龄组）：权威模板经外链净化过 OOXML 门',
      ],
