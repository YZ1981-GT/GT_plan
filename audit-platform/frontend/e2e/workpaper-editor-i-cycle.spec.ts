...
/** 调 API 找出 I2 底稿 ID（fallback I1） */
async function findICycleWpId(request: APIRequestContext, token: string): Promise<{ id: string; code: string } | null> {
  ...
  // 优先 I2，fallback I1
  const i2 = list.find((w: any) => (w.wp_code || '').toUpperCase() === 'I2')
  if (i2) return { id: i2.id, code: 'I2' }
  const i1 = list.find((w: any) => (w.wp_code || '').toUpperCase() === 'I1')
  if (i1) return { id: i1.id, code: 'I1' }
  return null
}

test.describe('I 循环底稿编辑器实测（Phase 3 Task 3.4）', () => {
  test('3.4.1 — I2 无形资产底稿加载无 ErrorBoundary + 无 console errors', async ({ page, request }) => {
    ...
    const wp = await findICycleWpId(request, token)
    test.skip(!wp, '辽宁卫生项目无 I 循环底稿（需先跑 chain orchestrator）')
    ...
    if (await overlay.count() > 0) {
      await expect(overlay).toBeHidden({ timeout: 15_000 }).catch(() => {
        throw new Error(`GtLoadingOverlay 在 23 秒后仍可见，疑似死锁；底稿: ${wp!.code}`)
      })
    }
  ...
  test('3.4.2 — I 循环 sheet 导航正常渲染', async ({ page, request }) => {
    test.setTimeout(60_000)
    const token = await loginAs(page, 'admin', 'admin123')
    const wp = await findICycleWpId(request, token)
    test.skip(!wp, 'I 循环底稿不存在')
    ...
    // 验证至少有 1 个 sheet 分组（I 循环使用 useIIntangibleAssetSheetGroups）
    const groups = page.locator('.gt-usn__group')
    const groupCount = await groups.count()
    expect(groupCount, `I 循环应有 ≥1 个 sheet 分组，实际 ${groupCount}`).toBeGreaterThanOrEqual(1)
  })

  test('3.4.3 — I 循环 useICycleEditor composable 集成验证（API 层）', async ({ request }) => {
    test.setTimeout(20_000)
    // 验证 I 循环底稿存在且可通过 API 访问
    ...
    // 验证 prerequisite-status 端点可达（I 循环前置状态）
