/**
 * G5-1：D4-3 统一路径真实浏览器取证（§9.6 条件 3–7 的可自动化部分）。
 *
 * D 循环 sheet 扩容 canary：D4-3 其他业务收入明细（字段键扁平行）。
 * 真栈：frontend 3030 / backend 9980 / OnlyOffice 8080。
 * 🔴 目标底稿：同 D4 entry wp b3ab3c46（project 0ec33ac9）。
 *    D4-3-rows 字段键扁平行；OO 写 A13 item 文本列。
 * 🔴 OO 写受管格 A13(item)。D4TabOtherRevenue 根 .d4-tab-other-revenue；
 *    切回后扫 item 输入框（本表无搜索框；排除 toolbar）。
 */
...
const OO_WRITE_TARGET = 'A13' // D4-3 受管区首数据行 item 文本列
const MANAGED_SHEET_NAME = '其他业务收入明细表D4-3'
const EVIDENCE_DIR = resolve(
  dirname(fileURLToPath(import.meta.url)),
  '../../../.kiro/specs/d-cycle-sheet-bidirectional-expansion/evidence/g5-1-d43-unified-path',
)
...
async function openD4Detail(page: Page): Promise<void> {
  ...
  const card = page
    ...
    .filter({ hasText: /D4-3(?!\d)/ })
    .first()
  await expect(card, 'D4-3 目录卡片应可见').toBeVisible({ timeout: 60_000 })
  await card.scrollIntoViewIfNeeded()
  await card.click()
  await expect(page.locator('.d4-tab-other-revenue').first()).toBeVisible({ timeout: 60_000 })
  await expect(page.locator('.d4-mode-toolbar')).toBeVisible({ timeout: 30_000 })
}

test.describe('G5-1 D4-3 unified host path', () => {
  test.setTimeout(600_000)

  test('D4-3 在线编辑走 USER_SYNC_PREFIX 且不打 /d2-sync/*，OO 回写镜像回 HTML', async ({ page }) => {
    const hits: NetHit[] = []
    ...
    try {
      ...
      try {
        const marker = `g5d43${Date.now().toString().slice(-6)}`
              ...
              ({ value, attemptNo, target, wantSheet }: { value: string; attemptNo: number; target: string; wantSheet: string }) => {
                ...
                try {
                  if (typeof api.asc_getWorksheetsCount === 'function') {
                    ...
                    if (want < 0) want = names.findIndex((nm) => nm.includes('D4-3'))
      ...
      if (writtenMarker && csOk) {
        ...
        try {
          ...
          await openD4Detail(page)
          await expect(page.locator('.d4-tab-other-revenue')).toBeVisible({ timeout: 120_000 })
          await expect(page.locator('[data-testid="wp-sync-host"]')).toHaveCount(0, { timeout: 60_000 })
          htmlDomRoundtrip.switched_to_html = true

          // D4TabOtherRevenue 无搜索框：等「共 N 行」芯片后直接扫 item 输入框
          await expect
            .poll(async () => {
              const chip = await page.locator('.d4-tab-other-revenue .tab-toolbar .el-tag').last().innerText().catch(() => '')
          ...
          await expect
            .poll(
              async () => {
                const byText = await page.locator('.d4-tab-other-revenue').getByText(writtenMarker, { exact: true }).count()
                const inputProp = await page.evaluate((m) => {
                  const root = document.querySelector('.d4-tab-other-revenue')
                  if (!root) return 0
                  let n = 0
                ...
                const tableText = await page.evaluate((m) => {
                  const r = document.querySelector('.d4-tab-other-revenue')
                  return r?.textContent?.includes(m) ? 1 : 0
                }, writtenMarker)
                return byText + inputProp + tableText
              },
              { timeout: 60_000 },
            )
            .toBeGreaterThan(0)
          ...
          htmlDomRoundtrip.dom_probe = {
            by_exact_text: await page.locator('.d4-tab-other-revenue').getByText(writtenMarker, { exact: true }).count(),
            ...
            search_filled: false,
            row_chip: await page.locator('.d4-tab-other-revenue .tab-toolbar .el-tag').last().innerText().catch(() => ''),
          }
        } catch (err) {
    ...
    const evidence = {
      script: 'e2e/g5-1-d43-unified-path.spec.ts',
      ...
      notes: [
        ...
        '§9.6 条件 6：切回结构化视图后扫 item 输入框（排除 .tab-toolbar；本表无搜索框）',
