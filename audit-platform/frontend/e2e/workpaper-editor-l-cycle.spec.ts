 ...
 * 4. useLCycleEditor composable 集成验证（API 层）
 *
...
/** 调 API 找出 L1 或 L3 底稿 ID（fallback 任意 L 循环） */
async function findLCycleWpId(request: APIRequestContext, token: string): Promise<{ id: string; code: string } | null> {
  ...
  // 优先 L1（利息测算），fallback L3，再 fallback 任意 L 循环
  ...
  const lAny = list.find((w: any) => /^L\d/i.test(w.wp_code || ''))
  if (lAny) return { id: lAny.id, code: (lAny.wp_code || '').toUpperCase() }
  return null
}

test.describe('L 循环底稿编辑器实测（Phase 3 Task 3.7）', () => {
  test('3.7.1 — L1/L3 底稿加载无 ErrorBoundary + 无 console errors', async ({ page, request }) => {
    ...
    const wp = await findLCycleWpId(request, token)
    test.skip(!wp, '辽宁卫生项目无 L 循环底稿（需先跑 chain orchestrator）')
    ...
    if (await errorBoundary.count() > 0) {
      console.log('⚠️ Pre-existing ErrorBoundary detected — not caused by useLCycleEditor')
  ...
  test('3.7.2 — L 循环 sheet 导航正常渲染', async ({ page, request }) => {
    test.setTimeout(60_000)
    const token = await loginAs(page, 'admin', 'admin123')
    const wp = await findLCycleWpId(request, token)
    test.skip(!wp, 'L 循环底稿不存在')
    ...
    expect(groupCount, `L 循环应有 ≥1 个 sheet 分组，实际 ${groupCount}`).toBeGreaterThanOrEqual(1)
  })

  test('3.7.3 — L 循环 useLCycleEditor composable 集成验证（API 层）', async ({ request }) => {
    test.setTimeout(20_000)
    // 验证 L 循环底稿存在且可通过 API 访问
    ...
    // 验证 prerequisite-status 端点可达（L 循环前置状态）
