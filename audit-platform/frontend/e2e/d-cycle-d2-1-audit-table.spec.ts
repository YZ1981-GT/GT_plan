/**
 * d-cycle-d2-1-audit-table.spec.ts — D2-1 应收账款审定表 E2E 验证
 *
...
test.describe('Task 44: D2-1 审定表编辑+保存+trial_balance.audited_amount 回写确认', () => {
  test('44.1 — D2-1 审定表加载 d-form-table 组件', async ({ page, request }) => {
    ...
    test.skip(!wpResult.exists, 'D2-1 底稿不存在，需先运行项目底稿生成')
  ...
  test('44.2 — D2-1 审定表字段渲染（审定金额等）', async ({ page, request }) => {
    test.setTimeout(60_000)
    const token = await loginAs(page, 'admin', 'admin123')
    ...
    expect(hasAuditFields, 'D2-1 应显示审定表相关字段').toBeTruthy()
  })

  test('44.3 — D2-1 填写 audited_amount 字段并保存', async ({ page, request }) => {
    test.setTimeout(60_000)
    const token = await loginAs(page, 'admin', 'admin123')
    ...
    // 尝试填写数字输入（audited_amount 或任一金额字段）
