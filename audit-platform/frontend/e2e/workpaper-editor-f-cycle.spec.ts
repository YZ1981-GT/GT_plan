 ...
 * 4. useFCycleEditor composable 集成验证（API 层）
 *
...
/** 调 API 找出 F2 底稿 ID */
async function findF2WpId(request: APIRequestContext, token: string): Promise<string | null> {
  ...
  const f2 = list.find((w: any) => (w.wp_code || '').toUpperCase() === 'F2')
  return f2?.id || null
}

test.describe('F 循环底稿编辑器实测（Phase 3 Task 3.2）', () => {
  test('3.2.1 — F2 存货底稿加载无 ErrorBoundary + 无 console errors', async ({ page, request }) => {
    ...
    const wpId = await findF2WpId(request, token)
    test.skip(!wpId, '辽宁卫生项目无 F2 底稿（需先跑 chain orchestrator）')
    ...
    if (await overlay.count() > 0) {
      await expect(overlay).toBeHidden({ timeout: 15_000 }).catch(() => {
        throw new Error('GtLoadingOverlay 在 23 秒后仍可见，疑似死锁；底稿: F2')
      })
    }
  ...
  test('3.2.2 — F2 sheet 导航正常渲染', async ({ page, request }) => {
    test.setTimeout(60_000)
    const token = await loginAs(page, 'admin', 'admin123')
    const wpId = await findF2WpId(request, token)
    test.skip(!wpId, 'F2 不存在')
    ...
    // 验证至少有 1 个 sheet 分组（F2 使用 useFPurchaseInventorySheetGroups）
    const groups = page.locator('.gt-usn__group')
    const groupCount = await groups.count()
    expect(groupCount, `F2 应有 ≥1 个 sheet 分组，实际 ${groupCount}`).toBeGreaterThanOrEqual(1)
  })

  test('3.2.3 — F 循环 useFCycleEditor composable 集成验证（API 层）', async ({ request }) => {
    test.setTimeout(20_000)
    // 验证 F2 底稿存在且可通过 API 访问
    ...
    expect(wpData.wp_code?.toUpperCase()).toBe('F2')

    // 验证 prerequisite-status 端点可达（F 循环前置状态）
    const preResp = await request.get(
      `/api/projects/${PROJECT_ID}/workpapers/prerequisite-status?wp_code=F2`,
      { headers: { Authorization: `Bearer ${token}` } },
    )
