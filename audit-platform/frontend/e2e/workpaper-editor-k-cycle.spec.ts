...
/** 调 API 找出 K8 或 K11 底稿 ID（fallback 任意 K 循环） */
async function findKCycleWpId(request: APIRequestContext, token: string): Promise<{ id: string; code: string } | null> {
  ...
  // 优先 K11（减值汇总），fallback K8（费用分析），再 fallback 任意 K 循环
  const k11 = list.find((w: any) => (w.wp_code || '').toUpperCase() === 'K11')
  if (k11) return { id: k11.id, code: 'K11' }
  const k8 = list.find((w: any) => (w.wp_code || '').toUpperCase() === 'K8')
  if (k8) return { id: k8.id, code: 'K8' }
  const kAny = list.find((w: any) => /^K\d/i.test(w.wp_code || ''))
  if (kAny) return { id: kAny.id, code: (kAny.wp_code || '').toUpperCase() }
  return null
}

test.describe('K 循环底稿编辑器实测（Phase 3 Task 3.6）', () => {
  test('3.6.1 — K8/K11 底稿加载无 ErrorBoundary + 无 console errors', async ({ page, request }) => {
    ...
    const wp = await findKCycleWpId(request, token)
    test.skip(!wp, '辽宁卫生项目无 K 循环底稿（需先跑 chain orchestrator）')
    ...
    if (await errorBoundary.count() > 0) {
      console.log('⚠️ Pre-existing ErrorBoundary detected — not caused by useKCycleEditor')
  ...
  test('3.6.2 — K 循环 sheet 导航正常渲染', async ({ page, request }) => {
    test.setTimeout(60_000)
    const token = await loginAs(page, 'admin', 'admin123')
    const wp = await findKCycleWpId(request, token)
    test.skip(!wp, 'K 循环底稿不存在')
    ...
    expect(groupCount, `K 循环应有 ≥1 个 sheet 分组，实际 ${groupCount}`).toBeGreaterThanOrEqual(1)
  })

  test('3.6.3 — K 循环 useKCycleEditor composable 集成验证（API 层）', async ({ request }) => {
    test.setTimeout(20_000)
    // 验证 K 循环底稿存在且可通过 API 访问
    ...
    // 验证 prerequisite-status 端点可达（K 循环前置状态）
