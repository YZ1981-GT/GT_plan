...
/** 调 API 找出 H1 底稿 ID */
async function findH1WpId(request: APIRequestContext, token: string): Promise<string | null> {
  ...
  const h1 = list.find((w: any) => (w.wp_code || '').toUpperCase() === 'H1')
  return h1?.id || null
}

test.describe('H 循环底稿编辑器实测（Phase 3 Task 3.3）', () => {
  test('3.3.1 — H1 固定资产底稿加载无 ErrorBoundary + 无 console errors', async ({ page, request }) => {
    ...
    const wpId = await findH1WpId(request, token)
    test.skip(!wpId, '辽宁卫生项目无 H1 底稿（需先跑 chain orchestrator）')
    ...
    if (await overlay.count() > 0) {
      await expect(overlay).toBeHidden({ timeout: 15_000 }).catch(() => {
        throw new Error('GtLoadingOverlay 在 23 秒后仍可见，疑似死锁；底稿: H1')
      })
    }
  ...
  test('3.3.2 — H1 sheet 导航正常渲染', async ({ page, request }) => {
    test.setTimeout(60_000)
    const token = await loginAs(page, 'admin', 'admin123')
    const wpId = await findH1WpId(request, token)
    test.skip(!wpId, 'H1 不存在')
    ...
    // 验证至少有 1 个 sheet 分组（H1 使用 useHFixedAssetSheetGroups）
    const groups = page.locator('.gt-usn__group')
    const groupCount = await groups.count()
    expect(groupCount, `H1 应有 ≥1 个 sheet 分组，实际 ${groupCount}`).toBeGreaterThanOrEqual(1)
  })

  test('3.3.3 — H 循环 useHCycleEditor composable 集成验证（API 层）', async ({ request }) => {
    test.setTimeout(20_000)
    // 验证 H1 底稿存在且可通过 API 访问
    ...
    expect(wpData.wp_code?.toUpperCase()).toBe('H1')

    // 验证 prerequisite-status 端点可达（H 循环前置状态）
    const preResp = await request.get(
      `/api/projects/${PROJECT_ID}/workpapers/prerequisite-status?wp_code=H1`,
      { headers: { Authorization: `Bearer ${token}` } },
    )
