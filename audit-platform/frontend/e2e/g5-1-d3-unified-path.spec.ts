/**
 * G5-1：D3-2 统一路径真实浏览器取证（§9.6 条件 3–7 的可自动化部分）。
 *
 * Phase 5 第三个非 pilot canary（harness 无关的独立 entry 双向路径）。
 * D3 预收账款权威模板经净化过 OOXML 外链门（sanitize_d3_template_external_links.py + .preclean.bak）。
 * 真栈：frontend 3030 / backend 9980 / OnlyOffice 8080。
 * 🔴 目标底稿：first_publication 无 --project-id 按 created_at 取最早的 D3 底稿 = wp d35c715a
 *    （project 2aa00f57）。D3-det-rows 全库 0 行（空首版合法）。故本 e2e 跑 2aa00f57 / d35c715a。
 *
 * 断言（与 g5-1-d7 同构）：
 * 1. 进入 D3-2 并点「在线编辑」后出现 USER_SYNC_PREFIX 请求（store-projection / pending-mutations）；
 * 2. 该会话不出现 /d2-sync/*；
 * 3. materialize 响应的 callbackUrl query 含 room-bound 五项 key（值脱敏不落盘）；
 * 4. WorkpaperSyncEditorHost 挂载；OO 编辑受管格 A12(customerName) → forcesave；
 * 5. §9.6 条件 6：CS0 后 checklist 镜像回 D3-det-rows 且切回结构化视图 DOM 可见 marker；
...
const WP_ID = 'd35c715a-9d71-4f4f-80ee-efc846533a24'
const ENTRY = 'xlsx/gt-d3-prepaid-accounts'
const USER_SYNC_NEEDLE = `/api/projects/${PROJECT_ID}/workpapers/${WP_ID}/sync/entries/`
const OO_WRITE_TARGET = 'A12' // D3-2 受管区首数据行的 customerName 列（A12..A23 数据区）
const MANAGED_SHEET_NAME = '预收账款明细表D3-2'
const EVIDENCE_DIR = resolve(
  dirname(fileURLToPath(import.meta.url)),
  '../../../.kiro/specs/workpaper-html-onlyoffice-bidirectional-writeback-closure/evidence/g5-1-d3-unified-path',
)
...
async function openD3Detail(page: Page): Promise<void> {
  await page.goto(`/projects/${PROJECT_ID}/workpapers/${WP_ID}/edit`, {
    waitUntil: 'domcontentloaded',
  })
  const card = page
    .locator('.gt-b-arch__card')
    .filter({ hasText: /D3-2(?!\d)/ })
    .first()
  await expect(card, 'D3-2 目录卡片应可见').toBeVisible({ timeout: 60_000 })
  await card.scrollIntoViewIfNeeded()
  await card.click()
  await expect(page.locator('.d3-detail').first()).toBeVisible({ timeout: 60_000 })
  await expect(page.locator('.d3-mode-toolbar')).toBeVisible({ timeout: 30_000 })
}

test.describe('G5-1 D3-2 unified host path', () => {
  test.setTimeout(600_000)

  test('D3-2 在线编辑走 USER_SYNC_PREFIX 且不打 /d2-sync/*，OO 回写镜像回 HTML', async ({ page }) => {
    const hits: NetHit[] = []
    ...
    await page.addInitScript(() => {
      const g = window as unknown as {
        __g5_d3_doc_editor_probe?: { called: boolean; has_root_token: boolean; document_mutable: boolean }
    ...
    await openD3Detail(page)

    const ooItem = page
      .locator('.d3-mode-toolbar .el-segmented__item')
      .filter({ hasText: '在线编辑' })
    ...
    try {
      ...
      throw new Error(
        `点「在线编辑」后 30s 内无 sync 请求；console=${JSON.stringify(consoleErrors.slice(0, 8))}；toolbar=${await page.locator('.d3-mode-toolbar').innerText().catch(() => '<missing>')}`,
      )
    }
    ...
    try {
      ...
      try {
        const marker = `g5d3${Date.now().toString().slice(-6)}`
              ...
              ({ value, attemptNo, target, wantSheet }: { value: string; attemptNo: number; target: string; wantSheet: string }) => {
                ...
                try {
                  if (typeof api.asc_getWorksheetsCount === 'function') {
                    ...
                    if (want < 0) want = names.findIndex((nm) => nm.includes('D3-2'))
      ...
      if (writtenMarker && csOk) {
        ...
        try {
          ...
          await openD3Detail(page)
          await expect(page.locator('.d3-detail')).toBeVisible({ timeout: 120_000 })
          await expect(page.locator('[data-testid="wp-sync-host"]')).toHaveCount(0, { timeout: 60_000 })
          htmlDomRoundtrip.switched_to_html = true

          const search = page.locator('.d3-detail .detail-toolbar .el-input__inner').first()
          await expect(search, '客户搜索框应可见').toBeVisible({ timeout: 30_000 })
          await search.click()
          await search.fill('')
          ...
          await expect
            .poll(
              async () => {
                const byText = await page.locator('.d3-detail').getByText(writtenMarker, { exact: true }).count()
                const inputProp = await page.evaluate((m) => {
                  const root = document.querySelector('.d3-detail')
                  if (!root) return 0
                  let n = 0
                  for (const el of root.querySelectorAll('input, textarea')) {
                    const v = (el as HTMLInputElement).value || ''
                    if (v === m && !el.closest('.detail-toolbar')) n += 1
                  }
                  return n
                }, writtenMarker)
                const virtualText = await page.evaluate((m) => {
                  const r = document.querySelector('.d3-detail .virtual-table, .d3-detail')
          ...
          htmlDomRoundtrip.dom_probe = {
            by_exact_text: await page.locator('.d3-detail').getByText(writtenMarker, { exact: true }).count(),
    ...
    const tokenProbe = await page.evaluate(() => {
      ...
      const probe = (window as unknown as { __g5_d3_doc_editor_probe?: Record<string, unknown> }).__g5_d3_doc_editor_probe
      return {
    ...
    const evidence = {
      script: 'e2e/g5-1-d3-unified-path.spec.ts',
      ...
      notes: [
        ...
        `OO 写格：Asc.editor 写受管 ${OO_WRITE_TARGET}(customerName)；权威判据是 forcesave cs_error（0=有变更，4=no_changes）`,
        'application applied / revision+1 由同目录 DB 核查脚本补齐（venv 查库）',
        '§9.6 条件 6：切回结构化视图后 D3TabDetail(.d3-detail) DOM 应出现 marker（getByText / input.value，排除 .detail-toolbar 搜索框）',
        'Phase 5 第三个非 pilot canary（两级表头+账龄组）：D3 预收账款权威模板经外链净化过 OOXML 门；目标 wp d35c715a(project 2aa00f57)由 first_publication 按 created_at 定',
      ],
