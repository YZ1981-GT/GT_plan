/**
 * G4-1：H1-8 统一路径真实浏览器取证（复制 G4-0d §9.6 条件 3–7）。
 *
 * 真栈：frontend 3030 / backend 9980 / OnlyOffice 8080。
 * 目标底稿：canary 项目内活 H1 实例（非已删项目 f663b18c）。
 *
 * 断言：
 * 1. 进入 H1-8 并点「在线编辑」后出现 USER_SYNC_PREFIX 请求；
 * 2. 该会话不出现 /d2-sync/*；
...
const WP_ID = 'c71b7c54-6868-4fb9-9e14-083034f57815'
const ENTRY = 'xlsx/gt-h1-fixed-assets'
const USER_SYNC_NEEDLE = `/api/projects/${PROJECT_ID}/workpapers/${WP_ID}/sync/entries/`
const EVIDENCE_DIR = resolve(
  dirname(fileURLToPath(import.meta.url)),
  '../../../.kiro/specs/workpaper-html-onlyoffice-bidirectional-writeback-closure/evidence/g4-1-h1-host-unified-path',
)
const OO_WRITE_CELL = 'D13'
const OO_SHEET_EXACT = '减少检查表H1-8'
const OO_SHEET_RE = /减少检查表H1-8|H1-8/
const MARKER_PREFIX = 'g4h1'
const HOST_ROOT = '.h1-fixed-assets'
const MODE_TOOLBAR = '.h1-mode-switch-bar'
const ARCH_CARD_RE = /H1-8(?!\d)|减少检查/
...
async function openH18Disposal(page: Page): Promise<void> {
  await page.goto(`/projects/${PROJECT_ID}/workpapers/${WP_ID}/edit`, {
    waitUntil: 'domcontentloaded',
  })
  const card = page
    .locator('.gt-b-arch__card')
    .filter({ hasText: ARCH_CARD_RE })
    .first()
  await expect(card, 'H1-8 目录卡片应可见').toBeVisible({ timeout: 60_000 })
  await card.scrollIntoViewIfNeeded()
  await card.click()
  await expect(page.locator(`${HOST_ROOT}, .h1-tab-disposal-check`).first()).toBeVisible({
    timeout: 60_000,
  })
  await expect(page.locator(MODE_TOOLBAR)).toBeVisible({ timeout: 30_000 })
}

test.describe('G4-1 H1-8 unified host path', () => {
  ...
  test('在线编辑走 USER_SYNC_PREFIX 且不打 /d2-sync/*', async ({ page }) => {
    ...
    try {
      ...
      // OO Community 无 createConnector：用 Asc.editor 写受管格 D13（asset_name）。
      // 占位/序号格不会进 HTML store —— §9.6 条件 6 要求 DOM 可见回读。
      ...
      if (writtenMarker && csOk) {
        ...
        try {
          ...
          const htmlRoot = page.locator('.h1-tab-disposal-check, .h1-fixed-assets').first()
          ...
          // H1-8 是 el-table + el-input（asset_name → row.name）；查 text / input.value。
          const byExactText = htmlRoot.getByText(writtenMarker, { exact: true })
          await expect
            .poll(
              async () => {
                ...
                const inputProp = await page.evaluate((m) => {
                  const root =
                    document.querySelector('.h1-tab-disposal-check') ||
                    document.querySelector('.h1-fixed-assets')
                  if (!root) return 0
                  let n = 0
                ...
                return textCount + inputProp + tableText
              },
              { timeout: 60_000 },
            )
            .toBeGreaterThan(0)
          ...
          htmlDomRoundtrip.dom_probe = {
            ...
            table_text_has: await page.evaluate(
              (m) =>
                (
                  document.querySelector('.h1-tab-disposal-check') ||
                  document.querySelector('.h1-fixed-assets')
    ...
    const tokenProbe = await page.evaluate(() => {
      ...
      const probe = (window as unknown as { __g4_1_h1_doc_editor_probe?: Record<string, unknown> })
        .__g4_1_h1_doc_editor_probe
      return {
    ...
    const evidence = {
      script: 'e2e/g4-1-h1-unified-path.spec.ts',
      ...
      notes: [
        ...
        'OO 写格：Asc.editor 写受管 D13(asset_name)；权威判据是 forcesave 响应 cs_error（0=有变更，4=no_changes）',
