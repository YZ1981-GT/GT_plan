/**
 * G5-1：D5-2 统一路径真实浏览器取证（§9.6 条件 3–7 的可自动化部分）。
 *
 * Phase 5 第五个非 pilot canary（最简：FVOCI 两级表头无账龄组）。
 * D5 应收款项融资权威模板经净化过 OOXML 外链门（sanitize_d5_template_external_links.py + .preclean.bak）。
 * 真栈：frontend 3030 / backend 9980 / OnlyOffice 8080。
 * 🔴 目标底稿：first_publication 按 created_at 取最早的 D5 底稿 = wp 629d762a（project 2aa00f57）。
 *    D5-2-rows 全库 0 行（空首版，OO 编辑创建第 1 行）。
 * 🔴 OO 写受管格 B12(itemName 文本列)——A 列是 category(enum)。D5TabDetail 无搜索框，marker
 *    直接扫 .d5-detail 表格 itemName 输入框（排除 .tab-toolbar）。
 */
...
const WP_ID = '629d762a-d363-4195-b1bc-5c28e3c2bd72'
const ENTRY = 'xlsx/gt-d5-receivables-financing'
const USER_SYNC_NEEDLE = `/api/projects/${PROJECT_ID}/workpapers/${WP_ID}/sync/entries/`
const OO_WRITE_TARGET = 'B12' // D5-2 受管区首数据行的 itemName 列（B12..B16；A 列是 category enum）
const MANAGED_SHEET_NAME = '应收款项融资明细表D5-2'
const EVIDENCE_DIR = resolve(
  dirname(fileURLToPath(import.meta.url)),
  '../../../.kiro/specs/workpaper-html-onlyoffice-bidirectional-writeback-closure/evidence/g5-1-d5-unified-path',
)
...
async function openD5Detail(page: Page): Promise<void> {
  await page.goto(`/projects/${PROJECT_ID}/workpapers/${WP_ID}/edit`, {
    waitUntil: 'domcontentloaded',
  })
  const card = page
    .locator('.gt-b-arch__card')
    .filter({ hasText: /D5-2(?!\d)/ })
    .first()
  await expect(card, 'D5-2 目录卡片应可见').toBeVisible({ timeout: 60_000 })
  await card.scrollIntoViewIfNeeded()
  await card.click()
  await expect(page.locator('.d5-detail').first()).toBeVisible({ timeout: 60_000 })
  await expect(page.locator('.d5-mode-toolbar')).toBeVisible({ timeout: 30_000 })
}

test.describe('G5-1 D5-2 unified host path', () => {
  test.setTimeout(600_000)

  test('D5-2 在线编辑走 USER_SYNC_PREFIX 且不打 /d2-sync/*，OO 回写镜像回 HTML', async ({ page }) => {
    const hits: NetHit[] = []
    ...
    await page.addInitScript(() => {
      const g = window as unknown as {
        __g5_d5_doc_editor_probe?: { called: boolean; has_root_token: boolean; document_mutable: boolean }
    ...
    await openD5Detail(page)

    const ooItem = page
      .locator('.d5-mode-toolbar .el-segmented__item')
      .filter({ hasText: '在线编辑' })
    ...
    try {
      ...
      throw new Error(
        `点「在线编辑」后 30s 内无 sync 请求；console=${JSON.stringify(consoleErrors.slice(0, 8))}；toolbar=${await page.locator('.d5-mode-toolbar').innerText().catch(() => '<missing>')}`,
      )
    }
    ...
    try {
      ...
      try {
        const marker = `g5d5${Date.now().toString().slice(-6)}`
              ...
              ({ value, attemptNo, target, wantSheet }: { value: string; attemptNo: number; target: string; wantSheet: string }) => {
                ...
                try {
                  if (typeof api.asc_getWorksheetsCount === 'function') {
                    ...
                    if (want < 0) want = names.findIndex((nm) => nm.includes('D5-2'))
      ...
      if (writtenMarker && csOk) {
        ...
        try {
          ...
          await openD5Detail(page)
          await expect(page.locator('.d5-detail')).toBeVisible({ timeout: 120_000 })
          await expect(page.locator('[data-testid="wp-sync-host"]')).toHaveCount(0, { timeout: 60_000 })
          htmlDomRoundtrip.switched_to_html = true

          // D5TabDetail 无搜索框，marker 直接出现在 itemName 输入框（排除 .tab-toolbar）
          await expect
            .poll(
              async () => {
                const byText = await page.locator('.d5-detail').getByText(writtenMarker, { exact: true }).count()
                const inputProp = await page.evaluate((m) => {
                  const root = document.querySelector('.d5-detail')
                  if (!root) return 0
                  let n = 0
                ...
                const virtualText = await page.evaluate((m) => {
                  const r = document.querySelector('.d5-detail .virtual-table, .d5-detail')
          ...
          htmlDomRoundtrip.dom_probe = {
            by_exact_text: await page.locator('.d5-detail').getByText(writtenMarker, { exact: true }).count(),
    ...
    const tokenProbe = await page.evaluate(() => {
      ...
      const probe = (window as unknown as { __g5_d5_doc_editor_probe?: Record<string, unknown> }).__g5_d5_doc_editor_probe
      return {
    ...
    const evidence = {
      script: 'e2e/g5-1-d5-unified-path.spec.ts',
      ...
      notes: [
        ...
        `OO 写格：Asc.editor 写受管 ${OO_WRITE_TARGET}(itemName 文本列，A 列是 category enum)；权威判据是 forcesave cs_error（0=有变更，4=no_changes）`,
        'application applied / revision+1 由同目录 DB 核查脚本补齐（venv 查库）',
        '§9.6 条件 6：切回结构化视图后 D5TabDetail(.d5-detail) DOM 应出现 marker（D5 无搜索框，直接扫 itemName 输入框，排除 .tab-toolbar）',
        'Phase 5 第五个非 pilot canary（FVOCI 两级表头无账龄组，最简）：D5 权威模板经外链净化过 OOXML 门；目标 wp 629d762a(project 2aa00f57)由 first_publication 按 created_at 定',
      ],
