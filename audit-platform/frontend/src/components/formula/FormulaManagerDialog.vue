 ...
 * 用户可在 Excel 中批量编辑公式后通过"Excel导入"回导
 */
async function onExportFormulaTemplate() {
  try {
    ...
    wsGuide.addRows([
      ...
      ['当前节点', selectedPath.value],
      ['报表类型', selectedNodeKey.value],
    ...
    a.href = url
    a.download = `公式模板_${selectedNodeKey.value}_${new Date().toISOString().slice(0, 10)}.xlsx`
