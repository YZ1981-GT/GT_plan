...
/**
 * M10 编制/使用手册弹窗 — 对齐 N5PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  <el-dialog
    ...
    title="M10 其他权益工具 · 编制手册"
    width="860px"
    top="4vh"
    destroy-on-close
    append-to-body
    class="m10-handbook-dialog"
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：M10-2 明细(永续债/优先股逐项) + M10-4 CAS37 负债与权益区分(有无交付现金义务) + M10-5 检查 → M10-1 审定(未审+AJE+RJE，按工具类型分组) → 回写 TB(4003)+附注。其他权益工具是权益类贷方：期末=期初+贷方(发行)−借方(赎回/转换)；权益+负债=工具总额。本弹窗不改各表页内说明。"
    />
