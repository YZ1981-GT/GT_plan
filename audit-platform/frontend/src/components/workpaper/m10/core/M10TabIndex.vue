  ...
  <div class="m10-tab-index">
    ...
    <div class="m10-dir">
      <div class="index-header">
        <h3 class="title">M10 底稿目录</h3>
        <GtReviewTrigger section-id="M10-index-directory" />
      ...
      <M10PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="m10-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>其他权益工具为<strong>权益类贷方科目</strong>（4003）：期末 = 期初 + 贷方（发行增加）− 借方（赎回/转换减少）</li>
          <li>主要包括<strong>永续债、优先股</strong>等混合金融工具</li>
          <li>核心判定按 <strong>CAS37《金融工具列报》</strong>：是否存在交付现金/其他金融资产的合同义务——无义务→权益工具（4003）；有义务→金融负债（负债科目）</li>
          <li>M10-4 负债与权益区分检查表为本底稿最核心程序，逐条判定分类；<strong>权益部分 + 负债部分 = 工具总额</strong>（金额守恒）</li>
          <li>审定合计（按工具类型分组小计）回写 TB(4003) 并驱动附注披露；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="m10-cycle">
...
/**
 * M10TabIndex.vue — M10 其他权益工具底稿目录（严格镜像 N5/E1 标准）
 *
 ...
 *
 * 权益类贷方科目（4003 其他权益工具）— 期末=期初+贷方−借方；含永续债/优先股，M10-4 CAS37 负债与权益区分为核心。
 */
...
const M10PreparationHandbookDialog = defineAsyncComponent(() => import('../M10PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { name: '其他权益工具实质性程序表', code: 'M10A', sheetKey: '其他权益工具实质性程序表M10A', progress: calcSheetProgress('M10-M10A-', 5) },
  { name: '审定表（按工具类型：永续债/优先股）', code: 'M10-1', sheetKey: '审定表M10-1', progress: calcSheetProgress('M10-1-', 6) },
  { name: '明细表（永续债/优先股明细）', code: 'M10-2', sheetKey: '明细表M10-2', progress: calcSheetProgress('M10-2-', 4) },
  { name: '调整分录汇总', code: 'M10-3', sheetKey: '调整分录汇总M10-3', progress: calcSheetProgress('M10-3-', 4) },
  { name: '负债与权益区分检查表（CAS37 核心）', code: 'M10-4', sheetKey: '负债与权益区分检查表M10-4', progress: calcSheetProgress('M10-4-', 4) },
  { name: '其他权益工具检查表', code: 'M10-5', sheetKey: '其他权益工具检查表M10-5', progress: calcSheetProgress('M10-5-', 4) },
  { name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', progress: calcSheetProgress('M10-disclosure-listed-', 3) },
  { name: '附注披露信息（国有企业）', code: '附注国企', sheetKey: '附注披露信息（国有企业）', progress: calcSheetProgress('M10-disclosure-soe-', 3) },
])
...
// ─── 跨表结论口径看板 ─────────────────────────────────────────────────────────
/** 有审计结论/说明的 sheet（程序表、调整分录、附注不计入；审定/明细/检查计入） */
const M10_CONCLUSION_CODES = new Set(['M10-1', 'M10-2', 'M10-4', 'M10-5'])
...
function isConclusionFilled(code: string): boolean {
  ...
  const token = `${code}-`
  const longer = Array.from(M10_CONCLUSION_CODES).filter(c => c !== code && c.startsWith(token))
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => M10_CONCLUSION_CODES.has(s.code))
