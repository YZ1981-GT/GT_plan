  ...
  <div class="i5-tab-index">
    ...
    <div class="i5-dir">
      <div class="index-header">
        <h3 class="title">I5 其他非流动资产底稿目录</h3>
        <GtReviewTrigger section-id="I5-index-directory" />
      ...
      <I5PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="i5-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          ...
          <li>审定合计回写 TB(1911) 并驱动附注披露；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="i5-cycle">
...
/**
 * I5TabIndex.vue — I5 其他非流动资产底稿目录（严格镜像 I1/K1/L1/E1 标准）
 *
 ...
 *
 * 两科目净值勾稽（1911 其他非流动资产：原值 / 减值准备）：账面净值 = 原值 − 减值。
 *
 * 🔴 I5 各表审计说明/结论 item_id 前缀经实证：审定表 I5-1 用 I5-adj-（I5-adj-audit-note/-audit-conclusion），
 *    明细表 I5-2 用 I5-2-、针对性检查 I5-4 用 I5-4-（-audit-note/-audit-conclusion）；conclPrefix 携真实前缀。
 */
...
const I5PreparationHandbookDialog = defineAsyncComponent(() => import('../I5PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { name: '其他非流动资产实质性程序表', code: 'I5A', sheetKey: '其他非流动资产实质性程序表I5A', prefix: 'I5A-' },
  { name: '审定表', code: 'I5-1', sheetKey: '审定表I5-1', prefix: 'I5-adj-', conclPrefix: 'I5-adj-' },
  { name: '明细表', code: 'I5-2', sheetKey: '明细表I5-2', prefix: 'I5-2-', conclPrefix: 'I5-2-' },
  { name: '调整分录汇总', code: 'I5-3', sheetKey: '调整分录汇总I5-3', prefix: 'I5-3-' },
  { name: '针对性检查表', code: 'I5-4', sheetKey: '针对性检查表I5-4', prefix: 'I5-4-', conclPrefix: 'I5-4-' },
  { name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', prefix: 'I5-disc-L-' },
  { name: '附注披露信息（国有企业）', code: '附注国企', sheetKey: '附注披露信息（国有企业）', prefix: 'I5-disc-S-' },
])
...
const I5_CONCLUSION_CODES = new Set([
  'I5-1', 'I5-2', 'I5-4',
])
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => s.conclPrefix && I5_CONCLUSION_CODES.has(s.code))
