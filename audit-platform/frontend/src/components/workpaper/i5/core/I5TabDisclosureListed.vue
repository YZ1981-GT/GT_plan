<template>
  <div class="i5-tab-disclosure-listed">
    <el-alert
      ...
      title="审计目标：核实其他非流动资产附注披露（上市公司）完整准确——分类账面价值与 I5-2/I5-1 勾稽，并同步至附注五、31。"
    />

    <div class="guide-area">
      <div class="guide-grid">
        <div class="guide-step"><span class="step-num">①</span> 从 I5-2 按分类聚合期末/上年账面余额与减值</div>
        <div class="guide-step"><span class="step-num">②</span> 账面价值 = 账面余额 − 减值准备（期末、上年年末）</div>
        <div class="guide-step"><span class="step-num">③</span> 不存在的项目请删除；可增补实际分类</div>
        <div class="guide-step"><span class="step-num">④</span> 「同步到附注」写入附注模块 §五、31</div>
      </div>
    </div>
    ...
    <div class="methodology-block">
      ...
      <div class="methodology-content">
        对齐源底稿「其他非流动资产附注披露表（上市公司）」：按项目列示期末与上年年末的账面余额、减值准备及账面价值。
        同步附注时写入 note_template「期末余额 / 上年年末余额」= 账面价值口径。
      </div>
    </div>
    ...
    <div class="toolbar-row">
      ...
      <el-button size="small" type="primary" plain :disabled="!projectId" @click="jumpToNote('listed')">↩ 跳转回附注（五、31）</el-button>
    ...
    <el-card shadow="never" class="disc-card">
      <template #header>
        <div class="card-header">
          <span>其他非流动资产附注披露表（上市公司）</span>
          <span class="hint">注：根据实际情况列示；不存在的项目请删除</span>
        </div>
      </template>
        ...
        <el-table-column label="期末数">
          <el-table-column label="账面余额" width="120" align="right">
            <template #default="{ row }">
              <WpAmountInput
                v-if="!isReadonly"
                v-model="row.endGross"
                size="small"
                :disabled="isReadonly"
                @change="(v: number) => disc.updateCell(row.rowId, 'endGross', v ?? 0)"
              />
              <span v-else class="amt">{{ displayPrefs.fmtAmount(row.endGross) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="减值准备" width="110" align="right">
            <template #default="{ row }">
              <WpAmountInput
                v-if="!isReadonly"
                v-model="row.endImpairment"
                size="small"
                :disabled="isReadonly"
                @change="(v: number) => disc.updateCell(row.rowId, 'endImpairment', v ?? 0)"
              />
              <span v-else class="amt">{{ displayPrefs.fmtAmount(row.endImpairment) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="账面价值" width="120" align="right">
            <template #default="{ row }">
              <span class="formula-cell" title="= 账面余额 − 减值准备">{{ displayPrefs.fmtAmount(row.endBookValue) }}</span>
            </template>
          </el-table-column>
        </el-table-column>
        <el-table-column label="上年年末数">
          <el-table-column label="账面余额" width="120" align="right">
            <template #default="{ row }">
              <WpAmountInput
                v-if="!isReadonly"
                v-model="row.priorGross"
                size="small"
                :disabled="isReadonly"
                @change="(v: number) => disc.updateCell(row.rowId, 'priorGross', v ?? 0)"
              />
              <span v-else class="amt">{{ displayPrefs.fmtAmount(row.priorGross) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="减值准备" width="110" align="right">
            <template #default="{ row }">
              <WpAmountInput
                v-if="!isReadonly"
                v-model="row.priorImpairment"
                size="small"
                :disabled="isReadonly"
                @change="(v: number) => disc.updateCell(row.rowId, 'priorImpairment', v ?? 0)"
              />
              <span v-else class="amt">{{ displayPrefs.fmtAmount(row.priorImpairment) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="账面价值" width="120" align="right">
            <template #default="{ row }">
              <span class="formula-cell" title="= 账面余额 − 减值准备">{{ displayPrefs.fmtAmount(row.priorBookValue) }}</span>
            </template>
          </el-table-column>
        </el-table-column>
    ...
    <el-card shadow="never" class="disc-card">
      ...
      <el-input
        ...
        :disabled="isReadonly"
        :autosize="{ minRows: 2 }"
        placeholder="A、附注披露完整准确，已同步五、31。B、除下列事项外未见异常。C、存在重大披露差异，不可确认。"
...
/**
 * I5TabDisclosureListed — 附注披露（上市公司）
 * 对齐源表 + 同步附注五、31
 */
...
function getSummary({ columns }: { columns: any[] }): string[] {
  ...
  const leafVals = [
    t.endGross, t.endImpairment, t.endBookValue,
    t.priorGross, t.priorImpairment, t.priorBookValue,
  ]
  let leaf = 0
  return columns.map((_col: any, i: number) => {
    if (i === 0) return ''
    if (i === 1) return '合计'
    const v = leafVals[leaf++]
