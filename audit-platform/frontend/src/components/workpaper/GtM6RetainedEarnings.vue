<template>
  <div class="m6-retained-earnings">
    ...
    <template v-else>
      ...
      />
      <!-- 程序表 M6A（复用 GtAProgramConsole，不参与双模式） -->
      <GtAProgramConsole
        ...
        :readonly="isReadonly"
      />
      <!-- Q6A修订前 sheet → 跳过/fallback（不参与双模式） -->
...
// index/procedure/skip-q6a 不参与双模式；其余 HTML sheet 上方显示 segmented 切换栏。
const dualMode = useM6EntryDualMode({
...
// ─── 监听 m6:save-items → 保存后自动快照（版本链能力来自 Runtime Boundary） ──
