...
/**
 * L1 编制/使用手册弹窗 — 对齐 M9PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：L1-2 明细(按银行/合同逐笔) + L1-5 利息测算(本金×利率×天数/365，联动 L2/L8) → L1-1 审定(按种类：信用/抵押/保证/质押，未审+账项调整+重分类，双期) → 回写 TB(2001)+附注。短期借款是负债类贷方：期末=期初+贷方(借入)−借方(归还)。L1-4 征信核对/L1-7 逾期检查保证完整性。本弹窗不改各表页内说明。"
    />
