  ...
  <div class="k10-tab-index">
    ...
    <div class="k10-dir">
      <div class="index-header">
        <h3 class="title">K10 底稿目录</h3>
        <GtReviewTrigger section-id="K10-index-directory" />
      ...
      <K10PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="k10-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>其他收益为<strong>损益类贷方科目</strong>（6117）：取本期<strong>贷方发生额</strong>累计（贷方 = 收益增加），非期末余额</li>
          <li>其他收益仅指与日常活动相关的政府补助；与日常活动无关计入营业外收入（6301，K12 底稿）</li>
          <li>政府补助核对（K10-4）：直接计入 + 递延分摊 → 与 K7 递延收益(2401)本期分摊一致性校验</li>
          <li>审定数 = 未审数 + AJE + RJE；审定发生额回写试算平衡表 6117；调整分录联动 A13</li>
          <li>K10-2 明细合计应与 K10-1 审定表一致；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="k10-cycle">
...
const K10PreparationHandbookDialog = defineAsyncComponent(() => import('../K10PreparationHandbookDialog.vue'))
...
/** 按 K10 真实存储 key（JSON-array 单键 / 前缀）计算完成度（二值：有数据→100，无→0）。 */
function calcSheetProgress(progressKeys: string[]): number {
  const map = props.allResponses
  if (!map || map.size === 0) return 0
  const keys = [...map.keys()]
  const hasData = progressKeys.some(pk => map.has(pk) || keys.some(k => k.startsWith(pk)))
  return hasData ? 100 : 0
}

const allSheets = computed<SheetRow[]>(() => {
  const rows: Omit<SheetRow, 'progress'>[] = [
    { seq: 1, name: '实质性程序表', code: 'K10A', sheetKey: '其他收益实质性程序表K10A', group: 'core', progressKeys: ['K10-K10A-'] },
    { seq: 2, name: '审定表', code: 'K10-1', sheetKey: '审定表K10-1', group: 'core', progressKeys: ['K10-1-rows'] },
    { seq: 3, name: '明细表', code: 'K10-2', sheetKey: '明细表K10-2', group: 'core', progressKeys: ['K10-2-detail-rows'] },
    { seq: 4, name: '调整分录汇总', code: 'K10-3', sheetKey: '调整分录汇总K10-3', group: 'core', progressKeys: ['K10-3-entries'] },
    { seq: 5, name: '政府补助核对表', code: 'K10-4', sheetKey: '政府补助核对表K10-4', group: 'inspection', progressKeys: ['K10-4-rows'] },
    { seq: 6, name: '应收政府补助检查表', code: 'K10-5', sheetKey: '应收政府补助检查表K10-5', group: 'inspection', progressKeys: ['K10-5-'] },
    { seq: 7, name: '其他收益检查表', code: 'K10-6', sheetKey: '其他收益检查表K10-6', group: 'inspection', progressKeys: ['K10-6-'] },
    { seq: 8, name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', group: 'disclosure', progressKeys: ['K10-disc-listed-'] },
    { seq: 9, name: '附注披露信息（国有企业）', code: '附注国企', sheetKey: '附注披露信息（国有企业）', group: 'disclosure', progressKeys: ['K10-disc-soe-'] },
  ]
  return rows.map(r => ({ ...r, progress: calcSheetProgress(r.progressKeys) }))
})
...
// ─── 跨表结论口径看板 ─────────────────────────────────────────────────────────
/** 有审计结论/说明的 sheet（K10A 程序表、K10-3 调整分录、附注、目录本身不计入） */
const K10_CONCLUSION_CODES = new Set(['K10-1', 'K10-2', 'K10-4', 'K10-5', 'K10-6'])
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => K10_CONCLUSION_CODES.has(s.code))
