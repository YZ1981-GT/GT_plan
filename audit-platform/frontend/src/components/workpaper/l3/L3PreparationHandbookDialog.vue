...
/**
 * L3 编制/使用手册弹窗 — 对齐 L1PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：L3-2 明细(按银行/合同逐笔，标记一年内到期) + L3-5 利息测算(本金×利率×天数/365，区分资本化/费用化，联动 L2/L8) → L3-1 审定(按种类：信用/抵押/保证/质押，未审+账项调整+重分类，双期) → 减一年内到期得披露审定 → 回写 TB(2501)+附注(非流动数)。长期借款是负债类贷方：期末=期初+贷方(借入)−借方(归还)。一年内到期经 L3-3 重分类(借 2501/贷 2801)。L3-4 征信核对/L3-7 逾期检查保证完整性。本弹窗不改各表页内说明。"
    />
