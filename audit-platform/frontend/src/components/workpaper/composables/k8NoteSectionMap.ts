/**
 * K8 销售费用披露 ↔ 附注章节映射与 sync payload 构建
 * 权威来源：note_template_variant_matrix.json xiao_shou_fei_yong
 * listed: 五、64 / soe: 八、65
 * 附注表结构：单表「销售费用」(项目/本期发生额/上期发生额)
 */
import type { ColumnDef } from './disclosureColumnDefs'

export type K8DisclosureVariant = 'listed' | 'soe'
...
export interface K8SyncPayload {
  wp_id: string
  sheet_name: string
  section_id: string
...
function resolveCurrentStandard(variant: K8DisclosureVariant): string {
...
export function buildK8SyncPayload(
  ...
  if (narrativeText.trim()) {
    sub._note_texts = [{ section: 'note-expense', title: '销售费用说明', text: narrativeText.trim() }]
  }

  return {
    wp_id: wpId,
    sheet_name: K8_DISCLOSURE_SHEET_NAME[variant],
    section_id: K8_NOTE_SECTION[variant],
