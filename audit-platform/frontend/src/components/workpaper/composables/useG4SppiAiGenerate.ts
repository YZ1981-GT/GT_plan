/**
 * useG4SppiAiGenerate — G4 债权投资(SPPI) AI 辅助
 * POST /api/workpapers/{wpId}/g4-sppi/ai/{section}
 */
import { onMounted, ref, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type G4SppiAiSection =
...
export function useG4SppiAiGenerate(wpId: Ref<string>) {
  ...
  async function generateAndConfirm(
    ...
    try {
      const response = await http.post(
        `/api/workpapers/${wpId.value}/g4-sppi/ai/${section}`,
...
export default useG4SppiAiGenerate
