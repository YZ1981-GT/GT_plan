/**
 * useM6DualMode — M6 未分配利润双模式切换 composable
 *
...
/** M6 渲染模式 */
export type M6RenderMode = 'html' | 'onlyoffice'

/** el-segmented 选项 */
export interface M6ModeOption {
  label: string
  value: M6RenderMode
  disabled?: boolean
}

export interface UseM6DualModeOptions {
  wpId: Ref<string>
  defaultMode?: M6RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key prefix（按 wpId 存储） */
const STORAGE_KEY_PREFIX = 'm6-dual-mode'
 ...
 */
export function useM6DualMode(options: UseM6DualModeOptions) {
  const { wpId, defaultMode = 'html' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<M6RenderMode>(defaultMode)
  ...
  function _loadPersistedMode(): M6RenderMode {
    try {
  ...
  function _persistMode(m: M6RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: M6RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<M6ModeOption[]>(() => [
