...
export interface CustomerRow {
  ...
  currentUnadjusted: number  // 自动计算: = priorAudited + currentIncrease - currentDecrease + reclassification
  currentAje: number
  currentRje: number
...
/** 重算行内公式字段（D1-3 含 currentBalance + reclassification） */
function recalcRow(row: CustomerRow): CustomerRow {
  ...
  // D1-3: currentUnadjusted = priorAudited + currentIncrease - currentDecrease + reclassification
  const currentUnadjusted = priorAudited + row.currentIncrease - row.currentDecrease + row.reclassification
...
export function useD1DetailCustomer(options: UseD1DetailCustomerOptions) {
  ...
  const subtotalRow: ComputedRef<CustomerRow> = computed(() => {
    ...
    return {
      ...
      currentBalance: calcSubtotal(currentRows.map(r => r.currentBalance)),
      reclassification: calcSubtotal(currentRows.map(r => r.reclassification)),
  ...
  function updateCell(rowId: string, field: string, value: number | string): void {
    ...
    if (field === 'customerName') {
      ...
      const numericFields: Array<keyof CustomerRow> = [
        ...
        'currentBalance', 'reclassification',
