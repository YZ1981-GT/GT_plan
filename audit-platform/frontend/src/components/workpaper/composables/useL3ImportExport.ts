 ...
 * - 多sheet导出（6 dynamic-row sheets）:
 *   明细表(L3-2) / 利息测算表(L3-5) / 征信核对表(L3-4) / 逾期检查表(L3-7) / 抵质押检查表(L3-8) / 贷款合同检查表(L3-6)
 * - 后端三端点:
 *   - GET  /api/l3-long-term-loans/{wpId}/export-template
 *   - GET  /api/l3-long-term-loans/{wpId}/export-data
 *   - POST /api/l3-long-term-loans/{wpId}/import-data
 * - RFC5987 中文文件名解析
...
/** L3 支持导入导出的 sheet（6 dynamic-row sheets） */
...
export interface UseL3ImportExportOptions {
  wpId: Ref<string>
...
/** 支持导入导出的sheet列表 */
const EXPORTABLE_SHEETS: L3ImportableSheet[] = [
  'L3-2', 'L3-4', 'L3-5', 'L3-6', 'L3-7', 'L3-8',
]

/** API base path */
const API_BASE = '/api/l3-long-term-loans'
 ...
 */
export function useL3ImportExport(options: UseL3ImportExportOptions) {
  ...
  function isSheetExportable(sheet: string): boolean {
    return EXPORTABLE_SHEETS.includes(sheet as L3ImportableSheet)
  }

  /** 获取所有可导出sheet列表 */
  function getExportableSheets(): L3ImportableSheet[] {
   ...
   * GET /api/l3-long-term-loans/{wpId}/export-template?sheet=xxx
   */
   ...
   * GET /api/l3-long-term-loans/{wpId}/export-data?sheet=xxx
   * 不指定sheet时导出所有6个动态行sheet
   */
  ...
  /**
   * 导入xlsx数据
   * POST /api/l3-long-term-loans/{wpId}/import-data (multipart/form-data)
   */
  async function importData(file: File, sheet?: L3ImportableSheet): Promise<L3ImportResult> {
    if (!wpId.value) {
