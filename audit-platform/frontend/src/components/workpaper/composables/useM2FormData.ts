/**
 * useM2FormData — M2 实收资本（股本）数据加载/debounce保存/即时保存/TB回写
 *
 ...
 * - item_id 命名: 前缀 "M2-{sheet}-{field}"（如 "M2-M2-1-audited_amount", "M2-M2-2-detail-row1-end"）
 * - writebackTB(4001): 审定数回写 trial_balance 科目 4001 实收资本/股本
...
export interface UseM2FormDataOptions {
  wpId: Ref<string>
...
const ITEM_PREFIX = 'M2-'
const ACCOUNT_CODE = '4001' // 实收资本/股本（贷方/权益类）
/** 审定表 sheet 名（含子码 M2-1，供后端 extract_determination_wp_code 解出） */
const DETERMINATION_SHEET_NAME = '审定表M2-1'

// ─── Composable ──────────────────────────────────────────────────────────────

export function useM2FormData(options: UseM2FormDataOptions) {
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      await api.get(
        `/api/workpapers/${wpId.value}/render-config?force_component_type=m2-paid-in-capital`,
        { _silent: true } as any,
      )
    } catch {
  ...
  /**
   * 从 checklist_responses 加载 M2 数据（item_id 前缀 "M2-"）
   */
  async function loadResponses(): Promise<void> {
    ...
    try {
      ...
      ElMessage.warning('M2数据加载失败，可手动填写')
    }
  }
  ...
  /**
   * 获取指定 sheet + field 的值
   * @param sheet - 如 "M2-1", "M2-2", "M2-4"
   * @param field - 如 "audited_amount", "investor_row1_end"
   ...
   * @param itemId - 如 "M2-M2-1-audited_amount"
  ...
  // ─── writebackTB (回写审定数到 trial_balance 4001) ─────────────────────────

  /**
   * 回写审定数到 trial_balance（科目 4001 实收资本/股本，贷方/权益类）。
   *
   * 权益类方向铁律：
   * - 期末 = 期初 + 贷方(增资) - 借方(减资)
   * - 回写的 audited_amount 代表期末余额（正数=实收资本余额）
   *
  ...
  // publish_confirmed/无幂等），且由 useM2Adjudication watcher 数据变化时自动触发
  // （违反 Req 1）。现改为显式发布门：仅在用户显式确认（publishToTb 二次确认）后调用 →
