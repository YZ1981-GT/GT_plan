/**
 * useD3FormData — D3 预收账款数据加载/debounce保存/即时保存/批量保存
 *
 * Spec: .kiro/specs/d3-prepaid-accounts/
 * Task: 3.1
 *
 * 职责：
 * - 从 GET /api/workpapers/:wpId/checklist-responses 加载 D3-* 数据
 * - debounce 2s 文本字段保存（per item_id 独立计时器）
 * - 结论/状态/选择类字段立即保存（saveImmediate）
 * - 批量保存（saveBatch）
 * - 组件卸载时 flush 未保存数据（onScopeDispose）
 * - trial_balance 回写：writebackTrialBalance（科目2203）
 * - selfLoad逻辑（render-config?force_component_type=d3-prepaid-accounts）
...
export function useD3FormData(options: UseD3FormDataOptions) {
  ...
  async function loadResponses(): Promise<void> {
    ...
    try {
      ...
      ElMessage.warning('D3数据加载失败，可手动填写')
    }
  }
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      await api.get(
        `/api/workpapers/${wpId.value}/render-config?force_component_type=d3-prepaid-accounts`,
        { _silent: true } as any,
      )
    } catch {
