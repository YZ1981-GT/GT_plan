/**
 * useK8FormData — K8 销售费用 selfLoad/checklist_responses/writebackTB(6601 发生额！)
 *
 ...
 * - loadTbData(): 从 trial_balance 获取科目 6601 发生额数据（损益类！）
 * - writebackTB(): 审定数回写 trial_balance(6601) + EventBus 'substantive:adjudicated'
 * - checklist_responses 持久化: GET/PUT /api/workpapers/:wpId/checklist-responses
 * - item_id 命名: 前缀 "K8-{sheet}-{field}"
 * - debounce/即时保存: 文本字段 debounce 2s，枚举/结论即时保存
 * - 组件卸载时 flush 未保存数据（onScopeDispose）
 *
 * 科目：6601 销售费用（**损益类/借方科目**）
 * ⚠️ 损益类！取发生额非余额！
...
/**
 * 损益类TB数据结构（发生额！非余额）
 * 6601销售费用：借方=费用增加，贷方=费用冲回/红冲
 */
export interface K8TbData {
  /** 未审借方发生额（费用增加） */
...
export function useK8FormData(opts: {
  wpId: Ref<string>
   ...
   * @param itemId 完整 item_id（如 "K8-1-audited-total"）
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      ...
      for (const r of responses) {
        if (r.item_id?.startsWith('K8-') || r.item_id?.startsWith('K8A-')) {
  ...
  /**
   * 从 trial_balance / tb_ledger 获取科目 6601 的发生额数据。
   * ⚠️ 损益类！取发生额非期末余额！
   *
  ...
  async function loadTbData(): Promise<void> {
    ...
    try {
      const res = await api.get(`/api/projects/${projectId.value}/trial-balance`, {
        params: { account_prefix: ACCOUNT_CODE_6601 },
        _silent: true,
      } as any)
      ...
      for (const item of list) {
        ...
        if (code.startsWith(ACCOUNT_CODE_6601)) {
          // 损益类：取发生额字段
      ...
      if (!found) {
        ElMessage.warning('科目6601销售费用未在试算表中找到，发生额显示为0')
      }
    } catch {
      tbData.value = {
        unadjustedDebit: 0,
   ...
   */
  function setTbValues(values: Partial<K8TbData>): void {
