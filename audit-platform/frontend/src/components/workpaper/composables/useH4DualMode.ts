/**
 * useH4DualMode — H4 工程物资 HTML ↔ OnlyOffice 双模式切换
 *
...
const STORAGE_PREFIX = 'h4-dual-mode:'

export interface UseH4DualModeOptions {
  wpId: Ref<string>
...
export function useH4DualMode(options: UseH4DualModeOptions) {
  const { wpId, autoSave, reloadAll } = options

  const currentMode = ref<H4RenderMode>('html')
  ...
  function persistMode(mode: H4RenderMode): void {
    try {
  ...
  async function switchMode(target: H4RenderMode): Promise<void> {
  ...
  function onModeChange(val: string | number | boolean): void {
    void switchMode(val as H4RenderMode)
  }
