/**
 * useM10DualMode — M10 其他权益工具双模式切换 composable
 *
...
/** M10 渲染模式 */
export type M10RenderMode = 'html' | 'onlyoffice'

/** el-segmented 选项 */
export interface M10ModeOption {
  label: string
  value: M10RenderMode
  disabled?: boolean
}

export interface UseM10DualModeOptions {
  wpId: Ref<string>
  defaultMode?: M10RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key prefix（按 wpId 存储） */
const STORAGE_KEY_PREFIX = 'm10-dual-mode'
 ...
 */
export function useM10DualMode(options: UseM10DualModeOptions) {
  const { wpId, defaultMode = 'html' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<M10RenderMode>(defaultMode)
  ...
  function _loadPersistedMode(): M10RenderMode {
    try {
  ...
  function _persistMode(m: M10RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: M10RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<M10ModeOption[]>(() => [
