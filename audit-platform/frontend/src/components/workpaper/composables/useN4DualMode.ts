/**
 * useN4DualMode — N4 税金及附加双模式切换 composable
 *
...
/** N4 渲染模式 */
export type N4RenderMode = 'html' | 'onlyoffice'

/** el-segmented 选项 */
export interface N4ModeOption {
  label: string
  value: N4RenderMode
  disabled?: boolean
}

export interface UseN4DualModeOptions {
  wpId: Ref<string>
  defaultMode?: N4RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key */
const STORAGE_KEY = 'n4-dual-mode'
 ...
 */
export function useN4DualMode(options: UseN4DualModeOptions) {
  const { wpId, defaultMode = 'html' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const currentMode = ref<N4RenderMode>(defaultMode)
  const isOOHealthy = ref<boolean>(false)
  const ooChecking = ref(false)

  // ─── localStorage 持久化 ───────────────────────────────────────────────

  function _loadPersistedMode(): N4RenderMode {
    try {
  ...
  function _persistMode(m: N4RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: N4RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modes = computed<N4ModeOption[]>(() => [
