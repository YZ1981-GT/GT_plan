/**
 * useG6EclAiGenerate — G6 其他债权投资(ECL) AI 辅助
 * POST /api/workpapers/{wpId}/g6-ecl/ai/{section}
 */
import { onMounted, ref, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type G6EclAiSection =
  | 'stage-conclusion'
...
export function useG6EclAiGenerate(wpId: Ref<string>) {
  ...
  async function generateAndConfirm(
    ...
    try {
      const response = await http.post(
        `/api/workpapers/${wpId.value}/g6-ecl/ai/${section}`,
...
export default useG6EclAiGenerate
