/**
 * useL8DualMode — L8 财务费用双模式切换 composable
 *
...
/** L8 渲染模式 */
export type L8RenderMode = 'structured' | 'onlyoffice'

/** el-segmented 选项 */
export interface L8ModeOption {
  label: string
  value: L8RenderMode
  disabled?: boolean
}

export interface UseL8DualModeOptions {
  wpId: Ref<string>
  defaultMode?: L8RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key prefix（按 wpId 存储） */
const STORAGE_KEY_PREFIX = 'l8-dual-mode'
 ...
 */
export function useL8DualMode(options: UseL8DualModeOptions) {
  const { wpId, defaultMode = 'structured' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<L8RenderMode>(defaultMode)
  ...
  function _loadPersistedMode(): L8RenderMode {
    try {
  ...
  function _persistMode(m: L8RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: L8RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<L8ModeOption[]>(() => [
