/**
 * useK13DualMode — K13 营业外支出 HTML ↔ OnlyOffice 双模式（比照 useD4DualMode / useK10DualMode）
 *
 ...
 *
 * 2026-07-23 重写：原实现用裸 fetch（无鉴权→登录态 401）+ 只 health 检查、无 config 预拉、
 *   无 ooConfig，未达 D4「拉取成功才可以」标准（K10/K12 已修，此为对齐）。
 *
 * Spec: .kiro/specs/k13-non-operating-expense/ + K13 复盘 P0-1
 */
  ...
  async function switchMode(target: K13RenderMode): Promise<void> {
    ...
    if (target === 'onlyoffice') {
      ...
      try {
        ...
        // 不置 isOoAvailable=false —— 单个 sheet 失败不应全局禁用在线编辑。
        ooConfig.value = null
