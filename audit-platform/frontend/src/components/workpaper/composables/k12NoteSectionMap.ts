/**
 * K12 营业外收入披露 ↔ 附注章节映射与 sync payload 构建
 * 权威来源：note_template_variant_matrix.json ying_ye_wai_shou_ru
 * listed: null (关键词 '营业外收入') / soe: 八、76
 * account_code: 6301
 * 附注表结构：单表「营业外收入」(项目/本期发生额/上期发生额)
 */
import type { ColumnDef } from './disclosureColumnDefs'

export type K12DisclosureVariant = 'listed' | 'soe'
...
export interface K12SyncPayload {
  wp_id: string
  sheet_name: string
  section_id: string
...
function resolveCurrentStandard(variant: K12DisclosureVariant): string {
...
export function buildK12SyncPayload(
  ...
  if (narrativeText.trim()) {
    sub._note_texts = [{ section: 'note-non-operating-income', title: '营业外收入说明', text: narrativeText.trim() }]
  }

  return {
    wp_id: wpId,
    sheet_name: K12_DISCLOSURE_SHEET_NAME[variant],
    section_id: K12_NOTE_SECTION[variant],
