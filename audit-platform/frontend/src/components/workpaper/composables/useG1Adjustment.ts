/**
 * useG1Adjustment — G1-3 调整分录汇总（对齐 D4-4 列结构 + 调整分录模块联动）
 */
...
/** 与 Excel G1-3 / D4-4 对齐的列 */
export interface G1AdjustmentRow {
  rowId: string
  description: string
  category: string
  reportItem: string
...
const STORAGE_KEY = 'G1-3-rows'
const BALANCE_TOLERANCE = 0.005

const G1_RELATED_PREFIXES = ['1501', '6101', '6111', '1503']

function generateRowId(): string {
  return `g1a-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 9)}`
}

export function createEmptyG1AdjustmentRow(): G1AdjustmentRow {
  return {
    rowId: generateRowId(),
    description: '',
    category: '账项调整',
...
function isG1RelatedAccount(code: string): boolean {
  const c = String(code || '')
  return G1_RELATED_PREFIXES.some((p) => c === p || c.startsWith(p))
}
...
export function useG1Adjustment(opts: {
  ...
  const rows = ref<G1AdjustmentRow[]>([])
  ...
  /** 影响 1501 的净调整（借−贷，资产增加为正） */
  const netAjeToG1 = computed(() => {
    let net = 0
    for (const r of rows.value) {
      if (!isG1RelatedAccount(r.accountCode) && !String(r.accountName).includes('交易性金融')) continue
  ...
  function addRow(): void {
    if (opts.isReadonly.value) return
    rows.value = [...rows.value, createEmptyG1AdjustmentRow()]
    persistRows()
  }
  ...
  function updateCell(rowId: string, field: keyof G1AdjustmentRow, value: unknown): void {
    if (opts.isReadonly.value) return
  ...
  function publishAdjustment(): void {
        ...
        window.dispatchEvent(
          new CustomEvent('adjustment:created', {
            detail: {
              ...
              accountCode: row.accountCode || G1_ACCOUNT_CODE,
    ...
    // 直接回写 G1-1 审定表（即使审定表未挂载也能落库）
    const net = netAjeToG1.value
    try {
      const raw = opts.allResponses.value.get('G1-1-rows')?.remark
      const store = applyG1AdjustmentWriteback(parseG1AdjStore(raw), net, G1_ADJ_WRITEBACK_ROW_KEY)
      opts.debouncedSave('G1-1-rows', { remark: JSON.stringify(store) })
    } catch {
      /* silent */
    }
    ...
    try {
      window.dispatchEvent(
        new CustomEvent('g1:adjustment-confirmed', {
          detail: {
            accountCode: G1_ACCOUNT_CODE,
            netAdjustment: net,
            rowKey: G1_ADJ_WRITEBACK_ROW_KEY,
  ...
  async function syncFromAdjustmentModule(): Promise<number> {
    ...
    try {
      ...
      const imported: G1AdjustmentRow[] = []
      for (const entry of items) {
        const lines = entry.line_items || entry.lines || []
        const related = lines.filter((li: any) =>
          isG1RelatedAccount(li.standard_account_code || li.account_code || ''),
        )
        ...
        for (const li of related) {
          ...
          imported.push({
            ...
            indexRef: entry.adjustment_no || 'G1-3',
            remark: '来自调整分录模块',
      ...
      if (!imported.length) {
        lastSyncMsg.value = '调整分录模块中无与 1501 等相关的分录'
        return 0
      }
  ...
  return {
    rows: rows as Ref<G1AdjustmentRow[]>,
