/**
 * useK1AiGenerate — K1 其他应收款 AI 辅助生成（POST /k1/ai-generate）
 */
import { ref, onMounted, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type K1AiSection =
  | 'policy-check'
  | 'large-amount-eval'
  | 'overdue-eval'
  | 'overall-opinion'
...
export function useK1AiGenerate(wpId: Ref<string>) {
  ...
  async function generate(params: K1AiGenerateParams): Promise<string> {
    const res = await http.post(
      `/api/workpapers/${wpId.value}/k1/ai-generate`,
      {
...
export default useK1AiGenerate
