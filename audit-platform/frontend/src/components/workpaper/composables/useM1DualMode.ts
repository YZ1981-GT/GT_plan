/**
 * useM1DualMode — M1 应付股利（利润）双模式切换 composable
 *
...
/** M1 渲染模式 */
export type M1RenderMode = 'structured' | 'onlyoffice'

/** el-segmented 选项 */
export interface M1ModeOption {
  label: string
  value: M1RenderMode
  disabled?: boolean
}

export interface UseM1DualModeOptions {
  wpId: Ref<string>
  defaultMode?: M1RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key prefix（按 wpId 存储） */
const STORAGE_KEY_PREFIX = 'm1-dual-mode'
 ...
 */
export function useM1DualMode(options: UseM1DualModeOptions) {
  const { wpId, defaultMode = 'structured' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<M1RenderMode>(defaultMode)
  ...
  function _loadPersistedMode(): M1RenderMode {
    try {
  ...
  function _persistMode(m: M1RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: M1RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<M1ModeOption[]>(() => [
