/**
 * useI5Adjustment — I5-3 其他非流动资产调整分录汇总
 *
 * 对齐致同 Excel「其他非流动资产调整分录汇总表 I5-3」：
 * 调整事项说明 / 类别(账项·报表·其他) / 报表项目 / 科目名称 / 附注项目 /
 * 借方调整金额 / 贷方调整金额 / 索引 / 备注
 * + 明细项目（供 I5-1/I5-2 精确匹配）
 *
 * 联动：保存 → i5:adjustments-changed → I5-1 AJE/RJE；账项可推送 A13
 */
import {
  ref,
  computed,
...
export type I5AdjCategory = '账项调整' | '报表调整' | '其他'

export interface I5AdjustmentRow {
  rowId: string
  seq: number
  description: string
  category: I5AdjCategory | string
  entryType: 'AJE' | 'RJE' | ''
  reportItem: string
  accountCode: string
  accountName: string
...
const ROWS_KEY = 'I5-3-rows'
const NOTE_KEY = 'I5-3-audit-note'
const CONCLUSION_KEY = 'I5-3-audit-conclusion'
const LEGACY_NOTE_KEY = 'I5-adjustment-audit-note'
const LEGACY_CONCLUSION_KEY = 'I5-adjustment-audit-conclusion'
const AJE_NET_KEY = 'I5-3-aje-net'
const RJE_NET_KEY = 'I5-3-rje-net'
const WP_CODE = 'I5'
const BALANCE_TOLERANCE = 0.01

const I5_RELATED_PREFIXES = ['1911', '1461', '6602', '5602', '6401', '1002', '2202', '1123', '1221']

export const I5_CATEGORY_OPTIONS: readonly I5AdjCategory[] = ['账项调整', '报表调整', '其他']

export const I5_ADJ_ACCOUNT_OPTIONS = [
  { code: '1911', name: '其他非流动资产' },
...
function generateRowId(): string {
  return `i5-adj-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
}
...
export function categoryFromLegacy(raw: any): I5AdjCategory {
...
function isOnaAccount(code: string): boolean {
  return String(code || '').startsWith('1911')
}

function isI5RelatedAccount(code: string): boolean {
  const c = String(code || '')
  return I5_RELATED_PREFIXES.some((p) => c.startsWith(p))
}

function reportItemForAccount(code: string): string {
  if (isOnaAccount(code)) return '其他非流动资产'
  if (String(code).startsWith('1461')) return '一年内到期的非流动资产'
  if (String(code).startsWith('6602') || String(code).startsWith('5602')) return '管理费用'
  if (String(code).startsWith('6401')) return '营业成本'
  if (String(code).startsWith('1123')) return '预付款项'
  return ''
}

/** 一年内到期重分类 RJE：借 1461 / 贷 1911 */
export function buildI5CurrentPortionReclassDraft(opts: {
  projectName: string
  amount: number
  indexRef?: string
}): I5AdjustmentRow[] {
  ...
  return [
    ...
    },
    {
      ...
      creditAmount: amt,
      indexRef: idx,
      remark: '贷记其他非流动资产（报表重分类转出）',
    },
  ]
}

/** 费用化/冲减：借费用 / 贷 1911 */
export function buildI5ExpenseReclassDraft(opts: {
  projectName: string
  amount: number
  ...
  const amt = Math.round((opts.amount || 0) * 100) / 100
  if (amt <= 0.005) return []
  const desc = `其他非流动资产费用化-${opts.projectName || '未命名项目'}`
  const idx = opts.indexRef || 'I5-4'
  const expCode = opts.expenseAccountCode || '6602'
  const known = I5_ADJ_ACCOUNT_OPTIONS.find((o) => o.code === expCode)
...
export function useI5Adjustment(params: {
  wpId: Ref<string>
  ...
  const rows = ref<I5AdjustmentRow[]>([])
  const auditNote = ref('')
  ...
  function _normalizeRow(raw: any, idx: number): I5AdjustmentRow {
    const category = categoryFromLegacy(raw)
    const entryType = entryTypeFromCategory(category)
    const accountCode = String(raw.accountCode ?? '1911')
    ...
    return {
      ...
      noteItem: String(raw.noteItem ?? (isOnaAccount(accountCode) ? '其他非流动资产' : '')),
      debitAmount,
      creditAmount,
  ...
  function _netsFromList(list: I5AdjustmentRow[]) {
    let ajeNet = 0
    let rjeNet = 0
    let onaAje = 0
    let onaRje = 0
    for (const r of list) {
      ...
      if (isOnaAccount(r.accountCode)) {
        if (isRje) onaRje += net
        else onaAje += net
      }
    }
    return { ajeNet, rjeNet, onaAje, onaRje }
  }

  const ajeNet = computed(() => _netsFromList(rows.value).ajeNet)
  const rjeNet = computed(() => _netsFromList(rows.value).rjeNet)
  const onaAjeNet = computed(() => _netsFromList(rows.value).onaAje)
  const onaRjeNet = computed(() => _netsFromList(rows.value).onaRje)
  ...
  const projectNameOptions = computed(() => {
    ...
    const detail = _getJson('I5-2-rows')
    if (Array.isArray(detail)) {
      for (const d of detail) {
        const n = String(d.projectName ?? d.name ?? '').trim()
        if (n && n !== '合计') names.add(n)
      }
    }
    const adj = _getJson('I5-adj-rows')
    if (Array.isArray(adj)) {
      for (const d of adj) {
        const n = String(d.projectName ?? '').trim()
        if (n && n !== '合计') names.add(n)
      }
    }
    return [...names]
  ...
  function _writeNets(n: ReturnType<typeof _netsFromList>) {
    if (!onSave) return
    onSave(AJE_NET_KEY, n.onaAje)
    onSave(RJE_NET_KEY, n.onaRje)
    for (const [key, val] of [[AJE_NET_KEY, n.onaAje], [RJE_NET_KEY, n.onaRje]] as const) {
  ...
  function _persist(list?: I5AdjustmentRow[]): void {
    if (!onSave) return
    ...
    try {
      window.dispatchEvent(new CustomEvent('i5:adjustments-changed', {
        detail: { source: 'I5-3', rowCount: normalized.length },
      }))
    } catch { /* silent */ }
  }

  function addRow(partial?: Partial<I5AdjustmentRow>): void {
    if (isReadonly.value) return
    const category = (partial?.category as I5AdjCategory) || '账项调整'
    const accountCode = partial?.accountCode || '1911'
    const known = I5_ADJ_ACCOUNT_OPTIONS.find((o) => o.code === accountCode)
    rows.value.push(_normalizeRow({
      ...
      accountCode,
      accountName: partial?.accountName || known?.name || '其他非流动资产',
      noteItem: partial?.noteItem || '其他非流动资产',
  ...
  function updateCell(rowId: string, field: string, value: any): void {
    ...
    if (field === 'category') {
      ...
      const known = I5_ADJ_ACCOUNT_OPTIONS.find((o) => o.code === row.accountCode)
      if (known) row.accountName = known.name
      if (!row.reportItem) row.reportItem = reportItemForAccount(row.accountCode)
      if (isOnaAccount(row.accountCode) && !row.noteItem) row.noteItem = '其他非流动资产'
  ...
  function publishAdjustment(): void {
    ...
    try {
      eventBus.emit('adjustment:created', {
        wpCode: WP_CODE,
        entryType: 'AJE',
        amount: Math.abs(nets.onaAje),
        accountCode: '1911',
        accountName: '其他非流动资产',
        description: `I5-3 其他非流动资产账项净额 ${nets.onaAje}`,
        ...payload,
      } as any)
  ...
  function pushToA13(rowIds?: string[]): void {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('a13:push-misstatement', {
        detail: { wpCode: WP_CODE, source: 'I5-3', entries: items },
      }))
  ...
  async function pushToAdjustmentModule(): Promise<number> {
    ...
    const groups = new Map<string, I5AdjustmentRow[]>()
      ...
      try {
        ...
        const res: any = await api.post(
          adjPaths.create(projectId),
          {
            ...
            description: `[I5] ${lines[0].description || '其他非流动资产调整'}`,
            line_items: lines.map((r) => ({
              standard_account_code: r.accountCode || '1911',
  ...
  async function syncFromAdjustmentModule(): Promise<number> {
    ...
    try {
      ...
      const imported: I5AdjustmentRow[] = []
      ...
      for (const item of items) {
        ...
        const hasRelated = lines.some((li: any) =>
          isI5RelatedAccount(li.standard_account_code || li.account_code),
        )
        if (!hasRelated) continue
        const hasCore = lines.some((li: any) =>
          isOnaAccount(String(li.standard_account_code || li.account_code || '')),
        )
        if (!hasCore) continue
      ...
      if (!imported.length) {
        lastSyncMsg.value = '调整分录模块中无含 1911 其他非流动资产的相关分录'
        return 0
      }
  ...
  function seedCurrentPortionReclass(
    candidates: Array<{ projectName: string; amount: number; indexRef?: string; remainingMonths?: number }>,
  ): number {
    if (isReadonly.value) return 0
    let added = 0
    const existingDescs = new Set(rows.value.map((r) => r.description))
    for (const c of candidates) {
      const lines = buildI5CurrentPortionReclassDraft(c)
      if (!lines.length) continue
      if (existingDescs.has(lines[0].description)) continue
      const monthsHint = c.remainingMonths != null ? `剩余${c.remainingMonths}月` : ''
      rows.value.push(...lines.map((l, i) => _normalizeRow({
        ...l,
        remark: monthsHint ? `${l.remark}；${monthsHint}` : l.remark,
      }, rows.value.length + i)))
  ...
  /** 从 I5-2 明细按到期日/剩余月数筛选一年内到期候选并生成 RJE 草稿 */
  function seedCurrentPortionFromDetail(
    detailRows: any[],
    asOfYear?: number,
  ): number {
    if (isReadonly.value) return 0
    const candidates = extractI5CurrentPortionCandidates(detailRows, asOfYear)
    if (!candidates.length) return 0
    return seedCurrentPortionReclass(
      candidates.map((c) => ({
  ...
  function seedExpenseReclass(opts: {
    ...
    if (isReadonly.value) return 0
    const lines = buildI5ExpenseReclassDraft(opts)
  ...
  function suggestConclusionTemplate(): string {
    if (!rows.value.length) {
      return '本期其他非流动资产未见需调整事项，调整分录汇总表无分录。'
    }
    if (!isBalanced.value) {
      return '调整分录尚未借贷平衡，请先修正后再形成结论。'
    }
    const aje = onaAjeNet.value
    const rje = onaRjeNet.value
    return `上述其他非流动资产相关调整分录依据充分、整表借贷平衡；1911 账项净额 ${aje.toLocaleString('zh-CN')}、报表重分类净额 ${rje.toLocaleString('zh-CN')}，已回写 I5-1 / 可推送 A13。`
  }
  ...
  return {
    ...
    onaRjeNet,
    projectNameOptions,
    categoryOptions: I5_CATEGORY_OPTIONS,
    accountOptions: [...I5_ADJ_ACCOUNT_OPTIONS],
    addRow,
    removeRow,
    updateCell,
    saveNote,
...
export default useI5Adjustment
