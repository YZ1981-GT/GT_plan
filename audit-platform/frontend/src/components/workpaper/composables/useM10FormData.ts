/**
 * useM10FormData — M10 其他权益工具数据加载/debounce保存/即时保存/TB回写
 *
 ...
 * - item_id 命名: 前缀 "M10-{sheet}-{field}"（如 "M10-1-adjudicated-amount", "M10-2-detail-begin"）
 * - writebackTB(4003): 审定数回写 trial_balance 科目 4003 其他权益工具
...
export interface UseM10FormDataOptions {
  wpId: Ref<string>
...
const ITEM_PREFIX = 'M10-'
const ACCOUNT_CODE = '4003' // 其他权益工具（贷方/权益类！）
/** 审定表 sheet 名（含子码 M10-1，供后端 extract_determination_wp_code 解出） */
const DETERMINATION_SHEET_NAME = '审定表M10-1'

// ─── Composable ──────────────────────────────────────────────────────────────

export function useM10FormData(options: UseM10FormDataOptions) {
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      await api.get(
        `/api/workpapers/${wpId.value}/render-config?force_component_type=m10-other-equity-instruments`,
        { _silent: true } as any,
      )
    } catch {
  ...
  /**
   * 从 checklist_responses 加载 M10 数据（item_id 前缀 "M10-"）
   */
  async function loadResponses(): Promise<void> {
    ...
    try {
      ...
      ElMessage.warning('M10数据加载失败，可手动填写')
    }
  }
  ...
  /**
   * 获取指定 sheet + field 的值
   * @param sheet - 如 "1", "2", "3", "4", "5"（不含M10-前缀，最终item_id为 "M10-1-xxx"）
   * @param field - 如 "adjudicated-amount", "detail-begin", "classification-result"
   ...
   * @param field - 如 "adjudicated-amount", "detail-end", "classification-conclusion"
   ...
   * @param itemId - 如 "M10-1-adjudicated-amount"
  ...
  // ─── writebackTB (回写审定数到 trial_balance 4003) ─────────────────────────

  /**
   * 回写审定数到 trial_balance（科目 4003 其他权益工具，**贷方/权益类！**）。
   *
   * 权益类贷方方向铁律：
   * - 期末 = 期初 + 贷方(增加：发行) - 借方(减少：赎回/转换)
   * - 回写的 audited_amount 代表期末余额（正数=其他权益工具贷方余额）
   * - 永续债发行计入贷方（增加权益），赎回计入借方（减少权益）
   * - 优先股发行计入贷方（增加权益），转换/赎回计入借方（减少权益）
   *
   * 并发布 EventBus 'substantive:adjudicated' 事件通知其他组件（附注刷新等）。
   */
  // spec: tb-writeback-explicit-publish-gate Task 5 / Req 1,2,8。改走显式发布门：
  // 仅在用户显式确认（useM10Adjudication.publishToTb 二次确认）后调用 →
