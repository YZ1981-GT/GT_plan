...
export function useG6OthBonFormData(opts: { wpId: Ref<string>; projectId: Ref<string> }) {
  ...
  async function loadResponses() {
    ...
    try {
      ...
      ElMessage.warning('G6数据加载失败，可手动填写')
    }
  }
      ...
      await Promise.all([
        ...
        (async () => {
          const res = await api.get(`/api/workpapers/${opts.wpId.value}/render-config`, {
            params: { force_component_type: 'g6-other-bond-investment-main' }, _silent: true,
          } as any)
