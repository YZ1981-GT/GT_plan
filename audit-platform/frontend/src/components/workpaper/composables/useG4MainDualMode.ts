/**
 * useG4MainDualMode — G4 债权投资(main组) HTML ↔ OnlyOffice 双模式切换
 *
...
const STORAGE_PREFIX = 'g4-main-dual-mode:'

export interface UseG4MainDualModeOptions {
  wpId: Ref<string>
  sheetName?: Ref<string>
  reloadAll?: () => Promise<void>
}

export function useG4MainDualMode(options: UseG4MainDualModeOptions) {
  const { wpId, reloadAll } = options

  const currentMode = ref<G4MainRenderMode>('html')
  ...
  function persistMode(mode: G4MainRenderMode): void {
    try {
  ...
  function onModeChange(val: string | number | boolean): void {
    void switchMode(val as G4MainRenderMode)
  }
...
export default useG4MainDualMode
