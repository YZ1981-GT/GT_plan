/**
 * useM5DualMode — M5 盈余公积双模式切换 composable
 *
...
/** M5 渲染模式 */
export type M5RenderMode = 'html' | 'onlyoffice'

/** el-segmented 选项 */
export interface M5ModeOption {
  label: string
  value: M5RenderMode
  disabled?: boolean
}

export interface UseM5DualModeOptions {
  wpId: Ref<string>
  defaultMode?: M5RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key prefix（按 wpId 存储） */
const STORAGE_KEY_PREFIX = 'm5-dual-mode'
 ...
 */
export function useM5DualMode(options: UseM5DualModeOptions) {
  const { wpId, defaultMode = 'html' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<M5RenderMode>(defaultMode)
  ...
  function _loadPersistedMode(): M5RenderMode {
    try {
  ...
  function _persistMode(m: M5RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: M5RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<M5ModeOption[]>(() => [
