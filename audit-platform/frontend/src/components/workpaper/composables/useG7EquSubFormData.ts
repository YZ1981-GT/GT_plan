...
export function useG7EquSubFormData(opts: { wpId: Ref<string>; projectId: Ref<string> }) {
  ...
  async function loadResponses() {
    ...
    try {
      ...
      ElMessage.warning('G7数据加载失败，可手动填写')
    }
  }
      ...
      await Promise.all([
        ...
        (async () => {
          const res = await api.get(`/api/workpapers/${opts.wpId.value}/render-config`, {
            params: { force_component_type: 'g7-long-term-equity-subsidiary' }, _silent: true,
          } as any)
