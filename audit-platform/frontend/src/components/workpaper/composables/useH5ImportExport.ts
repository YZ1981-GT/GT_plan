/**
 * useH5ImportExport — H5 油气资产 导入导出 composable
 *
 * Spec: .kiro/specs/h5-oil-gas-assets/ Task 3.3
 * Requirements: 3.3, 7.2-7.3
 *
 * 动态行表格 × 3端点：
 *   POST /api/workpapers/{wpId}/h5/export-template
 *   POST /api/workpapers/{wpId}/h5/export-data
 *   POST /api/workpapers/{wpId}/h5/import-data  (multipart/form-data)
 *
 * H5-2 明细表（动态行）需要导入导出
 *
...
/** H5 支持导入导出的 sheet 编码（H5-2 明细表，动态行） */
export type H5ImportableSheet = 'H5-2'

/** 导入结果 */
export interface H5ImportResult {
  rowCount: number
  fieldCount: number
  warning?: string
  errors?: string[]
...
/** sheet 元数据（供下拉菜单展示） */
export interface H5SheetMeta {
  code: H5ImportableSheet
  label: string
}

// ═══ 常量 ═══

export const H5_API_PREFIX = 'h5'

/** 可导入导出 sheet 的中文标签（H5-2 明细表，动态行） */
export const H5_IMPORT_EXPORT_SHEETS: H5SheetMeta[] = [
  { code: 'H5-2', label: 'H5-2 明细表' },
]
 ...
 */
export function resolveH5ImportableSheet(sheetName: string): H5ImportableSheet | null {
  const m = (sheetName || '').match(/H5-(\d+)/)
  if (!m) return null
  const code = `H5-${m[1]}` as H5ImportableSheet
  return H5_IMPORT_EXPORT_SHEETS.some((s) => s.code === code) ? code : null
}

// ═══ Composable 接口 ═══

export interface UseH5ImportExportOptions {
  wpId: Ref<string>
  sheetCode: string
}

export interface UseH5ImportExportReturn {
  /** 是否正在导出中 */
  ...
  importData: (file: File) => Promise<H5ImportResult | null>
  /** API前缀 */
  apiPrefix: string
}
...
export function useH5ImportExport(params: UseH5ImportExportOptions): UseH5ImportExportReturn {
  ...
  /**
   * 导出模板 — POST /api/workpapers/{wpId}/h5/export-template
   */
  ...
  /**
   * 导出数据 — POST /api/workpapers/{wpId}/h5/export-data
   */
  ...
  /**
   * 导入数据 — POST /api/workpapers/{wpId}/h5/import-data
   * multipart/form-data 上传 xlsx 文件, body 含 sheet_code
   */
  async function importData(file: File): Promise<H5ImportResult | null> {
    lastError.value = null
    ...
    try {
      ...
      const response = await http.post(`${apiBase()}/h5/import-data`, formData, {
  ...
  return {
    ...
    isImporting,
    lastError,
    sheets: H5_IMPORT_EXPORT_SHEETS,
    exportTemplate,
    exportData,
    importData,
    apiPrefix: H5_API_PREFIX,
  }
}

export default useH5ImportExport
