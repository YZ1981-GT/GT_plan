/**
 * useD5AiGenerate — D5 应收款项融资 AI 辅助生成（POST /d5/ai-generate）
 */
import { ref, onMounted, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type D5AiSection =
  | 'adj-change-analysis'
  | 'adj-conclusion'
  | 'analysis-note'
  | 'detail-change'
...
export interface D5AiGenerateParams {
  section: D5AiSection
  existingContent?: string
  relatedContext?: Record<string, unknown>
}

export function useD5AiGenerate(wpId: Ref<string>) {
  ...
  async function generate(params: D5AiGenerateParams): Promise<string> {
    const res = await http.post(
      `/api/workpapers/${wpId.value}/d5/ai-generate`,
      {
...
export default useD5AiGenerate
