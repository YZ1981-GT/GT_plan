/**
 * useM8DualMode — M8 一般风险准备双模式切换 composable
 *
...
/** M8 渲染模式 */
export type M8RenderMode = 'html' | 'onlyoffice'

/** el-segmented 选项 */
export interface M8ModeOption {
  label: string
  value: M8RenderMode
  disabled?: boolean
}

export interface UseM8DualModeOptions {
  wpId: Ref<string>
  defaultMode?: M8RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key prefix（按 wpId 存储） */
const STORAGE_KEY_PREFIX = 'm8-dual-mode'
 ...
 */
export function useM8DualMode(options: UseM8DualModeOptions) {
  const { wpId, defaultMode = 'html' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<M8RenderMode>(defaultMode)
  ...
  function _loadPersistedMode(): M8RenderMode {
    try {
  ...
  function _persistMode(m: M8RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: M8RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<M8ModeOption[]>(() => [
