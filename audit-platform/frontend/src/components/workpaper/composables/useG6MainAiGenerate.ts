/**
 * useG6MainAiGenerate — G6 其他债权投资(main) AI 辅助
 * POST /api/workpapers/{wpId}/g6-main/ai/{section}
 */
import { onMounted, ref, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type G6MainAiSection =
...
export function useG6MainAiGenerate(wpId: Ref<string>) {
  ...
  async function generateAndConfirm(
    ...
    try {
      const response = await http.post(
        `/api/workpapers/${wpId.value}/g6-main/ai/${section}`,
...
export default useG6MainAiGenerate
