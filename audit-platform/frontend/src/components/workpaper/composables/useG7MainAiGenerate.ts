/**
 * useG7MainAiGenerate — G7 长期股权投资(main) AI 辅助
 * POST /api/workpapers/{wpId}/g7-main/ai/{section}
 */
import { onMounted, ref, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type G7MainAiSection = 'adjudication-analysis' | 'disclosure-text'

export function useG7MainAiGenerate(wpId: Ref<string>) {
  ...
  async function generateAndConfirm(
    ...
    try {
      const response = await http.post(
        `/api/workpapers/${wpId.value}/g7-main/ai/${section}`,
...
export default useG7MainAiGenerate
