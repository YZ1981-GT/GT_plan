/**
 * useF4FormData — F4 应付账款数据加载/debounce保存/即时保存/批量保存
 *
 * Spec: .kiro/specs/f4-accounts-payable/ Task 3.1
 * 比照 useF3FormData
 */
...
export interface UseF4FormDataOptions {
  wpId: Ref<string>
  projectId: Ref<string>
}

export function useF4FormData(options: UseF4FormDataOptions) {
  ...
  async function loadResponses(): Promise<void> {
    ...
    try {
      ...
      for (const r of responses) {
        if (r.item_id?.startsWith('F4-') || r.item_id?.startsWith('F4A') || r.item_id?.startsWith('F4-note-')) {
          map.set(r.item_id, {
            item_id: r.item_id,
      ...
      ElMessage.warning('F4数据加载失败，可手动填写')
    }
  }
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      const res = await api.get(
        `/api/workpapers/${wpId.value}/render-config?force_component_type=f4-accounts-payable`,
        { _silent: true } as any,
      )
