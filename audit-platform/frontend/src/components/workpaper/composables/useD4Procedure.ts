...
export function useD4Procedure(
  ...
  const overallConclusion = ref<string>(getVal('D4-proc-overall').remark || '')

  const procedureSteps: ComputedRef<ProcedureStep[]> = computed(() =>
    D4_PROCEDURE_STEPS_CONFIG.map((cfg, idx) => {
      const n = idx + 1
      return {
        stepOrder: n,
        stepName: cfg.stepName,
        description: cfg.description,
        status: (getVal(`D4-proc-${n}-status`).conclusion as ProcedureStatus) || '未开始',
        executor: getVal(`D4-proc-${n}-executor`).remark || '',
        executeDate: getVal(`D4-proc-${n}-date`).remark || '',
        wpIndexRef: getVal(`D4-proc-${n}-wpindex`).remark || '',
        findings: getVal(`D4-proc-${n}-findings`).remark || '',
        conclusion: getVal(`D4-proc-${n}-conclusion`).remark || '',
  ...
  function updateStepField(stepIndex: number, field: StepField, value: string): void {
    ...
    const keyMap: Record<StepField, string> = {
      status: `D4-proc-${n}-status`,
      executor: `D4-proc-${n}-executor`,
      executeDate: `D4-proc-${n}-date`,
      wpIndexRef: `D4-proc-${n}-wpindex`,
      findings: `D4-proc-${n}-findings`,
      conclusion: `D4-proc-${n}-conclusion`,
    }
  ...
  function saveOverallConclusion(text: string): void {
    ...
    saveImmediate([setLocal('D4-proc-overall', null, text)])
  }
