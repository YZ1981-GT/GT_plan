/**
 * useK13FormData — K13 营业外支出 selfLoad/checklist_responses/writebackTB(6711 发生额！)
 *
 ...
 * - loadTbData(): 从 trial_balance 获取科目 6711 发生额数据（损益类！）
 * - writebackTB(): 审定数回写 trial_balance(6711) + EventBus 'substantive:adjudicated'
 * - checklist_responses 持久化: GET/PUT /api/workpapers/:wpId/checklist-responses
 * - item_id 命名: 前缀 "K13-{sheet}-{field}"
 * - debounce/即时保存: 文本字段 debounce 2s，枚举/结论即时保存
 * - 组件卸载时 flush 未保存数据（onScopeDispose）
 *
 * 科目：6711 营业外支出（**损益类/借方科目**）
 * ⚠️ 损益类！取发生额非余额！
 * - 6711为借方科目：借方=支出增加，贷方=支出冲回/红冲
 * - 支出类发生额 = 借方发生 - 贷方发生（红冲）
 * - TB回写发生额，非期末余额（期末结转本年利润后为0）
 * - ⚠️ 方向与K12(6301)相反！K12: credit-debit，K13: debit-credit
 */
import { ref, onScopeDispose, type Ref } from 'vue'
import { ElMessage } from 'element-plus'
import { api } from '@/services/apiProxy'
import { parseNum } from './useK13FormulaEngine'
...
/**
 * 损益类TB数据结构（发生额！非余额）
 * 6711营业外支出：借方=支出增加，贷方=支出冲回/红冲
 * ⚠️ 与K12(6301贷方)方向相反：K13净额=借方-贷方
 */
export interface K13TbData {
  /** 未审借方发生额（支出增加：非流动资产处置损失/捐赠支出/罚款滞纳金/债务重组损失/资产盘亏损失等） */
  unadjustedDebit: number
  /** 未审贷方发生额（支出冲回/红冲/期末结转） */
  unadjustedCredit: number
  /** 未审净发生额 = 借方 - 贷方（借方科目！与K12相反） */
...
const ACCOUNT_CODE_6711 = '6711'
const COMPONENT_TYPE = 'k13-non-operating-expense'

// ─── Composable ──────────────────────────────────────────────────────────────

export function useK13FormData(opts: {
  wpId: Ref<string>
   ...
   * @param itemId 完整 item_id（如 "K13-1-audited-total"）
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      ...
      for (const r of responses) {
        if (r.item_id?.startsWith('K13-') || r.item_id?.startsWith('K13A-')) {
  ...
  /**
   * 从 trial_balance / tb_ledger 获取科目 6711 的发生额数据。
   * ⚠️ 损益类！取发生额非期末余额！
   * ⚠️ 6711是借方科目！净额=借方-贷方（与K12相反）
   *
  ...
  async function loadTbData(): Promise<void> {
    ...
    try {
      const res = await api.get(`/api/projects/${projectId.value}/trial-balance`, {
        params: { account_prefix: ACCOUNT_CODE_6711 },
        _silent: true,
      } as any)
      ...
      for (const item of list) {
        ...
        if (code.startsWith(ACCOUNT_CODE_6711)) {
          // 损益类：取发生额字段
      ...
      if (!found) {
        ElMessage.warning('科目6711营业外支出未在试算表中找到，发生额显示为0')
      }
    } catch {
      tbData.value = {
        unadjustedDebit: 0,
  ...
  // 原 writebackTB（PUT /api/projects/{pid}/trial-balance/writeback，科目 6711 营业外支出发生额）
  // 为零消费死代码，已移除；K13 的 TB 回写走显式发布门 publish-to-tb（M6/task9 改造）。
  // 实证：K13TabAdjustment.vue 引入 useK13FormData 仅用其 save/subtotal（不 destructure writebackTB），
  // K13 真实回写路径在 K13TabAdjudication.vue 的 inline handleWritebackTBInternal（非本 FormData.writebackTB）。
   ...
   * ⚠️ 6711借方科目：净额=借方-贷方
   */
  function setTbValues(values: Partial<K13TbData>): void {
