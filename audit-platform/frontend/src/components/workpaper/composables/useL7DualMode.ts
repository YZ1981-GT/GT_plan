/**
 * useL7DualMode — L7 其他非流动负债双模式切换 composable
 *
...
/** L7 渲染模式 */
export type L7RenderMode = 'structured' | 'onlyoffice'

/** el-segmented 选项 */
export interface L7ModeOption {
  label: string
  value: L7RenderMode
  disabled?: boolean
}

export interface UseL7DualModeOptions {
  wpId: Ref<string>
  defaultMode?: L7RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key prefix（按 wpId 存储） */
const STORAGE_KEY_PREFIX = 'l7-dual-mode'
 ...
 */
export function useL7DualMode(options: UseL7DualModeOptions) {
  const { wpId, defaultMode = 'structured' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<L7RenderMode>(defaultMode)
  ...
  function _loadPersistedMode(): L7RenderMode {
    try {
  ...
  function _persistMode(m: L7RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: L7RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<L7ModeOption[]>(() => [
