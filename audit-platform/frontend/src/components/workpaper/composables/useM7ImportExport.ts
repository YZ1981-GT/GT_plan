/**
 * useM7ImportExport — M7 专项储备导入导出三级 composable
 *
 ...
 * - 多sheet导出支持（M7-2 明细表为主要导入导出目标，动态行——只有动态行表格才需要导入导出）:
 *   - M7-2 明细表（primary — 专项储备计提+使用明细动态行）
 * - 后端三端点:
 *   - GET  /api/m7-special-reserve/{wp_id}/export-template?sheet=xxx
 *   - GET  /api/m7-special-reserve/{wp_id}/export-data?sheet=xxx
 *   - POST /api/m7-special-reserve/{wp_id}/import-data (multipart/form-data)
 * - RFC5987 中文文件名解析
...
/** M7 支持导入导出的 sheet（动态行表格） */
export type M7ImportableSheet =
  | 'M7-2'   // 明细表（专项储备计提+使用明细，primary）

/** 导入结果 */
export interface M7ImportResult {
  /** 是否成功 */
  success: boolean
  /** 消息 */
  message: string
...
export interface UseM7ImportExportOptions {
  wpId: Ref<string>
...
export const M7_IMPORTABLE_SHEETS: { value: M7ImportableSheet; label: string }[] = [
  { value: 'M7-2', label: '明细表（专项储备计提+使用明细）' },
]

/** 可导出sheet编码列表 */
const EXPORTABLE_SHEETS: M7ImportableSheet[] = ['M7-2']
...
function buildApiUrl(wpId: string, action: string): string {
  return `/api/m7-special-reserve/${wpId}/${action}`
}
 ...
 */
export function useM7ImportExport(options: UseM7ImportExportOptions) {
  ...
  function isSheetExportable(sheet: string): boolean {
    return EXPORTABLE_SHEETS.includes(sheet as M7ImportableSheet)
  }

  /** 获取所有可导出sheet列表 */
  function getExportableSheets(): M7ImportableSheet[] {
  ...
  /**
   * 导入xlsx数据（M7-2 明细表为主要目标，专项储备计提+使用动态行）
   * POST /api/m7-special-reserve/{wp_id}/import-data (multipart/form-data)
   */
  async function importData(sheetName: string, file: File): Promise<M7ImportResult | null> {
...
export default useM7ImportExport
