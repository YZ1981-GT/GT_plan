/**
 * useG3AiGenerate — G3 应收股利 AI 辅助生成
 * POST /api/workpapers/{wpId}/g3/ai/{section}
 */
import { onMounted, ref, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type G3AiSection =
  | 'adjudication-note'
...
export function useG3AiGenerate(wpId: Ref<string>) {
  ...
  async function generateAndConfirm(
    ...
    try {
      const response = await http.post(
        `/api/workpapers/${wpId.value}/g3/ai/${section}`,
...
export default useG3AiGenerate
