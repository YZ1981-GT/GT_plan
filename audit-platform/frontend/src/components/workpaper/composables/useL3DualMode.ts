/**
 * useL3DualMode — L3 长期借款双模式切换 composable
 *
...
/** L3 渲染模式 */
export type L3RenderMode = 'structured' | 'onlyoffice'

/** el-segmented 选项 */
export interface L3ModeOption {
  label: string
  value: L3RenderMode
  disabled?: boolean
}

export interface UseL3DualModeOptions {
  wpId: Ref<string>
  defaultMode?: L3RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key */
const STORAGE_KEY = 'l3-dual-mode'
 ...
 */
export function useL3DualMode(options: UseL3DualModeOptions) {
  const { wpId, defaultMode = 'structured' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<L3RenderMode>(defaultMode)
  const isOOHealthy = ref<boolean>(false)
  const ooChecking = ref(false)

  // ─── localStorage 持久化 ───────────────────────────────────────────────

  function _loadPersistedMode(): L3RenderMode {
    try {
  ...
  function _persistMode(m: L3RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: L3RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<L3ModeOption[]>(() => [
