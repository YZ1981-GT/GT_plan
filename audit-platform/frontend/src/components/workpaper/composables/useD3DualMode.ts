/**
 * useD3DualMode — D3 预收账款双模式切换（HTML ↔ OnlyOffice）
 *
 * 管理 OO 健康检查 + 模式切换逻辑。
...
/** D3 Tab → OO sheet 名称映射 */
const D3_SHEET_MAP: D3SheetMapping[] = [
  { tabName: 'procedure', sheetName: '预收账款审计程序表D3A' },
  { tabName: 'adjudication', sheetName: '预收账款审定表D3-1' },
  { tabName: 'detail', sheetName: '预收账款明细表D3-2' },
  { tabName: 'adjustment', sheetName: '预收账款调整分录汇总D3-3' },
  { tabName: 'analysis', sheetName: '预收账款分析表D3-4' },
  { tabName: 'longterm', sheetName: '账龄1年以上检查表D3-5' },
  { tabName: 'related-party', sheetName: '关联关系及交易检查表D3-6' },
  { tabName: 'voucher-check', sheetName: '预收账款检查表D3-7' },
...
export function useD3DualMode(options: UseD3DualModeOptions) {
  ...
  function onDocumentReady(api: any): void {
    ...
    try {
      ...
      console.warn('[D3 DualMode] onDocumentReady hide sheets failed:', e)
    }
  }
