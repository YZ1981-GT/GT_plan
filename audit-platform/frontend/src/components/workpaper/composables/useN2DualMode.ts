/**
 * useN2DualMode — N2 应交税费双模式切换 composable
 *
...
/** N2 渲染模式 */
export type N2RenderMode = 'html' | 'onlyoffice'

/** el-segmented 选项 */
export interface N2ModeOption {
  label: string
  value: N2RenderMode
  disabled?: boolean
}

export interface UseN2DualModeOptions {
  wpId: Ref<string>
  defaultMode?: N2RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key */
const STORAGE_KEY = 'n2-dual-mode'
 ...
 */
export function useN2DualMode(options: UseN2DualModeOptions) {
  const { wpId, defaultMode = 'html' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const currentMode = ref<N2RenderMode>(defaultMode)
  const isOOHealthy = ref<boolean>(false)
  const ooChecking = ref(false)

  // ─── localStorage 持久化 ───────────────────────────────────────────────

  function _loadPersistedMode(): N2RenderMode {
    try {
  ...
  function _persistMode(m: N2RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: N2RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modes = computed<N2ModeOption[]>(() => [
