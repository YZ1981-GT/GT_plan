...
import { G8_ACCOUNT_CODE, G8_DISCLOSURE_LISTED_ROWS, G8_DISCLOSURE_SOE_ROWS } from './g8Constants'
...
export function useG8Disclosure(opts: {
  ...
  const rowDefs = opts.variant === 'listed' ? G8_DISCLOSURE_LISTED_ROWS : G8_DISCLOSURE_SOE_ROWS
  ...
  function publishNoteUpdate(): void {
    try {
      ...
      window.dispatchEvent(new CustomEvent('disclosure:note-text-updated', {
        detail: { accountCode: G8_ACCOUNT_CODE, text },
      }))
    } catch { /* silent */ }
  }
