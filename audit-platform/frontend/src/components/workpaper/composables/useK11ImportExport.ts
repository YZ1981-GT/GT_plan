/**
 * useK11ImportExport — K11 资产减值损失 导入导出 composable
 *
 ...
 *
 * 动态行表格 × 3端点：
 *   GET  /api/workpapers/{wpId}/k11-export-template?sheet_code={code}
 *   GET  /api/workpapers/{wpId}/k11-export-data?sheet_code={code}
 *   POST /api/workpapers/{wpId}/k11-import-data  (multipart/form-data, sheet_code={code})
 *
 * sheet codes: K11-2 / K11-3
 *   K11-2 明细表（18列2区段，50行动态行）
 *   K11-3 调整分录
 *
...
/** K11 支持导入导出的 sheet 编码（动态行表格） */
export type K11ImportableSheet = 'K11-2' | 'K11-3'

/** 导入结果 */
export interface K11ImportResult {
  rowCount: number
  fieldCount: number
  warning?: string
  errors?: string[]
...
/** sheet 元数据（供下拉菜单展示） */
export interface K11SheetMeta {
  code: K11ImportableSheet
  label: string
  /** 多区段分sheet导出 */
  multiSheet?: boolean
}
...
export const K11_API_PREFIX = 'k11'

/** 可导入导出 sheet 的中文标签 */
export const K11_IMPORT_EXPORT_SHEETS: K11SheetMeta[] = [
  { code: 'K11-2', label: 'K11-2 明细表', multiSheet: true },
  { code: 'K11-3', label: 'K11-3 调整分录' },
]
 ...
 */
export function resolveK11ImportableSheet(sheetName: string): K11ImportableSheet | null {
  const m = (sheetName || '').match(/K11-(\d+)/)
  if (!m) return null
  const code = `K11-${m[1]}` as K11ImportableSheet
  return K11_IMPORT_EXPORT_SHEETS.some((s) => s.code === code) ? code : null
}

// ═══ Composable 接口 ═══

export interface UseK11ImportExportOptions {
  wpId: Ref<string>
  projectId: Ref<string>
  sheetCode: string
}

export interface UseK11ImportExportReturn {
  /** 是否正在导出中 */
  ...
  importData: (file: File) => Promise<K11ImportResult | null>
  /** API前缀 */
  apiPrefix: string
}
...
export function useK11ImportExport(params: UseK11ImportExportOptions): UseK11ImportExportReturn {
  ...
  return {
    ...
    isImporting,
    lastError,
    sheets: K11_IMPORT_EXPORT_SHEETS,
    exportTemplate,
    exportData,
    importData,
    apiPrefix: K11_API_PREFIX,
  }
}

export default useK11ImportExport
