/**
 * useG9AiGenerate — G9 其他非流动金融资产 AI 辅助
 * POST /api/workpapers/{wpId}/g9/ai/{section}
 */
import { onMounted, ref, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type G9AiSection =
...
export function useG9AiGenerate(wpId: Ref<string>) {
  ...
  async function generateAndConfirm(
    ...
    try {
      const response = await http.post(
        `/api/workpapers/${wpId.value}/g9/ai/${section}`,
...
export default useG9AiGenerate
