/**
 * useL3FormData — L3 长期借款数据加载/debounce保存/即时保存/TB回写
 *
 ...
 * - item_id 命名: 前缀 "L3-{sheet}-{field}"（如 "L3-L3-1-adjudicated_amount"）
 * - writebackTB(2501): 审定数回写 trial_balance 科目 2501 长期借款
...
/** 审定表 sheet 名（含子码 L3-1，供后端 extract_determination_wp_code 解出） */
const DETERMINATION_SHEET_NAME = '审定表L3-1'
...
export interface UseL3FormDataOptions {
  wpId: Ref<string>
...
const ITEM_PREFIX = 'L3-'

// ─── Composable ──────────────────────────────────────────────────────────────

export function useL3FormData(options: UseL3FormDataOptions) {
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      await api.get(
        `/api/workpapers/${wpId.value}/render-config?force_component_type=l3-long-term-loans`,
        { _silent: true } as any,
      )
    } catch {
  ...
  /**
   * 从 checklist_responses 加载 L3 数据（item_id 前缀 "L3-"）
   */
  async function loadResponses(): Promise<void> {
    ...
    try {
      ...
      ElMessage.warning('L3数据加载失败，可手动填写')
    }
  }
   ...
   * @param itemId - 如 "L3-L3-1-adjudicated_amount"
  ...
  // ─── writebackTB (显式发布审定数到 trial_balance 2501) ─────────────────────

  /**
   * 发布审定数到 trial_balance（科目 2501 长期借款，贷方/负债类）。
   *
   * spec: tb-writeback-explicit-publish-gate Task 4 / Req 1,2,8。
   * 此前直调旧端点 `PUT /projects/{pid}/trial-balance/writeback`（无二次确认/无
   * publish_confirmed/无幂等）。现改为显式发布门：仅在用户显式确认（useL3Adjudication
  ...
  async function writebackTB(auditedAmount: number): Promise<void> {
    ...
    await api.post(`/api/workpapers/${wpId.value}/audit-determination/publish-to-tb`, {
      // sheet 名固定含审定表子码 L3-1，后端 extract_determination_wp_code 据此解出 L3-1
      sheet_name: DETERMINATION_SHEET_NAME,
      writeback_rows: [
        { account_code: L3_GROSS_FALLBACK_STANDARD, audited_amount: auditedAmount, amount_kind: 'balance' },
      ],
    })
