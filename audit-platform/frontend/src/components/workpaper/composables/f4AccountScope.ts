...
export const F4_REPORT_ROW_CODE = 'BS-045'

/** 兜底标准科目码（仅 report_config 解析失败时用） */
export const F4_GROSS_FALLBACK_STANDARD = '2202'

/** 从 render 下发的 tb_source_codes 取运行态科目码（展示 / 请求用） */
export function f4AccountCode(src?: TbSourceCodes | null): string {
  return src?.gross_standard?.[0] ?? F4_GROSS_FALLBACK_STANDARD
}

/** 从 render 下发取查询科目码集（前缀匹配用） */
export function f4GrossQueryCodes(src?: TbSourceCodes | null): string[] {
  const codes = src?.gross
  if (codes && codes.length > 0) return codes
  return [F4_GROSS_FALLBACK_STANDARD]
}
