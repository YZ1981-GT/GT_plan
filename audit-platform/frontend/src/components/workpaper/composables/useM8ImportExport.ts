/**
 * useM8ImportExport — M8 一般风险准备导入导出三级 composable
 *
 ...
 * - 多sheet导出支持（M8-2 明细表为主要导入导出目标，动态行——只有动态行表格才需要导入导出）:
 *   - M8-2 明细表（primary — 一般风险准备计提明细动态行）
 * - 后端三端点:
 *   - GET  /api/m8-general-risk-reserve/{wp_id}/export-template?sheet=xxx
 *   - GET  /api/m8-general-risk-reserve/{wp_id}/export-data?sheet=xxx
 *   - POST /api/m8-general-risk-reserve/{wp_id}/import-data (multipart/form-data)
 * - RFC5987 中文文件名解析
...
/** M8 支持导入导出的 sheet（动态行表格） */
export type M8ImportableSheet =
  | 'M8-2'   // 明细表（一般风险准备计提明细，primary）

/** 导入结果 */
export interface M8ImportResult {
  /** 是否成功 */
  success: boolean
  /** 消息 */
  message: string
...
export interface UseM8ImportExportOptions {
  wpId: Ref<string>
...
export const M8_IMPORTABLE_SHEETS: { value: M8ImportableSheet; label: string }[] = [
  { value: 'M8-2', label: '明细表（一般风险准备计提明细）' },
]

/** 可导出sheet编码列表 */
const EXPORTABLE_SHEETS: M8ImportableSheet[] = ['M8-2']
...
function buildApiUrl(wpId: string, action: string): string {
  return `/api/m8-general-risk-reserve/${wpId}/${action}`
}
 ...
 */
export function useM8ImportExport(options: UseM8ImportExportOptions) {
  ...
  function isSheetExportable(sheet: string): boolean {
    return EXPORTABLE_SHEETS.includes(sheet as M8ImportableSheet)
  }

  /** 获取所有可导出sheet列表 */
  function getExportableSheets(): M8ImportableSheet[] {
  ...
  /**
   * 导入xlsx数据（M8-2 明细表为主要目标，一般风险准备计提明细动态行）
   * POST /api/m8-general-risk-reserve/{wp_id}/import-data (multipart/form-data)
   */
  async function importData(sheetName: string, file: File): Promise<M8ImportResult | null> {
...
export default useM8ImportExport
