/**
 * useK3ImportExport — K3 其他应付款 导入导出 composable
 *
 * Spec: .kiro/specs/k3-other-payables/ Task 3.3
 * Requirements: 3.4, 8.2
 *
 * 动态行表格 × 3端点：
 *   GET  /api/workpapers/{wpId}/k3/export-template?sheet={code}
 *   GET  /api/workpapers/{wpId}/k3/export-data?sheet={code}
 *   POST /api/workpapers/{wpId}/k3/import-data  (multipart/form-data, sheet={code})
 *
 * sheet codes: K3-2 / K3-3
 *   K3-2 明细表（27列3区段+账龄+动态行）
 *   K3-3 调整分录
 *
...
/** K3 支持导入导出的 sheet 编码（动态行表格） */
export type K3ImportableSheet = 'K3-2' | 'K3-3'

/** 导入结果 */
export interface K3ImportResult {
  rowCount: number
  fieldCount: number
  warning?: string
  errors?: string[]
...
/** sheet 元数据（供下拉菜单展示） */
export interface K3SheetMeta {
  code: K3ImportableSheet
  label: string
  /** 多区段分sheet导出 */
  multiSheet?: boolean
}
...
export const K3_API_PREFIX = 'k3'

/** 可导入导出 sheet 的中文标签 */
export const K3_IMPORT_EXPORT_SHEETS: K3SheetMeta[] = [
  { code: 'K3-2', label: 'K3-2 明细表', multiSheet: true },
  { code: 'K3-3', label: 'K3-3 调整分录' },
]
 ...
 */
export function resolveK3ImportableSheet(sheetName: string): K3ImportableSheet | null {
  const m = (sheetName || '').match(/K3-(\d+)/)
  if (!m) return null
  const code = `K3-${m[1]}` as K3ImportableSheet
  return K3_IMPORT_EXPORT_SHEETS.some((s) => s.code === code) ? code : null
}

// ═══ Composable 接口 ═══

export interface UseK3ImportExportOptions {
  wpId: Ref<string>
  projectId: Ref<string>
  sheetCode: string
}

export interface UseK3ImportExportReturn {
  /** 是否正在导出中 */
  ...
  importData: (file: File) => Promise<K3ImportResult | null>
  /** API前缀 */
  apiPrefix: string
}
...
export function useK3ImportExport(params: UseK3ImportExportOptions): UseK3ImportExportReturn {
  ...
  function apiBase(): string {
    return `/api/workpapers/${wpId.value}/${K3_API_PREFIX}`
  }

  /**
   * 导出模板 — GET /api/workpapers/{wpId}/k3/export-template?sheet={code}
   */
  ...
  /**
   * 导出数据 — GET /api/workpapers/{wpId}/k3/export-data?sheet={code}
   */
  ...
  /**
   * 导入数据 — POST /api/workpapers/{wpId}/k3/import-data
   * multipart/form-data 上传 xlsx 文件, query param sheet={code}
   */
  async function importData(file: File): Promise<K3ImportResult | null> {
    lastError.value = null
  ...
  return {
    ...
    isImporting,
    lastError,
    sheets: K3_IMPORT_EXPORT_SHEETS,
    exportTemplate,
    exportData,
    importData,
    apiPrefix: K3_API_PREFIX,
  }
}

export default useK3ImportExport
