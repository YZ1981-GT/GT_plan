/**
 * useM4FormData — M4 资本公积数据加载/debounce保存/即时保存/TB回写
 *
 ...
 * - item_id 命名: 前缀 "M4-{sheet}-{field}"（如 "M4-1-adjudicated-amount", "M4-2-premium-begin"）
 * - writebackTB(4002): 审定数回写 trial_balance 科目 4002 资本公积
 ...
 * 资本溢价（出资超面值）在贷方增加，资本公积转增资本在借方减少
 * 注：M3库存股也是4002但属于权益备抵借方（方向完全相反！），M4是正常权益贷方
 */
...
export interface UseM4FormDataOptions {
  wpId: Ref<string>
...
const ITEM_PREFIX = 'M4-'
const ACCOUNT_CODE = '4002' // 资本公积（贷方/权益类！）
/** 审定表 sheet 名（含子码 M4-1，供后端 extract_determination_wp_code 解出） */
const DETERMINATION_SHEET_NAME = '审定表M4-1'

// ─── Composable ──────────────────────────────────────────────────────────────

export function useM4FormData(options: UseM4FormDataOptions) {
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      await api.get(
        `/api/workpapers/${wpId.value}/render-config?force_component_type=m4-capital-reserve`,
        { _silent: true } as any,
      )
    } catch {
  ...
  /**
   * 从 checklist_responses 加载 M4 数据（item_id 前缀 "M4-"）
   */
  async function loadResponses(): Promise<void> {
    ...
    try {
      ...
      ElMessage.warning('M4数据加载失败，可手动填写')
    }
  }
  ...
  /**
   * 获取指定 sheet + field 的值
   * @param sheet - 如 "1", "2", "3", "4"（不含M4-前缀，最终item_id为 "M4-1-xxx"）
   * @param field - 如 "adjudicated-amount", "premium-begin", "other-end"
   ...
   * @param field - 如 "adjudicated-amount", "premium-detail-row1"
   ...
   * @param itemId - 如 "M4-1-adjudicated-amount"
  ...
  /**
   * 回写审定数到 trial_balance（科目 4002 资本公积，**贷方/权益类！**）。
   *
   * 权益类贷方方向铁律：
   * - 期末 = 期初 + 贷方(增加：溢价、股份支付) - 借方(减少：转增资本)
   * - 回写的 audited_amount 代表期末余额（正数=资本公积贷方余额）
   * - 与 M3库存股（借方/权益备抵类）方向**完全相反**
   * - 与 M2实收资本（贷方/权益类）方向一致
   *
   * 并发布 EventBus 'substantive:adjudicated' 事件通知其他组件（附注刷新等）。
   */
  // spec: tb-writeback-explicit-publish-gate Task 5 / Req 1,2,8。改走显式发布门：
  // 仅在用户显式确认（useM4Adjudication.publishToTb 二次确认）后调用 →
