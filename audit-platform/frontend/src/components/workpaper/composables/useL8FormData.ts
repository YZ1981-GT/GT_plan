...
/** 审定表 sheet 名（含子码 L8-1，供后端 extract_determination_wp_code 解出） */
const DETERMINATION_SHEET_NAME = '审定表L8-1'
...
export function useL8FormData(options: UseL8FormDataOptions) {
  ...
  // ─── writebackTB (显式发布审定数到 trial_balance 6603, 发生额口径) ──────────

  /**
   * 发布审定数到 trial_balance（科目 6603 财务费用，**发生额口径！**）。
   *
   * ⚠️ 损益类特殊：
   * - 回写的 audited_amount 代表审定的本期发生额（非余额）→ amount_kind='occurrence'
   ...
   * publish_confirmed/无幂等），且由 useL8Adjudication 的 watcher 在数据变化时**自动**
   * 触发（违反 Req 1）。现改为显式发布门：仅在用户显式确认后调用本函数 →
   * `POST /workpapers/{wpId}/audit-determination/publish-to-tb`（携审定表 sheet 名 +
   * writeback_rows 预算行，amount_kind 标 occurrence），后端发 publish_confirmed=True +
   * token → handler 幂等回写。成功后 emit substantive:adjudicated（附注刷新）。
   */
  async function writebackTB(amount: number): Promise<void> {
    ...
    await api.post(`/api/workpapers/${wpId.value}/audit-determination/publish-to-tb`, {
      // sheet 名固定含审定表子码 L8-1，后端 extract_determination_wp_code 据此解出 L8-1
      sheet_name: DETERMINATION_SHEET_NAME,
      writeback_rows: [
        // 发生额口径！amount_kind='occurrence'（后端透传，区别于余额类 balance）
