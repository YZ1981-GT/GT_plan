/**
 * useL5FormData — L5 长期应付款数据加载/debounce保存/即时保存/TB回写
 *
 ...
 * - item_id 命名: 前缀 "L5-{sheet}-{field}"（如 "L5-adjudication-payable-end", "L5-3-unrecognized-balance"）
 * - writebackTB(2701+未确认融资费用): 审定数回写 trial_balance
...
export interface UseL5FormDataOptions {
  wpId: Ref<string>
...
export function useL5FormData(options: UseL5FormDataOptions) {
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      await api.get(
        `/api/workpapers/${wpId.value}/render-config?force_component_type=l5-long-term-payables`,
        { _silent: true } as any,
      )
    } catch {
  ...
  /**
   * 从 checklist_responses 加载 L5 数据（item_id 前缀 "L5-"）
   */
  async function loadResponses(): Promise<void> {
    ...
    try {
      ...
      ElMessage.warning('L5数据加载失败，可手动填写')
    }
  }
   ...
   * @param itemId - 如 "L5-adjudication-payable-end"
