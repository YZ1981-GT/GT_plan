 ...
 * 比照 useF2DualMode / useG9DualMode：http 健康检查 + onlyoffice-config + reloadAll
 */
import { ref, onMounted, type Ref } from 'vue'
import http from '@/utils/http'
import { dualModeHtmlOoOptions } from './dualModeLabels'

export type G11RenderMode = 'html' | 'onlyoffice'

const STORAGE_PREFIX = 'g11-dual-mode:'

export function useG11DualMode(options: {
  wpId: Ref<string>
  ...
  const currentMode = ref<G11RenderMode>('html')
  ...
  function persistMode(mode: G11RenderMode): void {
    try {
  ...
  async function switchMode(target: G11RenderMode): Promise<void> {
    if (target === currentMode.value) return
    if (target === 'onlyoffice' && !isOoAvailable.value) return

    if (target === 'onlyoffice') {
      const sn = sheetName?.value || 'G11'
      try {
  ...
  function onModeChange(val: string | number | boolean): void {
    void switchMode(val as G11RenderMode)
  }
