/**
 * useM7DualMode — M7 专项储备双模式切换 composable
 *
...
/** M7 渲染模式 */
export type M7RenderMode = 'html' | 'onlyoffice'

/** el-segmented 选项 */
export interface M7ModeOption {
  label: string
  value: M7RenderMode
  disabled?: boolean
}

export interface UseM7DualModeOptions {
  wpId: Ref<string>
  defaultMode?: M7RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key prefix（按 wpId 存储） */
const STORAGE_KEY_PREFIX = 'm7-dual-mode'
 ...
 */
export function useM7DualMode(options: UseM7DualModeOptions) {
  const { wpId, defaultMode = 'html' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<M7RenderMode>(defaultMode)
  ...
  function _loadPersistedMode(): M7RenderMode {
    try {
  ...
  function _persistMode(m: M7RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: M7RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<M7ModeOption[]>(() => [
