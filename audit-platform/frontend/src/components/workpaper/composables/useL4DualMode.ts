/**
 * useL4DualMode — L4 应付债券双模式切换 composable
 *
...
/** L4 渲染模式 */
export type L4RenderMode = 'structured' | 'onlyoffice'

/** el-segmented 选项 */
export interface L4ModeOption {
  label: string
  value: L4RenderMode
  disabled?: boolean
}

export interface UseL4DualModeOptions {
  wpId: Ref<string>
  defaultMode?: L4RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key prefix（按 wpId 存储） */
const STORAGE_KEY_PREFIX = 'l4-dual-mode'
 ...
 */
export function useL4DualMode(options: UseL4DualModeOptions) {
  const { wpId, defaultMode = 'structured' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<L4RenderMode>(defaultMode)
  ...
  function _loadPersistedMode(): L4RenderMode {
    try {
  ...
  function _persistMode(m: L4RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: L4RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<L4ModeOption[]>(() => [
