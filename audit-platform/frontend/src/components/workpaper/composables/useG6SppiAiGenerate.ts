/**
 * useG6SppiAiGenerate — G6 其他债权投资(SPPI) AI 辅助
 * POST /api/workpapers/{wpId}/g6-sppi/ai/{section}
 */
import { onMounted, ref, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type G6SppiAiSection =
...
export function useG6SppiAiGenerate(wpId: Ref<string>) {
  ...
  async function generateAndConfirm(
    ...
    try {
      const response = await http.post(
        `/api/workpapers/${wpId.value}/g6-sppi/ai/${section}`,
...
export default useG6SppiAiGenerate
