...
export interface UseAdjudicationBringInOptions {
   ...
   *
   * 🔴 2026-08-07 放宽为 `Resolvable` —— 走语义定位的循环科目码逐项目解析，
   * 且本值**进事件载荷**，写错会让披露/附注 handler 匹配不上（静默失联）。
   */
  subjectCode: Resolvable<string>
  /** 底稿编码（事件 wpCode，披露 handler 兜底匹配） */
  wpCode: string
  /** 提示文案用科目标签（如 '营业外收入(6301)'） */
  subjectLabel: Resolvable<string>
...
export function useAdjudicationBringIn(opts: UseAdjudicationBringInOptions) {
  ...
  async function open(): Promise<void> {
    ...
    if (!adjPull.matches.value.length) {
      ElMessage.info(`未找到命中${unwrap(opts.subjectLabel)}的调整分录`)
      return
    }
    visible.value = true
  }
  ...
  function apply(payload: { allocations: AdjudicationAllocation[] }): void {
    ...
    eventBus.emit('substantive:adjudicated' as any, {
      // 🔴 事件载荷里的科目码必须是**求值后的字符串** —— 传 ref 会让披露/附注
      //    handler 的 `accountCode` 匹配恒不成立（静默失联，四层验证全绿）。
      accountCode: unwrap(opts.subjectCode),
      wpCode: opts.wpCode,
