/**
 * useM7FormData — M7 专项储备数据加载/debounce保存/即时保存/TB回写
 *
 ...
 * - item_id 命名: 前缀 "M7-{sheet}-{field}"（如 "M7-1-adjudicated-amount", "M7-2-accrual-total"）
 * - writebackTB(4201): 审定数回写 trial_balance 科目 4201 专项储备
 ...
 * 计提（安全生产费等）在贷方增加，使用（费用化/资本化支出）在借方减少
...
export interface UseM7FormDataOptions {
  wpId: Ref<string>
...
const ITEM_PREFIX = 'M7-'
const ACCOUNT_CODE = '4201' // 专项储备（贷方/权益类！）
/** 审定表 sheet 名（含子码 M7-1，供后端 extract_determination_wp_code 解出） */
const DETERMINATION_SHEET_NAME = '审定表M7-1'

// ─── Composable ──────────────────────────────────────────────────────────────

export function useM7FormData(options: UseM7FormDataOptions) {
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      await api.get(
        `/api/workpapers/${wpId.value}/render-config?force_component_type=m7-special-reserve`,
        { _silent: true } as any,
      )
    } catch {
  ...
  /**
   * 从 checklist_responses 加载 M7 数据（item_id 前缀 "M7-"）
   */
  async function loadResponses(): Promise<void> {
    ...
    try {
      ...
      ElMessage.warning('M7数据加载失败，可手动填写')
    }
  }
  ...
  /**
   * 获取指定 sheet + field 的值
   * @param sheet - 如 "1", "2", "3", "4", "5"（不含M7-前缀，最终item_id为 "M7-1-xxx"）
   * @param field - 如 "adjudicated-amount", "accrual-total", "expenditure-capital"
   ...
   * @param itemId - 如 "M7-1-adjudicated-amount"
  ...
  // ─── writebackTB (回写审定数到 trial_balance 4201) ─────────────────────────

  /**
   * 回写审定数到 trial_balance（科目 4201 专项储备，**贷方/权益类！**）。
   *
   * 权益类贷方方向铁律：
   * - 期末 = 期初 + 贷方(增加：安全生产费计提) - 借方(减少：使用支出)
   * - 回写的 audited_amount 代表期末余额（正数=专项储备贷方余额）
   * - 安全生产费计提：贷方增加（按产量或营业收入计提）
   * - 专项储备使用：借方减少（费用化直接冲减 / 资本化转固定资产同时冲减）
   *
   * 并发布 EventBus 'substantive:adjudicated' 事件通知其他组件（附注刷新等）。
   */
  // spec: tb-writeback-explicit-publish-gate Task 5 / Req 1,2,8。改走显式发布门：
  // 仅在用户显式确认（useM7Adjudication.publishToTb 二次确认）后调用 →
