/**
 * useD6AiGenerate — D6 合同资产 AI 辅助生成（POST /d6/ai-generate）
 */
import { ref, onMounted, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type D6AiSection =
  | 'adj-aging-reason'
  | 'adj-change-analysis'
  | 'adj-conclusion'
  | 'detail-change'
...
export interface D6AiGenerateParams {
  section: D6AiSection
  existingContent?: string
  relatedContext?: Record<string, unknown>
}

export function useD6AiGenerate(wpId: Ref<string>) {
  ...
  async function generate(params: D6AiGenerateParams): Promise<string> {
    const res = await http.post(
      `/api/workpapers/${wpId.value}/d6/ai-generate`,
      {
...
export default useD6AiGenerate
