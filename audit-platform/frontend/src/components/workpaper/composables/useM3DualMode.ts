/**
 * useM3DualMode — M3 库存股双模式切换 composable
 *
...
/** M3 渲染模式 */
export type M3RenderMode = 'structured' | 'onlyoffice'

/** el-segmented 选项 */
export interface M3ModeOption {
  label: string
  value: M3RenderMode
  disabled?: boolean
}

export interface UseM3DualModeOptions {
  wpId: Ref<string>
  defaultMode?: M3RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key prefix（按 wpId 存储） */
const STORAGE_KEY_PREFIX = 'm3-dual-mode'
 ...
 */
export function useM3DualMode(options: UseM3DualModeOptions) {
  const { wpId, defaultMode = 'structured' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<M3RenderMode>(defaultMode)
  ...
  function _loadPersistedMode(): M3RenderMode {
    try {
  ...
  function _persistMode(m: M3RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: M3RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<M3ModeOption[]>(() => [
