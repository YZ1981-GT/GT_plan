/**
 * useF1Analysis — F1-4 分析表核心逻辑 composable
 *
 ...
 * - 实现差异行 = TB总计 - 分拆合计
 * - 实现 top5Debtors computed（从crossSheet取F1-2按余额降序前5）
...
import { parseNum, calcSubtotal, calcChangeAmount, calcChangeRate, isChangeRateExceeding } from './useF1FormulaEngine'
...
const ITEM_ID_DEBIT_ROWS = 'F1-ana-debit-rows'
const ITEM_ID_CREDIT_ROWS = 'F1-ana-credit-rows'
const ITEM_ID_NOTE = 'F1-ana-note'
const ITEM_ID_TB_DEBIT = 'F1-ana-tb-debit-total'
const ITEM_ID_TB_CREDIT = 'F1-ana-tb-credit-total'
...
export function useF1Analysis(options: UseF1AnalysisOptions) {
  ...
  const top5Debtors: ComputedRef<Top5DebtorRow[]> = computed(() => {
    // Get F1-2 rows from crossSheet detailRows
