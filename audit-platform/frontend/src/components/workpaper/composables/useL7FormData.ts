/**
 * useL7FormData — L7 其他非流动负债数据加载/debounce保存/即时保存/TB回写
 *
 ...
 * - item_id 命名: 前缀 "L7-{sheet}-{field}"（如 "L7-L7-1-audited_amount", "L7-L7-2-detail-row1-end"）
 * - writebackTB(2801): 审定数回写 trial_balance 科目 2801 其他非流动负债
...
export interface UseL7FormDataOptions {
  wpId: Ref<string>
...
export function useL7FormData(options: UseL7FormDataOptions) {
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      await api.get(
        `/api/workpapers/${wpId.value}/render-config?force_component_type=l7-other-noncurrent-liabilities`,
        { _silent: true } as any,
      )
    } catch {
  ...
  /**
   * 从 checklist_responses 加载 L7 数据（item_id 前缀 "L7-"）
   */
  async function loadResponses(): Promise<void> {
    ...
    try {
      ...
      ElMessage.warning('L7数据加载失败，可手动填写')
    }
  }
   ...
   * @param itemId - 如 "L7-L7-1-audited_amount"
