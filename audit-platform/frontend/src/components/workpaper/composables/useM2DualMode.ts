/**
 * useM2DualMode — M2 实收资本（股本）双模式切换 composable
 *
...
/** M2 渲染模式 */
export type M2RenderMode = 'structured' | 'onlyoffice'

/** el-segmented 选项 */
export interface M2ModeOption {
  label: string
  value: M2RenderMode
  disabled?: boolean
}

export interface UseM2DualModeOptions {
  wpId: Ref<string>
  defaultMode?: M2RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key prefix（按 wpId 存储） */
const STORAGE_KEY_PREFIX = 'm2-dual-mode'
 ...
 */
export function useM2DualMode(options: UseM2DualModeOptions) {
  const { wpId, defaultMode = 'structured' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const mode = ref<M2RenderMode>(defaultMode)
  ...
  function _loadPersistedMode(): M2RenderMode {
    try {
  ...
  function _persistMode(m: M2RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: M2RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modeOptions = computed<M2ModeOption[]>(() => [
