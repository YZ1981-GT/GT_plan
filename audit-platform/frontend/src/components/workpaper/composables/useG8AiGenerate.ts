/**
 * useG8AiGenerate — G8 其他权益工具投资 AI 辅助
 * POST /api/workpapers/{wpId}/g8/ai/{section}
 */
import { onMounted, ref, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type G8AiSection =
...
export function useG8AiGenerate(wpId: Ref<string>) {
  ...
  async function generateAndConfirm(
    ...
    try {
      const response = await http.post(
        `/api/workpapers/${wpId.value}/g8/ai/${section}`,
...
export default useG8AiGenerate
