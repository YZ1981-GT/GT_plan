/**
 * useK9FormData — K9 管理费用 selfLoad/checklist_responses/writebackTB(6602 发生额！)
 *
 * Spec: .kiro/specs/k9-admin-expenses/
 * Task: 3.1
 * Requirements: 1.9, 1.10, 2.6, 7.1-7.3
 *
 * 职责：
 * - selfLoad(): bundle内嵌场景从 render-config 加载上下文 + checklist_responses
 * - loadTbData(): 从 trial_balance 获取科目 6602 发生额数据（损益类！）
 * - writebackTB(): 审定数回写 trial_balance(6602) + EventBus 'substantive:adjudicated'
 * - checklist_responses 持久化: GET/PUT /api/workpapers/:wpId/checklist-responses
 * - item_id 命名: 前缀 "K9-{sheet}-{field}"
 * - debounce/即时保存: 文本字段 debounce 2s，枚举/结论即时保存
 * - 组件卸载时 flush 未保存数据（onScopeDispose）
 *
 * 科目：6602 管理费用（**损益类/借方科目**）
 * ⚠️ 损益类！取发生额非余额！
...
/**
 * 损益类TB数据结构（发生额！非余额）
 * 6602管理费用：借方=费用增加，贷方=费用冲回/红冲
 */
export interface K9TbData {
  /** 未审借方发生额（费用增加） */
...
export function useK9FormData(opts: {
  wpId: Ref<string>
   ...
   * @param itemId 完整 item_id（如 "K9-1-audited-total"）
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      ...
      for (const r of responses) {
        if (r.item_id?.startsWith('K9-') || r.item_id?.startsWith('K9A-')) {
  ...
  /**
   * 从 trial_balance / tb_ledger 获取科目 6602 的发生额数据。
   * ⚠️ 损益类！取发生额非期末余额！
   *
  ...
  async function loadTbData(): Promise<void> {
    ...
    try {
      ...
      if (!found) {
        ElMessage.warning('科目6602管理费用未在试算表中找到，发生额显示为0')
      }
    } catch {
      tbData.value = {
        unadjustedDebit: 0,
  ...
  // 原 writebackTB（PUT /api/projects/{pid}/trial-balance/writeback，科目 6602 管理费用发生额）
  // 为零消费死代码，已移除；K9 的 TB 回写走显式发布门 publish-to-tb（M6/task7 改造）。
  // 实证：useK9FormData 无任何渲染宿主 import（孤儿 composable，仅测试用 setTbValues），
  // K9 真实回写路径在 useK9Adjudication.writeback（inline http.put，被 K9.vue handleTbWriteback 消费）。
   ...
   */
  function setTbValues(values: Partial<K9TbData>): void {
