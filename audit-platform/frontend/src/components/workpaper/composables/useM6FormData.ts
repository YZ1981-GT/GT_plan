/**
 * useM6FormData — M6 未分配利润数据加载/debounce保存/即时保存/TB回写
 *
 ...
 * - item_id 命名: 前缀 "M6-{sheet}-{field}"（如 "M6-1-adjudicated-amount", "M6-2-retained-end"）
 * - writebackTB(4104): 审定数回写 trial_balance 科目 4104 利润分配-未分配利润
...
export interface UseM6FormDataOptions {
  wpId: Ref<string>
...
const ITEM_PREFIX = 'M6-'
const ACCOUNT_CODE = '4104' // 利润分配-未分配利润（贷方/权益类！）
/** 审定表 sheet 名（含子码 M6-1，供后端 extract_determination_wp_code 解出） */
const DETERMINATION_SHEET_NAME = '审定表M6-1'

// ─── Composable ──────────────────────────────────────────────────────────────

export function useM6FormData(options: UseM6FormDataOptions) {
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      await api.get(
        `/api/workpapers/${wpId.value}/render-config?force_component_type=m6-retained-earnings`,
        { _silent: true } as any,
      )
    } catch {
  ...
  /**
   * 从 checklist_responses 加载 M6 数据（item_id 前缀 "M6-"）
   */
  async function loadResponses(): Promise<void> {
    ...
    try {
      ...
      ElMessage.warning('M6数据加载失败，可手动填写')
    }
  }
  ...
  /**
   * 获取指定 sheet + field 的值
   * @param sheet - 如 "1", "2", "3", "4"（不含M6-前缀，最终item_id为 "M6-1-xxx"）
   * @param field - 如 "adjudicated-amount", "retained-end", "net-profit"
   ...
   * @param field - 如 "adjudicated-amount", "retained-end", "surplus-accrual"
   ...
   * @param itemId - 如 "M6-1-adjudicated-amount"
  ...
  /**
   * 回写审定数到 trial_balance（科目 4104 利润分配-未分配利润，**贷方/权益类！**）。
   *
   * 权益类贷方方向铁律：
   * - 期末 = 期初 + 贷方(本年净利润转入) - 借方(分配：提取盈余公积+宣告股利)
   * - 回写的 audited_amount 代表期末余额（正数=未分配利润贷方余额）
   * - 核心公式链：期末未分配利润=期初+本年净利润-提取盈余公积-分配股利
   *
   * 并发布 EventBus 'substantive:adjudicated' 事件通知其他组件（附注刷新等）。
   */
  // spec: tb-writeback-explicit-publish-gate Task 5 / Req 1,2,8。改走显式发布门：
  // 仅在用户显式确认（useM6Adjudication.publishToTb 二次确认）后调用 →
