/**
 * useM6ImportExport — M6 未分配利润导入导出三级 composable
 *
 ...
 * - 动态行表格（M6-2明细表）支持导入导出:
 *   - M6-2 明细表（primary — 利润分配结转明细动态行）
 * - 后端三端点:
 *   - GET  /api/m6-retained-earnings/{wp_id}/export-template?sheet=xxx
 *   - GET  /api/m6-retained-earnings/{wp_id}/export-data?sheet=xxx
 *   - POST /api/m6-retained-earnings/{wp_id}/import-data (multipart/form-data)
 * - RFC5987 中文文件名解析
...
/** M6 支持导入导出的 sheet（动态行表格） */
export type M6ImportableSheet =
  | 'M6-2'   // 明细表（利润分配结转明细，primary）

/** 导入结果 */
export interface M6ImportResult {
  /** 是否成功 */
  success: boolean
  /** 消息 */
  message: string
...
export interface UseM6ImportExportOptions {
  wpId: Ref<string>
...
export const M6_IMPORTABLE_SHEETS: { value: M6ImportableSheet; label: string }[] = [
  { value: 'M6-2', label: '明细表（利润分配结转明细）' },
]

/** 可导出sheet编码列表 */
const EXPORTABLE_SHEETS: M6ImportableSheet[] = ['M6-2']
...
function buildApiUrl(wpId: string, action: string): string {
  return `/api/m6-retained-earnings/${wpId}/${action}`
}
 ...
 */
export function useM6ImportExport(options: UseM6ImportExportOptions) {
  ...
  function isSheetExportable(sheet: string): boolean {
    return EXPORTABLE_SHEETS.includes(sheet as M6ImportableSheet)
  }

  /** 获取所有可导出sheet列表 */
  function getExportableSheets(): M6ImportableSheet[] {
  ...
  /**
   * 导入xlsx数据（M6-2 明细表为主要目标，利润分配结转明细动态行）
   * POST /api/m6-retained-earnings/{wp_id}/import-data (multipart/form-data)
   */
  async function importData(sheetName: string, file: File): Promise<M6ImportResult | null> {
...
export default useM6ImportExport
