...
5、购买资产：(1)具体描述（背景、审批、合同、付款、资产情况、对报表影响）；(2)审计程序；(3)财务处理正确性判断；(4)审计结论及披露。`,
  ...
  7: `利用专家工作：详见S12（或注明"不适用"）。
税务专家复核：详见A28（或注明"不适用"）。

根据总体审计策略和具体审计过程中遇到的重大事项，需征求事务所内部（质量控制部、专业技术委员会）和外部专家的专业意见的，应详细说明在对外部专家进行了解的前提下，与之签约情况、咨询事项、咨询结果以及项目组的判评价，并索引至具体记录中。征求专家意见的内容包括但不限于：
(1) 对复杂的金融工具、土地及建筑特、厂和机器设备、珠宝、艺术品、古董、无形资产、企业合并中收购的资产和承担的负债，以及可能发生减值的资产；
(2) 对保险合同或员工福利计划相关的负债进行精算；
(3) 对石油和天然气储量进行估算；
(4) 对环境负债和场地清理费用进行估价；
(5) 对合同、法律和法规进行解释；
(6) 对复杂或异常的纳税问题进行分析；
