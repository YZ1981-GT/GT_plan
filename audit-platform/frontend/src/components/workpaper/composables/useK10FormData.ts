/**
 * useK10FormData — K10 其他收益 selfLoad/checklist_responses/writebackTB(6117 发生额！)
 *
 * Spec: .kiro/specs/k10-other-income/
 * Task: 3.1
 * Requirements: 1.9, 1.10, 2.6, 6.1-6.2
 *
 * 职责：
 * - selfLoad(): bundle内嵌场景从 render-config 加载上下文 + checklist_responses
 * - loadTbData(): 从 trial_balance 获取科目 6117 发生额数据（损益类！）
 * - writebackTB(): 审定数回写 trial_balance(6117) + EventBus 'substantive:adjudicated'
 * - checklist_responses 持久化: GET/PUT /api/workpapers/:wpId/checklist-responses
 * - item_id 命名: 前缀 "K10-{sheet}-{field}"
 * - debounce/即时保存: 文本字段 debounce 2s，枚举/结论即时保存
 * - 组件卸载时 flush 未保存数据（onScopeDispose）
 *
 * 科目：6117 其他收益（**损益类/贷方科目**）
 * ⚠️ 损益类！取发生额非余额！
 * - 6117为贷方科目：贷方=收益增加（政府补助/即征即退/资源税返还等），借方=收益冲回/红冲
 * - 收益类发生额 = 贷方发生 - 借方发生（红冲）
 * - TB回写发生额，非期末余额（期末结转本年利润后为0）
 * - ⚠️ 方向与K12(6301)一致！K10/K12: credit-debit（贷方科目）
 */
...
/**
 * 损益类TB数据结构（发生额！非余额）
 * 6117其他收益：贷方=收益增加，借方=收益冲回/红冲
 * 方向与K12(6301)一致：净额=贷方-借方（贷方科目）
 */
export interface K10TbData {
  /** 未审贷方发生额（收益增加：政府补助-即征即退/财政贴息/研发补助/稳岗补贴/资源税返还等） */
  unadjustedCredit: number
  /** 未审借方发生额（收益冲回/红冲/期末结转） */
...
/** 科目：6117 其他收益（贷方/损益类） */
const ACCOUNT_CODE_6117 = '6117'
const COMPONENT_TYPE = 'k10-other-income'

// ─── Composable ──────────────────────────────────────────────────────────────

export function useK10FormData(opts: {
  wpId: Ref<string>
   ...
   * @param itemId 完整 item_id（如 "K10-1-audited-total"）
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      ...
      for (const r of responses) {
        if (r.item_id?.startsWith('K10-') || r.item_id?.startsWith('K10A-')) {
  ...
  /**
   * 从 trial_balance / tb_ledger 获取科目 6117 的发生额数据。
   * ⚠️ 损益类！取发生额非期末余额！
   * ⚠️ 6117是贷方科目！净额=贷方-借方（与K12/6301方向一致）
   *
  ...
  async function loadTbData(): Promise<void> {
    ...
    try {
      const res = await api.get(`/api/projects/${projectId.value}/trial-balance`, {
        params: { account_prefix: ACCOUNT_CODE_6117 },
        _silent: true,
      } as any)
      ...
      for (const item of list) {
        ...
        if (code.startsWith(ACCOUNT_CODE_6117)) {
          // 损益类：取发生额字段
      ...
      if (!found) {
        ElMessage.warning('科目6117其他收益未在试算表中找到，发生额显示为0')
      }
    } catch {
      tbData.value = {
        unadjustedCredit: 0,
  ...
  /**
   * 审定数回写 trial_balance：科目6117其他收益
   * ⚠️ 关键区别：损益类回写**发生额**，非期末余额！
   * ⚠️ 6117贷方科目：审定发生额 = 贷方发生 - 借方发生（红冲）
   *
   ...
   * ⚠️ 6117贷方科目：净额=贷方-借方
   */
  function setTbValues(values: Partial<K10TbData>): void {
