/**
 * useK10ImportExport — K10 其他收益 导入导出 composable
 *
 ...
 *
 * 动态行表格 × 3端点：
 *   POST /api/workpapers/{wpId}/k10/export-template
 *   POST /api/workpapers/{wpId}/k10/export-data
 *   POST /api/workpapers/{wpId}/k10/import-data  (multipart/form-data)
 *
 * K10-2 明细表（动态行）和 K10-4 政府补助核对表（动态行）需要导入导出
 *
...
/** K10 支持导入导出的 sheet 编码（K10-2 明细表 + K10-4 补助核对表，均为动态行） */
export type K10ImportableSheet = 'K10-2' | 'K10-4'

/** 导入结果 */
export interface K10ImportResult {
  rowCount: number
  fieldCount: number
  warning?: string
  errors?: string[]
...
/** sheet 元数据（供下拉菜单展示） */
export interface K10SheetMeta {
  code: K10ImportableSheet
  label: string
}

// ═══ 常量 ═══

export const K10_API_PREFIX = 'k10'

/** 可导入导出 sheet 的中文标签（K10-2 明细表 + K10-4 补助核对表） */
export const K10_IMPORT_EXPORT_SHEETS: K10SheetMeta[] = [
  { code: 'K10-2', label: 'K10-2 明细表' },
  { code: 'K10-4', label: 'K10-4 政府补助核对表' },
]
 ...
 */
export function resolveK10ImportableSheet(sheetName: string): K10ImportableSheet | null {
  const m = (sheetName || '').match(/K10-(\d+)/)
  if (!m) return null
  const code = `K10-${m[1]}` as K10ImportableSheet
  return K10_IMPORT_EXPORT_SHEETS.some((s) => s.code === code) ? code : null
}

// ═══ Composable 接口 ═══

export interface UseK10ImportExportOptions {
  wpId: Ref<string>
  projectId: Ref<string>
  sheetCode: string
}

export interface UseK10ImportExportReturn {
  /** 是否正在导出中 */
  ...
  importData: (file: File) => Promise<K10ImportResult | null>
  /** API前缀 */
  apiPrefix: string
}
...
export function useK10ImportExport(params: UseK10ImportExportOptions): UseK10ImportExportReturn {
  ...
  /**
   * 导出模板 — POST /api/workpapers/{wpId}/k10/export-template
   */
  ...
  /**
   * 导出数据 — POST /api/workpapers/{wpId}/k10/export-data
   */
  ...
  /**
   * 导入数据 — POST /api/workpapers/{wpId}/k10/import-data
   * multipart/form-data 上传 xlsx 文件, body 含 sheet_code
   */
  async function importData(file: File): Promise<K10ImportResult | null> {
    lastError.value = null
    ...
    try {
      ...
      const response = await http.post(`${apiBase()}/k10/import-data`, formData, {
  ...
  return {
    ...
    isImporting,
    lastError,
    sheets: K10_IMPORT_EXPORT_SHEETS,
    exportTemplate,
    exportData,
    importData,
    apiPrefix: K10_API_PREFIX,
  }
}

export default useK10ImportExport
