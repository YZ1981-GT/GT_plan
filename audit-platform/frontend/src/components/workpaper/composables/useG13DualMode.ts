...
export type G13RenderMode = 'html' | 'onlyoffice'

const STORAGE_PREFIX = 'g13-dual-mode:'

export function useG13DualMode(options: {
  wpId: Ref<string>
  reloadAll?: () => Promise<void>
}) {
  const currentMode = ref<G13RenderMode>('html')
  ...
  async function onModeChange(val: string | number | boolean): Promise<void> {
    const target = val as G13RenderMode
