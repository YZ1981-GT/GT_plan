/**
 * useL2FormData — L2 应付利息数据加载/debounce保存/即时保存/TB回写
 *
 ...
 * - item_id 命名: 前缀 "L2-{sheet}-{field}"（如 "L2-L2-1-adjudicated_amount"）
 * - writebackTB(2231): 审定数回写 trial_balance 科目 2231 应付利息
...
export interface UseL2FormDataOptions {
  wpId: Ref<string>
...
export function useL2FormData(options: UseL2FormDataOptions) {
  ...
  async function selfLoad(): Promise<void> {
    ...
    try {
      await api.get(
        `/api/workpapers/${wpId.value}/render-config?force_component_type=l2-interest-payable`,
        { _silent: true } as any,
      )
    } catch {
  ...
  /**
   * 从 checklist_responses 加载 L2 数据（item_id 前缀 "L2-"）
   */
  async function loadResponses(): Promise<void> {
    ...
    try {
      ...
      ElMessage.warning('L2数据加载失败，可手动填写')
    }
  }
   ...
   * @param itemId - 如 "L2-L2-1-adjudicated_amount"
