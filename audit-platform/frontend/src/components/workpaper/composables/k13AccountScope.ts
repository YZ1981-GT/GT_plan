 ...
 * K13 营业外支出 — 科目码单一真源。
 *
 * 报表行：IS-021（两准则同号；2026-08-09 由 IS-043 改正 soe 侧）
 * 公式：TB('6711','本期发生额') 四准则一致
 * 兜底码：6711
 * 损益类/借方科目 —— 取本期发生额非余额
 *
 ...
 * soe 侧原写 ``IS-043``，而该码在 `report_config` 里实际是
 * listed 侧「4. 其他债权投资信用减值准备」/ soe 侧「减： 营业外支出」，两侧 formula 均 NULL。
 *
 ...
 *
 *     IS-021 营业外支出 = TB('6711','本期发生额')
 *
 * 🔴 两准则**同号**（K 循环无一例外）。保留 LISTED/SOE 两个常量是为了与其余
 * K 循环形态一致，**不是**因为它们该不同。
 */
export const K13_REPORT_ROW_CODE_LISTED = 'IS-021'

/** 报表行编码 —— 国企准则（与上市同号，见上方改正说明） */
export const K13_REPORT_ROW_CODE_SOE = 'IS-021'

/** 兜底标准码 */
export const K13_FALLBACK_STANDARD = '6711'

/** 科目中文名（展示用） */
export const K13_ACCOUNT_NAME = '营业外支出'

/** render 下发的取数溯源结构（只取本文件需要的字段） */
export interface K13TbSourceCodes {
  gross?: string[] | null
 ...
 */
export function k13QueryCodes(src?: K13TbSourceCodes | null): string[] {
  const resolved = normalize(src?.gross)
  return resolved.length ? resolved : [K13_FALLBACK_STANDARD]
}
 ...
 */
export function k13AccountCode(src?: K13TbSourceCodes | null): string {
  const resolved = normalize(src?.gross_standard)
  return resolved[0] || K13_FALLBACK_STANDARD
}
