/**
 * useK5ImportExport — K5 预计负债 导入导出 composable
 *
 ...
 *
 * 动态行表格 × 3端点：
 *   POST /api/workpapers/{wpId}/k5/export-template?sheet={code}
 *   POST /api/workpapers/{wpId}/k5/export-data?sheet={code}
 *   POST /api/workpapers/{wpId}/k5/import-data  (multipart/form-data, sheet={code})
 *
 * sheet codes: K5-2 / K5-3
 *   K5-2 明细表（23列3区段+或有判断+42行动态行）
 *   K5-3 调整分录
 *
...
/** K5 支持导入导出的 sheet 编码（动态行表格） */
export type K5ImportableSheet = 'K5-2' | 'K5-3'

/** 导入结果 */
export interface K5ImportResult {
  rowCount: number
  fieldCount: number
  warning?: string
  errors?: string[]
...
/** sheet 元数据（供下拉菜单展示） */
export interface K5SheetMeta {
  code: K5ImportableSheet
  label: string
  /** 多区段分sheet导出 */
  multiSheet?: boolean
}
...
export const K5_API_PREFIX = 'k5'

/** 可导入导出 sheet 的中文标签 */
export const K5_IMPORT_EXPORT_SHEETS: K5SheetMeta[] = [
  { code: 'K5-2', label: 'K5-2 明细表', multiSheet: true },
  { code: 'K5-3', label: 'K5-3 调整分录' },
]
 ...
 */
export function resolveK5ImportableSheet(sheetName: string): K5ImportableSheet | null {
  const m = (sheetName || '').match(/K5-(\d+)/)
  if (!m) return null
  const code = `K5-${m[1]}` as K5ImportableSheet
  return K5_IMPORT_EXPORT_SHEETS.some((s) => s.code === code) ? code : null
}

// ═══ Composable 接口 ═══

export interface UseK5ImportExportOptions {
  wpId: Ref<string>
  projectId: Ref<string>
  sheetCode: string
}

export interface UseK5ImportExportReturn {
  /** 是否正在导出中 */
  ...
  importData: (file: File) => Promise<K5ImportResult | null>
  /** API前缀 */
  apiPrefix: string
}
...
export function useK5ImportExport(params: UseK5ImportExportOptions): UseK5ImportExportReturn {
  ...
  function apiBase(): string {
    return `/api/workpapers/${wpId.value}/${K5_API_PREFIX}`
  }

  /**
   * 导出模板 — POST /api/workpapers/{wpId}/k5/export-template?sheet={code}
   */
  ...
  /**
   * 导出数据 — POST /api/workpapers/{wpId}/k5/export-data?sheet={code}
   */
  ...
  /**
   * 导入数据 — POST /api/workpapers/{wpId}/k5/import-data
   * multipart/form-data 上传 xlsx 文件, query param sheet={code}
   */
  async function importData(file: File): Promise<K5ImportResult | null> {
    lastError.value = null
  ...
  return {
    ...
    isImporting,
    lastError,
    sheets: K5_IMPORT_EXPORT_SHEETS,
    exportTemplate,
    exportData,
    importData,
    apiPrefix: K5_API_PREFIX,
  }
}

export default useK5ImportExport
