/**
 * useK12ImportExport — K12 营业外收入 导入导出 composable
 *
 ...
 *
 * 动态行表格 × 3端点：
 *   POST /api/workpapers/{wpId}/k12/export-template
 *   POST /api/workpapers/{wpId}/k12/export-data
 *   POST /api/workpapers/{wpId}/k12/import-data  (multipart/form-data)
 *
 * 仅 K12-2 明细表需要导入导出（动态行）
 *
...
/** K12 支持导入导出的 sheet 编码（仅 K12-2 明细表，动态行） */
export type K12ImportableSheet = 'K12-2'

/** 导入结果 */
export interface K12ImportResult {
  rowCount: number
  fieldCount: number
  warning?: string
  errors?: string[]
...
/** sheet 元数据（供下拉菜单展示） */
export interface K12SheetMeta {
  code: K12ImportableSheet
  label: string
}

// ═══ 常量 ═══

export const K12_API_PREFIX = 'k12'

/** 可导入导出 sheet 的中文标签（仅 K12-2 明细表） */
export const K12_IMPORT_EXPORT_SHEETS: K12SheetMeta[] = [
  { code: 'K12-2', label: 'K12-2 明细表' },
]
 ...
 */
export function resolveK12ImportableSheet(sheetName: string): K12ImportableSheet | null {
  const m = (sheetName || '').match(/K12-(\d+)/)
  if (!m) return null
  const code = `K12-${m[1]}` as K12ImportableSheet
  return K12_IMPORT_EXPORT_SHEETS.some((s) => s.code === code) ? code : null
}

// ═══ Composable 接口 ═══

export interface UseK12ImportExportOptions {
  wpId: Ref<string>
  projectId: Ref<string>
  sheetCode: string
}

export interface UseK12ImportExportReturn {
  /** 是否正在导出中 */
  ...
  importData: (file: File) => Promise<K12ImportResult | null>
  /** API前缀 */
  apiPrefix: string
}
...
export function useK12ImportExport(params: UseK12ImportExportOptions): UseK12ImportExportReturn {
  ...
  /**
   * 导出模板 — POST /api/workpapers/{wpId}/k12/export-template
   */
  ...
  /**
   * 导出数据 — POST /api/workpapers/{wpId}/k12/export-data
   */
  ...
  /**
   * 导入数据 — POST /api/workpapers/{wpId}/k12/import-data
   * multipart/form-data 上传 xlsx 文件, body 含 sheet_code
   */
  async function importData(file: File): Promise<K12ImportResult | null> {
    lastError.value = null
    ...
    try {
      ...
      const response = await http.post(`${apiBase()}/k12/import-data`, formData, {
  ...
  return {
    ...
    isImporting,
    lastError,
    sheets: K12_IMPORT_EXPORT_SHEETS,
    exportTemplate,
    exportData,
    importData,
    apiPrefix: K12_API_PREFIX,
  }
}

export default useK12ImportExport
