/**
 * useI5DualMode — I5 其他非流动资产 HTML ↔ OnlyOffice 双模式切换
 *
...
export type I5RenderMode = 'html' | 'onlyoffice'

// ─── Constants ───────────────────────────────────────────────────────────────

const STORAGE_PREFIX = 'i5-dual-mode:'

// ─── Options ─────────────────────────────────────────────────────────────────

export interface UseI5DualModeOptions {
  wpId: Ref<string>
...
export function useI5DualMode(options: UseI5DualModeOptions) {
  const { wpId, sheetName, autoSave, reloadAll } = options

  const currentMode = ref<I5RenderMode>('html')
  ...
  function persistMode(mode: I5RenderMode): void {
    try {
   ...
   */
  async function switchMode(target: I5RenderMode): Promise<void> {
  ...
  function onModeChange(val: string | number | boolean): void {
    void switchMode(val as I5RenderMode)
  }
