/**
 * useK9CrossSheet — K9 管理费用跨Sheet交叉验证 computed 引擎
 *
 * 通过 allResponses Map 实现跨 sheet 数据流（纯 computed 响应式链，不走 API）。
 * 数据读取模式：allResponses.get('K9-{sheet}-{field}')?.remark 存数值。
 *
 * 跨Sheet映射（数据流图）：
 * - K9-1 审定表合计 vs K9-2 明细表合计（adjudicationVsDetail）
 * - K9-4 实质性分析合计 vs K9-2 明细表合计（analysisVsDetail）
 *
 * 科目方向：
 * - 6602 管理费用（借方/损益类）：取发生额=借方发生-贷方发生(红冲)
 *
...
/**
 * K9 跨Sheet computed links：
 * - K9-1 审定表审定合计 ↔ K9-2 明细表发生额合计
 * - K9-4 实质性分析合计 ↔ K9-2 明细表发生额合计
 *
...
export function useK9CrossSheet(allResponses: Ref<Map<string, any>>): {
  ...
  // ═══ adjudicationVsDetail: K9-1 审定表合计 vs K9-2 明细表合计（Req 2.5）═══

  /**
   * K9-1 审定表管理费用审定合计 与 K9-2 明细表本期发生额合计交叉验证。
   * diff = K9-1 审定 total - K9-2 明细发生额 total
  ...
  // ═══ analysisVsDetail: K9-4 实质性分析合计 vs K9-2 明细表合计（Req 3.2, 4.6）═══

  /**
   * K9-4 实质性分析本期发生额合计 与 K9-2 明细表发生额合计交叉验证。
   * diff = K9-4 分析本期合计 - K9-2 明细发生额合计
   * isMatch = |diff| < 0.01
   *
   * K9-4 按费用项目逐项分析（同比/占比/异常），其合计应与 K9-2 明细表一致。
...
export default useK9CrossSheet
