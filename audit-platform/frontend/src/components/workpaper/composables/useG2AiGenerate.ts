/**
 * useG2AiGenerate — G2 应收利息 AI 辅助生成
 * POST /api/workpapers/{wpId}/g2/ai/{section}
 */
import { onMounted, ref, type Ref } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import http from '@/utils/http'

export type G2AiSection =
  | 'adjudication-note'
...
export function useG2AiGenerate(wpId: Ref<string>) {
  ...
  async function generateAndConfirm(
    ...
    try {
      const response = await http.post(
        `/api/workpapers/${wpId.value}/g2/ai/${section}`,
...
export default useG2AiGenerate
