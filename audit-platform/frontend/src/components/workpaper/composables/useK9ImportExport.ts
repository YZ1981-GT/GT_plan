/**
 * useK9ImportExport — K9 管理费用 导入导出 composable
 *
...
/** K9 支持导入导出的 sheet 编码（动态行表格） */
export type K9ImportableSheet = 'K9-2' | 'K9-3'

/** 导入结果 */
export interface K9ImportResult {
  rowCount: number
  fieldCount: number
  warning?: string
  errors?: string[]
...
/** sheet 元数据（供下拉菜单展示） */
export interface K9SheetMeta {
  code: K9ImportableSheet
  label: string
  /** 多区段分sheet导出 */
  multiSheet?: boolean
}
...
export const K9_API_PREFIX = 'k9'

/** 可导入导出 sheet 的中文标签 */
export const K9_IMPORT_EXPORT_SHEETS: K9SheetMeta[] = [
  { code: 'K9-2', label: 'K9-2 明细表', multiSheet: true },
  { code: 'K9-3', label: 'K9-3 调整分录' },
]
 ...
 */
export function resolveK9ImportableSheet(sheetName: string): K9ImportableSheet | null {
  const m = (sheetName || '').match(/K9-(\d+)/)
  if (!m) return null
  const code = `K9-${m[1]}` as K9ImportableSheet
  return K9_IMPORT_EXPORT_SHEETS.some((s) => s.code === code) ? code : null
}

// ═══ Composable 接口 ═══

export interface UseK9ImportExportOptions {
  wpId: Ref<string>
  projectId: Ref<string>
  sheetCode: string
}

export interface UseK9ImportExportReturn {
  /** 是否正在导出中 */
  ...
  importData: (file: File) => Promise<K9ImportResult | null>
  /** API前缀 */
  apiPrefix: string
}
...
export function useK9ImportExport(params: UseK9ImportExportOptions): UseK9ImportExportReturn {
  ...
  return {
    ...
    isImporting,
    lastError,
    sheets: K9_IMPORT_EXPORT_SHEETS,
    exportTemplate,
    exportData,
    importData,
    apiPrefix: K9_API_PREFIX,
  }
}

export default useK9ImportExport
