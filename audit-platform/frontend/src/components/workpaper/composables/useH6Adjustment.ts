/**
 * useH6Adjustment — H6-3 调整分录 composable（10列+借贷平衡+EventBus同步AJE/RJE到H6-1）
 *
...
export function useH6Adjustment(params: {
  ...
  const { allResponses, onSave, onPublishEvent, onSyncToAdjudication } = params
  ...
  /** 发布调整分录事件 + 同步AJE/RJE到H6-1审定表 */
  function publishAndSync(): void {
    // 1. 发布EventBus事件（供A13错报汇总消费）
    if (onPublishEvent) {
    ...
    // 2. 双向同步AJE/RJE到H6-1审定表
    if (onSyncToAdjudication) {
      onSyncToAdjudication(ajeTotalNet.value, rjeTotalNet.value)
    }
  }
  ...
  function _persist(): void {
    ...
    // 同步写入AJE/RJE净额（供CrossSheet/H6-1读取）
    onSave(AJE_NET_KEY, ajeTotalNet.value)
    onSave(RJE_NET_KEY, rjeTotalNet.value)
  }
