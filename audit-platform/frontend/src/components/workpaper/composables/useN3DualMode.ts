/**
 * useN3DualMode — N3 递延所得税负债双模式切换 composable
 *
...
/** N3 渲染模式 */
export type N3RenderMode = 'html' | 'onlyoffice'

/** el-segmented 选项 */
export interface N3ModeOption {
  label: string
  value: N3RenderMode
  disabled?: boolean
}

export interface UseN3DualModeOptions {
  wpId: Ref<string>
  defaultMode?: N3RenderMode
}

// ─── Constants ───────────────────────────────────────────────────────────────

/** localStorage key */
const STORAGE_KEY = 'n3-dual-mode'
 ...
 */
export function useN3DualMode(options: UseN3DualModeOptions) {
  const { wpId, defaultMode = 'html' } = options

  // ─── State ─────────────────────────────────────────────────────────────

  const currentMode = ref<N3RenderMode>(defaultMode)
  const isOOHealthy = ref<boolean>(false)
  const ooChecking = ref(false)

  // ─── localStorage 持久化 ───────────────────────────────────────────────

  function _loadPersistedMode(): N3RenderMode {
    try {
  ...
  function _persistMode(m: N3RenderMode): void {
    try {
   ...
   */
  function switchMode(newMode: N3RenderMode): void {
  ...
  /** el-segmented 模式选项列表 */
  const modes = computed<N3ModeOption[]>(() => [
