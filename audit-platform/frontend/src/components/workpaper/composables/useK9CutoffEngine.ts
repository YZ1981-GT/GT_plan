 ...
 *
 * 截止测试双向：
 * - K9-6 记账凭证 → 原始凭证（真实性认定）
 * - K9-7 原始凭证 → 记账凭证（完整性认定）
 *
 * 所有函数为纯函数，便于PBT验证。
...
export interface CutoffSample {
  /** 源序时账分录 */
  entry: LedgerEntry
  /** 相对期末的方向：before=期末前记账，after=期末后记账 */
  direction: 'before_period_end' | 'after_period_end'
  /** 距期末天数（正=期末后，负=期末前） */
  daysDiff: number
}
 ...
 * @param _periodEnd 会计期间截止日 YYYY-MM-DD（预留扩展，当前以自然月比较）
...
export function autoSampleCutoff(
  ...
  // 计算日期窗口
  const msPerDay = 24 * 60 * 60 * 1000
  const windowStart = new Date(endDate.getTime() - safeDays * msPerDay)
  const windowEnd = new Date(endDate.getTime() + safeDays * msPerDay)
  ...
  for (const entry of ledger) {
    ...
    if (entryDate >= windowStart && entryDate <= windowEnd) {
      const diff = calcDaysDiff(entryDate, endDate)
      const direction: CutoffSample['direction'] = diff <= 0
        ? 'before_period_end'
        : 'after_period_end'
