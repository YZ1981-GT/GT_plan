...
describe('useH4Adjustment', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('loads and normalizes legacy debit/credit/entryType/refIndex fields', () => {
    ...
    expect(state.rows.value[0].indexRef).toBe('H4-7')
    expect(state.rows.value[0].accountName).toBe('资产减值损失')
    expect(state.rows.value[1].description).toBe('补提减值')
    expect(state.rows.value[1].reportItem).toBe('工程物资')
    expect(state.isBalanced.value).toBe(true)
    expect(state.emAjeNet.value).toBe(-1000)
  })

  it('addRow defaults to 账项调整 + 1605 and persists', () => {
    const allResponses = makeMap()
    ...
    expect(state.rows.value[0].category).toBe('账项调整')
    expect(state.rows.value[0].accountCode).toBe('1605')
    expect(state.rows.value[0].reportItem).toBe('工程物资')
    expect(saves.some((s) => s.id === 'H4-3-rows')).toBe(true)
    const persisted = saves.find((s) => s.id === 'H4-3-rows')!.val as any[]
    expect(persisted[0].debit).toBe(0)
    expect(persisted[0].debitAmount).toBe(0)
    expect(persisted[0].category).toBe('账项调整')
  })

  it('isBalanced and emAjeNet/emRjeNet', () => {
    const allResponses = makeMap([
      {
        category: '账项调整',
        accountCode: '1605',
    ...
    expect(state.isBalanced.value).toBe(true)
    expect(state.emAjeNet.value).toBe(500)
    expect(state.emRjeNet.value).toBe(100)
    expect(state.ajeNet.value).toBe(0)
  })

  it('adjGroups folds rows by description and reports group balance', () => {
    const allResponses = makeMap([
      {
        description: '补提减值-A',
        category: '账项调整',
    ...
    expect(state.adjGroups.value).toHaveLength(2)
    const impair = state.adjGroups.value.find((g) => g.description === '补提减值-A')!
    expect(impair.lineCount).toBe(2)
    expect(impair.isBalanced).toBe(true)
    expect(impair.debitTotal).toBe(100)
  })

  it('syncFromAdjustmentModule imports full groups containing 1605', async () => {
        ...
        items: [
          {
            ...
            line_items: [
              ...
              { standard_account_code: '1605', account_name: '工程物资', debit_amount: 0, credit_amount: 200 },
            ],
          },
          {
            entry_group_id: 'g2',
    ...
    const allResponses = makeMap([
      { rowId: 'manual-1', category: '账项调整', accountCode: '1605', debitAmount: 10, creditAmount: 0 },
    ])
    const state = useH4Adjustment({
      wpId: ref('wp1'),
      projectId: ref('p1'),
