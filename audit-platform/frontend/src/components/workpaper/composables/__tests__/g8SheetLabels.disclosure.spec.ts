/**
 * isG8SheetComplete — 附注完成度收紧（指定原因 + 审定勾稽 + 结论）
 */
...
describe('isG8SheetComplete disclosure', () => {
  ...
  it('有金额无结论不算完成', () => {
    const m = mapOf({
      'G8-disclosure-listed': { remark: v2Store(100, '战略持有') },
  ...
  it('有结论但无指定原因不算完成', () => {
    const m = mapOf({
      ...
      'G8-1-adjudicated-amount': { conclusion: '100' },
      'G8-disclosure-listed-audit-conclusion': { remark: 'A、未见异常' },
    })
    expect(isG8SheetComplete('附注上市', m)).toBe(false)
  })
  ...
  it('仅附注汇总 + 结论不算完成', () => {
    const m = mapOf({
      ...
      'G8-disclosure-soe-audit-conclusion': { remark: 'A、完整' },
    })
    expect(isG8SheetComplete('附注国企', m)).toBe(false)
  })

  it('兼容旧扁平键（含 designation）+ 勾稽 + 结论', () => {
    const m = mapOf({
