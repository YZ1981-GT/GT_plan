/**
 * useM5ImportExport — M5 盈余公积导入导出三级 composable
 *
 ...
 * - 多sheet导出支持（M5-2 明细表为主要导入导出目标，动态行按法定/任意盈余公积分类）:
 *   - M5-2 明细表（primary — 法定+任意盈余公积动态行）
 * - 后端三端点:
 *   - GET  /api/m5-surplus-reserve/{wp_id}/export-template?sheet=xxx
 *   - GET  /api/m5-surplus-reserve/{wp_id}/export-data?sheet=xxx
 *   - POST /api/m5-surplus-reserve/{wp_id}/import-data (multipart/form-data)
 * - RFC5987 中文文件名解析
...
/** M5 支持导入导出的 sheet（动态行表格） */
export type M5ImportableSheet =
  | 'M5-2'   // 明细表（法定+任意盈余公积，primary）

/** 导入结果 */
export interface M5ImportResult {
  /** 是否成功 */
  success: boolean
  /** 消息 */
  message: string
...
export interface UseM5ImportExportOptions {
  wpId: Ref<string>
...
export const M5_IMPORTABLE_SHEETS: { value: M5ImportableSheet; label: string }[] = [
  { value: 'M5-2', label: '明细表（法定+任意盈余公积）' },
]

/** 可导出sheet编码列表 */
const EXPORTABLE_SHEETS: M5ImportableSheet[] = ['M5-2']
...
function buildApiUrl(wpId: string, action: string): string {
  return `/api/m5-surplus-reserve/${wpId}/${action}`
}
 ...
 */
export function useM5ImportExport(options: UseM5ImportExportOptions) {
  ...
  function isSheetExportable(sheet: string): boolean {
    return EXPORTABLE_SHEETS.includes(sheet as M5ImportableSheet)
  }

  /** 获取所有可导出sheet列表 */
  function getExportableSheets(): M5ImportableSheet[] {
  ...
  /**
   * 导入xlsx数据（M5-2 明细表为主要目标，法定+任意盈余公积动态行）
   * POST /api/m5-surplus-reserve/{wp_id}/import-data (multipart/form-data)
   */
  async function importData(sheetName: string, file: File): Promise<M5ImportResult | null> {
...
export default useM5ImportExport
