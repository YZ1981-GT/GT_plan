/**
 * useN1ImportExport — N1 递延所得税资产导入导出三级 composable
 *
 ...
 *   - GET /api/n1-deferred-tax-assets/{wpId}/export-template?sheet=xxx
 *   - GET /api/n1-deferred-tax-assets/{wpId}/export-data?sheet=xxx
 *   - POST /api/n1-deferred-tax-assets/{wpId}/import-data?sheet=xxx
 * - 使用 http (axios) NOT native fetch（需 Authorization header → 401 without it!）
 * - 动态行sheet支持：N1-2明细表（按暂时性差异项目明细）
...
export interface UseN1ImportExportOptions {
  wpId: Ref<string>
...
/** API base path */
const API_BASE = '/api/n1-deferred-tax-assets'
 ...
 */
export function useN1ImportExport(options: UseN1ImportExportOptions) {
   ...
   * GET /api/n1-deferred-tax-assets/{wpId}/export-template?sheet=xxx
   */
  async function exportTemplate(sheet?: N1ImportableSheet): Promise<void> {
    ...
    try {
      ...
      const filename = parseFilename(
        contentDisposition,
        sheet ? `N1-递延所得税资产-${sheet}-模板.xlsx` : 'N1-递延所得税资产-模板.xlsx',
      )
   ...
   * GET /api/n1-deferred-tax-assets/{wpId}/export-data?sheet=xxx
   */
  async function exportData(sheet?: N1ImportableSheet): Promise<void> {
    if (!wpId.value) return
    isExporting.value = true
    lastError.value = null
    ...
    try {
      ...
      const filename = parseFilename(
        contentDisposition,
        sheet ? `N1-递延所得税资产-${sheet}-数据.xlsx` : 'N1-递延所得税资产-数据.xlsx',
      )
  ...
  /**
   * 导入xlsx数据
   * POST /api/n1-deferred-tax-assets/{wpId}/import-data?sheet=xxx (multipart/form-data)
   */
  async function importData(file: File, sheet?: N1ImportableSheet): Promise<N1ImportResult | null> {
...
export default useN1ImportExport
