...
/**
 * M3 编制/使用手册弹窗 — 对齐 N5PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：M3-2 明细(按回购批次) + M3-4 外币折算 + M3-5 回购/注销核对 → M3-1 审定(未审+AJE+RJE) → 回写 TB(4002)+附注上市。库存股是权益备抵借方(与其他 M 权益类方向相反！)：期末=期初+借方(回购)−贷方(注销/再售)；注销冲减 M2 实收资本、M4 资本公积。本弹窗不改各表页内说明。"
    />
