...
const currentSheet = computed(() => {
  ...
  if (m) return m[1]
  // 底稿目录 K9（无后缀）
  if (/\bK9\b/.test(name) && !/K9-/.test(name) && !/K9A/.test(name)) return 'K9'
  return ''
})
  ...
  try {
    ...
    if (props.htmlData) {
      ...
      const res = await http.get(`/api/workpapers/${props.wpId}/render-config`, {
        params: { force_component_type: 'k9-admin-expenses' },
        _silent: true,
      } as any)
...
function openReviewDialog(sectionId: string, sectionLabel?: string): void {
  console.log('[K9] openReviewDialog:', sectionId, sectionLabel)
}
...
provide('k9VersionTrailRef', versionTrailRef)
provide('k9OpenVersionHistory', openVersionHistory)
