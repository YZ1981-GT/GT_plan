...
const formData = useG3DivRecFormData({ wpId: wpIdRef, projectId: computed(() => props.projectId) })
...
const useGridFallback = computed(() => {
  ...
  return !!code && code !== 'G3A' && !adjudicationConfig.value && !code.startsWith('附注')
})
...
.g3-dividend-receivable-toolbar { margin-bottom: 8px; }
</style>
