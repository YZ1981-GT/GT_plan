...
const { centralStatus, syncing: centralSyncing, syncToCentral, refreshStatus } = useAdjustmentCentralSync({
  ...
  buildLineItems: () => adj.rows.value.map((r) => ({
    ...
    report_line_code: r.fsItem || undefined,
    debit_amount: r.debitAmount,
    credit_amount: r.creditAmount,
  })),
  buildMeta: () => ({
    description: adj.rows.value.find((r) => r.adjustmentDesc)?.adjustmentDesc || 'G12 净敞口套期收益调整',
    adjustmentType: adj.rows.value.length > 0
      && adj.rows.value.every((r) => String(r.category).toLowerCase().includes('rje') || String(r.category).includes('报表'))
      ? 'rje' : 'aje',
  }),
})
