<template>
  <div class="check-direction-table">
    ...
    >
      <el-table-column label="名称/类别" min-width="120" fixed>
      ...
      <el-table-column label="抽盘−账面" width="84" align="right" class-name="auto-calc-col">
        <template #default="{ row }">
          <el-tooltip content="抽盘数量 − 账面数量（+增 −减）" placement="top">
      ...
      <el-table-column label="品质状况" width="100">
        <template #default="{ row }">
          ...
          >
            <el-option v-for="q in H4_QUALITY_OPTS" :key="q" :label="q" :value="q" />
          </el-select>
        </template>
      </el-table-column>
      <el-table-column label="存放位置" min-width="90">
        <template #default="{ row }">
          <el-input
...
const emit = defineEmits<{
  (e: 'update', rowId: string, patch: Partial<H4StocktakeCheckRow>): void
...
function rowClassName({ row }: { row: H4StocktakeCheckRow & { hasVariance?: boolean } }): string {
  const parts: string[] = []
  if (row.hasVariance) parts.push('warn-row')
  if (['闲置', '积压', '毁损', '待报废'].includes(row.qualityStatus)) parts.push('concern-row')
  return parts.join(' ')
}
...
:deep(.concern-row) { --el-table-tr-bg-color: #fef9e7; }
:deep(.warn-row.concern-row) { --el-table-tr-bg-color: var(--el-color-danger-light-9); }
