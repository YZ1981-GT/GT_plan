        ...
        <div class="section-header">
          ...
          <div class="header-actions">
            <el-button
              ...
              @click="handleSave"
            >保存</el-button>
            <el-button size="small" @click="handleOpenReview('s10-adjudication', '审定表S10-1')">
              复核
            </el-button>
...
/**
 * S10AdjudicationSheet.vue — 审定表 S10-1 环境事项
 *
 * 功能：
 * - 25×6 审定表结构，包含未审数/调整/审定数/分录编号/索引号
 * - 审定数 = 未审数 + 审计调整（公式自动计算）
...
function handleAiAssist() {
  ElMessage.info('AI辅助功能开发中，将自动分析环境事项相关科目异常变动')
}
...
async function handleSave() {
  ...
  try {
    ...
    await http.put(
      ...
      { sheet_name: '审定表S10-1', data: payload }
    )
    isDirty.value = false
    emit('save')
...
async function loadData() {
  try {
    ...
    const sheet = sheets.find((s: any) =>
      s.sheet_name?.includes('审定表S10') || s.sheet_name?.match(/S10-1/)
    )
    ...
    if (htmlData?.adjudication_rows?.length) {
      ...
      adjudicationRows.value = [
        { id: 's10-1-0', accountName: '环保设施支出', accountCode: '', unadjustedAmount: 0, adjustmentAmount: 0, auditedAmount: 0, ajeRef: '', indexRef: '', editable: true },
        { id: 's10-1-1', accountName: '排污费/环境保护税', accountCode: '', unadjustedAmount: 0, adjustmentAmount: 0, auditedAmount: 0, ajeRef: '', indexRef: '', editable: true },
        { id: 's10-1-2', accountName: '环境修复费用', accountCode: '', unadjustedAmount: 0, adjustmentAmount: 0, auditedAmount: 0, ajeRef: '', indexRef: '', editable: true },
        { id: 's10-1-3', accountName: '废物处理费', accountCode: '', unadjustedAmount: 0, adjustmentAmount: 0, auditedAmount: 0, ajeRef: '', indexRef: '', editable: true },
        { id: 's10-1-4', accountName: '环境监测费', accountCode: '', unadjustedAmount: 0, adjustmentAmount: 0, auditedAmount: 0, ajeRef: '', indexRef: '', editable: true },
        { id: 's10-1-5', accountName: '环保罚款/赔偿', accountCode: '', unadjustedAmount: 0, adjustmentAmount: 0, auditedAmount: 0, ajeRef: '', indexRef: '', editable: true },
      ]
    }
