  ...
  <div class="s9-internal-control">
    ...
    <el-card shadow="never" class="audit-section">
        ...
        <div class="section-header">
          ...
          <div class="header-actions">
            <el-button
              ...
              @click="handleSave"
            >保存</el-button>
            <el-button size="small" @click="handleOpenReview('s9-internal-control', '内控调查表S9-2')">
              复核
            </el-button>
      ...
      <details class="compile-hint">
        ...
        <div class="methodology-context">
          <p>本调查表用于了解被审计单位与电子商务相关的内部控制设计与运行情况。</p>
          <p>审计人员应针对每项控制活动评价其设计有效性（"是"/"弱"/"否"/"不适用"），并记录相关审计证据。</p>
          <p>对于评价为"弱"或"否"的项目，应进一步分析控制缺陷对财务报表的潜在影响。</p>
        </div>
      </details>
    ...
    <el-card shadow="never" class="audit-section">
      ...
      <el-input
        ...
        :disabled="isReadonly"
        placeholder="根据上述内部控制调查结果，对电子商务相关内部控制的整体评价..."
        @change="markDirty"
      />
    </el-card>
  </div>
...
/**
 * S9InternalControlSheet.vue — S9-2 电子商务相关内部控制调查表
 *
 * 功能：
 * - 66×8 检查表结构，每行一个控制活动评价项
...
// ─── S9-2 默认检查项目（电子商务内部控制典型检查事项） ─────────────────────

const DEFAULT_CHECKLIST_ITEMS = [
  '被审计单位是否建立了电子商务相关的内部控制制度和管理流程',
  '电子商务系统是否具备完善的用户身份认证和授权管理机制',
  '电子商务交易数据是否与财务系统实时或定期核对',
  '是否对电子商务平台的订单处理、发货、退款等关键环节设置了适当的审批流程',
  '电子商务系统是否具有完善的日志记录功能，能追踪关键交易操作',
  '是否对电子商务相关的收入确认建立了适当的内部控制',
  '电子支付渠道（支付宝、微信支付、银联等）的对账是否及时和完整',
  '是否对电子商务退货退款建立了适当的授权审批和核算控制',
  '电子商务平台的库存管理是否与实物仓储系统有效衔接',
  '是否对客户个人信息和交易数据建立了信息安全保护机制',
  '电子商务涉及的增值税电子发票管理是否合规',
  '是否定期对电子商务平台的系统安全进行评估和更新',
  '跨境电子商务（如有）是否符合相关外汇管理和海关监管要求',
  '电子商务促销活动（优惠券/满减/积分等）的会计处理是否有明确的控制流程',
  '是否对第三方物流服务商的服务质量和账务核对建立了监控机制',
  '电子商务平台的系统备份和灾难恢复计划是否有效',
]
...
function handleAiAssist() {
  ElMessage.info('AI辅助功能开发中，将基于被审计单位的电子商务业务特征自动推荐关注要点')
}
...
async function handleSave() {
  ...
  try {
    ...
    await http.put(
      ...
      { sheet_name: '内部控制调查表S9-2', data: payload }
    )
    isDirty.value = false
...
async function loadData() {
  try {
    ...
    const sheet = sheets.find((s: any) =>
      s.sheet_name?.includes('内部控制调查表') || s.sheet_name?.includes('S9-2')
    )
