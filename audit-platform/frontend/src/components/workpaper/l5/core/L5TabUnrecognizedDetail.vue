<template>
  <div class="l5-tab-unrecognized-detail">
    ...
    <el-alert type="info" :closable="false" show-icon style="margin-bottom: 12px">
      <template #title>
        <strong>审计目标：</strong>核实未确认融资费用初始确认（名义金额−现值）与后续摊销的准确性，与 L5-2 各款项及 L5-5 摊销测算核对一致。
      </template>
    </el-alert>
...
function handleAI(section: string) {
  import('@/utils/http').then(({ default: h }) => {
    h.post(`/api/workpapers/${props.wpId}/ai/generate-text`, {
      section: `l5-unrecognized-detail-${section}`,
...
<style scoped>
.l5-tab-unrecognized-detail { padding: 12px; font-size: var(--wp-font-size, 13px); }
