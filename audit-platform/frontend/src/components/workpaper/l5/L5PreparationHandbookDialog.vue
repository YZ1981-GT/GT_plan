...
/**
 * L5 编制/使用手册弹窗 — 对齐 L1PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：L5-2 明细(按款项：融资租赁/分期购入/其他) + L5-3 未确认融资费用明细 + L5-5 摊销测算(实际利率法，联动 L8) → L5-1 审定(三区段：原值−未确认=净值，未审+账项调整+重分类，双期+减一年内到期) → 回写 TB(2701 原值/2702 未确认融资费用)+附注。长期应付款是负债类贷方：期末=期初+贷方(借入)−借方(偿还)；未确认融资费用是负债备抵借方：期末=期初+借方−贷方(摊销)。与经审计财报核对含专项应付款。本弹窗不改各表页内说明。"
    />
