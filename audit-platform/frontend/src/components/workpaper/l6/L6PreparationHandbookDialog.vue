...
/**
 * L6 编制/使用手册弹窗 — 对齐 L1PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：L6-2 明细(按专项项目逐项) → L6-1 审定(按专项项目，未审+账项调整+重分类，双期动态行) → 回写 TB(2601)+附注。专项应付款是负债类贷方：期末=期初+贷方(拨入)−借方(使用/结转)，无减一年内到期。L6-4 专款专用核查(实际用途 vs 批准用途 + 结余处理)为核心程序。与专项储备(4301)/递延收益(2401)区分。本弹窗不改各表页内说明。"
    />
