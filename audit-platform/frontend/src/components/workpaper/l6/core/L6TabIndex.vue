<template>
  <div class="l6-tab-index">
    <!-- ═══ 项目信息区 ═══ -->
    <el-card shadow="never" class="l6-project-info">
      <div class="info-grid">
        <div class="info-item">
    ...
    <!-- ═══ 蓝色渐变操作引导区 ═══ -->
    <div class="l6-guide">
      <div class="l6-guide-header">
        <el-icon><InfoFilled /></el-icon>
        <span>操作步骤引导</span>
      </div>
      <div class="l6-guide-steps">
        <div class="step-item">
          <span class="step-num">①</span>
          <span class="step-text">填写审定表（L6-1）确认专项应付款科目余额（贷方/负债类）</span>
        </div>
        <div class="step-item">
          <span class="step-num">②</span>
          <span class="step-text">录入明细（L6-2）按专项项目列示拨款来源与资金使用</span>
        </div>
        <div class="step-item">
          <span class="step-num">③</span>
          <span class="step-text">完成检查表（L6-4）核查专款专用 + 审计结论</span>
        </div>
        <div class="step-item">
          <span class="step-num">④</span>
          <span class="step-text">录入调整分录（L6-3）+ 核对附注披露信息</span>
        </div>
      </div>
    </div>

    <!-- ═══ 总体进度 ═══ -->
    <div class="l6-progress-section">
    ...
    <!-- ═══ 底稿目录表格 ═══ -->
    <el-card shadow="never" class="l6-index-card">
      <template #header>
        <span class="card-title">致同会计师事务所 / 专项应付款底稿</span>
      </template>
      <el-table
        :data="sheetRows"
        border
    ...
    <details class="l6-details-tip">
      ...
      <ul>
        <li>专项应付款为<strong>负债类贷方科目</strong>：期末 = 期初 + 贷方（拨入）− 借方（使用/结转）</li>
        <li>科目编码：2601 专项应付款</li>
        <li>专项应付款多为政府专项拨款，核查<strong>专款专用</strong>是核心审计程序</li>
        <li>明细表按专项项目列示，需与审定表合计交叉核对</li>
        <li>实际用途与批准用途不符时应重点关注，判断是否存在挪用</li>
        <li>结余资金按规定处理（返还/继续使用/结转收益）</li>
...
/**
 * L6TabIndex — L6 专项应付款底稿目录
 *
 * 8 行 sheet 目录列表 + 进度条。
 * 点击行 emit navigate 事件（由 GtL6SpecialPayables 监听切换 sheetName）。
 * 引导区 4 步：审定表 → 明细(专项项目) → 检查表(专款专用) → 调整+附注。
 *
 * Requirements: 1.2
 */
