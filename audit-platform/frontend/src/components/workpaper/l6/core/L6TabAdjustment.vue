...
onMounted(async () => {
  ...
  loadFromResponses(formData.responses.value)
  refreshStatus()
})
...
/**
 * 导入 xlsx 成功后重跑本底稿读回路径：L6-3 读回 = useL6Adjustment.loadFromResponses（任务 4.2 新增，per-field 族）。
 *
...
<style scoped>
.l6-tab-adjustment { padding: 12px; font-size: var(--wp-font-size, 13px); }
