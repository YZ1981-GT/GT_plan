...
const currentSheet = computed(() => {
  ...
  if (m) return m[1]
  // 底稿目录 K13（无后缀）
  if (/\bK13\b/.test(name) && !/K13-/.test(name) && !/K13A/.test(name)) return 'K13'
  return ''
})
  ...
  try {
    if (props.htmlData) {
      ...
      const res = await http.get(`/workpapers/${props.wpId}/render-config`, {
        params: { force_component_type: 'k13-non-operating-expense' },
        _silent: true,
      } as any)
...
function openReviewDialog(sectionId: string, sectionLabel?: string): void {
  console.log('[K13] openReviewDialog:', sectionId, sectionLabel)
}
...
provide('k13VersionTrailRef', versionTrailRef)
provide('k13OpenVersionHistory', openVersionHistory)
