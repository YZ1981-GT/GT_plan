...
const props = defineProps<{
  ...
  rootClass?: string
  /** 默认 true（循环 *A）；F2-21A 等需按类别筛选时传 false */
  hideCategories?: boolean
  cycleSheetMode?: boolean
}>()

const isReadonly = computed(() => props.isReadonly ?? props.readonly ?? false)
const hideCategories = computed(() => props.hideCategories ?? true)
const cycleSheetMode = computed(() => props.cycleSheetMode ?? true)
...
<template>
  <div :class="rootClass ?? 'cycle-program-console-tab'">
    ...
    <GtAProgramConsole
      ...
      :readonly="isReadonly"
      :cycle-sheet-mode="cycleSheetMode"
