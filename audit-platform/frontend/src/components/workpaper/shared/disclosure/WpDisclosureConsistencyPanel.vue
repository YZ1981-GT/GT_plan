<template>
  <el-card shadow="never" class="wp-consistency">
...
/**
 * WpDisclosureConsistencyPanel — 披露内部勾稽面板（平台共用，纯展示）
 *
 * 校验逻辑在各循环的 `xDisclosureConsistency.ts`（纯函数、可单测 + PBT），
 * 共用原语在 `composables/shared/disclosureConsistency.ts`。
 * 「勾稽依据」列直接展示源模板公式出处（如 `源模板 B40=B52`），
 * 「追溯」列用 `GtIndexChip` 跳到来源底稿。
 *
 * 起源于 H1 → N1，由 `n-cycle-tax-disclosure-alignment` Task 2.2 提升为平台共用。
 */
...
import {
  ...
  type WpCheckLevel,
  type WpCheckResult,
} from '../../composables/shared/disclosureConsistency'

const props = withDefaults(
  defineProps<{
    results: WpCheckResult[]
    projectId: string
    defaultExpanded?: boolean
    /** 面板标题（默认「披露内部勾稽」） */
    title?: string
  }>(),
  { defaultExpanded: false, title: '披露内部勾稽' },
)
...
const summary = computed(() => summarizeChecks(props.results))
...
function tagType(level: WpCheckLevel): 'success' | 'warning' | 'danger' {
  if (level === 'error') return 'danger'
  if (level === 'ok') return 'success'
  return 'warning'
}

function levelText(level: WpCheckLevel): string {
  if (level === 'error') return '不一致'
  if (level === 'ok') return '一致'
  return '待补数'
}

function rowClass({ row }: { row: WpCheckResult }): string {
...
<style scoped>
.wp-consistency { margin-bottom: 12px; }
.wp-consistency :deep(.el-card__body) { padding: 10px 12px; }
