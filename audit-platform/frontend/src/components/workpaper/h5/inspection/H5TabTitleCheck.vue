<template>
  <div class="h5-tab-title-check">
    <!-- 审计目标 -->
    <el-alert type="info" :closable="false" title="审计目标：核验油气资产(采矿权/探矿权)的权属、有效期及抵押状况，关注到期预警对持续经营的影响。" class="objective-alert" />

    <!-- 工具栏 -->
    <div class="tab-toolbar">
      <div class="toolbar-right">
        <span class="chip-wrap"><GtIndexChip value="wp:H5-16" :context-project-id="projectId" /></span>
    ...
    <el-card shadow="never" class="block-card">
      ...
      <el-table :data="state.rows.value" border stripe size="small" class="title-table" max-height="500">
        ...
        <el-table-column prop="pledgeAmount" label="抵押金额" min-width="100" align="right">
          <template #default="{ row }">
            <WpAmountInput v-if="!isReadonly" v-model="row.pledgeAmount" size="small" @change="state.updateCell(row.rowId, 'pledgeAmount', $event ?? 0)" />
    ...
    <el-card shadow="never" class="note-card"><template #header><div class="section-title"><span>审计结论</span></div></template>
      <el-input v-model="state.auditConclusion.value" type="textarea" :autosize="{ minRows: 3, maxRows: 6 }" placeholder="填写权属检查审计结论..." :disabled="isReadonly" @blur="state.saveConclusion(state.auditConclusion.value)" /></el-card>
...
const state = useH5TitleCheck({ allResponses: allResponsesRef as any, wpId: toRef(props, 'wpId'), projectId: toRef(props, 'projectId'), onSave: (itemId: string, value: any) => formData.setResponse(itemId, value) })
