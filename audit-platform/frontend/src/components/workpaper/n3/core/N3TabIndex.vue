  ...
  <div class="n3-tab-index">
    ...
    <div class="n3-dir">
      <div class="index-header">
        <h3 class="title">N3 底稿目录</h3>
        <GtReviewTrigger section-id="N3-index-directory" />
      ...
      <N3PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="n3-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>递延所得税负债为<strong>负债类贷方科目</strong>（2901）：期末 = 期初 + 贷方（确认）− 借方（转回），取期末余额</li>
          <li>核心公式：递延所得税负债 = 应纳税暂时性差异 × 适用税率</li>
          <li>应纳税暂时性差异产生：资产账面价值 &gt; 计税基础，或负债账面价值 &lt; 计税基础</li>
          <li>不确认的特殊项：商誉初始确认、拟长期持有长投相关差异一般不确认递延所得税负债</li>
          <li>与 N1 递延税资产同源对应（同一纳税主体可抵销）；审定期末回写 TB(2901)，本期变动额供 N5 递延所得税费用核对；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="n3-cycle">
...
/**
 * N3TabIndex.vue — N3 递延所得税负债底稿目录（严格镜像 N1 标准）
 *
 ...
 *
 * 负债类贷方科目（2901 递延所得税负债）— 取期末余额，与 N1 同源分列。
 * 注：N3 无独立附注 sheet（classification 未含），故目录不列附注。
 */
...
const N3PreparationHandbookDialog = defineAsyncComponent(() => import('../N3PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { name: '递延所得税负债审计程序表', code: 'N3A', sheetKey: '递延所得税负债审计程序表的N3A', progress: calcSheetProgress('N3-N3A-', 5) },
  { name: '审定表', code: 'N3-1', sheetKey: '递延所得税负债审定表N3-1', progress: calcSheetProgress('N3-1-', 8) },
  { name: '明细表', code: 'N3-2', sheetKey: '递延所得税负债明细表N3-2', progress: calcSheetProgress('N3-2-', 6) },
  { name: '调整分录汇总', code: 'N3-3', sheetKey: '调整分录汇总表N3-3', progress: calcSheetProgress('N3-3-', 4) },
])
...
// ─── 跨表结论口径看板 ─────────────────────────────────────────────────────────
/** 有审计结论/说明的 sheet（程序表、调整分录不计入；审定/明细计入） */
const N3_CONCLUSION_CODES = new Set(['N3-1', 'N3-2'])
...
function isConclusionFilled(code: string): boolean {
  ...
  const token = `${code}-`
  const longer = Array.from(N3_CONCLUSION_CODES).filter(c => c !== code && c.startsWith(token))
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => N3_CONCLUSION_CODES.has(s.code))
