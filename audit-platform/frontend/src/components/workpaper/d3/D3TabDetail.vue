  ...
  <div class="detail-toolbar">
    ...
    <div class="toolbar-actions">
      <el-button-group size="small">
        <el-button @click="exportTemplate('D3-2')">导出模板</el-button>
        <el-button @click="exportData('D3-2')">导出数据</el-button>
        <el-upload
          :show-file-list="false"
          accept=".xlsx"
          :before-upload="(file: any) => handleImport(file, 'D3-2')"
        >
...
async function exportTemplate(sheet: string) {
  try {
    const res = await http.post(
      `/api/workpapers/${props.wpId.value}/d3/export-template`,
      null,
...
async function exportData(sheet: string) {
  try {
    const res = await http.post(
      `/api/workpapers/${props.wpId.value}/d3/export-data`,
      null,
      { params: { sheet }, responseType: 'blob' } as any,
    )
    const url = window.URL.createObjectURL(new Blob([res.data]))
...
async function handleImport(file: File, sheet: string): Promise<boolean> {
  ...
  try {
    const res = await http.post(
      `/api/workpapers/${props.wpId.value}/d3/import-data`,
      formData,
      { params: { sheet }, headers: { 'Content-Type': 'multipart/form-data' } } as any,
    )
    const data = res.data?.data ?? res.data
    if (data.ok) {
      ElMessage.success(`成功导入 ${data.imported_count} 行数据${data.warning ? '（' + data.warning + '）' : ''}`)
    } else {
      ElMessage.error(`导入失败：${(data.errors || []).join('；')}`)
    }
