...
/**
 * D6TabIndex — 统一底稿目录（比照 D5TabIndex）
 */
...
const indexRows = computed(() =>
  D6_INDEX_ROWS.map(row => ({
    ...row,
    sheetLabel: resolveD6SheetLabel(row.code, props.availableSheets),
  })),
)
...
<template>
  <div class="d6-tab-index">
    <div class="index-header">
      <div class="title-row">
        <h3>D6 底稿目录</h3>
        <GtReviewTrigger section-id="D6-index-directory" />
    ...
    <D6PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />
    ...
    <details class="methodology-hint">
      ...
      <p>推荐工作流：D6A 程序表 → D6-2 明细（可从四表库取数）→ D6-6 检查 → D6-7 政策 → D6-8 减值测算 → D6-3 减值明细 → D6-4 调整分录 → 确认回写 D6-1。D6-8 ECL 测算与 D6-3 减值准备勾稽（差异>1 元告警）。</p>
    </details>
