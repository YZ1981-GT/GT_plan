  ...
  <div class="k4-tab-index">
    ...
    <div class="k4-dir">
      <div class="index-header">
        <h3 class="title">K4 底稿目录</h3>
        <GtReviewTrigger section-id="K4-index-directory" />
      ...
      <K4PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="k4-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          ...
          <li>关注流动性划分：结合合同到期日核对是否应重分类至非流动负债</li>
          <li>科目常含：短期应付债券、待转销项税额、预提费用、政府补助流动部分、代扣代缴</li>
          <li>K4-2 明细合计应与 K4-1 审定表一致；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="k4-cycle">
...
/**
 * K4TabIndex.vue — K4 其他流动负债底稿目录（严格镜像 E1 标准）
 *
...
const K4PreparationHandbookDialog = defineAsyncComponent(() => import('../K4PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { seq: 1, name: '其他流动负债实质性程序表', code: 'K4A', sheetKey: '其他流动负债实质性程序表K4A', group: 'core', progress: calcSheetProgress('K4-K4A-', 5) },
  { seq: 2, name: '审定表', code: 'K4-1', sheetKey: '审定表K4-1', group: 'core', progress: calcSheetProgress('K4-1-', 10) },
  { seq: 3, name: '明细表', code: 'K4-2', sheetKey: '明细表K4-2', group: 'core', progress: calcSheetProgress('K4-2-', 6) },
  { seq: 4, name: '调整分录汇总', code: 'K4-3', sheetKey: '调整分录汇总K4-3', group: 'inspection', progress: calcSheetProgress('K4-3-', 4) },
  { seq: 5, name: '其他流动负债检查表', code: 'K4-4', sheetKey: '其他流动负债检查表K4-4', group: 'inspection', progress: calcSheetProgress('K4-4-', 4) },
  { seq: 6, name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', group: 'disclosure', progress: calcSheetProgress('K4-disclosure-listed-', 5) },
  { seq: 7, name: '附注披露信息（国企）', code: '附注国企', sheetKey: '附注披露信息（国企）', group: 'disclosure', progress: calcSheetProgress('K4-disclosure-soe-', 5) },
])
...
// ─── 跨表结论口径看板 ─────────────────────────────────────────────────────────
/** 有审计结论/说明的 sheet（K4-3 调整分录、附注、目录本身、K4A 程序表不计入） */
const K4_CONCLUSION_CODES = new Set(['K4-1', 'K4-2', 'K4-4'])
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => K4_CONCLUSION_CODES.has(s.code))
