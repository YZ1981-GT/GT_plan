...
/**
 * L2 编制/使用手册弹窗 — 对齐 L1PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：L2-2 明细(按借款/债券来源逐笔) → L2-1 审定(按来源：分期付息长借/企业债券/短借/优先股永续债/其他，未审+账项调整+重分类，双期) → 回写 TB(2231)+附注。应付利息是负债类贷方：期末=期初+贷方(计提)−借方(支付)。本期计提与 L1-5/L3 利息测算联动核对，联动 L8 财务费用。本弹窗不改各表页内说明。"
    />
