...
import { G14_INDEX_ROWS, resolveG14SheetLabel, isG14SheetComplete } from '../composables/g14SheetLabels'
...
const indexRows = computed(() =>
  G14_INDEX_ROWS.map(row => ({
    ...row,
    sheetLabel: resolveG14SheetLabel(row.code, props.availableSheets),
  })),
)
...
const completedCount = computed(() =>
  applicableRows.value.filter(r => isG14SheetComplete(r.code, props.allResponses)).length,
)
...
<template>
  <div class="g14-dir" data-testid="g14-directory">
    <div class="index-header">
      <h3 class="title">G14 底稿目录</h3>
      <GtReviewTrigger section-id="G14-index-directory" />
    ...
    <el-table :data="indexRows" border size="small" style="font-size:13px">
      ...
      <el-table-column label="底稿名称" min-width="220">
        <template #default="{ row }">
          <span>{{ row.name }}</span>
          <span class="index-chip-wrap" :data-testid="`g14-dir-chip-${row.code}`">
            <GtIndexChip
          ...
          <el-tag
            v-if="row.applicable && isG14SheetComplete(row.code, allResponses)"
            type="success"
            size="small"
            class="done-tag"
          >已编制</el-tag>
          <GtReviewDot v-if="row.applicable" :section-id="`G14-index-${row.code}`" class="index-review-dot" />
        </template>
      </el-table-column>
    </el-table>
...
<style scoped>
.g14-dir { padding: 12px; font-size: var(--wp-font-size, 13px); }
