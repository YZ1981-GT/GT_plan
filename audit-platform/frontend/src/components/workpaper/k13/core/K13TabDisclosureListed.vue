...
function handleNarrativeSave(): void {
  ...
  eventBus.emit('disclosure:note-text-updated' as any, {
    ...
    sectionIds: [K13_NOTE_SECTION.listed],
...
async function syncToDisclosureNotes(): Promise<void> {
  ...
  try {
    ...
    eventBus.emit('disclosure:note-text-updated' as any, {
      ...
      projectId: props.projectId, sectionIds: [K13_NOTE_SECTION.listed],
    })
...
async function generateAI(section: string): Promise<void> {
  ...
  const content = await generateK13AiText(props.wpId, {
    section,
    prompt: '为 K13 营业外支出底稿生成上市公司附注披露文本，需包含各去向分类的本期/上期发生额、重大变动说明及计入当期非经常性损益金额的认定说明。口径以致同 2025 修订版底稿源模板与附注模版为准；只能使用已提供的项目名称与金额，不得虚构项目、金额或业务背景，无把握的内容留空由审计师补充。',
    context: {
      科目: '6711 营业外支出（上市公司版）',
