<template>
  <div class="k13-tab-index">
    <!-- ═══ 总体进度 ═══ -->
    <div class="k13-progress-section">
    ...
    <div class="k13-guide">
      ...
      <div class="k13-guide-steps">
        <div class="step-item">
          ...
          <span class="step-text">填写明细表（K13-2）逐笔登记营业外支出明细</span>
        </div>
        <div class="step-item">
          <span class="step-num">②</span>
          <span class="step-text">填写审定表（K13-1）确认发生额 → TB回写6711</span>
        </div>
        <div class="step-item">
          <span class="step-num">③</span>
          <span class="step-text">录入调整分录（K13-3）管理AJE/RJE → 联动A13</span>
        </div>
        <div class="step-item">
          <span class="step-num">④</span>
          <span class="step-text">完成检查表（K13-4）核实分类正确性与税前扣除性</span>
        </div>
      </div>
    </div>

    <!-- ═══ 跨底稿引用 ═══ -->
    <div class="k13-cross-refs">
      <span class="cross-refs-label">关联底稿：</span>
      <GtIndexChip value="K13A" :context-project-id="props.projectId" />
      <GtIndexChip value="K12" :context-project-id="props.projectId" />
      <GtIndexChip value="A13" :context-project-id="props.projectId" />
    </div>

    <!-- ═══ 底稿目录表 ═══ -->
    <el-card shadow="never" class="k13-group-card">
      <template #header>
...
/**
 * K13TabIndex.vue — K13 营业外支出底稿目录
 *
 * Spec: .kiro/specs/k13-non-operating-expense/ | Task: 4.1
 * Requirements: 1.2
 *
 * 功能：
...
const allSheets = computed<SheetRow[]>(() => [
  { seq: 1, name: '实质性程序表 K13A', code: 'K13A', sheetKey: '实质性程序表 K13A', progress: calcSheetProgress('K13-K13A-', 5) },
  { seq: 2, name: '审定表 K13-1', code: 'K13-1', sheetKey: '审定表K13-1', progress: calcSheetProgress('K13-1-', 10) },
  { seq: 3, name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', progress: calcSheetProgress('K13-disclosure-listed-', 5) },
  { seq: 4, name: '附注披露信息（国企）', code: '附注国企', sheetKey: '附注披露信息（国企）', progress: calcSheetProgress('K13-disclosure-soe-', 5) },
  { seq: 5, name: '明细表 K13-2', code: 'K13-2', sheetKey: '明细表K13-2', progress: calcSheetProgress('K13-2-', 8) },
  { seq: 6, name: '调整分录汇总 K13-3', code: 'K13-3', sheetKey: '调整分录汇总K13-3', progress: calcSheetProgress('K13-3-', 4) },
  { seq: 7, name: '营业外支出检查表 K13-4', code: 'K13-4', sheetKey: '营业外支出检查表K13-4', progress: calcSheetProgress('K13-4-', 6) },
])
...
<style scoped>
.k13-tab-index { padding: 12px; font-size: 13px; }

.k13-progress-section {
  margin-bottom: 16px;
  padding: 12px 16px;
  background: #fafafa;
  border-radius: 8px;
...
.k13-group-card { margin-bottom: 16px; }
.k13-group-card :deep(.el-card__header) {
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 10px 16px;
