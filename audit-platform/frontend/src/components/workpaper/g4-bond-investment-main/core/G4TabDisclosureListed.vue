    ...
    <div class="section-head">
      ...
      <div class="head-actions">
        ...
        <el-button
          ...
          type="primary"
          plain
          data-testid="g4-disclosure-listed-sync-notes"
          :loading="isSyncing"
...
// ─── 同步到附注（§五、14 债权投资）──────────────────────────────────────────
// 🔴 只推送底稿字段真实存在的表：主表（7 列两级）+ 六张三阶段表（6 列单级）。
//    其余 6 张模板表在底稿里复用了主表的 6 列行模型、无对应字段 → 见
//    `g4NoteSectionMap.G4_NOT_SYNCED_TABLES`（宁缺勿造，不推错位数字）。
...
async function syncToDisclosureNotes(): Promise<void> {
  ...
  const payload = buildG4SyncPayload(
    ...
    props.wpId, 'listed', props.applicableStandards, buildSyncSnapshot(),
