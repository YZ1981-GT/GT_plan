<template>
  <div class="h6-tab-disclosure-soe">
    ...
    <el-alert
      ...
      title="审计目标：核实国有企业固定资产清理相关附注披露完整、准确，处置审批与资产评估合规、公开处置及关联方处置充分披露，清理损益勾稽一致且过渡科目 1606 期末余额清零。"
    />
    ...
    <div class="section-header">
      ...
      <div class="section-header-actions">
        ...
        <el-button size="small" circle @click="openReview('H6-disclosure-soe')" style="margin-left: 8px">💬</el-button>
      </div>
    </div>
    ...
    <el-card shadow="never" class="audit-note-card">
      <template #header>
        <div class="section-header" style="margin-bottom:0">
          <span>审计说明</span>
          <div class="section-header-actions">
            <el-button size="small" circle @click="openReview('H6-disclosure-soe-note')">💬</el-button>
          </div>
        </div>
      </template>
    ...
    <el-card shadow="never" class="audit-note-card">
      <template #header>
        <div class="section-header" style="margin-bottom:0">
          <span>审计结论</span>
          <div class="section-header-actions">
            <el-button size="small" circle @click="openReview('H6-disclosure-soe-conclusion')">💬</el-button>
          </div>
        </div>
      </template>
...
// ─── 审计说明 / 审计结论 ─────────────────────────────────────────────────────
const auditNote = ref('')
const auditNoteResp = props.allResponses.get('H6-disclosure-soe-note')
if (auditNoteResp?.remark) auditNote.value = auditNoteResp.remark
function saveAuditNote() {
  saveResponse('H6-disclosure-soe-note', auditNote.value)
}
const auditConclusion = ref('')
const auditConcResp = props.allResponses.get('H6-disclosure-soe-conclusion')
if (auditConcResp?.remark) auditConclusion.value = auditConcResp.remark
function saveAuditConclusion() {
  saveResponse('H6-disclosure-soe-conclusion', auditConclusion.value)
}
...
<style scoped>
.h6-tab-disclosure-soe { padding: 16px; font-size: var(--wp-font-size, 13px); }
