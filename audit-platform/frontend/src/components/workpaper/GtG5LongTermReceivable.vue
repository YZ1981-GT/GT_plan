...
const formData = useG5LonRecFormData({ wpId: wpIdRef, projectId: computed(() => props.projectId) })
...
const useGridFallback = computed(() => {
  ...
  return !!code && code !== 'G5A' && !adjudicationConfig.value && !code.startsWith('附注')
})
...
.g5-long-term-receivable-toolbar { margin-bottom: 8px; }
</style>
