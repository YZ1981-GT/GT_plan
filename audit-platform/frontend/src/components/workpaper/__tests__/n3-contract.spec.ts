 ...
 * N3 递延所得税负债 — 注册契约测试
 *
 * 验证 'n3-deferred-tax-liabilities' componentType 在四个注册表中正确注册：
 * 1. htmlRendererRegistry（前端组件映射）
 * 2. VALID_COMPONENT_TYPES（后端 wp_classification_service.py）
 * 3. wp_code_overrides（N3/N3-1~N3-3/N3A 共5个映射）
...
const COMPONENT_TYPE = 'n3-deferred-tax-liabilities'

// N3/N3-1~N3-3/N3A 共5个wp_code应映射到n3-deferred-tax-liabilities
...
describe('N3 递延所得税负债 — 注册契约测试', () => {
  ...
  describe('htmlRendererRegistry 注册', () => {
    ...
    it('isHtmlComponentType 识别 n3-deferred-tax-liabilities', () => {
  ...
  describe('VALID_COMPONENT_TYPES 契约', () => {
    it('后端 wp_classification_service.py 包含 n3-deferred-tax-liabilities', () => {
      const servicePath = path.resolve(
        __dirname,
        '../../../../../../backend/app/services/wp_classification_service.py'
      )
      const content = fs.readFileSync(servicePath, 'utf-8')
      expect(content).toContain('"n3-deferred-tax-liabilities"')
    })
  })
  ...
  describe('wp_code_overrides 契约', () => {
    ...
    it.each(EXPECTED_WP_CODES)('%s 映射为 n3-deferred-tax-liabilities', (wpCode) => {
      expect(overrides[wpCode]).toBe(COMPONENT_TYPE)
    })

    it('覆盖完整性：共5个映射无遗漏', () => {
      const n3Mappings = Object.entries(overrides).filter(
        ([, v]) => v === COMPONENT_TYPE
      )
      expect(n3Mappings.length).toBe(EXPECTED_WP_CODES.length)
    })
  })
  ...
  describe('RENDERER_DISPATCH 契约', () => {
    it('后端 wp_render_strategies/__init__.py 包含 n3-deferred-tax-liabilities', () => {
