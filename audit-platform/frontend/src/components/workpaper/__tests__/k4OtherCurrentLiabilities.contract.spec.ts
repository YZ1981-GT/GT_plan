 ...
 * K4 其他流动负债 — 注册契约测试
 *
 * Spec: .kiro/specs/k4-other-current-liabilities/ Task 1.2
 * Validates: Requirements 1.6, 1.7, 1.8
 *
 * 验证 'k4-other-current-liabilities' componentType 在三个注册表中正确注册：
 * 1. htmlRendererRegistry（前端组件映射）
 * 2. VALID_COMPONENT_TYPES（后端 wp_classification_service.py）
 * 3. wp_code_overrides（K4/K4-1~K4-4/K4A 共6个映射）
...
describe('K4 其他流动负债 — 注册契约测试', () => {
  describe('htmlRendererRegistry 注册', () => {
    it('k4-other-current-liabilities 已注册且配置正确', () => {
    ...
    it('isHtmlComponentType 识别 k4-other-current-liabilities', () => {
  ...
  describe('VALID_COMPONENT_TYPES 契约', () => {
    it('后端 wp_classification_service.py 包含 k4-other-current-liabilities', () => {
      const servicePath = path.resolve(
        __dirname,
        '../../../../../../backend/app/services/wp_classification_service.py'
      )
      const content = fs.readFileSync(servicePath, 'utf-8')
      expect(content).toContain('"k4-other-current-liabilities"')
    })
  })
  ...
  describe('wp_code_overrides 契约', () => {
    ...
    for (const code of K4_WP_CODES) {
      it(`${code} 映射为 k4-other-current-liabilities`, () => {
        expect(overrides[code]).toBe(COMPONENT_TYPE)
      })
    }

    it('k4-other-current-liabilities 共有6个wp_code映射', () => {
      const actualCount = Object.entries(overrides)
        .filter(([, v]) => v === COMPONENT_TYPE)
        .length
      expect(actualCount).toBe(K4_WP_CODES.length)
    })
  })

  describe('RENDERER_DISPATCH 契约（Phase 5）', () => {
    it('后端 wp_render_strategies 包存在（Task 5.1 创建后完成注册）', () => {
      // RENDERER_DISPATCH 将在 Task 5.1 注册 k4-other-current-liabilities
      // 此测试通过读取后端目录验证模块存在
