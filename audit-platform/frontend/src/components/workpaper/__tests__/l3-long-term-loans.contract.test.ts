 ...
 * L3 长期借款 — 注册契约测试
 *
 * 验证 'l3-long-term-loans' componentType 在四个注册表中正确注册：
 * 1. htmlRendererRegistry（前端组件映射）
 * 2. VALID_COMPONENT_TYPES（后端 wp_classification_service.py）
 * 3. wp_code_overrides（L3/L3-1~L3-9/L3A 共11个映射）
...
const COMPONENT_TYPE = 'l3-long-term-loans'

// L3/L3-1~L3-9/L3A 共11个wp_code应映射到l3-long-term-loans
...
describe('L3 长期借款 — 注册契约测试', () => {
  ...
  describe('htmlRendererRegistry 注册', () => {
    it('l3-long-term-loans 已注册且配置正确', () => {
      ...
      expect(entry?.label).toBe('L3 长期借款')
    ...
    it('isHtmlComponentType 识别 l3-long-term-loans', () => {
  ...
  describe('VALID_COMPONENT_TYPES 契约', () => {
    it('后端 wp_classification_service.py 包含 l3-long-term-loans', () => {
      const servicePath = path.resolve(
        __dirname,
        '../../../../../../backend/app/services/wp_classification_service.py'
      )
      const content = fs.readFileSync(servicePath, 'utf-8')
      expect(content).toContain('"l3-long-term-loans"')
    })
  })
  ...
  describe('wp_code_overrides 契约', () => {
    ...
    it.each(EXPECTED_WP_CODES)('%s 映射为 l3-long-term-loans', (wpCode) => {
      expect(overrides[wpCode]).toBe(COMPONENT_TYPE)
    })

    it('覆盖完整性：共11个映射无遗漏', () => {
      const l3Mappings = Object.entries(overrides).filter(
        ([, v]) => v === COMPONENT_TYPE
      )
      expect(l3Mappings.length).toBe(EXPECTED_WP_CODES.length)
    })
  })
  ...
  describe('RENDERER_DISPATCH 契约', () => {
    it('后端 wp_render_strategies/__init__.py 包含 l3-long-term-loans', () => {
