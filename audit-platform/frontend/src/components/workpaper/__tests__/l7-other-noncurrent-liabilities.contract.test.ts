 ...
 * L7 其他非流动负债 — 注册契约测试
 *
 * 验证 'l7-other-noncurrent-liabilities' componentType 在四个注册表中正确注册：
 * 1. htmlRendererRegistry（前端组件映射）
 * 2. VALID_COMPONENT_TYPES（后端 wp_classification_service.py）
 * 3. wp_code_overrides（L7/L7-1~L7-4/L7A 共6个映射）
...
const COMPONENT_TYPE = 'l7-other-noncurrent-liabilities'

// L7/L7-1~L7-4/L7A 共6个wp_code应映射到l7-other-noncurrent-liabilities
...
describe('L7 其他非流动负债 — 注册契约测试', () => {
  ...
  describe('htmlRendererRegistry 注册', () => {
    ...
    it('isHtmlComponentType 识别 l7-other-noncurrent-liabilities', () => {
  ...
  describe('VALID_COMPONENT_TYPES 契约', () => {
    it('后端 wp_classification_service.py 包含 l7-other-noncurrent-liabilities', () => {
      const servicePath = path.resolve(
        __dirname,
        '../../../../../../backend/app/services/wp_classification_service.py'
      )
      const content = fs.readFileSync(servicePath, 'utf-8')
      expect(content).toContain('"l7-other-noncurrent-liabilities"')
    })
  })
  ...
  describe('wp_code_overrides 契约', () => {
    ...
    it.each(EXPECTED_WP_CODES)('%s 映射为 l7-other-noncurrent-liabilities', (wpCode) => {
      expect(overrides[wpCode]).toBe(COMPONENT_TYPE)
    })

    it('覆盖完整性：共6个映射无遗漏', () => {
      const l7Mappings = Object.entries(overrides).filter(
        ([, v]) => v === COMPONENT_TYPE
      )
      expect(l7Mappings.length).toBe(EXPECTED_WP_CODES.length)
    })
  })
  ...
  describe('RENDERER_DISPATCH 契约', () => {
    it('后端 wp_render_strategies/__init__.py 包含 l7-other-noncurrent-liabilities', () => {
