...
describe('M3 库存股 — 注册契约测试', () => {
  ...
  describe('htmlRendererRegistry 注册', () => {
    ...
    it('isHtmlComponentType 识别 m3-treasury-stock', () => {
  ...
  describe('VALID_COMPONENT_TYPES 契约', () => {
    it('后端 wp_classification_service.py 包含 m3-treasury-stock', () => {
      const servicePath = path.resolve(
        __dirname,
        '../../../../../../backend/app/services/wp_classification_service.py'
      )
      const content = fs.readFileSync(servicePath, 'utf-8')
      expect(content).toContain('"m3-treasury-stock"')
    })
  })
  ...
  describe('wp_code_overrides 契约', () => {
    ...
    it.each(EXPECTED_WP_CODES)('%s 映射为 m3-treasury-stock', (wpCode) => {
    ...
    it('覆盖完整性：共7个映射无遗漏', () => {
      ...
      expect(m3Mappings.length).toBe(EXPECTED_WP_CODES.length)
    })
  })
  ...
  describe('RENDERER_DISPATCH 契约', () => {
    it('后端 wp_render_strategies/__init__.py 包含 m3-treasury-stock', () => {
