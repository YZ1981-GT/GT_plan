 ...
 *
 * 判据：D4TabCutoffForward.vue 消费平台 useWorkpaperSyncBridge + WorkpaperSyncEditorHost +
 * capabilityForEntry(父级 entry) 现算 + readStoreProjection；禁裸 GtOnlyOfficeSheet；
 * flushHtml 内 flushPendingSave 先于 readStoreProjection；sheetKey=d417-managed；
...
describe('D4-17 截止测试必须消费平台 sync bridge（批次B 从零）', () => {
  ...
  it('sheetKey 锁 d417-managed', () => { expect(src).toContain('d417-managed') })
