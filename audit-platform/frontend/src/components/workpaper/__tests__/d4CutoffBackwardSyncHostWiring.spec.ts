/**
 * D4-18 截止测试（单据到账）宿主接线守卫（批次B 第三张）—— 参照 d4CutoffForwardSyncHostWiring。
 *
 * 2026-09-21 治本改造后更新：D4-18 的接桥不再直接调 `useWorkpaperSyncBridge`，而是通过
...
const TAB = resolve(_dir, '..', 'd4', 'inspection', 'D4TabCutoffBackward.vue')
...
describe('D4-18 截止测试必须消费平台 sync bridge（批次B 从零）', () => {
  ...
  it('sheetKey 走具名常量（不得内联字面量，防漂移）', () => {
    expect(src).toContain(`= 'd418-managed'`)
    expect(args).toMatch(/sheetKey:\s*D4_18_SHEET_KEY\b/)
    expect(args).not.toMatch(/sheetKey:\s*['"]d418-managed['"]/)
  })
...
describe('守卫自检（防恒真）', () => {
  it('extractBridgeCallArgs 抓到真实调用体', () => {
    const stub = "const b = useD4SyncMode({ sheetKey: D4_18_SHEET_KEY, flushHtml: async () => {} })"
    expect(extractBridgeCallArgs(stub)).toContain('flushHtml')
    expect(extractBridgeCallArgs('no bridge here')).toBe('')
  })
})

describe('宿主 GtD4OperatingRevenue 必须登记 D4-18 为 dedicated', () => {
  ...
  it('dedicated 列表含 D4-18', () => { expect(extractDedicatedList(hostSrc)).toContain("'D4-18'") })
})
