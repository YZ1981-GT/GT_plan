 ...
 *
 * 2026-09-21 治本改造后更新：D4-19 的接桥不再直接调 `useWorkpaperSyncBridge`，而是通过
...
describe('D4-19 折扣折让必须消费平台 sync bridge（批次B 从零）', () => {
  ...
  it('sheetKey 走具名常量（不得内联字面量，防漂移）', () => {
    expect(src).toContain(`= 'd419-managed'`)
    expect(args).toMatch(/sheetKey:\s*D4_19_SHEET_KEY\b/)
    expect(args).not.toMatch(/sheetKey:\s*['"]d419-managed['"]/)
  })
...
describe('守卫自检（防恒真）', () => {
  it('extractBridgeCallArgs 抓到真实调用体', () => {
    const stub = "const b = useD4SyncMode({ sheetKey: D4_19_SHEET_KEY, flushHtml: async () => {} })"
