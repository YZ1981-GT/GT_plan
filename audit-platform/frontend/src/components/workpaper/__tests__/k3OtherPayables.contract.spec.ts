 ...
 * K3 其他应付款 — 注册契约测试
 *
 * Spec: .kiro/specs/k3-other-payables/ Task 1.2
 * Validates: Requirements 1.6, 1.7, 1.8
 *
 * 验证 'k3-other-payables' componentType 在三个注册表中正确注册：
 * 1. htmlRendererRegistry（前端组件映射）
 * 2. VALID_COMPONENT_TYPES（后端 wp_classification_service.py）
 * 3. wp_code_overrides（K3/K3-1~K3-7/K3A 共9个映射）
...
describe('K3 其他应付款 — 注册契约测试', () => {
  describe('htmlRendererRegistry 注册', () => {
    it('k3-other-payables 已注册且配置正确', () => {
    ...
    it('isHtmlComponentType 识别 k3-other-payables', () => {
  ...
  describe('VALID_COMPONENT_TYPES 契约', () => {
    it('后端 wp_classification_service.py 包含 k3-other-payables', () => {
      const servicePath = path.resolve(
        __dirname,
        '../../../../../../backend/app/services/wp_classification_service.py'
      )
      const content = fs.readFileSync(servicePath, 'utf-8')
      expect(content).toContain('"k3-other-payables"')
    })
  })
