 ...
 *
 * 2026-09-21 治本改造后更新：D4-33 的接桥不再直接调 `useWorkpaperSyncBridge`，而是通过
...
describe('D4-33 毛利率分析必须消费平台 sync bridge（静态受管区 dict store）', () => {
  ...
  it('sheetKey 锁 d433-managed 身份', () => {
    expect(args).toMatch(/sheetKey:\s*['"]d433-managed['"]/)
  })
...
describe('守卫自检（防恒真）', () => {
  it('extractBridgeCallArgs 抓到真实调用体', () => {
    const stub = "const b = useD4SyncMode({ sheetKey: 'd433-managed', flushHtml: async () => {} })"
