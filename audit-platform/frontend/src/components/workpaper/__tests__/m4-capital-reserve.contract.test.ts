...
describe('M4 资本公积 — 注册契约测试', () => {
  ...
  describe('htmlRendererRegistry 注册', () => {
    ...
    it('isHtmlComponentType 识别 m4-capital-reserve', () => {
  ...
  describe('VALID_COMPONENT_TYPES 契约', () => {
    it('后端 wp_classification_service.py 包含 m4-capital-reserve', () => {
      const servicePath = path.resolve(
        __dirname,
        '../../../../../../backend/app/services/wp_classification_service.py'
      )
      const content = fs.readFileSync(servicePath, 'utf-8')
      expect(content).toContain('"m4-capital-reserve"')
    })
  })
  ...
  describe('wp_code_overrides 契约', () => {
    ...
    it.each(EXPECTED_WP_CODES)('%s 映射为 m4-capital-reserve', (wpCode) => {
    ...
    it('覆盖完整性：共8个映射无遗漏', () => {
      ...
      expect(m4Mappings.length).toBe(EXPECTED_WP_CODES.length)
    })
  })
  ...
  describe('RENDERER_DISPATCH 契约', () => {
    it('后端 wp_render_strategies/__init__.py 包含 m4-capital-reserve', () => {
