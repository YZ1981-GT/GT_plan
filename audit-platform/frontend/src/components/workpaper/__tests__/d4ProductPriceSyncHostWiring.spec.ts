/**
 * D4-11 产品销售价格分析宿主接线守卫（批次B 第五张）—— 参照 d4IndicatorSyncHostWiring。
 * 额外校验：PriceRow 补了 rowId 稳定身份（禁数组下标作身份）。
 *
 * 2026-09-21 治本改造后更新：D4-11 的接桥不再直接调 `useWorkpaperSyncBridge`，而是通过
...
const TAB = resolve(_dir, '..', 'd4', 'analysis', 'D4TabProductPrice.vue')
...
describe('D4-11 产品价格分析必须消费平台 sync bridge（批次B 从零）', () => {
  ...
  it('sheetKey 走具名常量（不得内联字面量，防漂移）', () => {
    expect(src).toContain(`= 'd411-managed'`)
    expect(args).toMatch(/sheetKey:\s*D4_11_SHEET_KEY\b/)
    expect(args).not.toMatch(/sheetKey:\s*['"]d411-managed['"]/)
  })
  ...
  it('PriceRow 补了 rowId 稳定身份（禁数组下标作身份）+ backfill', () => {
...
describe('守卫自检（防恒真）', () => {
  it('extractBridgeCallArgs 抓到真实调用体', () => {
    const stub = "const b = useD4SyncMode({ sheetKey: D4_11_SHEET_KEY, flushHtml: async () => {} })"
...
describe('宿主必须登记 D4-11 为 dedicated', () => {
  ...
  it('dedicated 列表含 D4-11', () => { expect(extractDedicatedList(hostSrc)).toContain("'D4-11'") })
})
