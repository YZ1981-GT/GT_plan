...
import F2DetailSheetDevCost from '../f2/detail/F2DetailSheetDevCost.vue'

describe('F2DetailSheetDevCost mount', () => {
  it('renders F2-11 three-section skeleton without crash', async () => {
    const wrapper = mount(F2DetailSheetDevCost, {
      props: {
        wpId: 'wp-1',
        projectId: 'p1',
    ...
    await flushPromises()
    await nextTick()
    expect(wrapper.text()).toContain('F2-11')
    expect(wrapper.text()).toContain('开发成本')
    expect(wrapper.text()).toMatch(/原值|跌价|净值/)
    expect(wrapper.text()).toContain('资本化利息')
  })
})
