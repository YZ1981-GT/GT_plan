/**
 * D4-34 其他业务收入合同测算表宿主接线守卫（批次B 第九张，同 sheet 双动态区 dict store）
 * —— 参照 d4ReturnSyncHostWiring。
 *
 * 2026-09-21 治本改造后更新：D4-34 的接桥不再直接调 `useWorkpaperSyncBridge`，而是通过
...
const TAB = resolve(_dir, '..', 'd4', 'other', 'D4TabOtherContract.vue')
...
describe('D4-34 合同测算必须消费平台 sync bridge（双区 dict store）', () => {
  ...
  it('sheetKey 锁 d434-managed 身份', () => {
    expect(args).toMatch(/sheetKey:\s*['"]d434-managed['"]/)
  })
...
describe('守卫自检（防恒真）', () => {
  it('extractBridgeCallArgs 抓到真实调用体', () => {
    const stub = "const b = useD4SyncMode({ sheetKey: 'd434-managed', flushHtml: async () => {} })"
...
describe('宿主必须登记 D4-34 为 dedicated', () => {
  ...
  it('dedicated 列表含 D4-34', () => { expect(extractDedicatedList(hostSrc)).toContain("'D4-34'") })
})
