<template>
  <div class="h7-tab-impairment">
    ...
    <div class="tab-toolbar">
      <span class="chip-wrap"><GtIndexChip value="wp:H7-15" :context-project-id="projectId" /></span>
    ...
    <el-card shadow="never" class="block-card">
      ...
      <el-table :data="rows" border stripe size="small" class="check-table" max-height="500">
        ...
        <el-table-column prop="bookValue" label="账面价值" min-width="110" align="right">
          <template #default="{ row }">
            <WpAmountInput v-if="!isReadonly" v-model="row.bookValue" size="small" @change="persistRows" />
        ...
        <el-table-column prop="recoverableAmount" label="可收回金额" min-width="110" align="right">
          <template #default="{ row }">
            <WpAmountInput v-if="!isReadonly" v-model="row.recoverableAmount" size="small" @change="persistRows" />
        ...
        <el-table-column prop="priorImpairment" label="已确认减值" min-width="110" align="right">
          <template #default="{ row }">
            <WpAmountInput v-if="!isReadonly" v-model="row.priorImpairment" size="small" @change="persistRows" />
    ...
    <el-card shadow="never" class="note-card">
      ...
      <el-input v-model="auditConclusion" type="textarea" :autosize="{ minRows: 3 }" :disabled="isReadonly" placeholder="A、未见异常。B、除上述调整事项外，其余未见异常。C、存在重大未调整事项或范围受限，不可确认。" @blur="persist('H7-15-conclusion', auditConclusion)" />
    </el-card>
...
function seed(): void {
  ...
  auditConclusion.value = impair.getString('H7-15-conclusion')
}
onMounted(seed)
...
<style scoped>
.h7-tab-impairment { padding: 16px; font-size: var(--wp-font-size, 13px); }
