...
async function handleAi(): Promise<void> {
  ...
  const ctx = `生产性生物资产本期减少 ${rows.value.length} 项，减少原值合计 ${fmtAmt(totalCost.value)}，处置净损益 ${fmtAmt(totalGainLoss.value)}。`
  const text = await generateAiText('h7-disposal', ctx, auditNote.value)
  if (text) { auditNote.value = text; persist('H7-7-cost-note', auditNote.value) }
}
