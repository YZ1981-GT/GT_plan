...
async function handleAi(): Promise<void> {
  ...
  const ctx = `生产性生物资产本期增加 ${rows.value.length} 项，合计 ${fmtAmt(totalAmount.value)}，其中符合确认条件 ${qualifiedCount.value} 项。`
  const text = await generateAiText('h7-addition', ctx, auditNote.value)
  if (text) { auditNote.value = text; persist('H7-6-cost-note', auditNote.value) }
}
...
<style scoped>
.h7-tab-addition-cost { padding: 16px; font-size: 13px; }
