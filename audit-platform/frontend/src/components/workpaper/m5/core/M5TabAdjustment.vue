<template>
  <div class="m5-tab-adjustment">
    <!-- ═══ 标题 + AJE/RJE切换 + 按钮 ═══ -->
    <div class="section-header">
      <div class="section-header-left">
        <el-button text size="small" @click="$emit('navigate', '底稿目录')">← 返回目录</el-button>
        <h3 class="section-title">M5-3 盈余公积调整分录汇总</h3>
    ...
    <div class="methodology-context">
      <div class="methodology-text">
        <strong>盈余公积（4101）调整分录方向说明：</strong>
        盈余公积为权益类贷方科目 —
        <strong>贷方增加 = 调增盈余公积</strong>（如补提法定盈余公积、补提任意盈余公积），
        <strong>借方减少 = 调减盈余公积</strong>（如转增资本、弥补亏损）。
        借贷必须平衡后方可保存发布。保存后自动通知M5-1审定表刷新AJE/RJE列。
      </div>
    </div>
    ...
    <div class="net-impact-section">
      <el-tag type="info" size="small" effect="plain">
        AJE对4101净影响: {{ fmtAmount(adjustment.ajeNet4101.value) }}（正=调增，负=调减）
      </el-tag>
      <el-tag type="info" size="small" effect="plain">
        RJE对4101净影响: {{ fmtAmount(adjustment.rjeNet4101.value) }}（正=调增，负=调减）
      </el-tag>
    </div>
    ...
    <details class="m5-details-tip">
      ...
      <ul>
        <li>科目4101盈余公积：<strong>贷方增加=调增</strong>（补提法定/任意盈余公积），<strong>借方减少=调减</strong>（转增资本/弥补亏损）</li>
        <li>借贷必须平衡（Σ借方 === Σ贷方）后方可保存发布</li>
        <li>保存后发布 'adjustment:created' 事件通知M5-1审定表刷新AJE/RJE列</li>
        <li>法定盈余公积计提：借:利润分配-提取法定盈余公积 贷:盈余公积-法定盈余公积</li>
        <li>转增资本：借:盈余公积 贷:实收资本/股本</li>
...
/**
 * M5TabAdjustment — M5-3 盈余公积调整分录汇总（借贷平衡）
 *
...
function handleReview() {
  openReviewDialog?.('M5-3-adjustment', '盈余公积调整分录汇总')
}
...
/**
 * 导入 xlsx 成功后重跑本底稿读回路径：M5-3 读回 = 本 Tab restoreEntries（-data 族，push 语义 ⇒ 必须先清空）。
 *
