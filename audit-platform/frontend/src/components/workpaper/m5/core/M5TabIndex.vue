  ...
  <div class="m5-tab-index">
    ...
    <div class="m5-dir">
      <div class="index-header">
        <h3 class="title">M5 底稿目录</h3>
        <GtReviewTrigger section-id="M5-index-directory" />
      ...
      <M5PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="m5-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>盈余公积为<strong>权益类贷方科目</strong>（4101）：期末 = 期初 + 贷方（计提）− 借方（转增资本/弥补亏损）</li>
          <li>明细表 M5-2 分法定盈余公积、任意盈余公积，与审定表 M5-1 合计交叉核对</li>
          <li>M5-4 计提检查：法定盈余公积按净利润 10% 计提，累计达注册资本 50% 可停提；计提基数来自 M6 净利润</li>
          <li>M5 计提盈余公积反向减少 M6 可供分配利润</li>
          <li>审定合计回写 TB(4101) 并驱动附注披露；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="m5-cycle">
...
/**
 * M5TabIndex.vue — M5 盈余公积底稿目录（严格镜像 N5 标准）
 *
 ...
 *
 * 权益类贷方科目（4101 盈余公积）— 期末=期初+贷方−借方；法定 10% 计提、核对 M6 净利润基数。
 */
...
const M5PreparationHandbookDialog = defineAsyncComponent(() => import('../M5PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { name: '盈余公积实质性程序表', code: 'M5A', sheetKey: '盈余公积实质性程序表 M5A', progress: calcSheetProgress('M5-M5A-', 5) },
  { name: '审定表', code: 'M5-1', sheetKey: '审定表M5-1', progress: calcSheetProgress('M5-1-', 6) },
  { name: '明细表', code: 'M5-2', sheetKey: '明细表M5-2', progress: calcSheetProgress('M5-2-', 4) },
  { name: '调整分录汇总', code: 'M5-3', sheetKey: '调整分录汇总M5-3', progress: calcSheetProgress('M5-3-', 4) },
  { name: '盈余公积计提检查表', code: 'M5-4', sheetKey: '盈余公积计提检查表M5-4', progress: calcSheetProgress('M5-4-', 4) },
  { name: '盈余公积检查表', code: 'M5-5', sheetKey: '盈余公积检查表M5-5', progress: calcSheetProgress('M5-5-', 4) },
  { name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', progress: calcSheetProgress('M5-disclosure-listed-', 3) },
  { name: '附注披露信息（国有企业）', code: '附注国企', sheetKey: '附注披露信息（国有企业）', progress: calcSheetProgress('M5-disclosure-soe-', 3) },
])
...
const M5_CONCLUSION_CODES = new Set(['M5-1', 'M5-2', 'M5-4', 'M5-5'])

function isConclusionFilled(code: string): boolean {
  const map = responses.value
  if (!map?.size) return false
  const token = `${code}-`
  const longer = Array.from(M5_CONCLUSION_CODES).filter(c => c !== code && c.startsWith(token))
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => M5_CONCLUSION_CODES.has(s.code))
