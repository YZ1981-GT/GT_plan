...
/**
 * M5 编制/使用手册弹窗 — 对齐 N5PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：M5-2 明细(法定+任意) + M5-4 计提测试(法定 10%，核对 M6 净利润基数，50% 上限) + M5-5 检查 → M5-1 审定(未审+AJE+RJE) → 回写 TB(4101)+附注。盈余公积是权益类贷方：期末=期初+贷方(计提)−借方(转增/弥补)。本弹窗不改各表页内说明。"
    />
