  ...
  <div class="m6-tab-index">
    ...
    <div class="m6-dir">
      <div class="index-header">
        <h3 class="title">M6 底稿目录</h3>
        <GtReviewTrigger section-id="M6-index-directory" />
      ...
      <M6PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="m6-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>未分配利润为<strong>权益类贷方枢纽科目</strong>（4104）：期末 = 期初 + 贷方（本年净利润转入）− 借方（提取盈余公积、宣告股利）</li>
          <li>M6-2 明细为利润分配结转核心公式链：期末 = 期初 + 本年净利润 − 提取盈余公积 − 宣告股利</li>
          <li>上游接收本年净利润；下游驱动 M5 盈余公积计提（基数=M6 净利润）、M1 应付股利</li>
          <li>M6-4 检查：利润分配决议、结转处理合规</li>
    ...
    <div v-if="cycleWorkpapers.length" class="m6-cycle">
...
/**
 * M6TabIndex.vue — M6 未分配利润底稿目录（严格镜像 N5 标准）
 *
 ...
 *
 * 权益类贷方枢纽科目（4104 利润分配-未分配利润）— 期末=期初+贷方−借方；利润分配结转公式链。
 */
...
const M6PreparationHandbookDialog = defineAsyncComponent(() => import('../M6PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { name: '未分配利润实质性程序表', code: 'M6A', sheetKey: '未分配利润实质性程序表 M6A ', progress: calcSheetProgress('M6-M6A-', 5) },
  { name: '审定表', code: 'M6-1', sheetKey: '审定表M6-1', progress: calcSheetProgress('M6-1-', 6) },
  { name: '明细表', code: 'M6-2', sheetKey: '明细表M6-2', progress: calcSheetProgress('M6-2-', 4) },
  { name: '调整分录汇总', code: 'M6-3', sheetKey: '调整分录汇总M6-3', progress: calcSheetProgress('M6-3-', 4) },
  { name: '未分配利润检查表', code: 'M6-4', sheetKey: '未分配利润检查表M6-4', progress: calcSheetProgress('M6-4-', 4) },
  { name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', progress: calcSheetProgress('M6-disclosure-listed-', 3) },
  { name: '附注披露信息（国有企业）', code: '附注国企', sheetKey: '附注披露信息（国有企业）', progress: calcSheetProgress('M6-disclosure-soe-', 3) },
])
...
const M6_CONCLUSION_CODES = new Set(['M6-1', 'M6-2', 'M6-4'])

function isConclusionFilled(code: string): boolean {
  const map = responses.value
  if (!map?.size) return false
  const token = `${code}-`
  const longer = Array.from(M6_CONCLUSION_CODES).filter(c => c !== code && c.startsWith(token))
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => M6_CONCLUSION_CODES.has(s.code))
