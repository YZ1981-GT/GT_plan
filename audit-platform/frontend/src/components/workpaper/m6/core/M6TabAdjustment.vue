...
import type { GenerateWorkpaperAiText } from '@/components/workpaper/composables/useWorkpaperScaffold'
...
const { centralStatus, syncing: centralSyncing, syncToCentral, refreshStatus } = useAdjustmentCentralSync({
  ...
  wpCode: 'M6',
  itemId: () => `M6-adj-${adjustment.activeType.value}`,
  ...
  buildMeta: () => ({
    description: adjustment.filteredEntries.value.find(e => e.description)?.description || 'M6 调整（' + adjustment.activeType.value + '）',
...
async function handleAI(section: string) {
  ...
  try {
    ...
    const context: Record<string, string> = {
      科目: '4104 利润分配-未分配利润 / 调整分录汇总（M6-3）',
      调整类型: adjustment.activeType.value,
      分录笔数: String(adjustment.filteredEntries.value.length),
      借方合计: fmtAmount(bal.totalDebit),
      贷方合计: fmtAmount(bal.totalCredit),
      是否借贷平衡: bal.isBalanced ? '平衡' : `不平衡（差额${fmtAmount(bal.diff)}）`,
      AJE对4104净影响: fmtAmount(adjustment.ajeNet4104.value),
      RJE对4104净影响: fmtAmount(adjustment.rjeNet4104.value),
    }
    const text = await generateAiText({ section: `m6-adjustment-${section}`, context, existingContent: '' })
...
/**
 * 导入 xlsx 成功后重跑本底稿读回路径：M6-3 读回 = useM6Adjustment.loadFromResponses（整体替换，幂等）。
 *
