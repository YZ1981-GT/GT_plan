...
/**
 * M6 编制/使用手册弹窗 — 对齐 N5PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：M6-2 明细(利润分配结转公式链：期末=期初+净利润−盈余公积−股利) + M6-4 检查 → M6-1 审定(未审+AJE+RJE) → 回写 TB(4104)+附注。未分配利润是权益类贷方枢纽：净利润转入贷方增加，提取盈余公积(M5)/宣告股利(M1)借方减少。本弹窗不改各表页内说明。"
    />
