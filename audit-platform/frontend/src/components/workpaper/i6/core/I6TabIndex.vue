  ...
  <div class="i6-tab-index">
    ...
    <div class="i6-dir">
      <div class="index-header">
        <h3 class="title">I6 研发费用底稿目录</h3>
        <GtReviewTrigger section-id="I6-index-directory" />
      ...
      <I6PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="i6-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          ...
          <li>审定发生额回写 TB(6602) 并驱动附注披露与利润表勾稽；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="i6-cycle">
...
/**
 * I6TabIndex.vue — I6 研发费用底稿目录（严格镜像 I1/E1 标准）
 *
 ...
 *
 * 科目 6602 研发费用（损益类，借方）：取本期发生额，非期末余额。
 * 费用化(I6) + 资本化(I2) = 研发总额（VR-I6-01）。
 *
 * props 多声明可选 applicableStandards（主入口透传 :applicable-standards，此处声明以吸收，无需使用）。
 */
...
const I6PreparationHandbookDialog = defineAsyncComponent(() => import('../I6PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { name: '研发费用实质性程序表', code: 'I6A', sheetKey: '研发费用实质性程序表I6A', prefix: 'I6A-' },
  { name: '审定表', code: 'I6-1', sheetKey: '审定表I6-1', prefix: 'I6-1-', conclPrefix: 'I6-1-' },
  { name: '明细表（月度12列）', code: 'I6-2', sheetKey: '明细表I6-2', prefix: 'I6-2-', conclPrefix: 'I6-2-' },
  { name: '调整分录汇总', code: 'I6-3', sheetKey: '调整分录汇总I6-3', prefix: 'I6-3-' },
  { name: '针对性检查表', code: 'I6-4', sheetKey: '针对性检查表I6-4', prefix: 'I6-4-', conclPrefix: 'I6-4-' },
  { name: '截止性测试（账到单据）', code: 'I6-5', sheetKey: '截止性测试（账到单据）I6-5', prefix: 'I6-5-', conclPrefix: 'I6-5-' },
  { name: '截止性测试（单据到账）', code: 'I6-6', sheetKey: '截止性测试（单据到账）I6-6', prefix: 'I6-6-', conclPrefix: 'I6-6-' },
  { name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', prefix: 'I6-disc-L-' },
  { name: '附注披露信息（国有企业）', code: '附注国企', sheetKey: '附注披露信息（国有企业）', prefix: 'I6-disc-S-' },
])
...
const I6_CONCLUSION_CODES = new Set([
  'I6-1', 'I6-2', 'I6-4', 'I6-5', 'I6-6',
])
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => s.conclPrefix && I6_CONCLUSION_CODES.has(s.code))
