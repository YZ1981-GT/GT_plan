...
function _recalcCross(): void { for (const row of rows.value) { row.isCrossPeriod = !!(row.recordPeriod && row.belongPeriod && row.recordPeriod !== row.belongPeriod) } }
function _persist(): void { emit('save', ITEM_ID, JSON.stringify(rows.value)) }

function addRow(): void { rows.value.push({ rowId: `row-${Date.now().toString(36)}`, amount: 0, expenseType: '', recordDate: '', documentDate: '', recordPeriod: '', belongPeriod: '', isCrossPeriod: false, conclusion: '' }) }
...
async function handleAutoSampling(): Promise<void> {
  try {
    const res = await http.get(`/api/projects/${props.projectId}/cutoff-sampling`, { params: { direction: 'backward', account_prefix: '6602', days: 5 }, _silent: true } as any)
    const samples = res.data?.data ?? res.data
    if (Array.isArray(samples) && samples.length > 0) {
      rows.value = samples.map((s: any) => ({ rowId: `row-${Math.random().toString(36).slice(2, 10)}`, amount: Number(s.amount) || 0, expenseType: s.expense_type ?? s.summary ?? '', recordDate: s.record_date ?? '', documentDate: s.document_date ?? '', recordPeriod: s.record_period ?? '', belongPeriod: s.belong_period ?? '', isCrossPeriod: (s.record_period ?? '') !== (s.belong_period ?? ''), conclusion: '' }))
...
<style scoped>
.i6-cutoff-backward { font-size: 13px; padding: 16px; }
