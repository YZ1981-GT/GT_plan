<template>
  ...
  <WpSamplingMethodologyBar :methodology="methodology" />
    ...
    <el-alert type="info" :closable="false" class="objective-alert"
      title="一、审计目标：确认研发费用（6602）已记录于正确的会计期间（从支出凭单追查至记账凭证，关注漏记/跨期）。" />
    ...
    <el-card shadow="never" class="sample-card">
      ...
      <div class="sample-form">
        <div class="sample-row">
          <span>抽取资产负债表日</span>
          <el-date-picker :model-value="criteria.cutoffDate" type="date" size="small" value-format="YYYY-MM-DD"
            placeholder="截止日" style="width:150px" :disabled="isReadonly"
            @update:model-value="(v: string) => updateBackwardCriteria({ cutoffDate: v || '' })" />
          <span>前</span>
          <el-input-number :model-value="criteria.daysBefore" size="small" :min="1" :max="90" :controls="false"
            style="width:64px" :disabled="isReadonly"
            @change="(v: number | undefined) => updateBackwardCriteria({ daysBefore: Number(v) || 5 })" />
          <span>天、后</span>
          <el-input-number :model-value="criteria.daysAfter" size="small" :min="1" :max="90" :controls="false"
            style="width:64px" :disabled="isReadonly"
            @change="(v: number | undefined) => updateBackwardCriteria({ daysAfter: Number(v) || 5 })" />
          <span>天内、金额大于</span>
          <el-input-number :model-value="criteria.amountThreshold" size="small" :min="0" :precision="2"
            :controls="false" style="width:120px" :disabled="isReadonly"
            @change="(v: number | undefined) => updateBackwardCriteria({ amountThreshold: Number(v) || 0 })" />
          <span>元的支出凭单，与研发费用记账凭证核对。</span>
          <el-button v-if="suggestedThreshold" size="small" type="primary" link :disabled="isReadonly"
            @click="applyMaterialityThreshold">采用建议门槛 {{ fmtAmount(suggestedThreshold) }}（约 PM×5%）</el-button>
        </div>
        <div class="sample-row first-year">
          <el-checkbox :model-value="criteria.firstYearClient" :disabled="isReadonly"
    ...
    <div class="tab-toolbar">
      ...
      <div class="toolbar-right">
        <GtIndexChip value="wp:I6-6" :context-project-id="projectId" />
        <el-tag size="small" type="info">共 {{ backwardRows.length }} 行</el-tag>
        <GtIndexChip value="I2-14" @click="emit('navigate-sheet', 'I2-14')" />
      </div>
    </div>
    ...
    <div class="stats-card">
      <div class="stat-item"><span class="stat-label">样本总数</span><span class="stat-value">{{ backwardRows.length }}</span></div>
      <div class="stat-item"><span class="stat-label">跨期笔数</span><span class="stat-value" :class="{ 'stat-danger': backwardCrossPeriodCount > 0 }">{{ backwardCrossPeriodCount }}</span></div>
      <div class="stat-item"><span class="stat-label">跨期金额</span><span class="stat-value" :class="{ 'stat-danger': backwardCrossPeriodAmount > 0 }">{{ fmtAmount(backwardCrossPeriodAmount) }}</span></div>
      <div class="stat-item"><span class="stat-label">跨期比例</span><span class="stat-value" :class="{ 'stat-danger': backwardCrossPeriodCount > 0 }">{{ backwardRows.length > 0 ? ((backwardCrossPeriodCount / backwardRows.length) * 100).toFixed(1) + '%' : '—' }}</span></div>
      <div class="stat-item"><span class="stat-label">金额不一致</span><span class="stat-value" :class="{ 'stat-warn': amountMismatchCount > 0 }">{{ amountMismatchCount }}</span></div>
      <div class="stat-item"><span class="stat-label">滞后异常</span><span class="stat-value" :class="{ 'stat-warn': lagAnomalyCount > 0 }">{{ lagAnomalyCount }}</span></div>
    </div>

    <div class="test-block">
      <div class="block-header"><span class="block-title">三、测试 — 截止日期前样本</span><span class="block-hint">支出凭单日期 ≤ 截止日，追查是否已在本期入账</span></div>
      <I2CutoffSampleTable direction="backward" :rows="backwardBeforeRows" :is-readonly="isReadonly" show-extended @update="onUpdate" @remove="onRemove" />
    </div>
    <div class="cutoff-divider">—— 截止日期：{{ criteria.cutoffDate || '20XX年12月31日' }} ——</div>
    <div class="test-block">
      <div class="block-header"><span class="block-title">截止日期后样本</span><span class="block-hint">支出凭单日期 &gt; 截止日，核查是否被提前计入本期</span></div>
      <I2CutoffSampleTable direction="backward" :rows="backwardAfterRows" :is-readonly="isReadonly" show-extended @update="onUpdate" @remove="onRemove" />
    </div>
    <div v-if="backwardUnsortedRows.length > 0" class="test-block">
      <div class="block-header"><span class="block-title">待归类样本</span><span class="block-hint">请填写支出凭单日期与截止日后自动归入截止日前/后</span></div>
      <I2CutoffSampleTable direction="backward" :rows="backwardUnsortedRows" :is-readonly="isReadonly" show-extended @update="onUpdate" @remove="onRemove" />
    </div>
    ...
    <div class="table-actions">
      ...
      <el-button size="small" plain :disabled="isReadonly" @click="syncCriteriaToPeer('backward')">同步标准→I6-5</el-button>
      <el-button size="small" plain :disabled="isReadonly || isExporting" @click="exportData('I6-6')">导出</el-button>
      ...
      <el-button size="small" type="danger" plain :disabled="isReadonly || backwardCrossPeriodCount === 0" @click="handleDraftAje">跨期→I6-3草稿</el-button>
      <el-button size="small" @click="emit('navigate-sheet', 'I6-5')">对照 I6-5 →</el-button>
      <el-button size="small" type="success" :disabled="isReadonly" @click="handleSave">保存</el-button>
    </div>

    <GtCutoffAutoSampling v-if="showCutoffPanel" account-code="6602" cutoff-direction="pre_cutoff"
      :default-conditions="cutoffPanelDefaults" :workpaper-id="wpId" :project-id="projectId" :year="currentYear"
      :readonly="isReadonly" @filled="handleCutoffFilled" @applied="saveAuditNote" />

    <el-dialog v-model="showSamplingDialog" title="抽凭引擎（6602 研发费用 · I6-6）" width="720px" destroy-on-close>
      <GtVoucherSamplingEngine v-if="showSamplingDialog" :project-id="projectId" :workpaper-id="wpId"
        account-code="6602" phase="final" :year="currentYear" @filled="handleVoucherFilled" />
    </el-dialog>
    <input ref="importInputRef" type="file" accept=".xlsx,.xls" style="display:none" @change="onImportFile" />

    <el-alert v-if="backwardCrossPeriodCount > 0" type="error" :closable="false" show-icon class="expand-alert"
      title="已发现跨期样本：请扩大测试期间与样本量（可扩至审计报告日），并与 I6-5（账→单据）交叉评价；必要时生成 I6-3 调整草稿。" />

    <el-card shadow="never" class="audit-note-card"><template #header><span>四、审计说明</span></template>
      <el-input type="textarea" :model-value="auditNote" :disabled="isReadonly" :autosize="{ minRows: 5 }"
        placeholder="记录样本选取依据、检查范围、发现的跨期事项及处理..." @change="saveAuditNote" /></el-card>
    <el-card shadow="never" class="audit-conclusion-card"><template #header><span>五、审计结论</span></template>
      <el-input type="textarea" :model-value="auditConclusion" :disabled="isReadonly" :autosize="{ minRows: 3 }"
        placeholder="基于测试结果，判断研发费用截止认定是否恰当..." @change="saveAuditConclusion" /></el-card>

    <details class="guidance-details"><summary>编制提示</summary><div class="guidance-content">
      <p>1. 本表为「单据→账簿」方向：从期末前后支出凭单出发，追查是否已记入6602。</p>
      <p>2. 跨期判定：支出凭单日期与记账日期分处截止日两侧即为跨期；跨期金额取支出凭单金额。</p>
      <p>3. 自动取数后须查原件补填单据日期；金额不一致行需追查原因。</p>
      <p>4. 发现跨期时应与 I6-5 交叉评价，并与 I2-14 开发支出截止底稿联动。</p>
    </div></details>
  </div>
</template>
...
const props = defineProps<{
  wpId: string; projectId: string; year?: number
...
const { exportData, importData, isExporting } = useI6ImportExport({ wpId: toRef(props, 'wpId'), onImported: () => emit('save') })
const cutoffPanelDefaults = computed(() => ({
  cutoffDate: criteria.value.cutoffDate, daysBefore: criteria.value.daysBefore,
  daysAfter: criteria.value.daysAfter, amountThreshold: criteria.value.amountThreshold,
}))

const AUDIT_NOTE_KEY = 'I6-6-audit-note'
const AUDIT_CONCLUSION_KEY = 'I6-6-audit-conclusion'
const auditNote = ref('')
...
function saveAuditNote(val: string) { auditNote.value = val; void props.saveResponse('I6-6', { [AUDIT_NOTE_KEY]: val }) }
function saveAuditConclusion(val: string) { auditConclusion.value = val; void props.saveResponse('I6-6', { [AUDIT_CONCLUSION_KEY]: val }) }
watch(() => props.allResponses, () => hydrateAudit(), { immediate: true })
onMounted(() => { hydrateAudit(); void fetchHint() })
...
async function onImportFile(e: Event) {
  ...
  if (!file) return
  await importData(file, 'I6-6')
  ;(e.target as HTMLInputElement).value = ''
}

function rowIndex(row: CutoffRow) { return backwardRows.value.indexOf(row) }
...
function handleVoucherFilled(payload: any) {
  ...
  importExtractedVouchers('backward', vouchers.map((v: any) => ({
    voucher_date: v.voucherDate ?? v.voucher_date, voucher_no: v.voucherNo ?? v.voucher_no,
    summary: v.summary, amount: Number(v.debitAmount ?? v.debit_amount ?? 0),
  })), { fillDocumentDate: false })
  showSamplingDialog.value = false
  if (vouchers.length) ElMessage.success(`抽凭已填入 ${vouchers.length} 笔`)
}
async function handleDraftAje() { await draftAjeFromCrossPeriod('backward'); emit('navigate-sheet', 'I6-3') }
...
.cutoff-divider { margin: 14px 0; text-align: center; font-weight: 600; color: #1d4ed8; border-top: 2px solid #93c5fd; border-bottom: 2px solid #93c5fd; padding: 8px 0; background: #eff6ff; }
