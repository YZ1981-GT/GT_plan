<template>
  <div class="k3-disclosure-soe">
    <!-- 蓝色渐变引导区 -->
    <div class="guide-area">
      <div class="guide-grid">
        <div class="guide-step"><span class="step-num">①</span> 按性质分类披露（保证金/暂收/代收等）</div>
        <div class="guide-step"><span class="step-num">②</span> 按账龄分析披露（1年内/1-2年/2-3年/3年以上）</div>
        <div class="guide-step"><span class="step-num">③</span> 重要其他应付款明细</div>
    ...
    <div class="methodology-block">
      ...
      <div class="methodology-content">
        按《企业会计准则第37号——金融工具列报》及国有企业报表附注格式，其他应付款应披露：
        按款项性质列示期末余额和期初余额；按账龄分析列示；期末重要其他应付款明细；
        长期未支付款项说明。国企版使用"一、二、三..."中文编号格式，关注负债完整性。
      </div>
    </div>
...
/**
 * K3TabDisclosureSoe.vue — 附注披露信息（国有企业版）45×15
 *
...
function persistSection(sIdx: number): void {
  const section = sections[sIdx]
  if (!section) return
  emit('save', `K3-disc-soe-${section.id}`, JSON.stringify({
    rows: section.rows,
...
function loadSavedData(): void {
  for (let i = 0; i < sections.length; i++) {
    ...
    const saved = props.allResponses.get(`K3-disc-soe-${section.id}`)
    if (saved?.value) {
      try {
