...
/**
 * L8 编制/使用手册弹窗 — 对齐 L1PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：L8-2 明细(按项目：利息支出/利息收入/汇兑损益/手续费/其他) → L8-1 审定(未审+账项调整+重分类) → 回写 TB(6603)+附注。财务费用是损益类借方：本期发生额 = 借方发生 − 贷方发生(非期末余额)，利息收入冲减、汇兑按净额。利息支出联动 L1 短期借款 / L3 长期借款 / L4 应付债券 利息测算 + L5 摊销；L8-4 评价非金融机构利息税前扣除性，L8-5 截止测试查费用归属期间。本弹窗不改各表页内说明。"
    />
