  ...
  <div class="m4-tab-index">
    ...
    <div class="m4-dir">
      <div class="index-header">
        <h3 class="title">M4 底稿目录</h3>
        <GtReviewTrigger section-id="M4-index-directory" />
      ...
      <M4PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="m4-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>资本公积为<strong>权益类贷方科目</strong>（4002）：期末 = 期初 + 贷方（增加）− 借方（转增资本/弥补亏损）</li>
          <li>明细表 M4-2 分资本溢价、其他资本公积两大区块，与审定表 M4-1 合计交叉核对</li>
          <li>J3 股份支付权益结算 → 其他资本公积；M2 外币投资折算差异 → 资本溢价</li>
          <li>M4-4 检查：资本溢价、其他资本公积形成依据充分合规</li>
          <li>审定合计回写 TB(4002) 并驱动附注披露；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="m4-cycle">
...
/**
 * M4TabIndex.vue — M4 资本公积底稿目录（严格镜像 N5 标准）
 *
 ...
 *
 * 权益类贷方科目（4002 资本公积）— 期末=期初+贷方−借方；资本溢价+其他资本公积双区块。
 */
...
const M4PreparationHandbookDialog = defineAsyncComponent(() => import('../M4PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { name: '资本公积实质性程序表', code: 'M4A', sheetKey: '资本公积实质性程序表 M4A', progress: calcSheetProgress('M4-M4A-', 5) },
  { name: '审定表', code: 'M4-1', sheetKey: '审定表M4-1', progress: calcSheetProgress('M4-1-', 6) },
  { name: '明细表', code: 'M4-2', sheetKey: '明细表M4-2', progress: calcSheetProgress('M4-2-', 4) },
  { name: '调整分录汇总', code: 'M4-3', sheetKey: '调整分录汇总M4-3', progress: calcSheetProgress('M4-3-', 4) },
  { name: '资本公积检查表', code: 'M4-4', sheetKey: '资本公积检查表M4-4', progress: calcSheetProgress('M4-4-', 4) },
  { name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', progress: calcSheetProgress('M4-disclosure-listed-', 3) },
  { name: '附注披露信息（国有企业）', code: '附注国企', sheetKey: '附注披露信息（国有企业）', progress: calcSheetProgress('M4-disclosure-soe-', 3) },
])
...
const M4_CONCLUSION_CODES = new Set(['M4-1', 'M4-2', 'M4-4'])

function isConclusionFilled(code: string): boolean {
  const map = responses.value
  if (!map?.size) return false
  const token = `${code}-`
  const longer = Array.from(M4_CONCLUSION_CODES).filter(c => c !== code && c.startsWith(token))
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => M4_CONCLUSION_CODES.has(s.code))
