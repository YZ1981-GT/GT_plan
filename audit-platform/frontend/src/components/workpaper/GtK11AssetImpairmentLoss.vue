...
const currentSheet = computed(() => {
  ...
  if (m) return m[1]
  // 底稿目录 K11（无后缀）
  if (/\bK11\b/.test(name) && !/K11-/.test(name) && !/K11A/.test(name)) return 'K11'
  return ''
})
  ...
  try {
    if (props.htmlData) {
      ...
      const res = await http.get(`/workpapers/${props.wpId}/render-config`, {
        params: { force_component_type: 'k11-asset-impairment-loss' },
        _silent: true,
      } as any)
...
function openReviewDialog(sectionId: string, sectionLabel?: string): void {
  console.log('[K11] openReviewDialog:', sectionId, sectionLabel)
}
...
provide('k11VersionTrailRef', versionTrailRef)
provide('k11OpenVersionHistory', openVersionHistory)
