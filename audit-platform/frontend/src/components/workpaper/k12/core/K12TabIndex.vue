  ...
  <div class="k12-tab-index">
    ...
    <div class="k12-dir">
      <div class="index-header">
        <h3 class="title">K12 底稿目录</h3>
        <GtReviewTrigger section-id="K12-index-directory" />
      ...
      <K12PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="k12-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>营业外收入为<strong>损益类贷方科目</strong>（6301）：取本期<strong>发生额</strong>（贷方 = 收入增加），非期末余额</li>
          <li>与日常活动<strong>无关</strong>的利得计入 6301 营业外收入；与日常相关计入 6117 其他收益（K10）</li>
          <li>审定数 = 未审数 + AJE + RJE；审定发生额回写试算平衡表 6301；调整分录联动 A13</li>
          <li>K12-2 明细合计应与 K12-1 审定表一致；K12-4 凭证级核实分类正确性；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="k12-cycle">
...
/**
 * K12TabIndex.vue — K12 营业外收入底稿目录（严格镜像 E1 标准）
 *
 * 结构：目录卡（标题+复核+编制/使用手册+进度→跨表结论口径→编制提示）
 *       + 底稿架构 4 阶段泳道（GtBArchitectureTree）+ 本循环底稿目录 grid。
 * 编制信息由页面级头部渲染，此处不重复。
 *
 * 损益类科目（6301营业外收入）— 贷方=收入增加，取发生额非余额。
 */
import { computed, ref, onMounted, defineAsyncComponent, type Ref } from 'vue'
...
const K12PreparationHandbookDialog = defineAsyncComponent(() => import('../K12PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { seq: 1, name: '营业外收入实质性程序表', code: 'K12A', sheetKey: '营业外收入实质性程序表K12A', group: 'core', progress: calcSheetProgress('K12-K12A-', 5) },
  { seq: 2, name: '审定表', code: 'K12-1', sheetKey: '审定表K12-1', group: 'core', progress: calcSheetProgress('K12-1-', 10) },
  { seq: 3, name: '明细表', code: 'K12-2', sheetKey: '明细表K12-2', group: 'core', progress: calcSheetProgress('K12-2-', 8) },
  { seq: 4, name: '调整分录汇总', code: 'K12-3', sheetKey: '调整分录汇总K12-3', group: 'core', progress: calcSheetProgress('K12-3-', 4) },
  { seq: 5, name: '营业外收入检查表', code: 'K12-4', sheetKey: '营业外收入检查表K12-4', group: 'inspection', progress: calcSheetProgress('K12-4-', 6) },
  { seq: 6, name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', group: 'disclosure', progress: calcSheetProgress('K12-disc-listed-', 5) },
  { seq: 7, name: '附注披露信息（国企）', code: '附注国企', sheetKey: '附注披露信息（国有企业）', group: 'disclosure', progress: calcSheetProgress('K12-disc-soe-', 5) },
])
...
// ─── 跨表结论口径看板 ─────────────────────────────────────────────────────────
/** 有审计结论/说明的 sheet（K12A 程序表、K12-3 调整分录、附注、目录本身不计入） */
const K12_CONCLUSION_CODES = new Set(['K12-1', 'K12-2', 'K12-4'])
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => K12_CONCLUSION_CODES.has(s.code))
