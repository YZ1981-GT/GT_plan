...
async function generateAuditNote(): Promise<void> {
  ...
  const text = await generateText({
    section: 'e1-18-audit-note',
    prompt: '你是注册会计师助理。请撰写 E1-18 企业信用报告查询 的审计说明，概述审计程序执行情况与主要发现。不得虚构。约 100～200 字。',
    context: { 底稿: 'E1-18 企业信用报告查询', 说明: auditNote.value },
...
async function generateAuditConclusion(): Promise<void> {
  if (props.isReadonly) return
  const text = await generateText({
    section: 'e1-18-audit-conclusion',
    prompt: '你是注册会计师助理。请撰写 E1-18 企业信用报告查询 的审计结论，对审计程序结果给出结论性评价。不得虚构。约 60～150 字。',
    context: { 底稿: 'E1-18 企业信用报告查询', 结论: auditConclusion.value },
