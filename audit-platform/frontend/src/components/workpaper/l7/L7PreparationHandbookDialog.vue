...
/**
 * L7 编制/使用手册弹窗 — 对齐 L1PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  <el-dialog
    ...
    title="L7 其他非流动负债 · 编制手册"
    width="860px"
    top="4vh"
    destroy-on-close
    append-to-body
    class="l7-handbook-dialog"
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：L7-2 明细(按项目逐行) → L7-1 审定(按项目，未审+账项调整+重分类，双期) → 回写 TB(2801)+附注。其他非流动负债是负债类贷方：期末=期初+贷方(增加)−借方(减少)。首要判断分类恰当性(是否应归入长期借款/应付债券/长期应付款/递延收益等更明确科目)，一年内到期部分经 L7-3 重分类至流动负债。本弹窗不改各表页内说明。"
    />
