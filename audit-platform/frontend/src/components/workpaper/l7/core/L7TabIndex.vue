  ...
  <div class="l7-tab-index">
    ...
    <div class="l7-dir">
      <div class="index-header">
        <h3 class="title">L7 其他非流动负债底稿目录</h3>
        <GtReviewTrigger section-id="L7-index-directory" />
      ...
      <L7PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="l7-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>其他非流动负债为<strong>负债类贷方科目</strong>（2801）：期末 = 期初 + 贷方（增加）− 借方（减少）</li>
          <li>审定表按<strong>项目</strong>双期列示（期初/期末各未审、账项调整、重分类、审定），项目行可增删</li>
          <li>首要判断<strong>分类恰当性</strong>：是否应归入长期借款/应付债券/长期应付款/递延收益等更明确科目</li>
          <li><strong>一年内到期</strong>部分经 L7-3 重分类至流动负债（一年内到期的非流动负债 / 其他流动负债）</li>
          <li>审定合计（按项目）回写 TB(2801) 并驱动附注披露；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="l7-cycle">
...
/**
 * L7TabIndex.vue — L7 其他非流动负债底稿目录（严格镜像 L1/E1 标准）
 *
 ...
 *
 * 负债类贷方科目（2801 其他非流动负债）— 期末=期初+贷方−借方；审定按项目双期，
 * 项目行可增删；首要判断分类恰当性（是否应归入更明确科目）；一年内到期经 L7-3 重分类。
 *
 * 🔴 L7 各表审计说明/结论存储键前缀不统一（L7-L7-1-auditNote / L7-2-auditNote /
 *    L7-L7-4-overall-conclusion），非编码式，故 conclusionSheets 携各自 conclPrefix；
 *    calcSheetProgress 用真实前缀。
 */
...
const L7PreparationHandbookDialog = defineAsyncComponent(() => import('../L7PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { name: '其他非流动负债实质性程序表', code: 'L7A', sheetKey: '其他非流动负债实质性程序表L7A', progress: calcSheetProgress('L7-L7A-', 5) },
  { name: '审定表（按项目，双期）', code: 'L7-1', sheetKey: '审定表L7-1', progress: calcSheetProgress('L7-L7-1-', 6), conclPrefix: 'L7-L7-1-' },
  { name: '明细表（按项目逐行）', code: 'L7-2', sheetKey: '明细表L7-2', progress: calcSheetProgress('L7-L7-2-', 4), conclPrefix: 'L7-2-auditNote' },
  { name: '调整分录汇总（含一年内到期重分类）', code: 'L7-3', sheetKey: '调整分录汇总L7-3', progress: calcSheetProgress('L7-L7-3-entry-', 2) },
  { name: '其他非流动负债检查表（分类恰当性/完整性/准确性）', code: 'L7-4', sheetKey: '其他非流动负债检查表L7-4', progress: calcSheetProgress('L7-L7-4-', 4), conclPrefix: 'L7-L7-4-overall-conclusion' },
  { name: '附注披露信息核对（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', progress: calcSheetProgress('L7-disclosure-listed-', 3) },
  { name: '附注披露信息核对（国企）', code: '附注国企', sheetKey: '附注披露信息（国企）', progress: calcSheetProgress('L7-disclosure-soe-', 3) },
])
...
// ─── 跨表结论口径看板 ─────────────────────────────────────────────────────────
/** 有审计结论/说明的核心表（审定/明细/检查类；程序表、调整分录、附注不计入） */
const L7_CONCLUSION_CODES = new Set(['L7-1', 'L7-2', 'L7-4'])
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => s.conclPrefix && L7_CONCLUSION_CODES.has(s.code))
