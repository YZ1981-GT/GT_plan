...
// ─── 从序时账取数（资产类科目1604 借方=增加 贷方=减少）────────────────────────
const ledgerPulling = ref(false)
async function handlePullFromLedger() {
  const year = new Date().getFullYear() - 1
  ledgerPulling.value = true
  try {
    const result = await pullAssetMovementFromLedger(props.projectId, year, '1604')
    if (!result.ok) {
    ...
    for (const item of result.rows) {
      const existing = state.rows.value.find(
        (r) => r.name === item.name || r.name.includes(item.name) || item.name.includes(r.name),
      )
      if (existing) {
        // H2 增加合计 = increaseTotal（公式列），手工列= increaseMaterial 等；填入 increaseMaterial 作为主增加入口
        if (!existing.increaseMaterial && !existing.increaseOther && item.increase > 0.005) {
          state.updateCell(existing.rowId, 'increaseOther', item.increase)
          matched++
        }
        // H2 减少 = decrease（其他减少，非转固）
        if (!existing.decrease && item.decrease > 0.005) {
          state.updateCell(existing.rowId, 'decrease', item.decrease)
          matched++
        }
      }
    }
