  ...
  <div class="h2-tab-disclosure-listed">
    ...
    <el-card shadow="never" class="audit-note-card">
      ...
      <el-input :model-value="auditNote" type="textarea" :autosize="{ minRows: 5 }"
        placeholder="填写审计说明：概述附注各子节取数来源、跨表勾稽核对及披露完整性核查情况。" :disabled="isReadonly"
    ...
    <el-card shadow="never" class="audit-note-card">
      ...
      <el-input :model-value="auditConclusion" type="textarea" :autosize="{ minRows: 3 }"
        placeholder="填写审计结论：如附注披露完整、与审定表勾稽一致，符合上市公司披露要求，未见异常。" :disabled="isReadonly"
...
// H2 附注(上市)审计说明/结论：本地变体键（listed/soe 共用同一 composable，避免串写）。
const NOTE_KEY = 'H2-disc-listed-audit-note'
const CONCLUSION_KEY = 'H2-disc-listed-audit-conclusion'
const auditNote = ref('')
