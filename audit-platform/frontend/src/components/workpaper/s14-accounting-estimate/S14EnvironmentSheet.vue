...
// ─── 评价项目（骨架数据，seed 时从 responses 覆盖） ─────────────────────
...
const ROWS_ID = 'S14-1-rows'
const CONCLUSION_ID = 'S14-1-conclusion'

const { seedOnMount, save } = useSExpertPersist(() => props.allResponses)
seedOnMount([
  { itemId: ROWS_ID, ref: evaluationItems },
  { itemId: CONCLUSION_ID, ref: auditConclusion },
])

function saveRows(): void {
  save(ROWS_ID, evaluationItems.value)
}
