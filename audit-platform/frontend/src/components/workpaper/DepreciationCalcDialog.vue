<template>
  ...
  >
    ...
    >
      <template #default>
        支持 4 种折旧方法：直线法 / 双倍余额递减法 / 年数总和法 / 工作量法。
    ...
    <el-form :model="form" label-width="140px" size="small">
      <el-form-item label="折旧方法" required>
        <el-radio-group v-model="form.method">
          ...
          <el-radio value="double_declining">双倍余额递减</el-radio>
          <el-radio value="sum_of_years">年数总和</el-radio>
      ...
      <el-form-item label="残值率" required>
        <el-input-number
          ...
          style="width: 160px"
        />
        <span class="gt-form-unit">（0~1，如 0.05 = 5%）</span>
      </el-form-item>
      ...
      <el-form-item label="已计提月数">
        <el-input-number
          v-model="form.already_depreciated_months"
          :min="0"
...
const result = ref<DepreciationResponse | null>(null)
...
async function onCalc() {
  ...
  try {
    const body: Record<string, any> = {
      ...
      already_depreciated_months: form.already_depreciated_months,
    }
    if (form.method === 'units_of_production') {
      body.total_units = form.total_units
      body.current_period_units = form.current_period_units
    }
    const resp = await api.post<DepreciationResponse>(
      `/api/projects/${props.projectId}/workpapers/${props.wpId}/h1/depreciation-calc`,
      body,
    )
    result.value = resp
