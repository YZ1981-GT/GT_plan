<template>
  <div class="l4-tab-book-recon-installment">
    <!-- ═══ 标题 + AI/复核 ═══ -->
    <div class="section-header">
      <div class="section-header-left">
        <h3 class="section-title">L4-8B 账面核对（分期付息到期一次还本）</h3>
      </div>
    ...
    <div class="methodology-context">
      <div class="methodology-text">
        <strong>账面核对（分期付息分支）：</strong>
        对比实际账面摊余成本与L4-7B测算值。分期付息分支末期摊余成本应趋面值，
        差异=账面−测算。超阈值红色高亮。特别关注末期是否与面值一致。
      </div>
    </div>
    ...
    >
      ...
      <el-table-column label="测算摊余成本" min-width="140" align="right">
        <template #header>
          <el-tooltip content="来自L4-7B后续计量测算值" placement="top">
    ...
    <details class="l4-details-tip">
      ...
      <ul>
        <li>分期付息分支末期摊余成本应≈面值</li>
        <li>差异超阈值需逐期追查：是否利息调整摊销遗漏或计算偏差</li>
        <li>核对通过后可出具"账面一致"结论</li>
      </ul>
    </details>
  </div>
</template>
...
/**
 * L4TabBookReconInstallment — L4-8B 账面核对（分期付息到期一次还本）
 *
...
const {
  ...
  setThreshold,
  updateBookCost,
} = useL4BookRecon(formData, ref('installment') as import('vue').Ref<L4BondBranch>, schedules, bookData)
...
<style scoped>
.l4-tab-book-recon-installment { padding: 12px; font-size: 13px; }
...
.threshold-row { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; font-size: 13px; color: #606266; }
