...
function saveAuditNote() {
  formData.debouncedSave('L4-7B-auditNote', { remark: auditNote.value || null })
}
...
function handleAI(section: string) {
  import('@/utils/http').then(({ default: h }) => {
    h.post(`/api/workpapers/${props.wpId}/ai/generate-text`, {
      section: `l4-subsequent-installment-${section}`,
      prompt: `请基于应付债券后续计量（分期付息到期一次还本）"${section}"数据，验证实际利率法摊销合理性`,
...
<style scoped>
.l4-tab-subsequent-installment { padding: 12px; font-size: var(--wp-font-size, 13px); }
