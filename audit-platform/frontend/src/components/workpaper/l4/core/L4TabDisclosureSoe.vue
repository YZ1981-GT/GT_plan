<template>
  <div class="l4-tab-disclosure-soe">
    <!-- ═══ 标题 + AI/复核 ═══ -->
    ...
    <div class="methodology-context">
      <div class="methodology-text">
        <strong>国有企业应付债券附注披露要求：</strong>
        除通用披露要求外，国企需额外披露：①债券审批机关及批准文号 ②资金用途及使用进度
        ③偿债计划及资金来源 ④国资委备案情况。核对审定表数据与附注一致。
      </div>
    </div>
    ...
    <details class="l4-details-tip">
      ...
      <ul>
        <li>国企额外披露：审批文号、资金用途、偿债计划、国资委备案</li>
...
/**
 * L4TabDisclosureSoe — 附注披露信息核对（国有企业）
 *
...
const disclosureItems = ref<DisclosureItem[]>([
  ...
  { seq: 8, item: '审批机关及批准文号', noteContent: '', auditedData: '', isConsistent: '', diffNote: '' },
  { seq: 9, item: '资金用途及使用进度', noteContent: '', auditedData: '', isConsistent: '', diffNote: '' },
  { seq: 10, item: '偿债计划及资金来源', noteContent: '', auditedData: '', isConsistent: '', diffNote: '' },
  { seq: 11, item: '国资委备案情况', noteContent: '', auditedData: '', isConsistent: '', diffNote: '' },
  { seq: 12, item: '担保情况', noteContent: '', auditedData: '', isConsistent: '', diffNote: '' },
])
...
function saveItem(index: number) {
  ...
  formData.debouncedSave(`L4-disc-soe-${item.seq}`, {
...
function saveNoteText() {
  formData.debouncedSave('L4-disc-soe-noteText', { remark: noteText.value || null })
}
...
function handleAI(section: string) {
  import('@/utils/http').then(({ default: h }) => {
    h.post(`/api/workpapers/${props.wpId}/ai/generate-text`, {
      section: `l4-disclosure-soe-${section}`,
      prompt: `请基于应付债券附注披露（国企）"${section}"区段，检查披露完整性并给出审计建议`,
...
<style scoped>
.l4-tab-disclosure-soe { padding: 12px; font-size: var(--wp-font-size, 13px); }
