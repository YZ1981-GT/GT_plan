  ...
  <div class="l4-tab-index">
    ...
    <div class="l4-dir">
      <div class="index-header">
        <h3 class="title">L4 应付债券底稿目录</h3>
        <GtReviewTrigger section-id="L4-index-directory" />
      ...
      <L4PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="l4-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>应付债券为<strong>负债类贷方科目</strong>（2502）：期末 = 期初 + 贷方（发行 + 利息调整摊销）− 借方（兑付）</li>
          <li>审定表按<strong>品种</strong>（普通/可转换）× <strong>子项</strong>（成本/利息调整/应计利息）双期列示，含品种小计、合计与「减一年内到期」重分类</li>
          <li><strong>实际利率法</strong>后续计量：每期利息费用 = 期初摊余成本 × 实际利率；2 分支（到期一次还本付息 / 分期付息到期一次还本）</li>
          <li>初始计量：初始入账 = 发行价 − 交易费用；溢折价 = 初始入账 − 面值；IRR 求解实际利率。最后一期摊余成本应≈面值（±1 元尾差）</li>
          <li><strong>权益负债划分</strong>（可转债）：负债成分 = 未来现金流按市场利率折现，权益成分 = 发行总额 − 负债成分现值</li>
          <li>审定合计（按品种）回写 TB(2502) 并驱动附注披露；L4-7 应计利息联动 L2 应付利息 / L8 财务费用；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="l4-cycle">
...
/**
 * L4TabIndex.vue — L4 应付债券底稿目录（严格镜像 L1/E1 标准）
 *
 ...
 *
 * 负债类贷方科目（2502 应付债券）— 期末=期初+贷方−借方；审定按品种(普通/可转换)×子项
 * (成本/利息调整/应计利息)双期，减一年内到期重分类；实际利率法后续计量(L4-7)联动 L2/L8，
 * L4-5 权益负债划分(可转债分拆)、L4-8 账面核对保证准确。
 *
 * 🔴 L4 各表审计说明/结论存储键为混合式（L4-L4-1-note / L4-2-auditNote / L4-7A-auditNote /
 *    L4-8A-conclusion / L4-9-conclusion 等），故 conclusionSheets 携各自 conclPrefix；
 *    calcSheetProgress 用真实前缀。L4 各子组件各自 useL4FormData（非 inject 共享），此处不触及。
 */
...
const L4PreparationHandbookDialog = defineAsyncComponent(() => import('../L4PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { name: '应付债券实质性程序表', code: 'L4A', sheetKey: '应付债券实质性程序表L4A', progress: calcSheetProgress('L4-L4A-', 5) },
  { name: '审定表（按品种：普通/可转换×成本/利息调整/应计利息，双期，减一年内到期）', code: 'L4-1', sheetKey: '审定表L4-1', progress: calcSheetProgress('L4-L4-1-', 6), conclPrefix: 'L4-L4-1-' },
  { name: '应付债券明细表（89 列极宽表，区段 Tab）', code: 'L4-2', sheetKey: '应付债券明细表L4-2', progress: calcSheetProgress('L4-2-', 4), conclPrefix: 'L4-2-auditNote' },
  { name: '划分为金融负债的其他金融工具明细表', code: 'L4-3', sheetKey: '划分为金融负债的其他金融工具明细表L4-3', progress: calcSheetProgress('L4-3-', 4), conclPrefix: 'L4-3-auditNote' },
  { name: '调整分录汇总', code: 'L4-4', sheetKey: '调整分录汇总L4-4', progress: calcSheetProgress('L4-4-entry', 2) },
  { name: '权益与负债划分检查表（复合工具分拆）', code: 'L4-5', sheetKey: '权益与负债划分检查表L4-5', progress: calcSheetProgress('L4-5-', 4), conclPrefix: 'L4-5-auditNote' },
  { name: '应付债券初始计量（发行价−交易费用，IRR 求实际利率）', code: 'L4-6', sheetKey: '应付债券初始计量L4-6', progress: calcSheetProgress('L4-6-', 4), conclPrefix: 'L4-6-auditNote' },
  { name: '应付债券后续计量（实际利率法，2 分支，联动 L2/L8）', code: 'L4-7', sheetKey: '应付债券后续计量L4-7', progress: calcSheetProgress('L4-7', 4), conclPrefix: 'L4-7' },
  { name: '应付债券账面核对（账面 vs L4-7 测算摊余成本）', code: 'L4-8', sheetKey: '应付债券账面核对L4-8', progress: calcSheetProgress('L4-8', 4), conclPrefix: 'L4-8' },
  { name: '应付债券检查表', code: 'L4-9', sheetKey: '应付债券检查表L4-9', progress: calcSheetProgress('L4-9-', 4), conclPrefix: 'L4-9-conclusion' },
  { name: '附注披露信息核对（上市公司）', code: '附注上市', sheetKey: '附注披露信息核对（上市公司）', progress: calcSheetProgress('L4-disc-listed-', 3) },
  { name: '附注披露信息核对（国企）', code: '附注国企', sheetKey: '附注披露信息核对（国企）', progress: calcSheetProgress('L4-disc-soe-', 3) },
])
...
// ─── 跨表结论口径看板 ─────────────────────────────────────────────────────────
/** 有审计结论/说明的核心表（审定/明细/检查/计量类；程序表、调整分录、附注不计入） */
const L4_CONCLUSION_CODES = new Set(['L4-1', 'L4-2', 'L4-3', 'L4-5', 'L4-6', 'L4-7', 'L4-8', 'L4-9'])

/**
 * 按各表真实 conclPrefix（混合式）判定审计说明/结论是否已填。
 * key 含 conclPrefix 且匹配 /conclusion|audit-note|note/ 且文本非空即为已填；
 * L4-7/L4-8 的跨sheet键（L4-L4-7-schedule-rows / L4-L4-8-book-amortized-cost）虽含 'L4-7'/'L4-8'
 * 子串，但不含 note/conclusion 故被正则过滤，不误判。
 */
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => s.conclPrefix && L4_CONCLUSION_CODES.has(s.code))
