...
/**
 * L4 编制/使用手册弹窗 — 对齐 L1PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：L4-2 明细(按债券逐笔) + L4-6 初始计量(发行价−交易费用, IRR 求实际利率) + L4-7 后续计量(实际利率法：每期利息费用=期初摊余成本×实际利率, 2分支) → L4-1 审定(按品种：普通/可转换×成本/利息调整/应计利息, 减一年内到期, 双期) → 回写 TB(2502)+附注。应付债券是负债类贷方：期末=期初+贷方(发行+利息调整)−借方(兑付)。L4-5 权益负债划分(可转债分拆)/L4-8 账面核对保证准确。本弹窗不改各表页内说明。"
    />
