      ...
      <main class="document-groups">
        <el-card shadow="never" class="document-card">
          ...
          <div class="field-grid">
            ...
            <label>{{ amountLabel }}<el-input-number v-model="form.amount" :controls="false" size="small" :disabled="readonly" /></label>
            <label>票据类别
              <el-select v-model="form.noteType" size="small" clearable :disabled="readonly">
                <el-option v-for="t in F3_VOUCHER_NOTE_TYPES" :key="t" :label="t" :value="t" />
              </el-select>
            </label>
          </div>
        </el-card>
        ...
        <template v-if="evidenceKind === 'payment'">
          <el-card shadow="never" class="document-card">
            <template #header>
              <span>② 付款审批单</span>
              <el-upload v-if="canOcr" :show-file-list="false" :auto-upload="false"
                accept=".pdf,.png,.jpg,.jpeg" :disabled="isOcrLoading('approval')"
                @change="(file: any) => runOcr(file?.raw, 'approval')">
                <el-button link type="primary" size="small" :loading="isOcrLoading('approval')">📎 上传审批单并识别</el-button>
              </el-upload>
            </template>
            <div class="field-grid">
          ...
          <el-card shadow="never" class="document-card">
            <template #header>
              <span>③ 银行回单</span>
              <el-upload v-if="canOcr" :show-file-list="false" :auto-upload="false"
                accept=".pdf,.png,.jpg,.jpeg" :disabled="isOcrLoading('bank-receipt')"
                @change="(file: any) => runOcr(file?.raw, 'bank-receipt')">
                <el-button link type="primary" size="small" :loading="isOcrLoading('bank-receipt')">📎 上传回单并识别</el-button>
              </el-upload>
            </template>
            <div class="field-grid">
              ...
              <label>金额<el-input-number v-model="form.bankAmount" :controls="false" size="small" :disabled="readonly" /></label>
            </div>
          </el-card>
        </template>
        ...
        <template v-else>
          <el-card shadow="never" class="document-card">
            <template #header>
              <span>② 入库单 / 验收单</span>
              <el-upload v-if="canOcr" :show-file-list="false" :auto-upload="false"
                accept=".pdf,.png,.jpg,.jpeg" :disabled="isOcrLoading('goods-receipt')"
                @change="(file: any) => runOcr(file?.raw, 'goods-receipt')">
                <el-button link type="primary" size="small" :loading="isOcrLoading('goods-receipt')">📎 上传入库单并识别</el-button>
              </el-upload>
            </template>
            <div class="field-grid">
              ...
              <label>数量<el-input-number v-model="form.receiptQty" :controls="false" size="small" :disabled="readonly" /></label>
            </div>
          </el-card>
          ...
          <el-card shadow="never" class="document-card">
            <template #header>
              <span>③ 采购发票</span>
              <el-upload v-if="canOcr" :show-file-list="false" :auto-upload="false"
                accept=".pdf,.png,.jpg,.jpeg" :disabled="isOcrLoading('invoice')"
                @change="(file: any) => runOcr(file?.raw, 'invoice')">
                <el-button link type="primary" size="small" :loading="isOcrLoading('invoice')">📎 上传发票并识别</el-button>
              </el-upload>
            </template>
            <div class="field-grid">
              ...
              <label>金额<el-input-number v-model="form.invoiceAmount" :controls="false" size="small" :disabled="readonly" /></label>
            </div>
          </el-card>
        </template>
        ...
        <el-card shadow="never" class="document-card">
          ...
          <div class="field-grid">
            <label class="full">其他检查证据说明<el-input v-model="form.otherEvidence" type="textarea" :rows="2" :disabled="readonly" placeholder="检查的关键证据和要素根据被审计单位具体情况补充" /></label>
          </div>
        </el-card>
        ...
        <el-card v-if="projectId && wpId" shadow="never" class="document-card">
          ...
          <ItemAttachment
            :project-id="projectId" :wp-id="wpId"
            :sheet-key="`F3-7-${section}`" :item-index="form.attSlot"
...
/** F3VoucherCheckDialog — F3-7 逐笔单据核对弹窗（参照 F2-56 模式） */
...
type F3OcrDocumentType = 'voucher' | 'approval' | 'bank-receipt' | 'goods-receipt' | 'invoice'

const props = defineProps<{
  modelValue: boolean
  row: F3VoucherCheckRow | null
...
const emit = defineEmits<{
  ...
  (event: 'save', section: F3VoucherSection, patch: F3VoucherCheckRow): void
}>()

const form = ref<F3VoucherCheckRow | null>(null)
watch(
...
const dialogTitle = computed(() =>
  `逐笔单据核对 · ${F3_VOUCHER_SECTION_LABELS[props.section]} · ${form.value?.voucherNo || '未命名凭证'}`,
)
const checks = computed(() =>
  form.value ? evaluateF3VoucherEvidence(form.value, evidenceKind.value) : [],
)
...
const ocrLoadingType = ref<F3OcrDocumentType | null>(null)

const { aiAvailable, loading: aiLoading, generateAndConfirm } = useF3AiGenerate(
  computed(() => props.wpId || '') as Ref<string>,
)

function isOcrLoading(documentType: F3OcrDocumentType): boolean {
  return ocrLoadingType.value === documentType
}

async function runOcr(file: File | undefined, documentType: F3OcrDocumentType): Promise<void> {
  ...
  try {
    ...
    const res = await http.post(`/api/workpapers/${props.wpId}/f3/contract-ocr`, formData, {
    ...
    await ElMessageBox.confirm(
      ...
      'OCR 提取结果',
      { confirmButtonText: '填入本单据区域', cancelButtonText: '取消', type: confidence < 0.8 ? 'warning' : 'info' },
    )
...
function applyOcrFields(documentType: F3OcrDocumentType, fields: Record<string, unknown>): void {
  ...
  if (documentType === 'voucher') {
    ...
    if (fields.noteType) row.noteType = text(fields.noteType)
...
async function runIssueAi(): Promise<void> {
  ...
  const context = {
    sheet: 'F3-7',
    section: F3_VOUCHER_SECTION_LABELS[props.section],
...
.field-grid :deep(.el-input-number), .field-grid :deep(.el-select), .check-side :deep(.el-select) { width: 100%; }
