<template>
  <div class="i2-cutoff-backward">
    ...
    <el-alert type="info" :closable="false" class="objective-alert" title="审计目标：从原始单据出发核查期末前后的研发支出是否及时、完整入账，确认不存在应记未记的开发支出跨期漏记。" />
    <details class="guidance-details"><summary>📋 编制提示</summary><div class="guidance-content"><p>1. 从期末前后各5天的原始单据（领料单、工时单等）出发；</p><p>2. 逐笔追查账簿记录，判断单据是否已在正确期间入账、有无漏记；</p><p>3. 依据 CAS6《无形资产》及 CAS8《资产减值》。</p></div></details>
    ...
    <div class="methodology-context">
      <p>反向截止测试：从原始单据出发，核对账簿记录（期末±5天）。验证期末附近的研发支出单据是否已及时入账，关注漏记风险。</p>
    </div>
    ...
    <div class="stats-card">
      ...
      <div class="stat-item">
        ...
        <span class="stat-value stat-danger">{{ backwardCrossPeriodCount }}</span>
      </div>
      <div class="stat-item">
    ...
    <!-- 数据表 -->
    <el-table :data="backwardRows" border size="small" class="cutoff-table" max-height="460" :row-class-name="rowClassName">
      <el-table-column type="index" label="#" width="40" fixed />
      <el-table-column prop="voucherNo" label="凭证号" min-width="110">
        <template #default="{ row, $index }">
          <el-input v-model="row.voucherNo" size="small" placeholder="凭证号" @change="(v: string) => updateBackwardRow($index, 'voucherNo', v)" />
        </template>
      </el-table-column>
      <el-table-column prop="recordDate" label="记账日期" min-width="130">
        <template #default="{ row, $index }">
          <el-date-picker v-model="row.recordDate" type="date" size="small" value-format="YYYY-MM-DD" placeholder="记账日期" style="width:100%" @change="(v: string) => updateBackwardRow($index, 'recordDate', v)" />
        </template>
      </el-table-column>
      <el-table-column prop="amount" label="金额" min-width="110" align="right">
        <template #default="{ row, $index }">
          <el-input-number v-model="row.amount" size="small" :controls="false" :precision="2" style="width:100%" @change="(v: number) => updateBackwardRow($index, 'amount', v)" />
        </template>
      </el-table-column>
      <el-table-column prop="documentDate" label="单据日期" min-width="130">
        <template #default="{ row, $index }">
          <el-date-picker v-model="row.documentDate" type="date" size="small" value-format="YYYY-MM-DD" placeholder="单据日期" style="width:100%" @change="(v: string) => updateBackwardRow($index, 'documentDate', v)" />
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="60" align="center" fixed="right">
        <template #default="{ $index }">
          <el-button size="small" type="danger" text @click="removeBackwardRow($index)">删</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <div class="table-actions">
      <el-button size="small" type="primary" plain @click="addBackwardRow()">+ 新增行</el-button>
...
function saveAuditNote(val: string) { auditNote.value = val; void props.saveResponse('I2-14', { [AUDIT_NOTE_KEY]: val }) }
function saveAuditConclusion(val: string) { auditConclusion.value = val; void props.saveResponse('I2-14', { [AUDIT_CONCLUSION_KEY]: val }) }
...
async function handleSave() {
  await saveCutoff()
  emit('save')
  ElMessage.success('反向截止性测试已保存')
}

function handleReview() { openReviewDialog('I2-14-截止性测试-反向') }
</script>
