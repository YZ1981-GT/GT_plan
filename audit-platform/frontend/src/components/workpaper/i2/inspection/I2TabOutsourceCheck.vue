<template>
  <div class="i2-outsource-check">
    ...
    <el-alert type="info" :closable="false" class="objective-alert"
      title="审计目标：核查委外研发支出的真实性、完整性与验收有效性，确认委外费用确用于资本化研发项目且归集金额准确。" />

    <!-- 编制提示 -->
    <details class="guidance-details">
      <summary>📋 编制提示</summary>
      <div class="guidance-content">
        <p>1. 核查受托方资质、委外合同、结算单据及交付物验收记录是否齐全并相互印证；</p>
        <p>2. 关注委外研发费用加计扣除按实际发生额 80% 计入的口径是否正确执行；</p>
...
// ─── 审计说明 / 审计结论 ───
const AUDIT_NOTE_KEY = 'I2-11-audit-note'
const AUDIT_CONCLUSION_KEY = 'I2-11-audit-conclusion'
const auditNote = ref('')
...
function saveAuditNote(val: string) { auditNote.value = val; void props.saveResponse('I2-11', { [AUDIT_NOTE_KEY]: val }) }
function saveAuditConclusion(val: string) { auditConclusion.value = val; void props.saveResponse('I2-11', { [AUDIT_CONCLUSION_KEY]: val }) }
