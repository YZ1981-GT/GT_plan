...
/**
 * M9 编制/使用手册弹窗 — 对齐 N5PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  <el-dialog
    ...
    title="M9 其他综合收益 · 编制手册"
    width="860px"
    top="4vh"
    destroy-on-close
    append-to-body
    class="m9-handbook-dialog"
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：M9-2 明细(OCI 分项+税后净额，不可/可重分类双大类) + M9-4 核对(G8 公允变动 + J2 重计量 + 外币折算) → M9-1 审定(未审+AJE+RJE，双大类小计) → 回写 TB(4103)+附注。OCI 是权益类贷方：期末=期初+贷方(增加)−借方(减少/重分类进损益)。本弹窗不改各表页内说明。"
    />
