<template>
  <div class="m8-tab-disclosure-soe">
    <!-- ═══ 标题 + AI/复核 ═══ -->
    ...
    <div class="methodology-context">
      <div class="methodology-text">
        <strong>国有企业一般风险准备附注披露（11×14）：</strong>
        国有金融企业应按财政部相关规定，在附注中披露一般风险准备的期初余额、本期增减变动及期末余额。
    ...
    <el-card shadow="never" class="disclosure-card">
      ...
      <el-input
        ...
        :disabled="isReadonly"
        placeholder="说明一般风险准备计提政策、计提比例、变动原因及国有金融企业特殊要求等..."
    ...
    <el-card shadow="never" class="disclosure-card">
      <template #header>
        <div class="card-header">
          <span>国有金融企业特殊披露要求</span>
          <el-button size="small" :loading="aiLoading === 'section-soe-special'" @click="handleAI('section-soe-special')">
            <el-icon><MagicStick /></el-icon> AI
          </el-button>
        </div>
      </template>
      <el-input
        ...
        :autosize="{ minRows: 3, maxRows: 8 }"
        :disabled="isReadonly"
        placeholder="说明是否满足财政部关于国有金融企业一般风险准备计提不低于风险资产期末余额1.5%的要求..."
    ...
    <details class="m8-details-tip">
      ...
      <ul>
        <li>国有金融企业应按财政部规定披露一般风险准备变动情况</li>
        ...
        <li>国有企业需额外说明是否满足"不低于风险资产1.5%"的监管要求</li>
        <li>3 data rows + 1 total row (合计SUM)，与上市公司版结构一致</li>
      </ul>
    </details>
  </div>
</template>
...
/**
 * M8TabDisclosureSoe — 附注披露信息（国有企业）
 *
 * Spec: .kiro/specs/m8-general-risk-reserve/
 * Task: 4.5
 * Requirements: 4.2-4.4
 *
 * Same structure as M8TabDisclosureListed but for SOE (国有企业).
 * Nearly identical — same columns, same formulas. Only the title and
 * special SOE regulatory requirements note section differ.
 *
 * xlsx结构：11×14, 16 formulas
 * Columns: 项目(A) | 期初余额(B) | 本期增加(C) | 本期减少(D) | 期末余额(E=B+C-D)
...
async function handleAI(section: string): Promise<void> {
  ...
  try {
    ...
    const isSpecial = section === 'section-soe-special'
    const existing = isSpecial ? soeSpecialNote.value : disclosureNote.value
    const text = await generateAiText({ section: `m8-disclosure-soe-${section}`, context, existingContent: existing })
    if (!text) { ElMessage.warning('AI 未生成内容，请稍后重试'); return }
    if (isSpecial) { soeSpecialNote.value = text; handleSoeSpecialNoteChange() }
    else { disclosureNote.value = text; handleNoteChange() }
  } catch { ElMessage.warning('AI 生成失败，请稍后重试') } finally { aiLoading.value = '' }
}
function handleReview(): void { openReviewDialog?.('M8-disclosure-soe', '附注披露（国企）') }
...
function _restoreData(): void {
  for (let i = 0; i < dataRows.value.length; i++) {
    ...
    for (const field of fields) {
      const resp = formData.allResponses.value.get(`M8-disclosure-soe-row-${i}-${field}`)
      if (resp?.remark) {
  ...
  // 恢复附注说明
  const noteResp = formData.allResponses.value.get('M8-disclosure-soe-note')
  if (noteResp?.remark) disclosureNote.value = noteResp.remark

  const specialResp = formData.allResponses.value.get('M8-disclosure-soe-special')
  if (specialResp?.remark) soeSpecialNote.value = specialResp.remark
}
</script>
