...
/**
 * M8 编制/使用手册弹窗 — 对齐 N5PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  <el-dialog
    ...
    title="M8 一般风险准备 · 编制手册"
    width="860px"
    top="4vh"
    destroy-on-close
    append-to-body
    class="m8-handbook-dialog"
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：仅金融企业适用。M8-2 明细(计提) + M8-4 计提测试(按风险资产期末余额 1.5%) → M8-1 审定(未审+AJE+RJE) → 回写 TB(4104)+附注。一般风险准备是权益类贷方：期末=期初+贷方(计提)−借方(转回/使用)；非金融企业标记不适用。本弹窗不改各表页内说明。"
    />
