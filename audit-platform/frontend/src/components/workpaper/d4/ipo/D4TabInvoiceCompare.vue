...
// ─── 双模式 sync bridge：统一 useD4SyncMode（治本收敛）── sheet_key=d423-managed。
...
async function handleImportClick() {
  ...
  input.onchange = async () => { const f = input.files?.[0]; if (f) await importData('D4-23', f) }
  input.click()
}
