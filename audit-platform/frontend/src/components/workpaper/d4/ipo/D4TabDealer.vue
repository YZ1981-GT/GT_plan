...
<div class="d4-dealer">
  ...
  <template v-if="editorMode !== '在线编辑'">
    ...
    <el-table :data="rows" border stripe class="dealer-table">
      ...
      <el-table-column label="终端销售金额" min-width="120" align="right"><template #default="{ row }"><WpAmountInput v-model="row.terminalSalesAmount" size="small" :disabled="isReadonly" style="width:100%" @change="updateCell(row.id,'terminalSalesAmount',row.terminalSalesAmount)" /></template></el-table-column>
