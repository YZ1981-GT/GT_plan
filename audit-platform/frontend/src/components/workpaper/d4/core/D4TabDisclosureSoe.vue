 ...
 *
 * 3子节卡片: (1)营业收入和营业成本 (2)分产品/分地区 (3)前五大客户
 * 同样结构，国企标准版本
 *
...
const costFromTb = computed(() => {
  const mainCost = parseNum(props.allResponses.get('D4-note-soe-cost-6401')?.remark)
  const otherCost = parseNum(props.allResponses.get('D4-note-soe-cost-6402')?.remark)
...
// ─── Section 2: 分产品/分地区（动态行） ──────────────────────────────────
interface ProductRegionRow {
  rowId: string
  dimension: string  // 产品名/地区名
  type: '产品' | '地区'
  currentAmount: number
  priorAmount: number
}

const SECTION2_KEY = 'D4-note-soe-section2-rows'

const section2Rows = ref<ProductRegionRow[]>([])
...
function addSection2Row(type: '产品' | '地区') {
...
const SECTION3_KEY = 'D4-note-soe-section3-rows'
...
const noteText = computed({
  get: () => props.allResponses.get('D4-note-soe-text')?.remark || '',
  set: (val: string) => {
    const map = props.allResponses as Map<string, any>
    map.set('D4-note-soe-text', { item_id: 'D4-note-soe-text', conclusion: null, remark: val })
    try {
      window.dispatchEvent(new CustomEvent('disclosure:note-text-updated', {
        detail: { wpCode: 'D4', section: 'soe', text: val },
      }))
    } catch { /* silent */ }
  },
})
...
<template>
  <div class="d4-tab-disclosure-soe">
    <!-- Section 1: 营业收入和营业成本 -->
    ...
    <el-card class="section-card" shadow="never">
      <template #header>
        <div class="section-header-row">
          <span class="section-title">(二) 分产品/分地区收入</span>
          <div class="header-actions">
            <el-button size="small" :disabled="isReadonly" @click="addSection2Row('产品')">+ 产品</el-button>
            <el-button size="small" :disabled="isReadonly" @click="addSection2Row('地区')">+ 地区</el-button>
          </div>
        </div>
      </template>
      <el-table :data="[...section2Rows, { rowId: 'total', dimension: '合计', type: '-', currentAmount: section2Total.currentAmount, priorAmount: section2Total.priorAmount }]" border size="small" style="width: 100%">
        <el-table-column label="维度" width="80">
          <template #default="{ row }">
            <el-tag v-if="row.rowId !== 'total'" size="small" :type="row.type === '产品' ? '' : 'success'">
              {{ row.type }}
            </el-tag>
          </template>
        </el-table-column>
          ...
          <template #default="{ row }">
            ...
            <template v-else>
              <el-input
                ...
                size="small"
                placeholder="产品/地区名称"
                @change="(v: string) => { row.dimension = v; persistSection2() }"
              />
    ...
    <div class="audit-note-section">
      <div class="note-header">
        ...
        <div class="note-actions">
          <el-button size="small" circle @click="openReviewDialog?.('D4-note-soe')">💬</el-button>
        </div>
      </div>
      <el-input
        v-model="noteText"
