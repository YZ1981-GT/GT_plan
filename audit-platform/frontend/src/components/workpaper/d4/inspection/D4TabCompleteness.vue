...
// ─── 双模式 sync bridge（B2：D4-15 迁 useWorkpaperSyncBridge + WorkpaperSyncEditorHost）──
// sheet_key=d4-15-managed，同 entry gt-d4-operating-revenue（后端 phase5_d4_inspection_sheets provider）。
const D4_15_ENTRY = 'xlsx/gt-d4-operating-revenue'
const D4_15_SHEET_KEY = 'd4-15-managed'
...
const syncBridge = useWorkpaperSyncBridge({
  entryId: ref(D4_15_ENTRY),
  wpId: toRef(props, 'wpId'),
  projectId: toRef(props, 'projectId'),
  sheetKey: ref(D4_15_SHEET_KEY),
  capability: capabilityForEntry(D4_15_ENTRY),
  flushHtml: async () => {
    flushPendingSave()
    const snap = await readStoreProjection({ projectId: props.projectId, wpId: props.wpId, entryId: D4_15_ENTRY })
    return { expectedRevision: snap.expectedRevision, projection: snap.projection, sheetKey: D4_15_SHEET_KEY }
  },
  reloadHtml: async () => { loadItems() },
})
...
<template>
  <div class="d4-completeness">
    ...
    <!-- ═══ 在线编辑：平台 sync bridge（B2 迁移，非裸 GtOnlyOfficeSheet）═══ -->
    <template v-if="editorMode === '在线编辑'">
      <div class="oo-container">
        ...
        <div v-else class="oo-loading">正在打开 D4-15 同步编辑器…</div>
      </div>
    </template>
