...
function checkCutoff(row: CutoffForwardRow): boolean | null {
  // D4-17（账到单据）：早侧=凭证，晚侧=发货单。单一真源 useD4FormulaEngine.isCutoffOk。
  return isCutoffOk(row.voucherDate, row.deliveryDate, cutoffDate.value)
}
...
function loadNoteConclusion() {
  ...
  const cd = props.allResponses.get('D4-17-cutoff-date')?.remark
...
function openConfirmDialog() {
  confirmDrafts.value = rows.value
    ...
    .map(r => ({
      sourceId: r.id,
      description: `截止跨期（凭证${r.voucherDate}/发货${r.deliveryDate}·${r.voucherNo || '-'}）${r.voucherProduct || ''}`.trim(),
      // D4-17 账到单据：本期确认收入而单据在期后 → 收入多计 → 贷方
      direction: 'credit' as const,
      amount: Number(r.voucherAmount || 0),
      evidence: r.remark || '',
...
function persistAll() {
  ...
  props.allResponses.set('D4-17-cutoff-date', { item_id: 'D4-17-cutoff-date', conclusion: null, remark: cutoffDate.value })
  debounceSave()
}
...
function flushSave() {
  const keys = ['D4-17-rows', 'D4-17-note', 'D4-17-conclusion', 'D4-17-cutoff-date']
...
async function handleImportFile(uploadFile: any) {
  const res = await importData('D4-17', uploadFile.raw || uploadFile)
...
<template>
  <div class="d4-cutoff-forward">
    ...
    <template v-if="editorMode !== '在线编辑'">
      ...
      <!-- 发现→人工确认门（跨期发现不自动造错报，需人工确认方向/金额/证据） -->
      <el-alert v-if="cutoffIssues > 0" type="warning" :closable="false" show-icon style="margin-bottom:12px">
        <template #title>
          <span>发现 {{ cutoffIssues }} 笔跨期问题（本期确认收入而发货单在期后＝收入多计错报）。风险发现≠错报，请人工确认方向/金额/证据后推送。</span>
    ...
    <el-dialog v-model="confirmDialogVisible" title="确认跨期错报并推送至 A13" width="720px" destroy-on-close>
      <el-alert type="info" :closable="false" show-icon style="margin-bottom:12px">
        <template #title>只有勾选且方向/金额/证据齐全的行才会形成错报请求。金额为 0 或证据缺失的行无法确认（错报汇总必须有金额与证据）。</template>
      </el-alert>
        ...
        <el-table-column label="方向" width="110" align="center">
          <template #default="{ row }">
            <el-select v-model="row.direction" size="small" placeholder="选择">
              ...
              <el-option label="贷（多计收入）" value="credit" />
            </el-select>
          </template>
        </el-table-column>
