...
// 双向回写 flushHtml/reloadHtml 用（先落库再 readStoreProjection，主控 §4.1）。
function flushPendingSave() { if (debounceTimer) { clearTimeout(debounceTimer); debounceTimer = null } flushSave() }
function reloadD417() { loadData(); loadNoteConclusion() }
onBeforeUnmount(() => { if (debounceTimer) { clearTimeout(debounceTimer); flushSave() } })

// ─── 双模式 sync bridge（批次B 第二张：D4-17 从 legacy GtOnlyOfficeSheet 迁 ─────────
//     useWorkpaperSyncBridge + WorkpaperSyncEditorHost；sheet_key=d417-managed，
//     同 entry gt-d4-operating-revenue（后端 phase5_d4_cutoff_forward_sheet provider）。
//     宿主 GtD4OperatingRevenue 须登记 'D4-17'。
const D4_17_ENTRY = 'xlsx/gt-d4-operating-revenue'
const D4_17_SHEET_KEY = 'd417-managed'
...
const syncBridge = useWorkpaperSyncBridge({
  entryId: ref(D4_17_ENTRY),
  wpId: toRef(props, 'wpId'),
  projectId: toRef(props, 'projectId'),
  sheetKey: ref(D4_17_SHEET_KEY),
  capability: capabilityForEntry(D4_17_ENTRY),
  flushHtml: async () => {
    flushPendingSave()
    const snap = await readStoreProjection({ projectId: props.projectId, wpId: props.wpId, entryId: D4_17_ENTRY })
    return { expectedRevision: snap.expectedRevision, projection: snap.projection, sheetKey: D4_17_SHEET_KEY }
  },
  reloadHtml: async () => { reloadD417() },
})
...
<template>
  <div class="d4-cutoff-forward">
    ...
    <template v-if="editorMode === '在线编辑'">
      <div class="oo-container"><WorkpaperSyncEditorHost v-if="syncOoDescriptor" ref="syncHostRef" :descriptor="syncOoDescriptor" :bridge="syncBridge" /><div v-else class="oo-loading">正在打开 D4-17 同步编辑器…</div></div>
    </template>
