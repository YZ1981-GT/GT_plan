...
/**
 * D4TabCutoffBackward — D4-18 营业收入截止测试（单据到账）
 *
 * 从发货单→记账凭证方向检查期末发货是否已记入恰当期间
 * 双模式：表格视图 / 在线编辑
...
import { isCrossPeriodBackward } from '../../composables/useD4FormulaEngine'
...
// ─── State ───────────────────────────────────────────────────────────
const rows = ref<CutoffBackwardRow[]>([])
const auditNote = ref('')
...
function checkCutoff(row: CutoffBackwardRow): boolean | null {
  ...
  // D4-18 单据到账方向：单据在期内、凭证在期后 → 跨期问题(×)。走公式引擎单一真源。
  // isCutoff=true 表示「无跨期问题(√)」= NOT isCrossPeriodBackward(单据, 凭证, 截止日)。
  return !isCrossPeriodBackward(row.deliveryDate, row.voucherDate, cutoffDate.value)
}
...
// ─── Load ────────────────────────────────────────────────────────────
function loadData() {
  const resp = props.allResponses.get('D4-18-rows')
  if (resp?.remark) {
...
watch(() => props.allResponses.get('D4-18-note')?.remark, () => loadNoteConclusion(), { immediate: true })

// ─── CRUD ────────────────────────────────────────────────────────────
function addRow() {
  if (props.isReadonly) return
  rows.value.push({ id: `r-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`, deliveryDate: '', deliveryNo: '', deliveryProduct: '', deliveryQty: '', deliveryAmount: 0, voucherDate: '', voucherNo: '', voucherProduct: '', voucherQty: '', voucherAmount: 0, isCutoff: null, remark: '' })
  persistAll()
}
function removeRow(id: string) { if (props.isReadonly) return; rows.value = rows.value.filter(r => r.id !== id); persistAll() }
function onCellChange(row: CutoffBackwardRow) { row.isCutoff = checkCutoff(row); persistAll() }
...
function handlePushToA13() {
  ...
  const items = rows.value
    ...
    .map(r => ({
      ...
      amount: r.deliveryAmount || 0,
      description: `单据到账截止跨期：发货单${r.deliveryDate || ''}／凭证${r.voucherDate || ''}${r.remark ? `（${r.remark}）` : ''}`,
      indexRef: 'D4-18',
    }))
...
function reloadD418() { loadData(); loadNoteConclusion() }
onBeforeUnmount(() => { if (debounceTimer) { clearTimeout(debounceTimer); flushSave() } })

// ─── 双模式 sync bridge（批次B 第三张：D4-18 迁 useWorkpaperSyncBridge，sheet_key=d418-managed）──
const D4_18_ENTRY = 'xlsx/gt-d4-operating-revenue'
const D4_18_SHEET_KEY = 'd418-managed'
...
const syncBridge = useWorkpaperSyncBridge({
  entryId: ref(D4_18_ENTRY),
  wpId: toRef(props, 'wpId'),
  projectId: toRef(props, 'projectId'),
  sheetKey: ref(D4_18_SHEET_KEY),
  capability: capabilityForEntry(D4_18_ENTRY),
  flushHtml: async () => {
    flushPendingSave()
    const snap = await readStoreProjection({ projectId: props.projectId, wpId: props.wpId, entryId: D4_18_ENTRY })
    return { expectedRevision: snap.expectedRevision, projection: snap.projection, sheetKey: D4_18_SHEET_KEY }
  },
  reloadHtml: async () => { reloadD418() },
})
...
async function genNote() {
  ...
  try {
    const res = await http.post(`/api/workpapers/${props.wpId}/d4/ai-generate`, { section: 'adj-note', existingContent: auditNote.value, relatedContext: { task: '基于截止测试（单据到账）结果，生成审计说明', totalRows: totalRows.value, cutoffIssues: cutoffIssues.value, totalAmount: totalAmount.value, cutoffDate: cutoffDate.value } }, { _silent: true } as any)
...
async function genConclusion() {
  ...
  try {
    const res = await http.post(`/api/workpapers/${props.wpId}/d4/ai-generate`, { section: 'adj-conclusion', existingContent: auditConclusion.value, relatedContext: { task: '基于截止测试（单据到账）结果生成审计结论', noteText: auditNote.value, cutoffIssues: cutoffIssues.value, totalRows: totalRows.value } }, { _silent: true } as any)
    const text = res.data?.data?.content ?? res.data?.content ?? ''
    if (!text) { ElMessage.warning('AI 未生成内容'); return }
  ...
  <div class="d4-cutoff-backward">
    ...
    <div class="toolbar">
      ...
      <div class="toolbar-right">
        ...
        <el-button v-if="openReviewDialog" size="small" @click="openReviewDialog('D4-18-cutoff')">💬 复核</el-button>
      </div>
    </div>
    ...
    <template v-if="editorMode !== '在线编辑'">
      ...
      <details class="methodology-collapse">
        ...
        <div class="methodology-body">
          ...
          <p><strong>审计过程：</strong>获取报表日前后一定天数、且金额大于设定阈值的发货单，与收入记账凭证核对确认收入是否记入了恰当的期间。</p>
        </div>
      </details>
      ...
      <div class="guide-strip">
        ...
        <el-tooltip content="从发货记录中选取期末附近的发货单" placement="bottom" :show-after="300"><span class="guide-chip">②选取期末发货单</span></el-tooltip>
        <span class="guide-arrow">→</span>
        <el-tooltip content="根据发货单追查对应的记账凭证信息" placement="bottom" :show-after="300"><span class="guide-chip">③追查记账凭证</span></el-tooltip>
        <span class="guide-arrow">→</span>
        <el-tooltip content="比较发货日期与凭证日期判断是否跨期" placement="bottom" :show-after="300"><span class="guide-chip">④判断跨期</span></el-tooltip>
        <span class="guide-arrow">→</span>
      ...
      <el-table :data="rows" border stripe class="cutoff-table" :row-class-name="rowClassName">
        ...
        <el-table-column label="发货单" align="center" class-name="col-delivery">
          ...
          <el-table-column label="金额" min-width="110" align="right">
            <template #default="{ row }"><WpAmountInput v-model="row.deliveryAmount" size="small" :disabled="isReadonly" style="width:100%" @change="onCellChange(row)" /></template>
          </el-table-column>
        </el-table-column>
        ...
        <el-table-column label="记账凭证" align="center" class-name="col-voucher">
          ...
          <el-table-column label="金额" min-width="110" align="right">
            <template #default="{ row }"><WpAmountInput v-model="row.voucherAmount" size="small" :disabled="isReadonly" style="width:100%" @change="persistAll()" /></template>
          </el-table-column>
        </el-table-column>
      ...
      <details class="tips-collapse">
        ...
        <ol class="tips-list">
          <li>本表适用于对"以发货单为确认收入时点"的被审计单位的截止测试（单据到账方向）。如被审计单位以验收单或签收单为收入确认时点，应以相应单据为检查对象。</li>
          <li>如果存在跨期舞弊风险或者所检查样本中发现有跨期的，应扩大测试期间和样本量，将检查结果与D4-17（账到单据方向）综合评价。</li>
        </ol>
      </details>
    </template>
    ...
    <template v-if="editorMode === '在线编辑'">
      <div class="oo-container"><WorkpaperSyncEditorHost v-if="syncOoDescriptor" ref="syncHostRef" :descriptor="syncOoDescriptor" :bridge="syncBridge" /><div v-else class="oo-loading">正在打开 D4-18 同步编辑器…</div></div>
    </template>
...
<style scoped>
.d4-cutoff-backward { padding: 16px 20px; font-size: var(--wp-font-size, 13px); }
