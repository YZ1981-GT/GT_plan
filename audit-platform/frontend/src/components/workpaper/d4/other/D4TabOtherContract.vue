  ...
  <template v-if="editorMode !== '在线编辑'">
    ...
    <div class="block-section rental-block">
      ...
      <el-table :data="rentals" border stripe size="small" class="contract-table" :row-class-name="rowClass">
        ...
        <el-table-column label="本期应计收入" min-width="100" align="right"><template #default="{ row }"><el-input-number v-model="row.expectedRevenue" size="small" :controls="false" :disabled="isReadonly" style="width:100%" @change="updateRental(row.id,'expectedRevenue',row.expectedRevenue)" /></template></el-table-column>
        <el-table-column label="本期实计收入" min-width="100" align="right"><template #default="{ row }"><el-input-number v-model="row.actualRevenue" size="small" :controls="false" :disabled="isReadonly" style="width:100%" @change="updateRental(row.id,'actualRevenue',row.actualRevenue)" /></template></el-table-column>
    ...
    <div class="block-section consult-block">
      ...
      <el-table :data="consults" border stripe size="small" class="contract-table" :row-class-name="rowClass">
        ...
        <el-table-column label="合同金额" min-width="100" align="right"><template #default="{ row }"><el-input-number v-model="row.contractAmount" size="small" :controls="false" :disabled="isReadonly" style="width:100%" @change="updateConsult(row.id,'contractAmount',row.contractAmount)" /></template></el-table-column>
        ...
        <el-table-column label="本期应计收入" min-width="100" align="right"><template #default="{ row }"><el-input-number v-model="row.expectedRevenue" size="small" :controls="false" :disabled="isReadonly" style="width:100%" @change="updateConsult(row.id,'expectedRevenue',row.expectedRevenue)" /></template></el-table-column>
        <el-table-column label="本期实计收入" min-width="100" align="right"><template #default="{ row }"><el-input-number v-model="row.actualRevenue" size="small" :controls="false" :disabled="isReadonly" style="width:100%" @change="updateConsult(row.id,'actualRevenue',row.actualRevenue)" /></template></el-table-column>
        <el-table-column label="差异" min-width="80" align="right"><template #default="{ row }"><span class="auto-calc" :class="{ 'diff-warn': row.diff !== 0 }">{{ fmtAmt(row.diff) }}</span></template></el-table-column>
