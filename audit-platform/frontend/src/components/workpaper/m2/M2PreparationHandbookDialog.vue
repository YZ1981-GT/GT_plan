...
/**
 * M2 编制/使用手册弹窗 — 对齐 N5PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  <el-dialog
    ...
    title="M2 实收资本（股本） · 编制手册"
    width="860px"
    top="4vh"
    destroy-on-close
    append-to-body
    class="m2-handbook-dialog"
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：M2-2 明细(上市/非上市，按出资人) + M2-4 外币折算 + M2-5 验资/工商/章程核对 → M2-1 审定(未审+AJE+RJE) → 回写 TB(4001)+附注。实收资本是权益类贷方：期末=期初+贷方(增资)−借方(减资)；外币折算差异计入 M4 资本公积。本弹窗不改各表页内说明。"
    />
