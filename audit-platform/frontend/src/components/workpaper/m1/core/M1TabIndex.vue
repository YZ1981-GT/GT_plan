  ...
  <div class="m1-tab-index">
    ...
    <div class="m1-dir">
      <div class="index-header">
        <h3 class="title">M1 底稿目录</h3>
        <GtReviewTrigger section-id="M1-index-directory" />
      ...
      <M1PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="m1-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>应付股利为<strong>负债类贷方科目</strong>（2232）：期末 = 期初 + 贷方（宣告分配）− 借方（实际支付），是 M 循环唯一负债类科目</li>
          <li>明细表 M1-2 按股东列示各项应付股利，与审定表 M1-1 合计交叉核对</li>
          <li>M1-4 外币汇率测算：对境外股东应付股利按期末汇率折算本位币</li>
          <li>M1-5 应付股利测算：接收 M6 利润分配的分配股利，验证宣告准确性</li>
          <li>M1-6 检查：宣告决议依据、实际支付、代扣代缴所得税合规</li>
          <li>审定合计回写 TB(2232) 并驱动附注披露；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="m1-cycle">
...
/**
 * M1TabIndex.vue — M1 应付股利（利润）底稿目录（严格镜像 N5 标准）
 *
 ...
 *
 * 负债类贷方科目（2232 应付股利）— 期末=期初+贷方−借方；按股东列示、外币折算、核对 M6。
 */
...
const M1PreparationHandbookDialog = defineAsyncComponent(() => import('../M1PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { name: '应付股利实质性程序表', code: 'M1A', sheetKey: '应付股利实质性程序表M1', progress: calcSheetProgress('M1-M1A-', 5) },
  { name: '审定表', code: 'M1-1', sheetKey: '审定表M1-1', progress: calcSheetProgress('M1-1-', 6) },
  { name: '明细表', code: 'M1-2', sheetKey: '明细表M1-2', progress: calcSheetProgress('M1-2-', 4) },
  { name: '调整分录汇总', code: 'M1-3', sheetKey: '调整分录汇总M1-3', progress: calcSheetProgress('M1-3-', 4) },
  { name: '外币汇率测算表', code: 'M1-4', sheetKey: '外币汇率测算表M1-4', progress: calcSheetProgress('M1-4-', 4) },
  { name: '应付股利（利润）测算表', code: 'M1-5', sheetKey: '应付股利（利润）测算表M1-5', progress: calcSheetProgress('M1-5-', 4) },
  { name: '应付股利（利润）检查表', code: 'M1-6', sheetKey: '应付股利（利润）检查表M1-6', progress: calcSheetProgress('M1-6-', 4) },
  { name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', progress: calcSheetProgress('M1-disclosure-listed-', 3) },
  { name: '附注披露信息（国有企业）', code: '附注国企', sheetKey: '附注披露信息（国有企业）', progress: calcSheetProgress('M1-disclosure-soe-', 3) },
])
...
const M1_CONCLUSION_CODES = new Set(['M1-1', 'M1-2', 'M1-4', 'M1-5', 'M1-6'])
...
function isConclusionFilled(code: string): boolean {
  ...
  const token = `${code}-`
  const longer = Array.from(M1_CONCLUSION_CODES).filter(c => c !== code && c.startsWith(token))
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => M1_CONCLUSION_CODES.has(s.code))
