...
/**
 * M1 编制/使用手册弹窗 — 对齐 N5PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  <el-dialog
    ...
    title="M1 应付股利（利润） · 编制手册"
    width="860px"
    top="4vh"
    destroy-on-close
    append-to-body
    class="m1-handbook-dialog"
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：M1-2 明细(按股东) + M1-4 外币折算 + M1-5 核对 M6 利润分配宣告 → M1-6 检查 → M1-1 审定(未审+AJE+RJE) → 回写 TB(2232)+附注。应付股利是 M 循环唯一负债类科目：期末=期初+贷方(宣告)−借方(支付)。本弹窗不改各表页内说明。"
    />
