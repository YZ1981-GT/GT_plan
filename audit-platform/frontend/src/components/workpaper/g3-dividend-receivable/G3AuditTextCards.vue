...
/**
 * G3 审计说明 / 结论卡片（含 AI）— 各子表统一复用，对齐 G2
 */
import { computed } from 'vue'
import { useG3AiGenerate, type G3AiSection } from '../composables/useG3AiGenerate'
...
const props = withDefaults(defineProps<{
  ...
  noteAiSection?: G3AiSection | ''
  conclusionAiSection?: G3AiSection | ''
...
const { aiAvailable, loading: aiLoading, generateAndConfirm } = useG3AiGenerate(wpIdRef)
...
<style scoped>
.g3-audit-text-cards { display: flex; flex-direction: column; gap: 0; }
.audit-text-card {
  margin-top: 14px;
