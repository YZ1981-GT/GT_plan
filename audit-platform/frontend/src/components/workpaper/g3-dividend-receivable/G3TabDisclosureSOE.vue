<!--
  ...
  合计数交叉索引：Excel M1-1 → K1-1 / Note:八、9
  Subscribe: substantive:adjudicated(1131)
  Publish: disclosure:note-text-updated；同步至附注八、9
-->
<template>
  <div class="g3-disclosure-soe" data-testid="g3-disclosure-soe">
    <div class="section-head">
      ...
      <div class="head-actions">
        ...
        <el-tag size="small" type="info">八、9 · 应收股利</el-tag>
        <el-button
          size="small"
          type="primary"
          plain
        ...
        <el-button v-if="openReviewDialog" size="small" @click="openReviewDialog('G3-disclosure-soe')">💬复核</el-button>
        <GtIndexChip value="wp:G3-note-soe" :context-project-id="projectId" />
    ...
    <el-alert
      ...
      title="审计目标：复核国企应收股利附注披露的完整性与准确性，确保与审定数勾稽一致，并回写附注八、9（其他应收款下应收股利明细）。"
    />
    ...
    <G3AuditTextCards
      ...
      :related-context="{ 科目: '1131', 披露类型: '国企', 表格行数: tableRows.length }"
      note-placeholder="国企应收股利附注：按被投资方列示余额，并说明与审定数及合计数（K1-1 / 八、9）勾稽…"
      note-hint="保存后发布 disclosure:note-text-updated；「同步至附注」写入八、9。合计数见 M1-1→K1-1。"
      :note-min-rows="6"
    />
    ...
    <details class="guidance-details">
      ...
      <div class="guidance-content">
        <p>1. 国企格式：被投资方 | 期初 | 本期变动 | 期末（= 期初 + 变动）。</p>
        <p>2. Excel「详见 M1-1」为合计数旧索引，平台跳转 <strong>K1-1</strong> 与附注 <strong>八、9</strong>。</p>
        <p>3. 监听 substantive:adjudicated(1131)；「同步至附注」写入八、9 应收股利子表。</p>
...
const combinedNoteChip = computed(() => noteTarget.value?.combinedNoteChip ?? `Note:${G3_NOTE_SECTION.soe}`)
...
function updateTableRow(id: string, field: keyof DisclosureSOERow, value: string | number) {
  if (props.isReadonly) return
...
function syncFromAdjudication(manual: boolean): void {
  ...
  const seeded = buildSoeDisclosureFromAdj(store)
  if (!seeded.length) {
