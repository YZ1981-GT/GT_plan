...
2. **G3-3 是调整枢纽**——列结构对齐 D4/Excel；借贷平衡后点**确认调整**，发布至调整分录模块并将净额落入 G3-1「账项调整汇总」。  
...
flowchart TB
  subgraph hub [金额枢纽]
    ...
    MOD[调整分录模块]
    G32 -->|对照未审| G31
    G33 -->|确认调整 AJE/RJE| G31
    G33 <-->|双向同步 1131/6111| MOD
...
| **G3-3** | 调整分录汇总（对齐 D4） | 列：调整事项说明/类别/报表项目/科目/附注/借方/贷方/索引/备注；借贷平衡后「确认调整」→ 发布调整分录模块 + 回写 G3-1 期末 AJE/RJE；可从模块同步 1131/6111 |
...
1. **只录 G3-3 不点确认**——不会写入 G3-1 AJE/RJE，也不会发布至调整分录模块。  
2. **科目编码未写 1131（且科目名称不含「应收股利」）**——确认回写时净额为 0。  
