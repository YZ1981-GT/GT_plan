  ...
  <div class="k9-cutoff-s2v">
    ...
    <el-alert v-if="adjustableRows.length > 0" type="warning" :closable="false" show-icon style="margin-bottom:12px">
      <template #title>
        <div class="cross-alert">
          <span>发现 {{ adjustableRows.length }} 笔跨期/需调整凭证（费用漏记或滞后入账，可能构成本期费用少计错报）</span>
    ...
    <details class="compile-hint">
      ...
      <ul>
        ...
        <li>跨期（费用漏记/滞后）可一键「推送跨期至 A13 错报」，纳入错报汇总</li>
...
// ═══ 跨期 → A13 错报联动（完整性方向：漏记/滞后入账 = 本期费用少计错报） ═══
  ...
  try {
    eventBus.emit('a13:push-misstatement' as any, {
      ...
      items: rows.map((r) => ({
        ...
        description: `截止漏记/滞后（原始${r.sourceDate || '-'}/记账${r.bookDate || '-'}）${r.summary || '管理费用完整性异常'}`.trim(),
        indexRef: 'K9-7',
      })),
      timestamp: Date.now(),
    })
...
async function handleAiAssist(): Promise<void> {
  ...
  try {
    const content = await generateK9AiText(props.wpId, {
      prompt: '请根据管理费用反向截止测试（原始凭证→记账凭证）结果，生成截止测试结论（概述抽样范围、是否存在应记未记/滞后入账及完整性判断）。',
      section: 'K9-7-cutoff-conclusion',
      context: {
        科目: '6602 管理费用',
