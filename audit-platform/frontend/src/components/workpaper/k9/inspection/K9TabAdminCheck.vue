<template>
  <div class="k9-admin-check">
    <!-- 审计目标（认定） -->
    <el-alert type="info" :closable="false" style="margin-bottom:12px">
      <template #title><span style="font-weight:600">审计目标（认定）</span></template>
      <ol style="margin:4px 0 0;padding-left:18px;line-height:1.55;font-size:12px">
        <li><b>发生：</b>记录的管理费用确已发生且与本期经营相关；</li>
        <li><b>完整性与准确性：</b>管理费用记录完整、金额准确，无虚列或漏记；</li>
    ...
    <div class="section-header">
      <h3>K9-8 管理费用综合检查表</h3>
      <div class="header-actions">
        <el-button size="small" type="primary" text @click="handleAiAssist"><el-icon><MagicStick /></el-icon> AI辅助</el-button>
        <el-button size="small" text @click="openReviewDialog?.('K9-8-admin-check')">💬 复核</el-button>
      </div>
    </div>

    <!-- ═══ 方法论上下文（琥珀色块） ═══ -->
    <div class="methodology-context">
      <p>逐项检查管理费用相关控制合规性，每项判断"合规/不合规/不适用"。不合规项需填写审计证据并红色标记。覆盖费用归属期间、分类正确性、关联方交易、大额异常支出、税前扣除限额、研发费用划分等。</p>
    </div>

    <!-- ═══ 不合规摘要 ═══ -->
    <el-alert v-if="adminNonComplianceCount > 0" type="error" :closable="false" show-icon style="margin-bottom:10px">
      <template #title>⚠️ 发现 {{ adminNonComplianceCount }} 项不合规</template>
    </el-alert>

    <!-- ═══ 综合检查表格 ═══ -->
    <el-table :data="adminCheckRows" border size="small" style="width:100%;font-size:13px" max-height="450" :row-class-name="adminRowClassName">
      ...
      <el-table-column label="合规判断" width="130" align="center">
        <template #default="{ row }">
          <el-radio-group v-if="!isReadonly" :model-value="row.compliance" size="small" @change="(v: string) => updateAdminCheckCompliance(row.rowKey, v as any)">
      ...
      <el-table-column prop="evidence" label="证据/说明" min-width="160">
        <template #default="{ row }">
          <el-input v-if="!isReadonly" :model-value="row.evidence" size="small" placeholder="审计证据" @change="(v: string) => updateAdminCheckEvidence(row.rowKey, v)" />
          <span v-else>{{ row.evidence || '-' }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="remark" label="备注" min-width="100">
        <template #default="{ row }">
          <el-input v-if="!isReadonly" :model-value="row.remark" size="small" @change="(v: string) => updateAdminCheckCell(row.rowKey, 'remark', v)" />
    ...
    <div class="check-summary">
      ...
      <span class="summary-item"><el-tag type="danger" size="small">不合规</el-tag> {{ adminNonComplianceCount }} 项</span>
    ...
    <el-card shadow="never" class="conclusion-card">
      ...
      <el-input
        :model-value="adminConclusion"
        type="textarea"
        :autosize="{ minRows: 2, maxRows: 6 }"
        :disabled="isReadonly"
        placeholder="请填写管理费用综合检查结论..."
        @blur="(e: FocusEvent) => saveAdminConclusion((e.target as HTMLTextAreaElement)?.value ?? '')"
      />
    </el-card>
    ...
    <details class="compile-hint">
      ...
      <ul>
        ...
        <li>覆盖：费用期间归属、分类正确性、关联方、大额异常、政策一致性、税前扣除限额、凭证完整性、审批流程、研发费用划分、折旧摊销合理性</li>
        <li>检查项来源：审计程序表K9A中确定的检查要点</li>
      </ul>
    </details>
  </div>
</template>
...
/**
 * K9TabAdminCheck.vue — K9-8 管理费用综合检查表
 *
...
import type { K9AdminCheckRow, ComplianceState } from '@/components/workpaper/composables/useK9Checks'
...
// ═══ 统计 ═══
const adminNonComplianceCount = computed(() => adminCheckRows.value.filter(r => r.compliance === '不合规').length)
const complianceCount = computed(() => adminCheckRows.value.filter(r => r.compliance === '合规').length)
const naCount = computed(() => adminCheckRows.value.filter(r => r.compliance === '不适用').length)
const pendingCount = computed(() => adminCheckRows.value.filter(r => !r.compliance).length)

// ═══ UI Helpers ═══
function adminRowClassName({ row }: { row: K9AdminCheckRow }): string {
...
<style scoped>
.k9-admin-check { padding: 12px; font-size: var(--wp-font-size, 13px); }
