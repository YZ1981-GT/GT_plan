<!--
  K9TabAdminCheck.vue — K9-8 管理费用检查表（凭证级测试）
...
<template>
  <div class="k9-admin-check">
    ...
    <div class="section-head">
      <h3 class="sheet-title">K9-8 管理费用检查表</h3>
    ...
    <div class="methodology-context">
      <p>管理费用（6602）为<strong>借方/损益类</strong>科目，取本期发生额（借方累计）。测试总体一般<strong>排除</strong>：结转损益、职工薪酬/折旧计提、无形资产及长期费用摊销、各项税费计提等凭证。检查要点：①原始凭证齐全（付款审批单/费用报销单/支出凭单/银行回单/合同）②记账凭证与原始凭证相符 ③账务处理正确 ④记录于恰当会计期间 ⑤费用分类正确。</p>
    </div>
    ...
    <el-card shadow="never" class="section-card">
      <template #header>
        <div class="card-header-row">
          <span class="card-title">三、测试 — 管理费用凭证检查</span>
          <div>
...
// ─── 列设置（宽表功能，参照 D4）─────────────────────────────────────────────
const COLUMN_PREFS_KEY = 'K9-8-column-prefs'
...
<style scoped>
.k9-admin-check { padding: 12px 14px; font-size: var(--wp-font-size, 13px); }
