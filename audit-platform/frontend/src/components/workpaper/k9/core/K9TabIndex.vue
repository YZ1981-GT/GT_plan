<template>
  <div class="k9-tab-index">
    ...
    <!-- ═══ 蓝色渐变操作引导区 ═══ -->
    <div class="k9-guide">
      <div class="k9-guide-header">
        <el-icon><InfoFilled /></el-icon>
        <span>操作步骤引导</span>
      </div>
      <div class="k9-guide-steps">
        <div class="step-item">
          <span class="step-num">①</span>
          <span class="step-text">填写明细表（K9-2）登记各项管理费用明细科目发生额</span>
        </div>
        <div class="step-item">
          <span class="step-num">②</span>
          <span class="step-text">完成实质性分析（K9-4）识别同比/环比异常波动</span>
        </div>
        <div class="step-item">
          <span class="step-num">③</span>
          <span class="step-text">执行截止测试（K9-6/K9-7）验证期末费用截止正确</span>
        </div>
        <div class="step-item">
          <span class="step-num">④</span>
          <span class="step-text">填写审定表（K9-1）确认发生额 → TB回写6602</span>
        </div>
        <div class="step-item">
          <span class="step-num">⑤</span>
          <span class="step-text">完成合同检查（K9-5）+ 综合检查（K9-8）→ 审计结论</span>
        </div>
        <div class="step-item">
          <span class="step-num">⑥</span>
    ...
    <!-- ═══ 跨底稿引用 ═══ -->
    <div class="k9-cross-refs">
      <span class="cross-refs-label">关联底稿：</span>
      <GtIndexChip value="K9A" :context-project-id="props.projectId" />
      <GtIndexChip value="K8" :context-project-id="props.projectId" />
      <GtIndexChip value="J1" :context-project-id="props.projectId" />
      <GtIndexChip value="A13" :context-project-id="props.projectId" />
    </div>

    <!-- ═══ 核心组（审定/明细/调整/程序表） ═══ -->
    <el-card shadow="never" class="k9-group-card group-core">
      <template #header>
    ...
    <!-- ═══ 分析与截止测试组 ═══ -->
    <el-card shadow="never" class="k9-group-card group-analysis">
      <template #header>
    ...
    <!-- ═══ 检查组 ═══ -->
    <el-card shadow="never" class="k9-group-card group-inspection">
      <template #header>
    ...
    <!-- ═══ 附注组 ═══ -->
    <el-card shadow="never" class="k9-group-card group-disclosure">
      <template #header>
    ...
    <details class="k9-details-tip">
      ...
      <ul>
        <li>管理费用为<strong>损益类借方科目</strong>（6602）：取<strong>发生额</strong>非期末余额！</li>
        ...
        <li>关联底稿：K8销售费用(同类损益)、J1职工薪酬(薪酬分配)、A13错报汇总</li>
      </ul>
    </details>
  </div>
</template>
...
/**
 * K9TabIndex.vue — K9 管理费用底稿目录（12 sheet进度表）
 *
 * 12个功能sheet分4组（核心/分析与截止/检查/附注）展示进度。
 * 点击行 emit navigate-sheet 事件切换 sheetName。
 * GtIndexChip 跨底稿跳转（K9A/K8/J1/A13）
 *
 * 损益类科目（6602管理费用）— 取发生额非余额！
 *
 * Spec: .kiro/specs/k9-admin-expenses/ | Task: 4.1
 * Requirements: 1.2
 */
...
/**
 * 12 sheets（从k9_structure_summary.json）：
 * 1. 底稿目录 (K9) — 本页面
 * 2. 实质性程序表K9A
 * 3. 审定表K9-1
 * 4. 附注披露信息（上市公司）
 ...
 * 8. 实质性分析K9-4
 * 9. 合同检查表K9-5
 * 10. 截止性测试(从记账凭证至原始凭证)K9-6
 * 11. 截止性测试(从原始凭证至记账凭证)K9-7
 * 12. 管理费用检查表K9-8
 */
const allSheets = computed<SheetRow[]>(() => [
  // ─── 核心组 ───
  { seq: 1, name: '实质性程序表', code: 'K9A', sheetKey: '实质性程序表K9A', description: '管理费用实质性程序清单与执行情况', group: 'core', progress: calcSheetProgress('K9A-', 5) },
  { seq: 2, name: '审定表', code: 'K9-1', sheetKey: '审定表K9-1', description: '损益类6602审定（发生额！73公式）+TB回写', group: 'core', progress: calcSheetProgress('K9-1-', 10) },
  { seq: 3, name: '明细表', code: 'K9-2', sheetKey: '明细表K9-2', description: '25列3区段+费用明细科目发生额+55行', group: 'core', progress: calcSheetProgress('K9-2-', 8) },
  { seq: 4, name: '调整分录汇总', code: 'K9-3', sheetKey: '调整分录汇总K9-3', description: 'AJE/RJE管理（借贷平衡）→联动A13', group: 'core', progress: calcSheetProgress('K9-3-', 4) },
  // ─── 分析与截止组 ───
  { seq: 5, name: '实质性分析', code: 'K9-4', sheetKey: '实质性分析K9-4', description: '同比/环比/占收入比/异常波动识别（16公式）', group: 'analysis', progress: calcSheetProgress('K9-4-', 8) },
  { seq: 6, name: '截止性测试（记账→原始）', code: 'K9-6', sheetKey: '截止性测试(从记账凭证至原始凭证)K9-6', description: '从记账凭证到原始凭证方向（存在认定）', group: 'analysis', progress: calcSheetProgress('K9-6-', 6) },
  { seq: 7, name: '截止性测试（原始→记账）', code: 'K9-7', sheetKey: '截止性测试(从原始凭证至记账凭证)K9-7', description: '从原始凭证到记账凭证方向（完整性认定）', group: 'analysis', progress: calcSheetProgress('K9-7-', 6) },
  // ─── 检查组 ───
  { seq: 8, name: '合同检查表', code: 'K9-5', sheetKey: '合同检查表K9-5', description: '重大费用合同真实性/金额匹配/审批检查', group: 'inspection', progress: calcSheetProgress('K9-5-', 5) },
  { seq: 9, name: '管理费用检查表', code: 'K9-8', sheetKey: '管理费用检查表K9-8', description: '综合检查逐项合规/不合规/不适用判断', group: 'inspection', progress: calcSheetProgress('K9-8-', 5) },
  // ─── 附注组 ───
  { seq: 10, name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', description: '42行13列按费用性质披露', group: 'disclosure', progress: calcSheetProgress('K9-disclosure-listed-', 5) },
  { seq: 11, name: '附注披露信息（国企）', code: '附注国企', sheetKey: '附注披露信息（国企）', description: '42行13列国企版附注披露', group: 'disclosure', progress: calcSheetProgress('K9-disclosure-soe-', 5) },
])
...
.k9-group-card :deep(.el-card__header) { display: flex; align-items: center; justify-content: space-between; padding: 10px 16px; }
