  ...
  <div class="k9-tab-adjustment">
    ...
    <div class="section-header">
      ...
      <div class="header-actions">
        ...
        <el-button size="small" @click="openReviewDialog?.('K9-3-adjustment')">💬 复核</el-button>
      </div>
    </div>

    <!-- ═══ 方法论上下文（琥珀色块） ═══ -->
    <div class="methodology-context">
      <p>调整分录（AJE/RJE）影响审定表K9-1审定数。<strong>借贷必须平衡</strong>（Σ借方 === Σ贷方）。保存后自动发布 'adjustment:created' 事件通知A13，并双向同步K9-1审定表。损益类6602：借方=增加费用，贷方=冲减费用。</p>
    </div>
    ...
    <details class="compile-hint">
      ...
      <ul>
        ...
        <li>损益类6602管理费用：借方=增加费用，贷方=冲减费用</li>
        <li>借贷必须平衡后方可保存回写</li>
...
/**
 * K9TabAdjustment.vue — K9-3 调整分录汇总
 *
...
async function handleAddEntry(): Promise<void> {
  try {
    const { value } = await ElMessageBox.prompt('请输入调整事项说明', '新增调整分录', {
      ...
      cancelButtonText: '取消',
      inputPlaceholder: '如：调增管理费用—XX咨询费',
    })
    if (value?.trim()) {
      entries.value.push({
        id: `entry-${++nextId}`,
...
function handleSaveWriteback(): void {
  ...
  // 损益类6602: AJE对科目影响 = 借方(增加费用) - 贷方(冲减费用)
  ...
  // 双向同步到allResponses供K9-1审定表读取
  emit('save', 'K9-1-aje-total', { remark: String(ajeTotal) })
  emit('save', 'K9-1-rje-total', { remark: String(rjeTotal) })
  persistEntries()
