...
// 真源在 confirmation/composables/（本文件在 confirmation/alternativeF05/，故为 '../composables/'）。
...
async function handleImportFile(event: Event) {
  ...
  if (!file) return
  try {
    // 走 useExcelIO 单一入口（B2 批，与 D05 同构）。
...
async function handleExportExcel() {
  try {
    ...
    await exportMultiSheetData({
      ...
      fileName: 'F0-5预付账款及销售替代程序_导入模板.xlsx',
      applyStyles: false,
...
async function handleExportData() {
  ...
  try {
    ...
    await exportMultiSheetData({
      ...
      fileName: 'F0-5预付账款及销售替代程序_数据导出.xlsx',
      applyStyles: false,
      successMessage: false,
    })
