/**
 * useAlternativeF06Data — F0-6 应付及采购替代程序数据 composable
 */
...
export function useAlternativeF06Data(props: UseAlternativeF06DataProps): UseAlternativeF06DataReturn {
  ...
  function initFromHtmlData(data: any) {
    if (!data || data._format !== 'alternative-f06-v1') {
      companies.value = []
      return
    }
  ...
  function importCompanies(items: Partial<AlternativeCompany>[]) {
    ...
    deduped.forEach((item, i) => {
      companies.value.push({
        ...
        balance: { item_name: '应付账款', ...(item.balance || {}) },
        block1_rows: [],
        block2_rows: [],
        block3_rows: [],
        block4_rows: [],
  ...
  function getBlockTotal(company: AlternativeCompany, blockType: BlockType): Record<string, number> {
    const rows = getBlockRows(company, blockType)
    const fields = getSumFieldsF06(blockType)
  ...
  function buildPayload(): AlternativeF06Payload {
    return {
