/**
 * blockColumnConfigsF06.ts — F0-6 应付及采购替代程序四区块列配置
 * 列结构对齐 requirements.md Requirement 3.5（openpyxl F0-6 66×28）
 */
...
const BLOCK1_COLUMNS: BlockColumnDef[] = [
  ...
  { field: 'contract_date', label: '合同日期', width: 90, type: 'date', group: '合同' },
  ...
  { field: 'invoice_amount', label: '金额', width: 110, type: 'number', sumField: true, group: '采购发票', align: 'right' },
...
const BLOCK2_COLUMNS: BlockColumnDef[] = [
  ...
  { field: 'business_desc', label: '业务内容', width: 140, type: 'text', group: '记账凭证' },
  { field: 'counter_account', label: '对方科目', width: 100, type: 'text', group: '记账凭证' },
  { field: 'payment_amount', label: '金额', width: 110, type: 'number', sumField: true, group: '记账凭证', align: 'right' },
...
const BLOCK3_COLUMNS: BlockColumnDef[] = [
  ...
  { field: 'contract_vendor', label: '供应商', width: 120, type: 'text', group: '合同' },
  { field: 'contract_amount', label: '金额', width: 110, type: 'number', sumField: true, group: '合同', align: 'right' },
  { field: 'invoice_date_no', label: '日期/编号', width: 110, type: 'text', group: '采购发票' },
  { field: 'invoice_counterparty', label: '对手方', width: 120, type: 'text', group: '采购发票' },
  { field: 'invoice_amount', label: '金额', width: 110, type: 'number', sumField: true, group: '采购发票', align: 'right' },
  { field: 'ref_index', label: '索引号', width: 90, type: 'text' },
...
export const BLOCK_COLUMN_CONFIGS_F06: Record<string, BlockConfig> = {
  block1: {
    blockType: 'block1',
    title: '1、应付余额支持性证据检查',
    columns: BLOCK1_COLUMNS,
    tips: ['②检查入库单、合同、采购发票等期末余额支持性证据'],
  },
  block2: {
    blockType: 'block2',
    title: '2、期后付款检查',
    columns: BLOCK2_COLUMNS,
    tips: ['①检查期后付款与银行回单、发票核对'],
  },
  block3: {
    blockType: 'block3',
    title: '3、本期采购入库证据',
...
export function getSumFieldsF06(blockType: string): string[] {
  const config = BLOCK_COLUMN_CONFIGS_F06[blockType]
  if (!config) return []
  return config.columns.filter((c) => c.sumField).map((c) => c.field)
}

export function getGroupsF06(blockType: string): string[] {
  const config = BLOCK_COLUMN_CONFIGS_F06[blockType]
  if (!config) return []
