<template>
  <div class="gt-confirmation-alternative-f06">
    <!-- 旧格式降级 -->
    <template v-if="!isNewFormat">
      <div class="gt-confirmation-alternative-f06__legacy-notice">
    ...
    <template v-else>
      ...
      <template v-if="selectedCompany">
        <div class="gt-confirmation-alternative-f06__detail">
...
const isNewFormat = computed(() => props.htmlData?._format === 'alternative-f06-v1')
...
function handleAddCompany() {
  ...
  nextTick(() => {
    const el = document.querySelector('.gt-confirmation-alternative-f06__detail')
...
async function handleImportFile(event: Event) {
  ...
  if (!file) return
  try {
    // 走 useExcelIO 单一入口（B2 批，与 D05/D06/F05 同构）。
    ...
    await exportMultiSheetData({
      sheets: [
        ...
        ...blockSheets.map(({ key, name }) => {
          const cols = BLOCK_COLUMN_CONFIGS_F06[key].columns
          return {
            sheetName: name,
      ...
      fileName: 'F0-6应付账款及销售替代程序_导入模板.xlsx',
      applyStyles: false,
...
async function handleExportData() {
  ...
  try {
    ...
    await exportMultiSheetData({
      ...
      fileName: 'F0-6应付账款及销售替代程序_数据导出.xlsx',
      applyStyles: false,
      successMessage: false,
    })
...
<style scoped>
.gt-confirmation-alternative-f06 {
  padding: 8px 0;
}
...
.gt-confirmation-alternative-f06__header-tip {
  margin-bottom: 12px;
}

.gt-confirmation-alternative-f06__detail {
  margin-top: 16px;
