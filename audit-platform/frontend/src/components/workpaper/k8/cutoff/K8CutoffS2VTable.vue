<template>
  <el-table :data="rows" border size="small" class="cutoff-s2v-table" :row-class-name="rowClass">
...
/**
 * K8CutoffS2VTable — K8-7 截止测试单段表格（支出凭单 → 记账凭证 双组列）
 * 供 K8TabCutoffS2V 的「资产负债表日前 / 日后」两段复用，避免列定义重复。
 */
...
<style scoped>
.cutoff-s2v-table { font-size: var(--wp-font-size, 13px); margin-bottom: 4px; }
.cutoff-s2v-table :deep(.cross-period-row td) { background-color: #fef2f2 !important; }
.cutoff-s2v-table :deep(.mismatch-row td) { background-color: #fff7ed !important; }
