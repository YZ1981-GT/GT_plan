  ...
  <div class="k8-cutoff-v2s">
    ...
    <el-alert type="info" :closable="false" style="margin-bottom:12px">
      ...
      <ol style="margin:4px 0 0;padding-left:18px;line-height:1.55;font-size:12px">
        <li><b>截止（发生方向）：</b>由记账凭证追查至原始凭证，验证期末前后记录的销售费用是否真实发生、是否记录于正确期间（防止多记/提前记）；</li>
    ...
    <div class="methodology-context">
      <p>正向截止测试：从<strong>记账凭证</strong>出发，核对原始凭证日期（期末±5天）。验证已入账的销售费用是否有真实原始凭证支持且记录在正确的会计期间。<br/>跨期判定标准：原始凭证日期与记账日期分属不同会计期间。跨期行红色高亮。</p>
    </div>
...
/**
 * K8TabCutoffV2S.vue — K8-6 截止性测试(记账凭证至原始凭证)，44行
 *
...
function handleCutoffFilled(payload: { samples: ExtractedVoucher[]; fillMode: FillMode }): void {
  ...
  for (const v of payload.samples) {
    ...
    if (lastRow) {
      ...
      updateCell(lastRow.rowKey, 'bookDate', v.voucherDate)
...
function handleVoucherFilled(payload: any): void {
  ...
  for (const v of vouchers) {
    ...
    if (lastRow) {
      ...
      updateCell(lastRow.rowKey, 'bookDate', v.voucherDate || v.voucher_date || '')
      updateCell(lastRow.rowKey, 'summary', v.summary || '')
      const amt = v.debitAmount ? parseFloat(v.debitAmount) : (v.creditAmount ? parseFloat(v.creditAmount) : 0)
      updateCell(lastRow.rowKey, 'amount', amt)
    }
