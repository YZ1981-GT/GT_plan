<template>
  <div class="k8-tab-detail">
    ...
    <div class="methodology-context">
      <p>销售费用明细表从<strong>tb_ledger明细科目</strong>取发生额（科目6601）。25列拆为3区段Tab提升可操作性。合计行联动审定表K8-1。同比变动率&gt;±30%红色高亮需说明波动原因。</p>
    </div>
    ...
    >
      ...
      <template v-if="activeTab === 'basic'">
        ...
        <el-table-column label="RJE" width="100" align="right">
          <template #default="{ row }">
            <el-input-number v-if="row.isEditable" :model-value="row.rje" :disabled="isReadonly" size="small" :controls="false" :precision="2" style="width: 80px" @change="(v: number) => handleCellChange(row.rowKey, 'rje', v ?? 0)" />
      ...
      <template v-if="activeTab === 'analysis'">
        ...
        <el-table-column label="上期发生" width="120" align="right">
          <template #default="{ row }"><span class="formula-cell">{{ fmtNum(row.priorAmount) }}</span></template>
        </el-table-column>
      ...
      <template v-if="activeTab === 'inspection'">
        ...
        <el-table-column label="凭证抽查" min-width="160">
          <template #default="{ row }">
            <el-input v-if="row.isEditable" :model-value="row.voucherCheckResult" :disabled="isReadonly" size="small" placeholder="凭证抽查结论" @blur="(e: FocusEvent) => handleCellChange(row.rowKey, 'voucherCheckResult', (e.target as HTMLInputElement)?.value ?? '')" />
          </template>
        </el-table-column>
        <el-table-column label="核查结论" min-width="160">
          <template #default="{ row }">
            <el-input v-if="row.isEditable" :model-value="row.inspectionConclusion" :disabled="isReadonly" size="small" placeholder="核查结论" @blur="(e: FocusEvent) => handleCellChange(row.rowKey, 'inspectionConclusion', (e.target as HTMLInputElement)?.value ?? '')" />
          </template>
        </el-table-column>
        <el-table-column label="备注" min-width="140">
          <template #default="{ row }">
            <el-input v-if="row.isEditable" :model-value="row.remark" :disabled="isReadonly" size="small" placeholder="备注" @blur="(e: FocusEvent) => handleCellChange(row.rowKey, 'remark', (e.target as HTMLInputElement)?.value ?? '')" />
          </template>
        </el-table-column>
      </template>
    </el-table>
...
const { rows, subtotal, updateCell, addRow, removeRow, applyI1AmortAmount } = useK8Detail({
...
function handleAiGenerate(): void { emit('save', 'K8-2-ai-trigger', { remark: 'generate' }) }
