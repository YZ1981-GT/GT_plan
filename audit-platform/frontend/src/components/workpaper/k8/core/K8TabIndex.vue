  ...
  <div class="k8-tab-index">
    ...
    <div class="k8-dir">
      <div class="index-header">
        <h3 class="title">K8 底稿目录</h3>
        <GtReviewTrigger section-id="K8-index-directory" />
      ...
      <K8PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="k8-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>销售费用为<strong>损益类借方科目</strong>（6601）：取<strong>发生额</strong>非期末余额；费用类发生额 = 借方发生累计 − 贷方发生（红冲）</li>
          <li>审定数 = 未审数 + 账项调整(AJE) + 重分类(RJE)</li>
          <li>实质性分析：同比变动率 =(本期−上期)/上期；占收入比 = 费用/营业收入</li>
          <li>双向截止：K8-6 记账凭证→原始凭证（存在认定）；K8-7 原始凭证→记账凭证（完整性认定）</li>
          <li>K8-2 明细合计应与 K8-1 审定表一致；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="k8-cycle">
...
/**
 * K8TabIndex.vue — K8 销售费用底稿目录（严格镜像 E1 标准）
 *
 * 结构：目录卡（标题+复核+编制/使用手册+进度→跨表结论口径→编制提示）
 *       + 底稿架构 4 阶段泳道（GtBArchitectureTree）+ 本循环底稿目录 grid。
 * 损益类科目（6601 销售费用）— 取发生额非余额。编制信息由页面级头部渲染，此处不重复。
 */
...
const K8PreparationHandbookDialog = defineAsyncComponent(() => import('../K8PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { seq: 1, name: '销售费用实质性程序表', code: 'K8A', sheetKey: '实质性程序表K8A', group: 'core', progress: calcSheetProgress('K8A-', 5) },
  { seq: 2, name: '审定表', code: 'K8-1', sheetKey: '审定表K8-1', group: 'core', progress: calcSheetProgress('K8-1-', 10) },
  { seq: 3, name: '明细表', code: 'K8-2', sheetKey: '明细表K8-2', group: 'core', progress: calcSheetProgress('K8-2-', 8) },
  { seq: 4, name: '调整分录汇总', code: 'K8-3', sheetKey: '调整分录汇总K8-3', group: 'core', progress: calcSheetProgress('K8-3-', 4) },
  { seq: 5, name: '实质性分析', code: 'K8-4', sheetKey: '实质性分析K8-4', group: 'analysis', progress: calcSheetProgress('K8-4-', 8) },
  { seq: 6, name: '合同检查表', code: 'K8-5', sheetKey: '合同检查表K8-5', group: 'inspection', progress: calcSheetProgress('K8-5-', 5) },
  { seq: 7, name: '截止性测试（记账→原始）', code: 'K8-6', sheetKey: '截止性测试(从记账凭证至原始凭证）K8-6', group: 'analysis', progress: calcSheetProgress('K8-6-', 6) },
  { seq: 8, name: '截止性测试（原始→记账）', code: 'K8-7', sheetKey: '截止性测试（从原始凭证至记账凭证）K8-7', group: 'analysis', progress: calcSheetProgress('K8-7-', 6) },
  { seq: 9, name: '销售费用检查表', code: 'K8-8', sheetKey: '销售费用检查表K8-8', group: 'inspection', progress: calcSheetProgress('K8-8-', 5) },
  { seq: 10, name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', group: 'disclosure', progress: calcSheetProgress('K8-disclosure-listed-', 5) },
  { seq: 11, name: '附注披露信息（国企）', code: '附注国企', sheetKey: '附注披露信息（国企）', group: 'disclosure', progress: calcSheetProgress('K8-disclosure-soe-', 5) },
])
...
// ─── 跨表结论口径看板 ─────────────────────────────────────────────────────────
/** 有审计结论/说明的 sheet（K8-3 调整分录、附注、目录本身、K8A 程序表不计入） */
const K8_CONCLUSION_CODES = new Set(['K8-1', 'K8-2', 'K8-4', 'K8-5', 'K8-6', 'K8-7', 'K8-8'])
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => K8_CONCLUSION_CODES.has(s.code))
