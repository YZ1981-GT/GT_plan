  ...
  <div class="k11-tab-index">
    ...
    <div class="k11-dir">
      <div class="index-header">
        <h3 class="title">K11 底稿目录</h3>
        <GtReviewTrigger section-id="K11-index-directory" />
      ...
      <K11PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="k11-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>资产减值损失为<strong>损益类借方科目</strong>（6701）：取本期<strong>发生额</strong>（借方 = 减值增加），非期末余额</li>
          <li>各类资产减值损失需与源底稿（F2 存货 / H1 固定资产 / I1 无形资产 / I3 商誉）交叉核对</li>
          <li>商誉减值损失一经确认不得转回（CAS8）</li>
          <li>审定数 = 未审数 + AJE + RJE；审定发生额回写试算平衡表 6701；调整分录联动 A13</li>
          <li>K11-2 明细合计应与 K11-1 审定表一致；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="k11-cycle">
...
const K11PreparationHandbookDialog = defineAsyncComponent(() => import('../K11PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { seq: 1, name: '实质性程序表', code: 'K11A', sheetKey: '实质性程序表 K11A', group: 'core', progress: calcSheetProgress('K11-K11A-', 5) },
  { seq: 2, name: '审定表', code: 'K11-1', sheetKey: '审定表K11-1', group: 'core', progress: calcSheetProgress('K11-1-', 10) },
  { seq: 3, name: '明细表', code: 'K11-2', sheetKey: '明细表K11-2', group: 'core', progress: calcSheetProgress('K11-2-', 8) },
  { seq: 4, name: '调整分录汇总', code: 'K11-3', sheetKey: '调整分录汇总K11-3', group: 'core', progress: calcSheetProgress('K11-3-', 4) },
  { seq: 5, name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', group: 'disclosure', progress: calcSheetProgress('K11-disclosure-listed-', 5) },
  { seq: 6, name: '附注披露信息（国企）', code: '附注国企', sheetKey: '附注披露信息（国企）', group: 'disclosure', progress: calcSheetProgress('K11-disclosure-soe-', 5) },
])
...
// ─── 跨表结论口径看板 ─────────────────────────────────────────────────────────
/** 有审计结论/说明的 sheet（K11A 程序表、K11-3 调整分录、附注、目录本身不计入） */
const K11_CONCLUSION_CODES = new Set(['K11-1', 'K11-2'])
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => K11_CONCLUSION_CODES.has(s.code))
