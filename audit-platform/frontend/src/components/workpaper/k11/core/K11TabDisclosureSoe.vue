...
function handleNarrativeSave(): void {
  ...
  eventBus.emit('disclosure:note-text-updated' as any, {
    ...
    sectionIds: [K11_NOTE_SECTION.soe],
...
async function syncToDisclosureNotes(): Promise<void> {
  ...
  try {
    ...
    eventBus.emit('disclosure:note-text-updated' as any, {
      ...
      projectId: props.projectId, sectionIds: [K11_NOTE_SECTION.soe],
    })
...
async function handleAiSection(section: string): Promise<void> {
  const text = await aiGenerate({
    section,
    prompt: `为K11资产减值损失附注（国企版）的"${section}"部分生成披露说明，按资产类别汇总本期减值损失，商誉减值不可转回。口径以致同 2025 修订版底稿源模板与附注模版为准；只能使用已提供的项目名称与金额，不得虚构项目、金额或业务背景，无把握的内容留空由审计师补充。`,
