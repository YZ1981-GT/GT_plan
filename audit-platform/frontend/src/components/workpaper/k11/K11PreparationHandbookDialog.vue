...
/**
 * K11 编制/使用手册弹窗 — 对齐 E1PreparationHandbookDialog，渲染 handbooks/*.md
 */
...
<template>
  <el-dialog
    ...
    title="K11 资产减值损失 · 编制手册"
    width="860px"
    top="4vh"
    destroy-on-close
    append-to-body
    class="k11-handbook-dialog"
  ...
  >
    <el-alert
      ...
      show-icon
      class="mb12"
      title="一句话：K11-2 明细按资产类别登记减值损失并与源底稿（F2/H1/I1/I3 等）核对（损益类取发生额非余额）→ 差异进 K11-3 → 确认回写 K11-1 → 勾稽 TB(6701) → 附注。商誉减值不得转回（CAS8）。本弹窗不改各表页内说明。"
    />
