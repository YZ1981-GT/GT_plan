<template>
  <div class="g8-dir" data-testid="g8-directory">
    ...
    <details class="methodology-hint">
      ...
      <p>推荐工作流：G8A 程序表 → G8-1 审定 ↔ G8-2 明细 → G8-3 调整 → G8-4/G8-5 估值 → G8-6 凭证 → 附注披露。</p>
    </details>
  </div>
</template>
...
const completedCount = computed(() =>
  applicableRows.value.filter(r => isG8SheetComplete(r.code, props.allResponses)).length,
)
