...
/**
 * GtA12Bundle — A12 期初余额审计聚合组件
 *
 * 将 A12 程序表 + A12-1 前任注册会计师 聚合为 Bundle，
...
const TABS = [
  ...
  { id: 'A12-1', label: 'A12-1 前任注册会计师', wpCode: 'A12-1' },
]
...
const wpIdMap = computed<Record<string, string>>(() => {
  ...
  for (const item of wpIndex.value) {
    if (item.wp_code?.startsWith('A12-')) {
...
<template>
  <div class="gt-a12-bundle" v-loading="loading">
