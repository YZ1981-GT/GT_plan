<template>
  <div class="g2-adjustment" data-testid="g2-adjustment">
    <details class="guidance-details">
      <summary>📋 编制提示</summary>
      <div class="guidance-content">
        <p>1. 本表对齐 Excel「应收利息调整分录汇总表 G2-4」：仅列示与应收利息（1132）相关的审计调整。</p>
        <p>2. 「账项调整」影响科目余额（AJE）；「报表调整」为重分类（RJE），仅影响列报；「其他」按账项调整处理。</p>
        <p>3. 借贷合计必须平衡；确认后发布至调整分录模块，并回写 G2-1 审定表期末账项调整。</p>
        <p>4. 可从「调整分录」模块拉取涉及 1132 等相关科目的分录；亦可推送选中行至 A13 错报汇总。</p>
    ...
    <el-alert
      ...
      title="审计目标：核实应收利息相关账项/报表调整的完整与借贷平衡，确认与调整分录模块、G2-1 审定表勾稽一致。"
    />
    ...
    <div class="tab-toolbar">
      ...
      <div class="toolbar-right">
        ...
        <el-button size="small" @click="openReviewDialog('G2-4-conclusion')">💬复核</el-button>
      </div>
    </div>
    ...
    <div class="balance-indicator">
      ...
      <el-tag v-if="netAjeToG2 !== 0" type="info" size="small">
        1132 净调整 {{ fmt(netAjeToG2) }}
      </el-tag>
    </div>
    ...
    <G2AuditTextCards
      ...
      note-placeholder="填写审计说明：调整依据、事项性质、对 1132 及损益的影响、与调整分录模块勾稽情况。"
...
const { centralStatus, syncing: centralSyncing, syncToCentral, refreshStatus } = useAdjustmentCentralSync({
  ...
  buildMeta: () => ({
    description: rows.value.find((r) => r.description)?.description || 'G2 应收利息调整',
    adjustmentType: rows.value.length > 0 && rows.value.every((r) => r.category === '报表调整') ? 'rje' : 'aje',
  }),
})
onMounted(() => refreshStatus())

const selectedRows = ref<G2AdjustmentRow[]>([])
function onSelectionChange(sel: G2AdjustmentRow[]) {
  selectedRows.value = sel
}

const NOTE_KEY = 'G2-4-audit-note'
const CONCLUSION_KEY = 'G2-4-conclusion'
const auditNote = ref(props.allResponses.get(NOTE_KEY)?.remark ?? '')
const conclusion = ref(props.allResponses.get(CONCLUSION_KEY)?.remark ?? props.allResponses.get(CONCLUSION_KEY)?.conclusion ?? '')
...
function onConfirm() {
  ...
  publishAdjustment()
  ElMessage.success('已确认调整并联动调整分录模块 / G2-1')
}
</script>

<style scoped>
.g2-adjustment { padding: 12px; font-size: var(--wp-font-size, 13px); }
