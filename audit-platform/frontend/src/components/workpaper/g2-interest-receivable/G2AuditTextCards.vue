...
/**
 * G2 审计说明 / 结论卡片（含 AI）— 各子表统一复用，对齐 F2/G1
 */
import { computed } from 'vue'
import { useG2AiGenerate, type G2AiSection } from '../composables/useG2AiGenerate'
...
const props = withDefaults(defineProps<{
  ...
  noteAiSection?: G2AiSection | ''
  conclusionAiSection?: G2AiSection | ''
...
const { aiAvailable, loading: aiLoading, generateAndConfirm } = useG2AiGenerate(wpIdRef)
...
<style scoped>
.g2-audit-text-cards { display: flex; flex-direction: column; gap: 0; }
.audit-text-card {
  margin-top: 14px;
