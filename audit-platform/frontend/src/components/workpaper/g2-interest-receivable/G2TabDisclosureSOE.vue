...
const STORAGE_KEY = 'G2-disclosure-soe-text'
const noteText = ref('')

// ─── 审计说明 / 审计结论（持久化）───────────────────────────────────────────
const NOTE_KEY = 'G2-disclosure-soe-audit-note'
const CONCLUSION_KEY = 'G2-disclosure-soe-audit-conclusion'
const auditNote = ref('')
...
watch(noteText, (val) => {
  ...
  try {
    window.dispatchEvent(new CustomEvent('disclosure:note-text-updated', {
      detail: { accountCode: '1132', section: 'soe', text: val },
    }))
  } catch { /* silent */ }
})
...
function handleAdjudicated(e: Event): void {
  ...
  if (d?.accountCode === '1132') { /* auto-refresh placeholder */ }
}
...
function fillAiDraft() {
  if (props.isReadonly) return
  const draft = '应收利息期末余额 [审定金额] 元，主要为债权投资利息及定期存款利息。'
