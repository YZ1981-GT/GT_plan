  ...
  <div class="k2-tab-index">
    ...
    <div class="k2-dir">
      <div class="index-header">
        <h3 class="title">K2 底稿目录</h3>
        <GtReviewTrigger section-id="K2-index-directory" />
      ...
      <K2PreparationHandbookDialog v-model="handbookVisible" :initial-tab="handbookTab" />

      <div class="conclusion-board" data-testid="k2-conclusion-board">
      ...
      <details class="methodology-hint">
        ...
        <ul>
          <li>其他流动资产为<strong>资产类借方科目</strong>（报表行 BS-014，实证科目 1901）：期末 = 期初 + 借方 − 贷方</li>
          <li>合同取得成本按 CAS14 判断资本化：增量成本 + 预期可收回 + 与合同直接相关</li>
          <li>直线法摊销 = 取得成本 ÷ 摊销期总期数 × 本期期数；摊余成本 = 取得成本 − 累计摊销</li>
          <li>K2-4 合同成本合计应与 K2-1 审定表一致；各表填妥"审计说明或结论"后目录标签转为"已填"</li>
        </ul>
      </details>
    </div>
    ...
    <div v-if="cycleWorkpapers.length" class="k2-cycle">
...
/**
 * K2TabIndex.vue — K2 其他流动资产底稿目录（严格镜像 E1 标准）
 *
...
const K2PreparationHandbookDialog = defineAsyncComponent(() => import('../K2PreparationHandbookDialog.vue'))
...
const allSheets = computed<SheetRow[]>(() => [
  { seq: 1, name: '其他流动资产实质性程序表', code: 'K2A', sheetKey: '其他流动资产实质性程序表K2A', group: 'core', progress: calcSheetProgress('K2-K2A-', 5) },
  { seq: 2, name: '审定表', code: 'K2-1', sheetKey: '审定表K2-1', group: 'core', progress: calcSheetProgress('K2-K2-1-', 10) },
  { seq: 3, name: '明细表', code: 'K2-2', sheetKey: '明细表K2-2', group: 'core', progress: calcSheetProgress('K2-K2-2-', 8) },
  { seq: 4, name: '调整分录汇总', code: 'K2-3', sheetKey: '调整分录汇总K2-3', group: 'core', progress: calcSheetProgress('K2-K2-3-', 4) },
  { seq: 5, name: '合同取得成本明细表', code: 'K2-4', sheetKey: '合同取得成本明细表K2-4', group: 'amortization', progress: calcSheetProgress('K2-K2-4-', 8) },
  { seq: 6, name: '摊销测算表', code: 'K2-5', sheetKey: '摊销测算表K2-5', group: 'amortization', progress: calcSheetProgress('K2-K2-5-', 8) },
  { seq: 7, name: '其他流动资产检查表', code: 'K2-6', sheetKey: '其他流动资产检查表K2-6', group: 'inspection', progress: calcSheetProgress('K2-K2-6-', 6) },
  { seq: 8, name: '附注披露信息（上市公司）', code: '附注上市', sheetKey: '附注披露信息（上市公司）', group: 'disclosure', progress: calcSheetProgress('K2-disclosure-listed-', 5) },
  { seq: 9, name: '附注披露信息（国企）', code: '附注国企', sheetKey: '附注披露信息（国企）', group: 'disclosure', progress: calcSheetProgress('K2-disclosure-soe-', 5) },
])
...
// ─── 跨表结论口径看板 ─────────────────────────────────────────────────────────
/** 有审计结论/说明的 sheet（K2-3 调整分录、附注、目录本身不计入） */
const K2_CONCLUSION_CODES = new Set(['K2-1', 'K2-2', 'K2-4', 'K2-5', 'K2-6'])

/** includes(`${code}-`) 兼容不同存储前缀深度（K2=`K2-K2-1-*`），尾部连字符保证边界安全。 */
...
const conclusionSheets = computed(() =>
  allSheets.value
    .filter(s => K2_CONCLUSION_CODES.has(s.code))
