<template>
  ...
  <div class="g13-procedure">

    <div class="g13-proc-toolbar">

      <GtVoucherSamplingEngine

        :project-id="projectId"

        :account-codes="[G13_ACCOUNT_CODE]"

        dialog-mode

      />

      <GtReviewTrigger section-id="G13A-procedure" />
...
async function selfLoad() {
  ...
  try {
    ...
    const res = await api.get(`/api/workpapers/${props.wpId}/render-config`, {
      ...
      params: { force_component_type: 'a-program-console', sheet_name: '公允价值变动收益审计程序表G13A' },
    ...
    const hit = sheets.find((s: any) => /G13A/i.test(s.sheet_name || s.name || ''))

    programData.value = hit?.html_data ?? sheets[0]?.html_data ?? renderData

  } catch (err) {

    console.warn('[G13TabProcedure] selfLoad failed:', err)
...
.g13-procedure { padding: 12px; font-size: 13px; }

.g13-proc-toolbar { display: flex; gap: 8px; margin-bottom: 12px; align-items: center; flex-wrap: wrap; }
