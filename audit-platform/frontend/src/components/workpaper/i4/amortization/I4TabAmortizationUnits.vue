<template>
  <div class="i4-tab-amortization-units">
    <!-- 审计目标 -->
    <el-alert type="info" :closable="false" title="审计目标：验证长期待摊费用工作量法摊销计算的准确性，确认月摊销额=原始金额×(当月工作量÷总预计工作量)，实际工作量数据可靠。" class="objective-alert" />
    ...
    <el-card shadow="never" class="audit-note-card">
      ...
      <el-input
        ...
        :disabled="isReadonly"
        :autosize="{ minRows: 5 }"
        placeholder="请填写审计说明（工作量法摊销测算复核情况、实际产量数据来源等）..."
