...
/** L3 渲染模式 */
export type L3RenderMode = 'structured' | 'matrix' | 'onlyoffice'
...
/** localStorage key 前缀 */
const STORAGE_KEY_PREFIX = 'l3-dual-mode-'
 ...
 */
export function useL3DualMode(wpId: Ref<string>) {
  // ─── State ─────────────────────────────────────────────────────────────

  const currentMode = ref<L3RenderMode>('structured')
  ...
  function _persistMode(mode: L3RenderMode): void {
    try {
   ...
   */
  function switchMode(mode: L3RenderMode): void {
